...
// FileDesk pnpm monorepo flat config. Adopts the shared @heejun/eslint-config
...
//
// 계층 경계(boundaries)는 적용하지 않는다 — apps/web 은 app/components/pages/
// services/utils 의 평면 구조이고 domains/infrastructure 4계층이 없어서 강제할
// 경계가 없다(없는 계층을 발명하지 않는다).
export default defineConfig(
  ...
  // FileDesk 자체 테스트 스타일과 충돌한다(shared base 의 일반 규칙만 채택).
  {
