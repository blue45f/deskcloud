...
import { KEY_RANDOM_BYTES, PUBLISHABLE_KEY_PREFIX, SECRET_KEY_PREFIX } from './constants'

/**
 * API 키 생성·해시·검증 — 순수 유틸(프레임워크 무관). TenantService 가 래핑한다.
 *
 * - publishable 키(`pk_…`): 공개 안전(프론트 임베드). 평문 그대로 저장·노출.
 * - secret 키(`sk_…`): 서버 전용. SHA-256(키 + pepper) **해시만** 저장. 평문은 발급/회전 시 1회만.
 *
 * 주: 이 키들은 **테넌트(앱)** 식별용이다. 테넌트 풀의 end-user 인증(scrypt + JWT)과 혼동하지 말 것.
 */
 ...
 * pepper 는 서버 비밀(AUTHDESK_KEY_PEPPER) — DB 유출 시 키 역산을 어렵게 한다.
 */
export function hashSecretKey(secretKey: string, pepper = ''): string {
  return createHash('sha256')
    .update(secretKey + pepper)
    .digest('hex')
}
 ...
 * (SecretKeyGuard 와 임의 호출자가 공유.)
 */
export function extractBearerSecretKey(authHeader: string | undefined | null): string | null {
  const token = extractBearerToken(authHeader)
  return token && isSecretKey(token) ? token : null
}

/** `Authorization: Bearer <token>` 에서 임의 토큰(end-user JWT 등)을 추출한다. */
