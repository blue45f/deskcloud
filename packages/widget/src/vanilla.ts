/**
 * @filedesk/widget/vanilla — 프레임워크 무관 로더.
 *
 * 비-React 사이트가 <script> 한 줄로 업로더를 띄울 수 있게 한다:
 *
 *   <div id="filedesk-upload"></div>
 *   <script src=".../filedesk-widget.js"></script>
 *   <script>
 *     FileDesk.init({
 *       target: '#filedesk-upload',
 *       publishableKey: 'pk_…',
 *       endpoint: 'https://files.example.com',
 *       onUploaded: function (f) { console.log(f.url) },
 *     })
 *   </script>
 *
 * 또는 명령형으로:
 *   const handle = FileDesk.mount({ publishableKey, endpoint })
 *   handle.unmount()
 *
...
import { FileUpload, type FileUploadProps } from './react'

export interface MountOptions extends FileUploadProps {
...
function resolveContainer(target: MountOptions['target']): HTMLElement {
  ...
  host.setAttribute('data-filedesk-widget', '')
...
export function mount(options: MountOptions): WidgetHandle {
  ...
  root.render(createElement(FileUpload, widgetProps))

  return {
    container,
    unmount: () => {
      root.unmount()
      if (container.hasAttribute('data-filedesk-widget') && container.parentNode) {
 ...
 * window.FileDesk.init({ publishableKey, endpoint }) 형태로 호출된다.
 */
export function init(options: MountOptions): WidgetHandle {
  return mount(options)
}

/** IIFE 빌드가 window.FileDesk 로 노출하는 네임스페이스 형태. */
export const FileDesk = { mount, init }
