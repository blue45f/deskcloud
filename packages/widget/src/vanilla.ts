/**
 * @addesk/widget/vanilla — 프레임워크 무관 로더.
 *
 * 비-React 사이트가 <script> 한 줄로 배너 슬롯을 띄울 수 있게 한다:
 *
 *   <div id="addesk-slot"></div>
 *   <script src=".../addesk-widget.js"></script>
 *   <script>
 *     AdDesk.init({
 *       target: '#addesk-slot',
 *       slot: 'sidebar',
 ...
 *
 * 또는 명령형으로:
 *   const handle = AdDesk.mount({ target, slot, publishableKey, endpoint })
 *   handle.unmount()
 *
...
import { AdSlot, type AdSlotProps } from './react'

export interface MountOptions extends AdSlotProps {
...
function resolveContainer(target: MountOptions['target']): HTMLElement {
  ...
  host.setAttribute('data-addesk-widget', '')
...
export function mount(options: MountOptions): WidgetHandle {
  ...
  root.render(createElement(AdSlot, widgetProps))

  return {
    container,
    unmount: () => {
      root.unmount()
      if (container.hasAttribute('data-addesk-widget') && container.parentNode) {
 ...
 * window.AdDesk.init({ slot, publishableKey, endpoint }) 형태로 호출된다.
 */
export function init(options: MountOptions): WidgetHandle {
  return mount(options)
}

/** IIFE 빌드가 window.AdDesk 로 노출하는 네임스페이스 형태. */
export const AdDesk = { mount, init }
