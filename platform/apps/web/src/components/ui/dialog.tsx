...
>(function DialogOverlay({ className, ...props }, ref) {
  return (
    <DialogPrimitive.Overlay
      ref={ref}
      className={cn(
        'fixed inset-0 z-50 bg-[oklch(0.2_0.01_270/0.45)] backdrop-blur-[1px] data-[state=open]:animate-[fade-in_150ms_ease-out]',
        className
      )}
      {...props}
    />
  ...
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          'fixed top-1/2 left-1/2 z-50 max-h-[85vh] w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl border border-border bg-surface p-6 shadow-lg outline-none data-[state=open]:animate-[pop-in_160ms_cubic-bezier(0.22,1,0.36,1)]',
          className
        )}
        {...props}
      >
