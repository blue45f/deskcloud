...
describe('VisitsService (PGlite, Drizzle store)', () => {
  ...
  it('빈 백엔드 — 시드 없이 0(가짜 데모 숫자 금지)', async () => {
    const stats = await service.stats('aidigestdesk')
    expect(stats.appId).toBe('aidigestdesk')
  ...
  it('ping — 방문 +1, newVisitor 면 고유 방문자도 +1', async () => {
    await service.ping('aidigestdesk', { newVisitor: true })
    await service.ping('aidigestdesk', {})
    await service.ping('aidigestdesk', { newVisitor: true })
  ...
  it('빈 본문(undefined) 핑 — 방문만 +1, 고유는 그대로', async () => {
    await service.ping('aidigestdesk', visitPingSchema.parse(undefined))
    const stats = await service.stats('aidigestdesk')
  ...
  it('appId 격리 — 다른 앱 집계는 서로 영향 없음', async () => {
    ...
    const a = await service.stats('aidigestdesk')
  ...
  it('전체 누적은 모든 일자 버킷 합 — 과거 일자 행도 totalVisits 에 포함', async () => {
    ...
    await db
      ...
      .values({ appId: 'aidigestdesk', day: '2020-01-01', visits: 5, uniques: 3 })
    const svc = new VisitsService(new DrizzleVisitsStore(dbs))
    await svc.ping('aidigestdesk', { newVisitor: true })

    const stats = await svc.stats('aidigestdesk')
