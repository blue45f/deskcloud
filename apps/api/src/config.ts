...
export interface AppConfig {
  ...
  adminToken: string
  /** secret 키 해시에 섞는 서버 페퍼(키 노출 시에도 DB 해시 역산을 어렵게). */
  keyPepper: string
  /** 무료 플랜 누적 서빙(ad_serves) 소프트 한도(초과 시 서빙 402). */
...
export function loadConfig(): AppConfig {
  const mode: DeploymentMode = process.env.ADDESK_MODE === 'saas' ? 'saas' : 'self-hosted'
  ...
  return {
    ...
    keyPepper: process.env.KEY_PEPPER?.trim() || 'dev-pepper-change-me',
    freePlanLimit:
      Number.isFinite(freeLimitRaw) && freeLimitRaw > 0 ? freeLimitRaw : FREE_PLAN_LIMIT,
  }
}

/** self-hosted 는 항상 시드, saas 는 ADDESK_SEED=true 일 때만. */
export const isSeedingEnabled = (cfg: AppConfig): boolean =>
  cfg.mode === 'self-hosted' || envBool(process.env.ADDESK_SEED, false)
