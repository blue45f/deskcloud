...
export function validateEnv(
  ...
  // web-push 는 두 키가 모두 있어야 동작 — 한쪽만 있으면 경고.
  const hasPub = Boolean(env.VAPID_PUBLIC_KEY?.trim())
  const hasPriv = Boolean(env.VAPID_PRIVATE_KEY?.trim())
  if (hasPub !== hasPriv) {
    warn('[env] web-push 경고: VAPID_PUBLIC_KEY/VAPID_PRIVATE_KEY 는 둘 다 설정해야 동작합니다(현재 한쪽만 설정). web-push 채널은 no-op 됩니다.')
  }
