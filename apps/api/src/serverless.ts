...
async function bootstrapServerless(): Promise<ExpressHandler> {
  ...
  // main.ts 와 동일한 API 베이스라인: 보안 헤더 · 압축 · CORS(공개 위젯/임베드 대비 개방).
  ...
  // listen() 대신 init(): HTTP 서버를 띄우지 않고 Nest 라이프사이클(onModuleInit = DB 마이그레이션,
  // onApplicationBootstrap = 멱등 시드)만 실행한 뒤 Express 핸들러를 반환.
  await app.init()
