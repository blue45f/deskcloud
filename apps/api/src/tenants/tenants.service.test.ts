...
describe('TenantsService (PGlite)', () => {
  ...
  it('가입 시 pk_/sk_ 키를 발급하고 secret 은 해시로만 저장', async () => {
    ...
    expect(res.tenant.postsCount).toBe(0)
    expect(res.tenant.readsCount).toBe(0)
  ...
  it('name 에서 slug 자동 생성, 충돌 시 접미사', async () => {
    const a = await service.createTenant({ name: 'My Cafe', corsOrigins: [] })
    const b = await service.createTenant({ name: 'My Cafe!', corsOrigins: [] })
    expect(a.tenant.slug).toBe('my-cafe')
    expect(b.tenant.slug).toBe('my-cafe-2')
  })
  ...
  it('incrementPosts/incrementReads 가 누적 증가', async () => {
    const res = await service.createTenant({ name: 'X', corsOrigins: [] })
    expect(await service.incrementPosts(res.tenant.id)).toBe(1)
    expect(await service.incrementPosts(res.tenant.id)).toBe(2)
    await service.incrementReads(res.tenant.id)
    await service.incrementReads(res.tenant.id)
    expect((await service.findById(res.tenant.id))?.readsCount).toBe(2)
  })
