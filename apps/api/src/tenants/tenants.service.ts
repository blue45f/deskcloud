...
export class TenantsService {
  ...
  async signup(input: CreateTenantInput): Promise<TenantCredentialsDto> {
    ...
    const inserted = await this.dbs.db
      ...
      .values({
        ...
        publishableKey,
        secretKeyHash: hashSecret(secretKey, this.cfg.keyPepper),
        secretKeyLookup: lookupHash(secretKey, this.cfg.keyPepper),
  ...
  async findBySecretKey(key: string): Promise<TenantRow | null> {
    const rows = await this.dbs.db
      .select()
      .from(tenants)
      .where(eq(tenants.secretKeyLookup, lookupHash(key, this.cfg.keyPepper)))
      .limit(1)
    const row = rows[0]
    if (!row) return null
    return verifySecret(key, row.secretKeyHash, this.cfg.keyPepper) ? row : null
  }
