 ...
 * 가드는 moderation/rules/reports/logs 등 다른 모듈에서도 @UseGuards 로 쓰므로 전역으로 노출한다.
 */
@Global()
@Module({
  controllers: [TenantsController, AdminTenantController],
  providers: [TenantsService, PublishableKeyGuard, SecretKeyGuard, ModerateAuthGuard],
  exports: [TenantsService, PublishableKeyGuard, SecretKeyGuard, ModerateAuthGuard],
})
