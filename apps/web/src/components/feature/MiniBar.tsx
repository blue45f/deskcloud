...
/**
 * 수평 막대 분포 — 채널별 메시지·연결 분포 등에 쓰는 작은 차트(의존성 0).
...
export function MiniBar({
  ...
  total: totalOverride,
  className,
  emptyText = '집계할 데이터가 없습니다.',
}: {
  rows: MiniBarRow[]
  /** 비율 분모(기본: 행 최댓값). 전체 대비로 보고 싶으면 명시. */
  total?: number
  className?: string
  emptyText?: string
}) {
          ...
          <li key={r.label} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3">
            <div className="min-w-0">
              <div className="mb-1 flex items-center justify-between gap-2">
                <span className="truncate font-mono text-[0.8125rem] text-text">{r.label}</span>
