import { ArrowUpRight, Check, Inbox, Moon, Search, Star, Sun, Trash2 } from 'lucide-react'
...
import { Badge, FeaturedBadge, PlanBadge, StatusBadge } from '@/components/ui/badge'
 ...
 * 라이브 DOM 의 computed-style 캐스케이드는 렌더 중 파생 불가한 외부 시스템이라
 * 이펙트에서 한 번 읽어 상태로 동기화한다(react-compiler set-state-in-effect 예외).
 */
function useResolvedToken(cssVar: string, themeKey: string): string {
  const [value, setValue] = useState('')
  useEffect(() => {
    const raw = getComputedStyle(document.documentElement).getPropertyValue(cssVar).trim()
    // eslint-disable-next-line react-hooks/set-state-in-effect -- 외부 시스템(computed CSS) 동기화 읽기
    setValue(raw)
  }, [cssVar, themeKey])
  return value
}
...
const TYPE_SCALE = [
  {
    cls: 'text-[2rem] font-semibold tracking-tight',
    label: 'Display · 2rem / 650',
  },
  {
    cls: 'text-2xl font-semibold tracking-tight',
    label: 'Heading · 1.5rem / 650',
  },
  ...
  {
    cls: 'text-[0.8125rem] text-text-muted',
    label: 'Body small · 0.8125rem / muted',
  },
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-bg/85 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            <Star className="size-5 shrink-0 fill-current text-accent-strong" aria-hidden />
            <span className="truncate text-sm font-bold tracking-tight text-text">ReviewDesk</span>
        ...
        <main id="main-content" tabIndex={-1} className="min-w-0 pb-20 outline-none">
          ...
          <div className="border-b border-border py-14">
            ...
            <h1 className="mt-3 max-w-[18ch] text-[clamp(2rem,5vw,3rem)] font-semibold tracking-tight text-balance text-text">
              평점·리뷰 인프라를 받치는 디자인 토큰과 컴포넌트
            </h1>
            <p className="mt-4 max-w-[65ch] text-pretty text-text-muted">
              이 페이지는 ReviewDesk 의 실제 디자인 시스템을 그대로 렌더링합니다. 색상은{' '}
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_minmax(0,32ch)]">
              ...
              <div className="rounded-lg border border-border bg-surface p-5">
                ...
                <p className="max-w-[68ch] text-pretty text-text-muted">
                  ReviewDesk 는 외부 서비스가 직접 가입해 별점·리뷰·후기를 모으는 멀티테넌트 SaaS
                  입니다. publishable 키로 위젯이 리뷰를 받고 노출하고, 운영자는 secret 키로
                  검수·집계합니다. 본문은 65–75ch 범위로 줄길이를 제한해 장문도 편안하게 읽힙니다.
                </p>
              </div>
            </div>
          </Section>
          ...
          >
            ...
            {/* Badges + Stars */}
            <h3 className="mt-12 mb-3 text-base font-semibold text-text">배지 · 상태 칩 · 별점</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Demo caption="도메인 칩 — StatusBadge(3종) · FeaturedBadge · PlanBadge(3종)">
              ...
              <Demo caption="별점 — Stars(읽기 전용), sm · md · lg">
                <Stars value={5} size="sm" />
                <Stars value={4} size="md" />
                <Stars value={3} size="lg" />
              </Demo>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <StatCard label="전체 리뷰" value="248" hint="모든 상태" />
              <StatCard
                label="평균 별점"
                value="4.38"
                hint="216개 승인본 · 5점 만점"
                tone="warning"
              />
              <StatCard label="추천" value="12" hint="후기 월 노출" tone="accent" />
            </div>
            <Card className="mt-4 max-w-md">
              <CardHeader action={<StatusBadge status="approved" />}>
                <CardTitle>Pro 플랜 — 별점 분포</CardTitle>
              </CardHeader>
              <CardContent>
                <MiniBar
                  rows={[
                    { label: '★ 5', count: 124, tone: 'accent' },
                    { label: '★ 4', count: 72, tone: 'accent' },
                    { label: '★ 3', count: 14, tone: 'accent' },
                    { label: '★ 2', count: 4, tone: 'accent' },
                    { label: '★ 1', count: 2, tone: 'accent' },
                  ]}
                  total={216}
                />
              </CardContent>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <Card>
                <CardContent className="space-y-4">
                  <Field label="서비스 이름" htmlFor="d-name" hint="대시보드·후기에 표시됩니다.">
                    <Input id="d-name" placeholder="예: Acme Inc." />
                  </Field>
                  <Field
                    label="subjectId"
                    htmlFor="d-error"
                    error="소문자·숫자·하이픈만 가능합니다."
                  >
                    <Input id="d-error" defaultValue="Pro Plan!" aria-invalid />
                  </Field>
                  <Field label="요금제" htmlFor="d-type">
                    <Select id="d-type" defaultValue="pro">
              ...
              <Card>
                <CardContent className="space-y-4">
                  <Field
                    label="운영자 답글"
                    htmlFor="d-textarea"
                    hint="승인본이면 위젯에 함께 노출됩니다."
                  >
                    <Textarea id="d-textarea" placeholder="소중한 후기 감사합니다…" />
                  </Field>
                  <Field label="잠긴 필드" htmlFor="d-disabled" hint="secret 키는 회전으로만 갱신">
            ...
            <Card>
              <CardContent>
                <Tabs defaultValue="pending">
                  ...
                  <TabsContent value="pending" className="pt-4 text-sm text-text-muted">
                    활성 탭은 하단 보더가 accent 색으로 강조됩니다.
                  </TabsContent>
                  <TabsContent value="approved" className="pt-4 text-sm text-text-muted">
                    승인된 리뷰가 여기에 들어갑니다.
                  </TabsContent>
                  <TabsContent value="rejected" className="pt-4 text-sm text-text-muted">
                    거절된 리뷰가 여기에 들어갑니다.
                  </TabsContent>
                </Tabs>
              </CardContent>
            ...
            <Demo caption="모두 포털로 렌더링되어 overflow 를 벗어납니다.">
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      새 키가 발급되고 기존 키는 즉시 무효가 됩니다. 배포된 위젯의 키도 교체해야
                      합니다.
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
              ...
              <Tooltip content="이 리뷰를 추천으로 지정합니다">
                <Button variant="ghost" size="sm">
                  Tooltip (hover)
                </Button>
              </Tooltip>
            ...
            <CodeBlock
              code={`<div data-reviewdesk="stars" data-subject-id="pro-plan"></div>
<script src="https://reviews.example.com/reviewdesk-widget.js" defer></script>
<script>
  ReviewDesk.init({ publishableKey: 'pk_live_xxx', endpoint: 'https://reviews.example.com' })
</script>`}
              language="html"
              className="max-w-2xl"
            />
            ...
            <Card className="max-w-md">
              <CardHeader action={<FeaturedBadge />}>
                <CardTitle>기대 이상이에요</CardTitle>
                <CardDescription>김서연 · pro-plan · 2일 전 · ★ 5</CardDescription>
              </CardHeader>
              ...
              <CardFooter className="flex items-center justify-between">
                <span className="font-mono text-xs text-text-subtle">subject: pro-plan</span>
                <Button variant="ghost" size="sm">
                  답글 <ArrowUpRight className="size-3.5" />
                </Button>
              </CardFooter>
            </Card>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="flex flex-col gap-2">
                <EmptyState
                  icon={Inbox}
                  title="검수 대기 중인 리뷰가 없습니다"
                  description="위젯으로 리뷰가 들어오면 여기에 표시됩니다."
                  action={
          ...
          <p className="border-t border-border pt-8 text-xs text-text-subtle">
            이 페이지는 ReviewDesk 의 실제 토큰과 컴포넌트만 사용합니다. 새 색·새 컴포넌트를 만들지
            않습니다.
          </p>
        </main>
      </div>
