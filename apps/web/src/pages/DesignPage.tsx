      ...
      <header className="sticky top-0 z-30 border-b border-border bg-bg/85 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            <ImagePlay className="size-5 shrink-0 text-accent-strong" aria-hidden />
            <span className="truncate text-sm font-bold tracking-tight text-text">MediaDesk</span>
        ...
        <main id="main-content" tabIndex={-1} className="min-w-0 pb-20 outline-none">
          ...
          <div className="border-b border-border py-14">
            ...
            <h1 className="mt-3 max-w-[18ch] text-[clamp(2rem,5vw,3rem)] font-semibold tracking-tight text-balance text-text">
              미디어 인프라를 받치는 디자인 토큰과 컴포넌트
            </h1>
            <p className="mt-4 max-w-[65ch] text-pretty text-text-muted">
              이 페이지는 MediaDesk 의 실제 디자인 시스템을 그대로 렌더링합니다. 색상은{' '}
          ...
          <Section
            ...
            intro={
              <>
                본문은 Pretendard Variable 한 가족으로 헤딩·라벨·데이터까지 운용합니다. 수치·키·코드는{' '}
                <code className="rounded bg-surface-2 px-1 py-0.5 font-mono text-[0.8125rem] text-text">
                  --font-mono
                </code>
                .
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_minmax(0,32ch)]">
              <div className="space-y-4">
                {TYPE_SCALE.map((t) => (
                  ...
                  >
                    <span className={cn('text-text', t.cls)}>자산이 업로드되는 중입니다</span>
                    <span className="shrink-0 font-mono text-xs text-text-subtle">{t.label}</span>
                  </div>
                ))}
                <div className="flex items-center gap-4 pt-1">
                  <span className="font-mono text-sm text-text">mono · 1.2 MB · 1280×720 · webp</span>
              ...
              <div className="rounded-lg border border-border bg-surface p-5">
                ...
                <p className="max-w-[68ch] text-pretty text-text-muted">
                  MediaDesk 는 형제 앱의 미디어를 한 곳에서 다룹니다. 임베드 업로더가 파일을 받고,
                  온더플라이 변환이 썸네일을 만들고, 공개 URL 로 어디서든 서빙합니다. 본문은 65–75ch
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div className="rounded-lg border border-border bg-surface p-5">
                ...
                <dl className="mt-4 grid grid-cols-2 gap-y-1.5 text-xs">
                  ...
                  <dt className="text-text-subtle">slide-up</dt>
          ...
          >
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Demo caption="도메인 칩 — PlanBadge · StorageBadge">
                <PlanBadge plan="free" />
                <PlanBadge plan="pro" />
                <StorageBadge driver="local" />
                <StorageBadge driver="s3" />
              </Demo>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <StatCard label="자산 수" value="248" hint="한도 500" />
              <StatCard label="저장 용량" value="42.6 MB" hint="한도 100 MB" tone="accent" />
              <StatCard label="스토리지" value="Local FS" hint="sharp 변환 사용 가능" icon={HardDrive} />
            </div>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <Card>
                <CardContent className="space-y-4">
                  <Field label="워크스페이스 이름" htmlFor="d-name" hint="공개 slug 로 변환됩니다.">
                    <Input id="d-name" placeholder="예: Acme Co" />
                  </Field>
                  <Field
                    label="Origin"
                    htmlFor="d-error"
                    error="유효한 URL 형식이어야 합니다."
                  >
                    <Input id="d-error" defaultValue="not a url" aria-invalid />
                  </Field>
                  <Field label="포맷" htmlFor="d-type">
                    <Select id="d-type" defaultValue="webp">
              ...
              <Card>
                <CardContent className="space-y-4">
                  <Field label="설명" htmlFor="d-textarea" hint="짧게 적어 주세요.">
                    <Textarea id="d-textarea" placeholder="이 워크스페이스의 용도…" />
                  </Field>
                  <Field label="잠긴 필드" htmlFor="d-disabled" hint="slug 는 가입 후 변경 불가">
                    <Input id="d-disabled" defaultValue="acme-co" disabled />
                  </Field>
                  <div className="flex items-center gap-2">
              ...
              <CardContent>
                <Tabs defaultValue="react">
                  ...
                  <TabsContent value="vanilla" className="pt-4 text-sm text-text-muted">
                    스크립트 태그 임베드 스니펫이 여기에 들어갑니다.
                  </TabsContent>
                  <TabsContent value="sdk" className="pt-4 text-sm text-text-muted">
                    브라우저 SDK 사용법이 여기에 들어갑니다.
                  </TabsContent>
                </Tabs>
              </CardContent>
            </Card>
            ...
            <Demo caption="모두 포털로 렌더링되어 overflow 를 벗어납니다.">
              ...
              <Tooltip content="이 자산의 변환 URL 을 만듭니다">
                <Button variant="ghost" size="sm">
                  <Wand2 className="size-3.5" /> Tooltip (hover)
                </Button>
              </Tooltip>
            </Demo>
            ...
            <CodeBlock
              code={`<script src="https://media.example.com/media-widget.js"></script>
<script>
  MediaDesk.mountUploader('#u', { publishableKey: 'pk_…', endpoint: 'https://media.example.com' })
</script>`}
              language="html"
              className="max-w-2xl"
            />
            ...
            <Card className="max-w-md">
              <CardHeader action={<PlanBadge plan="pro" />}>
                <CardTitle>Acme Co</CardTitle>
                <CardDescription>acme-co · 248 자산 · 42.6 MB</CardDescription>
              </CardHeader>
              ...
              <CardFooter className="flex items-center justify-between">
                <span className="font-mono text-xs text-text-subtle">pk_…0000</span>
                <Button variant="ghost" size="sm">
                  설정 <ArrowUpRight className="size-3.5" />
                </Button>
              </CardFooter>
            </Card>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="flex flex-col gap-2">
                <EmptyState
                  icon={Inbox}
                  title="아직 자산이 없습니다"
                  description="업로더로 이미지·파일을 올리면 여기 그리드에 표시됩니다."
                  action={
          ...
          <p className="border-t border-border pt-8 text-xs text-text-subtle">
            이 페이지는 MediaDesk 의 실제 토큰과 컴포넌트만 사용합니다. 새 색·새 컴포넌트를 만들지
            않습니다.
          </p>
        </main>
      </div>
