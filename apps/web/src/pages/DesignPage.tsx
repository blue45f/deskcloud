...
import { ActivePill, Badge, QuestionTypeBadge } from '@/components/ui/badge'
...
/**
 * getComputedStyle 으로 토큰을 즉시 해석한다. 외부 시스템(DOM 계산 스타일)을 읽는
 * 것이라 useSyncExternalStore 로 렌더 중 동기 스냅샷을 만든다(effect 내 setState 없음).
 * 테마 토글(`themeKey`)이 바뀌면 스냅샷도 무효화되어 다시 읽힌다.
 */
const noopSubscribe = () => () => {}
function useResolvedToken(cssVar: string, themeKey: string): string {
  return useSyncExternalStore(
    noopSubscribe,
    () =>
      `${themeKey}:${getComputedStyle(document.documentElement).getPropertyValue(cssVar).trim()}`,
    () => `${themeKey}:`
  ).slice(themeKey.length + 1)
}
...
function ColorSwatch({
  ...
  const tokenValue = useResolvedToken(cssVar, themeKey)
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0 leading-tight">
        ...
        <p className="truncate font-mono text-[0.6875rem] text-text-subtle">{tokenValue || '—'}</p>
      </div>
    </div>
  )
}
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-bg/85 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <span className="truncate text-sm font-bold tracking-tight text-text">SurveyDesk</span>
        ...
        <main id="main-content" tabIndex={-1} className="min-w-0 pb-20 outline-none">
          ...
          <div className="border-b border-border py-14">
            ...
            <h1 className="mt-3 max-w-[18ch] text-[clamp(2rem,5vw,3rem)] font-semibold tracking-tight text-balance text-text">
              피드백 인프라를 받치는 디자인 토큰과 컴포넌트
            </h1>
            <p className="mt-4 max-w-[65ch] text-pretty text-text-muted">
              이 페이지는 SurveyDesk 의 실제 디자인 시스템을 그대로 렌더링합니다. 색상은{' '}
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_minmax(0,32ch)]">
              <div className="space-y-4">
                {TYPE_SCALE.map((t) => (
                  ...
                  >
                    <span className={cn('text-text', t.cls)}>응답이 들어오는 중입니다</span>
                    <span className="shrink-0 font-mono text-xs text-text-subtle">{t.label}</span>
                  </div>
                ))}
                <div className="flex items-center gap-4 pt-1">
                  <span className="font-mono text-sm text-text">mono · NPS +62 · ★ 4.38</span>
              ...
              <div className="rounded-lg border border-border bg-surface p-5">
                ...
                <p className="max-w-[68ch] text-pretty text-text-muted">
                  SurveyDesk 는 형제 앱의 피드백을 한 곳에서 모읍니다. 임베드 위젯이
                  별점·NPS·객관식·자유서술을 수집하고, 대시보드가 평균·NPS·선택지 분포로 집계합니다.
          ...
          >
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Demo caption="도메인 칩 — ActivePill · QuestionTypeBadge(5종)">
                <ActivePill active />
                <ActivePill active={false} />
                <QuestionTypeBadge type="rating" />
                <QuestionTypeBadge type="nps" />
                <QuestionTypeBadge type="single_choice" />
                <QuestionTypeBadge type="multi_choice" />
                <QuestionTypeBadge type="text" />
              </Demo>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <StatCard label="응답 수" value="248" hint="v3 기준" />
              <StatCard
                label="평균 별점"
                value="4.38"
                hint="216개 응답 · 5점 만점"
                tone="warning"
              />
              <StatCard label="NPS" value="+62" hint="추천 168 · 비추천 14" tone="success" />
            </div>
            <Card className="mt-4 max-w-md">
              <CardHeader action={<QuestionTypeBadge type="single_choice" />}>
                <CardTitle>어떤 용도로 사용하시나요?</CardTitle>
              </CardHeader>
              <CardContent>
                <MiniBar
                  rows={[
                    { label: '팀/업무', count: 124, tone: 'accent' },
                    { label: '개인', count: 86, tone: 'accent' },
                    { label: '평가/테스트', count: 38, tone: 'accent' },
                  ]}
                  total={248}
                />
              </CardContent>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <Card>
                <CardContent className="space-y-4">
                  <Field label="설문 제목" htmlFor="d-name" hint="위젯 상단에 노출됩니다.">
                    <Input id="d-name" placeholder="예: 데모 앱은 어떠셨나요?" />
                  </Field>
                  <Field
                    label="질문 id"
                    htmlFor="d-error"
                    error="질문 id 는 영숫자·_·- 만 가능합니다."
                  >
                    <Input id="d-error" defaultValue="q rating!" aria-invalid />
                  </Field>
                  <Field label="질문 유형" htmlFor="d-type">
                    <Select id="d-type" defaultValue="rating">
                      <option value="rating">별점 (1–5)</option>
                      <option value="nps">NPS (0–10)</option>
                      <option value="single_choice">단일 선택</option>
                      <option value="text">자유서술</option>
                    </Select>
                  </Field>
                </CardContent>
              ...
              <Card>
                <CardContent className="space-y-4">
                  <Field label="안내문" htmlFor="d-textarea" hint="짧게 적어 주세요.">
                    <Textarea id="d-textarea" placeholder="몇 가지 짧은 질문에 답해 주시면…" />
                  </Field>
                  <Field label="잠긴 필드" htmlFor="d-disabled" hint="활성 버전은 직접 수정 불가">
                    <Input id="d-disabled" defaultValue="v2 (활성)" disabled />
                  </Field>
                  <div className="flex items-center gap-2">
            ...
            <Card>
              <CardContent>
                <Tabs defaultValue="overview">
                  <TabsList>
                    <TabsTrigger value="overview">집계</TabsTrigger>
                  ...
                  <TabsContent value="responses" className="pt-4 text-sm text-text-muted">
                    응답 테이블과 상세가 여기에 들어갑니다.
                  </TabsContent>
                  <TabsContent value="editor" className="pt-4 text-sm text-text-muted">
                    설문 질문 CRUD 가 여기에 들어갑니다.
                  </TabsContent>
                </Tabs>
              </CardContent>
            </Card>
            ...
            <Demo caption="모두 포털로 렌더링되어 overflow 를 벗어납니다.">
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>v3 을 활성화할까요?</DialogTitle>
                    <DialogDescription>
                      활성화하면 기존 활성본이 내려가고 위젯이 즉시 이 버전을 받습니다.
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
                    <DialogClose asChild>
              ...
              <Tooltip content="이 응답을 CSV 로 내보냅니다">
                <Button variant="ghost" size="sm">
                  Tooltip (hover)
                </Button>
              </Tooltip>
            ...
            <CodeBlock
              code={`<script src="https://surveys.example.com/feedback-widget.js" defer></script>
<script>
  SurveyDesk.init({ appId: 'offhours', endpoint: 'https://surveys.example.com' })
            ...
            <Card className="max-w-md">
              <CardHeader action={<ActivePill active />}>
                <CardTitle>데모 앱은 어떠셨나요?</CardTitle>
                <CardDescription>v3 · 응답 248건 · 최근 갱신 2026.06.15</CardDescription>
              </CardHeader>
              ...
              <CardFooter className="flex items-center justify-between">
                <span className="font-mono text-xs text-text-subtle">appId: demo</span>
                <Button variant="ghost" size="sm">
          ...
          <p className="border-t border-border pt-8 text-xs text-text-subtle">
            이 페이지는 SurveyDesk 의 실제 토큰과 컴포넌트만 사용합니다. 새 색·새 컴포넌트를 만들지
            않습니다.
          </p>
        </main>
      </div>
