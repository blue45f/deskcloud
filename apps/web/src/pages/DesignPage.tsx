...
import { Badge, ConversationKindBadge, OnlinePill, PlanBadge } from '@/components/ui/badge'
...
/* ──────────────────────────────────────────────────────────────────────────
   라이브 스타일 가이드 — ChatDesk 의 실제 토큰(styles/index.css)과 components
   를 그대로 렌더링한다. 새 팔레트·새 컴포넌트를 만들지 않는다.
   ────────────────────────────────────────────────────────────────────────── */
...
/** getComputedStyle 으로 토큰을 즉시 해석한다. 테마(nonce)가 바뀌면 다시 읽는다. */
function useResolvedToken(cssVar: string, nonce: string): string {
  const [value, setValue] = useState('')
  // 렌더된 DOM(외부 시스템)에서 해석된 CSS 변수 값을 읽어 동기화하는 정당한 effect 다.
  // CSS 변수의 계산값은 렌더 중 알 수 없어(브라우저만 안다) setState 가 불가피하다.
  useEffect(() => {
    const raw = getComputedStyle(document.documentElement).getPropertyValue(cssVar).trim()
    // eslint-disable-next-line react-hooks/set-state-in-effect -- DOM 계산값 동기화(위 주석)
    setValue(raw)
  }, [cssVar, nonce])
  return value
}
...
function ColorSwatch({
  ...
  cssVar: string
  name: string
  /** 테마 변경 시 토큰 재해석을 트리거하는 불투명 토큰(현재 테마 값). */
  nonce: string
  className?: string
}) {
...
/** 데모용 메시지 — 일반·시스템·삭제됨 3종을 보여 준다(MessageThread 그대로). */
...
export default function DesignPage() {
  ...
  // 테마가 바뀌면 토큰 값을 다시 읽도록 resolved 자체를 nonce 로 사용(effect 불필요).
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-bg/85 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <span className="truncate text-sm font-bold tracking-tight text-text">ChatDesk</span>
        ...
        <main id="main-content" tabIndex={-1} className="min-w-0 pb-20 outline-none">
          ...
          <div className="border-b border-border py-14">
            ...
            <h1 className="mt-3 max-w-[18ch] text-[clamp(2rem,5vw,3rem)] font-semibold tracking-tight text-balance text-text">
              메시징 인프라를 받치는 디자인 토큰과 컴포넌트
            </h1>
            <p className="mt-4 max-w-[65ch] text-pretty text-text-muted">
              이 페이지는 ChatDesk 의 실제 디자인 시스템을 그대로 렌더링합니다. 색상은{' '}
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_minmax(0,32ch)]">
              <div className="space-y-4">
                {TYPE_SCALE.map((t) => (
                  ...
                  >
                    <span className={cn('text-text', t.cls)}>메시지가 도착했습니다</span>
                    <span className="shrink-0 font-mono text-xs text-text-subtle">{t.label}</span>
                  </div>
                ))}
                <div className="flex items-center gap-4 pt-1">
                  <span className="font-mono text-sm text-text">mono · pk_3f9a · 14:32 · 248</span>
              ...
              <div className="rounded-lg border border-border bg-surface p-5">
                ...
                <p className="max-w-[68ch] text-pretty text-text-muted">
                  ChatDesk 는 1:1 쪽지 DM 과 그룹 채팅을 한 줄 임베드로 제공합니다.
                  타이핑·읽음·presence·unread 를 실시간으로 동기화하고, 운영자는 대화를 모니터하며
                  시스템 발송·모더레이션을 합니다. 본문은 65–75ch 범위로 줄길이를 제한해 장문도
                  편안하게 읽히도록 합니다.
                </p>
              </div>
            </div>
          ...
          >
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Demo caption="크기 — sm · md · lg · icon">
                ...
                <Button size="icon" aria-label="발송">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Demo caption="도메인 칩 — ConversationKindBadge · PlanBadge · OnlinePill">
                <ConversationKindBadge kind="dm" />
                <ConversationKindBadge kind="group" />
                <PlanBadge plan="free" />
                <PlanBadge plan="pro" />
                <OnlinePill count={3} />
                <OnlinePill count={0} />
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <StatCard
                ...
                tone="info"
              />
              <StatCard icon={Users} label="온라인" value="7" hint="현재 접속" tone="success" />
              <StatCard
                icon={MessageSquare}
                label="사용률"
                value="12%"
            ...
            {/* Keys */}
            <h3 className="mt-12 mb-3 text-base font-semibold text-text">키 필드 (pk · sk)</h3>
            <div className="grid gap-4 md:grid-cols-2">
              <KeyField
                label="Publishable 키 (pk_)"
                value="pk_3f9a1c8e7b2d4f6a9c0e1d2f3a4b5c6d"
                note="브라우저에 노출되는 공개 키입니다."
              />
              <KeyField
                label="Secret 키 (sk_)"
                value="sk_9a8b7c6d5e4f3a2b1c0d9e8f7a6b5c4d3e2f1a0b9c8d7e6f"
                secret
                note="기본 마스킹 · 보기 토글 · 복사."
              />
            </div>

            {/* Message thread */}
            <h3 className="mt-12 mb-3 text-base font-semibold text-text">메시지 뷰어</h3>
            <Card className="max-w-2xl">
              <CardHeader action={<ConversationKindBadge kind="group" />}>
                <CardTitle>지원 채널</CardTitle>
                <CardDescription>일반 · 시스템 · 삭제됨 메시지를 그대로 렌더링</CardDescription>
              </CardHeader>
              <CardContent>
                <MessageThread messages={DEMO_MESSAGES} onModerate={() => undefined} />
              </CardContent>
            </Card>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <Card>
                <CardContent className="space-y-4">
                  <Field label="테넌트 이름" htmlFor="d-name" hint="대시보드에 표시됩니다.">
                    <Input id="d-name" placeholder="예: Offhours" />
                  </Field>
                  <Field
                    label="허용 Origin"
                    htmlFor="d-error"
                    error="http(s)://host[:port] 형태여야 합니다."
                  >
                    <Input id="d-error" defaultValue="app.example" aria-invalid />
                  </Field>
              ...
              <Card>
                <CardContent className="space-y-4">
                  <Field label="시스템 메시지" htmlFor="d-textarea" hint="발신자 없는 공지">
                    <Textarea id="d-textarea" placeholder="점검이 예정되어 있습니다…" />
                  </Field>
                  <Field label="잠긴 필드" htmlFor="d-disabled" hint="secret 키는 수정 불가">
                    <Input id="d-disabled" defaultValue="sk_•••• (해시 저장)" disabled />
                  </Field>
                  <div className="flex items-center gap-2">
                    <Checkbox id={checkId} defaultChecked />
            ...
            <Card>
              <CardContent>
                <Tabs defaultValue="monitor">
                  <TabsList>
                    <TabsTrigger value="monitor">대화 모니터</TabsTrigger>
            ...
            <Demo caption="모두 포털로 렌더링되어 overflow 를 벗어납니다.">
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      새 키가 발급되고 이전 키는 즉시 무효화됩니다. 위젯과 서버의 키를 모두 교체해야
                      합니다.
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
              ...
              <Tooltip content="이 메시지를 모더레이션(삭제)합니다">
                <Button variant="ghost" size="sm">
                  Tooltip (hover)
                </Button>
              </Tooltip>
            ...
            <CodeBlock
              code={`<script src="https://chat.example.com/chat-widget.js" defer></script>
<script>
  ChatDesk.init({ publishableKey: 'pk_데모키', endpoint: 'https://chat.example.com', memberId: 'alice' })
</script>`}
              language="html"
              className="max-w-2xl"
            />
            ...
            <Card className="max-w-md">
              <CardHeader action={<OnlinePill count={2} />}>
                <CardTitle>alice ↔ bob</CardTitle>
                <CardDescription>1:1 DM · 메시지 18건 · 최근 14:32</CardDescription>
              </CardHeader>
              ...
              <CardFooter className="flex items-center justify-between">
                <span className="font-mono text-xs text-text-subtle">kind: dm</span>
                <Button variant="ghost" size="sm">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="flex flex-col gap-2">
                <EmptyState
                  icon={MessagesSquare}
                  title="아직 대화가 없습니다"
                  description="위젯이 pk 로 연결되거나 서버가 sk 로 대화를 만들면 여기에 표시됩니다."
                  action={
          ...
          <p className="border-t border-border pt-8 text-xs text-text-subtle">
            이 페이지는 ChatDesk 의 실제 토큰과 컴포넌트만 사용합니다. 새 색·새 컴포넌트를 만들지
            않습니다.
          </p>
        </main>
      </div>
