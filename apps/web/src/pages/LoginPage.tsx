...
import { getTenant } from '@/services/files'
...
export function LoginPage(): ReactElement {
  ...
  const onSubmit = async (e: FormEvent): Promise<void> => {
    ...
    setBusy(true)
    // 키를 먼저 보관하고 /tenant 로 검증한다(실패 시 해제).
  ...
  return (
    <section style={{ maxWidth: 440, margin: '0 auto' }}>
      ...
      <p className="fd-muted">
        secret 키(sk_)로 로그인합니다. 데모는 <code className="fd-inline">sk_demo</code> 를
        사용하세요.
      </p>
      {error ? <div className="fd-alert fd-alert-error">{error}</div> : null}
      <form className="fd-card" onSubmit={onSubmit}>
        <label className="fd-field">
          <span className="fd-label">secret 키</span>
          <input
            className="fd-input"
            value={key}
        ...
        <button type="submit" className="fd-btn fd-btn-primary" disabled={busy || !key.trim()}>
          {busy ? '확인 중…' : '로그인'}
        </button>
      </form>
    </section>
