import { ArrowRight, Inbox, MessageSquarePlus, Moon, RotateCcw, Send, Sun } from 'lucide-react'
...
/** 상태 → Badge 톤. 진행도에 따라 디자인 토큰 색을 매핑한다. */
...
/** ISO 1주 이내는 상대 표기, 그 이상은 절대 표기로 폴백. */
...
/**
 * 공개 게시판 — listInquiries 로 최신순 노출. 로딩/빈/오류 상태를 모두 다룬다.
 * 상태 변경은 비동기 콜백에서만 한다(set-state-in-effect 회피). 새로고침 시
 * 로딩 표시는 reload 핸들러가 동기로 처리한다. 부모(등록 성공)는 key 리마운트로 재조회.
 */
...
export default function SupportPage() {
  ...
  const [boardKey, setBoardKey] = useState(0) // 등록 성공 시 게시판 재조회 트리거.
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    // 허니팟이 채워졌으면 봇으로 간주, 조용히 성공 처리(서버도 202 무음).
    if (website.trim()) {
      setSubmitted(true)
      resetForm()
      return
    ...
    <div className="min-h-screen bg-bg text-text">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-bg/85 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between gap-4 px-4 sm:px-6">
          <Link to="/" aria-label="RealtimeDesk 홈">
            <Brand />
          </Link>
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <Button asChild size="sm">
              <Link to={isAuthed ? '/app' : '/login'}>{isAuthed ? '대시보드' : '시작하기'}</Link>
            </Button>
          </div>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-3xl space-y-6 px-4 py-10 sm:px-6">
          ...
          <section className="rounded-lg border border-border bg-surface p-6">
            ...
            <h1
              ref={headingRef}
              tabIndex={-1}
              className="mt-1.5 text-2xl font-semibold tracking-tight text-text outline-none"
            >
              무엇을 도와드릴까요?
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-7 text-pretty text-text-muted">
          ...
          <Card>
            ...
            <CardContent>
              {submitted ? (
                <div
                  role="status"
                  className="flex items-start gap-3 rounded-md border border-success/40 bg-success-soft p-4"
                >
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-success">문의가 접수되었습니다.</p>
                ...
                <form className="space-y-5" onSubmit={handleSubmit} noValidate>
                      ...
                      {INQUIRY_CATEGORIES.map((value) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              'rounded-md border px-3 py-1.5 text-xs font-medium transition-colors focus-visible:ring-2 focus-visible:ring-accent-strong',
                              selected
                  ...
                  <p role="alert" aria-live="assertive">
                    {error ? (
                      <span className="block rounded-md border border-danger/40 bg-danger-soft px-3 py-2 text-xs font-medium text-danger">
                        {error}
                      </span>
                    ) : null}
                  </p>
          ...
          {/* 공개 게시판 — 등록 성공 시 key 변경으로 리마운트되어 재조회된다. */}
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-3xl flex-col items-center justify-between gap-4 px-4 py-8 text-sm text-text-subtle sm:flex-row sm:px-6">
          <Brand compact className="text-text-muted" />
          <p>RealtimeDesk — 실시간 pub/sub·presence as a service</p>
          <div className="flex items-center gap-4">
            <Link to="/sitemap" className="hover:text-text">
              디자인
            </Link>
            <Link to="/login" className="inline-flex items-center gap-1 hover:text-text">
              시작하기
