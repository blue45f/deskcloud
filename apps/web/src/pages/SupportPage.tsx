...
/** 상태 → 뱃지 톤. 진행도에 따라 디자인 시스템 톤을 매핑한다. */
const statusTone: Record<InquiryStatus, BadgeProps['tone']> = {
  new: 'info',
  in_progress: 'warning',
  resolved: 'success',
  closed: 'neutral',
  ...
  return (
    <Card className="p-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Badge tone={statusTone[inquiry.status] ?? 'neutral'} size="sm">
    ...
    <section className="space-y-4" aria-labelledby="support-board-heading">
      ...
      <div aria-live="polite" aria-busy={loading}>
        {state.phase === 'loading' ? (
          ...
          <div className="rounded-lg border border-danger/30 bg-danger-soft/50 p-5">
            <p className="text-sm font-semibold text-danger">{state.message}</p>
            <Button
              variant="secondary"
              size="sm"
              onClick={reload}
              className="mt-3 border-danger/40 text-danger"
            >
              <RotateCcw className="size-3.5" aria-hidden />
              다시 시도
            </Button>
          ...
          <Card className="p-8 text-center">
            <p className="text-sm font-semibold text-text">아직 등록된 문의가 없습니다</p>
...
/**
 * 문의(Inquiry) 게시판(공개) — desk-platform 공개 API로 문의를 접수하고,
 * 하단에 공개 게시판 목록을 노출한다. 전화·이메일 대신 이 게시판으로 문의를 통합했다.
 */
        ...
        <div className="space-y-6">
          <section>
            ...
            <p className="mt-2 max-w-2xl text-pretty text-text-muted">
              ...
              ModerationDesk 운영자가 확인 후 상태를 업데이트합니다. 전화·이메일 대신 이 게시판으로
              문의를 통합했습니다.
            </p>
          </section>
          ...
          <section className="space-y-4">
            ...
            {submitted ? (
              <div
                role="status"
                className="flex items-start gap-3 rounded-lg border border-success/30 bg-success-soft/50 p-4"
              >
              ...
              <Card>
                <CardContent>
                  <form className="space-y-5" onSubmit={handleSubmit} noValidate>
                    <fieldset>
                      <legend className="mb-2 block text-[0.8125rem] font-medium text-text">
                        카테고리
                      </legend>
                        ...
                        {INQUIRY_CATEGORIES.map((value) => {
                          ...
                          return (
                            <button
                              ...
                              className={
                                selected
                                  ? 'rounded-md border border-accent-strong bg-accent-soft px-3 py-1.5 text-xs font-semibold text-accent-strong'
                                  : 'rounded-md border border-border bg-bg px-3 py-1.5 text-xs font-semibold text-text-muted transition-colors hover:border-border-strong hover:text-text'
                              }
                            >
                    ...
                    <p role="alert" aria-live="assertive" className="min-h-0">
                      {error ? (
                        <span className="block rounded-md border border-danger/30 bg-danger-soft/50 px-3 py-2 text-xs font-semibold text-danger">
                          {error}
                        </span>
                      ) : null}
                    </p>
          ...
          <div className="flex items-center gap-2 border-t border-border pt-5 text-xs font-semibold text-text-subtle">
