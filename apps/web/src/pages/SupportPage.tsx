  ...
  return (
    <Card className="h-full p-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="ml-auto text-xs text-text-subtle">{relativeTime(inquiry.createdAt)}</span>
      </div>
...
function InquiryBoard({ reloadKey }: { reloadKey: number }) {
  ...
  // 로딩 표시는 effect 가 아니라 키를 바꾸는 호출부(새로고침 버튼)에서 setState 로 처리한다.
  useEffect(() => {
...
/**
 * 문의(Support) 페이지(공개) — desk-platform 공개 게시판 API 로 문의를 등록하고,
 * 등록된 문의를 공개 게시판으로 노출한다. 전화·이메일 대신 이 게시판으로 문의를 통합한다.
 */
        ...
        <div className="mx-auto max-w-3xl space-y-6 px-4 py-8 sm:px-6">
          ...
          <section className="rounded-lg border border-border bg-surface p-5 sm:p-6">
            ...
            <h1
              ref={headingRef}
              tabIndex={-1}
              className="mt-3 text-2xl font-semibold tracking-tight text-balance text-text outline-none"
            >
              무엇을 도와드릴까요?
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-7 text-pretty text-text-muted">
              제휴·버그·의견·이용 문의를 남겨 주세요. 접수된 문의는 아래 게시판에 공개로 표시되며,
              CommunityDesk 운영자가 확인 후 상태를 업데이트합니다. 전화·이메일 대신 이 게시판으로
              문의를 통합했습니다.
            </p>
          </section>
          ...
          <section className="space-y-4" aria-labelledby="support-form-heading">
            <h2
              ...
              className="flex items-center gap-2 text-base font-semibold text-text"
            >
              <MessageSquarePlus className="size-4 text-text-subtle" aria-hidden />
              문의 남기기
            </h2>
                ...
                <CardContent>
                  <form className="space-y-5" onSubmit={handleSubmit} noValidate>
                        ...
                        {INQUIRY_CATEGORIES.map((value) => {
                          ...
                          return (
                            <button
                              ...
                              className={
                                selected
                                  ? 'rounded-md border border-ink bg-ink px-3 py-1.5 text-[0.8125rem] font-medium text-ink-fg'
                                  : 'rounded-md border border-border bg-bg px-3 py-1.5 text-[0.8125rem] font-medium text-text-muted transition-colors hover:border-border-strong hover:text-text'
                              }
                            >
                    ...
                    <div role="alert" aria-live="assertive">
                      {error ? (
                        <p className="rounded-md border border-danger/30 bg-danger-soft/40 px-3 py-2 text-xs font-medium text-danger">
                          {error}
                        </p>
                      ) : null}
                    </div>
