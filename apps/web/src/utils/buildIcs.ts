 ...
 *
 * 컴플라이언스 담당자가 약관 시행일(effectiveAt)을 캘린더 앱
 * (Apple Calendar, Google Calendar, Outlook 등)에 등록할 수 있도록
 * VCALENDAR + VEVENT 텍스트를 생성한다.
 */

export interface IcsEvent {
  /** VEVENT의 UID. 같은 버전은 같은 UID 를 갖게 해 캘린더 앱이 중복을 합치게 한다. */
  uid: string
  title: string
  description?: string
  location?: string
...
const PRODID = '-//TermsDesk//Effective Date Export//KO'
const CRLF = '\r\n'
 ...
 *
 * @param event 변환할 이벤트
 * @param filename 다운로드 파일명. 생략 시 `termsdesk-<제목>.ics`로 자동 생성.
 */
export function downloadIcs(event: IcsEvent, filename?: string): void {
  ...
  const finalName = filename
    ...
    : sanitizeFilename(`termsdesk-${event.title}`)
