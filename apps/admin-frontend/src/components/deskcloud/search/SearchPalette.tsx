...
function ResultsList(props: ResultsListProps): ReactElement {
  ...
  // 그룹 i 의 첫 항목이 flatHits 에서 갖는 시작 오프셋(앞선 그룹 길이의 누적합).
  // 렌더 중 변수 변이(let flatIndex) 대신 사전 계산 — react-compiler 불변성 규칙 준수.
        ...
        <li key={group.category} role="presentation">
          ...
          <ul role="presentation" style={{ listStyle: 'none', margin: 0, padding: 0 }}>
            {group.hits.map((hit, hitIdx) => {
              const index = groupOffsets[groupIdx]! + hitIdx
...
export function SearchPalette(props: SearchPaletteProps): ReactElement | null {
  ...
  return (
    <div className="sk-root" style={rootStyle}>
      {/* 마우스 전용 click-outside 백드롭(편의). 키보드 사용자는 Esc 로 동등하게 닫음. */}
      ...
      >
        {/* role="dialog" 컨테이너의 키보드 핸들러(↑↓/Enter/Esc/Tab) — 모달 표준 패턴. */}
