 ...
 * ──────────────────────────────────────────────────────────────────────────
 * npm publish 가 막힌 동안 형제 앱(spa-seo-gateway·offhours·…)에 그대로 복붙해서 쓰는 버전입니다.
 * 워크스페이스 의존(@notifydesk/shared·widget) 0 — 클라이언트·아이콘·상대시간·스타일·DTO 를
 * 이 파일에 인라인했습니다. 동작/디자인은 @notifydesk/widget 의 <NotificationBell> 과 동일합니다.
 *
 * 사용:
 *   import { NotificationBell } from './NotificationBell'
 *   <NotificationBell recipientId="user_42" publishableKey="pk_…" endpoint="https://notify.example.com" />
 *
 * 백엔드 계약(퍼블리시 키 · Bearer pk_… + Origin 검사):
...
/* ============================ 상대 시간(인라인) ============================ */
/* 외부 라이브러리 0(Intl.RelativeTimeFormat 만). "방금 전", "3분 전" … 일주일 이상은 짧은 절대 날짜. */
...
export function NotificationBell(props: NotificationBellProps): ReactElement {
  ...
  // 콜백을 "최신 ref" 로 보관 — effect 의존성을 안정화해 폴링 타이머 재시작을 막는다.
  // ref 쓰기는 렌더 중이 아니라 effect 안에서(react-hooks/refs 준수).
  ...
  const rootStyle: CSSProperties = {
    ['--nd-accent' as string]: accent,
    ['--nd-accent-ink' as string]: accentInk,
  }
