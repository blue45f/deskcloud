...
/** getComputedStyle 으로 토큰을 즉시 해석한다. 테마(themeKey)가 바뀌면 다시 읽는다. */
function useResolvedToken(cssVar: string, themeKey: string): string {
  const [value, setValue] = useState('')
  useEffect(() => {
    const raw = getComputedStyle(document.documentElement).getPropertyValue(cssVar).trim()
    // eslint-disable-next-line react-hooks/set-state-in-effect -- 외부 시스템(DOM 계산 스타일) 읽기 동기화
    setValue(raw)
  }, [cssVar, themeKey])
  return value
}
        ...
        <main id="main-content" tabIndex={-1} className="min-w-0 pb-20 outline-none">
          ...
          <div className="border-b border-border py-14">
            ...
            <h1 className="mt-3 max-w-[18ch] text-[clamp(2rem,5vw,3rem)] font-semibold tracking-tight text-balance text-text">
              모더레이션 콘솔을 받치는 디자인 토큰과 컴포넌트
            </h1>
            <p className="mt-4 max-w-[65ch] text-pretty text-text-muted">
              이 페이지는 ModerationDesk 의 실제 디자인 시스템을 그대로 렌더링합니다. 색상은{' '}
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_minmax(0,32ch)]">
              ...
              <div className="rounded-lg border border-border bg-surface p-5">
                ...
                <p className="max-w-[68ch] text-pretty text-text-muted">
                  ModerationDesk 는 유해 콘텐츠를 규칙과 AI 로 거릅니다. 금칙어·정규식 규칙은 항상,
                  Claude 독성 점수는 선택입니다. 본문은 65–75ch 범위로 줄길이를 제한해 장문도
                  편안하게 읽히도록 합니다.
                </p>
              </div>
            </div>
          ...
          >
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Demo caption="판정 — VerdictBadge(allow · flag · block)">
                <VerdictBadge verdict="allow" />
                <VerdictBadge verdict="flag" />
                <VerdictBadge verdict="block" />
              </Demo>
              <Demo caption="신고 상태 — ReportStatusBadge(open · reviewing · resolved · dismissed)">
                <ReportStatusBadge status="open" />
                <ReportStatusBadge status="reviewing" />
                <ReportStatusBadge status="resolved" />
                <ReportStatusBadge status="dismissed" />
              </Demo>
              <Demo caption="규칙 — kind · action · plan · enabled">
                <RuleKindBadge kind="substring" />
                <RuleKindBadge kind="regex" />
                <RuleActionBadge action="block" />
                <RuleActionBadge action="review" />
                <PlanBadge plan="free" />
                <PlanBadge plan="pro" />
                <EnabledPill enabled />
                <EnabledPill enabled={false} />
              </Demo>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <StatCard
                ...
                tone="warning"
              />
              <StatCard label="차단율" value="12%" hint="최근 1,000건 중" tone="info" />
            </div>
            <Card className="mt-4 max-w-md">
              <CardHeader action={<Badge tone="neutral">최근 1,000건</Badge>}>
                <CardTitle>판정 분포</CardTitle>
              </CardHeader>
              <CardContent>
                <MiniBar
                  rows={[
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <Card>
                <CardContent className="space-y-4">
                  <Field label="패턴" htmlFor="d-name" hint="검사 시 이 패턴을 평가합니다.">
                    <Input id="d-name" placeholder="예: spam" className="font-mono" />
                  </Field>
                  <Field label="정규식 패턴" htmlFor="d-error" error="유효한 정규식이 아닙니다">
                    <Input
                      id="d-error"
                      defaultValue="(buy|cheap"
                      aria-invalid
                      className="font-mono"
                    />
                  </Field>
                  <Field label="매칭 종류" htmlFor="d-type">
                    <Select id="d-type" defaultValue="substring">
                      <option value="substring">부분일치 (substring)</option>
                      <option value="exact">완전일치 (exact)</option>
              ...
              <Card>
                <CardContent className="space-y-4">
                  <Field label="신고 사유" htmlFor="d-textarea" hint="검토에 참고됩니다.">
                    <Textarea id="d-textarea" placeholder="신고 내용을 자세히 알려 주세요…" />
                  </Field>
                  <Field
                    label="잠긴 필드"
                    htmlFor="d-disabled"
                    hint="회전 전 secret 키는 표시 불가"
                  >
                    <Input
                      id="d-disabled"
                      defaultValue="sk_••••••••"
            ...
            <Card>
              <CardContent>
                <Tabs defaultValue="queue">
                  <TabsList>
                    <TabsTrigger value="queue">신고 큐</TabsTrigger>
                    <TabsTrigger value="rules">금칙 규칙</TabsTrigger>
                    <TabsTrigger value="logs">검사 로그</TabsTrigger>
                  </TabsList>
                  <TabsContent value="queue" className="pt-4 text-sm text-text-muted">
                    활성 탭은 하단 보더가 accent 색으로 강조됩니다.
                  </TabsContent>
                  <TabsContent value="rules" className="pt-4 text-sm text-text-muted">
                    금칙어·정규식 규칙 CRUD 가 여기에 들어갑니다.
                  </TabsContent>
                  <TabsContent value="logs" className="pt-4 text-sm text-text-muted">
                    모더레이션 검사 로그가 여기에 들어갑니다.
                  </TabsContent>
                </Tabs>
              </CardContent>
            </Card>
            ...
            <Demo caption="모두 포털로 렌더링되어 overflow 를 벗어납니다.">
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>이 신고를 기각할까요?</DialogTitle>
                    <DialogDescription>
                      기각하면 신고가 종료되고 큐에서 내려갑니다. 메모는 그대로 보존됩니다.
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
                    <DialogClose asChild>
              ...
              <Tooltip content="이 신고의 상태를 전이합니다">
                <Button variant="ghost" size="sm">
                  Tooltip (hover)
                </Button>
              </Tooltip>
            ...
            <CodeBlock
              code={`import { createModerationClient } from '@moderationdesk/sdk'

const md = createModerationClient({ secretKey: process.env.MD_SECRET, endpoint })
const { verdict } = await md.moderate(comment)
if (verdict === 'block') throw new Error('차단됨')`}
              language="ts"
              className="max-w-2xl"
            />
            ...
            <Card className="max-w-md">
              <CardHeader action={<ReportStatusBadge status="open" />}>
                <CardTitle>comment/c_1001</CardTitle>
                <CardDescription>접수 2026.06.15 · 신고자 user_42</CardDescription>
              </CardHeader>
              ...
              <CardFooter className="flex items-center justify-between">
                <span className="font-mono text-xs text-text-subtle">사유: 욕설/혐오 발언</span>
                <Button variant="ghost" size="sm">
                  처리 <ArrowUpRight className="size-3.5" />
                </Button>
              </CardFooter>
            </Card>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="flex flex-col gap-2">
                <EmptyState
                  icon={Inbox}
                  title="표시할 신고가 없습니다"
                  description="위젯의 신고 버튼이 접수를 보내면 여기에 표시됩니다."
                  action={
          ...
          <p className="border-t border-border pt-8 text-xs text-text-subtle">
            이 페이지는 ModerationDesk 의 실제 토큰과 컴포넌트만 사용합니다. 새 색·새 컴포넌트를
            만들지 않습니다.
          </p>
        </main>
      </div>
