...
export const envSchema = z.object({
  ...
  REALTIMEDESK_MODE: z.enum(["self-hosted", "saas"]).optional(),
  PORT: numericString,
  WEB_ORIGIN: optionalNonEmpty,
  REALTIME_PATH: optionalNonEmpty,
  REALTIME_HISTORY_LIMIT: numericString,
...
export function validateEnv(
  ...
  // neon.tech URL 은 이 프로젝트 정책상 금지(로컬은 PGlite/qa-postgres).
  const dbUrl = env.DATABASE_URL?.trim();
  if (dbUrl && /neon\.tech/i.test(dbUrl)) {
    warn(
      "[env] 정책 경고: DATABASE_URL 에 neon.tech 가 감지되었습니다. 로컬은 PGlite/qa-postgres 만 사용하세요.",
    );
  }
