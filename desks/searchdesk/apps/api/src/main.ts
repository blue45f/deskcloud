...
async function bootstrap(): Promise<void> {
  ...
  // publishable(검색) 경로의 실제 출처 허용목록은 PublishableKeyGuard 가 테넌트별로 강제한다.
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('SearchDesk API')
    .setDescription(
      '외부 온보딩형(멀티테넌트) Hosted Search-as-a-Service — 가입(pk_/sk_)·문서 색인·전문 검색(랭킹·하이라이트·패싯)·어드민'
    )
    .setVersion('0.1.0')
  ...
  Logger.log(
    `SearchDesk API (${cfg.mode}) → http://localhost:${cfg.port}  · docs: /api/docs`,
    'Bootstrap'
  )
}
