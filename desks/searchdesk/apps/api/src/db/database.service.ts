...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  db!: Database
  kind!: DbKind
  private pgPool: { query: (sql: string) => Promise<unknown>; end: () => Promise<void> } | null =
    null
  ...
  private async migrate(): Promise<void> {
    ...
    for (const m of MIGRATIONS) {
      ...
      // 드라이버 한정 마이그레이션(only)은 현재 kind 에서만 적용하고, 아니면 적용된 것으로 기록.
      if (m.only && !m.only.includes(this.kind)) {
