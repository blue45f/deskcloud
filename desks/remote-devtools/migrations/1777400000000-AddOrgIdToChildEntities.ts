 ...
 */
export class AddOrgIdToChildEntities1777400000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const { table, tsColumn } of tables) {
      ...
      const exists = await queryRunner.query(`SELECT to_regclass($1) AS reg`, [`public.${table}`]);
      if (!exists?.[0]?.reg) continue;

      await queryRunner.query(`ALTER TABLE "${table}" ADD COLUMN IF NOT EXISTS "org_id" uuid NULL`);

      const indexName = tsColumn ? `IDX_${table}_org_${tsColumn}` : `IDX_${table}_org`;
      const cols = tsColumn ? `("org_id", "${tsColumn}")` : `("org_id")`;
      await queryRunner.query(`CREATE INDEX IF NOT EXISTS "${indexName}" ON "${table}" ${cols}`);
    }
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const table of tables) {
      await queryRunner.query(`ALTER TABLE IF EXISTS "${table}" DROP COLUMN IF EXISTS "org_id"`);
    }
  }
}
