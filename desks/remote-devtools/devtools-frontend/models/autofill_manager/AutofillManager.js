...
export class AutofillManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      ...
      'AddressFormFilled' /* SDK.AutofillModel.Events.ADDRESS_FORM_FILLED */,
  ...
  #processAddressFormFilledData({ addressUi, filledFields }) {
    ...
    const concatAddressFields = (addressFields) =>
      addressFields.fields
        .filter((field) => field.value.length)
        .map((field) => field.value)
        .join(' ');
    this.#address = addressUi.addressFields
      .map((addressFields) => concatAddressFields(addressFields))
    ...
    for (let i = 0; i < this.#filledFields.length; i++) {
      ...
      const needle = Platform.StringUtilities.escapeForRegExp(
        this.#filledFields[i].value.replaceAll(/\s/g, ' '),
      ).replaceAll(/([.,]+)\s/g, '$1? ');
