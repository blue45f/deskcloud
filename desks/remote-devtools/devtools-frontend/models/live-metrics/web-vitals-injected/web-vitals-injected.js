...
const patchAddListener = (proto) => {
  ...
  proto.addEventListener = function (type, listener, options) {
    ...
    if (navOptions.signal) {
      navOptions.signal = AbortSignal.any([
        navOptions.signal,
        eventListenerCleanupController.signal,
      ]);
    } else {
  ...
  function onEachInteraction(interaction) {
    ...
    const event = {
      ...
      longAnimationFrameEntries: limitScripts(
        interaction.attribution.longAnimationFrameEntries
          .slice(-Spec.LOAF_LIMIT)
          .map((loaf) => loaf.toJSON()),
      ),
    };
