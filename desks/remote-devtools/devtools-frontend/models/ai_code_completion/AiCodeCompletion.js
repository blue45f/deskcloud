...
export class AiCodeCompletion {
  ...
  #buildRequest(
    prefix,
    suffix,
    inferenceLanguage = 'JAVASCRIPT' /* Host.AidaClient.AidaInferenceLanguage.JAVASCRIPT */,
    additionalFiles,
  ) {
    ...
    if (!additionalContextFiles) {
      additionalContextFiles =
        this.#panel === 'console' /* ContextFlavor.CONSOLE */
          ? [
              {
  ...
  #checkCachedRequestForResponse(request) {
    if (
      !this.#aidaRequestCache ||
      this.#aidaRequestCache.request.suffix !== request.suffix ||
    ...
    return {
      generatedSamples: possibleGeneratedSamples,
      metadata: this.#aidaRequestCache.response.metadata,
    };
  }
  ...
  static isAiCodeCompletionEnabled(locale) {
    ...
    if (
      !aidaAvailability ||
      aidaAvailability.blockedByGeo ||
      aidaAvailability.blockedByAge ||
      aidaAvailability.blockedByEnterprisePolicy
    ) {
      return false;
    }
    return Boolean(
      aidaAvailability.enabled && Root.Runtime.hostConfig.devToolsAiCodeCompletion?.enabled,
    );
  }
  static isAiCodeCompletionStylesEnabled(locale) {
    ...
    return Boolean(
      aidaAvailability.enabled && Root.Runtime.hostConfig.devToolsAiCodeCompletionStyles?.enabled,
    );
  }
}
