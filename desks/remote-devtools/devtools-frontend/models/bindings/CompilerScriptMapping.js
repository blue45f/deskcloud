...
export class CompilerScriptMapping {
  ...
  getLocationRangesForSameSourceLocation(rawLocation) {
    ...
    const ranges = sourceMap.findReverseRanges(
      entry.sourceURL,
      entry.sourceLineNumber,
      entry.sourceColumnNumber,
    );
    return ranges.map(({ startLine, startColumn, endLine, endColumn }) => {
      const start = script.relativeLocationToRawLocation({
        lineNumber: startLine,
        columnNumber: startColumn,
      });
      const end = script.relativeLocationToRawLocation({
        lineNumber: endLine,
  ...
  uiSourceCodeForURL(url, isContentScript) {
    const projectType = isContentScript
  ...
  uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber) {
    ...
    for (const sourceMap of this.#uiSourceCodeToSourceMaps.get(uiSourceCode)) {
      ...
      locations.push(
        script.debuggerModel.createRawLocation(script, location.lineNumber, location.columnNumber),
      );
    }
    return locations;
  }
  uiLocationRangeToRawLocationRanges(uiSourceCode, textRange) {
    ...
    for (const sourceMap of this.#uiSourceCodeToSourceMaps.get(uiSourceCode)) {
      ...
      for (const scriptTextRange of sourceMap.reverseMapTextRanges(uiSourceCode.url(), textRange)) {
        ...
        const start = script.debuggerModel.createRawLocation(
          script,
          startLocation.lineNumber,
          startLocation.columnNumber,
        );
        const end = script.debuggerModel.createRawLocation(
          script,
  ...
  async functionBoundsAtRawLocation(rawLocation) {
    ...
    const { lineNumber, columnNumber } = script.rawLocationToRelativeLocation(rawLocation);
    const { url, scope } =
      sourceMap.findOriginalFunctionScope({ line: lineNumber, column: columnNumber }) ?? {};
    if (!scope || !url) {
      return null;
    }
    const project = this.#sourceMapToProject.get(sourceMap);
  ...
  sourceMapWillAttach(event) {
    ...
    if (
      this.#ignoreListManager.isUserIgnoreListedURL(script.sourceURL, {
  ...
  sourceMapAttached(event) {
    ...
    if (!project) {
      const projectType = script.isContentScript()
        ? Workspace.Workspace.projectTypes.ContentScripts
        : Workspace.Workspace.projectTypes.Network;
    ...
    for (const url of sourceMap.sourceURLs()) {
      ...
      const mimeType =
        Common.ResourceType.ResourceType.mimeFromURL(url) ?? contentType.canonicalMimeType();
    ...
    void Promise.all(
      [...scripts].map((script) => this.#debuggerWorkspaceBinding.updateLocations(script)),
