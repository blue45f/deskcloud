...
export class DebuggerWorkspaceBinding {
  ...
  constructor(resourceMapping, targetManager, ignoreListManager, workspace) {
    ...
    this.ignoreListManager.addEventListener(
      'IGNORED_SCRIPT_RANGES_UPDATED' /* Workspace.IgnoreListManager.Events.IGNORED_SCRIPT_RANGES_UPDATED */,
      (event) => this.updateLocations(event.data),
    );
    this.#liveLocationPromises = new Set();
    this.pluginManager = new DebuggerLanguagePluginManager(
      targetManager,
  ...
  async computeAutoStepRanges(mode, callFrame) {
    ...
    if (uiLocation) {
      ranges =
        (await pluginManager.uiLocationToRawLocationRanges(
    ...
    const compilerMapping = this.#debuggerModelToData.get(
      rawLocation.debuggerModel,
    )?.compilerMapping;
    if (!compilerMapping) {
      return [];
    }
  ...
  async updateLocations(script) {
    const stackTraceUpdatePromise = script
      .target()
  ...
  async createStackTraceFromProtocolRuntime(stackTrace, target) {
    ...
    const stackTracePromise = model.createFromProtocolRuntime(
      stackTrace,
      this.#translateRawFrames.bind(this),
    );
  ...
  async createStackTraceFromDebuggerPaused(pausedDetails, target) {
    const model = target.model(StackTraceImpl.StackTraceModel.StackTraceModel);
    const stackTracePromise = model.createFromDebuggerPaused(
      pausedDetails,
      this.#translateRawFrames.bind(this),
    );
    this.recordLiveLocationChange(stackTracePromise);
  ...
  async createLiveLocation(rawLocation, updateDelegate, locationPool) {
    ...
    const liveLocationPromise = modelData.createLiveLocation(
      rawLocation,
      updateDelegate,
      locationPool,
    );
  ...
  async createStackTraceTopFrameLiveLocation(rawLocations, updateDelegate, locationPool) {
    ...
    const locationPromise = StackTraceTopFrameLocation.createStackTraceTopFrameLocation(
      rawLocations,
      this,
      updateDelegate,
      locationPool,
  ...
  async uiSourceCodeForSourceMapSourceURLPromise(debuggerModel, url, isContentScript) {
    const uiSourceCode = this.uiSourceCodeForSourceMapSourceURL(
      debuggerModel,
      url,
      isContentScript,
    );
  ...
  waitForUISourceCodeAdded(url, target) {
    return new Promise((resolve) => {
      const descriptor = this.workspace.addEventListener(
        Workspace.Workspace.Events.UISourceCodeAdded,
        (event) => {
          const uiSourceCode = event.data;
          if (
            uiSourceCode.url() === url &&
            NetworkProject.targetForUISourceCode(uiSourceCode) === target
          ) {
  ...
  async uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber) {
    const locations = await this.pluginManager.uiLocationToRawLocations(
      uiSourceCode,
      lineNumber,
      columnNumber,
    );
  ...
  async uiLocationRangeToRawLocationRanges(uiSourceCode, textRange) {
    const ranges = await this.pluginManager.uiLocationRangeToRawLocationRanges(
      uiSourceCode,
      textRange,
    );
    if (ranges) {
  ...
  async normalizeUILocation(uiLocation) {
    const rawLocations = await this.uiLocationToRawLocations(
      uiLocation.uiSourceCode,
      uiLocation.lineNumber,
      uiLocation.columnNumber,
    );
  ...
  async shouldPause(debuggerPausedDetails, autoSteppingContext) {
    ...
    if (
      !autoSteppingContext ||
      debuggerPausedDetails.reason !== 'step' /* Protocol.Debugger.PausedEventReason.Step */ ||
      !functionLocation ||
      !frame.script.isWasm() ||
      !Common.Settings.moduleSetting('wasm-auto-stepping').get() ||
    ...
    return (
      autoSteppingContext.script() !== functionLocation.script() ||
...
class ModelData {
  ...
  constructor(debuggerModel, debuggerWorkspaceBinding) {
    ...
    this.#defaultMapping = new DefaultScriptMapping(
      debuggerModel,
      workspace,
      debuggerWorkspaceBinding,
    );
    this.#resourceMapping = debuggerWorkspaceBinding.resourceMapping;
    this.#resourceScriptMapping = new ResourceScriptMapping(
      debuggerModel,
      workspace,
      debuggerWorkspaceBinding,
    );
    this.compilerMapping = new CompilerScriptMapping(
      debuggerModel,
      workspace,
      debuggerWorkspaceBinding,
    );
  ...
  uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber = 0) {
    ...
    let locations = this.compilerMapping.uiLocationToRawLocations(
      uiSourceCode,
      lineNumber,
      columnNumber,
    );
    locations = locations.length
      ? locations
      : this.#resourceScriptMapping.uiLocationToRawLocations(
          uiSourceCode,
          lineNumber,
          columnNumber,
        );
  ...
  uiLocationRangeToRawLocationRanges(uiSourceCode, textRange) {
    ...
    ranges ??= this.#resourceScriptMapping.uiLocationRangeToRawLocationRanges(
      uiSourceCode,
      textRange,
    );
  ...
  #defaultTranslateRawFramesStep(rawFrames, translatedFrames) {
    ...
    const rawLocation = scriptId
      ? this.#debuggerModel.createRawLocationByScriptId(scriptId, lineNumber, columnNumber)
      : url
        ? this.#debuggerModel.createRawLocationByURL(url, lineNumber, columnNumber)
        : null;
    if (rawLocation) {
...
class StackTraceTopFrameLocation extends LiveLocationWithPool {
  ...
  static async createStackTraceTopFrameLocation(
    ...
    const locationsPromises = rawLocations.map((rawLocation) =>
      binding.createLiveLocation(rawLocation, location.scheduleUpdate.bind(location), locationPool),
    );
