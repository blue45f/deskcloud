...
export class PresentationSourceFrameMessageManager {
  ...
  modelAdded(model) {
    ...
    const helper =
      this.#targetToMessageHelperMap.get(target) ?? new PresentationSourceFrameMessageHelper();
...
export class PresentationConsoleMessageManager {
  ...
  consoleMessageAdded(consoleMessage) {
    ...
    if (
      !consoleMessage.isErrorOrWarning() ||
      !consoleMessage.runtimeModel() ||
      consoleMessage.source === 'violation' /* Protocol.Log.LogEntrySource.Violation */ ||
      !runtimeModel
    ) {
      return;
    }
    const level =
      consoleMessage.level === 'error' /* Protocol.Log.LogEntryLevel.Error */
...
export class PresentationSourceFrameMessageHelper {
  ...
  setCSSModel(cssModel) {
    ...
    cssModel.addEventListener(SDK.CSSModel.Events.StyleSheetAdded, (event) =>
      queueMicrotask(() => this.#styleSheetAdded(event)),
    );
  }
  async addMessage(message, source) {
    const presentation = new PresentationSourceFrameMessage(message, this.#locationPool);
    const location =
      this.#rawLocation(source) ?? this.#cssLocation(source) ?? this.#uiLocation(source);
    if (location) {
  ...
  #cssLocation(source) {
    ...
    const locations = this.#cssModel.createRawLocationsByURL(
      source.url,
      source.line,
      source.column,
    );
  ...
  #uiSourceCodeAdded(event) {
    ...
    for (const { presentation, source } of messages ?? []) {
      promises.push(
        presentation.updateLocationSource(
  ...
  #styleSheetAdded(event) {
    ...
    for (const { source, presentation } of messages ?? []) {
      if (header.containsLocation(source.line, source.column)) {
        promises.push(
          presentation.updateLocationSource(
            new SDK.CSSModel.CSSLocation(header, source.line, source.column),
          ),
        );
      }
    }
...
export class PresentationSourceFrameMessage {
  ...
  async updateLocationSource(source) {
    if (source instanceof SDK.DebuggerModel.Location) {
      ...
      if (!this.#liveLocation) {
        // Don't "downgrade" the location if a debugger or css mapping was already successful
        this.#liveLocation = new FrozenLiveLocation(
          source,
          this.#updateLocation.bind(this),
          this.#locationPool,
        );
  ...
  async #updateLocation(liveLocation) {
    ...
    this.#message.range = TextUtils.TextRange.TextRange.createFromLocation(
      uiLocation.lineNumber,
      uiLocation.columnNumber || 0,
    );
