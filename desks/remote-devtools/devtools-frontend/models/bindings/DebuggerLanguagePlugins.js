...
class SourceScopeRemoteObject extends SDK.RemoteObject.RemoteObjectImpl {
  ...
  async doGetProperties(_ownProperties, accessorPropertiesOnly, _generatePreview) {
    ...
    for (const variable of this.variables) {
      let sourceVar;
      try {
        const evalResult = await this.#plugin.evaluate(
          variable.name,
          getRawLocation(this.#callFrame),
          this.stopId,
        );
        sourceVar = evalResult
          ? await wrapRemoteObject(this.#callFrame, evalResult, this.#plugin)
...
export class DebuggerLanguagePluginManager {
  ...
  async evaluateOnCallFrame(callFrame, options) {
    ...
    try {
      const object = await plugin.evaluate(
        expression,
        location,
        this.stopIdForCallFrame(callFrame),
      );
      if (object) {
      ...
      const { exception: object, exceptionDetails } = FormattingError.makeLocal(
        callFrame,
        error.message,
      );
      return { object, exceptionDetails };
  ...
  modelRemoved(debuggerModel) {
    ...
    this.#rawModuleHandles.forEach((rawModuleHandle, rawModuleId) => {
      const scripts = rawModuleHandle.scripts.filter(
        (script) => script.debuggerModel !== debuggerModel,
      );
      if (scripts.length === 0) {
    ...
    try {
      ...
      for (const sourceLocation of sourceLocations) {
        ...
        return uiSourceCode.uiLocation(
          ...
          sourceLocation.columnNumber >= 0 ? sourceLocation.columnNumber : undefined,
        );
      }
    } catch (error) {
      Common.Console.Console.instance().error(
  ...
  uiLocationToRawLocationRanges(uiSourceCode, lineNumber, columnNumber = -1) {
    ...
    async function getLocations(rawModuleId, plugin, script) {
      const pluginLocation = {
        rawModuleId,
        sourceFileURL: uiSourceCode.url(),
        lineNumber,
        columnNumber,
      };
  ...
  async uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber) {
    const locationRanges = await this.uiLocationToRawLocationRanges(
      uiSourceCode,
      lineNumber,
      columnNumber,
    );
  ...
  async uiLocationRangeToRawLocationRanges(uiSourceCode, textRange) {
    ...
    for (const locationRanges of await Promise.all(locationRangesPromises)) {
      ...
      for (const range of locationRanges) {
        ...
        const overlap = textRange.intersection(
    ...
    for (const plugin of this.#plugins) {
      ...
      if (!rawModuleHandle) {
        const sourceFileURLsPromise = (async () => {
          ...
          if (symbolsUrl) {
            console.log(
              i18nString(UIStrings.loadingDebugSymbolsForVia, {
                PH1: plugin.name,
                PH2: url,
                PH3: symbolsUrl,
              }),
            ...
            console.log(
              i18nString(UIStrings.loadingDebugSymbolsFor, { PH1: plugin.name, PH2: url }),
            );
          }
          try {
            const code =
              !symbolsUrl && Common.ParsedURL.schemeIs(url, 'wasm:')
                ? await script.getWasmBytecode()
                : undefined;
            const addModuleResult = await plugin.addRawModule(rawModuleId, symbolsUrl, {
              url,
              code,
            });
            ...
            if (sourceFileURLs.length === 0) {
              console.warn(
                i18nString(UIStrings.loadedDebugSymbolsForButDidnt, { PH1: plugin.name, PH2: url }),
              );
            } else {
              console.log(
                i18nString(UIStrings.loadedDebugSymbolsForFound, {
                  PH1: plugin.name,
                  PH2: url,
                  PH3: sourceFileURLs.length,
                }),
              );
            }
            ...
            console.error(
              i18nString(UIStrings.failedToLoadDebugSymbolsFor, {
                PH1: plugin.name,
                PH2: url,
                PH3: error.message,
              }),
        ...
        rawModuleHandle = {
          ...
          plugin,
          scripts: [script],
          addRawModulePromise: sourceFileURLsPromise,
        };
      ...
      void rawModuleHandle.addRawModulePromise.then((sourceFileURLs) => {
        ...
        if (script.debuggerModel.scriptForId(script.scriptId) === script) {
          ...
          if (modelData && Array.isArray(sourceFileURLs)) {
            // The DebuggerModel could have disappeared meanwhile...
  ...
  debuggerResumed(event) {
    const resumedFrames = Array.from(this.callFrameByStopId.values()).filter(
      (callFrame) => callFrame.debuggerModel === event.data,
    );
  ...
  async getFunctionInfo(script, location) {
    ...
    try {
      ...
      if ('missingSymbolFiles' in functionInfo) {
        ...
        return {
          missingSymbolFiles,
          ...('frames' in functionInfo && { frames: functionInfo.frames }),
        };
      }
      return functionInfo;
    } catch (error) {
  ...
  async getMappedLines(uiSourceCode) {
    const rawModuleIds = await Promise.all(
      this.scriptsForUISourceCode(uiSourceCode).map((s) => this.rawModuleIdAndPluginForScript(s)),
    );
...
class ModelData {
  ...
  addSourceFiles(script, urls) {
    ...
    for (const url of urls) {
      ...
      if (!uiSourceCode) {
        uiSourceCode = this.project.createUISourceCode(
          url,
