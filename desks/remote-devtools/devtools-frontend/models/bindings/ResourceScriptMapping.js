...
export class ResourceScriptMapping {
  ...
  inspectedURLChanged(event) {
    for (
      let target = this.debuggerModel.target();
      target !== event.data;
  ...
  async functionBoundsAtRawLocation(rawLocation) {
    ...
    const offset = scopeTreeAndText.text.offsetFromPosition(
  ...
  addScript(script) {
    ...
    if (script.hasSourceURL) {
      ...
      url = SDK.SourceMapManager.SourceMapManager.resolveRelativeSourceURL(
        script.debuggerModel.target(),
        url,
      );
    } else {
  ...
  async divergeFromVM() {
    if (this.script) {
      ...
      this.dispatchEventToListeners(
        'DidDivergeFromVM' /* ResourceScriptFile.Events.DID_DIVERGE_FROM_VM */,
      );
    }
  }
  async mergeToVM() {
