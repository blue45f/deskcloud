  ...
  constructor(resourceMapping, resourceTreeModel) {
    ...
    this.#eventListeners = [
      resourceTreeModel.addEventListener(
        SDK.ResourceTreeModel.Events.ResourceAdded,
        this.resourceAdded,
        this,
      ),
      resourceTreeModel.addEventListener(
      ...
      resourceTreeModel.addEventListener(
        SDK.ResourceTreeModel.Events.FrameDetached,
        this.frameDetached,
        this,
      ),
...
class Binding {
  ...
  inlineScripts() {
    ...
    return debuggerModel
      .scripts()
      .filter((script) => script.embedderName() === this.#uiSourceCode.url());
  }
  ...
  async innerStyleSheetChanged(content) {
    ...
    for (const data of this.#edits) {
      ...
      const startLocation =
        styleSheetRangeMap.get(stylesheet) ?? computeStyleSheetRange(stylesheet);
      const oldRange = edit.oldRange.relativeFrom(
        startLocation.startLine,
        startLocation.startColumn,
      );
      const newRange = edit.newRange.relativeFrom(
        startLocation.startLine,
        startLocation.startColumn,
      );
