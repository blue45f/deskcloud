  ...
  uiLocationToRawLocations(uiLocation) {
    ...
    for (const sourceMap of binding.getReferringSourceMaps()) {
      ...
      if (header) {
        locations.push(
          ...entries.map(
            (entry) => new SDK.CSSModel.CSSLocation(header, entry.lineNumber, entry.columnNumber),
          ),
        );
      }
    }
...
class Binding {
  ...
  recreateUISourceCodeIfNeeded(frameId) {
    ...
    const mimeType =
      Common.ResourceType.ResourceType.mimeFromURL(this.#url) || contentType.canonicalMimeType();
    const metadata =
