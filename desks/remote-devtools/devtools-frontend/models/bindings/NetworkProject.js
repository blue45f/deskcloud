...
export class NetworkProject {
  ...
  static addFrameAttribution(uiSourceCode, frameId) {
    ...
    NetworkProjectManager.instance().dispatchEventToListeners(
      'FrameAttributionAdded' /* Events.FRAME_ATTRIBUTION_ADDED */,
      data,
    );
  }
  static removeFrameAttribution(uiSourceCode, frameId) {
    ...
    console.assert(
      Boolean(attributionInfo),
      'Failed to remove frame attribution for url: ' + uiSourceCode.url(),
    );
    if (!attributionInfo) {
      return;
    }
    ...
    NetworkProjectManager.instance().dispatchEventToListeners(
      'FrameAttributionRemoved' /* Events.FRAME_ATTRIBUTION_REMOVED */,
      data,
    );
  }
  ...
  static framesForUISourceCode(uiSourceCode) {
    ...
    const frames = Array.from(attribution.keys()).map((frameId) =>
      resourceTreeModel.frameForId(frameId),
    );
