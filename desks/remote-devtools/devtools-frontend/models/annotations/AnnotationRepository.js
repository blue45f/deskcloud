...
export class AnnotationRepository {
  ...
  addElementsAnnotation(label, anchor, anchorToString) {
    ...
    this.#events.dispatchEventToListeners(
      'AnnotationAdded' /* Events.ANNOTATION_ADDED */,
      annotationData,
    );
  }
  addNetworkRequestAnnotation(label, anchor, anchorToString) {
    ...
    if (
      this.#updateExistingAnnotationLabel(
        label,
        AnnotationType.NETWORK_REQUEST_SUBPANEL_HEADERS,
        anchor,
      )
    ) {
      return;
  ...
  deleteAllAnnotations() {
    this.#annotationData = [];
    this.#events.dispatchEventToListeners(
      'AllAnnotationsDeleted' /* Events.ALL_ANNOTATIONS_DELETED */,
    );
    // eslint-disable-next-line no-console
  ...
  deleteAnnotation(id) {
    ...
    this.#events.dispatchEventToListeners('AnnotationDeleted' /* Events.ANNOTATION_DELETED */, {
      id,
    });
    // eslint-disable-next-line no-console
