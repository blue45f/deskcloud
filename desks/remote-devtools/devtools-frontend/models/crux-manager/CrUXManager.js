...
export class CrUXManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    const storageTypeForConsent = useSessionStorage
    ...
    try {
      ...
      for (const pageScope of pageScopeList) {
        for (const deviceScope of DEVICE_SCOPE_LIST) {
          const promise = this.#getScopedData(normalizedUrl, pageScope, deviceScope).then(
            (response) => {
  ...
  async #getFieldDataForCurrentPage() {
    ...
    const normalizedUrl = this.#configSetting.get().overrideEnabled
  ...
  async refresh() {
    ...
    this.dispatchEventToListeners(
      'field-data-changed' /* Events.FIELD_DATA_CHANGED */,
      this.#pageResult,
    );
  }
  ...
  async #getScopedData(normalizedUrl, pageScope, deviceScope) {
    ...
    try {
      ...
      const result =
        pageScope === 'origin'
          ? await this.#makeRequest({ origin, metrics, formFactor })
