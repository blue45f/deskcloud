...
export class Model extends EventTarget {
  ...
  async parse(traceEvents, config = {}) {
    ...
    const onTraceUpdate = (event) => {
      const { data } = event;
      this.dispatchEvent(
        new ModelUpdateEvent({
          type: 'PROGRESS_UPDATE' /* ModelUpdateType.PROGRESS_UPDATE */,
          data,
        }),
      );
    };
    ...
    try {
      ...
      this.dispatchEvent(
        new ModelUpdateEvent({ type: 'COMPLETE' /* ModelUpdateType.COMPLETE */, data: 'done' }),
      );
    }
  }
  #storeAndCreateParsedTraceFile(
    ...
    if (origin) {
      const nextSequenceForDomain = Platform.MapUtilities.getWithDefault(
        this.#nextNumberByDomain,
        origin,
        () => 1,
      );
