...
class PageDependencyGraph {
  ...
  static linkNetworkNodes(rootNode, networkNodeOutput) {
    networkNodeOutput.nodes.forEach((node) => {
      ...
      const canDependOnInitiator =
        !directInitiatorNode.isDependentOn(node) && node.canDependOn(directInitiatorNode);
  ...
  static linkCPUNodes(rootNode, networkNodeOutput, cpuNodes) {
    const linkableResourceTypes = new Set(['XHR', 'Fetch', 'Script']);
  ...
  static createGraph(mainThreadEvents, networkRequests, url) {
    ...
    const mainDocumentRequest = Core.NetworkAnalyzer.findLastDocumentForUrl(
      networkRequests,
      mainDocumentUrl,
    );
