...
class Simulator {
  static createSimulator(settings) {
    ...
    if (precomputedLanternData) {
      options.additionalRttByOrigin = new Map(
        Object.entries(precomputedLanternData.additionalRttByOrigin),
      );
      options.serverResponseTimeByOrigin = new Map(
  ...
  markNodeAsReadyToStart(node, queuedTime) {
    ...
    const insertionIndex =
      firstNodeIndexWithGreaterStartPosition === -1
        ? this.cachedNodeListByStartPosition.length
