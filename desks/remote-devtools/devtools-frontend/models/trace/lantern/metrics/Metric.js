...
class Metric {
  ...
  static compute(data, extras) {
    ...
    const optimisticEstimate = this.getEstimateFromSimulation(optimisticSimulation, {
      ...extras,
      optimistic: true,
    });
    const pessimisticEstimate = this.getEstimateFromSimulation(pessimisticSimulation, {
      ...extras,
      optimistic: false,
    });
    ...
    const interceptMultiplier =
      coefficients.intercept > 0 ? Math.min(1, optimisticEstimate.timeInMs / 1000) : 1;
    const timing =
