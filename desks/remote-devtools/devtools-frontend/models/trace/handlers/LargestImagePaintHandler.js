...
export function handleEvent(event) {
  ...
  const imagePaintsByNodeId = Platform.MapUtilities.getWithDefault(
...
export async function finalize() {
  ...
  for (const [navigationId, navigation] of navigationsByNavigationId) {
    const lcpMetric = metricScoresByFrameId
      .get(navigation.args.frame)
      ?.get(navigation)
      ?.get('LCP' /* MetricName.LCP */);
    ...
    for (const request of requests) {
      ...
      if (
        request.args.data.url === lcpUrl ||
        request.args.data.redirects.some((r) => r.url === lcpUrl)
      ) {
        lcpRequest = request;
        break;
      }
