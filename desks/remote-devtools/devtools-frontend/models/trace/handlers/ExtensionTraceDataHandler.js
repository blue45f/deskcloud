...
export function extractConsoleAPIExtensionEntries() {
  ...
  for (const currentTimeStamp of consoleTimeStamps) {
    ...
    const timeStampName = String(
      currentTimeStamp.args.data.name ?? currentTimeStamp.args.data.message,
    );
    timeStampByName.set(timeStampName, currentTimeStamp);
    const { devtoolsObj: extensionData, userDetail } =
      extensionDataInConsoleTimeStamp(currentTimeStamp);
    ...
    const startTimeStamp =
      typeof start === 'number'
        ? Types.Timing.Micro(start)
        : timeStampByName.get(String(start))?.ts;
    const endTimeStamp =
      typeof end === 'number' ? Types.Timing.Micro(end) : timeStampByName.get(String(end))?.ts;
    ...
    if (extensionData) {
      ...
      const extensionEntry = Helpers.SyntheticEvents.SyntheticEventsManager.registerSyntheticEvent(
        unregisteredExtensionEntry,
      );
...
export function extractPerformanceAPIExtensionEntries(timings) {
  for (const timing of timings) {
    ...
    const extensionSyntheticEntry = {
      name: timing.name,
      ph: Types.Extensions.isExtensionPayloadMarker(devtoolsObj)
...
export function extensionDataInPerformanceTiming(timing) {
  const timingDetail = Types.Events.isPerformanceMark(timing)
    ? timing.args.data?.detail
...
export function extensionDataInConsoleTimeStamp(timeStamp) {
  ...
  const devtoolsObj = {
    ...
    trackGroup:
      timeStamp.args.data.trackGroup !== undefined
        ? String(timeStamp.args.data.trackGroup)
        : undefined,
  };
  return { devtoolsObj, userDetail };
}
