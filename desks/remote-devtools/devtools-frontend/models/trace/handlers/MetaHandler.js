...
function updateRendererProcessByFrame(event, frame) {
  const framesInProcessById = Platform.MapUtilities.getWithDefault(
    framesByProcessId,
    frame.processId,
    () => new Map(),
  );
  ...
  const rendererProcessInfo = Platform.MapUtilities.getWithDefault(
    rendererProcessInFrame,
    frame.processId,
    () => {
      return [];
    },
...
export function handleEvent(event) {
  ...
  if (
    event.ts !== 0 &&
    !event.name.endsWith('::UMA') &&
    eventPhasesOfInterestForTraceBounds.has(event.ph)
  ) {
  ...
  if (
    Types.Events.isProcessName(event) &&
  ...
  if (Types.Events.isThreadName(event)) {
    const threads = Platform.MapUtilities.getWithDefault(
      threadsInProcess,
      event.pid,
      () => new Map(),
    );
...
export async function finalize(options) {
  ...
  for (const [, processWindows] of rendererProcessesByFrameId) {
    ...
    for (let i = 0; i < processWindowValues.length; i++) {
      ...
      if (!nextWindow) {
        ...
        currentWindow.globalThis.range = Types.Timing.Micro(
          traceBounds.max - currentWindow.globalThis.min,
        );
      } else {
        currentWindow.globalThis.max = Types.Timing.Micro(nextWindow.globalThis.min - 1);
        currentWindow.globalThis.range = Types.Timing.Micro(
          currentWindow.globalThis.max - currentWindow.globalThis.min,
        );
      }
    }
  }
  ...
  if (firstMainFrameNav) {
    const navigationIsWithinThreshold =
      firstMainFrameNav.ts - traceBounds.min < firstNavTimeThreshold;
    if (
      firstMainFrameNav.args.data?.isOutermostMainFrame &&
