...
function processForcedReflowWarning(event) {
  ...
  accomodateEventInStack(
    event,
    jsInvokeStack,
    /* pushEventToStack */ Types.Events.isJSInvocationEvent(event),
  );
  if (jsInvokeStack.length) {
    // Current event falls inside a JS call.
    if (
      event.name === 'Layout' /* Types.Events.Name.LAYOUT */ ||
      event.name === 'UpdateLayoutTree' /* Types.Events.Name.RECALC_STYLE */
    ) {
