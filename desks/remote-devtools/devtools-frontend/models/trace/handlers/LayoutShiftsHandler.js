...
function findScreenshots(timestamp) {
  ...
  if (data.legacySyntheticScreenshots) {
    const before = Helpers.Trace.findPreviousEventBeforeTimestamp(
      data.legacySyntheticScreenshots,
      timestamp,
    );
    const after = before
      ? data.legacySyntheticScreenshots[data.legacySyntheticScreenshots.indexOf(before) + 1]
      : null;
    return { before, after };
  }
  // No screenshots
...
function buildScoreRecords() {
  ...
  for (const cluster of clusters) {
    ...
    if (cluster.events[0].args.data) {
      scoreRecords.push({
        ts: cluster.clusterWindow.min,
        score: cluster.events[0].args.data.weighted_score_delta,
      });
    }
...
async function buildLayoutShiftsClusters() {
  ...
  for (const event of layoutShiftEvents) {
    ...
    // note it.
    const currentShiftNavigation = Platform.ArrayUtilities.nearestIndexFromEnd(
      navigations,
      (nav) => nav.ts < event.ts,
    );
    const hasNavigated =
      lastShiftNavigation !== currentShiftNavigation && currentShiftNavigation !== null;
    // If any of the above criteria are met or if we don't have any cluster yet we should
    // start a new one.
    if (
      clusterDurationExceeded ||
      maxTimeDeltaSinceLastShiftExceeded ||
      hasNavigated ||
      !clusters.length
    ) {
      ...
      const endTimeByMaxSessionDuration = clusterDurationExceeded
        ? firstShiftTime + MAX_CLUSTER_DURATION
        : Infinity;
      ...
      const endTimeByMaxShiftGap = maxTimeDeltaSinceLastShiftExceeded
        ? lastShiftTime + MAX_SHIFT_TIME_DELTA
        : Infinity;
      ...
      const previousClusterEndTime = Math.min(
        endTimeByMaxSessionDuration,
        endTimeByMaxShiftGap,
        endTimeByNavigation,
      );
      ...
      if (clusters.length > 0) {
        ...
        updateTraceWindowMax(
          currentCluster.clusterWindow,
          Types.Timing.Micro(previousClusterEndTime),
        );
      }
    ...
    const currentCluster = clusters[clusters.length - 1];
    const timeFromNavigation =
      currentShiftNavigation !== null
        ? Types.Timing.Micro(event.ts - navigations[currentShiftNavigation].ts)
        : undefined;
    currentCluster.clusterCumulativeScore += event.args.data
  ...
  for (const cluster of clusters) {
    ...
    if (cluster === clusters[clusters.length - 1]) {
      ...
      const nextNavigationIndex = Platform.ArrayUtilities.nearestIndexFromBeginning(
        navigations,
        (nav) => nav.ts > cluster.clusterWindow.max,
      );
      const nextNavigationTime = nextNavigationIndex
        ? navigations[nextNavigationIndex].ts
        : Infinity;
      const clusterEnd = Math.min(
        clusterEndByMaxDuration,
        clusterEndByMaxGap,
        traceBounds.max,
        nextNavigationTime,
      );
    ...
    for (const shift of cluster.events) {
      ...
      if (weightedScore < 0.1 /* LayoutShiftsThreshold.NEEDS_IMPROVEMENT */) {
        ...
        weightedScore >= 0.1 /* LayoutShiftsThreshold.NEEDS_IMPROVEMENT */ &&
        weightedScore < 0.25 /* LayoutShiftsThreshold.BAD */
      ) {
    ...
    const lastShiftTimings = Helpers.Timing.eventTimingsMicroSeconds(
      cluster.events[cluster.events.length - 1],
    );
    // Add MAX_SHIFT_TIME_DELTA, the section gap after the last layout shift. This marks the end of the cluster.
    cluster.dur = Types.Timing.Micro(
      lastShiftTimings.endTime - cluster.events[0].ts + MAX_SHIFT_TIME_DELTA,
    );
    ...
    if (cluster.navigationId) {
      const clustersForId = Platform.MapUtilities.getWithDefault(
        clustersByNavigationId,
        cluster.navigationId,
        () => {
          return [];
        },
      );
