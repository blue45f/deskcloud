...
function parseCPUProfileData(parseOptions) {
  ...
  for (const [processId, profiles] of preprocessedData) {
    ...
    for (const [profileId, preProcessedData] of profiles) {
      ...
      const listForThread = Platform.MapUtilities.getWithDefault(
        profilesByThread,
        threadId,
        () => [],
      );
    ...
    for (const [threadId, candidates] of profilesByThread) {
      ...
      const dataByThread = Platform.MapUtilities.getWithDefault(
        profilesInProcess,
        processId,
        () => new Map(),
      );
      ...
      function buildProfileCallsForCPUProfile() {
        ...
        function openFrameCallback(depth, node, sampleIndex, timeStampMilliseconds) {
          ...
          const profileCall = Helpers.Trace.makeProfileCall(
            node,
            selectedProfileId,
            sampleIndex,
            ts,
        ...
        function closeFrameCallback(
          ...
          const profileCall =
            profileCallIndex !== undefined && finalizedData.profileCalls[profileCallIndex];
          if (!profileCall) {
            return;
          }
...
export function handleEvent(event) {
  ...
  if (Types.Events.isProfileChunk(event)) {
    ...
    for (const n of nodesAndSamples?.nodes || []) {
      const lineNumber =
        typeof n.callFrame.lineNumber === 'undefined' ? -1 : n.callFrame.lineNumber;
      const columnNumber =
        typeof n.callFrame.columnNumber === 'undefined' ? -1 : n.callFrame.columnNumber;
    ...
    if (
      cdpProfile.samples &&
      cdpProfile.timeDeltas &&
      cdpProfile.samples.length !== cdpProfile.timeDeltas.length
    ) {
...
function getOrCreatePreProcessedData(processId, profileId) {
  const profileById = Platform.MapUtilities.getWithDefault(
    preprocessedData,
    processId,
    () => new Map(),
  );
