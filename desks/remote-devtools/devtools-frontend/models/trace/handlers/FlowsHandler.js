...
function processNonFlowEvent(event) {
  const flowDataForEvent = boundFlowData
    .get(event.ts)
    ?.get(event.pid)
    ?.get(event.tid)
    ?.get(event.cat);
...
function processFlowEvent(flowPhaseEvent) {
  ...
  switch (flowPhaseEvent.ph) {
    case 's' /* Types.Events.Phase.FLOW_START */: {
      const flowMetadata = {
        flowId: flowPhaseEvent.id,
        times: new Map([[flowPhaseEvent.ts, undefined]]),
      };
...
function addFlowIdToEventBinding(event, flowId) {
  ...
  const flowData = Platform.MapUtilities.getWithDefault(flowsByCat, event.cat, () => ({
    flows: new Set(),
    bindingParsed: false,
  }));
