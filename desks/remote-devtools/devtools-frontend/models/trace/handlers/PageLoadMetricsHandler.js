...
function storePageLoadMetricAgainstNavigationId(navigation, event) {
  ...
  if (Types.Events.isInteractiveTime(event)) {
    ...
    const tbtValue = Helpers.Timing.milliToMicro(
      Types.Timing.Milli(event.args.args.total_blocking_time_ms),
    );
    const tbt = {
      event,
  ...
  if (Types.Events.isAnyLargestContentfulPaintCandidate(event)) {
    ...
    const metricsByNavigation = Platform.MapUtilities.getWithDefault(
      metricScoresByFrameId,
      frameId,
      () => new Map(),
    );
    const metrics = Platform.MapUtilities.getWithDefault(
      metricsByNavigation,
      navigation,
      () => new Map(),
    );
...
export function getFrameIdForPageLoadEvent(event) {
  if (
    Types.Events.isFirstContentfulPaint(event) ||
    Types.Events.isInteractiveTime(event) ||
    Types.Events.isAnyLargestContentfulPaintCandidate(event) ||
    Types.Events.isNavigationStart(event) ||
    Types.Events.isSoftNavigationStart(event) ||
    Types.Events.isLayoutShift(event) ||
...
function getNavigationForPageLoadEvent(event) {
  if (
    ...
    if (
      ...
      if (!navigationId) {
        throw new Error(
          `Trace event unexpectedly had no navigation ID: ${JSON.stringify(event, null, 2)}`,
        );
      }
  ...
  if (
    Types.Events.isMarkDOMContent(event) ||
    Types.Events.isInteractiveTime(event) ||
    Types.Events.isLayoutShift(event) ||
    Types.Events.isMarkLoad(event)
  ) {
    const frameId = getFrameIdForPageLoadEvent(event);
...
export async function finalize() {
  ...
  for (const metaCharsetCheckEvent of metaCharsetCheckEventsArray) {
    ...
    const navigation = Helpers.Trace.getNavigationForTraceEvent(
      metaCharsetCheckEvent,
      frameId,
    ...
    const eventsForNavigation = Platform.MapUtilities.getWithDefault(
  ...
  const allEventsButLCP = pageLoadEventsArray.filter(
    (event) => !Types.Events.isAnyLargestContentfulPaintCandidate(event),
  );
  const markerEvents = [...allFinalLCPEvents, ...allEventsButLCP].filter(
    Types.Events.isMarkerEvent,
  );
  // Filter by main frame and sort.
  allMarkerEvents = markerEvents
    .filter((event) => getFrameIdForPageLoadEvent(event) === mainFrame)
    .sort((a, b) => a.ts - b.ts);
}
