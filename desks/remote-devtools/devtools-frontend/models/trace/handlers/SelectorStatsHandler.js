...
export function handleEvent(event) {
  if (Types.Events.isStyleRecalcInvalidationTracking(event)) {
     ...
     */
    if (
      event.args.data.subtree &&
      event.args.data.reason ===
        'Related style rule' /* Types.Events.StyleRecalcInvalidationReason.RELATED_STYLE_RULE */ &&
      lastInvalidatedNode &&
      event.args.data.nodeId === lastInvalidatedNode.backendNodeId
    ) {
  ...
  if (Types.Events.isStyleInvalidatorInvalidationTracking(event)) {
    ...
    if (selectorList.length > 0) {
      lastInvalidatedNode = {
        ...
        tts: event.tts,
        subtree: false,
        lastRecalcStyleEventTs: lastRecalcStyleEvent
          ? lastRecalcStyleEvent.ts
