...
export function handleEvent(event) {
  ...
  if (Types.Events.isPreloadRenderBlockingStatusChangeEvent(event)) {
    storeTraceEventWithRequestId(event.args.data.requestId, 'preloadRenderBlockingStatusChange', [
      event,
    ]);
  }
  if (
    Types.Events.isWebSocketCreate(event) ||
    Types.Events.isWebSocketInfo(event) ||
...
export async function finalize() {
  ...
  for (const [requestId, request] of requestMap.entries()) {
    ...
    if (isLightrider && request.resourceFinish && request.receiveResponse?.args.data.headers) {
      const lrSizeHeader = request.receiveResponse.args.data.headers.find(
        (h) => h.name === 'X-TotalFetchedSize',
      );
      if (lrSizeHeader) {
    ...
    if (isLightrider && request.receiveResponse?.args.data.headers) {
      ...
      const TCPMsHeader = request.receiveResponse.args.data.headers.find(
        (h) => h.name === 'X-TCPMs',
      );
      ...
      // Lightrider sets.
      const ResponseMsHeader = request.receiveResponse.args.data.headers.find(
        (h) => h.name === 'X-ResponseMs',
      );
      if (ResponseMsHeader) {
    ...
    const allowedProtocols = ['blob:', 'file:', 'filesystem:', 'http:', 'https:'];
    ...
    const startTime = request.willSendRequests?.length
    ...
    const networkDuration = Types.Timing.Micro(
      timing ? (finishTime || endRedirectTime) - endRedirectTime : 0,
    );
    // Processing duration
    // =======================
    ...
    const queueingFromTraceData = timing
      ? timing.requestTime * SECONDS_TO_MICROSECONDS - endRedirectTime
      : 0;
    const queueing = Types.Timing.Micro(
      Platform.NumberUtilities.clamp(queueingFromTraceData, 0, Number.MAX_VALUE),
    );
    // Stalled
    // =======================
    ...
    const sendStartTime = timing
      ? Types.Timing.Micro(
          timing.requestTime * SECONDS_TO_MICROSECONDS +
            timing.sendStart * MILLISECONDS_TO_MICROSECONDS,
        )
      : startTime;
    // Waiting
    ...
    const waiting = timing
      ? Types.Timing.Micro(
          (timing.receiveHeadersEnd - timing.sendEnd) * MILLISECONDS_TO_MICROSECONDS,
        )
      : Types.Timing.Micro(0);
    ...
    const downloadStart = timing
      ? Types.Timing.Micro(
          timing.requestTime * SECONDS_TO_MICROSECONDS +
            timing.receiveHeadersEnd * MILLISECONDS_TO_MICROSECONDS,
        )
      : startTime;
    const download = timing
      ? Types.Timing.Micro((finishTime || downloadStart) - downloadStart)
      : request.receiveResponse
    ...
    const dnsLookup = timing
      ? Types.Timing.Micro((timing.dnsEnd - timing.dnsStart) * MILLISECONDS_TO_MICROSECONDS)
      : Types.Timing.Micro(0);
    const ssl = timing
      ? Types.Timing.Micro((timing.sslEnd - timing.sslStart) * MILLISECONDS_TO_MICROSECONDS)
      : Types.Timing.Micro(0);
    ...
    const { encodedDataLength, decodedBodyLength } = request.resourceFinish
      ? request.resourceFinish.args.data
    ...
    const isRenderBlocking =
      preloadRenderBlockingStatusChange ?? sendRequestIsRenderBlocking ?? 'non_blocking';
    ...
    const networkEvent = Helpers.SyntheticEvents.SyntheticEventsManager.registerSyntheticEvent({
      ...
      args: {
        data: {
          ...
          resourceType:
            finalSendRequest.args.data.resourceType ??
            'Other' /* Protocol.Network.ResourceType.Other */,
    ...
    const initiatorUrl =
      networkEvent.args.data.initiator?.url ||
...
function finalizeWebSocketData() {
  ...
  webSocketData.forEach((data) => {
    ...
    data.syntheticConnection = createSyntheticWebSocketConnection(
      startEvent,
      endEvent,
      data.events[0],
    );
