...
export function removeNestedInteractionsAndSetProcessingTime(interactions) {
  ...
  function storeEventIfEarliestForCategoryAndEndTime(interaction) {
    ...
    if (interaction.ts < earliestCurrentEvent.ts) {
      ...
      // about.
      const currentProcessingDuration =
        earliestCurrentEvent.processingEnd - earliestCurrentEvent.processingStart;
  ...
  const keptEvents = Object.values(earliestEventForEndTimePerCategory).flatMap((eventsByEndTime) =>
    Array.from(eventsByEndTime.values()),
  );
...
export async function finalize() {
  ...
  for (const event of beginAndEndEvents) {
    if (Types.Events.isEventTimingStart(event)) {
      ...
      const { type, interactionId, timeStamp, processingStart, processingEnd } =
        beginEvent.args.data;
      ...
      const processingStartRelativeToTraceTime = Types.Timing.Micro(
        Helpers.Timing.milliToMicro(processingStart) -
          Helpers.Timing.milliToMicro(timeStamp) +
          beginEvent.ts,
      );
      const processingEndRelativeToTraceTime = Types.Timing.Micro(
        Helpers.Timing.milliToMicro(processingEnd) -
          Helpers.Timing.milliToMicro(timeStamp) +
          beginEvent.ts,
      );
  ...
  interactionEventsWithNoNesting.push(
    ...removeNestedInteractionsAndSetProcessingTime(interactionEvents),
  );
...
export function data() {
  return {
    ...
    interactionsOverThreshold: new Set(
      interactionEvents.filter((event) => {
