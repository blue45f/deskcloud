...
export function handleEvent(event) {
  const getOrMakeScript = (isolate, scriptIdAsNumber) => {
    ...
    return Platform.MapUtilities.getWithDefault(scriptById, key, () => ({
      isolate,
      scriptId,
      frame: '',
      ts: event.ts,
...
function findCachedRawSourceMap(script, options) {
  ...
  if (!isDataUrl) {
    const cachedSourceMap = options.metadata.sourceMaps.find(
      (m) => m.sourceMapUrl === script.sourceMapUrl,
    );
    if (cachedSourceMap) {
      return cachedSourceMap.sourceMap;
    }
...
export async function finalize(options) {
  ...
  for (const script of scriptById.values()) {
    ...
    if (script.sourceUrl) {
      sourceUrl =
        Common.ParsedURL.ParsedURL.completeURL(frameUrl, script.sourceUrl) ?? script.sourceUrl;
    }
    let sourceMapUrl;
    if (script.sourceMapUrl) {
