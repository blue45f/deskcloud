...
export async function finalize() {
  const pipelineReporterEvents =
    Helpers.Trace.createMatchedSortedSyntheticEvents(unpairedAsyncEvents);
  frameSequenceToTs = Object.fromEntries(
    pipelineReporterEvents.map((evt) => {
      const args = evt.args.data.beginEvent.args;
      const frameReporter =
        'frame_reporter' in args ? args.frame_reporter : args.chrome_frame_reporter;
