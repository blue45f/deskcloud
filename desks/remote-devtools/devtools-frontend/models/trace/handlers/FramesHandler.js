...
function isFrameEvent(event) {
  return (
    Types.Events.isSetLayerId(event) ||
    Types.Events.isBeginFrame(event) ||
    Types.Events.isDroppedFrame(event) ||
    Types.Events.isRequestMainThreadFrame(event) ||
    Types.Events.isBeginMainThreadFrame(event) ||
    ...
    Types.Events.isCommit(event) ||
    Types.Events.isCompositeLayers(event) ||
    Types.Events.isActivateLayerTree(event) ||
    Types.Events.isDrawFrame(event)
  );
}
function entryIsTopLevel(entry) {
  const devtoolsTimelineCategory = 'disabled-by-default-devtools.timeline';
  return (
    entry.name === 'RunTask' /* Types.Events.Name.RUN_TASK */ &&
    entry.cat.includes(devtoolsTimelineCategory)
  );
}
...
export function handleEvent(event) {
  ...
  if (
    isFrameEvent(event) ||
    Types.Events.isLayerTreeHostImplSnapshot(event) ||
    entryIsTopLevel(event) ||
    MAIN_FRAME_MARKERS.has(event.name) ||
    Types.Events.isPaint(event)
  ) {
...
export class TimelineFrameModel {
  ...
  constructor(allEvents, rendererData, auctionWorkletsData, metaData, layerTreeData) {
    ...
    // threads.)
    const mainThreads = Threads.threadsInRenderer(rendererData, auctionWorkletsData).filter(
      (thread) => {
        return (
          thread.type === 'MAIN_THREAD' /* Threads.ThreadType.MAIN_THREAD */ &&
          thread.processIsOnMainFrame
        );
      },
    );
  ...
  #handleDrawFrame(startTime, seqId) {
    ...
    if (this.#mainFrameCommitted || !this.#mainFrameRequested) {
      if (this.#lastNeedsBeginFrame) {
        const idleTimeEnd = this.#framePendingActivation
          ? this.#framePendingActivation.triggerTime
  ...
  #startFrame(startTime, seqId) {
    ...
    this.#lastFrame = new TimelineFrame(
      seqId,
      startTime,
      Types.Timing.Micro(startTime - metaHandlerData().traceBounds.min),
    );
  }
  ...
  #addTraceEvent(event, mainFrameId) {
    if (Types.Events.isSetLayerId(event) && event.args.data.frame === mainFrameId) {
      ...
      Types.Events.isLayerTreeHostImplSnapshot(event) &&
      Number(event.id) === this.#layerTreeId
    ) {
  ...
  #processCompositorEvents(entry) {
    ...
    if (Types.Events.isBeginFrame(entry)) {
      ...
      this.#handleNeedFrameChanged(
        entry.ts,
        entry.args['data'] && Boolean(entry.args['data']['needsBeginFrame']),
      );
    } else if (Types.Events.isDroppedFrame(entry)) {
      this.#handleDroppedFrame(
        entry.ts,
        entry.args['frameSeqId'],
        Boolean(entry.args['hasPartialUpdate']),
      );
    }
  }
  #addMainThreadTraceEvent(entry) {
    ...
    if (
      (Types.Events.isCompositeLayers(entry) || Types.Events.isCommit(entry)) &&
...
export function framesWithinWindow(frames, startTime, endTime) {
  const firstFrame = Platform.ArrayUtilities.lowerBound(
    frames,
    startTime || 0,
    (time, frame) => time - frame.endTime,
  );
  const lastFrame = Platform.ArrayUtilities.lowerBound(
    frames,
    endTime || Infinity,
    (time, frame) => time - frame.startTime,
  );
