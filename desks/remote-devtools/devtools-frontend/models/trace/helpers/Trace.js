...
export function getNavigationForTraceEvent(event, eventFrameId, navigationsByFrameId) {
  ...
  const eventNavigationIndex = Platform.ArrayUtilities.nearestIndexFromEnd(
    navigations,
    (navigation) => navigation.ts <= event.ts,
  );
...
function createSortedSyntheticEvents(matchedPairs) {
  ...
  for (const eventsTriplet of matchedPairs) {
    ...
    function eventsArePairable(data) {
      const instantEventsMatch = data.instantEvents
        ? data.instantEvents.some((e) => id === getSyntheticId(e))
        : false;
...
export function frameIDForEvent(event) {
  ...
  if (
    event.args &&
    'beginData' in event.args &&
    typeof event.args.beginData === 'object' &&
    event.args.beginData !== null &&
    'frame' in event.args.beginData &&
...
export function isTopLevelEvent(event) {
  return (
    event.cat.includes(DevToolsTimelineEventCategory) &&
    event.name === 'RunTask' /* Types.Events.Name.RUN_TASK */
  );
}
...
export function forEachEvent(events, config) {
  ...
  for (let i = startEventIndex; i < events.length; i++) {
    ...
    let lastEventEndTime = lastEventOnStack
      ? eventTimingsMicroSeconds(lastEventOnStack).endTime
      : null;
    while (
      lastEventOnStack &&
      lastEventEndTime &&
      lastEventEndTime <= currentEventTimings.startTime
    ) {
      stack.pop();
      config.onEndEvent(lastEventOnStack);
      lastEventOnStack = stack.at(-1);
      lastEventEndTime = lastEventOnStack
        ? eventTimingsMicroSeconds(lastEventOnStack).endTime
        : null;
    }
...
export function isMatchingCallFrame(eventFrame, nodeFrame) {
  return (
    eventFrame.columnNumber === nodeFrame.columnNumber &&
    eventFrame.lineNumber === nodeFrame.lineNumber &&
    String(eventFrame.scriptId) === nodeFrame.scriptId &&
    eventFrame.url === nodeFrame.url &&
