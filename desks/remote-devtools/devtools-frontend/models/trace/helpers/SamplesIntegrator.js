...
export class SamplesIntegrator {
  ...
  #onTraceEventStart(event) {
    ...
    if (
      event.name === 'RunMicrotasks' /* Types.Events.Name.RUN_MICROTASKS */ ||
  ...
  #getStackForSampleTraceId(traceId, timestamp) {
    ...
    const maybeCallForTraceId =
      node &&
      makeProfileCall(node, this.#profileId, -1, timestamp, this.#processId, this.#threadId);
  ...
  #extractStackTrace(event) {
    ...
    for (i = this.#lockedJsStackDepth.at(-1) || 0; i < minFrames; ++i) {
      ...
      this.#currentJSStack[i].dur = Types.Timing.Micro(
        Math.max(this.#currentJSStack[i].dur || 0, endTime - this.#currentJSStack[i].ts),
      );
    }
    ...
    for (; i < stackTrace.length; ++i) {
      ...
      if (
        call.nodeId === this.#profileModel.programNode?.id ||
        call.nodeId === this.#profileModel.root?.id ||
        call.nodeId === this.#profileModel.idleNode?.id ||
        call.nodeId === this.#profileModel.gcNode?.id
      ) {
  ...
  #truncateJSStack(depth, time) {
    ...
    for (let k = 0; k < this.#currentJSStack.length; ++k) {
      this.#currentJSStack[k].dur = Types.Timing.Micro(
        Math.max(time - this.#currentJSStack[k].ts, 0),
      );
    }
