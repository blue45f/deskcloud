...
export class EntityMapper {
  ...
  #findFirstPartyEntity() {
    ...
    const nav = Array.from(this.#parsedTrace.data.Meta.navigationsByNavigationId.values()).sort(
      (a, b) => a.ts - b.ts,
    )[0];
    const firstPartyUrl =
      nav?.args.data?.documentLoaderURL ?? this.#parsedTrace.data.Meta.mainFrameURL;
    if (!firstPartyUrl) {
      return null;
    }
  ...
  updateSourceMapEntities(callFrame, sourceURL) {
    ...
    const currentEntityEvents =
      (currentEntity && this.#entityMappings.eventsByEntity.get(currentEntity)) ?? [];
