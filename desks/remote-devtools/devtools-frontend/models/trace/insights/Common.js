...
function getPageResult(cruxFieldData, url, origin, scope = null) {
  return cruxFieldData.find((result) => {
    const key = scope
      ? result[`${scope.pageScope}-${scope.deviceScope}`]?.record.key
...
export function getFieldMetricsForInsightSet(insightSet, metadata, scope = null) {
  ...
  const pageResult = getPageResult(
    cruxFieldData,
    insightSet.url.href,
    insightSet.url.origin,
    scope,
  );
  if (!pageResult) {
    return null;
  ...
  return {
    ...
    lcpBreakdown: {
      ttfb: getMetricTimingResult(
        pageResult,
        'largest_contentful_paint_image_time_to_first_byte',
        scope,
      ),
      loadDelay: getMetricTimingResult(
        pageResult,
        'largest_contentful_paint_image_resource_load_delay',
        scope,
      ),
      loadDuration: getMetricTimingResult(
        pageResult,
        'largest_contentful_paint_image_resource_load_duration',
        scope,
      ),
      renderDelay: getMetricTimingResult(
        pageResult,
...
export function calculateMetricWeightsForSorting(insightSet, metadata) {
  ...
  const fieldLcpScore =
    fieldLcp !== null ? evaluateLCPMetricScore(Helpers.Timing.microToMilli(fieldLcp)) : 0;
  const fieldInpScore =
    fieldInp !== null ? evaluateINPMetricScore(Helpers.Timing.microToMilli(fieldInp)) : 0;
...
export function estimateCompressedContentSize(request, totalBytes, resourceType) {
  ...
  const compressionRatio =
    Number.isFinite(resourceSize) && resourceSize > 0 ? contentTransferSize / resourceSize : 1;
