...
function finalize(partialModel) {
  ...
  if (partialModel.data) {
    hasFailure =
      !partialModel.data.checklist.httpCharset.value &&
      !partialModel.data.checklist.metaCharset.value;
  }
  return {
...
function findMetaCharsetDisposition(data, context) {
  ...
  return data.PageLoadMetrics.metaCharsetCheckEventsByNavigation.get(context.navigation)?.at(-1)
...
export function generateInsight(data, context) {
  ...
  return finalize({
    ...
    data: {
      ...
      checklist: {
        httpCharset: {
          label: hasHttpCharset
            ? i18nString(UIStrings.passingHttpHeader)
            : i18nString(UIStrings.failedHttpHeader),
