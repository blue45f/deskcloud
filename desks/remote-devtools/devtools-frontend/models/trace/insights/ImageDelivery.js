...
function finalize(partialModel) {
  return {
    ...
    relatedEvents: new Map(
      partialModel.optimizableImages.map((image) => [
        image.request,
        image.optimizations.map(getOptimizationMessageWithBytes),
      ]),
    ),
  };
}
...
function getDisplayedSize(data, paintImage) {
  ...
  return (
    data.ImagePainting.paintEventToCorrectedDisplaySize.get(paintImage) ?? {
...
export function generateInsight(data, context) {
  ...
  for (const request of contextRequests) {
    ...
    const { filePixels: imageFilePixels, displayedPixels: largestImageDisplayPixels } =
    ...
    const imageBytes = Math.min(
      ...
      request.args.data.encodedDataLength,
    );
    ...
    if (request.args.data.mimeType === 'image/gif') {
      ...
      if (
        request.args.data.mimeType !== 'image/webp' &&
        request.args.data.mimeType !== 'image/avif'
      ) {
        optimizations.push({
    ...
    if (wastedPixelRatio > 0 && !largestImagePaint.args.data.isCSS) {
      ...
      const hadBreakpoints =
        largestImagePaint.args.data.isPicture || largestImagePaint.args.data.srcsetAttribute;
      if (!hadBreakpoints || byteSavings > BYTE_SAVINGS_THRESHOLD_RESPONSIVE_BREAKPOINTS) {
        ...
        imageByteSavings += Math.round(
          wastedPixelRatio * (imageBytes - imageByteSavingsFromFormat),
        );
    ...
    optimizations = optimizations.filter(
      (optimization) => optimization.byteSavings > BYTE_SAVINGS_THRESHOLD,
    );
