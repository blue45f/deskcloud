...
function finalize(partialModel) {
  ...
  if (partialModel.data) {
    hasFailure =
      !partialModel.data.checklist.usesCompression.value ||
      !partialModel.data.checklist.serverResponseIsFast.value ||
      !partialModel.data.checklist.noRedirects.value;
  }
  return {
...
export function generateInsight(data, context) {
  ...
  const millisToString =
    context.options.insightTimeFormatters?.milli ?? i18n.TimeUtilities.millisToString;
  ...
  const redirectDuration = Math.round(
    documentRequest.args.data.syntheticData.redirectionDuration / 1000,
  );
  ...
  return finalize({
    ...
    data: {
      ...
      checklist: {
        noRedirects: {
          label: noRedirects
            ? i18nString(UIStrings.passingRedirects)
            : i18nString(UIStrings.failedRedirects, {
        ...
        serverResponseIsFast: {
          label: serverResponseIsFast
            ? i18nString(UIStrings.passingServerResponseTime, {
                PH1: millisToString(serverResponseTime),
              })
        ...
        usesCompression: {
          label: usesCompression
            ? i18nString(UIStrings.passingTextCompression)
...
export function createOverlays(model) {
  ...
  if (!model.data.checklist.serverResponseIsFast.value) {
    ...
    sections.push({
      bounds,
      label: i18nString(UIStrings.serverResponseTimeLabel),
      showDuration: true,
    });
  }
  if (model.data.uncompressedResponseBytes) {
    ...
    sections.push({
      bounds,
      label: i18nString(UIStrings.uncompressedDownload),
      showDuration: true,
    });
    overlays.push({ type: 'CANDY_STRIPED_TIME_RANGE', bounds, entry: event });
  }
