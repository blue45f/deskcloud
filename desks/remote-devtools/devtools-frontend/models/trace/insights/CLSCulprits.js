...
const ACTIONABLE_FAILURE_REASONS = [
  {
    flag: 1 << 0,
    failure:
      'ACCELERATED_ANIMATIONS_DISABLED' /* AnimationFailureReasons.ACCELERATED_ANIMATIONS_DISABLED */,
  },
  {
    flag: 1 << 1,
    failure:
      'EFFECT_SUPPRESSED_BY_DEVTOOLS' /* AnimationFailureReasons.EFFECT_SUPPRESSED_BY_DEVTOOLS */,
  },
  {
    flag: 1 << 2,
    failure:
      'INVALID_ANIMATION_OR_EFFECT' /* AnimationFailureReasons.INVALID_ANIMATION_OR_EFFECT */,
  },
  {
    flag: 1 << 3,
    failure:
      'EFFECT_HAS_UNSUPPORTED_TIMING_PARAMS' /* AnimationFailureReasons.EFFECT_HAS_UNSUPPORTED_TIMING_PARAMS */,
  },
  {
    flag: 1 << 4,
    failure:
      'EFFECT_HAS_NON_REPLACE_COMPOSITE_MODE' /* AnimationFailureReasons.EFFECT_HAS_NON_REPLACE_COMPOSITE_MODE */,
  },
  {
    flag: 1 << 5,
    failure:
      'TARGET_HAS_INVALID_COMPOSITING_STATE' /* AnimationFailureReasons.TARGET_HAS_INVALID_COMPOSITING_STATE */,
  },
  {
    flag: 1 << 6,
    failure:
      'TARGET_HAS_INCOMPATIBLE_ANIMATIONS' /* AnimationFailureReasons.TARGET_HAS_INCOMPATIBLE_ANIMATIONS */,
  },
  {
    flag: 1 << 7,
  ...
  // The failure 1 << 8 is marked as obsolete in Blink
  {
    flag: 1 << 9,
    failure:
      'ANIMATION_AFFECTS_NON_CSS_PROPERTIES' /* AnimationFailureReasons.ANIMATION_AFFECTS_NON_CSS_PROPERTIES */,
  },
  {
    flag: 1 << 10,
    failure:
      'TRANSFORM_RELATED_PROPERTY_CANNOT_BE_ACCELERATED_ON_TARGET' /* AnimationFailureReasons.TRANSFORM_RELATED_PROPERTY_CANNOT_BE_ACCELERATED_ON_TARGET */,
  },
  {
    flag: 1 << 11,
    failure:
      'TRANSFROM_BOX_SIZE_DEPENDENT' /* AnimationFailureReasons.TRANSFROM_BOX_SIZE_DEPENDENT */,
  },
  {
    flag: 1 << 12,
    failure:
      'FILTER_RELATED_PROPERTY_MAY_MOVE_PIXELS' /* AnimationFailureReasons.FILTER_RELATED_PROPERTY_MAY_MOVE_PIXELS */,
  },
  {
    flag: 1 << 13,
  ...
  },
  {
    flag: 1 << 16,
    failure:
      'TIMELINE_SOURCE_HAS_INVALID_COMPOSITING_STATE' /* AnimationFailureReasons.TIMELINE_SOURCE_HAS_INVALID_COMPOSITING_STATE */,
  },
  {
    flag: 1 << 17,
    failure:
      'ANIMATION_HAS_NO_VISIBLE_CHANGE' /* AnimationFailureReasons.ANIMATION_HAS_NO_VISIBLE_CHANGE */,
  },
  {
    flag: 1 << 18,
  ...
  },
  {
    flag: 1 << 19,
    failure:
      'SVG_TARGET_HAS_INDEPENDENT_TRANSFORM_PROPERTY' /* AnimationFailureReasons.SVG_TARGET_HAS_INDEPENDENT_TRANSFORM_PROPERTY */,
  },
];
// 500ms globalThis.
...
export function getNonCompositedFailure(animationEvent) {
  ...
  for (const event of instantEvents) {
    ...
    const failureReasons = ACTIONABLE_FAILURE_REASONS.filter(
      (reason) => failureMask & reason.flag,
    ).map((reason) => reason.failure);
    const failure = {
...
function getShiftsByPrePaintEvents(layoutShifts, prePaintEvents) {
  ...
  for (const prePaintEvent of prePaintEvents) {
    const firstShiftIndex = Platform.ArrayUtilities.nearestIndexFromBeginning(
      layoutShifts,
      (shift) => shift.ts >= prePaintEvent.ts,
    );
    ...
    for (let i = firstShiftIndex; i < layoutShifts.length; i++) {
      ...
      if (shift.ts >= prePaintEvent.ts && shift.ts <= prePaintEvent.ts + prePaintEvent.dur) {
        const shiftsInPrePaint = Platform.MapUtilities.getWithDefault(
          shiftsByPrePaint,
          prePaintEvent,
          () => [],
        );
...
function getUnsizedImageRootCauses(
  ...
  shiftsByPrePaint.forEach((shifts, prePaint) => {
    ...
    const matchingNode = unsizedImageEvents.find(
      (unsizedImage) => unsizedImage.args.data.nodeId === paintImage.args.data.nodeId,
    );
    if (!matchingNode) {
      return;
    }
...
function getFontRootCauses(networkRequests, prePaintEvents, shiftsByPrePaint, rootCausesByShift) {
  const fontRequests = networkRequests.filter(
    (req) => req.args.data.resourceType === 'Font' && req.args.data.mimeType.startsWith('font'),
  );
...
function getTopCulprits(cluster, culpritsByShift) {
  ...
  for (const shift of shifts) {
    ...
    for (let i = 0; i < fontReq.length && causes.length < MAX_TOP_CULPRITS; i++) {
      causes.push({
        type: 0 /* LayoutShiftType.WEB_FONT */,
        description: i18nString(UIStrings.webFont),
      });
    }
    for (let i = 0; i < iframes.length && causes.length < MAX_TOP_CULPRITS; i++) {
      causes.push({
        type: 1 /* LayoutShiftType.IFRAMES */,
        description: i18nString(UIStrings.injectedIframe),
      });
    }
    for (let i = 0; i < animations.length && causes.length < MAX_TOP_CULPRITS; i++) {
      causes.push({
        type: 2 /* LayoutShiftType.ANIMATIONS */,
        description: i18nString(UIStrings.animation),
      });
    }
...
function finalize(partialModel) {
  ...
  if (partialModel.worstCluster) {
    ...
    if (
      classification ===
      'good' /* Handlers.ModelHandlers.PageLoadMetrics.ScoreClassification.GOOD */
    ) {
      state = 'informative';
    } else {
      state = 'fail';
...
export function generateInsight(data, context) {
  ...
  const iframeEvents =
    data.LayoutShifts.renderFrameImplCreateChildFrameEvents.filter(isWithinContext);
  ...
  const clustersByScore = clusters.toSorted(
    (a, b) => b.clusterCumulativeScore - a.clusterCumulativeScore,
  );
  ...
  getUnsizedImageRootCauses(
    unsizedImageEvents,
    paintImageEvents,
    shiftsByPrePaint,
    rootCausesByShift,
  );
  const animationFailures = getNonCompositedFailureRootCauses(
    compositeAnimationEvents,
    prePaintEvents,
    shiftsByPrePaint,
    rootCausesByShift,
  );
...
export function createOverlays(model) {
  const clustersByScore =
    model.clusters.toSorted((a, b) => b.clusterCumulativeScore - a.clusterCumulativeScore) ?? [];
