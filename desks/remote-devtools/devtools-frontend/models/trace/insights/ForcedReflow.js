  ...
  for (const event of forcedReflowEvents) {
    ...
    while (node) {
      ...
      if (Types.Events.isProfileCall(eventData)) {
        ...
        if (
          Types.Events.isFunctionCall(eventData) &&
          eventData.args.data &&
    ...
    const aggregatedData = Platform.MapUtilities.getWithDefault(
      dataByTopLevelFunction,
      aggregatedDataId,
      () => ({
...
export function generateInsight(traceParsedData, context) {
  ...
  const events =
    traceParsedData.Warnings.perWarning.get('FORCED_REFLOW')?.filter(isWithinContext) ?? [];
  for (const event of events) {
    ...
    const bottomUpData = Platform.MapUtilities.getWithDefault(
      bottomUpDataMap,
      bottomCallId,
      () => ({
...
export function createOverlays(model) {
  ...
  const allBottomUpEvents = [
    ...model.aggregatedBottomUpData.values().flatMap((data) => data.relatedEvents),
  ];
  return [
