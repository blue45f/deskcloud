...
function finalize(partialModel) {
  ...
  if (partialModel.lcpMs !== undefined) {
    const classification =
      Handlers.ModelHandlers.PageLoadMetrics.scoreClassificationForLargestContentfulPaint(
...
export function generateInsight(data, context) {
  ...
  const lcpTs = metricScore.event?.ts
    ? Helpers.Timing.microToMilli(metricScore.event?.ts)
    : undefined;
  ...
  if (!docRequest) {
    return finalize({
      ...
      lcpEvent,
      lcpRequest,
      warnings: [InsightWarning.NO_DOCUMENT_REQUEST],
    });
  }
  return finalize({
    lcpMs,
  ...
  const overlays = [
    {
      ...
      sections: Object.values(model.subparts).map((subpart) => ({
        bounds: subpart,
        label: subpart.label,
        showDuration: true,
      })),
