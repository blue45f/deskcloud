...
const str_ = i18n.i18n.registerUIStrings(
  'models/trace/insights/NetworkDependencyTree.ts',
  UIStrings,
);
...
function isCritical(request, context) {
  ...
  const isIframe =
    request.args.data.resourceType === 'Document' /* Protocol.Network.ResourceType.Document */ &&
    request.args.data.frame !== context.frameId;
  if (
    nonCriticalResourceTypes.has(request.args.data.resourceType) ||
    isIframe ||
  ...
  const initiatorUrl =
    request.args.data.initiator?.url ||
    Helpers.Trace.getStackTraceTopCallFrameInEventPayload(request)?.url;
  if (!initiatorUrl) {
    return false;
  }
...
function generateNetworkDependencyTree(context) {
  ...
  function addChain(path) {
    ...
    for (let depth = 0; depth < path.length; ++depth) {
      ...
      if (!found) {
        const timeFromInitialRequest = Types.Timing.Micro(
          request.ts + request.dur - initialRequest.ts,
        );
        found = {
          request,
...
export function generatePreconnectedOrigins(data, context, contextRequests, preconnectCandidates) {
  ...
  for (const event of data.NetworkRequests.linkPreconnectEvents) {
    preconnectedOrigins.push({
      ...
      unused: !contextRequests.some(
        (request) =>
          getSecurityOrigin(event.args.data.url) === getSecurityOrigin(request.args.data.url),
      ),
      ...
      crossorigin: preconnectCandidates.some(
        (candidate) => candidate.origin === getSecurityOrigin(event.args.data.url),
      ),
      source: 'DOM',
    });
  }
  ...
  documentRequest?.args.data.responseHeaders?.forEach((header) => {
    if (header.name.toLowerCase() === 'link') {
      ...
      preconnectedOriginsFromResponseHeader?.forEach((origin) =>
        preconnectedOrigins.push({
          url: origin.url,
          ...
          //   they are preconnect candidates), then we issue a unused warning with crossorigin hint
          crossorigin: preconnectCandidates.some(
            (candidate) => candidate.origin === getSecurityOrigin(origin.url),
          ),
...
function hasValidTiming(request) {
  return (
    ...
    request.args.data.timing.connectEnd >= 0 &&
...
function socketStartTimeIsBelowThreshold(request, mainResource) {
  const timeSinceMainEnd = Math.max(
    0,
    request.args.data.syntheticData.sendStartTime - mainResource.args.data.syntheticData.finishTime,
  );
...
export function generatePreconnectCandidates(data, context, contextRequests) {
  ...
  groupedOrigins.forEach((requests) => {
    ...
    const firstRequestOfOriginParsedURL = new Common.ParsedURL.ParsedURL(
      firstRequestOfOrigin.args.data.url,
    );
    ...
    const wastedMs = Math.min(
      connectionTime,
      Helpers.Timing.microToMilli(timeBetweenMainResourceAndDnsStart),
    );
