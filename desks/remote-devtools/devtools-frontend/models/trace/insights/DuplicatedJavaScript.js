...
function finalize(partialModel) {
  const requests = partialModel.scriptsWithDuplication
    .map((script) => script.request)
    .filter((e) => !!e);
  return {
...
export function generateInsight(data, context) {
  ...
  for (const script of scripts) {
    if (script.request) {
      compressionRatios.set(
        script.request.args.data.requestId,
        estimateCompressionRatioForScript(script),
      );
    }
  }
  ...
  const scriptsWithDuplication = [
    ...duplication.values().flatMap((data) => data.duplicates.map((d) => d.script)),
  ];
  ...
  for (const { duplicates } of duplication.values()) {
    for (let i = 1; i < duplicates.length; i++) {
      ...
      wastedBytesByRequestId.set(
        requestId,
        (wastedBytesByRequestId.get(requestId) || 0) + transferSize,
      );
    }
  }
  return finalize({
...
export function createOverlays(model) {
  return model.scriptsWithDuplication
    .map((script) => script.request)
    .filter((e) => !!e)
    .map((request) => {
      return {
