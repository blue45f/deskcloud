...
export function generateInsight(data, context) {
  ...
  const compositorEvents = data.UserInteractions.beginCommitCompositorFrameEvents.filter(
    (event) => {
  ...
  for (const event of compositorEvents) {
    if (!event.args.is_mobile_optimized) {
      ...
      const longPointerInteractions = [
        ...data.UserInteractions.interactionsOverThreshold.values(),
      ].filter(
        (interaction) =>
          Handlers.ModelHandlers.UserInteractions.categoryOfInteraction(interaction) ===
            'POINTER' && interaction.inputDelay >= 50_000,
      );
      ...
      const inputDelay =
        Math.max(0, ...longPointerInteractions.map((interaction) => interaction.inputDelay)) / 1000;
