...
export function isCacheable(request) {
  ...
  return Boolean(
    ...
    Helpers.Network.STATIC_RESOURCE_TYPES.has(
      request.args.data.resourceType || 'Other' /* Protocol.Network.ResourceType.Other */,
    ),
  );
}
/**
...
export function generateInsight(data, context) {
  ...
  results.sort((a, b) => {
    return (
      b.request.args.data.decodedBodyLength - a.request.args.data.decodedBodyLength || a.ttl - b.ttl
    );
  });
  return finalize({
