...
function finalize(partialModel) {
  ...
  return {
    ...
    state:
      ...
      partialModel.checklist &&
      (!partialModel.checklist.eagerlyLoaded.value ||
        !partialModel.checklist.requestDiscoverable.value ||
...
export function generateInsight(data, context) {
  ...
  const lcpNotLazyLoaded =
    lcpEvent.args.data?.loadingAttr !== 'lazy' || lcpRequest.args.data.isLinkPreload;
  return finalize({
    lcpEvent,
    lcpRequest,
    earliestDiscoveryTimeTs: earliestDiscoveryTime
      ? Types.Timing.Micro(earliestDiscoveryTime)
      : undefined,
    checklist: {
      priorityHinted: {
        label: priorityHintFound
          ? i18nString(UIStrings.fetchPriorityApplied)
          : missingPriorityHintLabel,
        value: priorityHintFound,
      },
      requestDiscoverable: {
        label: i18nString(UIStrings.requestDiscoverable),
        value: imgPreloadedOrFoundInHTML,
      },
...
export function getImageData(model) {
  ...
  const imageLCP =
    shouldIncreasePriorityHint !== undefined &&
    shouldPreloadImage !== undefined &&
