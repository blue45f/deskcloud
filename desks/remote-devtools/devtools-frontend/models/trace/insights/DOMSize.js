...
export function generateInsight(data, context) {
  ...
  for (const thread of threads) {
    ...
    const timeRange = Helpers.Timing.traceWindowFromMicroSeconds(
      first.ts,
      Types.Timing.Micro(last.ts + (last.dur ?? 0)),
    );
  ...
  const domStatsEvents =
    data.DOMStats.domStatsByFrameId.get(context.frameId)?.filter(isWithinContext) ?? [];
  let maxDOMStats;
