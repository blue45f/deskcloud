...
function finalize(partialModel) {
  const requests = [...partialModel.legacyJavaScriptResults.keys()]
...
export function generateInsight(data, context) {
  const scripts = data.Scripts.scripts.filter((script) => {
    ...
    return (
      Helpers.Timing.timestampIsInBounds(context.bounds, script.ts) ||
  ...
  const sorted = new Map(
    [...legacyJavaScriptResults].sort(
      (a, b) => b[1].estimatedByteSavings - a[1].estimatedByteSavings,
    ),
  );
  return finalize({
...
export function createOverlays(model) {
  return [...model.legacyJavaScriptResults.keys()]
    .map((script) => script.request)
    .filter((e) => !!e)
    .map((request) => {
      return {
