...
function estimateSavingsWithGraphs(deferredIds, lanternContext) {
  ...
  const estimateBeforeInline = Math.max(
    ...Array.from(Array.from(adjustedNodeTimings).map((timing) => timing[1].endTime)),
  );
...
function computeSavings(data, context, renderBlockingRequests) {
  ...
  const nodesAndTimingsByRequestId = getNodesAndTimingByRequestId(
    context.lantern.metrics.firstContentfulPaint.optimisticEstimate.nodeTimings,
  );
...
export function generateInsight(data, context) {
  ...
  const firstPaintTs = data.PageLoadMetrics.metricScoresByFrameId
    .get(context.frameId)
    ?.get(context.navigation)
    ?.get('FP' /* Handlers.ModelHandlers.PageLoadMetrics.MetricName.FP */)?.event?.ts;
  if (!firstPaintTs) {
    return finalize({
      renderBlockingRequests: [],
  ...
  for (const req of data.NetworkRequests.byTime) {
    ...
    if (req.args.data.renderBlocking === 'in_body_parser_blocking') {
      ...
      const isScript =
        req.args.data.resourceType === 'Script'; /* Protocol.Network.ResourceType.Script */
      const isBlockingScript =
        isScript && priority === 'High'; /* Protocol.Network.ResourcePriority.High */
      if (
        priority !== 'VeryHigh' /* Protocol.Network.ResourcePriority.VeryHigh */ &&
        !isBlockingScript
      ) {
        continue;
      }
