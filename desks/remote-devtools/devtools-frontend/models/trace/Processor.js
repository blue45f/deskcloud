...
function calculateProgress(value, phase) {
  ...
  if (phase === 0.8 /* ProgressPhase.FINALIZE */) {
    return (
      value * (0.8 /* ProgressPhase.FINALIZE */ - 0.2) /* ProgressPhase.HANDLE_EVENT */ +
      0.2 /* ProgressPhase.HANDLE_EVENT */
    );
  }
  return value * phase;
}
export class TraceProcessor extends EventTarget {
  ...
  async parse(traceEvents, options) {
    if (this.#status !== 'IDLE' /* Status.IDLE */) {
      throw new Error(
        `Trace processor can't start parsing when not idle. Current state: ${this.#status}`,
      );
    }
    if (typeof options.isCPUProfile === 'undefined' && options.metadata) {
      options.isCPUProfile =
        options.metadata.dataOrigin === 'CPUProfile' /* Types.File.DataOrigin.CPU_PROFILE */;
    }
    ...
    try {
      ...
      if (this.#data && !options.isCPUProfile) {
        // We do not calculate insights for CPU Profiles.
  ...
  #createLanternContext(data, traceEvents, frameId, navigation, options) {
    ...
    const endTime =
      navStartIndex + 1 < navStarts.length
        ? navStarts[navStartIndex + 1].ts
    ...
    const tbtResult = Lantern.Metrics.TotalBlockingTime.compute(computeData, {
      fcpResult,
      interactiveResult,
    });
    const metrics = {
  ...
  sortInsightSet(insightSet, metadata) {
    ...
    const observedLcp = observedLcpMicro
      ? Helpers.Timing.microToMilli(observedLcpMicro)
      : Types.Timing.Milli(0);
    ...
    const observedInp = observedInpMicro
      ? Helpers.Timing.microToMilli(observedInpMicro)
      : Types.Timing.Milli(200);
    ...
    for (const [name, insight] of Object.entries(insightSet.model)) {
      ...
      if (weights.lcp && lcp && observedLcpScore !== undefined && lcpPostSavings !== undefined) {
        score +=
          weights.lcp * (Insights.Common.evaluateLCPMetricScore(lcpPostSavings) - observedLcpScore);
      }
      if (weights.inp && inp && observedInpScore !== undefined) {
        score +=
          weights.inp * (Insights.Common.evaluateINPMetricScore(inpPostSavings) - observedInpScore);
      }
      if (weights.cls && cls && observedClsScore !== undefined) {
        score +=
          weights.cls * (Insights.Common.evaluateCLSMetricScore(clsPostSavings) - observedClsScore);
      }
    ...
    orderedKeys.sort(
      (a, b) => (insightToSortingRank.get(b) ?? 0) - (insightToSortingRank.get(a) ?? 0),
    );
    const newModel = {};
  ...
  #computeInsightSet(data, context) {
    ...
    if (context.navigation) {
      ...
      urlString =
        data.Meta.finalDisplayUrlByNavigationId.get(context.navigationId) ?? data.Meta.mainFrameURL;
    ...
    const everyInsightPasses = Object.values(insightSetModel).every(
      (model) => model && model.state === 'pass',
    );
    ...
    const shouldExclude =
      isNavigation &&
      context.bounds.range < trivialThreshold &&
      everyInsightPasses &&
      noLcp &&
      noInp &&
      noLayoutShifts;
  ...
  #computeInsights(data, traceEvents, options) {
    ...
    for (const [index, navigation] of navigations.entries()) {
      ...
      const max =
        index + 1 < navigations.length ? navigations[index + 1].ts : data.Meta.traceBounds.max;
