...
export function get(event, data) {
  ...
  if (Types.Extensions.isSyntheticExtensionEntry(event)) {
    ...
    const payloadCallFrames = getTraceEventPayloadStackAsProtocolCallFrame(event).filter(
      (callFrame) => !isNativeJSFunction(callFrame),
    );
...
function getForEvent(event, data) {
  ...
  // entries in its tree.
  const entryToNode =
    data.Renderer.entryToNode.size > 0 ? data.Renderer.entryToNode : data.Samples.entryToNode;
  ...
  while (node) {
    ...
    if (stackTraceFromCache) {
      stackTrace.callFrames.push(
        ...stackTraceFromCache.callFrames.filter((callFrame) => !isNativeJSFunction(callFrame)),
      );
    ...
    const maybeAsyncParentNode =
      maybeAsyncParentEvent && entryToNode.get(maybeAsyncParentEvent.scheduler);
    if (maybeAsyncParentNode) {
