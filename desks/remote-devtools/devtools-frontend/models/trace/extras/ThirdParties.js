...
function collectMainThreadActivity(data) {
  ...
  const mainFrameMainThread = data.Renderer.processes
    .values()
    .find((p) => {
      const url = p.url ?? '';
...
function getBottomUpTree(mainThreadEvents, tracebounds, groupingFunction) {
  ...
  const filter = new TraceFilter.VisibleEventsFilter(
    visibleEvents.concat([
      'SyntheticNetworkRequest' /* Types.Events.Name.SYNTHETIC_NETWORK_REQUEST */,
    ]),
  );
