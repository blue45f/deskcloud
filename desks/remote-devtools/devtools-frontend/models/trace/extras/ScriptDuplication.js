...
export function normalizeDuplication(duplication) {
  for (const [key, data] of duplication) {
    ...
    // Ignore modules smaller than an absolute threshold.
    data.duplicates = data.duplicates.filter(
      (duplicate) => duplicate.attributedSize >= ABSOLUTE_SIZE_THRESHOLD_BYTES,
    );
    ...
    data.estimatedDuplicateBytes = data.duplicates
      .slice(1)
      .reduce((acc, cur) => acc + cur.attributedSize, 0);
  }
}
...
function sorted(duplication) {
  return new Map(
    [...duplication].sort((a, b) => b[1].estimatedDuplicateBytes - a[1].estimatedDuplicateBytes),
  );
}
/**
...
export function computeScriptDuplication(scriptsData, compressionRatios) {
  ...
  for (const [script, sourceDataArray] of sourceDatasMap) {
    for (const sourceData of sourceDataArray) {
      ...
      const compressionRatio = script.request
        ? (compressionRatios.get(script.request?.args.data.requestId) ?? 1)
        : 1;
