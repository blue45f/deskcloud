...
export function isNestableAsyncPhase(phase) {
  return (
    phase === 'b' /* Phase.ASYNC_NESTABLE_START */ ||
    phase === 'e' /* Phase.ASYNC_NESTABLE_END */ ||
    phase === 'n' /* Phase.ASYNC_NESTABLE_INSTANT */
  );
}
export function isPhaseAsync(phase) {
  return (
    isNestableAsyncPhase(phase) ||
    phase === 'S' /* Phase.ASYNC_BEGIN */ ||
    phase === 'T' /* Phase.ASYNC_STEP_INTO */ ||
    phase === 'F' /* Phase.ASYNC_END */ ||
    phase === 'p' /* Phase.ASYNC_STEP_PAST */
  );
}
export function isFlowPhase(phase) {
  return (
    phase === 's' /* Phase.FLOW_START */ ||
    phase === 't' /* Phase.FLOW_STEP */ ||
    phase === 'f' /* Phase.FLOW_END */
  );
}
...
export function objectIsCallFrame(object) {
  return (
    'functionName' in object &&
    typeof object.functionName === 'string' &&
    'scriptId' in object &&
    (typeof object.scriptId === 'string' || typeof object.scriptId === 'number') &&
    'columnNumber' in object &&
    typeof object.columnNumber === 'number' &&
    'lineNumber' in object &&
...
export function isMarkerEvent(event) {
  if (
    event.ph === 'I' /* Phase.INSTANT */ ||
    'n' /* Phase.ASYNC_NESTABLE_INSTANT */ ||
    event.ph === 'R' /* Phase.MARK */
  ) {
    return markerTypeGuards.some((fn) => fn(event));
  }
  return false;
}
const pageLoadEventTypeGuards = [...markerTypeGuards, isInteractiveTime];
...
export function isScheduleStyleInvalidationTracking(event) {
  return (
    event.name ===
    'ScheduleStyleInvalidationTracking' /* Name.SCHEDULE_STYLE_INVALIDATION_TRACKING */
  );
}
export function isStyleRecalcInvalidationTracking(event) {
  return (
    event.name === 'StyleRecalcInvalidationTracking' /* Name.STYLE_RECALC_INVALIDATION_TRACKING */
  );
}
export function isStyleInvalidatorInvalidationTracking(event) {
  return (
    event.name ===
    'StyleInvalidatorInvalidationTracking' /* Name.STYLE_INVALIDATOR_INVALIDATION_TRACKING */
  );
}
...
export function isRenderFrameImplCreateChildFrame(event) {
  return (
    event.name ===
    'RenderFrameImpl::createChildFrame' /* Name.RENDER_FRAME_IMPL_CREATE_CHILD_FRAME */
  );
}
...
export function isAnimationFrameAsyncStart(data) {
  return (
    data.name === 'AnimationFrame' /* Name.ANIMATION_FRAME */ &&
    data.ph === 'b' /* Phase.ASYNC_NESTABLE_START */
  );
}
export function isAnimationFrameAsyncEnd(data) {
  return (
    data.name === 'AnimationFrame' /* Name.ANIMATION_FRAME */ &&
    data.ph === 'e' /* Phase.ASYNC_NESTABLE_END */
  );
}
...
export function isSyntheticInteraction(event) {
  return Boolean(
    ...
    event.args?.data &&
    'beginEvent' in event.args.data &&
    'endEvent' in event.args.data,
  );
}
...
export function isBeginFrame(event) {
  ...
  return Boolean(
    event.name === 'BeginFrame' /* Name.BEGIN_FRAME */ && event.args && 'frameSeqId' in event.args,
  );
}
export function isDroppedFrame(event) {
  // Old traces did not have frameSeqId; but we do not want to support these.
  return Boolean(
    event.name === 'DroppedFrame' /* Name.DROPPED_FRAME */ &&
    event.args &&
    'frameSeqId' in event.args,
  );
}
...
export function isCommit(event) {
  // Old traces did not have frameSeqId; but we do not want to support these.
  return Boolean(
    event.name === 'Commit' /* Name.COMMIT */ && event.args && 'frameSeqId' in event.args,
  );
}
...
export function isInvalidationTracking(event) {
  return (
    isScheduleStyleInvalidationTracking(event) ||
    isStyleRecalcInvalidationTracking(event) ||
    isStyleInvalidatorInvalidationTracking(event) ||
    isLayoutInvalidationTracking(event)
  );
}
...
export function isLayout(event) {
  return (
    event.name === 'Layout' /* Name.LAYOUT */ && Boolean(event.args && 'beginData' in event.args)
  );
}
...
export function isHandlePostMessage(event) {
  return (
    event.name === 'HandlePostMessage' /* Name.HANDLE_POST_MESSAGE */ &&
    event.ph === 'X' /* Phase.COMPLETE */
  );
}
...
export function isAnyLargestContentfulPaintCandidate(event) {
  return (
    event.name === 'largestContentfulPaint::Candidate' /* Name.MARK_LCP_CANDIDATE */ ||
    event.name ===
      'largestContentfulPaint::CandidateForSoftNavigation' /* Name.MARK_LCP_CANDIDATE_FOR_SOFT_NAVIGATION */
  );
}
...
export function isSyntheticCpuProfile(event) {
  return (
    event.name === 'CpuProfile' /* Name.CPU_PROFILE */ && event.ph === 'X' /* Phase.COMPLETE */
  );
}
...
export function isNetworkTrackEntry(event) {
  return (
    isSyntheticNetworkRequest(event) ||
    isSyntheticWebSocketConnection(event) ||
    isWebSocketTraceEvent(event)
  );
}
...
export function isDidCommitSameDocumentNavigation(event) {
  return (
    event.name === 'RenderFrameHostImpl::DidCommitSameDocumentNavigation' &&
    event.ph === 'X' /* Phase.COMPLETE */
  );
}
...
export function isPerformanceMark(event) {
  return (
    isUserTiming(event) &&
    (event.ph === 'R' /* Phase.MARK */ || event.ph === 'I') /* Phase.INSTANT */
  );
}
...
export function isPaintImage(event) {
  return (
    event.name === 'PaintImage' /* Name.PAINT_IMAGE */ && event.ph === 'X' /* Phase.COMPLETE */
  );
}
export function isScrollLayer(event) {
  return (
    event.name === 'ScrollLayer' /* Name.SCROLL_LAYER */ && event.ph === 'X' /* Phase.COMPLETE */
  );
}
...
export function isFireAnimationFrame(event) {
  return (
    event.name === 'FireAnimationFrame' /* Name.FIRE_ANIMATION_FRAME */ &&
    event.ph === 'X' /* Phase.COMPLETE */
  );
}
...
export function isWebSocketInfo(event) {
  return (
    event.name === 'WebSocketSendHandshakeRequest' /* Name.WEB_SOCKET_SEND_HANDSHAKE_REQUEST */ ||
    event.name ===
      'WebSocketReceiveHandshakeResponse' /* Name.WEB_SOCKET_RECEIVE_HANDSHAKE_REQUEST */ ||
    event.name === 'WebSocketDestroy' /* Name.WEB_SOCKET_DESTROY */
  );
}
export function isWebSocketTransfer(event) {
  return (
    event.name === 'WebSocketSend' /* Name.WEB_SOCKET_SEND */ ||
    event.name === 'WebSocketReceive' /* Name.WEB_SOCKET_RECEIVE */
  );
}
...
export function isFunctionCall(event) {
  return (
    event.name === 'FunctionCall' /* Name.FUNCTION_CALL */ && event.ph === 'X' /* Phase.COMPLETE */
  );
}
...
export function isRunPostTaskCallback(event) {
  return (
    event.name === 'RunPostTaskCallback' /* Name.RUN_POST_TASK_CALLBACK */ &&
    event.ph === 'X' /* Phase.COMPLETE */
  );
}
export function isAbortPostTaskCallback(event) {
  return (
    event.name === 'AbortPostTaskCallback' /* Name.ABORT_POST_TASK_CALLBACK */ &&
    event.ph === 'X' /* Phase.COMPLETE */
  );
}
/**
...
export function isFlowPhaseEvent(event) {
  return (
    event.ph === 's' /* Phase.FLOW_START */ ||
    event.ph === 't' /* Phase.FLOW_STEP */ ||
    event.ph === 'f' /* Phase.FLOW_END */
  );
}
export function isParseAuthorStyleSheetEvent(event) {
  return (
    event.name === 'ParseAuthorStyleSheet' /* Name.PARSE_AUTHOR_STYLE_SHEET */ &&
    event.ph === 'X' /* Phase.COMPLETE */
  );
}
/**
...
export function isRundownScript(event) {
  return (
    event.cat === 'disabled-by-default-devtools.v8-source-rundown' && event.name === 'ScriptCatchup'
  );
}
export function isRundownScriptSource(event) {
  return (
    event.cat === 'disabled-by-default-devtools.v8-source-rundown-sources' &&
    event.name === 'ScriptCatchup'
  );
}
export function isRundownScriptSourceLarge(event) {
  return (
    event.cat === 'disabled-by-default-devtools.v8-source-rundown-sources' &&
    event.name === 'LargeScriptCatchup'
  );
}
...
export function isPreloadRenderBlockingStatusChangeEvent(event) {
  return (
    event.name ===
    'PreloadRenderBlockingStatusChange' /* Name.PRELOAD_RENDER_BLOCKING_STATUS_CHANGE */
  );
}
