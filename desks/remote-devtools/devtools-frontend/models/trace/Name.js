...
export function forEntry(entry, parsedTrace) {
  if (Types.Events.isProfileCall(entry)) {
    if (parsedTrace) {
      const potentialCallName = Handlers.ModelHandlers.Samples.getProfileCallFunctionName(
        parsedTrace.data.Samples,
        entry,
      );
  ...
  if (Types.Events.isSyntheticNetworkRequest(entry)) {
    ...
    const text = parsedURL.isValid
      ? `${parsedURL.displayName} (${parsedURL.host})`
      : entry.args.data.url || 'Network request';
    return text;
  }
  ...
  if (
    Types.Events.isSyntheticAnimation(entry) &&
    entry.args.data.beginEvent.args.data.displayName
  ) {
