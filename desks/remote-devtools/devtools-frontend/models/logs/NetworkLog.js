...
export class NetworkLog extends Common.ObjectWrapper.ObjectWrapper {
  ...
  modelAdded(networkManager) {
    ...
    eventListeners.push(
      networkManager.addEventListener(
        SDK.NetworkManager.Events.RequestRedirected,
  ...
  setIsRecording(enabled) {
    ...
    if (enabled) {
      ...
      SDK.TargetManager.TargetManager.instance().unobserveModels(
        SDK.NetworkManager.NetworkManager,
        this,
      );
  ...
  static initiatorInfoForRequest(request, existingInitiatorData) {
    ...
    if (redirectSource) {
      ...
      if (initiator.type === 'parser' /* Protocol.Network.InitiatorType.Parser */) {
        ...
        initiator.type === 'SignedExchange' /* Protocol.Network.InitiatorType.SignedExchange */
      ) {
  ...
  initiatorGraphForRequest(request) {
    ...
    for (const otherRequest of this.#requests) {
      ...
      if (
        networkManager === otherRequestManager &&
        this.initiatorChain(otherRequest).has(request)
      ) {
  ...
  initiatorRequest(request) {
    ...
    const networkManager = SDK.NetworkManager.NetworkManager.forRequest(request);
    initiatorData.request = networkManager
      ? this.requestByManagerAndURL(networkManager, url)
      : null;
    return initiatorData.request;
  }
  willReloadPage() {
  ...
  onPrimaryPageChanged(event) {
    ...
    if (
      !manager ||
      mainFrame.resourceTreeModel().target().parentTarget()?.type() === SDK.Target.Type.FRAME
    ) {
      return;
    }
    ...
    // the network log.
    if (
      mainFrame.url !== mainFrame.unreachableUrl() &&
      Common.ParsedURL.schemeIs(mainFrame.url, 'chrome-error:')
    ) {
      return;
    }
    ...
    const oldManagerRequests = this.#requests.filter(
      (request) => SDK.NetworkManager.NetworkManager.forRequest(request) === manager,
    );
    ...
    for (const request of oldManagerRequests) {
      if (
        event.data.type !==
          'Activation' /* SDK.ResourceTreeModel.PrimaryPageChangeType.ACTIVATION */ &&
    ...
    for (const swRequest of oldRequests) {
      ...
      const keepRequest = requestsToAdd.some(
        (request) =>
          request.url() === swRequest.url() && request.issueTime() <= swRequest.issueTime(),
      );
      if (keepRequest) {
  ...
  onLoad(event) {
    const networkManager = event.data.resourceTreeModel
      .target()
  ...
  networkMessageGenerated(networkManager, event) {
    ...
    const consoleMessage = new SDK.ConsoleModel.ConsoleMessage(
      ...
      'network' /* Protocol.Log.LogEntrySource.Network */,
      warning
