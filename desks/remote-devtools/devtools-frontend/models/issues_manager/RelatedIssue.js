...
export function hasThirdPartyPhaseoutCookieIssue(obj) {
  ...
  return issuesAssociatedWith(issues, obj).some(
    (issue) =>
      CookieIssue.getSubCategory(issue.code()) ===
      'ThirdPartyPhaseoutCookie' /* CookieIssueSubCategory.THIRD_PARTY_PHASEOUT_COOKIE */,
  );
}
export function hasThirdPartyPhaseoutCookieIssueForDomain(domain) {
  const issues = Array.from(IssuesManager.instance().issues());
  const issuesForDomain = issues.filter((issue) =>
    Array.from(issue.cookies()).some((cookie) => cookie.domain === domain),
  );
...
export async function reveal(obj, category) {
  ...
  const issues = Array.from(IssuesManager.instance().issues());
  const candidates = issuesAssociatedWith(issues, obj).filter(
    (issue) => !category || issue.getCategory() === category,
  );
