...
export class SourceFrameIssuesManager {
  ...
  constructor(issuesManager) {
    ...
    this.issuesManager.addEventListener(
      'IssueAdded' /* Events.ISSUE_ADDED */,
      this.#onIssueAdded,
      this,
    );
    this.issuesManager.addEventListener(
      'FullUpdateRequired' /* Events.FULL_UPDATE_REQUIRED */,
  ...
  async #addIssue(issue) {
    if (
      !this.#isTrustedTypeIssue(issue) &&
      !this.#isLateImportIssue(issue) &&
      !this.#isPropertyRuleIssue(issue)
    ) {
      return;
    }
  ...
  #isTrustedTypeIssue(issue) {
    return (
      (issue instanceof ContentSecurityPolicyIssue &&
        issue.code() === trustedTypesSinkViolationCode) ||
