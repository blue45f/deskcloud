...
import {
  ...
  isCrossOriginEmbedderPolicyIssue,
} from './CrossOriginEmbedderPolicyIssue.js';
...
const issueCodeHandlers = new Map([
  ...
  [
    'BlockedByResponseIssue' /* Protocol.Audits.InspectorIssueCode.BlockedByResponseIssue */,
    createIssuesForBlockedByResponseIssue,
  ],
  [
  ...
  ['CorsIssue' /* Protocol.Audits.InspectorIssueCode.CorsIssue */, CorsIssue.fromInspectorIssue],
  [
...
export class IssuesManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static instance(
    ...
    if (!issuesManagerInstance || opts.forceNew) {
      issuesManagerInstance = new IssuesManager(
        opts.showThirdPartyIssuesSetting,
        opts.hideIssueSetting,
      );
    }
  ...
  #onFrameAddedToTarget(event) {
    ...
    if (
      frame.isOutermostFrame() &&
      SDK.TargetManager.TargetManager.instance().isInScope(frame.resourceTreeModel())
    ) {
      this.#updateFilteredIssues();
    }
  }
  modelAdded(issuesModel) {
    const listener = issuesModel.addEventListener(
      'IssueAdded' /* SDK.IssuesModel.Events.ISSUE_ADDED */,
      this.#onIssueAddedEvent,
      this,
    );
  ...
  addIssue(issuesModel, issue) {
    ...
    if (this.#issueFilter(issue)) {
      ...
      if (CookieIssue.isThirdPartyCookiePhaseoutRelatedIssue(issue)) {
        ...
        this.#hiddenIssueCount.set(
          issue.getKind(),
          1 + (this.#hiddenIssueCount.get(issue.getKind()) || 0),
        );
      }
  ...
  numberOfIssues(kind) {
    if (kind) {
      return (
        (this.#issueCounts.get(kind) ?? 0) -
        this.numberOfHiddenIssues(kind) -
        this.numberOfThirdPartyCookiePhaseoutIssues(kind)
      );
    }
    return (
      this.#filteredIssues.size -
      this.numberOfHiddenIssues() -
      this.numberOfThirdPartyCookiePhaseoutIssues()
    );
  }
