...
export class DeviceModeModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    this.#scaleSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.#scale = 1;
    this.#widthSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.#heightSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.#uaSetting = Common.Settings.Settings.instance().createSetting(
      'emulation.device-ua',
      'Mobile' /* UA.MOBILE */,
    );
    this.#uaSetting.addChangeListener(this.uaSettingChanged, this);
    this.#deviceScaleFactorSetting = Common.Settings.Settings.instance().createSetting(
      'emulation.device-scale-factor',
      0,
    );
    this.#deviceScaleFactorSetting.addChangeListener(this.deviceScaleFactorSettingChanged, this);
    this.#deviceOutlineSetting = Common.Settings.Settings.instance().moduleSetting(
  ...
  static scaleValidator(value) {
    ...
    if (!value) {
      ...
      errorMessage = i18nString(UIStrings.devicePixelRatioMustBeLessThanOr, {
        PH1: MaxDeviceScaleFactor,
      });
    } else if (Number(value) < MinDeviceScaleFactor) {
      errorMessage = i18nString(UIStrings.devicePixelRatioMustBeGreater, {
        PH1: MinDeviceScaleFactor,
      });
    } else {
      valid = true;
    }
  ...
  emulate(type, device, mode, scale) {
    const resetPageScaleFactor =
      this.#type !== type || this.#device !== device || this.#mode !== mode;
    this.#type = type;
    if (type === Type.Device && device && mode) {
      console.assert(
        Boolean(device) && Boolean(mode),
  ...
  outlineImage() {
    return this.#device && this.#mode && this.#deviceOutlineSetting.get()
      ? this.#device.outlineImage(this.#mode)
      : '';
  }
  outlineRect() {
  ...
  isMobile() {
    switch (this.#type) {
      ...
      case Type.Responsive:
        return (
          this.#uaSetting.get() === 'Mobile' /* UA.MOBILE */ ||
          this.#uaSetting.get() === 'Mobile (no touch)' /* UA.MOBILE_NO_TOUCH */
        );
    }
    return false;
  }
  ...
  modelAdded(emulationModel) {
    if (
      emulationModel.target() === SDK.TargetManager.TargetManager.instance().primaryPageTarget() &&
      ...
      emulationModel.addEventListener(
        'ScreenOrientationLockChanged' /* SDK.EmulationModel.EmulationModelEvents.SCREEN_ORIENTATION_LOCK_CHANGED */,
        this.onScreenOrientationLockChanged,
        this,
      );
      const resourceTreeModel = emulationModel
        .target()
        .model(SDK.ResourceTreeModel.ResourceTreeModel);
      if (resourceTreeModel) {
        resourceTreeModel.addEventListener(
          SDK.ResourceTreeModel.Events.FrameResized,
          this.onFrameChange,
          this,
        );
        resourceTreeModel.addEventListener(
  ...
  applyOrientationLock(orientation) {
    const wantsLandscape =
      orientation.type ===
        'landscapePrimary' /* Protocol.Emulation.ScreenOrientationType.LandscapePrimary */ ||
      orientation.type ===
        'landscapeSecondary'; /* Protocol.Emulation.ScreenOrientationType.LandscapeSecondary */
  ...
  calculateAndEmulate(resetPageScaleFactor) {
    ...
    if (this.#type === Type.Device && this.#device && this.#mode) {
      ...
      if (mobile) {
        this.#appliedUserAgentType = this.#device.touch()
          ? 'Mobile' /* UA.MOBILE */
          : 'Mobile (no touch)' /* UA.MOBILE_NO_TOUCH */;
      } else {
        this.#appliedUserAgentType = this.#device.touch()
          ? 'Desktop (touch)' /* UA.DESKTOP_TOUCH */
          : 'Desktop' /* UA.DESKTOP */;
      }
      ...
      this.applyDeviceMetrics(
        ...
        screenHeight >= screenWidth
          ? 'portraitPrimary' /* Protocol.Emulation.ScreenOrientationType.PortraitPrimary */
          : 'landscapePrimary' /* Protocol.Emulation.ScreenOrientationType.LandscapePrimary */,
        resetPageScaleFactor,
      );
      this.applyUserAgent(
        mobile ? defaultMobileUserAgent : '',
        mobile ? defaultMobileUserAgentMetadata : null,
      );
      this.applyTouch(
        this.#uaSetting.get() === 'Desktop (touch)' /* UA.DESKTOP_TOUCH */ ||
          this.#uaSetting.get() === 'Mobile' /* UA.MOBILE */,
        this.#uaSetting.get() === 'Mobile' /* UA.MOBILE */,
      );
    }
  ...
  applyDeviceMetrics(
    ...
    this.#visiblePageRect = new Rect(
      ...
      Math.min(
        pageWidth * scale,
        this.#availableSize.width - this.#screenRect.left - positionX * scale,
      ),
      Math.min(
        pageHeight * scale,
        this.#availableSize.height - this.#screenRect.top - positionY * scale,
      ),
    );
    this.#scale = scale;
    ...
    if (!displayFeature) {
      ...
      if (
        scale === 1 &&
        this.#availableSize.width >= screenSize.width &&
      ...
      if (
        this.#visiblePageRect.width === pageWidth * scale &&
        this.#visiblePageRect.height === pageHeight * scale &&
    ...
    if (
      ...
      if (displayFeature) {
        ...
        metrics.devicePosture = {
          type: 'folded' /* Protocol.Emulation.DevicePostureType.Folded */,
        };
      } else {
        metrics.devicePosture = {
          type: 'continuous' /* Protocol.Emulation.DevicePostureType.Continuous */,
        };
      }
  ...
  async captureScreenshot(fullSize, clip) {
    const screenCaptureModel = this.#emulationModel
      ? this.#emulationModel.target().model(SDK.ScreenCaptureModel.ScreenCaptureModel)
      : null;
  ...
  showHingeIfApplicable(overlayModel) {
    const orientation =
      this.#device && this.#mode ? this.#device.orientationByName(this.#mode.orientation) : null;
