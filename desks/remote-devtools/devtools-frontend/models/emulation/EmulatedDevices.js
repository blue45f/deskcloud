  ...
  static fromJSONV1(json) {
    try {
      ...
      function parseInsets(json) {
        return new Insets(
          parseIntValue(json, 'left'),
          parseIntValue(json, 'top'),
          parseIntValue(json, 'right'),
      ...
      result.userAgent =
        SDK.NetworkManager.MultitargetNetworkManager.patchUserAgentWithChromeVersion(rawUserAgent);
      ...
      if (result.isDualScreen || result.isFoldableScreen) {
        result.verticalSpanned = parseOrientation(
          parseValue(json['screen'], 'vertical-spanned', 'object', null),
        );
        result.horizontalSpanned = parseOrientation(
          parseValue(json['screen'], 'horizontal-spanned', 'object', null),
        );
      }
      if (
        (result.isDualScreen || result.isFoldableScreen) &&
        (!result.verticalSpanned || !result.horizontalSpanned)
      ) {
        throw new Error(
          "Emulated device '" + result.title + "'has dual screen without spanned orientations",
        );
      }
      ...
      for (let i = 0; i < modes.length; ++i) {
        ...
        if (
          ...
          mode.orientation !== VerticalSpanned &&
        ...
        mode.insets = parseInsets(
          parseValue(modes[i], 'insets', 'object', { left: 0, top: 0, right: 0, bottom: 0 }),
        );
        if (
          mode.insets.top < 0 ||
          mode.insets.left < 0 ||
