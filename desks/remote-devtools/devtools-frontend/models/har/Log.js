...
export class Entry {
  ...
  static async build(request, options) {
    ...
    const connection =
      portPositionInString !== -1 ? ipAddress.substring(portPositionInString + 1) : undefined;
    ...
    if (options.sanitize) {
      ...
      entry.response.headers = entry.response.headers.filter(
        ({ name }) => !['set-cookie'].includes(name.toLocaleLowerCase()),
      );
      entry.request.cookies = [];
      entry.request.headers = entry.request.headers.filter(
        ({ name }) => !['authorization', 'cookie'].includes(name.toLocaleLowerCase()),
      );
    }
    // Chrome specific.
  ...
  buildResponse() {
    ...
    return {
      ...
      _serviceWorkerRouterRuleIdMatched:
        this.request.serviceWorkerRouterInfo?.ruleIdMatched ?? undefined,
      _serviceWorkerRouterMatchedSourceType:
        this.request.serviceWorkerRouterInfo?.matchedSourceType ?? undefined,
      _serviceWorkerRouterActualSourceType:
        this.request.serviceWorkerRouterInfo?.actualSourceType ?? undefined,
    };
  }
  buildContent() {
    const content = {
