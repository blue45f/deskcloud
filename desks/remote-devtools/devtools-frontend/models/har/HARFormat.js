...
class HARRequest extends HARBase {
  ...
  constructor(data) {
    ...
    this.cookies = Array.isArray(data['cookies'])
      ? data['cookies'].map((cookie) => new HARCookie(cookie))
      : [];
    this.headers = Array.isArray(data['headers'])
      ? data['headers'].map((header) => new HARHeader(header))
      : [];
    this.queryString = Array.isArray(data['queryString'])
      ? data['queryString'].map((qs) => new HARQueryString(qs))
      : [];
...
class HARResponse extends HARBase {
  ...
  constructor(data) {
    ...
    this.custom.set(
      ...
      HARBase.optionalString(data['_responseCacheStorageCacheName']),
    );
    this.custom.set(
      'serviceWorkerResponseSource',
      HARBase.optionalString(data['_serviceWorkerResponseSource']),
    );
    this.custom.set(
...
class HARPostData extends HARBase {
  ...
  constructor(data) {
    ...
    this.params = Array.isArray(data['params'])
      ? data['params'].map((param) => new HARParam(param))
      : [];
...
export class HARTimings extends HARBase {
  ...
  constructor(data) {
    ...
    this.custom.set(
      ...
      HARBase.optionalNumber(data['_workerRespondWithSettled']),
    );
    this.custom.set(
      'workerRouterEvaluationStart',
      HARBase.optionalNumber(data['_workerRouterEvaluationStart']),
    );
    this.custom.set(
      'workerCacheLookupStart',
      HARBase.optionalNumber(data['_workerCacheLookupStart']),
    );
  }
}
export class HARInitiator extends HARBase {
  ...
  constructor(data) {
    super(data);
    this.type =
      HARBase.optionalString(data['type']) ?? 'other' /* SDK.NetworkRequest.InitiatorType.OTHER */;
