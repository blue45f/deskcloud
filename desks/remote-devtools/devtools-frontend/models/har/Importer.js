...
export class Importer {
  ...
  static fillRequestFromHAREntry(request, entry, pageLoad) {
    ...
    const { mimeType, charset } = Platform.MimeType.parseContentType(
    ...
    if (
      ...
      const parser = new SDK.ServerSentEventProtocol.ServerSentEventsParser(
        onEvent,
        charset ?? undefined,
      );
    ...
    // Response cookies.
    const responseCookies = entry.response.cookies.map(
      this.fillCookieFromHARCookie.bind(this, 1 /* SDK.Cookie.Type.RESPONSE */),
    );
    ...
    const serviceWorkerResponseSource = entry.response.customAsString(
    ...
    const responseCacheStorageCacheName = entry.response.customAsString(
  ...
  static getResourceType(request, entry, pageLoad) {
    ...
    const resourceTypeFromMime = Common.ResourceType.ResourceType.fromMimeType(
      entry.response.content.mimeType,
    );
