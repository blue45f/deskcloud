...
export function calculateRequestTimeRanges(request, navigationStart) {
  ...
  if (!timing) {
    ...
    const hasDifferentIssueAndStartTime =
      request.issueTime() !== -1 &&
      request.startTime !== -1 &&
      request.issueTime() !== request.startTime;
    const middle =
      request.responseReceivedTime === -1
        ? hasDifferentIssueAndStartTime
          ? request.startTime
          : Number.MAX_VALUE
    ...
    const state =
      request.responseReceivedTime === -1
        ? 'connecting' /* RequestTimeRangeNames.CONNECTING */
        : 'receiving'; /* RequestTimeRangeNames.RECEIVING */
  ...
  addRange(
    'total' /* RequestTimeRangeNames.TOTAL */,
  ...
  if (timing.pushStart) {
    ...
    if (pushEnd > navigationStart) {
      addRange(
        'push' /* RequestTimeRangeNames.PUSH */,
        Math.max(timing.pushStart, navigationStart),
        pushEnd,
      );
    }
  }
  ...
  if (request.fetchedViaServiceWorker) {
    ...
    const blockingEnd =
      firstPositive([timing.dnsStart, timing.connectStart, timing.sendStart, responseReceived]) ||
      0;
    ...
    addOffsetRange(
      'waiting' /* RequestTimeRangeNames.WAITING */,
    ...
    if (timing.workerRouterEvaluationStart) {
      ...
      if (
        serviceWorkerRouterInfo?.matchedSourceType ===
          'cache' /* Protocol.Network.ServiceWorkerRouterSource.Cache */ &&
        timing.workerCacheLookupStart
      ) {
        routerEvaluationEnd = timing.workerCacheLookupStart;
      } else if (
        serviceWorkerRouterInfo?.actualSourceType ===
        'fetch-event' /* Protocol.Network.ServiceWorkerRouterSource.FetchEvent */
      ) {
  ...
  if (request.endTime !== -1) {
    addRange(
      timing.pushStart
        ? 'receiving-push' /* RequestTimeRangeNames.RECEIVING_PUSH */
        : 'receiving' /* RequestTimeRangeNames.RECEIVING */,
      request.responseReceivedTime,
      endTime,
    );
