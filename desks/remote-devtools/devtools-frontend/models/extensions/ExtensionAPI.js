...
globalThis.injectedExtensionAPI = function (
  ...
  EventSinkImpl.prototype = {
    addListener: function (callback) {
      ...
      if (this._listeners.length === 0) {
        extensionServer.sendRequest({
          command: 'subscribe' /* PrivateAPI.Commands.Subscribe */,
          type: this._type,
        });
      }
    ...
    removeListener: function (callback) {
      ...
      if (this._listeners.length === 0) {
        extensionServer.sendRequest({
          command: 'unsubscribe' /* PrivateAPI.Commands.Unsubscribe */,
          type: this._type,
        });
      }
    },
  ...
  function Network() {
    ...
    this.onRequestFinished = new (Constructor(EventSink))(
      'network-request-finished' /* PrivateAPI.Events.NetworkRequestFinished */,
    ...
    this.onNavigated = new (Constructor(EventSink))(
      'inspected-url-changed' /* PrivateAPI.Events.InspectedURLChanged */,
    );
  }
  Network.prototype = {
    getHAR: function (callback) {
      ...
      extensionServer.sendRequest(
        { command: 'getHAR' /* PrivateAPI.Commands.GetHAR */ },
        callback && callbackWrapper,
      );
    },
    addRequestHeaders: function (headers) {
      extensionServer.sendRequest({
        command: 'addRequestHeaders' /* PrivateAPI.Commands.AddRequestHeaders */,
        headers,
  ...
  RequestImpl.prototype = {
    getContent: function (callback) {
      ...
      extensionServer.sendRequest(
        { command: 'getRequestContent' /* PrivateAPI.Commands.GetRequestContent */, id: this._id },
        callback && callbackWrapper,
      );
    },
  };
  ...
  Panels.prototype = {
    create: function (title, _icon, page, callback) {
      ...
      extensionServer.sendRequest(
        { command: 'createPanel' /* PrivateAPI.Commands.CreatePanel */, id, title, page },
    ...
    setOpenResourceHandler: function (callback, urlScheme) {
      const hadHandler = extensionServer.hasHandler(
        'open-resource' /* PrivateAPI.Events.OpenResource */,
      );
      ...
      if (hadHandler === !callback) {
        extensionServer.sendRequest({
          command: 'setOpenResourceHandler' /* PrivateAPI.Commands.SetOpenResourceHandler */,
          handlerPresent: Boolean(callback),
          urlScheme,
        });
      }
    },
    setThemeChangeHandler: function (callback) {
      const hadHandler = extensionServer.hasHandler(
        'host-theme-change' /* PrivateAPI.Events.ThemeChange */,
      );
      function callbackWrapper(message) {
      ...
      if (hadHandler === !callback) {
        extensionServer.sendRequest({
          command: 'setThemeChangeHandler' /* PrivateAPI.Commands.SetThemeChangeHandler */,
          handlerPresent: Boolean(callback),
        });
      }
    },
    openResource: function (url, lineNumber, columnNumber, _callback) {
      ...
      extensionServer.sendRequest(
        {
          command: 'openResource' /* PrivateAPI.Commands.OpenResource */,
          url,
          lineNumber,
          columnNumber: columnNumberArg,
        },
        callbackArg,
      );
    },
  ...
  function ExtensionViewImpl(id) {
    ...
    if (id) {
      this.onShown = new (Constructor(EventSink))(
        'view-shown-' /* PrivateAPI.Events.ViewShown */ + id,
        dispatchShowEvent,
      );
      this.onHidden = new (Constructor(EventSink))(
        'view-hidden,' /* PrivateAPI.Events.ViewHidden */ + id,
      );
    }
  }
  function PanelWithSidebarImpl(hostPanelName) {
    ...
    this.onSelectionChanged = new (Constructor(EventSink))(
      'panel-objectSelected-' /* PrivateAPI.Events.PanelObjectSelected */ + hostPanelName,
    );
  }
  PanelWithSidebarImpl.prototype = {
    createSidebarPane: function (title, callback) {
      ...
      extensionServer.sendRequest(
        {
          command: 'createSidebarPane' /* PrivateAPI.Commands.CreateSidebarPane */,
  ...
  async function registerRecorderExtensionPluginImpl(plugin, pluginName, mediaType) {
    ...
    await new Promise((resolve) => {
      extensionServer.sendRequest(
        {
          command:
            'registerRecorderExtensionPlugin' /* PrivateAPI.Commands.RegisterRecorderExtensionPlugin */,
          pluginName,
          mediaType,
          capabilities,
          port: channel.port2,
  ...
  RecorderServicesAPIImpl.prototype = {
    ...
    unregisterRecorderExtensionPlugin: async function (plugin) {
      ...
      port.postMessage({
        event:
          'unregisteredRecorderExtensionPlugin' /* PrivateAPI.RecorderExtensionPluginEvents.UnregisteredRecorderExtensionPlugin */,
      });
      port.close();
    },
    createView: async function (title, pagePath) {
      ...
      await new Promise((resolve) => {
        extensionServer.sendRequest(
          {
            command: 'createRecorderView' /* PrivateAPI.Commands.CreateRecorderView */,
            id,
            title,
            pagePath,
          },
  ...
  LanguageServicesAPIImpl.prototype = {
    registerLanguageExtensionPlugin: async function (plugin, pluginName, supportedScriptTypes) {
        ...
        switch (request.method) {
          ...
          case 'getMappedLines' /* PrivateAPI.LanguageExtensionPluginCommands.GetMappedLines */:
            if ('getMappedLines' in plugin) {
              return plugin.getMappedLines(
                request.parameters.rawModuleId,
                request.parameters.sourceFileURL,
              );
            }
          ...
          case 'getProperties' /* PrivateAPI.LanguageExtensionPluginCommands.GetProperties */:
            ...
            if (!('evaluate' in plugin && plugin.evaluate)) {
              // If evalute is defined but the remote objects methods aren't, that's a bug
              return Promise.resolve(undefined);
            }
            break;
      ...
      await new Promise((resolve) => {
        extensionServer.sendRequest(
          {
            command:
              'registerLanguageExtensionPlugin' /* PrivateAPI.Commands.RegisterLanguageExtensionPlugin */,
            pluginName,
            port: channel.port2,
    ...
    unregisterLanguageExtensionPlugin: async function (plugin) {
      ...
      port.postMessage({
        event:
          'unregisteredLanguageExtensionPlugin' /* PrivateAPI.LanguageExtensionPluginEvents.UnregisteredLanguageExtensionPlugin */,
      });
      port.close();
    },
    getWasmLinearMemory: async function (offset, length, stopId) {
      const result = await new Promise((resolve) =>
        extensionServer.sendRequest(
          {
            command: 'getWasmLinearMemory' /* PrivateAPI.Commands.GetWasmLinearMemory */,
            offset,
            length,
            stopId,
          },
    ...
    getWasmLocal: async function (local, stopId) {
      return await new Promise((resolve) =>
        extensionServer.sendRequest(
          { command: 'getWasmLocal' /* PrivateAPI.Commands.GetWasmLocal */, local, stopId },
          resolve,
        ),
      );
    },
    getWasmGlobal: async function (global, stopId) {
      return await new Promise((resolve) =>
        extensionServer.sendRequest(
          { command: 'getWasmGlobal' /* PrivateAPI.Commands.GetWasmGlobal */, global, stopId },
          resolve,
        ),
      );
    },
    getWasmOp: async function (op, stopId) {
      return await new Promise((resolve) =>
        extensionServer.sendRequest(
          { command: 'getWasmOp' /* PrivateAPI.Commands.GetWasmOp */, op, stopId },
          resolve,
        ),
      );
    },
  ...
  NetworkPanelImpl.prototype = {
    show: function (options) {
      return new Promise((resolve) =>
        extensionServer.sendRequest(
          {
            command: 'showNetworkPanel' /* PrivateAPI.Commands.ShowNetworkPanel */,
  ...
  function PerformanceImpl() {
    ...
    this.onProfilingStarted = new (Constructor(EventSink))(
      'profiling-started-' /* PrivateAPI.Events.ProfilingStarted */,
      dispatchProfilingStartedEvent,
    );
    this.onProfilingStopped = new (Constructor(EventSink))(
      'profiling-stopped-' /* PrivateAPI.Events.ProfilingStopped */,
      dispatchProfilingStoppedEvent,
    );
  }
  ...
  function ExtensionPanelImpl(id) {
    ...
    this.onSearch = new (Constructor(EventSink))(
      'panel-search-' /* PrivateAPI.Events.PanelSearch */ + id,
    );
  }
  ExtensionPanelImpl.prototype = {
    ...
    show: function () {
      ...
      extensionServer.sendRequest({
        command: 'showPanel' /* PrivateAPI.Commands.ShowPanel */,
        id: this._id,
      });
    },
    __proto__: ExtensionViewImpl.prototype,
  ...
  RecorderViewImpl.prototype = {
    show: function () {
      ...
      extensionServer.sendRequest({
        command: 'showRecorderView' /* PrivateAPI.Commands.ShowRecorderView */,
        id: this._id,
      });
    },
    __proto__: ExtensionViewImpl.prototype,
  ...
  ExtensionSidebarPaneImpl.prototype = {
    setHeight: function (height) {
      extensionServer.sendRequest({
        command: 'setSidebarHeight' /* PrivateAPI.Commands.SetSidebarHeight */,
        id: this._id,
        height,
      });
    },
    ...
    setPage: function (page) {
      extensionServer.sendRequest({
        command: 'setSidebarPage' /* PrivateAPI.Commands.SetSidebarPage */,
        id: this._id,
        page,
      });
    },
  ...
  function ButtonImpl(id) {
    this._id = id;
    this.onClicked = new (Constructor(EventSink))(
      'button-clicked-' /* PrivateAPI.Events.ButtonClicked */ + id,
    );
  }
  ...
  function InspectedWindow() {
    ...
    this.onResourceAdded = new (Constructor(EventSink))(
      'resource-added' /* PrivateAPI.Events.ResourceAdded */,
      dispatchResourceEvent,
    );
    this.onResourceContentCommitted = new (Constructor(EventSink))(
      'resource-content-committed' /* PrivateAPI.Events.ResourceContentCommitted */,
  ...
  InspectedWindow.prototype = {
    ...
    getResources: function (callback) {
      ...
      extensionServer.sendRequest(
        { command: 'getPageResources' /* PrivateAPI.Commands.GetPageResources */ },
        callback && callbackWrapper,
      );
    },
  };
  ...
  ResourceImpl.prototype = {
    ...
    getContent: function (callback) {
      ...
      extensionServer.sendRequest(
        {
          command: 'getResourceContent' /* PrivateAPI.Commands.GetResourceContent */,
          url: this._url,
        },
        callback && callbackWrapper,
      );
    ...
    setContent: function (content, commit, callback) {
      extensionServer.sendRequest(
        {
          command: 'setResourceContent' /* PrivateAPI.Commands.SetResourceContent */,
          url: this._url,
          content,
          commit,
        },
    ...
    setFunctionRangesForScript: function (ranges) {
      return new Promise((resolve, reject) =>
        extensionServer.sendRequest(
          {
            command:
              'setFunctionRangesForScript' /* PrivateAPI.Commands.SetFunctionRangesForScript */,
            scriptUrl: this._url,
            ranges,
          },
          (response) => {
    ...
    attachSourceMapURL: function (sourceMapURL) {
      return new Promise((resolve, reject) =>
        extensionServer.sendRequest(
          {
            command:
              'attachSourceMapToResource' /* PrivateAPI.Commands.AttachSourceMapToResource */,
            contentUrl: this._url,
            sourceMapURL,
          },
          (response) => {
            const result = response;
  ...
  function forwardEventQueue() {
    forwardTimer = null;
    extensionServer.sendRequest({
      command: '_forwardKeyboardEvent' /* PrivateAPI.Commands.ForwardKeyboardEvent */,
  ...
  const extensionServer = new (Constructor(ExtensionServerClient))(
    targetWindowForTest || globalThis.parent,
  );
...
globalThis.buildExtensionAPIInjectedScript = function (
  ...
  const argumentsJSON = [extensionInfo, inspectedTabId || null, themeName, keysToForward]
