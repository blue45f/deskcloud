...
function parseHostAndPort(pattern, scheme) {
  ...
  try {
    asUrl = new URL(
      pattern.startsWith(SUBDOMAIN_WILDCARD)
        ? `http://${pattern.substr(SUBDOMAIN_WILDCARD.length)}`
        : `http://${pattern}`,
    );
  } catch {
    return undefined;
...
export class HostUrlPattern {
  ...
  matchesUrl(url) {
    ...
    return (
      this.matchesScheme(scheme) &&
      this.matchesHost(parsedUrl.hostname) &&
      (!port || this.matchesPort(port))
    );
  }
  ...
  matchesHost(host) {
    ...
    if (this.pattern.host.startsWith('*.')) {
      return (
        normalizedHost === this.pattern.host.substr(2) ||
        normalizedHost.endsWith(this.pattern.host.substr(1))
      );
    }
