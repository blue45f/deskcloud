...
export class UserBadges extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    this.#allBadges = UserBadges.BADGE_REGISTRY.map(
      (badgeCtor) =>
        new badgeCtor({
  ...
  async #onTriggerBadge(badge, opts) {
    ...
    const delay = opts?.immediate
      ? 0
      : Math.max(DELAY_BEFORE_TRIGGER - timeElapsedAfterTriggerCall, 0);
    setTimeout(() => {
      this.dispatchEventToListeners('BadgeTriggered' /* Events.BADGE_TRIGGERED */, {
        badge,
        reason,
      });
    }, delay);
  ...
  async #reconcileBadges() {
    ...
    for (const badge of this.#allBadges) {
      ...
      const shouldActivateStarterBadge =
        badge.isStarterBadge &&
        isEligibleToCreateProfile &&
        Host.GdpClient.isStarterBadgeEnabled() &&
        !this.#isStarterBadgeDismissed() &&
        !this.#isStarterBadgeSnoozed();
      const shouldActivateActivityBasedBadge =
        !badge.isStarterBadge && hasGdpProfile && receiveBadgesSettingEnabled;
