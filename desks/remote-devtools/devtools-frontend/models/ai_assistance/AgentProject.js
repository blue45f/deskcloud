...
export class AgentProject {
  ...
  async readFile(filepath) {
    ...
    const content = uiSourceCode.isDirty()
      ? uiSourceCode.workingCopyContentData()
  ...
  getLinesChanged(currentContent, updatedContent) {
    ...
    if (currentContent) {
      const diff = Diff.Diff.DiffWrapper.lineDiff(
        updatedContent.split(LINE_END_RE),
        currentContent.split(LINE_END_RE),
      );
  ...
  async searchFiles(query, caseSensitive, isRegex, { signal } = {}) {
    ...
    for (const [filepath, file] of map.entries()) {
      ...
      const content = file.isDirty()
        ? file.workingCopyContentData()
