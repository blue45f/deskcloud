...
function getLCPData(parsedTrace, frameId, navigation) {
  const navMetrics = parsedTrace.data.PageLoadMetrics.metricScoresByFrameId
    .get(frameId)
    ?.get(navigation);
  if (!navMetrics) {
    return null;
  ...
  const metric = navMetrics.get(
    'LCP' /* Trace.Handlers.ModelHandlers.PageLoadMetrics.MetricName.LCP */,
  );
  ...
  return {
    lcpEvent,
    lcpRequest: navigationId
      ? parsedTrace.data.LargestImagePaint.lcpRequestByNavigationId.get(navigationId)
      : undefined,
    metricScore: metric,
  };
}
export class PerformanceInsightFormatter {
  ...
  #formatUrl(url) {
    const request = this.#parsedTrace.data.NetworkRequests.byTime.find(
      (request) => request.args.data.url === url,
    );
    if (request) {
  ...
  #lcpMetricSharedContext() {
    ...
    if (lcpRequest) {
      parts.push(
        `${theLcpElement} is an image fetched from ${this.#formatRequestUrl(lcpRequest)}.`,
      );
      const request = this.#traceFormatter.formatNetworkRequests([lcpRequest], {
        verbose: true,
  ...
  #formatLayoutShift(shift, index, rootCauses) {
    ...
    if (rootCauses) {
      rootCauses.iframes.forEach((iframe) =>
      ...
      rootCauses.webFonts.forEach((req) => {
        potentialRootCauses.push(
          `- A font that was loaded over the network: ${this.#formatRequestUrl(req)}.`,
        );
      });
      rootCauses.nonCompositedAnimations.forEach((nonCompositedFailure) => {
        ...
        potentialRootCauses.push(
          `- non-composited animation: \`${nonCompositedFailure.name || '(unnamed)'}\``,
        );
    ...
    const startTime = Trace.Helpers.Timing.microToMilli(
      Trace.Types.Timing.Micro(shift.ts - baseTime),
    );
    const impactedNodeNames =
      shift.rawSourceEvent.args.data?.impacted_nodes
        ?.map((n) => n.debug_name)
...
${checklistBulletPoints.map((point) => `- ${point.name}: ${point.passed ? 'PASSED' : 'FAILED'}`).join('\n')}`;
  ...
  formatDomSizeInsight(insight) {
    ...
    if (insight.maxDOMStats) {
      ...
      output +=
        Trace.Insights.Models.DOMSize.UIStrings.totalElements +
        ': ' +
        insight.maxDOMStats.args.data.totalElements +
        '.\n';
      if (maxDepthStats) {
        output +=
          Trace.Insights.Models.DOMSize.UIStrings.maxDOMDepth +
          ': ' +
          maxDepthStats.depth +
          ` nodes, starting with element '${maxDepthStats.nodeName}'` +
          ' (node id: ' +
          maxDepthStats.nodeId +
          ').\n';
      }
      if (maxChildrenStats) {
        output +=
          Trace.Insights.Models.DOMSize.UIStrings.maxChildren +
          ': ' +
          maxChildrenStats.numChildren +
          `, for parent '${maxChildrenStats.nodeName}'` +
          ' (node id: ' +
          maxChildrenStats.nodeId +
          ').\n';
      }
    }
...
Duplication grouped by Node modules: ${filesFormatted}`;
  ...
  formatForcedReflowInsight(insight) {
    ...
    if (insight.aggregatedBottomUpData.length > 0) {
      output +=
        '\n' +
        Trace.Insights.Models.ForcedReflow.UIStrings.reflowCallFrames +
...
3. Presentation delay: ${this.#formatMicro(event.presentationDelay)}.`;
  ...
  formatLcpBreakdownInsight(insight) {
    ...
    Object.values(subparts).forEach((subpart) => {
      ...
      phaseBulletPoints.push({
        name: subpart.label,
        value: this.#formatMilli(phaseMilli),
        percentage,
      });
    });
    return `${this.#lcpMetricSharedContext()}
...
${phaseBulletPoints
  .map((phase) => `- ${phase.name}: ${phase.value} (${phase.percentage}% of total LCP time)`)
  .join('\n')}`;
  }
  /**
   * Create an AI prompt string out of the LCP Brekdown Insight model to use with Ask AI.
...
${filesFormatted}`;
  ...
  formatModernHttpInsight(insight) {
    const requestSummary =
      insight.http1Requests.length === 1
...
${requestSummary}`;
  ...
  - ${origin.url}
    ...
    if (
      insight.preconnectCandidates.length > 0 &&
      insight.preconnectedOrigins.length <
        Trace.Insights.Models.NetworkDependencyTree.TOO_MANY_PRECONNECTS_THRESHOLD
    ) {
  ...
  formatRenderBlockingInsight(insight) {
    const requestSummary = this.#traceFormatter.formatNetworkRequests(
      insight.renderBlockingRequests,
    );
    if (requestSummary.length === 0) {
...
${requestSummary}`;
  ...
  formatThirdPartiesInsight(insight) {
    ...
    const thirdPartyTransferSizeEntries = entitySummaries
      .filter((s) => s.entity !== firstPartyEntity)
      .toSorted((a, b) => b.transferSize - a.transferSize);
    const thirdPartyMainThreadTimeEntries = entitySummaries
      .filter((s) => s.entity !== firstPartyEntity)
  ...
  formatCharacterSetInsight(insight) {
    ...
    if (insight.data) {
      output +=
        ...
        (insight.data.hasHttpCharset ? 'present' : 'missing') +
        '.\n';
      output +=
        'HTML meta charset disposition: ' +
        (insight.data.metaCharsetDisposition ?? 'unknown') +
        '.\n';
      if (
        !insight.data.hasHttpCharset &&
        insight.data.metaCharsetDisposition !== 'found-in-first-1024-bytes'
      ) {
        output +=
  ...
  formatViewportInsight(insight) {
    ...
    output +=
      'The webpage is ' +
      (insight.mobileOptimized ? 'already' : 'not') +
