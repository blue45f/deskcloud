...
export class PerformanceTraceFormatter {
  ...
  #getCruxTraceSummary(insightSet) {
    ...
    try {
      ...
      const parts = [];
      const fieldMetrics = Trace.Insights.Common.getFieldMetricsForInsightSet(
        insightSet,
        this.#parsedTrace.metadata,
        cruxScope,
      );
      ...
      if (fieldLcp || fieldInp || fieldCls) {
        ...
        if (fieldLcp) {
          ...
          if (
            fieldLcpBreakdown &&
            (fieldLcpBreakdown.ttfb ||
              fieldLcpBreakdown.loadDelay ||
              fieldLcpBreakdown.loadDuration ||
            ...
            if (fieldLcpBreakdown.loadDelay) {
              parts.push(
                `    - Load delay: ${serializeFieldMetricTimingResult(fieldLcpBreakdown.loadDelay)}`,
              );
            }
            if (fieldLcpBreakdown.loadDuration) {
              parts.push(
                `    - Load duration: ${serializeFieldMetricTimingResult(fieldLcpBreakdown.loadDuration)}`,
              );
            }
            if (fieldLcpBreakdown.renderDelay) {
              parts.push(
                `    - Render delay: ${serializeFieldMetricTimingResult(fieldLcpBreakdown.renderDelay)}`,
              );
            }
          }
        }
  ...
  formatTraceSummary() {
    ...
    parts.push(
      'CPU throttling: ' +
        (traceMetadata.cpuThrottling ? `${traceMetadata.cpuThrottling}x` : 'none'),
    );
    ...
    for (const insightSet of parsedTrace.insights?.values() ?? []) {
      ...
      if (lcp || cls || inp) {
        ...
        if (inp) {
          parts.push(
            `  - INP: ${Math.round(inp.value / 1000)} ms, event: ${this.serializeEvent(inp.event)}`,
          );
        }
        if (cls) {
          const eventText = cls.worstClusterEvent
            ? `, event: ${this.serializeEvent(cls.worstClusterEvent)}`
            : '';
  ...
  formatCriticalRequests() {
    ...
    return this.#formatFactByInsightSet({
      ...
      cb: async (insightSet) => {
        ...
        return criticalRequests.length
  ...
  async #serializeBottomUpRootNode(rootNode, limit) {
    ...
    return (
      topNodes.map((node) => nodeToText.call(this, node)).join('\n') +
  ...
  #formatThirdPartyEntitySummaries(summaries) {
    const topMainThreadTimeEntries = summaries
  ...
  formatThirdPartySummary() {
    ...
    return this.#formatFactByInsightSet({
      ...
      cb: async (insightSet) => {
        const thirdPartySummaries = Trace.Extras.ThirdParties.summarizeByThirdParty(
          parsedTrace.data,
          insightSet.bounds,
        );
        return thirdPartySummaries.length
          ? this.#formatThirdPartyEntitySummaries(thirdPartySummaries)
          : null;
      },
    });
  }
  formatLongestTasks() {
    ...
    return this.#formatFactByInsightSet({
      ...
      empty: 'none',
      cb: async (insightSet) => {
        const longestTaskTrees = AIQueries.longestTasks(
          insightSet.navigation?.args.data?.navigationId,
          insightSet.bounds,
          parsedTrace,
          3,
  ...
  #serializeRelatedInsightsForEvents(events) {
    ...
    if (this.#insightSet) {
      for (const model of Object.values(this.#insightSet.model)) {
        ...
        const modeRelatedEvents = Array.isArray(model.relatedEvents)
          ? model.relatedEvents
          : [...model.relatedEvents.keys()];
    ...
    for (const [insightKey, events] of insightNameToRelatedEvents) {
      ...
      const eventsString = events
        .slice(0, 5)
        .map((e) => Trace.Name.forEntry(e) + ' ' + this.serializeEvent(e))
        .join(', ');
  ...
  async formatMainThreadTrackSummary(bounds) {
    ...
    const results = [];
    const insightSet = this.#parsedTrace.insights
      ?.values()
      .find((insightSet) =>
        Trace.Helpers.Timing.boundsIncludeTimeRange({ bounds, timeRange: insightSet.bounds }),
      );
    ...
    const relatedInsightsText = this.#serializeRelatedInsightsForEvents([
      ...(topDownTree?.rootNode.events ?? []),
      ...(bottomUpRootNode?.events ?? []),
    ]);
  ...
  formatNetworkTrackSummary(bounds) {
    const results = [];
    const requests = this.#parsedTrace.data.NetworkRequests.byTime.filter((request) =>
      Trace.Helpers.Timing.eventIsInBounds(request, bounds),
    );
  ...
  #networkRequestVerbosely(request, options) {
    ...
    const renderBlocking =
      Trace.Helpers.Network.isSyntheticNetworkRequestEventRenderBlocking(request);
...
`;
  ...
  #networkRequestCompressedFormat(urlIndex, request, urlIdToIndex) {
    ...
    const mainThreadProcessingDuration = micros(
      request.ts + request.dur - syntheticData.finishTime,
    );
    const renderBlocking = Trace.Helpers.Network.isSyntheticNetworkRequestEventRenderBlocking(
      request,
    )
      ? 't'
      : 'f';
    ...
    const headerValues = responseHeaders
      ?.map((header) => {
        const value = NetworkRequestFormatter.allowHeader(header.name)
          ? header.value
          : '<redacted>';
    ...
    const redirects = request.args.data.redirects
      ...
      .join(',');
    const initiators = this.#getInitiatorChain(parsedTrace, request);
    const initiatorUrlIndices = initiators.map((initiator) =>
      this.#getOrAssignUrlIndex(urlIdToIndex, initiator.args.data.url),
    );
    const parts = [
      urlIndex,
  ...
  #formatFunctionCode(code) {
    ...
    const parts = [];
    parts.push(
      `${name} @ ${url}:${startLine}:${startColumn}. With added context, chunk is from ${contextStartLine}:${contextStartColumn} to ${contextEndLine}:${contextEndColumn}`,
    );
    parts.push('```');
  ...
  async #serializeRelevantFunctions(callFrames) {
    ...
    const functionCodes = await Promise.all(
      callFrames.map((frame) =>
        resolveFunctionCode(frame.url, frame.lineNumber, frame.columnNumber),
      ),
    );
    ...
    return (
      ...
      [
        ...
        functionCodeStrings.length > 1
          ? `Here are ${functionCodeStrings.length} relevant functions:`
