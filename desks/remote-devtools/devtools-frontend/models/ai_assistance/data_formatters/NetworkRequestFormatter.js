...
export class NetworkRequestFormatter {
  ...
  static formatFailureReasons(reasons) {
    ...
    if (reasons.corsErrorStatus) {
      lines.push(
        `CORS error: ${reasons.corsErrorStatus.corsError} ${reasons.corsErrorStatus.failedParameter}`,
      );
    }
...
Request initiator chain:\n${this.formatRequestInitiatorChain()}`;
  ...
  formatRequestInitiatorChain() {
    ...
    for (const initiator of Array.from(graph.initiators).reverse()) {
      initiatorChain =
        initiatorChain + lineStart + _a.formatInitiatorUrl(initiator.url(), allowedOrigin) + '\n';
      lineStart = '\t' + lineStart;
      if (initiator === this.#request) {
        initiatorChain = this.#formatRequestInitiated(
          graph.initiated,
          this.#request,
          initiatorChain,
          lineStart,
  ...
  formatNetworkRequestTiming() {
    const results = NetworkTimeCalculator.calculateRequestTimeRanges(
      this.#request,
