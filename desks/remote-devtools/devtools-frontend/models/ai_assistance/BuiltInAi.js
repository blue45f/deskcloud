...
export class BuiltInAi extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    this.initDoneForTesting = this.getLanguageModelAvailability()
      .then(() => this.#sendAvailabilityMetrics())
  ...
  isEventuallyAvailable() {
    if (
      !this.#hasGpu &&
      !Boolean(Root.Runtime.hostConfig.devToolsConsoleInsightsTeasers?.allowWithoutGpu)
    ) {
      return false;
    }
    return (
      this.#availability === 'available' /* LanguageModelAvailability.AVAILABLE */ ||
  ...
  #setDownloadProgress(newValue) {
    ...
    this.dispatchEventToListeners(
      'downloadProgressChanged' /* Events.DOWNLOAD_PROGRESS_CHANGED */,
  ...
  - Your response should be concise and to the point. Avoid lengthy explanations
    or unnecessary details.
      ...
      if (this.#availability !== 'available' /* LanguageModelAvailability.AVAILABLE */) {
        this.dispatchEventToListeners(
          'downloadedAndSessionCreated' /* Events.DOWNLOADED_AND_SESSION_CREATED */,
        );
        void this.getLanguageModelAvailability();
      }
    } catch (e) {
    ...
    if (this.#hasGpu) {
      switch (this.#availability) {
        case 'unavailable' /* LanguageModelAvailability.UNAVAILABLE */:
          Host.userMetrics.builtInAiAvailability(
            0 /* Host.UserMetrics.BuiltInAiAvailability.UNAVAILABLE_HAS_GPU */,
          );
          break;
        case 'downloadable' /* LanguageModelAvailability.DOWNLOADABLE */:
          Host.userMetrics.builtInAiAvailability(
            1 /* Host.UserMetrics.BuiltInAiAvailability.DOWNLOADABLE_HAS_GPU */,
          );
          break;
        case 'downloading' /* LanguageModelAvailability.DOWNLOADING */:
          Host.userMetrics.builtInAiAvailability(
            2 /* Host.UserMetrics.BuiltInAiAvailability.DOWNLOADING_HAS_GPU */,
          );
          break;
        case 'available' /* LanguageModelAvailability.AVAILABLE */:
          Host.userMetrics.builtInAiAvailability(
            3 /* Host.UserMetrics.BuiltInAiAvailability.AVAILABLE_HAS_GPU */,
          );
          break;
        case 'disabled' /* LanguageModelAvailability.DISABLED */:
          Host.userMetrics.builtInAiAvailability(
            4 /* Host.UserMetrics.BuiltInAiAvailability.DISABLED_HAS_GPU */,
          );
          break;
      }
    } else {
      switch (this.#availability) {
        case 'unavailable' /* LanguageModelAvailability.UNAVAILABLE */:
          Host.userMetrics.builtInAiAvailability(
            5 /* Host.UserMetrics.BuiltInAiAvailability.UNAVAILABLE_NO_GPU */,
          );
          break;
        case 'downloadable' /* LanguageModelAvailability.DOWNLOADABLE */:
          Host.userMetrics.builtInAiAvailability(
            6 /* Host.UserMetrics.BuiltInAiAvailability.DOWNLOADABLE_NO_GPU */,
          );
          break;
        case 'downloading' /* LanguageModelAvailability.DOWNLOADING */:
          Host.userMetrics.builtInAiAvailability(
            7 /* Host.UserMetrics.BuiltInAiAvailability.DOWNLOADING_NO_GPU */,
          );
          break;
        case 'available' /* LanguageModelAvailability.AVAILABLE */:
          Host.userMetrics.builtInAiAvailability(
            8 /* Host.UserMetrics.BuiltInAiAvailability.AVAILABLE_NO_GPU */,
          );
          break;
        case 'disabled' /* LanguageModelAvailability.DISABLED */:
          Host.userMetrics.builtInAiAvailability(
            9 /* Host.UserMetrics.BuiltInAiAvailability.DISABLED_NO_GPU */,
          );
          break;
      }
    }
