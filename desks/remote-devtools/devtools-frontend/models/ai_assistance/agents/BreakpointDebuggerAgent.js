...
export class BreakpointDebuggerAgent extends AiAgent {
  ...
  constructor(opts) {
    ...
    this.declareFunction('resume', {
      ...
      handler: async () => {
        ...
        const debuggerModel = targetManager
  ...
  async #getFunctionSource(args) {
    ...
    while (currentNode) {
      if (
        currentNode.kind === 2 /* ScopeKind.FUNCTION */ ||
        currentNode.kind === 4 /* ScopeKind.ARROW_FUNCTION */
      ) {
        functionNode = currentNode;
      }
      const child = currentNode.children.find(
        (c) => c.start <= selectedOffset && c.end > selectedOffset,
      );
      currentNode = child;
    }
    ...
    return {
      result: {
        functionSource: this.#formatLines(
          text,
  ...
  async #getCallStack() {
    ...
    const callFrames = stackTrace.syncFragment.frames.map((frame) => {
      return {
        ...
        url: frame.uiSourceCode
          ? frame.uiSourceCode.url()
          : frame.url || frame.sdkFrame.script.contentURL(),
          ...
          for (const prop of properties) {
            ...
            if (prop.value) {
              if (prop.value.type === 'string') {
                ...
                const props = prop.value.preview.properties
  ...
  async #setBreakpoint(args) {
    ...
    const breakpointLocations =
      Breakpoints.BreakpointManager.BreakpointManager.instance().breakpointLocationsForUISourceCode(
        uiSourceCode,
      );
    const alreadyExists = breakpointLocations.some(
      (bp) => bp.uiLocation.lineNumber === args.lineNumber - 1,
    );
    if (alreadyExists) {
      return { result: { status: `Breakpoint already exists at ${args.url}:${args.lineNumber}.` } };
    }
    const breakpoint =
      await Breakpoints.BreakpointManager.BreakpointManager.instance().setBreakpoint(
        uiSourceCode,
        args.lineNumber - 1,
        0,
    ...
    if (actualLineNumber !== args.lineNumber) {
      return {
        result: {
          status: `Breakpoint requested at ${args.url}:${args.lineNumber}, but ACTUALLY resolved to line ${actualLineNumber}.${
            warning
              ? '\n' + warning
  ...
  async #getExecutionLocation() {
    ...
    const url = currentFrame.uiSourceCode
      ? currentFrame.uiSourceCode.url()
  ...
  async enhanceQuery(query, selectedBreakpoint) {
    ...
    const locationPart = `I am investigating a breakpoint that is already set at ${item.uiSourceCode.url()}:${item.lineNumber + 1}${
