...
export class PerformanceTraceContext extends ConversationContext {
  ...
  async getSuggestions() {
    ...
    if (insightSet) {
      ...
      const GOOD =
        'good'; /* Trace.Handlers.ModelHandlers.PageLoadMetrics.ScoreClassification.GOOD */
      const poorMetrics = new Set();
      if (
        lcp &&
        ModelHandlers.PageLoadMetrics.scoreClassificationForLargestContentfulPaint(lcp.value) !==
          GOOD
      ) {
      ...
      if (
        inp &&
        ModelHandlers.UserInteractions.scoreClassificationForInteractionToNextPaint(inp.value) !==
          GOOD
      ) {
...
export class PerformanceAgent extends AiAgent {
  ...
  #parseForKnownUrls(response) {
    ...
    return response.replace(
      urlRegex,
      (match, markdownLink, linkText, linkDest, standaloneUrlText) => {
        if (markdownLink) {
        ...
        const request = focus.parsedTrace.data.NetworkRequests.byTime.find(
          (request) => request.args.data.url === urlText,
        );
        if (!request) {
          return match;
        }
  ...
  async enhanceQuery(query, context) {
    ...
    if (focus.event) {
      ...
      if (includeEventInfo) {
        selected.push(
          `User selected an event ${this.#formatter?.serializeEvent(focus.event)}.\n\n`,
        );
      }
    }
    if (focus.callTree) {
  ...
  #createFactForTraceSummary() {
    ...
    this.#traceFacts.push({
      text: `Trace summary:\n${text}`,
      metadata: { source: 'devtools', score: ScorePriority.REQUIRED },
    });
  }
  ...
  #declareFunctions(context) {
      ...
      handler: async (params) => {
        ...
        if (!insightSet) {
          const valid = [...(parsedTrace.insights?.values() ?? [])]
            .map(
              (insightSet) =>
                `id: ${insightSet.id}, url: ${insightSet.url}, bounds: ${this.#formatter?.serializeBounds(insightSet.bounds)}`,
            )
            .join('; ');
        ...
        const widgets = [];
        if (
          Trace.Insights.Models.LCPDiscovery.isLCPDiscoveryInsight(insight) ||
                ...
                if (node) {
                  ...
                  if (lcpSyntheticRequest) {
                    networkRequest = {
                      ...
                      size:
                        lcpSyntheticRequest.args.data.decodedBodyLength ??
                        lcpSyntheticRequest.args.data.encodedDataLength ??
                        0,
    ...
    this.declareFunction('getEventByKey', {
      ...
      displayInfoFromArgs: (params) => {
        return {
          title: lockedString('Looking at trace event'),
          action: `getEventByKey('${params.eventKey}')`,
        };
      },
      handler: async (params) => {
    ...
    this.declareFunction('getDetailedCallTree', {
      ...
      displayInfoFromArgs: (args) => {
        return {
          title: lockedString('Looking at call tree'),
          action: `getDetailedCallTree('${args.eventKey}')`,
        };
      },
      handler: async (args) => {
    ...
    this.declareFunction('getFunctionCode', {
      ...
      handler: async (args) => {
        ...
        const code = await this.#formatter.resolveFunctionCodeAtLocation(
          url,
          args.line,
          args.column,
        );
    ...
    this.declareFunction('getResourceContent', {
      ...
      displayInfoFromArgs: (args) => {
        return {
          title: lockedString('Looking at resource content'),
          action: `getResourceContent('${args.url}')`,
        };
      },
      handler: async (args) => {
    ...
    if (!context.external) {
      this.declareFunction('selectEventByKey', {
        ...
        displayInfoFromArgs: (params) => {
          return {
            title: lockedString('Selecting event'),
            action: `selectEventByKey('${params.eventKey}')`,
          };
        },
        handler: async (params) => {
  ...
  async addElementAnnotation(elementId, annotationMessage) {
    ...
    console.log(
      `AI AGENT EVENT: Performance Agent adding annotation for element ${elementId}: '${annotationMessage}'`,
    );
  ...
  async addNetworkRequestAnnotation(eventKey, annotationMessage) {
    ...
    Annotations.AnnotationRepository.instance().addNetworkRequestAnnotation(
      annotationMessage,
      requestId,
    );
    return { result: { success: true } };
