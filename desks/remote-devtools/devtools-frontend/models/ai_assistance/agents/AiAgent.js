...
export class AiAgent {
  ...
  buildRequest(part, role) {
    ...
    const request = {
      ...
      metadata: {
        ...
        user_tier: userTier,
        client_version:
          Root.Runtime.getChromeVersion() +
          this.preambleFeatures()
            .map((feature) => `+${feature}`)
            .join(''),
      },
      functionality_type: enableAidaFunctionCalling
        ? Host.AidaClient.FunctionalityType.AGENTIC_CHAT
  ...
  parseTextResponseForSuggestions(text) {
    ...
    if (!suggestions && answerLines.at(-1)?.includes('SUGGESTIONS:')) {
      const [answer, suggestionsText] = answerLines[answerLines.length - 1].split(
        'SUGGESTIONS:',
        2,
      );
      try {
  ...
  async *run(initialQuery, options, multimodalInput) {
    ...
    query = multimodalInput
      ? [{ text: enhancedQuery }, multimodalInput.input]
      : [{ text: enhancedQuery }];
  ...
  #removeLastRunParts() {
    this.#history.splice(
      this.#history.findLastIndex((item) => {
