...
const MULTIMODAL_ENHANCEMENT_PROMPTS = {
  ['screenshot' /* MultimodalInputType.SCREENSHOT */]:
    promptForScreenshot + considerationsForMultimodalInputEvaluation,
  ['uploaded-image' /* MultimodalInputType.UPLOADED_IMAGE */]:
...
export class NodeContext extends ConversationContext {
  ...
  async getSuggestions() {
    const layoutProps = await this.#node
      .domModel()
      .cssModel()
      .getLayoutPropertiesFromComputedStyle(this.#node.id);
    if (!layoutProps) {
      return;
    }
...
export class StylingAgent extends AiAgent {
  ...
  get userTier() {
    const greenDevEmulationEnabled =
      Greendev.Prototypes.instance().isEnabled('emulationCapabilities');
    return greenDevEmulationEnabled
      ? 'TESTERS'
      : Root.Runtime.hostConfig.devToolsFreestyler?.userTier;
  }
  get executionMode() {
    return (
      Root.Runtime.hostConfig.devToolsFreestyler?.executionMode ??
  ...
  constructor(opts) {
    ...
    this.#createExtensionScope =
      opts.createExtensionScope ??
      ((changes) => {
        return new ExtensionScope(
          changes,
          this.sessionId,
          this.context?.getItem() ?? null,
          this.#currentTurnId,
        );
      });
...
**CRITICAL** Always provide the explanation argument to explain what and why you query.`,
    ...
    if (Annotations.AnnotationRepository.annotationsEnabled()) {
      this.declareFunction('addElementAnnotation', {
        description:
          'Adds a visual annotation in the Elements panel, attached to a node with ' +
...
* Its selector is \`${element.simpleSelector()}\``;
    ...
    if (childNodes) {
      const textChildNodes = childNodes.filter(
        (childNode) => childNode.nodeType() === Node.TEXT_NODE,
      );
      const elementChildNodes = childNodes.filter(
    ...
    if (parentNode) {
      ...
      const elementOrNodeElementNodeText =
        parentNode.nodeType() === Node.ELEMENT_NODE ? 'an element' : 'a non element';
      output += `\n* Its parent is ${elementOrNodeElementNodeText} node`;
      if (parentNode.isShadowRoot()) {
        output += '\n* Its parent is a shadow root.';
      }
      if (parentChildrenNodes) {
        const childElementNodes = parentChildrenNodes.filter(
          (siblingNode) => siblingNode.nodeType() === Node.ELEMENT_NODE,
        );
        switch (childElementNodes.length) {
          ...
          case 1:
            ...
            break;
          default:
            output += `\n* Its parent has ${childElementNodes.length} child element nodes: ${childElementNodes
              .map((node) => `\`${node.simpleSelector()}\` (uid=${node.backendNodeId()})`)
              .join(', ')}`;
            break;
        }
        const siblingTextNodes = parentChildrenNodes.filter(
          (siblingNode) => siblingNode.nodeType() === Node.TEXT_NODE,
        );
  ...
  async activateDeviceEmulation(deviceName, visionDeficiency) {
    ...
    // eslint-disable-next-line no-console
    console.log(
      'activateDeviceEmulation called with device:',
      deviceName,
      'visionDeficiency:',
      visionDeficiency,
    );
    ...
    if (selectedNode) {
      try {
        ...
        const result = await this.#execJs(heightJs, {
    ...
    try {
      if (selectedNode) {
        const accessibilityModel = selectedNode
          .domModel()
          .target()
          .model(SDK.AccessibilityModel.AccessibilityModel);
  ...
  async enhanceQuery(query, selectedElement, multimodalInputType) {
    let multimodalInputEnhancementQuery =
      this.multimodalInputEnabled && multimodalInputType
        ? MULTIMODAL_ENHANCEMENT_PROMPTS[multimodalInputType]
        : '';
