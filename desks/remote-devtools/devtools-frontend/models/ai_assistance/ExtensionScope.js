...
export class ExtensionScope {
  ...
  static getStyleRuleFromMatchesStyles(matchedStyles) {
    for (const style of matchedStyles.nodeStyles()) {
      ...
      if (rule instanceof SDK.CSSRule.CSSStyleRule) {
        if (
          rule.nestingSelectors?.at(0)?.includes(AI_ASSISTANCE_CSS_CLASS_NAME) ||
  ...
  static getSourceLocation(styleRule) {
    ...
    const uiLocation =
      Bindings.CSSWorkspaceBinding.CSSWorkspaceBinding.instance().rawLocationToUILocation(location);
  ...
  async #bindingCalled(executionContext, event) {
    ...
    await this.#bindingMutex.run(async () => {
      ...
      try {
        ...
        await this.#simpleEval(
          executionContext,
          `freestyler.respond(${id}, ${JSON.stringify(styleChanges)})`,
        );
      } catch (error) {
        await this.#simpleEval(
          executionContext,
          `freestyler.respond(${id}, new Error("${error?.message}"))`,
        );
      }
    });
  }
