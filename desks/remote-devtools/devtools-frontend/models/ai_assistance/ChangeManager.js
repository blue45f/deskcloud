...
function formatStyles(styles, indent = 2) {
  const lines = Object.entries(styles).map(
    ([key, value]) => `${' '.repeat(indent)}${key}: ${value};`,
  );
  return lines.join('\n');
}
/**
...
export class ChangeManager {
  ...
  async popStashedChanges() {
    ...
    await Promise.allSettled(
      cssModelAndStyleSheets.map(async ([cssModel, stylesheetMap]) => {
        const frameAndStylesheet = Array.from(stylesheetMap.entries());
        return await Promise.allSettled(
          frameAndStylesheet.map(async ([frameId, stylesheetId]) => {
    ...
    for (const changes of this.#stylesheetChanges.values()) {
      for (const change of changes) {
        if (
          ...
          change.backendNodeId &&
          (turnId === undefined || change.turnId === turnId)
        ) {
...
${formatStyles(change.styles, 4)}
  ...
  #formatChange(change, includeMetadata = false) {
    const sourceLocation =
      includeMetadata && change.sourceLocation
        ? `/* related resource: ${change.sourceLocation} */\n`
        : '';
    ...
    const simpleSelector =
      includeMetadata && change.simpleSelector
        ? ` /* the element was ${change.simpleSelector} */`
        : '';
