...
export class IgnoreListManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  modelRemoved(debuggerModel) {
    ...
    const sourceMapManager = debuggerModel.sourceMapManager();
    sourceMapManager.removeEventListener(
      SDK.SourceMapManager.Events.SourceMapAttached,
      this.sourceMapAttached,
      this,
    );
  ...
  setIgnoreListPatterns(debuggerModel) {
    const regexPatterns = this.enableIgnoreListing
      ? this.getSkipStackFramesPatternSetting().getAsArray()
      : [];
    const patterns = [];
  ...
  async updateScriptRanges(script, sourceMap) {
    ...
    if (
      !this.isUserIgnoreListedURL(script.sourceURL, { isContentScript: script.isContentScript() })
    ) {
      hasIgnoreListedMappings =
        sourceMap
          ?.sourceURLs()
          .some((url) =>
            this.isUserIgnoreListedURL(url, {
              isKnownThirdParty: sourceMap.hasIgnoreListHint(url),
            }),
          ) ?? false;
    }
    if (!hasIgnoreListedMappings) {
      ...
      this.dispatchEventToListeners(
        'IGNORED_SCRIPT_RANGES_UPDATED' /* Events.IGNORED_SCRIPT_RANGES_UPDATED */,
        script,
      );
      return;
    }
    ...
    const newRanges = sourceMap
      .findRanges(
        (srcURL) =>
          this.isUserIgnoreListedURL(srcURL, {
            isKnownThirdParty: sourceMap.hasIgnoreListHint(srcURL),
          }),
    ...
    function isEqual(rangesA, rangesB) {
      ...
      for (let i = 0; i < rangesA.length; ++i) {
        if (
          rangesA[i].lineNumber !== rangesB[i].lineNumber ||
          rangesA[i].columnNumber !== rangesB[i].columnNumber
        ) {
          return false;
        }
      }
  ...
  unIgnoreListUISourceCode(uiSourceCode) {
    this.unIgnoreListURL(
      this.uiSourceCodeURL(uiSourceCode),
      this.getGeneralRulesForUISourceCode(uiSourceCode),
    );
  }
  ...
  addRegexToIgnoreList(regexValue, disabledForUrl) {
    ...
    for (let i = 0; i < regexPatterns.length; ++i) {
      const item = regexPatterns[i];
      if (
        item.pattern === regexValue ||
        (disabledForUrl && item.disabledForUrl === disabledForUrl)
      ) {
        item.disabled = false;
  ...
  ignoreListHasPattern(regexValue, enabledOnly) {
    ...
    return regexPatterns.some(
      (item) => !(enabledOnly && item.disabled) && item.pattern === regexValue,
    );
  }
  ...
  urlToRegExpString(url) {
    ...
    return (
      prefix + Platform.StringUtilities.escapeForRegExp(name) + (url.endsWith(name) ? '$' : '\\b')
    );
  }
  getIgnoreListURLContextMenuItems(uiSourceCode) {
    ...
    const { isContentScript, isKnownThirdParty } =
      this.getGeneralRulesForUISourceCode(uiSourceCode);
    if (isIgnoreListed) {
      ...
      menuItems.push(
        ...this.getIgnoreListGeneralContextMenuItems({ isContentScript, isKnownThirdParty }),
      );
    }
    return menuItems;
  }
