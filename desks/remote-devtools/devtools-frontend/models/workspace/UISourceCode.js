...
export class UISourceCode extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(project, url, contentType) {
    ...
    if (parsedURL) {
      ...
      this.#parentURL = Common.ParsedURL.ParsedURL.concatenate(
        this.#origin,
        parsedURL.folderPathComponents,
      );
      if (
        parsedURL.queryParams &&
        !(parsedURL.lastPathComponent && contentType.isFromSourceMap())
      ) {
  ...
  async #requestContent() {
    if (this.#content) {
      throw new Error(
        'Called UISourceCode#requestContentImpl even though content is available for ' + this.#url,
      );
    }
    try {
  ...
  #decodeContent(content) {
    ...
    return content.isEncoded && content.content
      ? globalThis.atob(content.content)
      : content.content;
  }
  ...
  async checkContentUpdated() {
    ...
    const updatedContent = TextUtils.ContentData.ContentData.asDeferredContent(
      await this.#project.requestFileContent(this),
    );
  ...
  #contentCommitted(content, committedByUser) {
    this.#lastAcceptedContent = null;
    this.#content = new TextUtils.ContentData.ContentData(
      content,
      Boolean(this.#contentEncoded),
      this.mimeType(),
    );
    ...
    this.#project
      .workspace()
      .dispatchEventToListeners(WorkspaceImplEvents.WorkingCopyCommitted, data);
    if (committedByUser) {
      this.#project
        .workspace()
        .dispatchEventToListeners(WorkspaceImplEvents.WorkingCopyCommittedByUser, data);
    }
  }
  addRevision(content) {
  ...
  workingCopyContentData() {
    ...
    const contentData = this.#content
      ? TextUtils.ContentData.ContentData.contentDataOrEmpty(this.#content)
  ...
  #workingCopyChanged() {
    ...
    this.#project
      .workspace()
      .dispatchEventToListeners(WorkspaceImplEvents.WorkingCopyChanged, { uiSourceCode: this });
  }
