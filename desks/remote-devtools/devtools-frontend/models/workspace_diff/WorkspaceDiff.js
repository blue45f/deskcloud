...
export class WorkspaceDiffImpl extends Common.ObjectWrapper.ObjectWrapper {
  ...
  subscribeToDiffChange(uiSourceCode, callback, thisObj) {
    this.#uiSourceCodeDiff(uiSourceCode).addEventListener(
      'DiffChanged' /* UISourceCodeDiffEvents.DIFF_CHANGED */,
      callback,
      thisObj,
    );
  }
  unsubscribeFromDiffChange(uiSourceCode, callback, thisObj) {
    this.#uiSourceCodeDiff(uiSourceCode).removeEventListener(
      'DiffChanged' /* UISourceCodeDiffEvents.DIFF_CHANGED */,
      callback,
      thisObj,
    );
  ...
  #markAsUnmodified(uiSourceCode) {
    ...
    if (this.#modified.delete(uiSourceCode)) {
      this.dispatchEventToListeners('ModifiedStatusChanged' /* Events.MODIFIED_STATUS_CHANGED */, {
        uiSourceCode,
        isModified: false,
      });
    }
...
export class UISourceCodeDiff extends Common.ObjectWrapper.ObjectWrapper {
  ...
  async #requestDiff() {
    ...
    baseline = (
      await FormatterModule.ScriptFormatter.format(
    ...
    const reNewline = /\r\n?|\n/;
    const diff = Diff.Diff.DiffWrapper.lineDiff(
      baseline.split(reNewline),
      current.split(reNewline),
    );
    return {
      diff,
