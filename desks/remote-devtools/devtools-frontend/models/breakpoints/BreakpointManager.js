...
export class BreakpointManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  #setInitialBreakpoints(restoreInitialBreakpointCount) {
    ...
    for (const storageState of this.storage.breakpoints.values()) {
      ...
      const breakpoint = new Breakpoint(
        ...
        null,
        storageState,
        'RESTORED' /* BreakpointOrigin.OTHER */,
      );
  ...
  static instance(
    ...
    if (!breakpointManagerInstance || forceNew) {
      ...
      breakpointManagerInstance = new _a(
        targetManager,
        workspace,
        debuggerWorkspaceBinding,
        settings,
  ...
  async copyBreakpoints(fromSourceCode, toSourceCode) {
    const toSourceCodeIsRemoved =
      toSourceCode.project().uiSourceCodeForURL(toSourceCode.url()) !== toSourceCode ||
      this.#workspace.project(toSourceCode.project().id()) !== toSourceCode.project();
    const breakpointItems = this.storage.breakpointItems(
      fromSourceCode.url(),
      fromSourceCode.contentType().name(),
    );
    for (const item of breakpointItems) {
      if (toSourceCodeIsRemoved) {
        ...
        this.storage.updateBreakpoint({
          ...item,
          url: toSourceCode.url(),
          resourceTypeName: toSourceCode.contentType().name(),
        });
      } else {
    ...
    if (sourceMap) {
      for (const sourceURL of sourceMap.sourceURLs()) {
        if (this.#hasBreakpointsForUrl(sourceURL)) {
          const uiSourceCode =
            await this.debuggerWorkspaceBinding.uiSourceCodeForSourceMapSourceURLPromise(
              debuggerModel,
              sourceURL,
  ...
  async #restoreBreakpointsForUrl(uiSourceCode) {
    ...
    const affectedBreakpoints = Array.from(breakpoints).filter((x) =>
      x.uiSourceCodes.has(uiSourceCode),
    );
  ...
  static getScriptForInlineUiSourceCode(uiSourceCode) {
    const script =
      Bindings.DefaultScriptMapping.DefaultScriptMapping.scriptForUISourceCode(uiSourceCode);
  ...
  static breakpointLocationFromUiLocation(uiLocation) {
    ...
    const { lineNumber, columnNumber } = script
      ? script.relativeLocationToRawLocation(uiLocation)
      : uiLocation;
  ...
  static uiLocationFromBreakpointLocation(uiSourceCode, lineNumber, columnNumber) {
    const script = _a.getScriptForInlineUiSourceCode(uiSourceCode);
    if (script) {
      ({ lineNumber, columnNumber } = script.rawLocationToRelativeLocation({
        lineNumber,
        columnNumber,
      }));
    }
  ...
  async setBreakpoint(
    ...
    for (const compatibleUiSourceCode of compatibleUiSourceCodes) {
      const uiLocation = new Workspace.UISourceCode.UILocation(
        compatibleUiSourceCode,
        lineNumber,
        columnNumber,
      );
      const normalizedLocation =
        await this.debuggerWorkspaceBinding.normalizeUILocation(uiLocation);
  ...
  async possibleBreakpoints(uiSourceCode, textRange) {
    const rawLocationRanges =
      await this.debuggerWorkspaceBinding.uiLocationRangeToRawLocationRanges(
        uiSourceCode,
        textRange,
      );
    const breakLocationLists = await Promise.all(
      rawLocationRanges.map(({ start, end }) =>
        start.debuggerModel.getPossibleBreakpoints(start, end, /* restrictToFunction */ false),
      ),
    );
    const breakLocations = breakLocationLists.flat();
    const uiLocations = new Map();
    await Promise.all(
      breakLocations.map(async (breakLocation) => {
        const uiLocation =
          await this.debuggerWorkspaceBinding.rawLocationToUILocation(breakLocation);
        if (uiLocation === null) {
          return;
        }
  ...
  #getAllBreakpointsForUISourceCode(uiSourceCode) {
    const uiBreakpoints = this.breakpointLocationsForUISourceCode(uiSourceCode).map(
      (b) => b.breakpoint,
    );
    return uiBreakpoints.concat(
      Array.from(this.#breakpointsForHomeUISourceCode.get(uiSourceCode) ?? []),
    );
  }
...
export class Breakpoint {
  ...
  updateLastResolvedState(locations) {
    ...
    if (locations) {
      locationsOrUndefined = locations.map((p) => ({
        url: p.url,
  ...
  async refreshInDebugger() {
    if (!this.isRemoved) {
      ...
      await Promise.all(
        modelBreakpoints.map(async (modelBreakpoint) => {
  ...
  modelAdded(debuggerModel) {
    ...
    debuggerModel.addEventListener(
      ...
      this.#onDebuggerEnabled,
      this,
    );
    debuggerModel.addEventListener(
      SDK.DebuggerModel.Events.DebuggerWasDisabled,
      this.#onDebuggerDisabled,
      this,
    );
    debuggerModel.addEventListener(
      SDK.DebuggerModel.Events.ScriptSourceWasEdited,
      this.#onScriptWasEdited,
      this,
    );
  }
  modelRemoved(debuggerModel) {
  ...
  backendCondition(location) {
    ...
    if (location) {
      return SourceMapScopes.NamesResolver.allVariablesAtPosition(location)
        .then((nameMap) =>
          Formatter.FormatterWorkerPool.formatterWorkerPool().javaScriptSubstitute(
            condition,
            nameMap,
          ),
        )
        .catch(() => condition)
        .then(
          (subsitutedCondition) => addSourceUrl(subsitutedCondition),
          () => addSourceUrl(condition),
        );
    }
  ...
  updateState(newState) {
    ...
    if (
      this.#storageState &&
      (this.#storageState.url !== newState.url ||
        this.#storageState.lineNumber !== newState.lineNumber ||
        this.#storageState.columnNumber !== newState.columnNumber)
    ) {
      throw new Error('Invalid breakpoint state update');
    }
    if (
      this.#storageState?.enabled === newState.enabled &&
      this.#storageState?.condition === newState.condition &&
  ...
  async #updateModels() {
    await Promise.all(
      Array.from(this.#modelBreakpoints.values()).map((model) => this.#updateModel(model)),
    );
  }
  async #updateModel(model) {
    ...
    if (result === 'ERROR_BACKEND' /* DebuggerUpdateResult.ERROR_BACKEND */) {
      ...
      result === 'ERROR_BREAKPOINT_CLASH' /* DebuggerUpdateResult.ERROR_BREAKPOINT_CLASH */
    ) {
...
export class ModelBreakpoint {
  ...
  scriptDiverged() {
    for (const uiSourceCode of this.#breakpoint.getUiSourceCodes()) {
      const scriptFile = this.#debuggerWorkspaceBinding.scriptFile(
        uiSourceCode,
        this.#debuggerModel,
      );
  ...
  async #updateInDebugger() {
    ...
    if (!this.#breakpoint.getIsRemoved() && this.#breakpoint.enabled() && !this.scriptDiverged()) {
      ...
      for (const uiSourceCode of this.#breakpoint.getUiSourceCodes()) {
        const { lineNumber: uiLineNumber, columnNumber: uiColumnNumber } =
          BreakpointManager.uiLocationFromBreakpointLocation(
            uiSourceCode,
            lineNumber,
            columnNumber,
          );
        const locations = await this.#debuggerWorkspaceBinding.uiLocationToRawLocations(
          uiSourceCode,
          uiLineNumber,
          uiColumnNumber,
        );
        debuggerLocations = locations.filter(
          (location) => location.debuggerModel === this.#debuggerModel,
        );
      ...
      if (debuggerLocations.length && debuggerLocations.every((loc) => loc.script())) {
        const positions = await Promise.all(
          debuggerLocations.map(async (loc) => {
    ...
    this.#breakpointIds.forEach((breakpointId) =>
      this.#debuggerModel.addBreakpointListener(breakpointId, this.breakpointResolved, this),
    );
    const resolvedResults = await Promise.all(
      locations.map((location) => this.addResolvedLocation(location)),
    );
  ...
  didRemoveFromDebugger() {
    ...
    this.resetLocations();
    this.#breakpointIds.forEach((breakpointId) =>
      this.#debuggerModel.removeBreakpointListener(breakpointId, this.breakpointResolved, this),
    );
    this.#breakpointIds = [];
  }
    ...
    function subset(stateA, stateB) {
      ...
      for (const positionA of stateA) {
        if (
          stateB.find(
            (positionB) =>
              positionA.url === positionB.url &&
              positionA.scriptHash === positionB.scriptHash &&
              positionA.lineNumber === positionB.lineNumber &&
              positionA.columnNumber === positionB.columnNumber &&
              positionA.condition === positionB.condition,
          ) === undefined
        ) {
          return false;
        }
...
function resolvedStateEqual(lhs, rhs) {
  ...
  for (let i = 0; i < lhs.length; i++) {
    ...
    const rhsLoc = rhs[i];
    if (
      lhsLoc.url !== rhsLoc.url ||
      lhsLoc.lineNumber !== rhsLoc.lineNumber ||
      lhsLoc.columnNumber !== rhsLoc.columnNumber ||
      lhsLoc.condition !== rhsLoc.condition
    ) {
      return false;
    }
  }
