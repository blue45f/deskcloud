...
export class IsolatedFileSystem extends PlatformFileSystem {
  ...
  constructor(manager, path, embedderPath, domFileSystem, type, automatic) {
    ...
    this.excludedFoldersSetting = Common.Settings.Settings.instance().createLocalSetting(
  ...
  static async create(manager, path, embedderPath, type, name, rootURL, automatic) {
    const domFileSystem =
      Host.InspectorFrontendHost.InspectorFrontendHostInstance.isolatedFileSystem(name, rootURL);
    if (!domFileSystem) {
      return null;
    }
      ...
      function innerCallback(entries) {
        for (let i = 0; i < entries.length; ++i) {
          ...
          if (!entry.isDirectory) {
            if (
              this.isFileExcluded(
                Common.ParsedURL.ParsedURL.rawPathToEncodedPathString(entry.fullPath),
              )
            ) {
              continue;
            }
            this.#initialFilePaths.add(
            ...
            if (entry.fullPath.endsWith('/.git')) {
              ...
              this.#initialGitFolders.add(
                Common.ParsedURL.ParsedURL.rawPathToEncodedPathString(parentFolder),
              );
            }
            if (
              this.isFileExcluded(
                Common.ParsedURL.ParsedURL.concatenate(
                  Common.ParsedURL.ParsedURL.rawPathToEncodedPathString(entry.fullPath),
                  '/',
                ),
              )
  ...
  async createFoldersIfNotExist(folderPath) {
    ...
    let dirEntry = await new Promise((resolve) =>
      this.domFileSystem.root.getDirectory(folderPath, undefined, resolve, () => resolve(null)),
    );
    if (dirEntry) {
      return dirEntry;
    }
          ...
          this.domFileSystem.root.getFile(
            ...
            () =>
              this.dispatchEventToListeners(
                'file-system-error' /* PlatformFileSystemEvents.FILE_SYSTEM_ERROR */,
  ...
  async createFile(path, name) {
    const dirEntry = await this.createFoldersIfNotExist(
      Common.ParsedURL.ParsedURL.encodedPathToRawPathString(path),
    );
    if (!dirEntry) {
      return null;
    ...
    function createFileCandidate(name, newFileIndex) {
      return new Promise((resolve) => {
        const nameCandidate = Common.ParsedURL.ParsedURL.concatenate(
          name,
          (newFileIndex || '').toString(),
        );
  ...
  requestFileBlob(path) {
    return new Promise((resolve) => {
      ...
      function errorHandler(error) {
        ...
        const errorMessage = _a.errorMessage(error);
        console.error(
          errorMessage + " when getting content for file '" + (this.path() + '/' + path) + "'",
        );
        resolve(null);
      }
    });
  ...
  async setFileContent(path, content, isBase64) {
    ...
    function errorHandler(error) {
      ...
      const errorMessage = _a.errorMessage(error);
      console.error(
        errorMessage + " when setting content for file '" + (this.path() + '/' + path) + "'",
      );
      resolve(undefined);
    }
  }
