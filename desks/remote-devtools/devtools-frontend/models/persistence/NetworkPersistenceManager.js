...
export class NetworkPersistenceManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  #enabledSetting = Common.Settings.Settings.instance().moduleSetting(
  ...
  async enabledChanged() {
    ...
    this.dispatchEventToListeners(
      'LocalOverridesProjectUpdated' /* Events.LOCAL_OVERRIDES_PROJECT_UPDATED */,
      this.#enabled,
    );
  }
  ...
  async updateActiveProject() {
    ...
    if (this.#active && this.#project) {
      await Promise.all(
        [...this.#project.uiSourceCodes()].map((uiSourceCode) =>
          this.filesystemUISourceCodeAdded(uiSourceCode),
        ),
      );
      const networkProjects = this.#workspace.projectsForType(
        Workspace.Workspace.projectTypes.Network,
      );
      for (const networkProject of networkProjects) {
        await Promise.all(
          [...networkProject.uiSourceCodes()].map((uiSourceCode) =>
            this.networkUISourceCodeAdded(uiSourceCode),
          ),
        );
      }
      ...
      await Promise.all(
        [...this.#project.uiSourceCodes()].map((uiSourceCode) =>
          this.filesystemUISourceCodeRemoved(uiSourceCode),
        ),
      );
  ...
  encodedPathFromUrl(url, ignoreInactive) {
    return Common.ParsedURL.ParsedURL.rawPathToEncodedPathString(
      this.rawPathFromUrl(url, ignoreInactive),
    );
  }
  rawPathFromUrl(url, ignoreInactive) {
    ...
    let initialEncodedPath = Common.ParsedURL.ParsedURL.urlWithoutHash(
      url.replace(/^https?:\/\//, ''),
    );
    ...
    let encodedPathParts =
      NetworkPersistenceManager.encodeEncodedPathToLocalPathParts(initialEncodedPath);
  ...
  static encodeEncodedPathToLocalPathParts(encodedPath) {
    ...
    for (const pathPart of this.#fileNamePartsFromEncodedPath(encodedPath)) {
      ...
      let encodedName = encodeURI(pathPart).replace(
        /[\/\*]/g,
  ...
  getHeadersUISourceCodeFromUrl(url) {
    ...
    const folderUrlFromRequest = Common.ParsedURL.ParsedURL.substring(
      fileUrlFromRequest,
      0,
      fileUrlFromRequest.lastIndexOf('/'),
    );
    const headersFileUrl = Common.ParsedURL.ParsedURL.concatenate(
      folderUrlFromRequest,
      '/',
      HEADERS_FILENAME,
    );
  ...
  async getOrCreateHeadersUISourceCodeFromUrl(url) {
    ...
    if (!uiSourceCode && this.#project) {
      ...
      const encodedPath = Common.ParsedURL.ParsedURL.substring(
        encodedFilePath,
        0,
  ...
  async #unbind(uiSourceCode) {
    ...
    if (binding) {
      ...
      this.dispatchEventToListeners(
        'RequestsForHeaderOverridesFileChanged' /* Events.REQUEST_FOR_HEADER_OVERRIDES_FILE_CHANGED */,
        uiSourceCode,
      );
    }
  }
  ...
  async #innerAddBinding(networkUISourceCode, fileSystemUISourceCode) {
    ...
    const uiSourceCodeOfTruth = this.#savingForOverrides.has(networkUISourceCode)
      ? networkUISourceCode
      : fileSystemUISourceCode;
    const contentDataOrError = await uiSourceCodeOfTruth.requestContentData();
    const { content, isEncoded } =
      TextUtils.ContentData.ContentData.asDeferredContent(contentDataOrError);
  ...
  isActiveHeaderOverrides(uiSourceCode) {
    ...
    return (
      uiSourceCode.url().endsWith(HEADERS_FILENAME) &&
  ...
  #shouldPromptSaveForOverridesDialog(uiSourceCode) {
    return (
      this.isUISourceCodeOverridable(uiSourceCode) &&
  ...
  async setupAndStartLocalOverrides(uiSourceCode) {
    ...
    if (this.#shouldPromptSaveForOverridesDialog(uiSourceCode)) {
      ...
      await new Promise((resolve) =>
        this.dispatchEventToListeners(
          'LocalOverridesRequested' /* Events.LOCAL_OVERRIDES_REQUESTED */,
          resolve,
        ),
      );
    ...
    if (!this.#enabledSetting.get()) {
      Host.userMetrics.actionTaken(
        Host.UserMetrics.Action.OverrideContentContextMenuActivateDisabled,
      );
    ...
    if (!this.#isUISourceCodeAlreadyOverridden(uiSourceCode)) {
      ...
      Host.userMetrics.actionTaken(
        Host.UserMetrics.Action.OverrideContentContextMenuOpenExistingFile,
      );
    }
    return true;
  }
  ...
  patternForFileSystemUISourceCode(uiSourceCode) {
    ...
    const path = this.decodeLocalPathToUrlPath(
      this.decodeLocalPathToUrlPath(relativePathParts.join('/')),
    );
  ...
  isForbiddenFileUrl(uiSourceCode) {
    ...
    const host = this.decodeLocalPathToUrlPath(
      this.decodeLocalPathToUrlPath(relativePathParts[0] || ''),
    );
  ...
  async filesystemUISourceCodeAdded(uiSourceCode) {
    ...
    const networkUISourceCode = this.#networkUISourceCodeForEncodedPath.get(
      Common.ParsedURL.ParsedURL.join(relativePath, '/'),
    );
  ...
  async #getHeaderOverridesFromUiSourceCode(uiSourceCode) {
    const contentData = await uiSourceCode
      .requestContentData()
      .then(TextUtils.ContentData.ContentData.contentDataOrEmpty);
  ...
  async generateHeaderPatterns(uiSourceCode) {
    ...
    if (
      relativePathParts.length > 2 &&
      relativePathParts[1] === 'longurls' &&
      headerOverrides.length
    ) {
      patterns = this.#generateHeaderPatternsForLongUrl(
        decodedPath,
        headerOverrides,
        relativePathParts[0],
      );
    ...
    for (const headerOverride of headerOverrides) {
      ...
      if (decodedPath === '') {
        ...
        overridesWithRegex.push({
          applyToRegex: new RegExp(
            '^file:\/\/\/' + escapeRegex(decodedPath + headerOverride.applyTo) + '$',
          ),
  ...
  #generateHeaderPatternsForLongUrl(decodedPath, headerOverrides, relativePathPart) {
    ...
    let { decodedPath: decodedPattern } = this.#doubleDecodeEncodedPathString(
      Common.ParsedURL.ParsedURL.concatenate(relativePathPart, '/*'),
    );
    ...
    for (const headerOverride of headerOverrides) {
      overridesWithRegex.push({
        applyToRegex: new RegExp(
          `^${isFileUrl ? 'file:\/' : ''}${escapeRegex(decodedPath + headerOverride.applyTo)}$`,
        ),
        headers: headerOverride.headers,
      });
    }
  ...
  updateInterceptionPatterns() {
    void this.#updateInterceptionThrottler.schedule(
      this.#innerUpdateInterceptionPatterns.bind(this),
    );
  }
    ...
    for (const uiSourceCode of this.#project.uiSourceCodes()) {
      ...
      if (uiSourceCode.name() === HEADERS_FILENAME) {
        const { headerPatterns, path, overridesWithRegex } =
          await this.generateHeaderPatterns(uiSourceCode);
  ...
  #maybeDispatchRequestsForHeaderOverridesFileChanged(uiSourceCode) {
    ...
    for (let i = project.fileSystemPath().length; i < fileUrl.length; i++) {
      ...
      const headersFilePath = Common.ParsedURL.ParsedURL.concatenate(
        Common.ParsedURL.ParsedURL.substring(fileUrl, 0, i + 1),
        '.headers',
      );
      ...
      void this.#eventDispatchThrottler.schedule(
        this.#dispatchRequestsForHeaderOverridesFileChanged.bind(this),
      );
    }
  }
  ...
  mergeHeaders(baseHeaders, overrideHeaders) {
    ...
    const originalSetCookieHeaders =
      baseHeaders.filter((header) => header.name.toLowerCase() === 'set-cookie') || [];
    const setCookieHeadersFromOverrides = overrideHeaders.filter(
  ...
  #maybeMergeHeadersForPathSegment(path, requestUrl, headers) {
    ...
    for (const headerOverride of headerOverrides) {
      const requestUrlWithLongUrlReplacement = this.decodeLocalPathToUrlPath(
        this.rawPathFromUrl(requestUrl),
      );
  ...
  async interceptionHandler(interceptedRequest) {
    ...
    if (!mimeType) {
      const expectedResourceType =
        Common.ResourceType.resourceTypes[interceptedRequest.resourceType] ||
    ...
    if (fileSystemUISourceCode) {
      ...
      if (!TextUtils.ContentData.ContentData.isError(responseBody)) {
        ...
        void interceptedRequest.continueRequestWithContent(
          ...
          /* encoded */ !responseBody.isTextContent,
          responseHeaders,
          /* isBodyOverridden */ false,
        );
      }
...
export function isHeaderOverride(arg) {
  if (
    !(arg && typeof arg.applyTo === 'string' && arg.headers?.length && Array.isArray(arg.headers))
  ) {
    return false;
  }
...
export function extractDirectoryIndex(pattern) {
  ...
  if (
    tail !== '*' &&
    (regex.test('index.html') || regex.test('index.htm') || regex.test('index.php'))
  ) {
    return { head, tail };
  }
  return { head: pattern };
