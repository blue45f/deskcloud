...
export class AutomaticFileSystemManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(inspectorFrontendHost, projectSettingsModel) {
    ...
    this.#projectSettingsModel.addEventListener(
      'AvailabilityChanged' /* ProjectSettings.ProjectSettingsModel.Events.AVAILABILITY_CHANGED */,
      this.#availabilityChanged,
      this,
    );
    this.#availabilityChanged({ data: this.#projectSettingsModel.availability });
    this.#projectSettingsModel.addEventListener(
      'ProjectSettingsChanged' /* ProjectSettings.ProjectSettingsModel.Events.PROJECT_SETTINGS_CHANGED */,
   ...
   */
  static instance(
    { forceNew, inspectorFrontendHost, projectSettingsModel } = {
      forceNew: false,
  ...
  #availabilityChanged(event) {
    ...
    if (this.#availability !== availability) {
      ...
      this.dispatchEventToListeners(
        'AvailabilityChanged' /* Events.AVAILABILITY_CHANGED */,
        this.#availability,
      );
    }
  }
  #fileSystemRemoved(event) {
    ...
    if (this.#automaticFileSystem.root === event.data) {
      ...
      this.dispatchEventToListeners(
        'AutomaticFileSystemChanged' /* Events.AUTOMATIC_FILE_SYSTEM_CHANGED */,
        this.#automaticFileSystem,
      );
    }
  }
  ...
  async connectAutomaticFileSystem(addIfMissing = false) {
    ...
    if (state === 'disconnected') {
      const automaticFileSystem = (this.#automaticFileSystem = Object.freeze({
        ...this.#automaticFileSystem,
        state: 'connecting',
      }));
      this.dispatchEventToListeners(
      ...
      const { success } = await new Promise((resolve) =>
        this.#inspectorFrontendHost.connectAutomaticFileSystem(root, uuid, addIfMissing, resolve),
      );
      if (this.#automaticFileSystem === automaticFileSystem) {
