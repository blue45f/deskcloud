...
export class PersistenceImpl extends Common.ObjectWrapper.ObjectWrapper {
  ...
  async #addBinding(binding) {
    ...
    if (binding.fileSystem.isDirty()) {
      ...
      binding.network.hasCommits() &&
      binding.network.content() !== binding.fileSystem.content()
    ) {
  ...
  syncWorkingCopy(uiSourceCode) {
    ...
    if (target && target.type() === SDK.Target.Type.NODE) {
      ...
      void other.requestContentData().then(() => {
        const nodeJSContent = PersistenceImpl.rewrapNodeJSContent(
          other,
          other.workingCopy(),
          newContent,
        );
    ...
    await Promise.all(
      breakpoints.map(async (breakpoint) => {
        ...
        return await this.#breakpointManager.setBreakpoint(
          ...
          'RESTORED' /* BreakpointManager.BreakpointManager.BreakpointOrigin.OTHER */,
        );
      }),
    );
  }
