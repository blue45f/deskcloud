...
export class FileSystemWorkspaceBinding {
  ...
  constructor(isolatedFileSystemManager, workspace) {
    ...
    this.#eventListeners = [
      ...
      this.isolatedFileSystemManager.addEventListener(
        Events.FileSystemFilesChanged,
        this.fileSystemFilesChanged,
        this,
      ),
    ];
    void this.isolatedFileSystemManager
      .waitForFileSystems()
...
export class FileSystem extends Workspace.Workspace.ProjectStore {
  ...
  constructor(fileSystemWorkspaceBinding, isolatedFileSystem, workspace) {
    ...
    this.#fileSystemParentURL = Common.ParsedURL.ParsedURL.substr(
      this.fileSystemBaseURL,
      0,
      fileSystemPath.lastIndexOf('/') + 1,
    );
  ...
  initialGitFolders() {
    return this.#fileSystem
      .initialGitFolders()
      .map((folder) => Common.ParsedURL.ParsedURL.concatenate(this.#fileSystemPath, '/', folder));
  }
  filePathForUISourceCode(uiSourceCode) {
    return Common.ParsedURL.ParsedURL.sliceUrlToEncodedPathString(
      uiSourceCode.url(),
      this.#fileSystemPath.length,
    );
  }
  isServiceProject() {
    return false;
  ...
  requestMetadata(uiSourceCode) {
    ...
    function onMetadata(metadata) {
      ...
      return new Workspace.UISourceCode.UISourceCodeMetadata(
  ...
  async findFilesMatchingSearchRequest(searchConfig, filesMatchingFileQuery, progress) {
    ...
    for (const query of queriesToRun) {
      const files = await this.#fileSystem.searchInPath(
        searchConfig.isRegex() ? '' : query,
        progress,
      );
  ...
  addFile(filePath, content, isBase64) {
    ...
    const uiSourceCode = this.createUISourceCode(
      Common.ParsedURL.ParsedURL.concatenate(this.fileSystemBaseURL, filePath),
      contentType,
    );
