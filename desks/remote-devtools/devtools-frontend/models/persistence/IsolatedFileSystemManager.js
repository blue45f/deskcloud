...
const str_ = i18n.i18n.registerUIStrings(
  'models/persistence/IsolatedFileSystemManager.ts',
  UIStrings,
);
...
export class IsolatedFileSystemManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    const defaultWinExcludedFolders = [
      '/Thumbs.db$',
      '/ehthumbs.db$',
      '/Desktop.ini$',
      '/\\$RECYCLE.BIN/',
    ...
    this.#workspaceFolderExcludePatternSetting =
      Common.Settings.Settings.instance().createRegExpSetting(
  ...
  addFileSystem(type) {
    Host.userMetrics.actionTaken(
      type === 'overrides'
        ? Host.UserMetrics.Action.OverrideTabAddFolder
  ...
  removeFileSystem(fileSystem) {
    Host.userMetrics.actionTaken(
      fileSystem.type() === PlatformFileSystemType.OVERRIDES
        ? Host.UserMetrics.Action.OverrideTabRemoveFolder
        : Host.UserMetrics.Action.WorkspaceTabRemoveFolder,
    );
    Host.InspectorFrontendHost.InspectorFrontendHostInstance.removeFileSystem(
      fileSystem.embedderPath(),
    );
  }
  waitForFileSystems() {
    ...
    if (errorMessage) {
      if (errorMessage !== '<selection cancelled>' && errorMessage !== '<permission denied>') {
        Common.Console.Console.instance().error(
          i18nString(UIStrings.unableToAddFilesystemS, { PH1: errorMessage }),
        );
      }
  ...
  onFileSystemRemoved(event) {
    ...
    isolatedFileSystem.removeEventListener(
      'file-system-error' /* PlatformFileSystemEvents.FILE_SYSTEM_ERROR */,
      this.#onFileSystemError,
      this,
    );
    ...
    function groupFilePathsIntoFileSystemPaths(embedderPaths) {
      ...
      for (const embedderPath of embedderPaths) {
        ...
        for (const fileSystemPath of this.#fileSystems.keys()) {
          ...
          if (
            fileSystem?.isFileExcluded(
              Common.ParsedURL.ParsedURL.rawPathToEncodedPathString(embedderPath),
            )
          ) {
            continue;
          }
