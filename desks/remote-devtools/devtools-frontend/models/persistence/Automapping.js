...
export class Automapping {
  ...
  constructor(workspace, onStatusAdded, onStatusRemoved) {
    ...
    this.#workspace.addEventListener(Workspace.Workspace.Events.UISourceCodeAdded, (event) =>
      this.#onUISourceCodeAdded(event.data),
    );
    this.#workspace.addEventListener(Workspace.Workspace.Events.UISourceCodeRemoved, (event) =>
      this.#onUISourceCodeRemoved(event.data),
    );
    this.#workspace.addEventListener(
      Workspace.Workspace.Events.UISourceCodeRenamed,
      this.#onUISourceCodeRenamed,
      this,
    );
    this.#workspace.addEventListener(
  ...
  computeNetworkStatus(networkSourceCode) {
    ...
    const createBindingPromise = this.#createBinding(networkSourceCode)
      .then(validateStatus.bind(this))
      .then(onStatus.bind(this));
    ...
    async function validateStatus(status) {
      ...
      if (
        status.network.contentType().isFromSourceMap() ||
        !status.fileSystem.contentType().isTextType()
      ) {
        return status;
      }
      ...
      if (
        status.fileSystem.isDirty() &&
        (status.network.isDirty() || status.network.hasCommits())
      ) {
        return null;
      }
      const [fileSystemContent, networkContent] = (
        await Promise.all([
      ...
      if (target && target.type() === SDK.Target.Type.NODE) {
        if (networkContent.content) {
          const rewrappedNetworkContent = PersistenceImpl.rewrapNodeJSContent(
            status.fileSystem,
            fileContent,
    ...
    async function onStatus(status) {
      ...
      if (
        this.#sourceCodeToAutoMappingStatusMap.has(status.network) ||
      ...
      if (status.exactMatch) {
        const projectFolder = this.#projectFoldersIndex.closestParentFolder(
          status.fileSystem.url(),
        );
        const newFolderAdded = projectFolder
          ? this.#activeFoldersIndex.addFolder(projectFolder)
          : false;
        if (newFolderAdded) {
          this.#scheduleSweep();
        }
  ...
  async #createBinding(networkSourceCode) {
    ...
    if (Common.ParsedURL.schemeIs(url, 'file:') || Common.ParsedURL.schemeIs(url, 'snippet:')) {
      ...
      const status = fileSourceCode
        ? new AutomappingStatus(networkSourceCode, fileSourceCode, false)
        : null;
      return status;
    }
    ...
    const similarFiles = this.#filesIndex
      .similarFiles(networkPath)
      .map((path) => this.#fileSystemUISourceCodes.get(path));
    if (!similarFiles.length) {
      return null;
    }
    await Promise.all(
      similarFiles.concat(networkSourceCode).map(async (sourceCode) => {
        this.#sourceCodeToMetadataMap.set(sourceCode, await sourceCode.requestMetadata());
      }),
    );
    const activeFiles = similarFiles.filter(
      (file) => !!this.#activeFoldersIndex.closestParentFolder(file.url()),
    );
    const networkMetadata = this.#sourceCodeToMetadataMap.get(networkSourceCode);
    if (
      !networkMetadata ||
      (!networkMetadata.modificationTime && typeof networkMetadata.contentSize !== 'number')
    ) {
  ...
  #filterWithMetadata(files, networkMetadata) {
    return files.filter((file) => {
      ...
      const timeMatches =
        !networkMetadata.modificationTime ||
        !fileMetadata.modificationTime ||
        Math.abs(
          networkMetadata.modificationTime.getTime() - fileMetadata.modificationTime.getTime(),
        ) < 1000;
      const contentMatches =
        !networkMetadata.contentSize || fileMetadata.contentSize === networkMetadata.contentSize;
