...
export class SourceMapsResolver extends EventTarget {
  ...
  static resolvedURLForEntry(parsedTrace, entry) {
    const resolvedCallFrameURL = _a
      .resolvedCodeLocationForEntry(entry)
      ?.devtoolsLocation?.uiSourceCode.url();
      ...
      for (const [tid, threadProfile] of threadsInProcess) {
        ...
        for (const node of nodes) {
          const resolvedFunctionName =
            await SourceMapScopes.NamesResolver.resolveProfileFrameFunctionName(
              node.callFrame,
              target,
            );
          ...
          if (uiLocation?.uiSourceCode.url() && this.#entityMapper) {
            ...
            this.#entityMapper.updateSourceMapEntities(
              node.callFrame,
              uiLocation.uiSourceCode.url(),
            );
          }
          _a.storeResolvedCodeDataForCallFrame(node.callFrame, {
            name: resolvedFunctionName,
            devtoolsLocation: uiLocation,
            script,
          });
        }
      }
  ...
  #updateExtensionNames() {
    for (const runtimeModel of SDK.TargetManager.TargetManager.instance().models(
