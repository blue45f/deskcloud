...
export class ComputedStyleModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  updateModel(cssModel) {
    ...
    const resourceTreeModel = cssModel
    ...
    if (cssModel && domModel && resourceTreeModel) {
      this.eventListeners = [
        ...
        cssModel.addEventListener(
          SDK.CSSModel.Events.StartingStylesStateForced,
          this.onCSSModelChanged,
          this,
        ),
        cssModel.addEventListener(
          SDK.CSSModel.Events.ModelWasEnabled,
          this.onCSSModelChanged,
          this,
        ),
        cssModel.addEventListener(
          SDK.CSSModel.Events.ComputedStyleUpdated,
  ...
  onCSSModelChanged(event) {
    ...
    this.dispatchEventToListeners(
      'CSSModelChanged' /* Events.CSS_MODEL_CHANGED */,
      event?.data ?? null,
    );
  }
