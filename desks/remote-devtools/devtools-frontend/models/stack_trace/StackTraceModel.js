...
export class StackTraceModel extends SDK.SDKModel.SDKModel {
  ...
  async #createAsyncFragments(stackTraceOrPausedEvent, rawFramesToUIFrames) {
    ...
    if (debuggerModel) {
      for await (const { stackTrace: asyncStackTrace, target } of debuggerModel.iterateAsyncParents(
        ...
        const asyncFragmentPromise = model
          .#createFragment(asyncStackTrace.callFrames, rawFramesToUIFrames)
  ...
  async #translateFragment(fragment, rawFramesToUIFrames) {
    ...
    const rawFrames = fragment.node
      .getCallStack()
      .map((node) => node.rawFrame)
      .toArray();
    const uiFrames = await rawFramesToUIFrames(rawFrames, this.target());
    console.assert(
      rawFrames.length === uiFrames.length,
      'Broken rawFramesToUIFrames implementation',
    );
    let i = 0;
  ...
  #affectedFragments(script) {
    ...
    this.#trie.walk(null, (node) => {
      ...
      if (
        node.rawFrame.scriptId === script.scriptId ||
...
_a = StackTraceModel;
SDK.SDKModel.SDKModel.register(StackTraceModel, {
