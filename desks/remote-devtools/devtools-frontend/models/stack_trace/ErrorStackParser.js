...
export function parseSourcePositionsFromErrorStack(runtimeModel, stack) {
  ...
  for (const line of lines) {
    ...
    if (splitResult.url === '<anonymous>') {
      if (
        linkInfos.length &&
        linkInfos[linkInfos.length - 1].isCallFrame &&
        !linkInfos[linkInfos.length - 1].link
      ) {
    ...
    if (!url && Common.ParsedURL.ParsedURL.isRelativeURL(splitResult.url)) {
      url = parseOrScriptMatch(
        debuggerModel,
        Common.ParsedURL.ParsedURL.completeURL(baseURL, splitResult.url),
      );
    }
    if (!url) {
      return null;
...
function framesMatch(parsedFrame, protocolFrame) {
  ...
  return (
    url === protocolFrame.url &&
    lineNumber === protocolFrame.lineNumber &&
