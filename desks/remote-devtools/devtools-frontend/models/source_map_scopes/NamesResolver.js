...
export const scopeIdentifiers = async function (script, scope, ancestorScopes) {
  ...
  for (const variable of scope.variables) {
    ...
    // without any uses.
    if (
      variable.kind === 3 /* Formatter.FormatterWorkerPool.DefinitionKind.FIXED */ &&
      variable.offsets.length <= 1
    ) {
      continue;
    }
...
const resolveScope = async (script, scopeChain) => {
  ...
  async function resolveSourceName(script, sourceMap, name, position) {
    ...
    if (
      compiledPunctuation === 'comma' /* Punctuation.COMMA */ &&
      sourcePunctuation === 'semicolon' /* Punctuation.SEMICOLON */
    ) {
      return sourceName;
    }
    return null;
...
export const resolveScopeChain = async function (callFrame) {
  ...
  return callFrame
    .scopeChain()
    .map((scope) => new ScopeWithSourceMappedVariables(scope, thisObject));
};
/**
...
export const resolveScopeInObject = function (scope) {
  ...
  if (
    scope.type() === 'global' /* Protocol.Debugger.ScopeType.Global */ ||
    !startLocationScript ||
    !endLocation ||
...
export async function resolveDebuggerFrameFunctionName(frame) {
  ...
  return await getFunctionNameFromScopeStart(
    frame.script,
...
export async function resolveProfileFrameFunctionName(
  ...
  const functionInfoFromPlugin = await debuggerWorkspaceBinding.pluginManager.getFunctionInfo(
    script,
    location,
  );
