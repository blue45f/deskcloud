...
async function prepareInput(uiSourceCode, content) {
  ...
  const text = new TextUtils.Text.Text(
    formattedContent ? formattedContent.formattedContent : content,
  );
  let performanceData = uiSourceCode.getDecorationData(
    'performance' /* Workspace.UISourceCode.DecoratorType.PERFORMANCE */,
  );
  // Map profile data to the formatted view of the text.
  if (formattedContent && performanceData) {
    performanceData = Workspace.UISourceCode.createMappedProfileData(
      performanceData,
      (line, column) => {
...
function createFunctionCode(inputData, functionBounds, options) {
  ...
  if (inputData.formattedContent) {
    const startMapped = inputData.formattedContent.formattedMapping.originalToFormatted(
      startLine,
      startColumn,
    );
    startLine = startMapped[0];
    startColumn = startMapped[1];
    const endMapped = inputData.formattedContent.formattedMapping.originalToFormatted(
      endLine,
      endColumn,
    );
    endLine = endMapped[0];
  ...
  if (options?.contextLineLength !== undefined) {
    const contextLineLength = options.contextLineLength;
    const position = text.offsetFromPosition(
      Math.min(endLine + contextLineLength, text.lineCount() - 1),
...
export async function getFunctionCodeFromLocation(target, url, line, column, options) {
  ...
  const projects = debuggerWorkspaceBinding.workspace.projectsForType(
  ...
  const rawLocations = await debuggerWorkspaceBinding.uiLocationToRawLocations(
    uiSourceCode,
    line,
    column,
  );
...
async function format(uiSourceCode, content) {
  ...
  const shouldFormat =
    ...
    (contentType.isDocument() || contentType.isScript()) &&
    TextUtils.TextUtils.isMinified(content);
  if (!shouldFormat) {
    return null;
  }
  return await Formatter.ScriptFormatter.formatScriptContent(
