...
(function (window) {
  ...
  const DevToolsAPIImpl = class {
    ...
    _dispatchOnInspectorFrontendAPI(method, args) {
      const inspectorFrontendAPI = /** @type {!Object<string, function()>} */ (
    ...
    fileSystemFilesChangedAddedRemoved(changedPaths, addedPaths, removedPaths) {
      ...
      if (
        window['InspectorFrontendAPI'] &&
        window['InspectorFrontendAPI']['fileSystemFilesChanged']
      ) {
  ...
  const InspectorFrontendHostImpl = class {
    ...
    recordCountHistogram(histogramName, sample, min, exclusiveMax, bucketSize) {
      DevToolsAPI.sendMessageToEmbedder(
        ...
        [histogramName, sample, min, exclusiveMax, bucketSize],
        null,
      );
    }
    ...
    recordEnumeratedHistogram(actionName, actionCode, bucketSize) {
      ...
      DevToolsAPI.sendMessageToEmbedder(
        ...
        [actionName, actionCode, bucketSize],
        null,
      );
    }
    ...
    indexPath(requestId, fileSystemPath, excludedFolders) {
      ...
      DevToolsAPI.sendMessageToEmbedder(
        'indexPath',
        [requestId, fileSystemPath, excludedFolders],
        null,
      );
    }
  ...
  function installObjectObserve() {
    ...
    function objectObserve(object, observer) {
      if (window['WebInspector']) {
        const settingPrototype = /** @type {!Object} */ (
          window['WebInspector']['Setting']['prototype']
        );
      ...
      Document.prototype.registerElement = function (typeExtension, options) {
        ...
        const callback = function () {
          ...
          for (const key of Object.keys(
            Object.getOwnPropertyDescriptors(prototype.__proto__ || {}),
          )) {
            if (skip.has(key)) {
              continue;
            }
    ...
    if (majorVersion <= 66) {
      ...
      if (!ShadowRoot.prototype.__originalShadowRootElementFromPoint) {
        ...
        ShadowRoot.prototype.elementFromPoint = function (x, y) {
          const originalResult = ShadowRoot.prototype.__originalShadowRootElementFromPoint.apply(
            this,
            arguments,
          );
