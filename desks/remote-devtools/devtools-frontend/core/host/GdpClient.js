...
export class GdpClient {
  ...
  async getProfile() {
    try {
      ...
      if (
        err instanceof DispatchHttpRequestClient.DispatchHttpRequestError &&
  ...
  async #getProfile() {
    ...
    this.#cachedProfilePromise = makeHttpRequest({
      ...
      this.#cachedEligibilityPromise = Promise.resolve({
        createProfile: EligibilityStatus.ELIGIBLE,
      });
      return profile;
    });
    return await this.#cachedProfilePromise;
  ...
  async #checkEligibility() {
    ...
    this.#cachedEligibilityPromise = makeHttpRequest({
      service: SERVICE_NAME,
      path: '/v1beta1/eligibility:check',
      method: 'GET',
    });
    return await this.#cachedEligibilityPromise;
  }
...
export function isGdpProfilesAvailable() {
  ...
  return (
    isBaseFeatureEnabled &&
    isBrandedBuild &&
    !isOffTheRecordProfile &&
    !isDisabledByEnterprisePolicy
  );
}
...
export function isBadgesEnabled() {
  ...
  const isBadgesEnabledByFeatureFlag = Boolean(
    Root.Runtime.hostConfig.devToolsGdpProfiles?.badgesEnabled,
  );
