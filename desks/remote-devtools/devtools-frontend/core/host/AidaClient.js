...
export class AidaClient {
  ...
  static async checkAccessPreconditions() {
    ...
    const syncInfo = await new Promise((resolve) =>
      InspectorFrontendHostInstance.getSyncInformation((syncInfo) => resolve(syncInfo)),
    );
  ...
  async completeCode(request) {
    ...
    if (completeCodeResult.error) {
      throw new Error(
        `Cannot send request: ${completeCodeResult.error} ${completeCodeResult.detail || ''}`,
      );
    }
...
export class HostConfigTracker extends Common.ObjectWrapper.ObjectWrapper {
  ...
  async pollAidaAvailability() {
    this.#pollTimer = globalThis.setTimeout(() => this.pollAidaAvailability(), 2000);
    const currentAidaAvailability = await AidaClient.checkAccessPreconditions();
    if (currentAidaAvailability !== this.#aidaAvailability) {
      this.#aidaAvailability = currentAidaAvailability;
      const config = await new Promise((resolve) =>
        InspectorFrontendHostInstance.getHostConfig(resolve),
      );
      ...
      this.dispatchEventToListeners(
        'aidaAvailabilityChanged' /* Events.AIDA_AVAILABILITY_CHANGED */,
      );
    }
  }
}
