...
export function aidaEventToGcaTelemetryRequest(clientEvent) {
  try {
    ...
    feedbackMetrics.push(
      ...convertCodeTelemetry(
        clientEvent.complete_code_client_event,
        GCA.Method.COMPLETE_CODE,
        responseId,
        eventTime,
      ),
    );
    ...
    AIDA.debugLog(
      ...
      JSON.stringify(gcaTelemetryRequest),
    );
...
export function aidaGenerateCodeRequestToGcaRequest(request) {
  try {
    const gcaRequest = createBaseGcaRequest(
      request,
      [convertAidaContentToGcaContent(request.current_message)],
      'generate_code',
    );
    ...
    const chunks = parts.map((part) => {
      ...
      if (part.executableCode) {
        aidaChunkResponse.codeChunk = {
          ...
          inferenceLanguage: part.executableCode.language
            ? 'PYTHON' /* AIDA.AidaInferenceLanguage.PYTHON */
