...
export class CSSContainerQuery extends CSSQuery {
  ...
  async getContainerForNode(nodeId) {
    const containerNode = await this.cssModel
      .domModel()
      .getContainerForNode(
        nodeId,
        this.name,
...
export class CSSContainerQueryContainer {
  ...
  async getContainerSizeDetails() {
    const styles = await this.containerNode
      .domModel()
      .cssModel()
    ...
    let width, height;
    if (
      physicalAxis === 'Both' /* PhysicalAxis.BOTH */ ||
  ...
  switch (queryAxis) {
    ...
    case 'inline-size' /* QueryAxis.INLINE */:
      return isVerticalWritingMode
        ? 'Vertical' /* PhysicalAxis.VERTICAL */
        : 'Horizontal' /* PhysicalAxis.HORIZONTAL */;
