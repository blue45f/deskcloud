...
export class EventBreakpointsManager {
  ...
  constructor(targetManager = TargetManager.instance()) {
    ...
    this.createInstrumentationBreakpoints(
      'shared-storage-worklet' /* Category.SHARED_STORAGE_WORKLET */,
      [
  ...
  resolveEventListenerBreakpoint({ eventName }) {
    ...
    const instrumentationName = eventName.substring(
      EventListenerBreakpoint.instrumentationPrefix.length,
    );
...
SDKModel.register(EventBreakpointsModel, {
  capabilities: 524288 /* Capability.EVENT_BREAKPOINTS */,
  autostart: false,
});
