...
export class Cookie {
  ...
  static fromProtocolCookie(protocolCookie) {
    ...
    if ('sourceScheme' in protocolCookie) {
      cookie.addAttribute(
        'source-scheme' /* Attribute.SOURCE_SCHEME */,
  ...
  partitioned() {
    return (
      this.#attributes.has('partitioned' /* Attribute.PARTITIONED */) ||
      Boolean(this.partitionKey()) ||
  ...
  session() {
    ...
    return !(
      this.#attributes.has('expires' /* Attribute.EXPIRES */) ||
      this.#attributes.has('max-age' /* Attribute.MAX_AGE */)
    );
  }
  path() {
