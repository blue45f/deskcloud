...
export class ServiceWorkerCacheModel extends SDKModel {
  ...
  enable() {
    ...
    this.#storageBucketModel.addEventListener(
      'BucketAdded' /* StorageBucketsModelEvents.BUCKET_ADDED */,
      this.storageBucketAdded,
      this,
    );
    this.#storageBucketModel.addEventListener(
      'BucketRemoved' /* StorageBucketsModelEvents.BUCKET_REMOVED */,
  ...
  async deleteCache(cache) {
    ...
    if (response.getError()) {
      console.error(
        `ServiceWorkerCacheAgent error deleting cache ${cache.toString()}: ${response.getError()}`,
      );
      return;
    }
  ...
  async deleteCacheEntry(cache, request) {
    ...
    if (response.getError()) {
      Common.Console.Console.instance().error(
        i18nString(UIStrings.serviceworkercacheagentError, {
          PH1: cache.toString(),
          PH2: String(response.getError()),
        }),
      );
      return;
    }
  ...
  addStorageBucket(storageBucket) {
    ...
    if (!this.#storageKeysTracked.has(storageBucket.storageKey)) {
      ...
      void this.#storageAgent.invoke_trackCacheStorageForStorageKey({
  ...
  removeStorageBucket(storageBucket) {
    ...
    if (storageKeyCount === 0) {
      ...
      void this.#storageAgent.invoke_untrackCacheStorageForStorageKey({
        storageKey: storageBucket.storageKey,
      });
    }
  }
  ...
  cacheRemoved(cache) {
    this.dispatchEventToListeners('CacheRemoved' /* Events.CACHE_REMOVED */, {
      model: this,
      cache,
    });
  }
  async requestEntries(cache, skipCount, pageSize, pathFilter, callback) {
    const response = await this.cacheAgent.invoke_requestEntries({
  ...
  cacheStorageListUpdated({ bucketId }) {
    ...
    if (storageBucket) {
      ...
      void this.#throttler.schedule(
        () => {
          const promises = Array.from(this.#storageBucketsUpdated, (storageBucket) =>
            this.loadCacheNames(storageBucket),
          );
  ...
  cacheStorageContentUpdated({ bucketId, cacheName }) {
    ...
    if (storageBucket) {
      this.dispatchEventToListeners(
        'CacheStorageContentUpdated' /* Events.CACHE_STORAGE_CONTENT_UPDATED */,
        { storageBucket, cacheName },
      );
    }
  }
...
export class Cache {
  ...
  inBucket(storageBucket) {
    return (
      this.storageKey === storageBucket.storageKey && this.storageBucket.name === storageBucket.name
    );
  }
  equals(cache) {
...
SDKModel.register(ServiceWorkerCacheModel, {
  capabilities: 8192 /* Capability.STORAGE */,
  autostart: false,
});
