...
export class CSSStyleRule extends CSSRule {
  ...
  constructor(cssModel, payload, wasUsed) {
    ...
    this.supports = payload.supports
      ? CSSSupports.parseSupportsPayload(cssModel, payload.supports)
      : [];
    this.layers = payload.layers ? CSSLayer.parseLayerPayload(cssModel, payload.layers) : [];
    this.startingStyles = payload.startingStyles
      ? CSSStartingStyle.parseStartingStylePayload(cssModel, payload.startingStyles)
      : [];
    this.navigations = payload.navigations
      ? CSSNavigation.parseNavigationPayload(cssModel, payload.navigations)
      : [];
...
export class CSSKeyframesRule {
  ...
  constructor(cssModel, payload) {
    ...
    this.#keyframes = payload.keyframes.map(
      (keyframeRule) => new CSSKeyframeRule(cssModel, keyframeRule, this.#animationName.text),
    );
  }
  name() {
...
export class CSSFunctionRule extends CSSRule {
  ...
  protocolNodeToNestedStyle(node) {
    if (node.style) {
      return {
        style: new CSSStyleDeclaration(this.cssModelInternal, this, node.style, Type.Regular),
      };
    }
    if (node.condition) {
