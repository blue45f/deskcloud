...
export class RemoteObject {
  ...
  static arrayNameFromDescription(description) {
    return description
      .replace(descriptionLengthParenRegex, '')
      .replace(descriptionLengthSquareRegex, '');
  }
  static arrayLength(object) {
    ...
    return parenMatches
      ? parseInt(parenMatches[1], 10)
      : squareMatches
        ? parseInt(squareMatches[1], 10)
        : 0;
  }
  ...
  static unserializableDescription(object) {
    if (typeof object === 'number') {
      ...
      if (
        description === 'NaN' /* UnserializableNumber.NAN */ ||
        description === 'Infinity' /* UnserializableNumber.INFINITY */ ||
  ...
  constructor(
    ...
    if (objectId) {
      ...
      if (typeof unserializableValue === 'string') {
        ...
        if (
          unserializableValue === 'Infinity' /* UnserializableNumber.INFINITY */ ||
  ...
  async doGetProperties(
    ...
    for (const property of properties) {
      ...
      const propertySymbol = property.symbol
        ? this.#runtimeModel.createRemoteObject(property.symbol)
        : null;
    ...
    for (const property of privateProperties) {
      const propertyValue = property.value
        ? this.#runtimeModel.createRemoteObject(property.value)
        : null;
      const remoteProperty = new RemoteObjectProperty(
        property.name,
        propertyValue,
...
export class RemoteArray {
  ...
  static async createFromRemoteObjects(objects) {
    ...
    const result = await objects[0].callFunction(
      createArray,
      objects.map(RemoteObject.toCallArgument),
    );
...
export class RemoteFunction {
  ...
  async targetFunction() {
    ...
    const targetFunction = ownProperties.internalProperties?.find(
      ({ name }) => name === '[[TargetFunction]]',
    );
    return targetFunction?.value ?? this.#object;
  }
  async targetFunctionDetails() {
    const targetFunction = await this.targetFunction();
    const functionDetails = await targetFunction
      .debuggerModel()
