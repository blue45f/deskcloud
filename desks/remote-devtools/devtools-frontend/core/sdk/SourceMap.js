...
export class SourceMap {
  ...
  constructor(compiledURL, sourceMappingURL, payload, script) {
    ...
    this.#baseURL = Common.ParsedURL.schemeIs(sourceMappingURL, 'data:')
      ? compiledURL
      : sourceMappingURL;
  ...
  findEntry(lineNumber, columnNumber, inlineFrameIndex) {
    ...
    if (inlineFrameIndex && this.#scopesInfo !== null) {
      ...
      if (!callsite) {
        console.error(
          'Malformed source map. Expected to have a callsite info for index',
          inlineFrameIndex,
        );
        return null;
      }
  ...
  findEntryRanges(lineNumber, columnNumber) {
    ...
    const endReverseIndex = Platform.ArrayUtilities.upperBound(
      ...
      undefined,
      (_, i) =>
        startSourceLine - mappings[i].sourceLineNumber ||
        startSourceColumn - mappings[i].sourceColumnNumber,
    );
    if (!endReverseIndex) {
      return null;
    }
    const endSourceLine =
      endReverseIndex < reverseMappings.length
        ? mappings[reverseMappings[endReverseIndex]].sourceLineNumber
        : 2 ** 31 - 1;
    const endSourceColumn =
      endReverseIndex < reverseMappings.length
  ...
  sourceLineMapping(sourceURL, lineNumber, columnNumber) {
    ...
    if (
      first >= reverseMappings.length ||
      mappings[reverseMappings[first]].sourceLineNumber !== lineNumber
    ) {
      return null;
    }
    ...
    const index = Platform.ArrayUtilities.lowerBound(
      columnMappings,
      columnNumber,
      (columnNumber, i) => columnNumber - mappings[i].sourceColumnNumber,
    );
  ...
  findReverseIndices(sourceURL, lineNumber, columnNumber) {
    ...
    const endIndex = Platform.ArrayUtilities.upperBound(
      ...
      undefined,
      (_, i) =>
        lineNumber - mappings[i].sourceLineNumber || columnNumber - mappings[i].sourceColumnNumber,
    );
  ...
  parseMap(map, baseSourceIndex, baseLineNumber, baseColumnNumber) {
    ...
    if (
      Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.USE_SOURCE_MAP_SCOPES)
    ) {
      if (!this.#scopesInfo) {
        this.#scopesInfo = new SourceMapScopesInfo(this, { scopes: [], ranges: [] });
      }
      if (map.scopes) {
        const { scopes, ranges } = ScopesCodec.decode(map, {
          mode: 2 /* ScopesCodec.DecodeMode.LAX */,
  ...
  parseBloombergScopes(map) {
    ...
    if (!scopeList) {
      ...
      throw new Error(
        `x_com_bloomberg_sourcesFunctionMappings must have ${map.sources.length} scope trees`,
      );
    }
    const names = map.names ?? [];
  ...
  reverseMapTextRanges(url, textRange) {
    ...
    let startReverseIndex = Platform.ArrayUtilities.lowerBound(
      reverseMappings,
      textRange,
      ({ startLine, startColumn }, index) => {
    ...
    while (
      startReverseIndex === reverseMappings.length ||
      (startReverseIndex > 0 &&
  ...
  findRanges(predicate, options) {
    ...
    if (
      (mappings[0].lineNumber !== 0 || mappings[0].columnNumber !== 0) &&
      options?.isStartMatching
    ) {
    ...
    for (const { sourceURL, lineNumber, columnNumber } of mappings) {
      ...
      if (!current && ignoreListHint) {
        current = TextUtils.TextRange.TextRange.createUnboundedFromLocation(
          lineNumber,
          columnNumber,
        );
        ranges.push(current);
  ...
  compatibleForURL(sourceURL, other) {
    return (
      this.embeddedContentByURL(sourceURL) === other.embeddedContentByURL(sourceURL) &&
  ...
  nextVLQ() {
    ...
    while (digit & VLQ_CONTINUATION_MASK) {
      ...
      if (charCode !== 65 /* 'A' */ && digit === 0) {
        throw new Error(
          `Unexpected char '${String.fromCharCode(charCode)}' encountered while decoding`,
        );
      }
