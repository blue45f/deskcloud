...
export class TargetManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  #autoStartModels() {
    ...
    const shouldAutostart = (model, info) =>
  ...
  primaryPageTarget() {
    ...
    if (target?.type() === TargetType.TAB) {
      target =
        this.targets().find(
          (t) =>
            t.parentTarget() === target &&
            t.type() === TargetType.FRAME &&
  ...
  async maybeAttachInitialTarget() {
    ...
    const targetId =
      await Host.InspectorFrontendHost.InspectorFrontendHostInstance.initialTargetId();
  ...
  setScopeTarget(scopeTarget) {
    ...
    for (const target of this.targets()) {
      ...
      for (const modelClass of this.#modelObservers.keysArray()) {
        ...
        for (const observer of [...this.#modelObservers.get(modelClass)].filter((o) =>
          this.#scopedObservers.has(o),
        )) {
          observer.modelRemoved(model);
        }
      }
