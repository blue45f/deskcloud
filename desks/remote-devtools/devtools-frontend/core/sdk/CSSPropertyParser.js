...
const fontVariationSettingsRegexp = new RegExp(
  `(?:'(${tagRegexp.source})')|(?:"(${tagRegexp.source})")\\s+(${numRegexp.source})`,
);
/**
...
export class BottomUpTreeMatching extends TreeWalker {
  ...
  constructor(ast, matchers) {
    ...
    this.#matchers.push(
      ...matchers.filter((m) => !ast.propertyName || m.accepts(ast.propertyName)),
    );
  ...
  hasMatches(...matchTypes) {
    return Boolean(
      this.#matchedNodes
        .values()
        .find((match) => matchTypes.some((matchType) => match instanceof matchType)),
    );
  }
  getMatch(node) {
  ...
  hasUnresolvedSubstitutionsRange(from, to) {
    return this.computedText.hasUnresolvedSubstitutions(
      from.from - this.ast.tree.from,
      to.to - this.ast.tree.from,
    );
  }
...
class ComputedTextChunk {
  ...
  get topLevelValueCount() {
    ...
    if (this.#topLevelValueCount === null) {
      ...
      this.#topLevelValueCount = ASTUtils.siblings(
        ASTUtils.declValue(
          tokenizeDeclaration('--p', computedText ?? this.match.text)?.tree ?? null,
        ),
      ).length;
    }
...
export class ComputedText {
  ...
  *#getPieces(begin, end) {
  ...
  #countTopLevelValuesInStringPiece(piece) {
    ...
    if (count === undefined) {
      count = ASTUtils.siblings(
        ASTUtils.declValue(tokenizeDeclaration('--p', piece)?.tree ?? null),
      ).length;
  ...
  countTopLevelValues(begin, end) {
    ...
    const counts = pieces.map((chunk) =>
      chunk instanceof ComputedTextChunk
        ? chunk.topLevelValueCount
...
export function requiresSpace(a, b) {
  ...
  return (
    !/\s/.test(trailingChar) &&
    !/\s/.test(leadingChar) &&
    !noSpaceAfter.includes(trailingChar) &&
...
function declaration(rule) {
  ...
  return (
    cssParser.parse(rule).topNode.getChild('RuleSet')?.getChild('Block')?.getChild('Declaration') ??
    null
  );
}
