...
export class CSSProperty extends Common.ObjectWrapper.ObjectWrapper {
  ...
  ensureRanges() {
    ...
    this.#valueRange = rebase(
      text.toTextRange(valueSourceRange),
      range.startLine,
      range.startColumn,
    );
  ...
  async setText(propertyText, majorChange, overwrite) {
    ...
    if (majorChange) {
      ...
      if (this.ownerStyle.parentRule?.isKeyframeRule()) {
        HostModule.userMetrics.actionTaken(
          HostModule.UserMetrics.Action.StylePropertyInsideKeyframeEdited,
        );
      }
    ...
    const range = this.range.relativeTo(
    ...
    const indentation = this.ownerStyle.cssText
      ...
      : this.ownerStyle
          ...
          .get();
    const endIndentation = this.ownerStyle.cssText
      ? indentation.substring(0, this.ownerStyle.range.endColumn)
      : '';
    const text = new TextUtils.Text.Text(this.ownerStyle.cssText || '');
    const newStyleText = text.replaceRange(
      range,
      Platform.StringUtilities.sprintf(';%s;', propertyText),
    );
    ...
    function processToken(token, tokenType) {
      if (!insideProperty) {
        ...
        const isPropertyStart =
          tokenType?.includes('def') ||
          tokenType?.includes('string') ||
          tokenType?.includes('meta') ||
  ...
  async setDisabled(disabled) {
    ...
    const appendSemicolonIfMissing = (propertyText) =>
