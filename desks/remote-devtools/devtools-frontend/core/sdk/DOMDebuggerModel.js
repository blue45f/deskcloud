...
export class DOMDebuggerModel extends SDKModel {
  ...
  constructor(target) {
    ...
    this.#domBreakpoints = [];
    this.#domBreakpointsSetting = this.target()
      .targetManager()
  ...
  async eventListeners(remoteObject) {
    ...
    const listeners = await this.agent.invoke_getEventListeners({
      objectId: remoteObject.objectId,
    });
    const eventListeners = [];
    for (const payload of listeners.listeners || []) {
      const location = this.#runtimeModel
        .debuggerModel()
        .createRawLocationByScriptId(payload.scriptId, payload.lineNumber, payload.columnNumber);
      if (!location) {
        continue;
      }
      eventListeners.push(
        new EventListener(
          ...
          payload.originalHandler
            ? this.#runtimeModel.createRemoteObject(payload.originalHandler)
            : null,
          location,
          null,
        ),
  ...
  hasDOMBreakpoint(node, type) {
    return this.#domBreakpoints.some(
      (breakpoint) => breakpoint.node === node && breakpoint.type === type,
    );
  }
  setDOMBreakpoint(node, type) {
    ...
    this.dispatchEventToListeners(
      'DOMBreakpointAdded' /* Events.DOM_BREAKPOINT_ADDED */,
      breakpoint,
    );
    return breakpoint;
  }
  ...
  toggleDOMBreakpoint(breakpoint, enabled) {
    ...
    this.saveDOMBreakpoints();
    this.dispatchEventToListeners(
      'DOMBreakpointToggled' /* Events.DOM_BREAKPOINT_TOGGLED */,
      breakpoint,
    );
  }
  ...
  async documentUpdated() {
    ...
    this.#domBreakpoints = [];
    this.dispatchEventToListeners(
      'DOMBreakpointsRemoved' /* Events.DOM_BREAKPOINTS_REMOVED */,
      removed,
    );
    ...
    for (const breakpoint of this.#domBreakpointsSetting.get()) {
      if (breakpoint.url === currentURL) {
        void this.#domModel
          .pushNodeByPathToFrontend(breakpoint.path)
          .then(appendBreakpoint.bind(this, breakpoint));
      }
    }
  ...
  nodeRemoved(event) {
    ...
    this.removeDOMBreakpoints(
      (breakpoint) => breakpoint.node === node || children.indexOf(breakpoint.node) !== -1,
    );
  }
  saveDOMBreakpoints() {
    const currentURL = this.currentURL();
    const breakpoints = this.#domBreakpointsSetting
      .get()
      .filter((breakpoint) => breakpoint.url !== currentURL);
    for (const breakpoint of this.#domBreakpoints) {
      breakpoints.push({
        url: currentURL,
...
export class EventListener {
  ...
  canRemove() {
    return (
      Boolean(this.#customRemoveFunction) ||
      this.#origin !== 'FrameworkUser' /* EventListener.Origin.FRAMEWORK_USER */
    );
  }
  remove() {
    if (!this.canRemove()) {
...
export class DOMEventListenerBreakpoint extends CategorizedBreakpoint {
  ...
  updateOnModel(model) {
    for (const eventTargetName of this.eventTargetNames) {
      if (this.enabled()) {
        void model.agent.invoke_setEventListenerBreakpoint({
          eventName: this.name,
...
export class DOMDebuggerManager {
  ...
  constructor(targetManager = TargetManager.instance()) {
    this.#targetManager = targetManager;
    this.#xhrBreakpointsSetting = this.#targetManager.settings.createLocalSetting(
      'xhr-breakpoints',
      [],
    );
    ...
    this.#cspViolationsToBreakOn.push(
      new CSPViolationBreakpoint(
        'trusted-type-violation' /* Category.TRUSTED_TYPE_VIOLATION */,
    ...
    this.createEventListenerBreakpoints(
      'picture-in-picture' /* Category.PICTURE_IN_PICTURE */,
    ...
    this.createEventListenerBreakpoints(
      'dom-mutation' /* Category.DOM_MUTATION */,
      [
        'DOMActivate',
        'DOMFocusIn',
        'DOMFocusOut',
  ...
  resolveEventListenerBreakpoint({ eventName, targetName }) {
    ...
    for (const breakpoint of this.#eventListenerBreakpoints) {
      if (
        eventName &&
        breakpoint.name === eventName &&
        breakpoint.eventTargetNames.indexOf(targetName) !== -1
      ) {
        result = breakpoint;
      }
      if (
  ...
  updateCSPViolationBreakpoints() {
    const violationTypes = this.#cspViolationsToBreakOn
      .filter((v) => v.enabled())
      .map((v) => v.type());
    for (const model of this.#targetManager.models(DOMDebuggerModel)) {
