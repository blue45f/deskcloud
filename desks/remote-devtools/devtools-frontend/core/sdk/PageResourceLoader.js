...
export class PageResourceLoader extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static instance(
    { forceNew, targetManager, settings, userAgentProvider, loadOverride, maxConcurrentLoads } = {
      forceNew: false,
      loadOverride: null,
    },
  ) {
  ...
  onPrimaryPageChanged(event) {
    ...
    for (const [key, pageResource] of this.#pageResources.entries()) {
      if (
        type === 'Activation' /* PrimaryPageChangeType.ACTIVATION */ &&
        mainFrameTarget === pageResource.initiator.target
      ) {
  ...
  getScopedResourcesLoaded() {
    return new Map(
      [...this.#pageResources].filter(
        ([_, pageResource]) =>
          this.#targetManager.isInScope(pageResource.initiator.target) ||
  ...
  getNumberOfResources() {
    return {
      ...
      queued: this.#queuedLoads.length,
      resources: this.#pageResources.size,
    };
  }
  ...
  async dispatchLoad(url, initiator, isBinary) {
    ...
    if (eligibleForLoadFromTarget) {
      ...
      try {
        Host.userMetrics.developerResourceLoaded(
          0 /* Host.UserMetrics.DeveloperResourceLoaded.LOAD_THROUGH_PAGE_VIA_TARGET */,
        );
        const result = await this.loadFromTarget(
          initiator.target,
          initiator.frameId,
          url,
          isBinary,
        ...
        if (e instanceof Error) {
          Host.userMetrics.developerResourceLoaded(
            2 /* Host.UserMetrics.DeveloperResourceLoaded.LOAD_THROUGH_PAGE_FAILURE */,
          );
      ...
      Host.userMetrics.developerResourceLoaded(
        3 /* Host.UserMetrics.DeveloperResourceLoaded.LOAD_THROUGH_PAGE_FALLBACK */,
      );
    } else {
    ...
    if (eligibleForLoadFromTarget && !result.success) {
      Host.userMetrics.developerResourceLoaded(
        7 /* Host.UserMetrics.DeveloperResourceLoaded.FALLBACK_FAILURE */,
      );
    }
    if (failureReason) {
  ...
  getDeveloperResourceScheme(parsedURL) {
    ...
    switch (parsedURL.scheme) {
      ...
      case 'http':
        return isLocalhost
          ? 4 /* Host.UserMetrics.DeveloperResourceScheme.HTTP_LOCALHOST */
          : 2 /* Host.UserMetrics.DeveloperResourceScheme.HTTP */;
      case 'https':
        return isLocalhost
          ? 5 /* Host.UserMetrics.DeveloperResourceScheme.HTTPS_LOCALHOST */
  ...
  async loadFromTarget(target, frameId, url, isBinary) {
    ...
    const resource = await networkManager.loadNetworkResource(frameId, url, {
      disableCache,
    ...
    try {
      const content = resource.stream
        ? isBinary
          ? await ioModel.readToBuffer(resource.stream)
          : await ioModel.readToString(resource.stream)
        : '';
      return {
        ...
        errorDescription: {
          ...
          message:
            Host.ResourceLoader.netErrorToMessage(
              resource.netError,
  ...
  async loadFromHostBindings(url) {
    ...
    return await new Promise((resolve) =>
      Host.ResourceLoader.load(
        url,
        headers,
        (success, _responseHeaders, content, errorDescription) => {
