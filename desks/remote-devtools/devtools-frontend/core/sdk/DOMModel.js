...
export class DOMNode extends Common.ObjectWrapper.ObjectWrapper {
  ...
  init(doc, isInShadowTree, payload, retainedNodes) {
    ...
    if (payload.templateContent) {
      this.templateContentInternal = DOMNode.create(
        this.#domModel,
        this.ownerDocument,
        true,
    ...
    if (payload.contentDocument) {
      ...
      this.childDocumentPromiseForTesting = this.requestChildDocument(
        payload.frameId,
        this.#domModel.target(),
      );
      this.childrenInternal = [];
    }
    if (payload.importedDocument) {
      this.#importedDocument = DOMNode.create(
        this.#domModel,
        this.ownerDocument,
        true,
  ...
  carouselPseudoElements() {
    return [
      ...
      ...(this.#pseudoElements.get('scroll-marker' /* Protocol.DOM.PseudoType.ScrollMarker */) ||
        []),
      ...(this.#pseudoElements.get(
        'scroll-marker-group' /* Protocol.DOM.PseudoType.ScrollMarkerGroup */,
      ) || []),
    ];
  }
  hasAssignedSlot() {
  ...
  ancestorUserAgentShadowRoot() {
    ...
    return ancestorShadowRoot.shadowRootType() === DOMNode.ShadowRootTypes.UserAgent
      ? ancestorShadowRoot
      : null;
  }
  isShadowRoot() {
  ...
  setAttribute(name, text, callback) {
    void this.#agent
      .invoke_setAttributesAsText({ nodeId: this.id, text, name })
      .then((response) => {
        if (!response.getError()) {
          this.#domModel.markUndoableState();
        }
  ...
  async getOuterHTML(includeShadowDOM = false) {
    const { outerHTML } = await this.#agent.invoke_getOuterHTML({
      nodeId: this.id,
      includeShadowDOM,
    });
    return outerHTML;
  ...
  removeChild(node) {
    ...
    if (pseudoType) {
      const updatedPseudoElements = this.#pseudoElements
        .get(pseudoType)
  ...
  setAssignedSlot(payload) {
    this.assignedSlot = new DOMNodeShortcut(
      this.#domModel.target(),
      payload.backendNodeId,
      payload.nodeType,
      payload.nodeName,
  ...
  copyTo(targetNode, anchorNode, callback) {
    void this.#agent
      .invoke_copyTo({
        ...
        insertBeforeNodeId: anchorNode ? anchorNode.id : undefined,
      })
      .then((response) => {
        if (!response.getError()) {
          this.#domModel.markUndoableState();
  ...
  async callFunction(fn, args = []) {
    ...
    const result = await object.callFunction(
      fn,
      args.map((arg) => RemoteObject.toCallArgument(arg)),
    );
    object.release();
  ...
  simpleSelector() {
    ...
    if (classes) {
      ...
      return (
        (lowerCaseName === 'div' ? '' : lowerCaseName) +
        '.' +
  ...
  async takeSnapshot(ownerDocumentSnapshot) {
    const snapshot =
      this instanceof DOMDocument
        ? new DOMDocumentSnapshot(this.domModel(), {
            nodeId: this.id,
    ...
    if (this.childrenInternal) {
      ...
      for (const child of this.childrenInternal) {
        ...
        childSnapshot.ownerDocument =
          snapshot instanceof DOMDocument ? snapshot : snapshot.ownerDocument;
    ...
    if (this.templateContentInternal) {
      const templateSnapshot = await this.templateContentInternal.takeSnapshot(
        snapshot.ownerDocument || undefined,
      );
    ...
    if (this.#importedDocument) {
      const importedDocSnapshot = await this.#importedDocument.takeSnapshot(
        snapshot.ownerDocument || undefined,
      );
...
export class DeferredDOMNode {
  ...
  async resolvePromise() {
    const nodeIds = await this.#domModelInternal.pushNodesByBackendIdsToFrontend(
      new Set([this.#backendNodeIdInternal]),
    );
  ...
  highlight() {
    this.#domModelInternal
      .overlayModel()
      .highlightInOverlay({ deferredNode: this, selectorList: undefined });
  }
}
...
export class DOMModel extends SDKModel {
  ...
  constructor(target) {
    ...
    this.#resourceTreeModel?.addEventListener(
      ResourceTreeModelEvents.DocumentOpened,
  ...
  async requestDocumentInternal() {
    ...
    if (this.#frameOwnerNode) {
      ...
      if (this.#document) {
        ...
        this.dispatchEventToListeners(Events.NodeRemoved, {
          node: oldDocument,
          parent: this.#frameOwnerNode,
        });
      }
    }
  ...
  async pushNodesByBackendIdsToFrontend(backendNodeIds) {
    ...
    const { nodeIds } = await this.agent.invoke_pushNodesByBackendIdsToFrontend({
      backendNodeIds: backendNodeIdsArray,
    });
    if (!nodeIds) {
      return null;
    }
  ...
  inlineStyleInvalidated(nodeIds) {
    ...
    if (!this.#loadNodeAttributesTimeout) {
      this.#loadNodeAttributesTimeout = globalThis.setTimeout(
        this.loadNodeAttributes.bind(this),
        20,
      );
    }
  }
  ...
  async getNodesByStyle(computedStyles, pierce = false) {
    ...
    const response = await this.agent.invoke_getNodesForSubtreeByStyle({
      nodeId: this.#document.id,
      computedStyles,
      pierce,
    });
    if (response.getError()) {
  ...
  async searchResult(index) {
    ...
    const { nodeIds } = await this.agent.invoke_getSearchResults({
  ...
  classNamesPromise(nodeId) {
    return this.agent
      .invoke_collectClassNamesFromSubtree({ nodeId })
      .then(({ classNames }) => classNames || []);
  }
  ...
  topLayerElementsUpdated() {
    void this.#topLayerThrottler.schedule(async () => {
      ...
      const nodes = result.nodeIds
        .map((id) => this.idToDOMNode.get(id))
      ...
      for (const [document, nodes] of nodesByDocument) {
        ...
        for (const [idx, node] of nodes.entries()) {
          ...
          const previousNode = result.nodeIds[idx - 1]
            ? this.idToDOMNode.get(result.nodeIds[idx - 1])
            : null;
          if (previousNode && previousNode.nodeName() === '::backdrop') {
            childShortcuts.push(
              new DOMNodeShortcut(
                this.target(),
                previousNode.backendNodeId(),
                0,
                previousNode.nodeName(),
              ),
            );
          }
          const shortcut = new DOMNodeShortcut(
            this.target(),
            node.backendNodeId(),
            0,
            node.nodeName(),
  ...
  getElementByRelation(nodeId, relation) {
    return this.agent
      .invoke_getElementByRelation({ nodeId, relation })
      .then(({ nodeId }) => nodeId);
  }
  ...
  async nodeForLocation(x, y, includeUserAgentShadowDOM) {
    const response = await this.agent.invoke_getNodeForLocation({
      x,
      y,
  ...
  pushObjectAsNodeToFrontend(object) {
    return object.isNode() && object.objectId
      ? this.pushNodeToFrontend(object.objectId)
