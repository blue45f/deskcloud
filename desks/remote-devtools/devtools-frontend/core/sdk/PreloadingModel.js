...
export class PreloadingModel extends SDKModel {
  ...
  constructor(target) {
    ...
    if (targetInfo?.subtype === 'prerender') {
      this.lastPrimaryPageModel =
        target.targetManager().primaryPageTarget()?.model(PreloadingModel) || null;
    }
    target
      .targetManager()
      .addModelListener(
  ...
  onPrefetchStatusUpdated(event) {
    ...
    if (
      event.prefetchStatus ===
      'PrefetchEvictedAfterCandidateRemoved' /* Protocol.Preload.PrefetchStatus.PrefetchEvictedAfterCandidateRemoved */
    ) {
      return;
    }
...
class PreloadingAttemptRegistry {
  ...
  upsert(attempt) {
    ...
    if (attempt.pipelineId !== null) {
      this.pipelines
        .getOrInsertComputed(attempt.pipelineId, () => new Map())
        .set(attempt.action, id);
    }
  }
