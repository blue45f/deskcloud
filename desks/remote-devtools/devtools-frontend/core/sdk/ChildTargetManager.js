...
export class ChildTargetManager extends SDKModel {
  ...
  constructor(parentTarget) {
    ...
    if (browserTarget) {
      if (browserTarget !== parentTarget) {
        void browserTarget
          .targetAgent()
          .invoke_autoAttachRelated({ targetId: parentTarget.id(), waitForDebuggerOnStart: true });
      }
    } else if (parentTarget.type() === Type.NODE) {
      void this.#targetAgent.invoke_setAutoAttach({
        autoAttach: true,
  ...
  static install(attachCallback) {
    ...
    SDKModel.register(ChildTargetManager, {
      capabilities: 32 /* Capability.TARGET */,
      autostart: true,
    });
  }
  childTargets() {
  ...
  async suspendModel() {
    await this.#targetAgent.invoke_setAutoAttach({
      autoAttach: true,
      ...
      if (target.targetInfo()?.subtype === 'prerender' && !targetInfo.subtype) {
        ...
        if (resourceTreeModel?.mainFrame) {
          resourceTreeModel.primaryPageChanged(
            ...
            'Activation' /* PrimaryPageChangeType.ACTIVATION */,
          );
        }
  ...
  fireAvailableTargetsChanged() {
    this.#targetManager.dispatchEventToListeners(
      'AvailableTargetsChanged' /* TargetManagerEvents.AVAILABLE_TARGETS_CHANGED */,
  ...
  async getParentTargetId() {
    if (!this.#parentTargetId) {
      this.#parentTargetId = (
        await this.#parentTarget.targetAgent().invoke_getTargetInfo({})
      ).targetInfo.targetId;
    }
  ...
  async attachedToTarget({ sessionId, targetInfo, waitingForDebugger }) {
    ...
    if (targetInfo.type === 'worker' && targetInfo.title && targetInfo.title !== targetInfo.url) {
      ...
      if (KNOWN_FRAME_PATTERNS.some((p) => targetInfo.url.match(p))) {
        ...
        targetName = parsedURL
          ? parsedURL.lastPathComponentWithFragment()
          : '#' + ++ChildTargetManager.lastAnonymousTargetId;
      }
    }
