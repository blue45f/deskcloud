...
export function buildOriginalScopes(ranges) {
  ...
  for (const range of ranges) {
     ...
     */
    if (
      comparePositions(range.start, stackTop.end) < 0 &&
      comparePositions(stackTop.end, range.end) < 0
    ) {
      throw new Error(
        `Range ${JSON.stringify(range)} and ${JSON.stringify(stackTop)} partially overlap.`,
      );
    }
