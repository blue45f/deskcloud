...
export class NetworkRequest extends Common.ObjectWrapper.ObjectWrapper {
  ...
  pseudoWallTime(monotonicTime) {
    return this.#wallIssueTime
      ? this.#wallIssueTime - this.#issueTime + monotonicTime
      : monotonicTime;
  }
  ...
  hasMatchingServiceWorkerRouter() {
    ...
    return (
      this.#serviceWorkerRouterInfo !== undefined &&
  ...
  set mimeType(x) {
    this.#mimeType = x;
    if (
      x === 'text/event-stream' /* Platform.MimeType.MimeType.EVENTSTREAM */ &&
      !this.#serverSentEvents
    ) {
      const parseFromStreamedData =
        this.resourceType() !== Common.ResourceType.resourceTypes.EventSource;
  ...
  parseNameAndPathFromURL() {
    if (this.#parsedURL.isDataURL()) {
      ...
      const inspectedURL = networkManager
        ? Common.ParsedURL.ParsedURL.fromString(networkManager.target().inspectedURL())
        : null;
      ...
      if (this.#parsedURL.lastPathComponent || this.#parsedURL.queryParams) {
        this.#name =
          this.#parsedURL.lastPathComponent +
          (this.#parsedURL.queryParams ? '?' + this.#parsedURL.queryParams : '');
      } else if (this.#parsedURL.folderPathComponents) {
        this.#name =
          this.#parsedURL.folderPathComponents.substring(
  ...
  requestStreamingContent() {
    ...
    const contentPromise = this.finished
      ? this.requestContentData()
      : NetworkManager.streamResponseBody(this);
  ...
  addExtraRequestInfo(extraRequestInfo) {
    ...
    this.#hasThirdPartyCookiePhaseoutIssue = this.#blockedRequestCookies.some((item) =>
      item.blockedReasons.includes(
        'ThirdPartyPhaseout' /* Protocol.Network.CookieBlockedReason.ThirdPartyPhaseout */,
      ),
    );
  }
  ...
  addExtraResponseInfo(extraResponseInfo) {
    ...
    this.#responseCookiesPartitionKey = extraResponseInfo.cookiePartitionKey
      ? extraResponseInfo.cookiePartitionKey
      : null;
    ...
    this.originalResponseHeaders = extraResponseInfo.responseHeaders.map((headerEntry) => ({
      ...headerEntry,
    }));
    ...
    for (const blockedCookie of this.#blockedResponseCookies) {
      if (
        blockedCookie.blockedReasons.includes(
          'NameValuePairExceedsMaxSize' /* Protocol.Network.SetCookieBlockedReason.NameValuePairExceedsMaxSize */,
        )
      ) {
        const message = i18nString(UIStrings.setcookieHeaderIsIgnoredIn, {
          PH1: this.url(),
        });
        networkManager.dispatchEventToListeners(NetworkManagerEvents.MessageGenerated, {
          message,
  ...
  redirectSourceSignedExchangeInfoHasNoErrors() {
    return (
      ...
      this.#redirectSource.#signedExchangeInfo !== null &&
...
export const setCookieBlockedReasonToUiString = function (blockedReason) {
  switch (blockedReason) {
    ...
    case 'SchemefulSameSiteStrict' /* Protocol.Network.SetCookieBlockedReason.SchemefulSameSiteStrict */:
      return i18nString(UIStrings.thisSetcookieWasBlockedBecauseItHadTheSamesiteStrictLax, {
        PH1: 'SameSite=Strict',
      });
