...
export class OverlayPersistentHighlighter {
  ...
  constructor(model, settings, callbacks) {
    ...
    this.#persistentHighlightSetting = settings.createLocalSetting(
  ...
  refreshHighlights() {
    ...
    const containerQueriesNeedUpdate = this.updateHighlightsForDeletedNodes(
      this.#containerQueryHighlights,
    );
    const isolatedElementsNeedUpdate = this.updateHighlightsForDeletedNodes(
      this.#isolatedElementHighlights,
    );
    if (
      flexboxesNeedUpdate ||
      gridsNeedUpdate ||
      scrollSnapsNeedUpdate ||
      containerQueriesNeedUpdate ||
  ...
  resetOverlay() {
    ...
    for (const nodeId of this.#containerQueryHighlights.keys()) {
      this.#containerQueryHighlights.set(
        nodeId,
  ...
  updateHighlightsInOverlay() {
    const hasNodesToHighlight =
      this.#gridHighlights.size > 0 ||
      this.#flexHighlights.size > 0 ||
      this.#containerQueryHighlights.size > 0 ||
      this.#isolatedElementHighlights.size > 0;
  ...
  updateScrollSnapHighlightsInOverlay() {
    ...
    ] of this.#scrollSnapHighlights.entries()) {
      scrollSnapHighlightConfigs.push({ nodeId, scrollSnapContainerHighlightConfig });
    }
    overlayModel
      .target()
      .overlayAgent()
      .invoke_setShowScrollSnapOverlays({ scrollSnapHighlightConfigs });
  }
  updateContainerQueryHighlightsInOverlay() {
    ...
    ] of this.#containerQueryHighlights.entries()) {
      containerQueryHighlightConfigs.push({ nodeId, containerQueryContainerHighlightConfig });
    }
    overlayModel
      .target()
      .overlayAgent()
      .invoke_setShowContainerQueryOverlays({ containerQueryHighlightConfigs });
  }
  updateIsolatedElementHighlightsInOverlay() {
    ...
    ] of this.#isolatedElementHighlights.entries()) {
      isolatedElementHighlightConfigs.push({ nodeId, isolationModeHighlightConfig });
    }
    overlayModel
      .target()
      .overlayAgent()
      .invoke_setShowIsolatedElements({ isolatedElementHighlightConfigs });
  }
  async restoreHighlightsForDocument() {
    ...
    await Promise.all(
      this.#persistentHighlightSetting.get().map(async (persistentHighlight) => {
        if (persistentHighlight.url === currentURL) {
          return await this.#model
            .getDOMModel()
            .pushNodeByPathToFrontend(persistentHighlight.path)
            .then((nodeId) => {
              ...
              switch (persistentHighlight.type) {
                ...
                case 'FLEX' /* HighlightType.FLEX */:
                  this.#flexHighlights.set(
                    node.id,
                    this.buildFlexContainerHighlightConfig(node.id),
                  );
                ...
                case 'CONTAINER_QUERY' /* HighlightType.CONTAINER_QUERY */:
                  ...
                  this.#callbacks.onContainerQueryOverlayStateChanged({
                    nodeId: node.id,
                    enabled: true,
                  });
                  break;
                case 'SCROLL_SNAP' /* HighlightType.SCROLL_SNAP */:
                  this.#scrollSnapHighlights.set(
                    node.id,
                    this.buildScrollSnapContainerHighlightConfig(node.id),
                  );
                  this.#callbacks.onScrollSnapOverlayStateChanged({
                    nodeId: node.id,
                    enabled: true,
                  });
                  break;
                case 'ISOLATED_ELEMENT' /* HighlightType.ISOLATED_ELEMENT */:
                  this.#isolatedElementHighlights.set(
                    node.id,
  ...
  savePersistentHighlightSetting() {
    ...
    const highlightsInOtherDocuments = this.#persistentHighlightSetting
      .get()
      .filter((persistentSetting) => persistentSetting.url !== currentURL);
    const persistentHighlights = [
      ...highlightsInOtherDocuments,
      ...this.getPersistentHighlightSettingForOneType(
        this.#gridHighlights,
        'GRID' /* HighlightType.GRID */,
      ),
      ...this.getPersistentHighlightSettingForOneType(
        this.#flexHighlights,
        'FLEX' /* HighlightType.FLEX */,
      ),
      ...this.getPersistentHighlightSettingForOneType(
        this.#containerQueryHighlights,
        'CONTAINER_QUERY' /* HighlightType.CONTAINER_QUERY */,
      ),
      ...this.getPersistentHighlightSettingForOneType(
        this.#scrollSnapHighlights,
        'SCROLL_SNAP' /* HighlightType.SCROLL_SNAP */,
      ),
      ...this.getPersistentHighlightSettingForOneType(
        this.#isolatedElementHighlights,
        'ISOLATED_ELEMENT' /* HighlightType.ISOLATED_ELEMENT */,
      ),
    ];
