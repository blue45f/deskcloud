...
export class ConsoleModel extends SDKModel {
  ...
  constructor(target) {
    ...
    const eventListener = resourceTreeModel.addEventListener(
      ResourceTreeModelEvents.CachedResourcesLoaded,
      () => {
    ...
    if (cpuProfilerModel) {
      eventListeners.push(
        cpuProfilerModel.addEventListener(
          'ConsoleProfileStarted' /* CPUProfilerModelEvents.CONSOLE_PROFILE_STARTED */,
          this.consoleProfileStarted.bind(this, cpuProfilerModel),
        ),
      );
      eventListeners.push(
        cpuProfilerModel.addEventListener(
          'ConsoleProfileFinished' /* CPUProfilerModelEvents.CONSOLE_PROFILE_FINISHED */,
    ...
    if (runtimeModel) {
      ...
      if (target.parentTarget()?.type() !== Type.FRAME) {
        eventListeners.push(
          runtimeModel
            .debuggerModel()
            .addEventListener(DebuggerModelEvents.GlobalObjectCleared, this.clearIfNecessary, this),
        );
      }
      eventListeners.push(
        runtimeModel.addEventListener(
  ...
  async evaluateCommandInConsole(
    ...
    this.dispatchEventToListeners(Events.CommandEvaluated, {
  ...
  addMessage(msg) {
    ...
    if (
      msg.source === Common.Console.FrontendMessageSource.ConsoleAPI &&
  ...
  exceptionThrown(runtimeModel, event) {
    ...
    const affectedResources = extractExceptionMetaData(
      exceptionWithTimestamp.details.exceptionMetaData,
    );
  ...
  consoleAPICalled(runtimeModel, event) {
    ...
    if (call.type === 'debug' /* Protocol.Runtime.ConsoleAPICalledEventType.Debug */) {
      ...
      call.type === 'error' /* Protocol.Runtime.ConsoleAPICalledEventType.Error */ ||
      ...
      call.type === 'info' /* Protocol.Runtime.ConsoleAPICalledEventType.Info */ ||
    ...
    if (settings.moduleSetting('preserve-console-log').get()) {
      ...
      if (frame.backForwardCacheDetails.restoredFromCache) {
        Common.Console.Console.instance().log(
          i18nString(UIStrings.bfcacheNavigation, { PH1: frame.url }),
        );
      } else {
        Common.Console.Console.instance().log(
          i18nString(UIStrings.navigatedToS, { PH1: frame.url }),
        );
      }
    }
  }
  consoleProfileStarted(cpuProfilerModel, event) {
    ...
    this.addConsoleProfileMessage(
      cpuProfilerModel,
      'profile' /* Protocol.Runtime.ConsoleAPICalledEventType.Profile */,
      data.scriptLocation,
  ...
  consoleProfileFinished(cpuProfilerModel, event) {
    ...
    this.addConsoleProfileMessage(
      cpuProfilerModel,
      'profileEnd' /* Protocol.Runtime.ConsoleAPICalledEventType.ProfileEnd */,
      data.scriptLocation,
  ...
  async saveToTempVariable(currentExecutionContext, remoteObject) {
    ...
    const callFunctionResult = await globalObject.callFunction(saveVariable, [
      RemoteObject.toCallArgument(remoteObject),
    ]);
...
function areStackTracesEquivalent(stackTrace1, stackTrace2) {
  ...
  for (let i = 0, n = callFrames1.length; i < n; ++i) {
    if (
      callFrames1[i].scriptId !== callFrames2[i].scriptId ||
...
export class ConsoleMessage {
  ...
  constructor(runtimeModel, source, level, messageText, details) {
    ...
    if (!this.#executionContextId && this.#runtimeModel) {
      if (this.scriptId) {
        ...
        this.#executionContextId = this.#runtimeModel.executionContextForStackTrace(
          this.stackTrace,
        );
      }
    }
  ...
  isGroupMessage() {
    return (
      this.type === 'startGroup' /* Protocol.Runtime.ConsoleAPICalledEventType.StartGroup */ ||
      this.type ===
  ...
  isErrorOrWarning() {
    return (
      this.level === 'warning' /* Protocol.Log.LogEntryLevel.Warning */ ||
      this.level === 'error' /* Protocol.Log.LogEntryLevel.Error */
    );
  }
  isGroupable() {
    const isUngroupableError =
      this.level === 'error' /* Protocol.Log.LogEntryLevel.Error */ &&
      (this.source === 'javascript' /* Protocol.Log.LogEntrySource.Javascript */ ||
        this.source === 'network') /* Protocol.Log.LogEntrySource.Network */;
    return (
      this.source !== Common.Console.FrontendMessageSource.ConsoleAPI &&
      this.type !== FrontendMessageType.Command &&
      this.type !== FrontendMessageType.Result &&
      this.type !== FrontendMessageType.System &&
      !isUngroupableError
    );
  }
  groupCategoryKey() {
  ...
  isEqual(msg) {
    ...
    if (this.parameters) {
      ...
      for (let i = 0; i < msg.parameters.length; ++i) {
        ...
        if (msgParam.type === 'object' && msgParam.subtype !== 'error') {
          if (
            !msgParam.objectId ||
            msgParam.objectId !== param.objectId ||
        ...
        if (
          param.type !== msgParam.type ||
          param.value !== msgParam.value ||
    ...
    return (
      this.runtimeModel() === msg.runtimeModel() &&
  ...
  get originatesFromConditionalBreakpoint() {
    return (
      this.#originatingBreakpointType ===
  ...
  static #stackFrameWithBreakpoint({ callFrames }) {
    ...
    const lastBreakpointFrameIndex = callFrames.findLastIndex(({ url }) =>
      markerSourceUrls.includes(url),
    );
    ...
    const type =
      callFrames[lastBreakpointFrameIndex].url === LOGPOINT_SOURCE_URL
