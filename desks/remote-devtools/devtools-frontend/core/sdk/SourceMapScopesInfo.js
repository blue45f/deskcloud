...
export class SourceMapScopesInfo {
  ...
  static createFromAst(sourceMap, scopeTree, text) {
    ...
    while (stack.length > 0) {
      ...
      const canMapOriginalPosition =
        startEntry &&
        endEntry &&
        sourceIndex !== undefined &&
        startEntry.sourceIndex === endEntry.sourceIndex &&
        startEntry.sourceIndex !== undefined &&
        sourceIndex >= 0 &&
        sourceIndex < numSourceUrls;
      const isStackFrame =
        node.kind === 2 /* Formatter.FormatterWorkerPool.ScopeKind.FUNCTION */ ||
      ...
      for (let i = node.children.length - 1; i >= 0; --i) {
        stack.push({
          ...
          parentRange: range,
          parentScopeHint: nextParentScopeHint,
        });
      }
    }
    return new SourceMapScopesInfo(sourceMap, {
      scopes: scopeBySourceUrl,
      ranges: rootRange ? [rootRange] : [],
    });
    /**
    ...
    for (let i = rangeChain.length - 1; i >= 0; --i) {
      ...
      if (range.callSite) {
        ...
        result.inlinedFunctions.push({
          ...
          callsite: {
            ...range.callSite,
            sourceURL: this.#sourceMap.sourceURLForSourceIndex(range.callSite.sourceIndex),
          },
        });
      }
  ...
  resolveMappedScopeChain(callFrame) {
    ...
    for (
      let originalScope = rangeChain.at(-1)?.originalScope;
      originalScope;
    ...
    if (callFrame.returnValue() !== null) {
      while (
        result.length &&
        result[0].type() !== 'local' /* Protocol.Debugger.ScopeType.Local */
      ) {
        result.shift();
      }
    }
  ...
  #findGeneratedRangeChainForFrame(callFrame) {
    const rangeChain = this.#findGeneratedRangeChain(
  ...
  findOriginalFunctionScope({ line, column }) {
    ...
    const url =
      sourceIndex !== -1 ? this.#sourceMap.sourceURLForSourceIndex(sourceIndex) : undefined;
  ...
  #findFunctionScopeInOriginalScopeChain(innerOriginalScope) {
    for (
      let originalScope = innerOriginalScope;
      originalScope;
      originalScope = originalScope.parent
    ) {
    ...
    const result = [
      {
        ...
        name:
          this.findOriginalFunctionName({ line: generatedLine, column: generatedColumn }) ??
          undefined,
