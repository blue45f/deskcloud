...
export class HeapProfilerModel extends SDKModel {
  ...
  async addInspectedHeapObject(snapshotObjectId) {
    const response = await this.#heapProfilerAgent.invoke_addInspectedHeapObject({
      heapObjectId: snapshotObjectId,
    });
    return Boolean(response.getError());
  }
  ...
  async startTrackingHeapObjects(recordAllocationStacks) {
    const response = await this.#heapProfilerAgent.invoke_startTrackingHeapObjects({
      trackAllocations: recordAllocationStacks,
    });
    return Boolean(response.getError());
  }
  async stopTrackingHeapObjects(reportProgress) {
    const response = await this.#heapProfilerAgent.invoke_stopTrackingHeapObjects({
      reportProgress,
    });
    return Boolean(response.getError());
  }
  ...
  lastSeenObjectId(lastSeenObjectId, timestamp) {
    this.dispatchEventToListeners('LastSeenObjectId' /* Events.LAST_SEEN_OBJECT_ID */, {
      lastSeenObjectId,
      timestamp,
    });
  }
  addHeapSnapshotChunk(chunk) {
    this.dispatchEventToListeners(
      'AddHeapSnapshotChunk' /* Events.ADD_HEAP_SNAPSHOT_CHUNK */,
      chunk,
    );
  }
  reportHeapSnapshotProgress(done, total, finished) {
    this.dispatchEventToListeners(
      'ReportHeapSnapshotProgress' /* Events.REPORT_HEAP_SNAPSHOT_PROGRESS */,
      { done, total, finished },
    );
  }
  resetProfiles() {
