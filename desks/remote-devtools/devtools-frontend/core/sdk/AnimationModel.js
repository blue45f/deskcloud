...
export class AnimationDOMNode {
  ...
  async addScrollEventListener(onScroll) {
    ...
    await object.callFunction(
      ...
      [id, REPORT_SCROLL_POSITION_BINDING_NAME, getScrollListenerNameInPage(id)].map((arg) =>
        RemoteObject.toCallArgument(arg),
      ),
    );
    object.release();
    return id;
  ...
  async verticalScrollRange() {
    return await this.#domNode
      .callFunction(verticalScrollRangeInPage)
  ...
  async horizontalScrollRange() {
    return await this.#domNode
      .callFunction(horizontalScrollRangeInPage)
      .then((res) => res?.value ?? null);
...
function shouldGroupAnimations(firstAnimation, anim) {
  ...
  if (firstAnimationTimeline) {
    ...
    return Boolean(
      animationTimeline &&
      firstAnimationTimeline.sourceNodeId === animationTimeline.sourceNodeId &&
...
export class AnimationModel extends SDKModel {
  ...
  async devicePixelRatio() {
    const evaluateResult = await this.target()
      .runtimeAgent()
  ...
  async animationUpdated(payload) {
    ...
    let foundAnimation;
    for (const animationGroup of this.animationGroups.values()) {
      foundAnimation = animationGroup
        .animations()
        .find((animation) => animation.id() === payload.id);
      if (foundAnimation) {
...
export class KeyframesRule {
  ...
  setPayload(payload) {
    this.#payload = payload;
    if (!this.#keyframes) {
      this.#keyframes = this.#payload.keyframes.map(
        (keyframeStyle) => new KeyframeStyle(keyframeStyle),
      );
    } else {
...
export class AnimationGroup {
  ...
  seekTo(currentTime) {
    void this.#animationModel.agent.invoke_seekAnimations({
  ...
  currentTimePromise() {
    ...
    return this.#animationModel.agent
      .invoke_getCurrentTime({ id: longestAnim.id() })
      .then(({ currentTime }) => currentTime || 0);
  }
  matches(group) {
    function extractId(anim) {
      const timelineId =
        (anim.viewOrScrollTimeline()?.sourceNodeId ?? '') +
        (anim.viewOrScrollTimeline()?.axis ?? '');
      const regularId =
        anim.type() === 'WebAnimation' /* Protocol.Animation.AnimationType.WebAnimation */
          ? anim.type() + anim.id()
          : anim.cssId();
