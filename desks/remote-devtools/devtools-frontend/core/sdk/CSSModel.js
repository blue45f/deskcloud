...
export class CSSModel extends SDKModel {
  ...
  async colorScheme() {
    if (!this.#colorScheme) {
      const colorSchemeResponse = await this.domModel()
        ?.target()
        .runtimeAgent()
        .invoke_evaluate({
          expression:
      ...
      if (
        colorSchemeResponse &&
        !colorSchemeResponse.exceptionDetails &&
        !colorSchemeResponse.getError()
      ) {
        this.#colorScheme = colorSchemeResponse.result.value
  ...
  createRawLocationsByURL(sourceURL, lineNumber, columnNumber = 0) {
    ...
    for (
      ...
      index >= 0 &&
      headers[index].startLine === last.startLine &&
      headers[index].startColumn === last.startColumn;
      --index
    ) {
  ...
  static trimSourceURL(text) {
    ...
    return (
      text.substr(0, sourceURLLineIndex) +
  ...
  async setStyleText(styleSheetId, range, text, majorChange) {
    try {
      ...
      const { styles } = await this.agent.invoke_setStyleTexts({
        edits: [{ styleSheetId, range: range.serializeToObject(), text }],
      });
      if (styles?.length !== 1) {
        return false;
      }
  ...
  async setSelectorText(styleSheetId, range, text) {
    ...
    try {
      await this.ensureOriginalStyleSheetText(styleSheetId);
      const { selectorList } = await this.agent.invoke_setRuleSelector({
        styleSheetId,
        range,
        selector: text,
      });
  ...
  async setPropertyRulePropertyName(styleSheetId, range, text) {
    ...
    try {
      await this.ensureOriginalStyleSheetText(styleSheetId);
      const { propertyName } = await this.agent.invoke_setPropertyRulePropertyName({
        styleSheetId,
        range,
        propertyName: text,
      });
  ...
  async setKeyframeKey(styleSheetId, range, text) {
    ...
    try {
      await this.ensureOriginalStyleSheetText(styleSheetId);
      const { keyText } = await this.agent.invoke_setKeyframeKey({
        styleSheetId,
        range,
        keyText: text,
      });
  ...
  async getMatchedStyles(nodeId) {
    ...
    const shouldGetAnimatedStyles =
      Root.Runtime.hostConfig.devToolsAnimationStylesInStylesTab?.enabled;
  ...
  async getLayoutPropertiesFromComputedStyle(nodeId) {
    ...
    const isContainer =
      Boolean(containerType) && containerType !== '' && containerType !== 'normal';
    const hasScroll =
      Boolean(styles.get('scroll-snap-type')) && styles.get('scroll-snap-type') !== 'none';
    return {
      isFlex,
      isGrid,
      isSubgrid,
  ...
  async setMediaText(styleSheetId, range, newMediaText) {
    ...
    try {
      await this.ensureOriginalStyleSheetText(styleSheetId);
      const { media } = await this.agent.invoke_setMediaText({
        styleSheetId,
        range,
        text: newMediaText,
      });
  ...
  async setContainerQueryText(styleSheetId, range, newContainerQueryText) {
    ...
    try {
      await this.ensureOriginalStyleSheetText(styleSheetId);
      const { containerQuery } = await this.agent.invoke_setContainerQueryText({
        styleSheetId,
        range,
        text: newContainerQueryText,
      });
      if (!containerQuery) {
        return false;
      }
  ...
  async setSupportsText(styleSheetId, range, newSupportsText) {
    ...
    try {
      await this.ensureOriginalStyleSheetText(styleSheetId);
      const { supports } = await this.agent.invoke_setSupportsText({
        styleSheetId,
        range,
        text: newSupportsText,
      });
      if (!supports) {
        return false;
      }
  ...
  async setNavigationText(styleSheetId, range, newNavigationText) {
    ...
    try {
      await this.ensureOriginalStyleSheetText(styleSheetId);
      const { navigation } = await this.agent.invoke_setNavigationText({
        styleSheetId,
        range,
        text: newNavigationText,
      });
      if (!navigation) {
        return false;
      }
  ...
  async setScopeText(styleSheetId, range, newScopeText) {
    ...
    try {
      await this.ensureOriginalStyleSheetText(styleSheetId);
      const { scope } = await this.agent.invoke_setScopeText({
        styleSheetId,
        range,
        text: newScopeText,
      });
  ...
  async addRule(styleSheetId, ruleText, ruleLocation) {
    try {
      await this.ensureOriginalStyleSheetText(styleSheetId);
      const { rule } = await this.agent.invoke_addRule({
        styleSheetId,
        ruleText,
  ...
  async requestViaInspectorStylesheet(maybeFrameId) {
    const frameId =
      maybeFrameId ||
      (this.#resourceTreeModel && this.#resourceTreeModel.mainFrame
        ? this.#resourceTreeModel.mainFrame.id
        : null);
    const headers = [...this.#styleSheetIdToHeader.values()];
    const styleSheetHeader = headers.find(
      (header) => header.frameId === frameId && header.isViaInspector(),
    );
    if (styleSheetHeader) {
  ...
  async setStyleSheetText(styleSheetId, newText, majorChange) {
    ...
    await this.ensureOriginalStyleSheetText(styleSheetId);
    const response = await this.agent.invoke_setStyleSheetText({
    ...
    if (this.#isCSSPropertyTrackingEnabled) {
      ...
      if (this.#cssPropertyTracker) {
        this.#cssPropertyTracker.dispatchEventToListeners(
          'TrackedCSSPropertiesUpdated' /* CSSPropertyTrackerEvents.TRACKED_CSS_PROPERTIES_UPDATED */,
...
class ComputedStyleLoader {
  ...
  #getResponsePromise(nodeId) {
    ...
    promise = this.#cssModel
      .getAgent()
      .invoke_getComputedStyleForNode({ nodeId })
      .then(({ computedStyle, extraFields }) => {
