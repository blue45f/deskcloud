...
export class ResourceTreeModel extends SDKModel {
  ...
  constructor(target) {
    ...
    if (networkManager) {
      networkManager.addEventListener(
        NetworkManagerEvents.RequestFinished,
        this.onRequestFinished,
        this,
      );
      networkManager.addEventListener(
        NetworkManagerEvents.RequestUpdateDropped,
  ...
  static frameForRequest(request) {
    ...
    const resourceTreeModel = networkManager
      ? networkManager.target().model(ResourceTreeModel)
      : null;
  ...
  frameAttached(frameId, parentFrameId, stackTrace) {
    const sameTargetParentFrame = parentFrameId
      ? this.framesInternal.get(parentFrameId) || null
      : null;
  ...
  frameNavigated(framePayload, type) {
    const sameTargetParentFrame = framePayload.parentId
      ? this.framesInternal.get(framePayload.parentId) || null
      : null;
    // Do nothing unless cached resource tree is processed - it will overwrite everything.
    if (!this.#cachedResourcesProcessed && sameTargetParentFrame) {
      return;
    ...
    if (type) {
      frame.backForwardCacheDetails.restoredFromCache =
        type ===
        'BackForwardCacheRestore' /* Protocol.Page.NavigationType.BackForwardCacheRestore */;
    }
    if (frame.isMainFrame()) {
  ...
  createResourceFromFramePayload(frame, url, type, mimeType, lastModifiedTime, contentSize) {
    const lastModified =
      typeof lastModifiedTime === 'number' ? new Date(lastModifiedTime * 1000) : null;
    return new Resource(
      this,
      null,
      url,
  ...
  async getStorageKeyData() {
    ...
    for (const { isMainFrame, storageKey } of await Promise.all(
      [...this.framesInternal.values()].map((f) =>
        f.getStorageKey(/* forceFetch */ false).then((k) => ({
  ...
  onBackForwardCacheNotUsed(event) {
    if (
      this.mainFrame &&
      this.mainFrame.id === event.frameId &&
      this.mainFrame.loaderId === event.loaderId
    ) {
...
export class ResourceTreeFrame {
  ...
  isCrossOriginIsolated() {
    return (
      ...
      this.#crossOriginIsolatedContextType.startsWith('Isolated')
    );
  }
  ...
  isOutermostFrame() {
    return (
      this.#model.target().parentTarget()?.type() !== Type.FRAME &&
      !this.#sameTargetParentFrame &&
  ...
  async highlight() {
    ...
    if (document) {
      this.resourceTreeModel()
        .domModel()
        .overlayModel()
        .highlightInOverlay({ node: document, selectorList: '' }, 'all', true);
    }
  }
  async getPermissionsPolicyState() {
    const response = await this.resourceTreeModel()
      .target()
      .pageAgent()
      .invoke_getPermissionsPolicyState({ frameId: this.#id });
    if (response.getError()) {
      return null;
    }
  ...
  async getOriginTrials() {
    const response = await this.resourceTreeModel()
      .target()
      .pageAgent()
      .invoke_getOriginTrials({ frameId: this.#id });
    if (response.getError()) {
      return [];
    }
...
export class PageDispatcher {
  ...
  loadEventFired({ timestamp }) {
    this.#resourceTreeModel.dispatchEventToListeners(Events.Load, {
      resourceTreeModel: this.#resourceTreeModel,
      loadTime: timestamp,
    });
  }
  ...
  frameDetached({ frameId, reason }) {
    this.#resourceTreeModel.frameDetached(
      frameId,
      reason === 'swap' /* Protocol.Page.FrameDetachedEventReason.Swap */,
    );
  }
...
SDKModel.register(ResourceTreeModel, {
  capabilities: 2 /* Capability.DOM */,
  autostart: true,
  early: true,
});
