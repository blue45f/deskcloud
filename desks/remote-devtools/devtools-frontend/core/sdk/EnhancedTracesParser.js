...
export class EnhancedTracesParser {
  ...
  parseEnhancedTrace() {
    for (const event of this.#trace.traceEvents) {
      if (this.isTracingStartedInBrowser(event)) {
        ...
        const data = event.args?.data;
        this.#scriptToV8Context.set(
          this.getScriptIsolateId(data.isolate, data.scriptId),
          data.v8context,
        );
        this.#scriptToFrame.set(this.getScriptIsolateId(data.isolate, data.scriptId), data.frame);
        ...
        if (
          !this.#executionContexts.find(
            (executionContext) => executionContext.v8Context === data.v8context,
          )
        ) {
        ...
        if (
          !this.#scripts.find(
            (script) =>
              script.scriptId === String(data.scriptId) && script.isolate === String(data.isolate),
          )
        ) {
          this.#scripts.push({
        ...
        if ('splitIndex' in data && 'splitCount' in data) {
          if (!this.#largeScriptToScriptSource.has(scriptIsolateId)) {
            this.#largeScriptToScriptSource.set(
              scriptIsolateId,
              new Array(data.splitCount).fill(''),
            );
          }
  ...
  data() {
    ...
    this.#scriptRundownEvents.forEach((scriptRundownEvent) => {
      ...
      const v8Context = this.#scriptToV8Context.get(
        this.getScriptIsolateId(data.isolate, data.scriptId),
      );
      if (v8Context) {
  ...
  getSourceMapFromMetadata(script) {
    ...
    const resolvedSourceMapUrl = Common.ParsedURL.ParsedURL.completeURL(
      resolvedSourceUrl,
      sourceMapURL,
    );
    ...
    const { sourceMap } =
      this.#trace.metadata.sourceMaps.find((m) => m.sourceMapUrl === resolvedSourceMapUrl) ?? {};
    return sourceMap;
  }
  ...
  isRundownScript(event) {
    return (
      this.isTraceEvent(event) && event.cat === 'disabled-by-default-devtools.v8-source-rundown'
    );
  }
  isRundownScriptSource(event) {
    return (
      this.isTraceEvent(event) &&
  ...
  isFunctionCallEvent(event) {
    return (
      this.isTraceEvent(event) && event.cat === 'devtools.timeline' && event.name === 'FunctionCall'
    );
  }
  groupContextsAndScriptsUnderTarget(targets, executionContexts, scripts, resources) {
    ...
    for (const script of scripts) {
      const scriptExecutionContextIsolateId = this.getExecutionContextIsolateId(
        script.isolate,
      ...
      if (script.executionContextAuxData?.frameId && targetIds.has(scriptFrameId)) {
        ...
        this.#scriptToFrame.has(this.getScriptIsolateId(script.isolate, script.scriptId))
      ) {
        const targetId = this.#scriptToFrame.get(
          this.getScriptIsolateId(script.isolate, script.scriptId),
        );
        if (targetId) {
    ...
    for (const orphanScript of orphanScripts) {
      const orphanScriptExecutionContextIsolateId = this.getExecutionContextIsolateId(
        orphanScript.isolate,
        orphanScript.executionContextId,
      );
