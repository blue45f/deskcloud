...
export class DebuggerModel extends SDKModel {
  ...
  constructor(target) {
    ...
    if (resourceTreeModel) {
      resourceTreeModel.addEventListener(
        ResourceTreeModelEvents.FrameNavigated,
  ...
  async enableDebugger() {
    ...
    if (
      ...
      instrumentationPromise = this.agent.invoke_setInstrumentationBreakpoint({
        instrumentation:
          'beforeScriptExecution' /* Protocol.Debugger.SetInstrumentationBreakpointRequestInstrumentation.BeforeScriptExecution */,
      });
    }
  ...
  asyncStackTracesStateChanged() {
    ...
    const settings = this.target().targetManager().settings;
    const enabled =
      !settings.moduleSetting('disable-async-stack-traces').get() && this.#debuggerEnabled;
  ...
  breakpointsActiveChanged() {
    ...
    void this.agent.invoke_setBreakpointsActive({
      active: settings.moduleSetting('breakpoints-active').get(),
    });
  }
  ...
  async computeAutoStepSkipList(mode) {
    let ranges = [];
    if (
      this.#computeAutoStepRangesCallback &&
      this.#debuggerPausedDetails &&
  ...
  async stepOver() {
    this.#autoSteppingContext =
      this.#debuggerPausedDetails?.callFrames[0]?.functionLocation() ?? null;
  ...
  breakpointResolved(breakpointId, location) {
    this.#breakpointResolvedEventTarget.dispatchEventToListeners(
      breakpointId,
      Location.fromPayload(this, location),
    );
  }
  globalObjectCleared() {
  ...
  async setDebuggerPausedDetails(debuggerPausedDetails) {
    ...
    if (this.#beforePausedCallback) {
      if (
        !(await this.#beforePausedCallback.call(
          null,
          debuggerPausedDetails,
          this.#autoSteppingContext,
        ))
  ...
  createRawLocation(script, lineNumber, columnNumber, inlineFrameIndex) {
    return this.createRawLocationByScriptId(
      script.scriptId,
      lineNumber,
      columnNumber,
      inlineFrameIndex,
  ...
  async setVariableValue(scopeNumber, variableName, newValue, callFrameId) {
    const response = await this.agent.invoke_setVariableValue({
      scopeNumber,
      variableName,
      newValue,
      callFrameId,
...
export class Location {
  ...
  continueToLocation(pausedCallback) {
    ...
    void this.debuggerModel.agent.invoke_continueToLocation({
      ...
      targetCallFrames:
        'current' /* Protocol.Debugger.ContinueToLocationRequestTargetCallFrames.Current */,
    });
  }
  paused(pausedCallback, debuggerPausedDetails) {
    const location = debuggerPausedDetails.callFrames[0].location();
    if (
      location.scriptId === this.scriptId &&
...
export class CallFrame {
  ...
  constructor(debuggerModel, script, payload, inlineFrameIndex, functionName, exception = null) {
    ...
    this.#returnValue = payload.returnValue
      ? this.debuggerModel.runtimeModel().createRemoteObject(payload.returnValue)
      : null;
  }
  ...
  async setReturnValue(expression) {
    ...
    const response = await this.debuggerModel.agent.invoke_setReturnValue({
      newValue: evaluateResponse.result,
    });
    if (response.getError()) {
      return null;
    }
    this.#returnValue = this.debuggerModel
      .runtimeModel()
      .createRemoteObject(evaluateResponse.result);
    return this.#returnValue;
  }
  location() {
  ...
  async evaluate(options) {
    ...
    return {
      object: runtimeModel.createRemoteObject(response.result),
  ...
  async restart() {
    ...
    await this.debuggerModel.agent.invoke_restartFrame({
      callFrameId: this.id,
      mode: 'StepInto' /* Protocol.Debugger.RestartFrameRequestMode.StepInto */,
    });
  }
  getPayload() {
    return this.payload;
...
export class Scope {
  ...
  constructor(callFrame, ordinal) {
    ...
    const start = this.#payload.startLocation
      ? Location.fromPayload(callFrame.debuggerModel, this.#payload.startLocation)
      : null;
    const end = this.#payload.endLocation
      ? Location.fromPayload(callFrame.debuggerModel, this.#payload.endLocation)
      : null;
  ...
  object() {
    ...
    const declarativeScope =
      this.#type !== 'with' /* Protocol.Debugger.ScopeType.With */ &&
      this.#type !== 'global'; /* Protocol.Debugger.ScopeType.Global */
    if (declarativeScope) {
      this.#object = runtimeModel.createScopeRemoteObject(
        this.#payload.object,
        new ScopeRef(this.#ordinal, this.#callFrame.id),
      );
    } else {
  ...
  extraProperties() {
    if (
      this.#ordinal !== 0 ||
      this.#type !== 'local' /* Protocol.Debugger.ScopeType.Local */ ||
      this.#callFrame.script.isWasm()
    ) {
      return [];
    }
...
export class DebuggerPausedDetails {
  ...
  exception() {
    if (
      this.reason !== 'exception' /* Protocol.Debugger.PausedEventReason.Exception */ &&
