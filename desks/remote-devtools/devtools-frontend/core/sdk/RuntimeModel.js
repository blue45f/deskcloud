...
export class RuntimeModel extends SDKModel {
  ...
  releaseEvaluationResult(result) {
    ...
    if ('exceptionDetails' in result && result.exceptionDetails?.exception) {
      ...
      const exceptionObject = this.createRemoteObject({
        type: exception.type,
  ...
  async runScript(
    ...
    return {
      object: this.createRemoteObject(response.result),
  ...
  async queryObjectsRequested(object, executionContextId) {
    ...
    this.dispatchEventToListeners(Events.QueryObjectRequested, {
...
class RuntimeDispatcher {
  ...
  consoleAPICalled({ type, args, executionContextId, timestamp, stackTrace, context }) {
    this.#runtimeModel.consoleAPICalled(
      type,
      args,
      executionContextId,
      timestamp,
...
export class ExecutionContext {
  ...
  async globalLexicalScopeNames() {
    const response = await this.runtimeModel.agent.invoke_globalLexicalScopeNames({
