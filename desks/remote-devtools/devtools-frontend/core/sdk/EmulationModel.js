...
export class EmulationModel extends SDKModel {
  ...
  constructor(target) {
    ...
    if (this.#overlayModel) {
      this.#overlayModel.addEventListener(
    ...
    disableJavascriptSetting.addChangeListener(
      async () =>
        await this.#emulationAgent.invoke_setScriptExecutionDisabled({
          value: disableJavascriptSetting.get(),
        }),
    );
    ...
    const mediaFeatureColorGamutSetting = settings.moduleSetting(
      'emulated-css-media-feature-color-gamut',
    );
    const mediaFeaturePrefersColorSchemeSetting = settings.moduleSetting(
      'emulated-css-media-feature-prefers-color-scheme',
    );
    const mediaFeatureForcedColorsSetting = settings.moduleSetting(
      'emulated-css-media-feature-forced-colors',
    );
    const mediaFeaturePrefersContrastSetting = settings.moduleSetting(
      'emulated-css-media-feature-prefers-contrast',
    );
    const mediaFeaturePrefersReducedDataSetting = settings.moduleSetting(
      'emulated-css-media-feature-prefers-reduced-data',
    );
    const mediaFeaturePrefersReducedTransparencySetting = settings.moduleSetting(
      'emulated-css-media-feature-prefers-reduced-transparency',
    );
    const mediaFeaturePrefersReducedMotionSetting = settings.moduleSetting(
    ...
    mediaFeaturePrefersColorSchemeSetting.addChangeListener(() => {
      this.#mediaConfiguration.set(
        'prefers-color-scheme',
        mediaFeaturePrefersColorSchemeSetting.get(),
      );
      void this.updateCssMedia();
    });
    ...
    mediaFeaturePrefersReducedDataSetting.addChangeListener(() => {
      this.#mediaConfiguration.set(
        ...
        mediaFeaturePrefersReducedDataSetting.get(),
      );
      void this.updateCssMedia();
    });
    mediaFeaturePrefersReducedMotionSetting.addChangeListener(() => {
      this.#mediaConfiguration.set(
        ...
        mediaFeaturePrefersReducedMotionSetting.get(),
      );
      void this.updateCssMedia();
    });
    mediaFeaturePrefersReducedTransparencySetting.addChangeListener(() => {
      this.#mediaConfiguration.set(
        ...
        mediaFeaturePrefersReducedTransparencySetting.get(),
      );
      void this.updateCssMedia();
    });
    void this.updateCssMedia();
    ...
    visionDeficiencySetting.addChangeListener(() =>
    ...
    localFontsDisabledSetting.addChangeListener(() =>
    ...
    if (
      avifFormatDisabledSetting.get() ||
      jpegXlFormatDisabledSetting.get() ||
      webpFormatDisabledSetting.get()
    ) {
    ...
    this.#touchConfiguration = {
      enabled: false,
      configuration:
        'mobile' /* Protocol.Emulation.SetEmitTouchEventsForMouseRequestConfiguration.Mobile */,
    };
  ...
  async setDataSaverOverride(dataSaverOverride) {
    const dataSaverEnabled =
      dataSaverOverride === 'unset' /* DataSaverOverride.UNSET */
        ? undefined
        : dataSaverOverride === 'enabled' /* DataSaverOverride.ENABLED */
          ? true
          : false;
  ...
  async updateTouch() {
    let configuration = {
      ...
      configuration: this.#touchMobile
        ? 'mobile' /* Protocol.Emulation.SetEmitTouchEventsForMouseRequestConfiguration.Mobile */
    ...
    if (
      this.#touchConfiguration.enabled &&
    ...
    await this.#emulationAgent.invoke_setTouchEmulationEnabled({
  ...
  screenOrientationLockChanged(event) {
    ...
    this.dispatchEventToListeners(
      'ScreenOrientationLockChanged' /* EmulationModelEvents.SCREEN_ORIENTATION_LOCK_CHANGED */,
...
SDKModel.register(EmulationModel, {
  capabilities: 256 /* Capability.EMULATION */,
  autostart: true,
});
