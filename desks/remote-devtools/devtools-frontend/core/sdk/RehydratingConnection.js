...
export class RehydratingConnectionTransport {
  ...
  async startHydration(trace) {
    ...
    for (const hydratingDataPerTarget of hydratingData) {
      ...
      sessionId += 1;
      const session = new RehydratingSession(
        sessionId,
        target,
        executionContexts,
        scripts,
...
export class RehydratingSession extends RehydratingSessionBase {
  ...
  handleDebuggerGetScriptSource(id, scriptId) {
    ...
    this.sendMessageToFrontend({
      ...
      result: {
        scriptSource:
          typeof script.sourceText === 'undefined'
            ? i18nString(UIStrings.noSourceText)
            : script.sourceText,
      },
    });
  }
  ...
  handleDebuggerEnable(id) {
    const htmlResourceUrls = new Set(
      this.resources.filter((r) => r.mimeType === 'text/html').map((r) => r.url),
    );
  ...
  handleGetResourceTree(id) {
    const resources = this.resources.filter(
      (r) => r.mimeType === 'text/html' || r.mimeType === 'text/css',
    );
    if (!resources.length) {
      return;
    }
