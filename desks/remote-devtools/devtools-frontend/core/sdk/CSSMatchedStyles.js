...
function cleanUserAgentPayload(payload) {
  ...
  function mergeRule(from, to) {
    ...
    to.rule.style.shorthandEntries = [...shorthands.entries()].map(([name, value]) => ({
      name,
      value,
    }));
    to.rule.style.cssProperties = [...properties.entries()].map(([name, value]) => ({
      name,
      value,
    }));
  }
  ...
  function cleanUserAgentSelectors(ruleMatch) {
    ...
    rule.selectorList.selectors = rule.selectorList.selectors.filter((_, i) =>
      matchingSelectors.includes(i),
    );
...
export class CSSRegisteredProperty {
  ...
  propertyName() {
    return this.#registration instanceof CSSPropertyRule
      ? this.#registration.propertyName().text
      : this.#registration.propertyName;
  }
  initialValue() {
    return this.#registration instanceof CSSPropertyRule
      ? this.#registration.initialValue()
      : (this.#registration.initialValue?.text ?? null);
  }
  inherits() {
    return this.#registration instanceof CSSPropertyRule
      ? this.#registration.inherits()
      : this.#registration.inherits;
  }
  syntax() {
    return this.#registration instanceof CSSPropertyRule
  ...
  style() {
    if (!this.#style) {
      this.#style =
        this.#registration instanceof CSSPropertyRule
...
export class CSSMatchedStyles {
  ...
  async init({
    ...
    [this.#pseudoDOMCascades, this.#customHighlightPseudoDOMCascades] = this.buildPseudoCascades(
      pseudoPayload,
      inheritedPseudoPayload,
    );
  ...
  async buildMainCascade(
    ...
    for (const animationsStyle of animationStylesPayload) {
      const style = new CSSStyleDeclaration(
        this.#cssModel,
        null,
        animationsStyle.style,
        Type.Animation,
        animationsStyle.name,
      );
      this.#nodeForStyle.set(style, this.#node);
      nodeStyles.push(style);
    }
  ...
  buildSplitCustomHighlightCascades(rules, node, isInherited, pseudoCascades) {
    ...
    for (let j = rules.length - 1; j >= 0; --j) {
      const highlightNamesToMatchingSelectorIndices = customHighlightNamesToMatchingSelectorIndices(
        rules[j],
      );
  ...
  buildPseudoCascades(pseudoPayload, inheritedPseudoPayload) {
    ...
    for (let i = 0; i < pseudoPayload.length; ++i) {
      ...
      const pseudoElement =
        this.#node.pseudoElements().get(entryPayload.pseudoType)?.at(-1) || null;
      const pseudoStyles = [];
      ...
      if (entryPayload.pseudoType === 'highlight' /* Protocol.DOM.PseudoType.Highlight */) {
        ...
        for (let j = rules.length - 1; j >= 0; --j) {
          ...
          const nodeForStyle = cssMetadata().isHighlightPseudoType(entryPayload.pseudoType)
            ? this.#node
            : pseudoElement;
    ...
    if (inheritedPseudoPayload) {
      ...
      for (let i = 0; parentNode && i < inheritedPseudoPayload.length; ++i) {
        ...
        for (let j = 0; j < inheritedPseudoMatches.length; ++j) {
          ...
          if (
            inheritedEntryPayload.pseudoType === 'highlight' /* Protocol.DOM.PseudoType.Highlight */
          ) {
            this.buildSplitCustomHighlightCascades(
              rules,
              parentNode,
            ...
            const isHighlightPseudoCascade = cssMetadata().isHighlightPseudoType(
              inheritedEntryPayload.pseudoType,
            );
            const nodeCascade = new NodeCascade(
              this,
              pseudoStyles,
  ...
  hasMatchingSelectors(rule) {
    return (
      (rule.selectors.length === 0 || this.getMatchingSelectors(rule).length > 0) &&
      queryMatches(rule.style)
    );
  }
  ...
  async recomputeMatchingSelectors(rule) {
    ...
    async function querySelector(node, selectorText) {
      ...
      const matchingNodeIds = await this.#node
        .domModel()
        .querySelectorAll(ownerDocument.id, selectorText);
      if (matchingNodeIds) {
        if (typeof node.id === 'number') {
  ...
  animationStyles() {
    return (
      this.#mainDOMCascade
        ?.styles()
        .filter((style) => !this.isInherited(style) && style.type === Type.Animation) ?? []
    );
  }
  transitionsStyle() {
    return (
      this.#mainDOMCascade
        ?.styles()
        .find((style) => !this.isInherited(style) && style.type === Type.Transition) ?? null
    );
  }
  registeredProperties() {
  ...
  resolveGlobalKeyword(property, keyword) {
    const resolved = this.#styleToDOMCascade
      .get(property.ownerStyle)
      ?.resolveGlobalKeyword(property, keyword);
...
class NodeCascade {
  ...
  updatePropertyState(propertyWithHigherSpecificity, canonicalName) {
    ...
    if (
      (activeProperty?.important && !propertyWithHigherSpecificity.important) ||
      (activeProperty &&
        this.#needsCascadeContextStep() &&
        this.#treeScopeDistance(activeProperty) >
          this.#treeScopeDistance(propertyWithHigherSpecificity))
    ) {
      this.propertiesState.set(
        propertyWithHigherSpecificity,
        'Overloaded' /* PropertyState.OVERLOADED */,
      );
      return;
    }
    if (activeProperty) {
      ...
      if (
        propertyWithHigherSpecificity.ownerStyle.type === Type.Animation ||
...
class DOMInheritanceCascade {
  ...
  #findPropertyInPreviousStyle(property, filter) {
    ...
    for (const style of forEach(cascade.styles, property.ownerStyle)) {
      const candidate = style
        .allProperties()
        .findLast((candidate) => candidate.name === property.name && filter(candidate));
      if (candidate) {
        return candidate;
      }
    }
  ...
  #findPropertyInParentCascade(property) {
    ...
    for (const cascade of forEach(this.#nodeCascades, nodeCascade)) {
      for (const style of cascade.styles) {
        const inheritedProperty = style
          .allProperties()
          .findLast((inheritedProperty) => inheritedProperty.name === property.name);
    ...
    if (
      ...
      (!nodeCascade.isHighlightPseudoCascade || property.name.startsWith('--'))
    ) {
  ...
  #findPropertyInParentCascadeIfInherited(property) {
    if (
      !cssMetadata().isPropertyInherited(property.name) ||
  ...
  #findCustomPropertyRegistration(property) {
    const registration = this.#registeredProperties.find(
      (registration) => registration.propertyName() === property,
    );
    ...
    const isPreviousLayer = (other) => {
      ...
      if (
        ...
        other.ownerStyle.parentRule?.origin ===
          'regular' /* Protocol.CSS.StyleSheetOrigin.Regular */
      ) {
        return (
  ...
  #computeCSSVariable(nodeCascade, variableName, sccRecord = new SCCRecord()) {
    ...
    if (
      definedValue.declaration.declaration instanceof CSSProperty &&
      definedValue.declaration.value &&
      CSSMetadata.isCSSWideKeyword(definedValue.declaration.value)
    ) {
      const resolvedProperty = this.resolveGlobalKeyword(
        definedValue.declaration.declaration,
        definedValue.declaration.value,
      );
      if (!resolvedProperty) {
        return definedValue;
  ...
  attributeValueWithSubstitutions(nodeCascade, style, attributeName, sccRecord) {
    ...
    return (
      this.#walkTree(nodeCascade, ast, style, `attr(${attributeName})`, sccRecord)?.value ?? null
    );
  }
  #computeAttribute(nodeCascade, style, attributeName, type, sccRecord = new SCCRecord()) {
    if (type.isCSSTokens) {
      const value = this.attributeValueWithSubstitutions(
        nodeCascade,
        style,
        attributeName,
        sccRecord,
  ...
  #walkTree(outerNodeCascade, ast, parentStyle, substitutionName, sccRecord, declaration) {
    ...
    const matching = PropertyParser.BottomUpTreeMatching.walk(ast, [
      new BaseVariableMatcher((match) => {
        const { value, mayFallback } = recurseWithCycleDetection(
          match.name,
          (nodeCascade) =>
            this.#computeCSSVariable(nodeCascade, match.name, sccRecord)?.value ?? null,
        );
      ...
      new AttributeMatcher(this.#matchedStyles, parentStyle, (match) => {
        ...
        if (!match.isCSSTokens) {
          const { value, mayFallback } = recurseWithCycleDetection(recordName, () =>
          ...
          const { value, mayFallback } = recurseWithCycleDetection(recordName, (nodeCascade) =>
            this.attributeValueWithSubstitutions(nodeCascade, parentStyle, match.name, sccRecord),
          );
          if (value === null && !mayFallback) {
            return null;
          }
    ...
    const decl = PropertyParser.ASTUtils.siblings(
      PropertyParser.ASTUtils.declValue(matching.ast.tree),
    );
    const declText =
      decl.length > 0 ? matching.getComputedTextRange(decl[0], decl[decl.length - 1]) : '';
    ...
    if (record.isRootEntry) {
      ...
      if (scc.length > 1) {
        for (const entry of scc) {
          console.assert(
            entry.nodeCascade === outerNodeCascade,
            'Circles should be within the cascade',
          );
  ...
  ensureInitialized() {
    ...
    for (const rule of this.#registeredProperties) {
      ...
      accumulatedCSSVariables.set(
        rule.propertyName(),
        initialValue !== null
          ? { value: initialValue, declaration: new CSSValueSource(rule) }
          : null,
      );
    }
    ...
    for (let i = this.#nodeCascades.length - 1; i >= 0; --i) {
      ...
      for (const entry of nodeCascade.activeProperties.entries()) {
        ...
        if (propertyName.startsWith('--')) {
          accumulatedCSSVariables.set(propertyName, {
            value: property.value,
            declaration: new CSSValueSource(property),
          });
