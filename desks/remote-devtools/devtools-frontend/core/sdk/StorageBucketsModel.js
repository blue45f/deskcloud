...
export class StorageBucketsModel extends SDKModel {
  ...
  getDefaultBucketForStorageKey(storageKey) {
    const buckets = [...this.bucketsById.values()];
    return (
      buckets.find(({ bucket }) => bucket.storageKey === storageKey && bucket.name === undefined) ??
      null
    );
  }
  ...
  getBucketByName(storageKey, bucketName) {
    ...
    return (
      buckets.find(
        ({ bucket }) => bucket.storageKey === storageKey && bucket.name === bucketName,
      ) ?? null
    );
  }
  deleteBucket(bucket) {
  ...
  enable() {
    ...
    if (this.storageKeyManager) {
      this.storageKeyManager.addEventListener(
        'StorageKeyAdded' /* StorageKeyManagerEvents.STORAGE_KEY_ADDED */,
        this.storageKeyAdded,
        this,
      );
      this.storageKeyManager.addEventListener(
        'StorageKeyRemoved' /* StorageKeyManagerEvents.STORAGE_KEY_REMOVED */,
  ...
  bucketAdded(bucketInfo) {
    ...
    this.dispatchEventToListeners('BucketAdded' /* Events.BUCKET_ADDED */, {
      model: this,
      bucketInfo,
    });
  }
  bucketRemoved(bucketInfo) {
    ...
    this.dispatchEventToListeners('BucketRemoved' /* Events.BUCKET_REMOVED */, {
      model: this,
      bucketInfo,
    });
  }
  bucketChanged(bucketInfo) {
    this.dispatchEventToListeners('BucketChanged' /* Events.BUCKET_CHANGED */, {
      model: this,
      bucketInfo,
    });
  }
  bucketInfosAreEqual(bucketInfo1, bucketInfo2) {
    return (
      bucketInfo1.bucket.storageKey === bucketInfo2.bucket.storageKey &&
      bucketInfo1.id === bucketInfo2.id &&
      bucketInfo1.bucket.name === bucketInfo2.bucket.name &&
      bucketInfo1.expiration === bucketInfo2.expiration &&
      bucketInfo1.quota === bucketInfo2.quota &&
      bucketInfo1.persistent === bucketInfo2.persistent &&
...
SDKModel.register(StorageBucketsModel, {
