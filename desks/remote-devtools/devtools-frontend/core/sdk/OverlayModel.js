...
export class OverlayModel extends SDKModel {
  ...
  constructor(target) {
    ...
    this.#persistentHighlighter = new OverlayPersistentHighlighter(this, settings, {
      onGridOverlayStateChanged: ({ nodeId, enabled }) => {
        this.#domModel
          .nodeForId(nodeId)
          ?.dispatchEventToListeners(DOMNodeEvents.GRID_OVERLAY_STATE_CHANGED, { enabled });
        this.dispatchEventToListeners(
          'PersistentGridOverlayStateChanged' /* Events.PERSISTENT_GRID_OVERLAY_STATE_CHANGED */,
          { nodeId, enabled },
        );
      },
      onFlexOverlayStateChanged: ({ nodeId, enabled }) => {
        this.#domModel
          .nodeForId(nodeId)
          ?.dispatchEventToListeners(DOMNodeEvents.FLEX_CONTAINER_OVERLAY_STATE_CHANGED, {
            enabled,
          });
        this.dispatchEventToListeners(
          'PersistentFlexContainerOverlayStateChanged' /* Events.PERSISTENT_FLEX_CONTAINER_OVERLAY_STATE_CHANGED */,
          { nodeId, enabled },
        );
      },
      onContainerQueryOverlayStateChanged: ({ nodeId, enabled }) => {
        this.#domModel
          .nodeForId(nodeId)
          ?.dispatchEventToListeners(DOMNodeEvents.CONTAINER_QUERY_OVERLAY_STATE_CHANGED, {
            enabled,
          });
        this.dispatchEventToListeners(
          'PersistentContainerQueryOverlayStateChanged' /* Events.PERSISTENT_CONTAINER_QUERY_OVERLAY_STATE_CHANGED */,
          { nodeId, enabled },
        );
      },
      onScrollSnapOverlayStateChanged: ({ nodeId, enabled }) => {
        this.#domModel
          .nodeForId(nodeId)
          ?.dispatchEventToListeners(DOMNodeEvents.SCROLL_SNAP_OVERLAY_STATE_CHANGED, { enabled });
        this.dispatchEventToListeners(
          'PersistentScrollSnapOverlayStateChanged' /* Events.PERSISTENT_SCROLL_SNAP_OVERLAY_STATE_CHANGED */,
          { nodeId, enabled },
        );
      },
    });
  ...
  static async muteHighlight(targetManager = TargetManager.instance()) {
    return await Promise.all(
      targetManager.models(OverlayModel).map((model) => model.suspendModel()),
    );
  }
  static async unmuteHighlight(targetManager = TargetManager.instance()) {
    return await Promise.all(
      targetManager.models(OverlayModel).map((model) => model.resumeModel()),
    );
  }
  ...
  async wireAgentToSettings() {
    this.#registeredListeners = [
      ...
      this.#showLayoutShiftRegionsSetting.addChangeListener(() =>
        this.overlayAgent.invoke_setShowLayoutShiftRegions({
          result: this.#showLayoutShiftRegionsSetting.get(),
        }),
      ),
    ...
    await this.overlayAgent.invoke_setShowViewportSizeOnResize({
      show: this.#showViewportSizeOnResize,
    });
  ...
  async setInspectMode(mode, showDetailedTooltip = true) {
    ...
    this.dispatchEventToListeners(
      'InspectModeWillBeToggled' /* Events.INSPECT_MODE_WILL_BE_TOGGLED */,
      this,
    );
    void this.#highlighter.setInspectMode(
      mode,
      this.buildHighlightConfig('all', showDetailedTooltip),
    );
  }
  inspectModeEnabled() {
  ...
  colorOfGridInPersistentOverlay(nodeId) {
    ...
    return this.#persistentHighlighter
      .colorOfGrid(nodeId)
      .asString('hex' /* Common.Color.Format.HEX */);
  }
  ...
  async toggleWindowControlsToolbar(show) {
    ...
    const setWindowControlsOverlayOperation =
      this.overlayAgent.invoke_setShowWindowControlsOverlay(wcoConfigObj);
    ...
    if (mode.endsWith('gap')) {
      ...
      if (mode === 'gap' || mode === 'row-gap') {
        highlightConfig.gridHighlightConfig.rowGapColor =
          Common.Color.PageHighlight.GapBackground.toProtocolRGBA();
        highlightConfig.gridHighlightConfig.rowHatchColor =
          Common.Color.PageHighlight.GapHatch.toProtocolRGBA();
      }
  ...
  nodeHighlightRequested({ nodeId }) {
    ...
    if (node) {
      this.dispatchEventToListeners(
        'HighlightNodeRequested' /* Events.HIGHLIGHT_NODE_REQUESTED */,
        node,
      );
    }
  }
  ...
  screenshotRequested({ viewport }) {
    this.dispatchEventToListeners(
      'ScreenshotRequested' /* Events.SCREENSHOT_REQUESTED */,
      viewport,
    );
    this.dispatchEventToListeners('InspectModeExited' /* Events.EXITED_INSPECT_MODE */);
  }
  ...
  inspectPanelShowRequested({ backendNodeId }) {
    this.dispatchEventToListeners(
      'InspectPanelShowRequested' /* Events.INSPECT_PANEL_SHOW_REQUESTED */,
      backendNodeId,
    );
  }
  inspectedElementWindowRestored({ backendNodeId }) {
    this.dispatchEventToListeners(
      'InspectedElementWindowRestored' /* Events.INSPECTED_ELEMENT_WINDOW_RESTORED */,
      backendNodeId,
    );
  }
}
export class WindowControls {
  ...
  async toggleEmulatedOverlay(showOverlay) {
    ...
    if (showOverlay) {
      ...
      await this.#cssModel.setStyleSheetText(
        this.#stylesheetId,
        this.#originalStylesheetText,
        false,
      );
  ...
  #fetchCssSourceUrl(url) {
    ...
    const header = cssHeaders.find(
      (header) => header.sourceURL && header.sourceURL.includes(parentURL),
    );
  ...
  static #transformStyleSheet(x, y, width, height, originalStyleSheet) {
    ...
    const updatedStylesheet = stylesheetText
      .replace(/: env\(titlebar-area-x(?:,[^)]*)?\);/g, `: env(titlebar-area-x, ${x}px);`)
      ...
      .replace(
        ...
        `: env(titlebar-area-height, ${height}px);`,
      );
...
export class SourceOrderHighlighter {
  ...
  highlightSourceOrderInOverlay(node, sourceOrderConfig) {
    ...
    void this.#model
      .getOverlayAgent()
      .invoke_highlightSourceOrder({ sourceOrderConfig, nodeId: node.id });
  }
