...
export class CookieModel extends SDKModel {
  ...
  constructor(target) {
    ...
    target
      .model(NetworkManager)
      ?.addEventListener(NetworkManagerEvents.LoadingFinished, this.#onLoadingFinished, this);
  }
    ...
    const newCookies = new Map(
      await Promise.all(
        urls
          ...
          .map((domain) =>
            networkAgent
              .invoke_getCookies({ urls: [...urls.get(domain).values()] })
  ...
  async saveCookie(cookie) {
    ...
    const schemeBindingEnabled = Boolean(
      Root.Runtime.hostConfig.devToolsEnableOriginBoundCookies?.schemeBindingEnabled,
    );
    const portBindingEnabled = Boolean(
      Root.Runtime.hostConfig.devToolsEnableOriginBoundCookies?.portBindingEnabled,
    );
    const preserveUnset = (scheme) =>
      scheme === 'Unset' /* Protocol.Network.CookieSourceScheme.Unset */ ? scheme : undefined;
    const protocolCookie = {
      ...
      sourceScheme: schemeBindingEnabled
        ? cookie.sourceScheme()
        : preserveUnset(cookie.sourceScheme()),
  ...
  async deleteCookies(cookies) {
    ...
    await Promise.all(
      cookies.map((cookie) =>
        networkAgent.invoke_deleteCookies({
          name: cookie.name(),
