...
export class AccessibilityNode {
  ...
  constructor(accessibilityModel, payload) {
    ...
    if (payload.backendDOMNodeId) {
      ...
      this.#deferredDOMNode = new DeferredDOMNode(
        accessibilityModel.target(),
  ...
  coreProperties() {
    ...
    if (this.#description) {
      properties.push({
        name: 'description' /* CoreAxPropertyName.DESCRIPTION */,
...
export class AccessibilityModel extends SDKModel {
  ...
  async requestAndLoadSubTreeToNode(node) {
    ...
    const { nodes } = await this.agent.invoke_getAXNodeAndAncestors({
