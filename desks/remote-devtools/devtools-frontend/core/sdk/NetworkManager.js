...
export function activeNetworkThrottlingKeySetting(settings = Common.Settings.Settings.instance()) {
  return settings.createSetting(
    ...
    'NO_THROTTLING' /* PredefinedThrottlingConditionKey.NO_THROTTLING */,
  );
}
export class NetworkManager extends SDKModel {
  ...
  static canReplayRequest(request) {
    return (
      Boolean(requestToManagerMap.get(request)) &&
      Boolean(request.backendRequestId()) &&
      !request.isRedirect() &&
  ...
  static async requestPostData(request) {
    ...
    try {
      const { postData, base64Encoded } = await manager.#networkAgent.invoke_getRequestPostData({
        requestId,
      });
      if (base64Encoded && postData) {
        ...
        const charset = requestContentType
          ? (Platform.MimeType.parseContentType(requestContentType).charset ?? 'utf-8')
          : 'utf-8';
        ...
        if (contentEncoding) {
          const decompressed = await NetworkManager.#tryDecompressBody(
            bytes.buffer,
            contentEncoding,
            charset,
          );
  ...
  static connectionType(conditions) {
    ...
    try {
      const title =
        typeof conditions.title === 'function'
          ? conditions.title().toLowerCase()
          : conditions.title.toLowerCase();
  ...
  bypassServiceWorkerChanged() {
    void this.#networkAgent.invoke_setBypassServiceWorker({
      bypass: this.#bypassServiceWorkerSetting.get(),
    });
  }
  async getSecurityIsolationStatus(frameId) {
    const result = await this.#networkAgent.invoke_getSecurityIsolationStatus({
  ...
  async loadNetworkResource(frameId, url, options) {
    const result = await this.#networkAgent.invoke_loadNetworkResource({
      frameId: frameId ?? undefined,
      url,
      options,
    });
...
export class FetchDispatcher {
  ...
  requestPaused({
    ...
    void MultitargetNetworkManager.instance().requestIntercepted(
...
export class NetworkDispatcher {
  ...
  constructor(manager) {
    ...
    MultitargetNetworkManager.instance().addEventListener(
      'RequestIntercepted' /* MultitargetNetworkManager.Events.REQUEST_INTERCEPTED */,
  ...
  updateNetworkRequestWithRequest(networkRequest, request) {
    ...
    networkRequest.setRequestFormData(
      Boolean(request.hasPostData),
      isCompressed ? null : request.postData || null,
    );
    networkRequest.setInitialPriority(request.initialPriority);
    networkRequest.mixedContentType =
      request.mixedContentType || 'none' /* Protocol.Security.MixedContentType.None */;
  ...
  updateNetworkRequestWithResponse(networkRequest, response) {
    ...
    const newResourceType = Common.ResourceType.ResourceType.fromMimeTypeOverride(
  ...
  signedExchangeReceived({ requestId, info }) {
    ...
    this.#manager.dispatchEventToListeners(Events.ResponseReceived, {
      request: networkRequest,
      response: info.outerResponse,
    });
  }
  requestWillBeSent({
    requestId,
  ...
  loadingFailed({
    ...
    if (blockedReason) {
      ...
      if (blockedReason === 'inspector' /* Protocol.Network.BlockedReason.Inspector */) {
        const message = i18nString(UIStrings.requestWasBlockedByDevtoolsS, {
          PH1: networkRequest.url(),
        });
        this.#manager.dispatchEventToListeners(Events.MessageGenerated, {
          message,
          requestId,
          warning: true,
        });
  ...
  responseReceivedEarlyHints({ requestId, headers }) {
    this.getExtraInfoBuilder(requestId).setEarlyHintsHeaders(
      this.headersMapToHeadersArray(headers),
    );
  }
  ...
  maybeAdoptMainResourceRequest(requestId) {
    const request =
      MultitargetNetworkManager.instance().inflightMainResourceRequests.get(requestId);
    if (!request) {
      return null;
    }
  ...
  startNetworkRequest(networkRequest, originalRequest) {
    ...
    if (networkRequest.loaderId === networkRequest.requestId() || networkRequest.loaderId === '') {
      MultitargetNetworkManager.instance().inflightMainResourceRequests.set(
        networkRequest.requestId(),
        networkRequest,
      );
    }
    this.#manager.dispatchEventToListeners(Events.RequestStarted, {
      request: networkRequest,
      originalRequest,
    });
  }
  ...
  finishNetworkRequest(networkRequest, finishTime, encodedDataLength) {
    ...
    MultitargetNetworkManager.instance().inflightMainResourceRequests.delete(
      networkRequest.requestId(),
    );
    const settings = this.#manager.target().targetManager().settings;
    if (
      settings.moduleSetting('monitoring-xhr-enabled').get() &&
      networkRequest.resourceType().category() === Common.ResourceType.resourceCategories.XHR
    ) {
      let message;
      const failedToLoad = networkRequest.failed || networkRequest.hasErrorStatusCode();
      if (failedToLoad) {
        message = i18nString(UIStrings.sFailedLoadingSS, {
          PH1: networkRequest.resourceType().title(),
          PH2: networkRequest.requestMethod,
          PH3: networkRequest.url(),
        });
      } else {
        message = i18nString(UIStrings.sFinishedLoadingSS, {
          PH1: networkRequest.resourceType().title(),
          PH2: networkRequest.requestMethod,
          PH3: networkRequest.url(),
        });
...
export class RequestURLPattern {
  ...
  static isValidPattern(pattern) {
    try {
      ...
      return urlPattern.hasRegExpGroups
        ? 'has-regexp-groups' /* RequestURLPatternValidity.HAS_REGEXP_GROUPS */
        : 'valid' /* RequestURLPatternValidity.VALID */;
    } catch {
  ...
  static create(constructorString) {
    try {
      ...
      return urlPattern.hasRegExpGroups
        ? null
        : new RequestURLPattern(constructorString, urlPattern);
    } catch {
      return null;
    }
  }
...
export class RequestCondition extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static createFromSetting(setting, settings = Common.Settings.Settings.instance()) {
    if ('urlPattern' in setting) {
      ...
      const conditions =
        getPredefinedOrBlockingCondition(setting.conditions) ??
        customUserNetworkConditionsSetting(settings)
          .get()
          .find((condition) => condition.key === setting.conditions) ??
  ...
  set pattern(pattern) {
    ...
    this.dispatchEventToListeners(
      'request-condition-changed' /* RequestCondition.Events.REQUEST_CONDITION_CHANGED */,
    );
  }
  get enabled() {
    return this.#enabled;
  ...
  toSetting() {
    ...
    if (this.#pattern instanceof RequestURLPattern) {
      return {
        enabled,
        urlPattern: this.#pattern.constructorString,
    ...
    if (this.#conditions !== BlockingConditions && this.#pattern.upgradedPattern) {
      return {
        enabled,
        urlPattern: this.#pattern.upgradedPattern.constructorString,
        conditions: this.#conditions.key,
      };
    }
  ...
  get originalOrUpgradedURLPattern() {
    return this.#pattern instanceof RequestURLPattern
      ? this.#pattern.pattern
      : this.#pattern.upgradedPattern?.pattern;
  }
}
export class RequestConditions extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(settings) {
    ...
    this.#conditionsEnabledSetting.addChangeListener(() =>
      this.dispatchEventToListeners(
        'request-conditions-changed' /* RequestConditions.Events.REQUEST_CONDITIONS_CHANGED */,
      ),
    );
  }
  get count() {
  ...
  applyConditions(offline, globalConditions, ...agents) {
    ...
    for (const agent of agents) {
      ...
      promises.push(
        agent.invoke_overrideNetworkState({
          offline,
          ...
          connectionType: globalConditions
            ? NetworkManager.connectionType(globalConditions)
            : 'none' /* Protocol.Network.ConnectionType.None */,
        }),
      );
    }
    this.#conditionsAppliedForTestPromise = this.#conditionsAppliedForTestPromise.then(() =>
...
export class MultitargetNetworkManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(targetManager) {
    ...
    const blockedPatternChanged = () => {
      ...
      this.dispatchEventToListeners(
        'BlockedPatternsChanged' /* MultitargetNetworkManager.Events.BLOCKED_PATTERNS_CHANGED */,
      );
    };
  ...
  static patchUserAgentMetadataWithChromeVersion(userAgentMetadata) {
    ...
    if (userAgentMetadata.fullVersion) {
      if (userAgentMetadata.fullVersion.includes('%s')) {
        userAgentMetadata.fullVersion = Platform.StringUtilities.sprintf(
          userAgentMetadata.fullVersion,
          chromeVersion,
        );
      }
    }
  ...
  modelAdded(networkManager) {
    ...
    if (this.currentUserAgent()) {
      void networkAgent.invoke_setUserAgentOverride({
        ...
        userAgentMetadata: this.#userAgentMetadataOverride || undefined,
      });
    }
  ...
  isThrottling() {
    return (
      this.#networkConditions.download >= 0 ||
      this.#networkConditions.upload >= 0 ||
  ...
  setNetworkConditions(conditions) {
    ...
    this.dispatchEventToListeners(
      'ConditionsChanged' /* MultitargetNetworkManager.Events.CONDITIONS_CHANGED */,
    );
  }
  networkConditions() {
  ...
  setUserAgentOverride(userAgent, userAgentMetadataOverride) {
    ...
    if (uaChanged) {
      this.dispatchEventToListeners(
        'UserAgentChanged' /* MultitargetNetworkManager.Events.USER_AGENT_CHANGED */,
      );
    }
  }
  ...
  setCustomAcceptedEncodingsOverride(acceptedEncodings) {
    ...
    this.dispatchEventToListeners(
      'AcceptedEncodingsChanged' /* MultitargetNetworkManager.Events.ACCEPTED_ENCODINGS_CHANGED */,
    );
  }
  ...
  updateInterceptionPatternsOnNextTick() {
    ...
    if (!this.#updatingInterceptionPatternsPromise) {
      this.#updatingInterceptionPatternsPromise = Promise.resolve().then(
  ...
  async updateInterceptionPatterns() {
    ...
    for (const agent of this.#fetchAgents) {
      promises.push(
        agent.invoke_enable({ patterns: this.#urlsForRequestInterceptor.valuesArray() }),
      );
    }
    this.dispatchEventToListeners(
      'InterceptorsChanged' /* MultitargetNetworkManager.Events.INTERCEPTORS_CHANGED */,
    );
...
export class InterceptedRequest {
  ...
  async continueRequestWithContent(contentBlob, encoded, responseHeaders, isBodyOverridden) {
    ...
    const body = encoded
      ? await contentBlob.text()
      : await Common.Base64.encode(contentBlob).catch((err) => {
          console.error(err);
          return '';
        });
    ...
    if (this.networkRequest) {
      ...
      const setCookieHeadersFromOverrides = responseHeaders.filter(
        (header) => header.name === 'set-cookie',
      );
      this.networkRequest.setCookieHeaders = InterceptedRequest.mergeSetCookieHeaders(
    ...
    MultitargetNetworkManager.instance().dispatchEventToListeners(
      'RequestFulfilled' /* MultitargetNetworkManager.Events.REQUEST_FULFILLED */,
      this.request.url,
    );
  }
...
export function networkConditionsEqual(first, second) {
  ...
  // locally.
  const firstTitle =
    first.i18nTitleKey || (typeof first.title === 'function' ? first.title() : first.title);
  const secondTitle =
    second.i18nTitleKey || (typeof second.title === 'function' ? second.title() : second.title);
  return (
    second.download === first.download &&
    second.upload === first.upload &&
    second.latency === first.latency &&
    first.packetLoss === second.packetLoss &&
    first.packetQueueLength === second.packetQueueLength &&
    first.packetReordering === second.packetReordering &&
...
export function getPredefinedOrBlockingCondition(key) {
  return key === 'BLOCKING' /* PredefinedThrottlingConditionKey.BLOCKING */
    ? BlockingConditions
    : getPredefinedCondition(key);
}
/**
