...
export class BaseVariableMatch {
  ...
  fallbackValue() {
    ...
    return this.matching.getComputedTextRange(
      this.fallback[0],
      this.fallback[this.fallback.length - 1],
    );
  }
...
export class VariableMatch extends BaseVariableMatch {
  ...
  constructor(text, node, name, fallback, matching, matchedStyles, style) {
    super(
      ...
      fallback,
      matching,
      () => this.resolveVariable()?.value ?? this.fallbackValue(),
    );
...
export class AttributeMatch extends BaseVariableMatch {
  ...
  constructor(
    ...
    super(text, node, name, fallback, matching, (_, matching) =>
      computedTextCallback(this, matching),
    );
    this.type = type;
...
function getCssEvaluationElement() {
  ...
  if (!cssEvaluationElement) {
    ...
    if (!cssEvaluationElement) {
      ...
      cssEvaluationElement.setAttribute(
        'style',
        'hidden: true; --evaluation: attr(data-custom-expr type(*))',
      );
...
export class ColorMixMatcher extends matcherBase(ColorMixMatch) {
  ...
  matches(node, matching) {
    if (
      ...
      matching.ast.text(node.getChild('Callee')) !== 'color-mix'
    ) {
      return null;
    }
    ...
    if (
      space.length < 2 ||
      computedValueTree.text(ASTUtils.stripComments(space).next().value) !== 'in' ||
      color1.length < 1 ||
      color2.length < 1
    ) {
      return null;
    ...
    const p1 = color1.filter(
      (n) => n.type.name === 'NumberLiteral' && computedValueTree.text(n.getChild('Unit')) === '%',
    );
    const p2 = color2.filter(
      (n) => n.type.name === 'NumberLiteral' && computedValueTree.text(n.getChild('Unit')) === '%',
    );
    ...
    if (
      p1[0] &&
      p2[0] &&
      (literalToNumber(p1[0], computedValueTree) ?? 0) === 0 &&
...
export class ContrastColorMatcher extends matcherBase(ContrastColorMatch) {
  ...
  matches(node, matching) {
    if (
      node.name !== 'CallExpression' ||
      matching.ast.text(node.getChild('Callee')) !== 'contrast-color'
    ) {
      return null;
    }
...
export class LinearGradientMatcher extends matcherBase(LinearGradientMatch) {
  ...
  matches(node, matching) {
    ...
    if (
      node.name === 'CallExpression' &&
      matching.ast.text(node.getChild('Callee')) === 'linear-gradient'
    ) {
  ...
  matches(node, matching) {
    ...
    if (node.name === 'CallExpression') {
      ...
      if (callee && colorFunc.match(/^(rgba?|hsla?|hwba?|lab|lch|oklab|oklch|color)$/)) {
        ...
        const colorText =
          args.length >= 2 ? matching.getComputedTextRange(args[0], args[args.length - 1]) : '';
        ...
        const isRelativeColorSyntax = Boolean(
          colorText.match(/^[^)]*\(\W*from\W+/) &&
          !matching.hasUnresolvedSubstitutions(node) &&
          CSS.supports('color', colorFunc + colorText),
        );
        if (!isRelativeColorSyntax) {
          return new ColorMatch(text, node);
        }
        const tokenized = matchDeclaration('--color', '--colorFunc' + colorText, [
          new ColorMatcher(),
        ]);
        if (!tokenized) {
          return null;
        ...
        const colorSpace = Common.Color.getFormat(
          colorFunc !== 'color' ? colorFunc : matching.ast.text(colorArgs[2]),
        );
        if (!colorSpace) {
          return null;
        }
...
export class LightDarkColorMatcher extends matcherBase(LightDarkColorMatch) {
  ...
  matches(node, matching) {
    if (
      node.name !== 'CallExpression' ||
      matching.ast.text(node.getChild('Callee')) !== 'light-dark'
    ) {
      return null;
    }
    const args = ASTUtils.callArgs(node);
...
export class AutoBaseMatcher extends matcherBase(AutoBaseMatch) {
  ...
  matches(node, matching) {
    if (
      node.name !== 'CallExpression' ||
      matching.ast.text(node.getChild('Callee')) !== '-internal-auto-base'
    ) {
      return null;
    }
    const args = ASTUtils.callArgs(node);
...
export class LinkableNameMatcher extends matcherBase(LinkableNameMatch) {
  ...
  matchAnimationNameInShorthand(node, matching) {
    ...
    const currentDeclarationNodes = declarations.find(
      (declaration) =>
        declaration[0].from <= node.from && declaration[declaration.length - 1].to >= node.to,
    );
    ...
    for (
      let itNode = ASTUtils.declValue(tokenized.tree);
      itNode?.nextSibling;
      ...
      if (itNode.name === 'ValueName') {
        const categoryValue = LinkableNameMatcher.identifierAnimationLonghandMap.get(
          tokenized.text(itNode),
        );
  ...
  matches(node, matching) {
    ...
    const isInsideVarCall =
      parentNode.name === 'ArgList' &&
      parentNode.prevSibling?.name === 'Callee' &&
    ...
    const shouldMatchOnlyVariableName =
      propertyName === 'position-try' /* LinkableNameProperties.POSITION_TRY */ ||
    ...
    if (
      !propertyName ||
      (node.name !== 'ValueName' && node.name !== 'VariableName') ||
      !isAParentDeclarationOrVarCall ||
      (node.name === 'ValueName' && shouldMatchOnlyVariableName)
    ) {
      return null;
    }
...
export class BezierMatcher extends matcherBase(BezierMatch) {
  ...
  matches(node, matching) {
    ...
    const isCubicBezierOrLinearFunction =
      node.name === 'CallExpression' &&
...
export class ShadowMatcher extends matcherBase(ShadowMatch) {
  ...
  matches(node, matching) {
    ...
    return new ShadowMatch(
      valueText,
      node,
      matching.ast.propertyName === 'text-shadow'
        ? 'textShadow' /* ShadowType.TEXT_SHADOW */
        : 'boxShadow' /* ShadowType.BOX_SHADOW */,
    );
  }
}
...
export class FontMatcher extends matcherBase(FontMatch) {
  ...
  matches(node, matching) {
    ...
    const validNodes =
      matching.ast.propertyName === 'font-family'
...
export class MathFunctionMatcher extends matcherBase(MathFunctionMatch) {
  ...
  matches(node, matching) {
    ...
    if (
      args.some(
        (arg) =>
          arg.length === 0 || matching.hasUnresolvedSubstitutionsRange(arg[0], arg[arg.length - 1]),
      )
    ) {
      return null;
    }
...
export class FlexGridGridLanesMatcher extends matcherBase(FlexGridGridLanesMatch) {
  ...
  matches(node, matching) {
    ...
    const values = valueNodes
      .filter((node) => node.name !== 'Important')
    ...
    if (FlexGridGridLanesMatcher.GRID_LANES.includes(text)) {
      return new FlexGridGridLanesMatch(
        matching.ast.text(node),
        node,
        'grid-lanes' /* LayoutType.GRID_LANES */,
      );
    }
    return null;
  }
    ...
    function parseNodes(nodes, varParsingMode = false) {
      for (const curNode of nodes) {
        if (matching.getMatch(curNode) instanceof BaseVariableMatch) {
          const computedValueTree = tokenizeDeclaration(
            '--property',
            matching.getComputedText(curNode),
          );
          if (!computedValueTree) {
            continue;
          }
          ...
          if (
            (varNodes[0].name === 'StringLiteral' && !hasLeadingLineNames) ||
...
export class CSSWideKeywordMatcher extends matcherBase(CSSWideKeywordMatch) {
  ...
  matches(node, matching) {
    ...
    if (
      Array.from(ASTUtils.stripComments(ASTUtils.siblings(ASTUtils.declValue(parentNode)))).some(
        (child) => !ASTUtils.equals(child, node),
      )
    ) {
      return null;
    }
  ...
  matches(node, matching) {
    ...
    const fallbackValue =
      fallbackNodes.length > 0
        ? matching.getComputedTextRange(...ASTUtils.range(fallbackNodes.flat()))
        : undefined;
