...
export class CPUThrottlingManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  setCPUThrottlingOption(option) {
    ...
    this.dispatchEventToListeners(
      'RateChanged' /* Events.RATE_CHANGED */,
      this.#cpuThrottlingOption.rate(),
    );
  }
  setHardwareConcurrency(concurrency) {
    ...
    this.dispatchEventToListeners(
      'HardwareConcurrencyChanged' /* Events.HARDWARE_CONCURRENCY_CHANGED */,
...
function makeFixedPresetThrottlingOption(rate) {
  return {
    title:
      rate === 1
        ? i18nLazyString(UIStrings.noThrottling)
        : i18nLazyString(UIStrings.dSlowdown, { PH1: rate }),
    rate: () => rate,
...
export const MidTierThrottlingOption = makeFixedPresetThrottlingOption(
  CPUThrottlingRates.MID_TIER_MOBILE,
);
export const LowTierThrottlingOption = makeFixedPresetThrottlingOption(
  CPUThrottlingRates.LOW_TIER_MOBILE,
);
export const ExtraSlowThrottlingOption = makeFixedPresetThrottlingOption(
  CPUThrottlingRates.EXTRA_SLOW,
);
function makeCalibratedThrottlingOption(calibratedDeviceType) {
  ...
  return {
    title() {
      const typeString =
        calibratedDeviceType === 'low-tier-mobile'
          ? i18nString(UIStrings.calibratedLowTierMobile)
...
export const CalibratedMidTierMobileThrottlingOption =
