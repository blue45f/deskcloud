...
export class FrameManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static instance({ forceNew } = { forceNew: false }) {
    if (!Root.DevToolsContext.globalInstance().has(FrameManager) || forceNew) {
      Root.DevToolsContext.globalInstance().set(
        FrameManager,
        new FrameManager(TargetManager.instance()),
      );
    }
  ...
  modelAdded(resourceTreeModel) {
    const addListener = resourceTreeModel.addEventListener(
      ResourceTreeModelEvents.FrameAdded,
      this.frameAdded,
      this,
    );
    const detachListener = resourceTreeModel.addEventListener(
      ResourceTreeModelEvents.FrameDetached,
      this.frameDetached,
      this,
    );
    const navigatedListener = resourceTreeModel.addEventListener(
      ResourceTreeModelEvents.FrameNavigated,
      this.frameNavigated,
      this,
    );
    const resourceAddedListener = resourceTreeModel.addEventListener(
      ResourceTreeModelEvents.ResourceAdded,
      this.resourceAdded,
      this,
    );
    this.#eventListeners.set(resourceTreeModel, [
      addListener,
      detachListener,
      navigatedListener,
      resourceAddedListener,
    ]);
  ...
  frameAdded(event) {
    ...
    if (frameData) {
      ...
      if (
        cachedFrameAttributes?.creationStackTrace &&
        cachedFrameAttributes?.creationStackTraceTarget
      ) {
    ...
    this.dispatchEventToListeners('FrameAddedToTarget' /* Events.FRAME_ADDED_TO_TARGET */, {
      frame,
    });
  ...
  frameNavigated(event) {
    ...
    if (frame.isOutermostFrame()) {
      this.dispatchEventToListeners(
        'OutermostFrameNavigated' /* Events.OUTERMOST_FRAME_NAVIGATED */,
        { frame },
      );
    }
  }
  resourceAdded(event) {
    this.dispatchEventToListeners('ResourceAdded' /* Events.RESOURCE_ADDED */, {
      resource: event.data,
    });
  }
