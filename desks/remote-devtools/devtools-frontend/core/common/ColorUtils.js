...
export function desiredLuminanceAPCA(luminance, contrast, lighter) {
  ...
  function computeLuminance() {
    if (!lighter) {
      // Black text on white.
      return Math.pow(
        Math.abs(Math.pow(luminance, normBgExp) - (contrast + loConOffset) / scaleBoW),
        1 / normFgExp,
      );
    }
    // White text on black.
    return Math.pow(
      Math.abs(Math.pow(luminance, revBgExp) - (-contrast - loConOffset) / scaleWoB),
      1 / revFgExp,
    );
  }
...
export function isLargeFont(fontSize, fontWeight) {
  ...
  const isBold = isNaN(Number(fontWeight))
    ? boldWeights.includes(fontWeight)
