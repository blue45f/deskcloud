...
export class VersionController {
  ...
  updateVersion() {
    ...
    const minimumVersion = Math.min(
      this.#globalVersionSetting.get(),
      this.#syncedVersionSetting.get(),
      this.#localVersionSetting.get(),
    );
  ...
  updateVersionFrom3To4() {
    const advancedMode = this.#settings.createSetting(
  ...
  updateVersionFrom18To19() {
    ...
    const visibleColumnSettings = this.#settings.createSetting(
  ...
  updateVersionFrom24To25() {
    const defaultColumns = { status: true, type: true, initiator: true, size: true, time: true };
    const networkLogColumnsSetting = this.#settings.createSetting(
      'networkLogColumns',
      defaultColumns,
    );
  ...
  updateVersionFrom32To33() {
    const previouslyViewedFilesSetting = this.#settings.createLocalSetting(
    ...
    previouslyViewedFiles = previouslyViewedFiles.filter(
      (previouslyViewedFile) => 'url' in previouslyViewedFile,
    );
  ...
  updateVersionFrom33To34() {
    ...
    for (const breakpoint of breakpoints) {
      const isLogpoint =
        breakpoint.condition.startsWith(logpointPrefix) &&
        breakpoint.condition.endsWith(logpointSuffix);
  ...
  updateVersionFrom34To35() {
    ...
    for (const breakpoint of breakpoints) {
      ...
      if (isLogpoint) {
        breakpoint.condition = condition.slice(
          logpointPrefix.length,
          condition.length - logpointSuffix.length,
        );
      }
    }
    breakpointsSetting.set(breakpoints);
  ...
  updateVersionFrom36To37() {
    ...
    for (const key of this.#settings.globalStorage.keys()) {
      if (
        (key.startsWith('data-grid-') && key.endsWith('-column-weights')) ||
        key.endsWith('-tab-order') ||
        key === 'views-location-override' ||
        key === 'closeable-tabs'
      ) {
  ...
  updateVersionFrom37To38() {
    ...
    const onboardingFinished = this.#settings.createLocalSetting(
      'console-insights-onboarding-finished',
      false,
    );
    if (
      consoleInsightsEnabled &&
      consoleInsightsEnabled.get() === true &&
      onboardingFinished.get() === false
    ) {
  ...
  updateVersionFrom39To40() {
    ...
    try {
      const networkSetting = JSON.parse(setting);
      if (
        networkSetting.i18nTitleKey &&
        UI_STRING_TO_NEW_KEY.hasOwnProperty(networkSetting.i18nTitleKey)
      ) {
        ...
        const newSetting = this.#settings.createSetting(
  ...
  updateVersionFrom41To42() {
    ...
    for (const recording of recordings) {
      recording.flow.title = Platform.StringUtilities.trimEndWithMaxLength(
        recording.flow.title,
        300,
      );
  ...
  updateVersionFrom42To43() {
    const timelineShowAllEventsExperimentEnabled = Root.Runtime.experiments.getValueFromStorage(
    ...
    if (timelineShowAllEventsExperimentEnabled !== undefined) {
      ...
      try {
        const timelineShowAllEventsSetting = this.#settings.moduleSetting(
          'timeline-show-all-events',
        );
