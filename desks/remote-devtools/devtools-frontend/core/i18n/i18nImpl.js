...
export async function fetchAndRegisterLocaleData(
  ...
  const localeDataTextPromise = fetch(getLocaleFetchUrl(locale, location)).then((result) =>
    result.json(),
  );
  const timeoutPromise = new Promise((_, reject) =>
    globalThis.setTimeout(() => reject(new Error('timed out fetching locale')), 5000),
  );
...
export function getLocalizedLanguageRegion(localeString, devtoolsLocale) {
  ...
  const languageInCurrentLocale = new Intl.DisplayNames([devtoolsLocale.locale], {
    type: 'language',
  }).of(language);
  const languageInTargetLocale = new Intl.DisplayNames([targetLanguage], { type: 'language' }).of(
    language,
  );
  let wrappedRegionInCurrentLocale = '';
  let wrappedRegionInTargetLocale = '';
  if (locale.region) {
    const regionInCurrentLocale = new Intl.DisplayNames([devtoolsLocale.locale], {
      type: 'region',
      style: 'short',
    }).of(locale.region);
