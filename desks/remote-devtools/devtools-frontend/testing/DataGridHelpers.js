...
export const assertGridContents = (gridComponent, headerExpected, rowsExpected) => {
  ...
  const rowsActual = getValuesOfAllBodyRows(grid.shadowRoot).map((row) =>
...
export const assertGridWidgetContents = (gridComponent, headerExpected, rowsExpected) => {
  ...
  const headerActual = getHeaderCells(grid, { onlyVisible: false, withJslog: false }).map(
...
export const getValuesOfAllBodyRows = (
  node,
  options = {
    onlyVisible: false,
    withJslog: true,
