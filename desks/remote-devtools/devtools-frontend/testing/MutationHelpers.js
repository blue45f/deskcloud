...
const generateOutputForMutationList = (observedMutations) => {
  ...
  for (const [elem, mutations] of allMutations) {
    ...
    if (removeMutations) {
      mutationOutput.push(
        `${removeMutations} ${pluralize(removeMutations, 'removal', 'removals')}`,
      );
    }
...
export const withMutations = async (expectedMutations, shadowRoot, functionToObserve) => {
  ...
  for (const expectedMutation of expectedMutations) {
    ...
    const matchingMutations = observedMutationsThatMatchExpected(
      expectedMutation,
      observedMutations,
    );
    ...
    if (amountOfMatchingMutations > maxMutationsAllowed) {
      assert.fail(
        `Expected no more than ${maxMutationsAllowed} mutations for ${expectedMutation.type || 'ADD/REMOVE'} ${expectedMutation.target}, but got ${amountOfMatchingMutations}`,
      );
    } else if (amountOfMatchingMutations === 0 && maxMutationsAllowed > 0) {
      assert.fail(
        `Expected at least one mutation for ${expectedMutation.type || 'ADD/REMOVE'} ${expectedMutation.target}, but got ${amountOfMatchingMutations}`,
      );
    }
  }
  // These are mutations that happened but the user did not explicitly list as
  // expected, so we want to fail the test on them.
  const unmatchedMutations = observedMutations.filter(
    (mutation) => !mutationsMatchedToExpected.has(mutation),
  );
