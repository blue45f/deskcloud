...
export function ruleMatch(selectorOrList, properties, options = {}) {
  ...
  const cssProperties = Array.isArray(properties)
    ? properties
    : Object.keys(properties).map((name) => ({ name, value: properties[name] }));
  const selectorList =
    typeof selectorOrList === 'string' ? getSimpleList(selectorOrList) : selectorOrList;
  const matchingSelectors =
    matchingSelectorsIndexes ?? selectorList.selectors.map((_, index) => index);
  return {
    rule: {
      nestingSelectors,
      selectorList,
...
export function getMatchedStylesWithProperties(payload) {
  ...
  const matchedPayload = [
    ruleMatch(payload.selector ?? 'div', payload.properties, { range, origin, styleSheetId }),
  ];
 ...
 */
export function createStubbedDomNodeWithModels(
  opts = {
    nodeId: 1,
  },
) {
