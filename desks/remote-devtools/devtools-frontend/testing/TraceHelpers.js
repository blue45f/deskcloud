...
export async function renderFlameChartIntoDOM(context, options) {
  ...
  if (
    dataProvider instanceof Timeline.TimelineFlameChartDataProvider.TimelineFlameChartDataProvider
  ) {
  ...
  const minTime =
    ...
    Trace.Helpers.Timing.microToMilli(parsedTrace.data.Meta.traceBounds.min);
  const maxTime =
    options.customEndTime ??
    Trace.Helpers.Timing.microToMilli(parsedTrace.data.Meta.traceBounds.max);
...
export function prettyPrint(
  ...
  function printNodes(nodes) {
    for (const node of nodes) {
      ...
      const jsFunctionName = Trace.Types.Events.isProfileCall(event)
...
export function makeFlowEvents(events, flowId = 0) {
  ...
  const flowStart = makeFlowPhaseEvent(
    ...
    firstEvent.ts,
    firstEvent.cat,
    's' /* Trace.Types.Events.Phase.FLOW_START */,
    flowId,
    firstEvent.pid,
    firstEvent.tid,
  );
  const flowEnd = makeFlowPhaseEvent(
    ...
    lastEvent.ts,
    lastEvent.cat,
    'f' /* Trace.Types.Events.Phase.FLOW_END */,
    flowId,
    lastEvent.pid,
    lastEvent.tid,
  );
  ...
  for (let i = 1; i < events.length - 1; i++) {
    flowSteps.push(
      makeFlowPhaseEvent(
        ...
        events[i].ts,
        events[i].cat,
        't' /* Trace.Types.Events.Phase.FLOW_STEP */,
        flowId,
        events[i].pid,
        events[i].tid,
      ),
...
export function makeMockRendererHandlerData(entries, pid = 1, tid = 1) {
  const { tree, entryToNode } = Trace.Helpers.TreeHelpers.treify(entries, {
    filter: { has: () => true },
  });
  const mockThread = {
    tree,
...
export function makeMockSamplesHandlerData(profileCalls) {
  const { tree, entryToNode } = Trace.Helpers.TreeHelpers.treify(profileCalls, {
    filter: { has: () => true },
  });
  const profile = {
    nodes: [],
...
export function makeTimingEventWithPerformanceExtensionData({
  ...
  const firstEvent = {
    ...
    name,
    ph: isMark
      ? 'I' /* Trace.Types.Events.Phase.INSTANT */
      : 'b' /* Trace.Types.Events.Phase.ASYNC_NESTABLE_START */,
...
export async function createTraceExtensionDataFromPerformanceAPITestInput(extensionData) {
  const events = extensionData
    .flatMap(makeTimingEventWithPerformanceExtensionData)
    .sort((e1, e2) => e1.ts - e2.ts);
