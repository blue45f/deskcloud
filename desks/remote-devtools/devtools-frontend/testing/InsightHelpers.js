...
export function createContextForNavigation(data, navigation, frameId) {
  ...
  const max =
    navigationIndex + 1 < data.Meta.mainFrameNavigations.length
...
export function getInsightSetOrError(insights, navigationOrNavigationId) {
  ...
  if (navigationOrNavigationId) {
    const navigationId =
      typeof navigationOrNavigationId === 'string'
    ...
    if (!insightSet) {
      throw new Error(
        `Could not find Insights for navigation ${navigationId}. If you are trying to load an Insight for a particular navigation, you must supply it as an argument to \`getInsightOrError\``,
      );
    }
  } else {
