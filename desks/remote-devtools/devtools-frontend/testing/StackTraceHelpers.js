...
export function stringifyStackTrace(stackTrace) {
  return [
    stringifyFragment(stackTrace.syncFragment),
    ...stackTrace.asyncFragments.map(stringifyAsyncFragment),
  ].join('\n');
}
export class StubStackTrace extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static create(syncFragmentDescriptor, asyncFragmentDescriptors = []) {
    ...
    return new StubStackTrace(
      ...
      asyncFragmentDescriptors.map((fragment) => ({
        description: fragment.description,
        frames: fragment.frames.map(toFrame),
      })),
    );
  }
