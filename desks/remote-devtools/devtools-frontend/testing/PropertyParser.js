...
export class Printer extends SDK.CSSPropertyParser.TreeWalker {
  ...
  enter({ node }) {
    ...
    this.#printedText.push(
      `${'|'.repeat(this.#indent)} ${node.name}${text !== node.name ? `: ${text}` : ''}`,
    );
    this.#indent++;
    return true;
  }
  ...
  static rule(rule) {
    const ast = new SDK.CSSPropertyParser.SyntaxTree(
      '',
      rule,
      CodeMirror.css.cssLanguage.parser.parse(rule).topNode,
    );
