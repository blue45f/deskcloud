...
class BaseSnapshotTester {
  ...
  constructor(context, meta) {
    ...
    this.snapshotPath = meta.url
      .substring(meta.url.lastIndexOf('front_end'))
      .replace('.test.js', '.snapshot.txt')
      .split('?')[0];
  }
  async load() {
  ...
  assert(context, actual) {
    ...
    if (expected === undefined) {
      ...
      throw new Error(
        `snapshot assertion failed! new snapshot found (${title}), must run \`npm run test -- --on-diff=update ...\` to accept it.`,
      );
    }
  ...
  async finish() {
    ...
    if (notRanTest) {
      throw new Error(
        `Snapshots are out of sync (a test was likely deleted or renamed).\nExpected test:\n${notRanTest}\nRun with '--on-diff=update' to fix.`,
      );
    }
  }
