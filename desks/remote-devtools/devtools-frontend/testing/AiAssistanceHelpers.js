...
export function mockAidaClient(data = []) {
  ...
  async function* provideAnswer(_, options) {
    ...
    for (const [idx, chunk] of data[callId].entries()) {
      ...
      if (
        metadata?.attributionMetadata?.attributionAction === Host.AidaClient.RecitationAction.BLOCK
      ) {
...
export function createNetworkRequest(opts) {
  const networkRequest = SDK.NetworkRequest.NetworkRequest.create(
    ...
    opts?.documentURL ?? Platform.DevToolsPath.urlString``,
    null,
    null,
    null,
  );
  ...
  networkRequest.responseHeaders = [
    { name: 'content-type', value: 'bar2' },
    { name: 'x-forwarded-for', value: 'bar3' },
  ];
  if (opts?.includeInitiators) {
    const initiatorNetworkRequest = SDK.NetworkRequest.NetworkRequest.create(
      'requestId-1',
      Platform.DevToolsPath.urlString`https://www.initiator.com`,
    ...
    const initiatedNetworkRequest1 = SDK.NetworkRequest.NetworkRequest.create(
      'requestId-2',
      Platform.DevToolsPath.urlString`https://www.example.com/1`,
      Platform.DevToolsPath.urlString``,
      null,
      null,
      null,
    ...
    const initiatedNetworkRequest2 = SDK.NetworkRequest.NetworkRequest.create(
      'requestId-3',
      Platform.DevToolsPath.urlString`https://www.example.com/2`,
      Platform.DevToolsPath.urlString``,
      null,
      null,
      null,
    ...
    sinon
      .stub(Logs.NetworkLog.NetworkLog.instance(), 'initiatorGraphForRequest')
      ...
      .returns({
        initiators: new Set([]),
        initiated: new Map([[initiatedNetworkRequest1, networkRequest]]),
      })
      .withArgs(initiatedNetworkRequest2)
      .returns({
        initiators: new Set([]),
        initiated: new Map([[initiatedNetworkRequest2, networkRequest]]),
      });
  }
...
export async function createAiAssistancePanel(options) {
  let aidaAvailabilityForStub =
    options?.aidaAvailability ??
  ...
  const checkAccessPreconditionsStub = sinon
    .stub(Host.AidaClient.AidaClient, 'checkAccessPreconditions')
    .callsFake(() => {
...
export const setupAutomaticFileSystem = (
  options = {
    hasFileSystem: false,
  },
) => {
  ...
  const inspectorFrontendHost = sinon.createStubInstance(
    Host.InspectorFrontendHost.InspectorFrontendHostStub,
  );
  inspectorFrontendHost.events = sinon.createStubInstance(Common.ObjectWrapper.ObjectWrapper);
  const projectSettingsModel = sinon.createStubInstance(
    ProjectSettings.ProjectSettingsModel.ProjectSettingsModel,
  );
  sinon.stub(projectSettingsModel, 'availability').value('available');
  sinon
    .stub(projectSettingsModel, 'projectSettings')
    .value(options.hasFileSystem ? { workspace: { root, uuid } } : {});
...
export async function createPatchWidgetWithDiffView(options) {
  ...
  assert.strictEqual(
    (await view.nextInput).patchSuggestionState,
    AiAssistancePanel.PatchWidget.PatchSuggestionState.SUCCESS,
  );
...
export function initializePersistenceImplForTests() {
  ...
  const debuggerWorkspaceBinding =
    Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance({
      ...
      resourceMapping: new Bindings.ResourceMapping.ResourceMapping(
...
export function createTestFilesystem(fileSystemPath, files) {
  ...
  for (const file of files ?? []) {
    const uiSourceCode = project.createUISourceCode(
      Platform.DevToolsPath.urlString`${fileSystemPath}/${file.path}`,
