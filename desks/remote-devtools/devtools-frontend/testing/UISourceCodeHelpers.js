...
export function createContentProviderUISourceCodes(options) {
  ...
  for (const item of options.items) {
    const resourceType =
      item.resourceType || Common.ResourceType.ResourceType.fromMimeType(item.mimeType);
...
export function createContentProviderUISourceCode(options) {
  ...
  const { project, uiSourceCodes } = createContentProviderUISourceCodes({
    items: [{ url, content, mimeType, metadata }],
    projectType,
    projectId,
    target,
  });
...
class TestFileSystem extends Persistence.FileSystemWorkspaceBinding.FileSystem {
  ...
  requestFileContent(_uiSourceCode) {
    return Promise.resolve(
      new TextUtils.ContentData.ContentData(this.#content, /* isBase64 */ false, 'text/plain'),
    );
  }
...
export function createFileSystemUISourceCode(options) {
  ...
  const uiSourceCode = project.createUISourceCode(
    options.url,
    Common.ResourceType.ResourceType.fromMimeType(options.mimeType),
  );
  project.addUISourceCode(uiSourceCode);
  return { uiSourceCode, project };
}
export function setupMockedUISourceCode(url = 'https://example.com/') {
  const projectStub = sinon.createStubInstance(
