...
export class TestUniverse {
  ...
  get cssWorkspaceBinding() {
    if (!this.#context.has(Bindings.CSSWorkspaceBinding.CSSWorkspaceBinding)) {
      this.#context.set(
        ...
        new Bindings.CSSWorkspaceBinding.CSSWorkspaceBinding(
  ...
  get multitargetNetworkManager() {
    if (!this.#context.has(SDK.NetworkManager.MultitargetNetworkManager)) {
      const multitargetNetworkManager = new SDK.NetworkManager.MultitargetNetworkManager(
        this.targetManager,
      );
    ...
    if (!this.#context.has(SDK.TargetManager.TargetManager)) {
      ...
      const context = new (class LazyContext {
        ...
        get(ctor) {
          ...
          throw new Error(
            `Class ${ctor.name} not set-up as a dependency for SDKModels in TestUniverse.ts. Add it to LazyContext#get in TestUniverse.ts`,
          );
        }
      })();
