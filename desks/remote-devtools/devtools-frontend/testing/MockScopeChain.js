...
export class MockProtocolBackend {
  ...
  constructor() {
    ...
    setMockConnectionResponseHandler(
      ...
      this.#getScriptSourceHandler.bind(this),
    );
    setMockConnectionResponseHandler(
  ...
  async addScript(target, scriptDescription, sourceMap) {
    ...
    let endColumn =
      scriptDescription.content.length - scriptDescription.content.lastIndexOf('\n') - 1;
    ...
    if (sourceMap) {
      ...
      const loadedSourceMap = await debuggerModel
        .sourceMapManager()
        .sourceMapForClientPromise(scriptObject);
  ...
  async createCallFrame(target, script, scopeDescriptor, sourceMap, scopeObjects = []) {
    ...
    const scopeChain = parsedScopes.map((s) =>
      this.#createProtocolScope(
        s.type,
        { type: 'object' /* Protocol.Runtime.RemoteObjectType.Object */ },
        scriptObject.scriptId,
        s.startLine,
        s.startColumn,
        s.endLine,
        s.endColumn,
  ...
  responderToBreakpointByUrlRequest(url, lineNumber) {
    ...
    this.#setBreakpointByUrlResponses.set(key, {
...
export class MockDebuggerBackend {
  ...
  constructor() {
    ...
    this.cdpConnection.setHandler('Storage.getStorageKey', () => ({
      result: { storageKey: 'test-key' },
    }));
    this.cdpConnection.setHandler(
      'Debugger.removeBreakpoint',
      this.#removeBreakpointHandler.bind(this),
    ...
    this.cdpConnection.setHandler('Debugger.enable', () => ({
      result: { debuggerId: 'DEBUGGER_ID' },
    }));
