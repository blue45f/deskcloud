...
export class TraceLoader {
  ...
  static async traceEngine(
    ...
    const fileContents = await wrapInTimeout(
      context,
      async () => {
        return await TraceLoader.fixtureContents(context, name);
      },
      30_000,
      `Loading fixtureContents for ${name}`,
    );
    const parsedTraceFileAndModel = await wrapInTimeout(
      context,
      async () => {
        return await TraceLoader.executeTraceEngineOnFileContents(
          fileContents,
          /* emulate fresh recording */ false,
          config,
        );
      },
      30_000,
      `Executing traceEngine for ${name}`,
    );
    ...
    if (timelineModule) {
      await wrapInTimeout(
        ...
        () => {
          timelineModule.ModificationsManager.ModificationsManager.reset();
          timelineModule.ModificationsManager.ModificationsManager.initAndActivateModificationsManager(
            parsedTraceFileAndModel.model,
            0,
          );
        },
        5_000,
        `Creating modification manager for ${name}`,
      );
    }
...
async function wrapInTimeout(mochaContext, callback, timeoutMs, stepName) {
  ...
  const timeoutId = setTimeout(() => {
    ...
    console.error(
      `TraceLoader: [${stepName}]: took longer than ${timeoutMs}ms in test "${testTitle}"`,
    );
