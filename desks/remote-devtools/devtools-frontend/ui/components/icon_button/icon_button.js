...
var l = (c, t) => {
  for (var e in t) m(c, e, { get: t[e], enumerable: !0 });
};
var h = {};
l(h, { FileSourceIcon: () => n });
import './../../kit/kit.js';
import { Directives as v, html as g, render as f } from './../../lit/lit.js';
var a = `:host{height:var(--sys-size-9);width:var(--sys-size-9);devtools-icon{height:100%;width:100%}devtools-icon.document{color:var(--override-file-source-icon-color,var(--icon-default))}devtools-icon.script,
...
var { classMap: b } = v,
  n = class extends HTMLElement {
    ...
    #e;
    set data(t) {
      ((this.#i = t.contentType),
        (this.#t = t.hasDotBadge),
        (this.#e = t.isDotPurple),
        (this.#o = t.iconType),
        this.#s());
    }
    get data() {
...
var { html: s } = o,
  r = class extends HTMLElement {
    ...
    #r;
    set data(t) {
      ((this.#i = t.groups.map((e) => ({ ...e }))),
        (this.#o = t.clickHandler),
        (this.#s = t.trailingText ?? ''),
        (this.#e = t.leadingText ?? ''),
        (this.#r = t.accessibleName),
        (this.#t = !!t.compact),
        this.#l());
    }
    get data() {
      return {
        groups: this.#i.map((t) => ({ ...t })),
        accessibleName: this.#r,
        clickHandler: this.#o,
        leadingText: this.#e,
        trailingText: this.#s,
        compact: this.#t,
    ...
    #c(t) {
      this.#o && (t.preventDefault(), this.#o());
    }
    #l() {
      let t = o.Directives.classMap({
          'icon-button': !0,
          'with-click-handler': !!this.#o,
          compact: this.#t,
        }),
        e = this.#i.filter((i) => i.text !== void 0).filter((i, u) => (this.#t ? u === 0 : !0));
      o.render(
        s`
      <style>${d}</style>
      <button class=${t} @click=${this.#c} aria-label=${o.Directives.ifDefined(this.#r)}>
      ${!this.#t && this.#e ? s`<span class="icon-button-title">${this.#e}</span>` : o.nothing}
      ${e.map(
        (i) => s`
      <devtools-icon class="status-icon" name=${i.iconName} style="color: ${i.iconColor}; width: ${i.iconWidth || 'var(--sys-size-7)'}; height: ${i.iconHeight || 'var(--sys-size-7)'}">
      </devtools-icon>
      ${this.#t ? s`<!-- Force line-height for this element --><span>&#8203;</span>` : o.nothing}
      <span class="icon-button-title">${i.text}</span>`,
      )}
      </button>
...
customElements.define('icon-button', r);
export { h as FileSourceIcon, p as IconButton };
