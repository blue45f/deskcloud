...
const positioningUtils = {
  ...
  isInBounds: ({ inspectorViewRect, currentPopoverRect, proposedRect }) => {
    return (
      inspectorViewRect.left <= proposedRect.left &&
...
export const proposedRectForRichTooltip = ({
  inspectorViewRect,
  anchorRect,
  currentPopoverRect,
  preferredPositions,
  ...
  const uniqueOrder = [...new Set([...preferredPositions, ...Object.values(PositionOption)])];
  ...
  const bottomVerticalOutOfBounds = Math.max(
    0,
    bottomProposed.top + currentPopoverRect.height - inspectorViewRect.bottom,
  );
  ...
  const fallbackOption =
    uniqueOrder.find((option) => {
      if (prefersBottom) {
        return (
          option === PositionOption.BOTTOM_SPAN_LEFT || option === PositionOption.BOTTOM_SPAN_RIGHT
        );
      }
  ...
  return positioningUtils.insetAdjustedRect({
    currentPopoverRect,
    inspectorViewRect,
...
export const proposedRectForSimpleTooltip = ({
  ...
  const bottomVerticalOutOfBoundsAmount = Math.max(
    0,
    proposedRect.top + currentPopoverRect.height - inspectorViewRect.bottom,
  );
...
export class Tooltip extends HTMLElement {
  ...
  hideTooltip = (event) => {
    ...
    if (
      event &&
      this.variant === 'rich' &&
      (event.target === this.#anchor || this.#containsNode(event.target)) &&
  ...
  #positionPopover = () => {
    ...
    if (
      positioningUtils.isSameRect(this.#previousAnchorRect, anchorRect) &&
    ...
    const preferredPositions = this.preferSpanLeft
      ? [PositionOption.BOTTOM_SPAN_LEFT, PositionOption.TOP_SPAN_LEFT]
      : [];
    const proposedPopoverRect =
      this.variant === 'rich'
        ? proposedRectForRichTooltip({
            inspectorViewRect,
            anchorRect,
            currentPopoverRect,
            preferredPositions,
    ...
    const actualVerticalOffset =
      anchorRect.top < proposedPopoverRect.top
        ? this.verticalDistanceIncrease
        : -this.verticalDistanceIncrease;
  ...
  #updateJslog() {
    if (this.jslogContext && this.#anchor) {
      ...
      this.setAttribute(
        'jslog',
        VisualLogging.popover(this.jslogContext).parent('mapped').toString(),
      );
    } else {
  ...
  #attachToAnchor() {
    if (!this.#anchor) {
      ...
      if (this.variant === 'rich' && describedbyAnchor) {
        console.warn(
          `The anchor for tooltip ${id} was defined with "aria-describedby". For rich tooltips "aria-details" is more appropriate.`,
        );
      }
    }
