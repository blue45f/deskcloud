...
export class Dialog extends HTMLElement {
  ...
  set state(state) {
    ...
    if (
      this.#props.state === 'collapsed' /* DialogState.COLLAPSED */ ||
      this.#props.state === 'disabled' /* DialogState.DISABLED */
    ) {
      this.#forceDialogCloseInDevToolsBound();
    }
    this.#onStateChange();
  ...
  connectedCallback() {
    ...
    this.#devtoolsMutationObserver.observe(this.#devToolsBoundingElement, {
      childList: true,
      subtree: true,
    });
    ...
    this.style.setProperty(
      '--override-dialog-content-border',
      `${CONNECTOR_HEIGHT}px solid transparent`,
    );
    this.style.setProperty(
      '--dialog-padding',
      `${DIALOG_VERTICAL_PADDING}px ${DIALOG_SIDE_PADDING}px`,
    );
  }
  ...
  #mouseEventWasInDialogContent(evt) {
    ...
    let animationOffSetValue =
      this.bestVerticalPosition === 'bottom' /* DialogVerticalPosition.BOTTOM */
    ...
    const eventWasDialogContentY =
      evt.pageY >= dialogBounds.top + animationOffSetValue &&
  ...
  #mouseEventWasInHitArea(evt) {
    ...
    const eventWasInHitAreaX =
      evt.pageX >= hitAreaBounds.left && evt.pageX <= hitAreaBounds.left + hitAreaBounds.width;
    const eventWasInHitAreaY =
      evt.pageY >= hitAreaBounds.top && evt.pageY <= hitAreaBounds.top + hitAreaBounds.height;
  ...
  #getBestVerticalPosition(originBounds, dialogHeight, devtoolsBounds) {
    ...
    if (
      originBounds.bottom + dialogHeight > devtoolsBounds.height &&
    ...
    void RenderCoordinator.read(() => {
      ...
      if (this.#props.origin === MODAL) {
        void RenderCoordinator.write(() => {
          ...
          this.style.setProperty(
            '--dialog-max-height',
            `${devToolsHeight - DIALOG_PADDING_FROM_WINDOW}px`,
          );
          this.style.setProperty(
            '--dialog-max-width',
            `${devToolsWidth - DIALOG_PADDING_FROM_WINDOW}px`,
          );
          this.style.setProperty(
            '--dialog-right',
            `${document.body.clientWidth - devToolsRight}px`,
          );
        });
        return;
      }
      ...
      const {
        ...
        bottom: anchorBottom,
        left: anchorLeft,
      } = absoluteAnchorBounds;
      const originCenterX = (anchorLeft + anchorRight) / 2;
      const hitAreaWidth = anchorRight - anchorLeft + CONNECTOR_HEIGHT;
      const windowWidth = document.body.clientWidth;
      const connectorFixedXValue = this.#props.getConnectorCustomXPosition
        ? this.#props.getConnectorCustomXPosition()
        : originCenterX;
      void RenderCoordinator.write(() => {
        ...
        this.#bestHorizontalAlignment =
          this.#props.horizontalAlignment === 'auto' /* DialogHorizontalAlignment.AUTO */
            ? this.#getBestHorizontalAlignment(absoluteAnchorBounds, devtoolsBounds)
            : this.#props.horizontalAlignment;
        this.#bestVerticalPosition =
          this.#props.position === 'auto' /* DialogVerticalPosition.AUTO */
            ? this.#getBestVerticalPosition(absoluteAnchorBounds, dialogHeight, devtoolsBounds)
            : this.#props.position;
        if (
          this.#bestHorizontalAlignment === 'auto' /* DialogHorizontalAlignment.AUTO */ ||
        ...
        switch (this.#bestHorizontalAlignment) {
          ...
          case 'right' /* DialogHorizontalAlignment.RIGHT */: {
            ...
            const windowRightBorderToDialogRight = Math.max(
              windowRightBorderToAnchorRight,
              windowRightBorderToDevToolsRight,
            );
          ...
          case 'center' /* DialogHorizontalAlignment.CENTER */: {
            ...
            const dialogCappedWidth = Math.min(
              devToolsWidth - DIALOG_PADDING_FROM_WINDOW,
              dialogWidth,
            );
        ...
        switch (this.#bestVerticalPosition) {
          ...
          default:
            Platform.assertNever(
              this.#bestVerticalPosition,
              `Unknown position type: ${this.#bestVerticalPosition}`,
            );
        }
        dialog.close();
  ...
  #handleScrollAttempt(event) {
    if (
      this.#mouseEventWasInDialogContent(event) ||
  ...
  #render() {
    ...
    Lit.render(
      html`
        ...
        <dialog
          @click=${this.#handlePointerEvent}
          @pointermove=${this.#handlePointerEvent}
          @cancel=${this.#onCancel}
          @animationend=${this.#animationEndedEvent}
          jslog=${VisualLogging.dialog(this.#props.jslogContext)
            .track({ resize: true, keydown: 'Escape' })
            .parent('mapped')}
        >
          ${dialogContent}
        </dialog>
