...
const {
  html,
  Directives: { ifDefined, ref, classMap },
} = Lit;
export class Button extends HTMLElement {
  ...
  #onClick(event) {
    ...
    if (
      ...
      this.#props.variant === 'icon_toggle' /* Variant.ICON_TOGGLE */ &&
      this.#props.iconName
    ) {
  ...
  #isToolbarVariant() {
    return (
      this.#props.variant === 'toolbar' /* Variant.TOOLBAR */ ||
      this.#props.variant === 'primary_toolbar' /* Variant.PRIMARY_TOOLBAR */
    );
  }
  #render() {
    ...
    const classes = {
      ...
      icon:
        this.#props.variant === 'icon' /* Variant.ICON */ ||
        this.#props.variant === 'icon_toggle' /* Variant.ICON_TOGGLE */ ||
    ...
    const jslog =
      this.#props.jslogContext &&
      VisualLogging.action().track({ click: true }).context(this.#props.jslogContext);
    // clang-format off
    Lit.render(
      html`
        <style>
        ...
        >
          ${hasIcon
            ? html` <devtools-icon
                name=${ifDefined(
                  this.#props.toggled ? this.#props.toggledIconName : this.#props.iconName,
                )}
              >
              </devtools-icon>`
            : ''}
          ${this.#props.longClickable
            ? html` <devtools-icon name="triangle-bottom-right" class="long-click">
              </devtools-icon>`
            : ''}
