...
export class TreeOutline extends HTMLElement {
  ...
  #defaultRenderer = (node, _state) => {
    if (typeof node.treeNodeData !== 'string') {
      console.warn(
        `The default TreeOutline renderer simply stringifies its given value. You passed in ${JSON.stringify(node.treeNodeData, null, 2)}. Consider providing a different defaultRenderer that can handle nodes of this type.`,
      );
    }
  ...
  async expandRecursively(maxDepth = 2) {
    await Promise.all(
      this.#treeData.map((rootNode) => this.#expandAndRecurse(rootNode, 0, maxDepth)),
    );
    await this.#render();
  }
  /**
  ...
  #setNodeKeyNoWrapCSSVariable(attributeValue) {
    this.style.setProperty(
      ...
      attributeValue !== null ? 'nowrap' : 'initial',
    );
  }
  #setTopLevelNodeBorderColorCSSVariable(attributeValue) {
    this.style.setProperty(
      '--override-top-node-border',
      attributeValue ? `1px solid ${attributeValue}` : '',
    );
  }
  async #recursivelyCollapseTreeNodeChildren(treeNode) {
    ...
    const childRecursions = Promise.all(
      children.map((child) => this.#recursivelyCollapseTreeNodeChildren(child)),
    );
  ...
  async #expandAndRecurse(node, currentDepth, maxDepth) {
    ...
    await Promise.all(
      children.map((child) => this.#expandAndRecurse(child, currentDepth + 1, maxDepth)),
    );
  }
  #onArrowClick(node) {
    return (event) => {
  ...
  #renderNode(node, { depth, setSize, positionInSet }) {
    ...
    if (!isExpandableNode(node) || !nodeIsExpanded) {
      ...
      // clang-format off
      childrenToRender = html`<ul role="group">
        ${Lit.Directives.until(childNodes)}
      </ul>`;
      // clang-format on
    }
    ...
    const ariaExpandedAttribute = !isExpandableNode(node)
      ? undefined
      : nodeIsExpanded
        ? 'true'
        : 'false';
    ...
    return html`
      <li
        ...
        jslog=${VisualLogging.treeItem(node.jslogContext).track({
          click: true,
          resize: true,
