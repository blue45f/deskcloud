...
const getFirstChildOfExpandedTreeNode = (currentDOMNode) => {
  const firstChild = currentDOMNode.querySelector(
    ':scope > [role="group"] > [role="treeitem"]:first-child',
  );
  if (!firstChild) {
...
const getDeepLastChildOfExpandedTreeNode = (currentDOMNode) => {
  const lastChild = currentDOMNode.querySelector(
    ':scope > [role="group"] > [role="treeitem"]:last-child',
  );
  if (!lastChild) {
    throw new Error('Could not find child of expanded node.');
  }
...
const getParentListItemForDOMNode = (currentDOMNode) => {
  ...
  while (
    parentNode &&
    parentNode.getAttribute('role') !== 'treeitem' &&
...
const getPathToTreeNodeRecursively = async (currentNode, nodeIdToFind, pathToNode) => {
  ...
  if (currentNode.children) {
    ...
    for (const child of children) {
      const foundPathOrNull = await getPathToTreeNodeRecursively(child, nodeIdToFind, [
        ...pathToNode,
        child,
      ]);
      if (foundPathOrNull !== null) {
