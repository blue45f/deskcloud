...
const str_ = i18n.i18n.registerUIStrings(
  'ui/components/request_link_icon/RequestLinkIcon.ts',
  UIStrings,
);
...
export class RequestLinkIcon extends HTMLElement {
  ...
  set data(data) {
    ...
    if (!this.#request && typeof data.affectedRequest?.requestId !== 'undefined') {
      ...
      this.#requestResolver
        .waitFor(data.affectedRequest.requestId)
        .then((request) => {
  ...
  #maybeRenderURL() {
    ...
    return html`<span aria-label=${i18nString(UIStrings.shortenedURL)} title=${url}
      >${filename}</span
    >`;
  }
  async #render() {
    return await RenderCoordinator.write(() => {
      ...
      if (this.#request || this.#affectedRequest?.requestId !== undefined) {
        ...
        template = html` <button
          class=${Lit.Directives.classMap({ link: Boolean(this.#request) })}
