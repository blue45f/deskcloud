...
class DiffRenderer {
  ...
  #renderRow(row) {
    const baseNumber =
      row.type === 'equal' /* RowType.EQUAL */ || row.type === 'deletion' /* RowType.DELETION */
        ? String(row.originalLineNumber)
        : '';
    const curNumber =
      row.type === 'equal' /* RowType.EQUAL */ || row.type === 'addition' /* RowType.ADDITION */
    ...
    if (row.type === 'addition' /* RowType.ADDITION */) {
      ...
      screenReaderText = html`<span class="diff-hidden-text"
        >${i18nString(UIStrings.additions)}</span
      >`;
    } else if (row.type === 'deletion' /* RowType.DELETION */) {
      marker = '-';
      markerClass += ' diff-line-deletion';
      screenReaderText = html`<span class="diff-hidden-text"
        >${i18nString(UIStrings.deletions)}</span
      >`;
    }
    return html` <div class="diff-line-number" aria-hidden="true">${baseNumber}</div>
      <div class="diff-line-number" aria-hidden="true">${curNumber}</div>
      <div class=${markerClass} aria-hidden="true">${marker}</div>
      <div
        class="diff-line-content diff-line-${row.type}"
        data-line-number=${curNumber}
        jslog=${VisualLogging.link('changes.reveal-source').track({ click: true })}
      >
        ${screenReaderText}${this.#renderRowContent(row)}
      </div>`;
  }
  #renderRowContent(row) {
    ...
    const [doc, startPos] =
      row.type === 'deletion' /* RowType.DELETION */
    ...
    for (const token of row.tokens) {
      ...
      content.push(
        token.className
          ? html`<span class=${token.className}>${tokenContent}</span>`
          : html`${tokenContent}`,
      );
