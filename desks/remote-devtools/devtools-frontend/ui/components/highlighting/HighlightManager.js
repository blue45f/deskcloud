...
export class HighlightManager {
  ...
  highlightOrderedTextRanges(root, sourceRanges, isCurrent = false) {
    ...
    const ranges = sourceRanges
      .map((range) => rangeWalker.nextRange(range.offset, range.length))
  ...
  apply(node) {
    ...
    if (state.currentRange) {
      state.activeRanges.push(
        ...this.highlightOrderedTextRanges(node, [state.currentRange], /* isCurrent=*/ true),
      );
    }
  }
