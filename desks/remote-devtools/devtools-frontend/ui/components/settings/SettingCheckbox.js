...
export class SettingCheckbox extends HTMLElement {
  ...
  icon() {
    ...
    if (this.#setting.deprecation) {
      return html`<devtools-setting-deprecation-warning
        .data=${this.#setting.deprecation}
      ...
      if (learnMore.tooltip) {
        ...
        return html`
          ...
          <devtools-tooltip id=${id} variant="rich">
            ...
            ${url
              ? html`<devtools-link href=${url} class="link" .jslogContext=${jsLogContext}
    ...
    const reason = disabledReasons.length
      ? html`
          <devtools-button
            ...
            .variant=${'icon' /* Buttons.Button.Variant.ICON */}
            .size=${'SMALL' /* Buttons.Button.Size.SMALL */}
            title=${ifDefined(disabledReasons.join('\n'))}
            @click=${onclick}
          ></devtools-button>
        `
      : Lit.nothing;
