...
var { render: m, html: o } = t,
  r = class extends HTMLElement {
    ...
    #e = [];
    set data(e) {
      ((this.#o = e.nodeTitle), (this.#s = e.nodeId), (this.#e = e.nodeClasses), this.#r());
    }
    #r() {
      let e = !!this.#s,
        s = !!(this.#e && this.#e.length > 0),
        l = [o`<span class="node-label-name">${this.#o}</span>`];
      if (this.#s) {
        let a = t.Directives.classMap({ 'node-label-id': !0, 'node-multiple-descriptors': s });
        l.push(o`<span class=${a}>#${CSS.escape(this.#s)}</span>`);
      }
      if (this.#e && this.#e.length > 0) {
        let a = this.#e.map((p) => `.${CSS.escape(p)}`).join(''),
          c = t.Directives.classMap({ 'node-label-class': !0, 'node-multiple-descriptors': e });
        l.push(o`<span class=${c}>${a}</span>`);
      }
      m(
        o`
      <style>${n}</style>
...
customElements.define('devtools-node-text', r);
export { d as NodeText };
