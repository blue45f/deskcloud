...
export function done(options) {
  ...
  return new Promise((resolve) =>
    globalThis.addEventListener(RenderCoordinatorQueueEmptyEvent.eventName, () => resolve(), {
      once: true,
    }),
  );
}
...
function logIfEnabled(value) {
  ...
  if (loggingEnabled.onlyNamed) {
    if (
      value.endsWith(UNNAMED_READ) ||
      value.endsWith(UNNAMED_WRITE) ||
      value.endsWith(UNNAMED_SCROLL)
    ) {
      return;
    }
  }
