...
export async function completeInContext(textBefore, query, force = false) {
  ...
  const result = await javascriptCompletionSource(
    new CodeMirror.CompletionContext(state, state.doc.length, force),
  );
  return result
    ? result.options
...
export function getQueryType(tree, pos, doc) {
  ...
  if (node.name === 'PropertyName' || node.name === 'PrivatePropertyName') {
    return parent?.name !== 'MemberExpression'
      ? null
  ...
  if (
    ...
    (!node.firstChild &&
      node.to - node.from < 20 &&
      !/[^a-z]/.test(doc.sliceString(node.from, node.to)))
  ) {
  ...
  if (node.name === '(') {
    ...
    if (parent?.name === 'ArgList' && parent?.parent?.name === 'CallExpression') {
      ...
      if (callReceiver?.name === 'MemberExpression') {
        ...
        if (
          propertyExpression &&
          doc.sliceString(propertyExpression.from, propertyExpression.to) === 'get'
        ) {
          // map
          const potentiallyMapObject = callReceiver?.firstChild;
          return {
            type: 3 /* QueryType.POTENTIALLY_RETRIEVING_FROM_MAP */,
            relatedNode: potentiallyMapObject || undefined,
          };
        }
      }
    }
...
export async function javascriptCompletionSource(cx) {
  ...
  if (
    !query ||
    (query.from === undefined && !cx.explicit && query.type === 0) /* QueryType.EXPRESSION */
  ) {
    return null;
  }
  ...
  if (query.type === 0 /* QueryType.EXPRESSION */) {
    ...
    query.type === 1 /* QueryType.PROPERTY_NAME */ ||
    query.type === 2 /* QueryType.PROPERTY_EXPRESSION */
  ) {
    ...
    result = await maybeCompleteKeysFromMap(
      cx.state.sliceDoc(potentialMapObject.from, potentialMapObject.to),
    );
  } else {
    return null;
  }
...
async function completePropertiesInner(expression, context, quoted, hasBracket = false) {
  ...
  if (object && (object.type === 'object' || object.type === 'function')) {
    ...
    for (const prop of properties.properties || []) {
      if (
        !prop.symbol &&
        !(isFunction && (prop.name === 'arguments' || prop.name === 'caller')) &&
        (quoted || SPAN_IDENT.test(prop.name))
      ) {
        const label = quoted
          ? quoted + prop.name.replaceAll('\\', '\\\\').replaceAll(quoted, '\\' + quoted) + quoted
          : prop.name;
...
async function completeExpressionInScope() {
  ...
  const scopes = await Promise.all(
    selectedFrame
      .scopeChain()
      .map((scope) =>
        SourceMapScopes.NamesResolver.resolveScopeInObject(scope).getAllProperties(false, false),
      ),
  );
...
export async function isExpressionComplete(expression) {
  ...
  const result = await currentExecutionContext.runtimeModel.compileScript(
    expression,
    '',
    false,
    currentExecutionContext.id,
  );
  ...
  if (description) {
    return (
      !description.startsWith('SyntaxError: Unexpected end of input') &&
...
async function getArgumentsForExpression(callee, doc) {
  ...
  return await getArgumentsForFunctionValue(result, objGetter, expression).finally(() =>
...
export function argumentsList(input) {
  ...
  try {
    try {
      ...
      const { parser } = CodeMirror.javascript.javascriptLanguage.configure({
        strict: true,
        top: 'SingleClassItem',
      });
          ...
          if (cursor.name === 'ClassBody' && cursor.firstChild()) {
            do {
              if (cursor.name === 'MethodDeclaration' && cursor.firstChild()) {
                if (
                  cursor.name === 'PropertyDefinition' &&
...
async function getArgumentsForFunctionValue(object, receiverObjGetter, functionName) {
  ...
  const javaScriptMetadata =
    JavaScriptMetaData.JavaScriptMetadata.JavaScriptMetadataImpl.instance();
  ...
  if (
    receiverObj.description &&
    receiverObj.type === 'function' &&
...
async function prototypesFromObject(object) {
  ...
  return (
    (await object.callFunctionJSON(function () {
      const result = [];
...
async function getArgumentsForBoundFunction(object) {
  ...
  const originalSignatures = await getArgumentsForFunctionValue(target, () =>
    Promise.resolve(thisValue),
  );
  ...
  return originalSignatures.map((signature) => {
    ...
    return restIndex > -1 && restIndex < boundArgsLength
      ? signature.slice(restIndex)
