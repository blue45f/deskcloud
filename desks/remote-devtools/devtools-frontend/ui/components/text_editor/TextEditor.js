...
export class TextEditor extends HTMLElement {
  ...
  #createEditor() {
    ...
    ThemeSupport.ThemeSupport.instance().addEventListener(
      ...
      () => {
        const currentTheme =
          ThemeSupport.ThemeSupport.instance().themeName() === 'dark' ? dummyDarkTheme : [];
  ...
  revealPosition(selection, highlight = true) {
    ...
    if (!selection.main.empty) {
      ...
      !targetPos ||
      targetPos.top < editorRect.top ||
      targetPos.bottom > editorRect.bottom
    ) {
    ...
    for (const effect of tr.effects) {
      if (effect.is(clearHighlightedLine)) {
        ...
        value = CodeMirror.Decoration.set([
          CodeMirror.Decoration.line({ attributes: { class: 'cm-highlightedLine' } }).range(
            effect.value,
          ),
        ]);
      }
