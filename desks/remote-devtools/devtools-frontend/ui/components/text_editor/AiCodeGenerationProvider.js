...
export class AiCodeGenerationProvider {
  ...
  #aiCodeGenerationEnabledSetting = Common.Settings.Settings.instance().createSetting(
    'ai-code-completion-enabled',
    false,
  );
  #aiCodeGenerationSettingEnabled = this.#aiCodeGenerationEnabledSetting.get();
  #aiCodeGenerationOnboardingCompletedSetting = Common.Settings.Settings.instance().createSetting(
  ...
  extension() {
    return [
      ...
      CodeMirror.EditorView.updateListener.of((update) =>
        this.#abortOrDismissGenerationDuringUpdate(update),
      ),
      aiAutoCompleteSuggestion,
      aiAutoCompleteSuggestionState,
  ...
  dispose() {
    ...
    this.#aiCodeGenerationEnabledSetting.removeChangeListener(
  ...
  editorInitialized(editor) {
    ...
    this.#aiCodeGenerationEnabledSetting.addChangeListener(
      this.#boundOnUpdateAiCodeGenerationState,
    );
  ...
  async #setupAiCodeGeneration() {
    ...
    this.#editor?.dispatch({
      effects: [
        this.#generationTeaserCompartment.reconfigure([
          aiCodeGenerationTeaserExtension(this.#generationTeaser),
        ]),
      ],
    });
  }
        ...
        run: () => {
          ...
          if (hasActiveAiSuggestion(this.#editor.state)) {
            if (
              ...
              'completion' /* AiSuggestionSource.COMPLETION */
            ) {
  ...
  #activateTeaser(update) {
    ...
    update.view.dispatch({
      effects: setAiCodeGenerationTeaserMode.of(AiCodeGenerationTeaserMode.ACTIVE),
    });
  }
  /**
  ...
  async #triggerAiCodeGeneration(options) {
    ...
    const commentNodeInfo = AiCodeGenerationParser.extractCommentNodeInfo(
      this.#editor.state,
      cursor,
    );
    if (!commentNodeInfo) {
    ...
    try {
      ...
      const preamble =
        AiCodeGeneration.AiCodeGeneration.basePreamble +
      ...
      const shouldBlock =
        topSample.attributionMetadata?.attributionAction === Host.AidaClient.RecitationAction.BLOCK;
      if (shouldBlock) {
        return;
      }
      ...
      this.#editor.dispatch({
        effects: [
          setAiAutoCompleteSuggestion.of({
            ...
            startTime,
            onImpression: this.#aiCodeGeneration?.registerUserImpression.bind(
              this.#aiCodeGeneration,
            ),
      ...
      if (
        e instanceof Host.DispatchHttpRequestClient.DispatchHttpRequestError &&
  ...
  return CodeMirror.ViewPlugin.fromClass(
    class {
      ...
      get decorations() {
        ...
        const canShowDiscoveryState = UI.UIUtils.PromotionManager.instance().canShowPromotion(
          PanelCommon.AiCodeGenerationTeaser.PROMOTION_ID,
        );
        if (isEmptyLine && canShowDiscoveryState) {
          return CodeMirror.Decoration.set([
            CodeMirror.Decoration.widget({
              widget: new AccessiblePlaceholder(teaser),
              side: 1,
            }).range(cursorPosition),
          ]);
        }
        const commentInfo = AiCodeGenerationParser.extractCommentNodeInfo(
          this.#view.state,
          cursorPosition,
        );
        if (commentInfo) {
