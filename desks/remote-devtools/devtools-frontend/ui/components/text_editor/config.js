...
export class DynamicSetting {
  ...
  instance() {
    return [this.compartment.of(this.getExtension(this.settingValue())), dynamicSetting.of(this)];
  }
  sync(state, value) {
...
export const tabMovesFocus = DynamicSetting.bool(
...
export const conservativeCompletion = CM.StateField.define({
  ...
  update(value, tr) {
    ...
    if (
      (CM.selectedCompletionIndex(tr.startState) ?? 0) !==
        (CM.selectedCompletionIndex(tr.state) ?? 0) ||
...
export const bracketMatching = DynamicSetting.bool(
...
const AutoDetectIndent = CM.StateField.define({
  ...
  update: (indent, tr) => {
    return tr.docChanged && preservedLength(tr.changes) <= RECOMPUTE_INDENT_MAX_SIZE
      ? detectIndentation(tr.state.doc)
      : indent;
  },
...
function detectIndentation(doc) {
  ...
  return (
    indentUnit ?? Common.Settings.Settings.instance().moduleSetting('text-editor-indent').get()
  );
}
export const autoDetectIndent = DynamicSetting.bool(
...
const baseKeymap = CM.keymap.of([
  ...
  { key: 'Mod-d', run: CM.selectNextOccurrence },
  {
    key: 'Alt-ArrowLeft',
    mac: 'Ctrl-ArrowLeft',
    run: CM.cursorSyntaxLeft,
    shift: CM.selectSyntaxLeft,
  },
  {
    key: 'Alt-ArrowRight',
    mac: 'Ctrl-ArrowRight',
    run: CM.cursorSyntaxRight,
    shift: CM.selectSyntaxRight,
  },
  {
    key: 'Ctrl-ArrowLeft',
    mac: 'Alt-ArrowLeft',
    run: CM.cursorGroupLeft,
    shift: CM.selectGroupLeft,
  },
  {
    key: 'Ctrl-ArrowRight',
    mac: 'Alt-ArrowRight',
    run: CM.cursorGroupRight,
    shift: CM.selectGroupRight,
  },
  ...CM.standardKeymap,
  ...CM.historyKeymap,
]);
function themeIsDark() {
  ...
  return setting === 'systemPreferred'
...
export function baseConfiguration(text) {
  return [
    ...
    baseKeymap,
    CM.EditorView.clickAddsSelectionRange.of(
      (mouseEvent) => mouseEvent.altKey || mouseEvent.ctrlKey,
    ),
    tabMovesFocus.instance(),
    bracketMatching.instance(),
    indentUnit.instance(),
    CM.Prec.lowest(
      CM.EditorView.contentAttributes.of({ 'aria-label': i18nString(UIStrings.codeEditor) }),
    ),
...
export const showCompletionHint = CM.ViewPlugin.fromClass(
  class {
    ...
    update(update) {
      ...
      if (!top || update.state.field(conservativeCompletion, false)) {
        ...
        this.decorations = CM.Decoration.set([
          CM.Decoration.widget({ widget: new CompletionHint(top), side: 1 }).range(
    ...
    class {
      ...
      update(update) {
        ...
        if (selectedCompletion) {
          ...
          const overlapsWithSelectedCompletion = (lineAtAiSuggestion + overlappingText).endsWith(
        ...
        this.decorations = CM.Decoration.set([
          CM.Decoration.widget({ widget: new CompletionHint(ghostText), side: 1 }).range(head),
        ]);
      ...
      #registerImpressionIfNeeded(activeSuggestion) {
        ...
        if (
          this.#lastLoggedSuggestion?.rpcGlobalId === activeSuggestion?.rpcGlobalId &&
        ...
        activeSuggestion.onImpression(
          activeSuggestion.rpcGlobalId,
          latency,
          activeSuggestion.sampleId,
        );
