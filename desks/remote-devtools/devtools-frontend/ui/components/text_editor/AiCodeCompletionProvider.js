...
export class AiCodeCompletionProvider {
  ...
  #aiCodeCompletion;
  #aiCodeCompletionSetting = Common.Settings.Settings.instance().createSetting(
    'ai-code-completion-enabled',
    false,
  );
  #aiCodeCompletionTeaserDismissedSetting = Common.Settings.Settings.instance().createSetting(
  ...
  constructor(aiCodeCompletionConfig) {
    ...
    if (
      AiCodeGeneration.AiCodeGeneration.AiCodeGeneration.isAiCodeGenerationEnabled(
        devtoolsLocale.locale,
      )
    ) {
      this.#aiCodeGenerationConfig = {
        generationContext: {
          ...
          additionalPreambleContext:
            this.#aiCodeCompletionConfig.generationContext.additionalPreambleContext,
        },
      ...
      this.#aiCodeGenerationProvider = AiCodeGenerationProvider.createInstance(
        this.#aiCodeGenerationConfig,
      );
    }
  }
  ...
  editorInitialized(editor) {
    ...
    if (
      ...
      !this.#aiCodeCompletionTeaserDismissedSetting.get()
    ) {
      ...
      this.#editor.editor.dispatch({
        effects: this.#teaserCompartment.reconfigure([
          aiCodeCompletionTeaserExtension(this.#teaser),
        ]),
      });
    }
    ...
    return [
      {
        ...
        run: () => {
          if (
            ...
            !this.#editor ||
            !hasActiveAiSuggestion(this.#editor.state)
          ) {
            return false;
          }
          if (
            this.#editor.state.field(aiAutoCompleteSuggestionState)?.source ===
            'generation' /* AiSuggestionSource.GENERATION */
          ) {
      ...
      },
      {
        ...
        run: () => {
          ...
          if (suggestion?.rpcGlobalId) {
            this.#aiCodeCompletion?.registerUserAcceptance(
              suggestion.rpcGlobalId,
              suggestion.sampleId,
            );
          }
  ...
  async #requestAidaSuggestion(
    ...
    try {
      ...
      const remainingDelay = Math.max(
        DELAY_BEFORE_SHOWING_RESPONSE_MS - (performance.now() - startTime),
        0,
      );
      this.#suggestionRenderingTimeout = globalThis.setTimeout(() => {
        ...
        if (this.#aiCodeCompletion) {
          this.#editor?.dispatch({
            effects: setAiAutoCompleteSuggestion.of({
              ...
              onImpression: this.#aiCodeCompletion?.registerUserImpression.bind(
                this.#aiCodeCompletion,
              ),
        ...
        if (fromCache) {
          Host.userMetrics.actionTaken(
            Host.UserMetrics.Action.AiCodeCompletionResponseServedFromCache,
          );
        }
  ...
  async #generateSampleForRequest(response, prefix, suffix) {
    ...
    const shouldBlock =
      suggestionSample.attributionMetadata?.attributionAction ===
      Host.AidaClient.RecitationAction.BLOCK;
    if (shouldBlock) {
      return null;
    }
    const isRepetitive = this.#checkIfSuggestionRepeatsExistingText(
    ...
    const suggestionText = AiCodeCompletionProvider.trimSuggestionOverlap(
      suggestionSample.generationString,
      suffix,
    );
  ...
  #pickSampleFromResponse(response) {
    ...
    return (
      response.generatedSamples.find((sample) =>
        sample.generationString.startsWith(currentHintInMenu),
      ) ?? response.generatedSamples[0]
    );
  }
  #checkIfSuggestionRepeatsExistingText(generationString, prefix, suffix) {
    return Boolean(
      prefix.includes(generationString.trim()) || suffix?.includes(generationString.trim()),
    );
  }
  /**
  ...
  return CodeMirror.ViewPlugin.fromClass(
    class {
      ...
      update(update) {
        ...
        if (
          this.#teaserMode === AiCodeCompletionTeaserMode.ONLY_SHOW_ON_EMPTY &&
          update.docChanged
        ) {
      ...
      #addTeaserWidget(pos) {
        this.#teaserDecoration = CodeMirror.Decoration.set([
          CodeMirror.Decoration.widget({
            widget: new AccessiblePlaceholder(this.teaser),
            side: 1,
          }).range(pos),
        ]);
      }
