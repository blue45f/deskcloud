...
var j = {};
m(j, { CodeBlock: () => g, languageFromToken: () => S });
import './../../kit/kit.js';
import * as y from './../../../core/i18n/i18n.js';
import * as s from './../../../third_party/codemirror.next/codemirror.next.js';
import './../buttons/buttons.js';
import * as L from './../text_editor/text_editor.js';
import * as T from './../../legacy/legacy.js';
import * as n from './../../lit/lit.js';
import * as f from './../../visual_logging/visual_logging.js';
...
var { html: d } = n,
  ...
  R = y.i18n.registerUIStrings('ui/components/markdown_view/CodeBlock.ts', c),
  p = y.i18n.getLocalizedString.bind(void 0, R);
async function S(r) {
  switch (r) {
    case 'javascript':
    case 'js':
    case 'jsx':
      return s.javascript.javascript({ jsx: !0 });
    case 'typescript':
    case 'ts':
      return s.javascript.javascript({ typescript: !0 });
    case 'tsx':
      return s.javascript.javascript({ typescript: !0, jsx: !0 });
    case 'less':
    case 'scss':
    case 'sass':
    case 'css':
      return s.css.css();
    case 'html':
      return s.html.html({ autoCloseTags: !1, selfClosingTags: !0 });
    case 'xml':
      return (await s.xml()).xml();
    case 'cpp':
      return (await s.cpp()).cpp();
    case 'go':
      return new s.LanguageSupport(await s.go());
    case 'java':
      return (await s.java()).java();
    case 'kotlin':
      return new s.LanguageSupport(await s.kotlin());
    case 'json': {
      let e = s.javascript.javascriptLanguage.configure({ top: 'SingleExpression' });
      return new s.LanguageSupport(e);
    }
    case 'php':
      return (await s.php()).php();
    case 'python':
    case 'py':
      return (await s.python()).python();
    case 'markdown':
    case 'md':
      return (await s.markdown()).markdown();
    case 'sh':
    case 'bash':
      return new s.LanguageSupport(await s.shell());
    case 'dart':
      return new s.LanguageSupport(await s.dart());
    case 'angular':
      return (await s.angular()).angular();
    case 'svelte':
      return (await s.svelte()).svelte();
    case 'vue':
      return (await s.vue()).vue();
    default:
      return s.html.html({ autoCloseTags: !1, selfClosingTags: !0 });
  }
}
var g = class extends HTMLElement {
  ...
  set code(e) {
    ((this.#t = e),
      (this.#a = s.EditorState.create({
        doc: this.#t,
        extensions: [
          L.Config.baseConfiguration(this.#t),
          s.EditorState.readOnly.of(!0),
          s.EditorView.lineWrapping,
          this.#l.of(s.javascript.javascript()),
          this.#d.of([]),
        ],
      })),
      this.#r());
  ...
  #g() {
    (T.UIUtils.copyTextToClipboard(this.#t, p(c.copied)),
      (this.#n = !0),
      this.#r(),
      clearTimeout(this.#o),
      (this.#o = setTimeout(() => {
  ...
  #w() {
    return d`
      <div class="copy-button-container">
        <devtools-button
          .data=${{ variant: 'icon', size: 'SMALL', jslogContext: 'copy', iconName: 'copy', title: p(c.copy) }}
          @click=${this.#g}
        ></devtools-button>
        ${this.#n ? d`<span>${p(c.copied)}</span>` : n.nothing}
      </div>`;
  }
  #v() {
    return this.#p.length
      ...
      ${this.#p.map(
        ...
        <button
          class="citation"
          jslog=${f.link('inline-citation').track({ click: !0 })}
  ...
  async #r() {
    let e = (this.#m ?? this.#s) || p(c.code);
    if (!this.#a)
      throw new Error('Unexpected: trying to render the text editor without editorState');
    let t = this.#a.doc.lines,
      i = t > this.#c;
    n.render(
      d`<div class='codeblock' jslog=${f.section('code')}>
      <style>${M}</style>
        ...
        ${
          ...
          <div class="show-all-container">
            <devtools-button
              ...
              .title=${p(c.showAllLines, { PH1: t - this.#c })}
              @click=${() => {
                this.displayLimit = Number.MAX_VALUE;
              }}
            >${p(c.showAllLines, { PH1: t - this.#c })}</devtools-button>
          </div>
        `
            : n.nothing
        }
    ...
    let a = this.#e?.querySelector('devtools-text-editor')?.editor;
    if (!a) return;
    let q = await S(this.#s),
      $ = [];
    (i &&
      ($ = s.EditorView.decorations.of(
        s.Decoration.set(
          s.Decoration.replace({}).range(this.#a.doc.line(this.#c).to, this.#a.doc.length),
        ),
      )),
      a.dispatch({ effects: [this.#l.reconfigure(q), this.#d.reconfigure($)] }));
  }
};
customElements.define('devtools-code-block', g);
var I = {};
m(I, { MarkdownImage: () => w });
import './../../kit/kit.js';
import * as h from './../../lit/lit.js';
var z = `.markdown-image{display:block}
/*# sourceURL=${import.meta.resolve('./markdownImage.css')} */`;
var A = {};
m(A, { getMarkdownImage: () => b, markdownImages: () => E });
var E = new Map([]),
  b = (r) => {
    let e = E.get(r);
    if (!e)
      throw new Error(
        `Markdown image with key '${r}' is not available, please check MarkdownImagesMap.ts`,
      );
    return e;
  };
var {
    html: x,
    Directives: { ifDefined: V },
  } = h,
  w = class extends HTMLElement {
    ...
    set data(e) {
      let { key: t, title: i } = e,
        a = b(t);
      ((this.#t = a), (this.#s = i), this.#n());
    }
    #i() {
      if (!this.#t) return h.nothing;
      let { src: e, color: t, width: i = '100%', height: a = '100%' } = this.#t;
      return x`
      <devtools-icon .data=${{ iconPath: e, color: t, width: i, height: a }}></devtools-icon>
    `;
    }
    #o() {
      if (!this.#t) return h.nothing;
      let { src: e, width: t = '100%', height: i = '100%' } = this.#t;
      return x`
      <img class="markdown-image" src=${e} alt=${V(this.#s)} width=${t} height=${i} />
    `;
    }
    #n() {
      if (!this.#t) return;
...
customElements.define('devtools-markdown-image', w);
var P = {};
m(P, { getMarkdownLink: () => C, markdownLinks: () => F });
var F = new Map([
    ...
    [
      'signatureHeader',
      'https://www.rfc-editor.org/rfc/rfc9421.html#name-the-signature-http-field',
    ],
    [
      'signatureInputHeader',
      'https://www.rfc-editor.org/rfc/rfc9421.html#name-the-signature-input-http-fi',
    ],
    [
      'signatureParameters',
    ...
    [
      'componentParameterSf',
      'https://www.rfc-editor.org/rfc/rfc9421.html#name-strict-serialization-of-htt',
    ],
    [
      'componentParameterReq',
      'https://www.rfc-editor.org/rfc/rfc9421.html#content-request-response',
    ],
    [
    ...
    [
      'storagePartitioningExplainer',
      'https://developers.google.com/privacy-sandbox/cookies/storage-partitioning',
    ],
    [
      'storageAccessAPI',
      'https://developer.mozilla.org/en-US/docs/Web/API/StorageAccessHandle/createObjectURL',
    ],
  ...
  C = (r) => {
    if (
      /^https:\/\/www\.chromestatus\.com\//.test(r) ||
      /^https:\/\/developer\.chrome\.com\//.test(r) ||
      /^https:\/\/developers\.google\.com\//.test(r) ||
      /^https:\/\/web\.dev\//.test(r) ||
      /^https:\/\/developer\.mozilla\.org\//.test(r) ||
      r === 'https://philipwalton.com/articles/the-state-of-es5-on-the-web/'
    )
      return r;
    let e = F.get(r);
    if (!e)
      throw new Error(
        `Markdown link with key '${r}' is not available, please check MarkdownLinksMap.ts`,
      );
    return e;
  };
var _ = {};
m(_, { MarkdownInsightRenderer: () => k, MarkdownLitRenderer: () => u, MarkdownView: () => v });
import './../../kit/kit.js';
import * as l from './../../lit/lit.js';
import * as H from './../../visual_logging/visual_logging.js';
...
var o = l.html,
  ...
  v = class extends HTMLElement {
    ...
    set data(e) {
      ((this.#t = e.tokens),
        e.renderer && (this.#s = e.renderer),
        e.animationEnabled
          ? ((this.#i = !0),
            this.#s.addCustomClasses({
              paragraph: 'pending',
              heading: 'pending',
              list_item: 'pending',
              code: 'pending',
    ...
    #n() {
      let e = this.#e.querySelectorAll('.animating');
      for (let i of e) i.classList.remove('animating');
      let t = this.#e.querySelectorAll('.pending');
      for (let i of t) i.classList.remove('pending');
      ((this.#o = !1),
        (this.#i = !1),
        this.#s.removeCustomClasses({
          paragraph: 'pending',
          heading: 'pending',
          list_item: 'pending',
          code: 'pending',
    ...
    #a() {
      ...
      this.#o = !0;
      let e = () => {
        let t = this.#e.querySelector('.pending');
        if (!t) {
          this.#o = !1;
          return;
        }
        (t.addEventListener(
          'animationend',
          () => {
            (t.classList.remove('animating'), e());
          },
          { once: !0 },
        ),
...
customElements.define('devtools-markdown-view', v);
var u = class {
    #e = {};
    addCustomClasses(e) {
      for (let [t, i] of Object.entries(e))
        (this.#e[t] || (this.#e[t] = new Set()), this.#e[t].add(i));
    }
    removeCustomClasses(e) {
      for (let [t, i] of Object.entries(e)) this.#e[t] && this.#e[t].delete(i);
    }
    customClassMapForToken(e) {
      let t = this.#e[e] || new Set(),
        i = Object.fromEntries([...t].map((a) => [a, !0]));
      return l.Directives.classMap(i);
    }
    renderChildTokens(e) {
      if ('tokens' in e && e.tokens) return e.tokens.map((t) => this.renderToken(t));
    ...
    unescape(e) {
      ...
      return e.replace(/&(amp|lt|gt|quot|#39);/g, (i) => {
        let a = t.get(i);
        return a || i;
      });
    }
    renderText(e) {
      return 'tokens' in e && e.tokens
        ? o`${this.renderChildTokens(e)}`
        : o`${this.unescape('text' in e ? e.text : '')}`;
    }
    renderHeading(e) {
      let t = this.customClassMapForToken('heading');
      switch (e.depth) {
        case 1:
          return o`<h1 class=${t}>${this.renderText(e)}</h1>`;
        case 2:
          return o`<h2 class=${t}>${this.renderText(e)}</h2>`;
        case 3:
          return o`<h3 class=${t}>${this.renderText(e)}</h3>`;
        case 4:
          return o`<h4 class=${t}>${this.renderText(e)}</h4>`;
        case 5:
          return o`<h5 class=${t}>${this.renderText(e)}</h5>`;
        default:
          return o`<h6 class=${t}>${this.renderText(e)}</h6>`;
      }
    }
    renderCodeBlock(e) {
      return o`<devtools-code-block
    ...
    templateForToken(e) {
      switch (e.type) {
        case 'paragraph':
          return o`<p class=${this.customClassMapForToken('paragraph')}>${this.renderChildTokens(e)}</p>`;
        case 'list':
          return o`<ul class=${this.customClassMapForToken('list')}>${e.items.map((t) => this.renderToken(t))}</ul>`;
        case 'list_item':
          return o`<li class=${this.customClassMapForToken('list_item')}>${this.renderChildTokens(e)}</li>`;
        case 'text':
          return this.renderText(e);
        case 'codespan':
          return o`<code class=${this.customClassMapForToken('codespan')}>${this.unescape(e.text)}</code>`;
        case 'code':
          return this.renderCodeBlock(e);
        case 'space':
          return l.nothing;
        case 'link':
          return o`<devtools-link
        class=${this.customClassMapForToken('link')}
        href=${C(e.href)}
        >${e.text}</devtools-link>`;
        case 'image':
          return o`<devtools-markdown-image
        class=${this.customClassMapForToken('image')}
        .data=${{ key: e.href, title: e.text }}></devtools-markdown-image>`;
        case 'heading':
          return this.renderHeading(e);
        case 'strong':
          return o`<strong class=${this.customClassMapForToken('strong')}>${this.renderText(e)}</strong>`;
        case 'em':
          return o`<em class=${this.customClassMapForToken('em')}>${this.renderText(e)}</em>`;
        default:
          return null;
      }
    }
    renderToken(e) {
      let t = this.templateForToken(e);
      if (t === null) throw new Error(`Markdown token type '${e.type}' not supported.`);
      return t;
    }
  },
  k = class extends u {
    #e;
    constructor(e) {
      (super(), (this.#e = e || (() => {})), this.addCustomClasses({ heading: 'insight' }));
    }
    renderToken(e) {
      let t = this.templateForToken(e);
      return t === null ? o`${e.raw}` : t;
    }
    sanitizeUrl(e) {
      try {
        let t = new URL(e);
    ...
    detectCodeLanguage(e) {
      return e.lang
        ? e.lang
        : /^(\.|#)?[\w:\[\]="'-\.]+ ?{/m.test(e.text) || /^@import/.test(e.text)
          ? 'css'
          : /^(var|const|let|function|async|import)\s/.test(e.text)
            ? 'js'
            : '';
    }
    templateForToken(e) {
      switch (e.type) {
        ...
        case 'link':
        case 'image':
          return this.sanitizeUrl(e.href) ? o`${e.text ?? e.href}` : null;
        case 'code':
          return o`<devtools-code-block
          class=${this.customClassMapForToken('code')}
          .code=${this.unescape(e.text)}
        ...
        case 'citation':
          return o`<sup><button
            class="citation"
            jslog=${H.link('inline-citation').track({ click: !0 })}
            @click=${this.#e.bind(this, Number(e.linkText))}
          >[${e.linkText}]</button></sup>`;
      }
...
export {
  j as CodeBlock,
  I as MarkdownImage,
  A as MarkdownImagesMap,
  P as MarkdownLinksMap,
  _ as MarkdownView,
};
