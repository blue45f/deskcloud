...
export async function languageFromToken(lang) {
  switch (lang) {
    ...
    case 'json': {
      const jsonLanguage = CodeMirror.javascript.javascriptLanguage.configure({
...
export class CodeBlock extends HTMLElement {
  ...
  #renderCopyButton() {
    // clang-format off
    return html` <div class="copy-button-container">
      <devtools-button
        .data=${{
  ...
  async #render() {
            ...
            ? html`
                <div class="show-all-container">
                  <devtools-button
                    ...
                    .title=${i18nString(UIStrings.showAllLines, {
                      PH1: linesCount - this.#displayLimit,
                    })}
                    @click=${() => {
                      this.displayLimit = Number.MAX_VALUE;
                    }}
                    >${i18nString(UIStrings.showAllLines, {
                      PH1: linesCount - this.#displayLimit,
                    })}</devtools-button
                  >
                </div>
    ...
    if (isTruncated) {
      truncationExtension = CodeMirror.EditorView.decorations.of(
