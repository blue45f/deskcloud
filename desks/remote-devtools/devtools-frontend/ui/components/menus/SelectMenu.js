...
export class SelectMenuButton extends HTMLElement {
      ...
      switch (this.arrowDirection) {
        ...
        default:
          Platform.assertNever(
            this.arrowDirection,
            `Unknown position type: ${this.arrowDirection}`,
          );
      }
    });
  }
  ...
  #handleButtonKeyDown(evt) {
    const key = evt.key;
    const shouldShowDialogBelow =
      this.arrowDirection === 'bottom' /* Dialogs.Dialog.DialogVerticalPosition.BOTTOM */ &&
      key === 'ArrowDown'; /* Platform.KeyboardUtilities.ArrowKey.DOWN */
    const shouldShowDialogAbove =
      this.arrowDirection === 'top' /* Dialogs.Dialog.DialogVerticalPosition.TOP */ &&
  ...
  async #render() {
    ...
    // clang-format off
    const buttonTitle = html` <span id="button-label-wrapper">
