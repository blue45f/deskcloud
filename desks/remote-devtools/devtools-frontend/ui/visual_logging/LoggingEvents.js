...
export const logResize = (loggable, size) => {
  ...
  processEventForDebugging('Resize', loggingState, {
...
export const logClick = (throttler) => (loggable, event, options) => {
  ...
  void throttler.schedule(async () => {
    ...
    processEventForDebugging('Click', loggingState, {
      mouseButton: clickEvent.mouseButton,
      doubleClick: clickEvent.doubleClick,
    });
...
export const logKeyDown = (throttler) => async (loggable, event, context) => {
  ...
  const codes =
    typeof loggingState?.config.track?.keydown === 'string'
      ? loggingState.config.track.keydown
      : '';
  if (
    codes.length &&
    !codes.split('|').includes(event.code) &&
    !codes.split('|').includes(event.key)
  ) {
    return;
  }
