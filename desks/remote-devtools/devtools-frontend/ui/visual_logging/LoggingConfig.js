...
export function makeConfigStringBuilder(veName, context) {
  ...
  return {
    ...
    track: function (options) {
      components.push(
        `track: ${Object.entries(options)
          .map(([key, value]) => (value !== true ? `${key}: ${value}` : key))
          .join(', ')}`,
      );
      return this;
    },
