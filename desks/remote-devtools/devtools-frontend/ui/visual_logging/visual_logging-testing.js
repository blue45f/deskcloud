...
S(et, {
  debugString: () => Xe,
  expectVeEvents: () => Ve,
  processEventForAdHocAnalysisDebugging: () => We,
  processEventForDebugging: () => w,
  processEventForIntuitiveDebugging: () => Ue,
  processEventForTestDebugging: () => Ke,
  processForDebugging: () => K,
  processImpressionsForDebugging: () => de,
  processStartLoggingForDebugging: () => ge,
  setHighlightedVe: () => Tt,
  setVeDebugLoggingEnabled: () => Ye,
  setVeDebuggingEnabled: () => _e,
});
...
S(Ae, {
  VisualElements: () => b,
  getLoggingConfig: () => re,
  makeConfigStringBuilder: () => St,
  needsLogging: () => q,
  parseJsLog: () => Ee,
});
import * as Fe from './../../core/host/host.js';
...
var qe = 'jslog';
function q(e) {
  return e.hasAttribute(qe);
}
function re(e) {
  return Ee(e.getAttribute(qe) || '');
}
var b;
(function (e) {
...
var Ie = new Set();
function oe(e) {
  if (!(typeof e != 'string' || !e.length || De.has(e) || Ie.has(e))) {
    if (
      Re.Runtime.Runtime.queryParam('debugFrontend') ||
      Fe.InspectorFrontendHost.isUnderTest() ||
...
function Ee(e) {
  let t = e.replace(/ /g, '').split(';'),
    i = (l) => t.find((d) => d.startsWith(l))?.substr(l.length),
    o = zt(t[0]);
  if (o === 0) throw new Error('Unkown VE: ' + e);
  let r = { ve: o },
    n = i('context:');
  n?.trim().length && (oe(n), (r.context = n));
  let s = i('parent:');
  s && (r.parent = s);
  let a = i('track:');
  if (a) {
    ...
    for (let l of a.split(','))
      l.startsWith('keydown:') ? (r.track.keydown = l.substr(8)) : (r.track[l] = !0);
  }
  return r;
}
function St(e, t) {
  let i = [e];
  return (
    typeof t == 'string' && t.trim().length && (i.push(`context: ${t}`), oe(t)),
    {
      context: function (o) {
        return (
...
S(He, {
  getLoggingState: () => g,
  getOrCreateLoggingState: () => _,
  registerParentProvider: () => Oe,
  setMappedParent: () => jt,
});
var $ = new WeakMap();
function Ct() {
...
function _(e, t, i) {
  if (t.parent && B.has(t.parent) && e instanceof Element)
    for (i = B.get(t.parent)?.(e); i instanceof Element && !q(i); )
      i = i.parentElementOrShadowHost() ?? void 0;
  if ($.has(e)) {
    let r = $.get(e);
    return (i && r.parent !== g(i) && (r.parent = g(i)), r);
  }
  let o = {
    impressionLogged: !1,
    processed: !1,
...
var B = new Map();
function Oe(e, t) {
  if (B.has(e)) throw new Error(`Parent provider with the name '${e} is already registered'`);
  B.set(e, t);
}
var Me = Symbol('veParent');
Oe('mapped', (e) => e[Me]);
function jt(e, t) {
  e[Me] = t;
}
...
function Be() {
  m ||
    ...
    c.classList.add('ve-debug'),
    (c.style.position = 'absolute'),
    (c.style.background = 'var(--sys-color-cdt-base-container)'),
    (c.style.borderRadius = '2px'),
    (c.style.padding = '8px'),
    (c.style.boxShadow = 'var(--drop-shadow)'),
    m.appendChild(c));
}
function _e(e, t) {
  ((A = e), e && Be(), ($e = t), e || W(null));
}
globalThis.setVeDebuggingEnabled = _e;
var ae = null;
function Tt(e) {
  (Be(), (ae = e), W(null));
}
function Lt(e, t) {
  t = t.trim();
  let i = g(e),
    o = i?.config?.ve ? b[i?.config?.ve] : null;
  for (; i && t; ) {
    let r = Je(i.config);
...
function K(e) {
  ae && e instanceof HTMLElement && Lt(e, ae);
  let t = g(e);
  !A || !t || t.processedForDebugging || (e instanceof HTMLElement && It(e, t));
}
function Dt(e, t) {
  if (c && ((c.style.display = 'block'), (c.textContent = e), t)) {
    let i = c.getBoundingClientRect();
    (globalThis.innerHeight < t.bottom + i.height + 8
      ? (c.style.top = `${t.top - i.height - 8}px`)
      : (c.style.top = `${t.bottom + 8}px`),
      globalThis.innerWidth < t.left + i.width
        ? ((c.style.right = '0px'), (c.style.left = ''))
        : ((c.style.right = ''), (c.style.left = `${t.left}px`)));
  }
}
function W(e, t = !1) {
  if (
...
function It(e, t) {
  e.tagName === 'OPTION'
    ? t.parent?.selectOpen &&
      c &&
      ((c.innerHTML += '<br>' + Xe(t.config)), (t.processedForDebugging = !0))
    : (e.addEventListener(
        'mousedown',
        (i) => {
          D.length && c && A && (i.stopImmediatePropagation(), i.preventDefault());
        },
        { capture: !0 },
      ),
      e.addEventListener(
        'click',
        (i) => {
          D.includes(i.currentTarget) &&
            c &&
            A &&
            ($e?.(c.textContent || ''), i.stopImmediatePropagation(), i.preventDefault());
        },
        { capture: !0 },
      ),
      e.addEventListener(
        ...
        () => {
          ...
          Dt(i.map((r) => Je(r.config)).join(' > '), e.getBoundingClientRect());
        },
        { capture: !0 },
      ),
      e.addEventListener(
...
function Ke(e, t, i) {
  (e !== 'SettingAccess' && e !== 'FunctionCall' && e !== 'Resize' && (E = null),
    I({ interaction: e, veid: t?.veid || 0 }),
    ue());
}
function We(e, t, i) {
  let o = t ? Ge.get(t.veid) : null;
  if (o) {
    let r = { time: Date.now() - F, type: e, ...i };
    (G(r), o.interactions.push(r));
  }
}
function G(e) {
  for (let t in e) {
...
function Ft(e) {
  ...
  for (let o of e) {
    ...
    if ((G(r), t.set(o.veid, r), !o.parent || !t.has(o.parent?.veid))) r.parent = o.parent?.veid;
    else {
  ...
  i.length === 1
    ? ((i[0].time = Date.now() - F), I(i[0]))
    : I({ event: 'Impression', children: i, time: Date.now() - F });
}
var se = new Map(),
  E = null;
function Rt(e) {
  E || ((E = { impressions: [] }), k.push(E));
  for (let t of e) {
    let i = '';
...
function qt(e) {
  for (let t of e) {
    let i = (r) => {
        ...
        return (G(n), r.parent && (n.parent = i(r.parent)), n);
      },
      o = { ...i(t), interactions: [], time: Date.now() - F };
    (Ge.set(t.veid, o), I(o));
  }
}
function Je(e) {
  return `${b[e.ve]}${e.context ? `: ${e.context}` : ''}`;
}
function Xe(e) {
  let t = [b[e.ve]];
  return (
...
function I(e) {
  ...
  t && (k.push(e), t === 'Intuitive' && console.info('VE Debug:', e));
}
function Ye(e, t = 'Intuitive') {
  e
...
function At() {
  let e = { strings: ['ve', 'context'], numerics: ['veid', 'width', 'height'], booleans: [] },
    ...
    i = (a) => a.map((l, d) => `$${d + 1} as ${l}`).join(', '),
    o = (a) => `$VeFields(${Pe(a, e)}, ${a.parent ? `STRUCT(${o(a.parent)})` : null})`,
    r = (a) => `$Interaction(${Pe(a, t)})`,
...
function Ne(e, t, i, o) {
  ...
  for (let n of t.data) {
    ...
    let s = r.children[r.children.length - 1];
    ((s?.type === 'Impression' ? s.veid : null) !== n.veid &&
      (r.children.push(...o),
      (o.length = 0),
      (s = {
        type: 'Impression',
...
function Mt() {
  let e = Ot(),
    ...
    r = [];
  for (let n of e) {
    if ((n.time > o + 1e3 && (Ne(i, t, o, r), (r.length = 0)), !('type' in n))) t.add(n);
    else if (n.type === 'Resize') {
      let s = t.get(n.veid);
      if (!s) continue;
      let a = t.getArea(s);
      ((s.width = n.width), (s.height = n.height), t.getArea(s) !== 0 && a !== 0 && r.push(n));
    } else r.push(n);
    o = n.time;
  }
  return (Ne(i, t, o, r), Ze(i), i);
}
var F = Date.now();
function ge() {
  ((F = Date.now()),
    localStorage.getItem('veDebugLoggingEnabled') === 'Intuitive' && I({ event: 'SessionStart' }));
}
function Ht(e, t) {
...
function ce(e) {
  return e.map((t) => ('interaction' in t ? Qe(t) : Pt(t.impressions))).join(`
`);
}
async function Ve(e) {
  if (p)
  ...
  ((p = { expectedEvents: e, success: i, fail: o, unmatchedEvents: [] }), ue());
  let r = setTimeout(() => {
    if (p?.missingEvents?.length) {
      let n = k.filter((s) => ('interaction' in s ? s.interaction !== 'SettingAccess' : !0));
      p.fail(
        new Error(`
Missing VE Events:
${ce(p.missingEvents)}
Unmatched VE Events:
${ce(p.unmatchedEvents)}
All events:
...
function $t(e, t, i, o) {
  ...
  ('impressions' in r &&
    ...
    (r.impressions = r.impressions.filter((n) => {
      let s = i.impressions.includes(n);
      return (s && o.add(n), !s);
    })),
...
function ue() {
  ...
  let e = k,
    ...
    o = new Set();
  p.unmatchedEvents = [];
  for (let r = 0; r < p.expectedEvents.length; ++r) {
    let n = p.expectedEvents[r],
      s = !1;
    for (; t < e.length; ) {
      if (Ht(e[t], n)) {
        ((s = !0), (i = !0), t++);
    ...
    if (!s) {
      (Bt(p, r, o), p.missingEvents?.length || ((U = t), p.success()));
      return;
    }
  }
  ((U = t), p.success());
...
function _t() {
  return (console.error(U), ce(k.slice(U)));
}
globalThis.setVeDebugLoggingEnabled = Ye;
globalThis.getUnmatchedVeEvents = _t;
globalThis.veDebugEventsLog = k;
globalThis.findVeDebugImpression = Et;
globalThis.exportAdHocAnalysisLogForSql = At;
globalThis.buildStateFlow = Mt;
globalThis.expectVeEvents = Ve;
var tt = {};
S(tt, { getDomState: () => pe, visibleOverlap: () => J });
function pe(e) {
  let t = [],
    i = [],
    o = [],
    r = (a, l) => {
      for (let d of a) o.push({ element: d, parent: l });
    };
  ...
  for (;;) {
    ...
    if (!a) break;
    let { element: l } = a;
    if (l.localName === 'template') continue;
    let { parent: d } = a;
    (q(l) && (t.push({ element: l, parent: d }), (d = l)),
      l.localName === 'slot' && l.assignedElements().length
        ? r(l.assignedElements(), d)
        : l.shadowRoot
          ? (i.push(l.shadowRoot), r(l.shadowRoot.children, d))
          : r(l.children, d));
  }
...
function J(e, t) {
  let i = e.getBoundingClientRect(),
    o = Kt(t, i),
    r = Math.max(Math.min(Ut, i.width, i.height), 1);
...
S(yt, {
  ...
  clickLogThrottler: () => T,
  isLogging: () => ii,
  keyboardLogThrottler: () => R,
  process: () => Le,
  resizeLogThrottler: () => P,
  scheduleProcessing: () => x,
  startLogging: () => oi,
  stopLogging: () => ri,
});
import * as C from './../../core/common/common.js';
import * as H from './../../core/host/host.js';
import * as lt from './../components/render_coordinator/render_coordinator.js';
var it = {};
S(it, {
  contextAsNumber: () => y,
  logChange: () => we,
...
import { assertNotNullOrUndefined as Y } from './../../core/platform/platform.js';
async function me(e) {
  let t = await Promise.all(
    e.map(async (i) => {
      ...
      return (
        typeof o.config.context < 'u' && (r.context = await y(o.config.context)),
...
var he = (e, t) => {
    ...
    (v.InspectorFrontendHost.InspectorFrontendHostInstance.recordResize(o),
      w('Resize', i, { width: Math.round(t.width), height: Math.round(t.height) }));
  },
  O = (e) => (t, i, o) => {
    let r = g(t);
    if (!r) return;
    ...
    (i instanceof MouseEvent &&
      ...
      e.schedule(async () => {
        (v.InspectorFrontendHost.InspectorFrontendHostInstance.recordClick(n),
          w('Click', r, { mouseButton: n.mouseButton, doubleClick: n.doubleClick }));
      }, 'Delayed'));
  },
  fe = (e) => async (t) => {
    ...
    let o = { veid: i.veid };
    e.schedule(async () => {
      (v.InspectorFrontendHost.InspectorFrontendHostInstance.recordHover(o), w('Hover', i));
    }, 'Delayed');
  },
  be = (e) => async (t) => {
    let i = g(t.currentTarget);
    ...
    let o = { veid: i.veid };
    e.schedule(async () => {
      (v.InspectorFrontendHost.InspectorFrontendHostInstance.recordDrag(o), w('Drag', i));
    }, 'Delayed');
  };
async function we(e) {
  let t = g(e);
...
var X = null,
  ve = (e) => async (t, i, o) => {
    ...
    (!o && n?.length && (o = Wt(i)),
      ...
      (X = o || null),
      e.schedule(async () => {
        (o && (s.context = await y(o)),
          v.InspectorFrontendHost.InspectorFrontendHostInstance.recordKeyDown(s),
          w('KeyDown', r, { context: o }),
          (X = null));
      }));
  };
function Wt(e) {
...
async function Gt(e, t) {
  ...
  let r = await y(e);
  if (!r) return;
  let n = { name: r, numeric_value: i, string_value: await y(o) };
  (v.InspectorFrontendHost.InspectorFrontendHostInstance.recordSettingAccess(n),
    w('SettingAccess', null, { name: e, numericValue: i, stringValue: o }));
}
async function Jt(e, t) {
  let i = await y(e);
  if (typeof i > 'u') return;
  let o = { name: i, context: await y(t) };
  (v.InspectorFrontendHost.InspectorFrontendHostInstance.recordFunctionCall(o),
    w('FunctionCall', null, { name: e, context: t }));
}
var rt = {};
S(rt, {
  getNonDomLoggables: () => ke,
  hasNonDomLoggables: () => Z,
  registerLoggable: () => Xt,
  unregisterAllLoggables: () => xe,
  unregisterLoggables: () => ye,
});
var M = new WeakMap();
function ot(e) {
  return M.get(e || Q) || [];
...
function Xt(e, t, i, o) {
  let r = ot(i);
  (r.push({ loggable: e, config: t, parent: i, size: o }), M.set(i || Q, r));
}
function Z(e) {
  return M.has(e || Q);
}
...
var Yt = 500,
  ...
  T = L,
  P = L,
  ct = new MutationObserver(x),
  dt = new ResizeObserver(kt),
  gt = new IntersectionObserver(kt),
  te = [],
  j = new Map(),
  ie = new Set();
function ut(e) {
  for (let t of e)
    (ct.observe(t, { attributes: !0, childList: !0, subtree: !0 }),
      t.querySelectorAll('[popover]')?.forEach((i) => i.addEventListener('toggle', x)));
}
var Te = !1;
function ii() {
  return Te;
...
async function oi(e) {
  ((Te = !0),
    (ee = e?.processingThrottler || new C.Throttler.Throttler(Yt)),
    (R = e?.keyboardLogThrottler || new C.Throttler.Throttler(Zt)),
    (Se = e?.hoverLogThrottler || new C.Throttler.Throttler(Qt)),
    (je = e?.dragLogThrottler || new C.Throttler.Throttler(Vt)),
    (T = e?.clickLogThrottler || new C.Throttler.Throttler(ei)),
    (P = e?.resizeLogThrottler || new C.Throttler.Throttler(ti)),
    ge(),
    await pt(document));
}
async function pt(e) {
  (te.push(e),
    ['interactive', 'complete'].includes(e.readyState) && (await Le()),
    e.addEventListener('visibilitychange', x),
    e.addEventListener('scroll', x),
    ut([e.body]));
}
async function ri() {
  (await R.schedule(async () => {}, 'AsSoonAsPossible'), (Te = !1), xe());
  for (let e of te)
    (e.removeEventListener('visibilitychange', x), e.removeEventListener('scroll', x));
  (ct.disconnect(),
    dt.disconnect(),
    gt.disconnect(),
    (te.length = 0),
...
async function ni() {
  for (; P.process; ) await P.processCompleted;
}
async function mt() {
  for (; T.process; ) await T.processCompleted;
  for (; R.process; ) await R.processCompleted;
}
function ht() {
  for (let e of ie) V(e);
}
function x() {
  ee && ee.schedule(() => lt.read('processForLogging', Le));
}
var Ce = new Map(),
  ft = (e) => {
...
async function Le() {
  ...
  let e = performance.now(),
    { loggables: t, shadowRoots: i } = pe(te),
    o = [];
  ut(i);
  let r = [void 0];
  for (let { element: n, parent: s } of t) {
    let a = _(n, re(n), s);
    if (!a.impressionLogged) {
      let l = J(n, ft(n)),
        d = n.tagName === 'OPTION' && a.parent?.selectOpen;
      ((l && n.checkVisibility({ checkVisibilityCSS: !0 }) && (!s || a.parent?.impressionLogged)) ||
        d) &&
        (l && (a.size = l), o.push(n), (a.impressionLogged = !0));
    }
    if ((a.impressionLogged && Z(n) && r.push(n), !a.processed)) {
      let l = (h) => (u) => {
        let f = u.currentTarget;
        (ze(u), O(T)(f, u, { doubleClick: h }));
      };
      if (
        (a.config.track?.click &&
          (n.addEventListener('click', l(!1), { capture: !0 }),
          n.addEventListener('auxclick', l(!1), { capture: !0 }),
          n.addEventListener('contextmenu', l(!1), { capture: !0 })),
        a.config.track?.dblclick && n.addEventListener('dblclick', l(!0), { capture: !0 }),
        a.config.track?.hover &&
          (n.addEventListener('mouseover', fe(Se), { capture: !0 }),
          n.addEventListener('mouseout', () => Se.schedule(bt, 'AsSoonAsPossible'), {
            capture: !0,
          })),
        a.config.track?.drag &&
          (n.addEventListener('pointerdown', ai, { capture: !0 }),
          document.addEventListener('pointerup', ze, { capture: !0 }),
          document.addEventListener('dragend', ze, { capture: !0 })),
        a.config.track?.change &&
          (n.addEventListener(
            'input',
            (h) => {
              h instanceof InputEvent &&
                (a.pendingChangeContext && a.pendingChangeContext !== h.inputType && V(n),
                (a.pendingChangeContext = h.inputType),
                ie.add(n));
            },
            { capture: !0 },
          ),
          n.addEventListener(
            ...
            (h) => {
              let u = h?.target ?? n;
              (['checkbox', 'radio'].includes(u.type) &&
                (a.pendingChangeContext = u.checked ? 'on' : 'off'),
                V(n));
            },
            { capture: !0 },
          ),
          n.addEventListener(
            'focusout',
            () => {
              a.pendingChangeContext && V(n);
            },
            { capture: !0 },
          )),
        a.config.track?.keydown &&
          n.addEventListener('keydown', (h) => ve(R)(h.currentTarget, h), { capture: !0 }),
        a.config.track?.resize && (dt.observe(n), gt.observe(n)),
        n.tagName === 'SELECT')
      ) {
        let h = (u) => {
          (O(T)(n, u), !a.selectOpen && ((a.selectOpen = !0), x()));
        };
        (n.addEventListener('click', h, { capture: !0 }),
          n.addEventListener(
            'keydown',
            (u) => {
              let f = u;
              (((H.Platform.isMac() || f.altKey) &&
                (f.code === 'ArrowDown' || f.code === 'ArrowUp')) ||
                (!f.altKey && !f.ctrlKey && f.code === 'F4')) &&
                h(u);
            },
            { capture: !0 },
          ),
          n.addEventListener(
            ...
            (u) => {
              let f = u;
              (f.key === ' ' || (!H.Platform.isMac() && f.key === '\r')) && h(u);
            },
            { capture: !0 },
          ),
          n.addEventListener(
            'change',
            (u) => {
              for (let f of n.selectedOptions) g(f)?.config.track?.click && O(T)(f, u);
            },
            { capture: !0 },
          ));
      }
  ...
  for (let n = 0; n < r.length; ++n) {
    let s = r[n];
    for (let { loggable: a, config: l, parent: d, size: z } of ke(s)) {
      let N = _(a, l, d);
      N.impressionLogged ||
        (z && (N.size = z), K(a), o.push(a), (N.impressionLogged = !0), Z(a) && r.push(a));
    }
    ye(s);
  }
  (o.length && (await mt(), await ni(), ht(), await me(o)),
    H.userMetrics.visualLoggingProcessingDone(performance.now() - e));
}
function V(e) {
  let t = g(e);
  t && (we(e), delete t.pendingChangeContext, ie.delete(e));
}
async function bt() {}
var wt = 0,
  vt = 0;
function ai(e) {
  e instanceof MouseEvent && ((wt = e.screenX), (vt = e.screenY), be(je)(e));
}
function ze(e) {
  e instanceof MouseEvent &&
    (Math.abs(e.screenX - wt) >= nt ||
      Math.abs(e.screenY - vt) >= nt ||
      je.schedule(bt, 'AsSoonAsPossible'));
}
function st(e, t) {
  for (; t; ) {
    if (t === e) return !0;
...
async function kt(e) {
  for (let t of e) {
    let i = t.target,
      o = g(i),
      r = J(i, ft(i)) || new DOMRect(0, 0, 0, 0);
    if (!o?.size) continue;
    ...
    for (let [a, l] of j.entries()) {
      if (a === i) continue;
      let d = g(a),
        z = l.width * l.height * (d?.size?.width || 0) * (d?.size?.height || 0) === 0;
      if (st(d, o) && n && z) {
        s = !0;
        break;
      }
      st(o, d) && n && z && j.delete(a);
    }
    s ||
      (j.set(i, r),
      P.schedule(async () => {
        ...
        for (let [a, l] of j.entries()) {
          let d = g(a);
          d &&
            (Math.abs(l.width - d.size.width) >= at || Math.abs(l.height - d.size.height) >= at) &&
            he(a, l);
        }
        j.clear();
      }, 'Delayed'));
