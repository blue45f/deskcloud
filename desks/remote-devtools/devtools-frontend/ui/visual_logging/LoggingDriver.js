...
function observeMutations(roots) {
  for (const root of roots) {
    ...
    root
      ...
      ?.forEach((e) => e.addEventListener('toggle', scheduleProcessing));
  }
}
let logging = false;
...
export async function startLogging(options) {
  logging = true;
  processingThrottler =
    options?.processingThrottler || new Common.Throttler.Throttler(PROCESS_DOM_INTERVAL);
  keyboardLogThrottler =
    options?.keyboardLogThrottler || new Common.Throttler.Throttler(KEYBOARD_LOG_INTERVAL);
  hoverLogThrottler =
    options?.hoverLogThrottler || new Common.Throttler.Throttler(HOVER_LOG_INTERVAL);
  dragLogThrottler = options?.dragLogThrottler || new Common.Throttler.Throttler(DRAG_LOG_INTERVAL);
  clickLogThrottler =
    options?.clickLogThrottler || new Common.Throttler.Throttler(CLICK_LOG_INTERVAL);
  resizeLogThrottler =
    options?.resizeLogThrottler || new Common.Throttler.Throttler(RESIZE_LOG_INTERVAL);
...
export async function stopLogging() {
  await keyboardLogThrottler.schedule(
    async () => {},
    'AsSoonAsPossible' /* Common.Throttler.Scheduling.AS_SOON_AS_POSSIBLE */,
  );
  logging = false;
  unregisterAllLoggables();
...
const viewportRectFor = (element) => {
  ...
  const viewportRect =
    viewportRects.get(ownerDocument) ||
    new DOMRect(
      0,
      0,
      ownerDocument.defaultView?.innerWidth || 0,
      ownerDocument.defaultView?.innerHeight || 0,
    );
  ...
  for (const { element, parent } of loggables) {
    ...
    if (!loggingState.processed) {
      ...
      if (trackKeyDown) {
        element.addEventListener(
          'keydown',
          (e) => logKeyDown(keyboardLogThrottler)(e.currentTarget, e),
          { capture: true },
        );
      }
...
function maybeCancelDrag(event) {
  ...
  if (
    Math.abs(event.screenX - dragStartX) >= DRAG_REPORT_THRESHOLD ||
  ...
  for (const entry of entries) {
    ...
    for (const [pendingElement, overlap] of pendingResize.entries()) {
      ...
      const pendingResizeToOrFromZero =
        overlap.width *
          overlap.height *
          (pendingState?.size?.width || 0) *
          (pendingState?.size?.height || 0) ===
        0;
      if (
        isAncestorOf(pendingState, loggingState) &&
        resizeToOrFromZero &&
        pendingResizeToOrFromZero
      ) {
        suppressedByParentResize = true;
        break;
      }
      if (
        isAncestorOf(loggingState, pendingState) &&
        resizeToOrFromZero &&
        pendingResizeToOrFromZero
      ) {
    ...
    void resizeLogThrottler.schedule(async () => {
      ...
      for (const [element, overlap] of pendingResize.entries()) {
        ...
        if (
          Math.abs(overlap.width - loggingState.size.width) >= RESIZE_REPORT_THRESHOLD ||
