...
function processElementForDebugging(element, loggingState) {
  if (element.tagName === 'OPTION') {
    ...
    element.addEventListener(
      ...
      (event) => {
        if (
          highlightedElements.includes(event.currentTarget) &&
          debugPopover &&
          veDebuggingEnabled
        ) {
    ...
    element.addEventListener(
      ...
      () => {
        ...
        showDebugPopover(
          pathToRoot.map((s) => elementKey(s.config)).join(' > '),
...
export function debugString(config) {
  ...
  if (config.track) {
    components.push(
      `track: ${Object.entries(config.track)
...
export function setVeDebugLoggingEnabled(
  enabled,
  format = 'Intuitive' /* DebugLoggingFormat.INTUITIVE */,
) {
  if (enabled) {
...
DEFINE MACRO VeFields ${fieldsDefsForSql([...VE_FIELDS.strings, ...VE_FIELDS.numerics, 'parent'])};
...
class StateFlowElementsByArea {
  ...
  get data() {
    return [...this.#data.values()]
      .filter((e) => this.getArea(e))
      .sort((e1, e2) => this.getArea(e2) - this.getArea(e1));
  }
}
...
export function processStartLoggingForDebugging() {
  ...
  if (
    localStorage.getItem('veDebugLoggingEnabled') === 'Intuitive' /* DebugLoggingFormat.INTUITIVE */
  ) {
...
export async function expectVeEvents(expectedEvents) {
  if (pendingEventExpectation) {
    throw new Error(
      'VE events expectation already set. Cannot set another one until the previous is resolved',
    );
  }
...
function processMissingEvents(pendingExpectation, expectedEventIndex, matchedImpressions) {
  ...
  for (const event of pendingExpectation.missingEvents) {
    if ('impressions' in event) {
      event.impressions = event.impressions.filter(
        (impression) => !matchedImpressions.has(impression),
      );
    }
  }
  pendingExpectation.missingEvents = pendingExpectation.missingEvents.filter(
...
function checkPendingEventExpectation() {
  ...
  for (
    ...
    expectedEventIndex < pendingEventExpectation.expectedEvents.length;
    ++expectedEventIndex
  ) {
