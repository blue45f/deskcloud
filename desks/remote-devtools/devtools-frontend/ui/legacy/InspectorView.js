...
export class InspectorView extends VBox {
  ...
  constructor() {
    ...
    this.drawerOrientationByDockSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.drawerSplitWidget = new SplitWidget(
      isVertical,
      true,
    ...
    this.#drawerView = new InspectorDrawerView({
      ...
      enableOrientationToggle: Boolean(
        Root.Runtime.hostConfig.devToolsFlexibleLayout?.verticalDrawerEnabled,
      ),
      isVertical,
    ...
    mainHeaderElement.setAttribute(
      'jslog',
      `${VisualLogging.toolbar('main').track({
        drag: true,
    ...
    DockController.instance().addEventListener(
      'DockSideChanged' /* DockControllerEvents.DOCK_SIDE_CHANGED */,
  ...
  #getOrientationForDockMode() {
    ...
    if (orientation === DrawerOrientation.UNSET) {
      ...
      orientation =
        dockMode === DockMode.BOTTOM ? DrawerOrientation.VERTICAL : DrawerOrientation.HORIZONTAL;
    }
    return orientation;
  }
  ...
  #observedResize() {
    ...
    this.element.style.setProperty(
      ...
      `${globalThis.innerWidth - rect.right}px`,
    );
    ...
    this.element.style.setProperty(
      ...
      `${globalThis.innerHeight - rect.bottom}px`,
    );
  ...
  toggleDrawerOrientation({ force } = {}) {
    ...
    if (force) {
      ...
      newOrientation =
        currentOrientation === DrawerOrientation.VERTICAL
          ? DrawerOrientation.HORIZONTAL
          : DrawerOrientation.VERTICAL;
    }
...
function shouldShowLocaleInfobar() {
  ...
  return (
    !i18n.DevToolsLocale.localeLanguagesMatch(navigator.language, languageSettingValue) &&
...
function createLocaleInfobar() {
  ...
  const closestSupportedLanguageInCurrentLocale =
    new Intl.DisplayNames([devtoolsLocale.locale], { type: 'language' }).of(
      locale.language || 'en',
    ) || 'English';
  const languageSetting = Common.Settings.Settings.instance().moduleSetting('language');
  return new Infobar(
    'info' /* InfobarType.INFO */,
    i18nString(UIStrings.devToolsLanguageMissmatch, {
      PH1: closestSupportedLanguageInCurrentLocale,
    }),
    [
      {
      ...
      {
        text: i18nString(UIStrings.setToSpecificLanguage, {
          PH1: closestSupportedLanguageInCurrentLocale,
        }),
        delegate: () => {
...
function reloadDevTools() {
  if (
    DockController.instance().canDock() &&
    DockController.instance().dockSide() === 'undocked' /* DockState.UNDOCKED */
  ) {
    Host.InspectorFrontendHost.InspectorFrontendHostInstance.setIsDocked(true, function () {});
  }
  Host.InspectorFrontendHost.InspectorFrontendHostInstance.reattach(() =>
...
export class InspectorViewTabDelegate {
  ...
  onContextMenu(tabId, contextMenu) {
    ...
    if (locationName === 'drawer-view') {
      ...
      contextMenu
        .defaultSection()
        .appendItem(i18nString(UIStrings.moveToDrawer), this.moveToDrawer.bind(this, tabId), {
