...
export class Dialog extends Common.ObjectWrapper.eventMixin(GlassPane) {
  tabIndexBehavior =
    'DisableAllTabIndex' /* OutsideTabIndexBehavior.DISABLE_ALL_OUTSIDE_TAB_INDEX */;
  tabIndexMap = new Map();
  focusRestorer = null;
  closeOnEscape = true;
  targetDocument = null;
  ...
  disableTabIndexOnElements(document) {
    if (
      this.tabIndexBehavior === 'PreserveTabIndex' /* OutsideTabIndexBehavior.PRESERVE_TAB_INDEX */
    ) {
      return;
    }
    let exclusionSet = null;
    if (
      this.tabIndexBehavior ===
      'PreserveMainViewTabIndex' /* OutsideTabIndexBehavior.PRESERVE_MAIN_VIEW_TAB_INDEX */
    ) {
