...
export class SuggestBox {
  ...
  constructor(suggestBoxDelegate, maxItemsHeight) {
    ...
    const shadowRoot = createShadowRootWithCoreStyles(this.glassPane.contentElement, {
      cssFile: suggestBoxStyles,
    });
  ...
  maxWidth(items) {
    ...
    const preferredWidth =
      measurePreferredSize(element, this.element).width +
      measuredScrollbarWidth(this.element.ownerDocument);
  ...
  show() {
    ...
    this.rowHeight = measurePreferredSize(
      this.createElementForItem(suggestion),
      this.element,
    ).height;
  ...
  applySuggestion(isIntermediateSuggestion) {
    if (this.onlyCompletion) {
      isIntermediateSuggestion
        ? ARIAUtils.LiveAnnouncer.alert(
            i18nString(UIStrings.sSuggestionSOfS, {
        ...
        : ARIAUtils.LiveAnnouncer.alert(
            i18nString(UIStrings.sSuggestionSSelected, { PH1: this.onlyCompletion.text }),
          );
    ...
    if (suggestion?.text) {
      isIntermediateSuggestion
        ...
        : ARIAUtils.LiveAnnouncer.alert(
            i18nString(UIStrings.sSuggestionSSelected, {
              PH1: suggestion.title || suggestion.text,
            }),
          );
    }
  ...
  createElementForItem(item) {
    ...
    const displayText = Platform.StringUtilities.trimEndWithMaxLength(
    ...
    if (index > -1) {
      titleElement.createChild('span', 'query').textContent = displayText.substring(
        index,
        index + query.length,
      );
    }
    titleElement.createChild('span').textContent = displayText.substring(
      index > -1 ? index + query.length : 0,
    );
    ...
    if (item.subtitleRenderer) {
      ...
      subtitleElement.textContent = Platform.StringUtilities.trimEndWithMaxLength(
        item.subtitle,
  ...
  updateSuggestions(
    ...
    this.onlyCompletion = null;
    const highestPriorityItem = selectHighestPriority
      ? completions.reduce((a, b) => ((a.priority || 0) >= (b.priority || 0) ? a : b))
      : null;
