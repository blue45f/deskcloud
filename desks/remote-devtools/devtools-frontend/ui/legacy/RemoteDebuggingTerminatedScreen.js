...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html` <style>
        ${remoteDebuggingTerminatedScreenStyles}
      </style>
      <div class="header">${i18nString(UIStrings.debuggingConnectionWasClosed)}</div>
      <div class="content">
        <div class="reason">
          ${i18nString(UIStrings.connectionClosedReason, { PH1: input.reason })}
        </div>
      ...
      <div class="button-container">
        <div class="button">
          <devtools-button
            @click=${input.onReconnect}
            .jslogContext=${'reconnect'}
            .variant=${'outlined' /* Buttons.Button.Variant.OUTLINED */}
            >${i18nString(UIStrings.reconnectDevtools)}</devtools-button
          >
        </div>
      </div>`,
    target,
