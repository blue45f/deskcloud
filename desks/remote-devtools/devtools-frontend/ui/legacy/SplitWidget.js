...
export class SplitWidget extends Common.ObjectWrapper.eventMixin(Widget) {
  ...
  constructor(
    ...
    this.#sidebarElement = this.contentElement.createChild(
      'div',
      'shadow-split-widget-contents shadow-split-widget-sidebar vbox',
    );
    this.#mainElement = this.contentElement.createChild(
      'div',
    ...
    mainSlot.addEventListener('slotchange', (_) => {
      ...
      const widget =
        assignedNode instanceof HTMLElement ? Widget.getOrCreateWidget(assignedNode) : null;
    ...
    this.#resizerWidget.addEventListener(
      'ResizeStart' /* ResizerWidgetEvents.RESIZE_START */,
      this.#onResizeStart,
      this,
    );
    this.#resizerWidget.addEventListener(
      'ResizeUpdatePosition' /* ResizerWidgetEvents.RESIZE_UPDATE_POSITION */,
      this.#onResizeUpdate,
      this,
    );
    this.#resizerWidget.addEventListener(
      'ResizeEnd' /* ResizerWidgetEvents.RESIZE_END */,
      this.#onResizeEnd,
      this,
    );
    ...
    this.setting = settingName
      ? Common.Settings.Settings.instance().createSetting(settingName, {})
      : null;
  ...
  setMainWidget(widget) {
    ...
    if (widget) {
      ...
      if (
        this.#showMode === 'OnlyMain' /* ShowMode.ONLY_MAIN */ ||
        this.#showMode === 'Both' /* ShowMode.BOTH */
      ) {
  ...
  setSidebarWidget(widget) {
    ...
    if (widget) {
      ...
      if (
        this.#showMode === 'OnlySidebar' /* ShowMode.ONLY_SIDEBAR */ ||
        this.#showMode === 'Both' /* ShowMode.BOTH */
      ) {
        widget.show(this.element);
      }
      ...
      if (sideToShow) {
        ...
        if (sideToShow === this.#mainWidget) {
          this.#mainWidget.show(
            this.element,
            this.#sidebarWidget ? this.#sidebarWidget.element : null,
          );
  ...
  #totalSizeDIP() {
    if (!this.#totalSizeCSS) {
      this.#totalSizeCSS = this.#isVertical
        ? this.contentElement.offsetWidth
        : this.contentElement.offsetHeight;
      this.#totalSizeOtherDimensionCSS = this.#isVertical
        ? this.contentElement.offsetHeight
        : this.contentElement.offsetWidth;
    }
  ...
  #setSidebarSizeDIP(sizeDIP, animate, userAction) {
    ...
    if (!this.#resizerElementSize) {
      this.#resizerElementSize = this.#isVertical
        ? this.#resizerElement.offsetWidth
        : this.#resizerElement.offsetHeight;
    }
    ...
    if (animate) {
      ...
      this.dispatchEventToListeners(
        'SidebarSizeChanged' /* Events.SIDEBAR_SIZE_CHANGED */,
        this.sidebarSize(),
      );
    }
  }
  #animate(reverse, callback) {
    ...
    const marginFrom = reverse
      ? '0'
      : '-' + ZoomManager.instance().dipToCSS(this.#sidebarSizeDIP) + 'px';
    ...
    function animationFrame() {
      ...
      this.#animationFrameHandle = this.contentElement
        .window()
        .requestAnimationFrame(boundAnimationFrame);
    }
    this.#animationFrameHandle = this.contentElement
      .window()
      .requestAnimationFrame(boundAnimationFrame);
  ...
  #applyConstraints(sidebarSize, userAction) {
    ...
    let constraints = this.#sidebarWidget
      ? this.#sidebarWidget.constraints()
      : new Geometry.Constraints();
    ...
    let preferredSidebarSize = this.isVertical()
    ...
    let preferredMainSize = this.isVertical()
      ? constraints.preferred.width
      : constraints.preferred.height;
    ...
    const savedMainSize = this.isVertical()
      ? this.#savedVerticalMainSize
      : this.#savedHorizontalMainSize;
    ...
    if (totalPreferred <= totalSize) {
      return Platform.NumberUtilities.clamp(
        sidebarSize,
        preferredSidebarSize,
        totalSize - preferredMainSize,
      );
    }
  ...
  willHide() {
    super.willHide();
    ZoomManager.instance().removeEventListener(
      'ZoomChanged' /* ZoomManagerEvents.ZOOM_CHANGED */,
      this.onZoomChanged,
      this,
    );
  ...
  calculateConstraints() {
    ...
    let mainConstraints = this.#mainWidget
      ? this.#mainWidget.constraints()
      : new Geometry.Constraints();
    let sidebarConstraints = this.#sidebarWidget
      ? this.#sidebarWidget.constraints()
      : new Geometry.Constraints();
  ...
  #onResizeUpdate(event) {
    ...
    const newSizeDIP = this.#secondIsSidebar
      ? this.#resizeStartSizeDIP - offsetDIP
      : this.#resizeStartSizeDIP + offsetDIP;
  ...
  createShowHideSidebarButton(showTitle, hideTitle, shownString, hiddenString, jslogContext) {
    ...
    this.#showHideSidebarButton.addEventListener(
      'Click' /* ToolbarButton.Events.CLICK */,
      buttonClicked,
      this,
    );
    if (jslogContext) {
  ...
  #updateShowHideSidebarButton() {
    ...
    this.#showHideSidebarButton.setTitle(
      sidebarHidden ? this.#showSidebarButtonTitle : this.#hideSidebarButtonTitle,
    );
  }
}
