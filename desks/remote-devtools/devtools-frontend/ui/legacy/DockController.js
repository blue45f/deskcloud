...
export class DockController extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(canDock) {
    ...
    this.lastDockStateSetting = Common.Settings.Settings.instance().createSetting(
      'last-dock-state',
      'bottom' /* DockState.BOTTOM */,
    );
    if (!canDock) {
  ...
  isVertical() {
    return (
      this.#dockSide === 'right' /* DockState.RIGHT */ ||
      this.#dockSide === 'left' /* DockState.LEFT */
    );
  }
  setDockSide(dockSide) {
    ...
    this.dispatchEventToListeners(
      'BeforeDockSideChanged' /* Events.BEFORE_DOCK_SIDE_CHANGED */,
      eventData,
    );
    ...
    Host.InspectorFrontendHost.InspectorFrontendHostInstance.setIsDocked(
      dockSide !== 'undocked' /* DockState.UNDOCKED */,
  ...
  setIsDockedResponse(eventData) {
    this.dispatchEventToListeners(
      'AfterDockSideChanged' /* Events.AFTER_DOCK_SIDE_CHANGED */,
      eventData,
    );
