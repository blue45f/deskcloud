...
export class ListWidget extends VBox {
  ...
  createControls(item, element, controlLabels) {
    ...
    // clang-format off
    render(
      html` <div class="controls-gradient"></div>
  ...
  commitEditing() {
    ...
    const focusElementIndex = this.editElement
...
export class Editor {
  ...
  constructor() {
    ...
    this.cancelButton = createTextButton(
      i18nString(UIStrings.cancelString),
    ...
    this.cancelButton.setAttribute(
      'jslog',
      `${VisualLogging.action('cancel').track({ click: true })}`,
    );
  ...
  createInput(name, type, title, validator) {
    ...
    input.setAttribute(
      'jslog',
      `${VisualLogging.textField().track({ change: true, keydown: 'Enter' }).context(name)}`,
    );
  ...
  createSelect(name, options, validator, title) {
    ...
    select.setAttribute(
      'jslog',
      `${VisualLogging.dropDown().track({ change: true }).context(name)}`,
    );
  ...
  validateControls(forceValid) {
    ...
    for (let index = 0; index < this.controls.length; ++index) {
      ...
      const { valid, errorMessage } = this.validators[index].call(
        null,
        this.item,
        this.index,
        input,
