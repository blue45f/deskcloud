...
export class ListControl {
  ...
  constructor(model, delegate, mode) {
    ...
    this.model.addEventListener(
      'ItemsReplaced' /* ListModelEvents.ITEMS_REPLACED */,
      this.replacedItemsInRange,
      this,
    );
  ...
  replacedItemsInRange(event) {
    ...
    const oldSelectedElement =
      oldSelectedItem !== null ? this.itemToElement.get(oldSelectedItem) || null : null;
  ...
  invalidate(from, to, inserted) {
    ...
    if (this.mode === ListMode.VariousHeightItems) {
      ...
      for (let i = from + 1; i <= this.model.length; i++) {
        this.variableOffsets[i] =
          this.variableOffsets[i - 1] + this.delegate.heightForItem(this.model.at(i - 1));
      }
    }
    const viewportHeight = this.element.offsetHeight;
    ...
    if (this.renderedHeight < viewportHeight || totalHeight < viewportHeight) {
      this.clearViewport();
      this.updateViewport(
        Platform.NumberUtilities.clamp(scrollTop, 0, totalHeight - viewportHeight),
        viewportHeight,
      );
      return;
    }
