...
export class ViewManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(universe) {
    ...
    this.locationOverrideSetting = Common.Settings.Settings.instance().createSetting(
  ...
  async showView(viewId, userGesture, omitFocus) {
    ...
    const location =
      locationForView.get(view) ??
      (await this.resolveLocation(this.locationNameByViewId.get(viewId)));
    if (!location) {
  ...
  async resolveLocation(location) {
    ...
    const registeredResolvers = getRegisteredLocationResolvers().filter(
      (resolver) => resolver.name === location,
    );
...
export class ContainerWidget extends VBox {
  ...
  materialize() {
    ...
    promises.push(
      this.view.toolbarItems().then((toolbarItems) => {
...
class ExpandableContainerWidget extends VBox {
  ...
  constructor(view) {
    ...
    this.titleElement.setAttribute(
      'jslog',
      `${VisualLogging.sectionHeader().context(view.viewId()).track({
        click: true,
...
class TabbedLocation extends Location {
  ...
  constructor(
    ...
    this.#tabbedPane.addEventListener(
      TabbedPaneEvents.PaneVisibilityChanged,
      this.tabbedPaneVisibilityChanged,
      this,
    );
    this.closeableTabSetting = Common.Settings.Settings.instance().createSetting(
      'closeable-tabs',
      {},
    );
    ...
    this.tabOrderSetting = Common.Settings.Settings.instance().createSetting(
      location + '-tab-order',
      {},
    );
    this.#tabbedPane.addEventListener(TabbedPaneEvents.TabOrderChanged, this.persistTabOrder, this);
    if (restoreSelection) {
      this.lastSelectedTabSetting = Common.Settings.Settings.instance().createSetting(
        location + '-selected-tab',
        '',
      );
    }
    ...
    if (this.defaultTab) {
      if (this.#tabbedPane.hasTab(this.defaultTab)) {
        ...
        const view = Array.from(this.views.values()).find(
          (view) => view.viewId() === this.defaultTab,
        );
        if (view) {
      ...
      this.#tabbedPane.hasTab(this.lastSelectedTabSetting.get())
    ) {
  ...
  appendTabsToMenu(contextMenu) {
    ...
    for (const view of views) {
      ...
      contextMenu
        .defaultSection()
        .appendItem(title, this.showView.bind(this, view, undefined, true), {
          isPreviewFeature,
  ...
  isViewVisible(view) {
    const locationVisible = this.isLocationVisible
      ? this.isLocationVisible()
      : this.#tabbedPane.isShowing();
  ...
  tabbedPaneVisibilityChanged(event) {
    ...
    this.manager.dispatchEventToListeners(
      'ViewVisibilityChanged' /* Events.VIEW_VISIBILITY_CHANGED */,
      {
...
class StackLocation extends Location {
  ...
  constructor(manager, revealCallback, location, jslogContext) {
    const vbox = new VBox();
    vbox.element.setAttribute(
      'jslog',
      `${VisualLogging.pane(jslogContext || 'sidebar').track({ resize: true })}`,
    );
