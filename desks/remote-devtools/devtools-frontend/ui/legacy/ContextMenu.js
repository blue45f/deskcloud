...
export class ContextMenu extends SubMenu {
  ...
  async show() {
    ...
    const loadedProviders = await Promise.all(
      this.pendingTargets.map(async (target) => {
    ...
    if (useSoftMenu) {
      ...
      VisualLogging.registerLoggable(
        ...
        new DOMRect(
          0,
          0,
          MENU_ITEM_WIDTH_FOR_LOGGING,
          MENU_ITEM_HEIGHT_FOR_LOGGING * menuObject.length,
        ),
      );
    ...
    if (this.openHostedMenu) {
      ...
      if (item && featuresUsed.length > 0) {
        featuresUsed.map((feature) =>
          Host.InspectorFrontendHost.InspectorFrontendHostInstance.recordNewBadgeUsage(feature),
        );
      }
    }
    this.menuCleared();
...
export class MenuButton extends HTMLElement {
  ...
  set jslogContext(jslogContext) {
    this.setAttribute(
      'jslog',
      VisualLogging.dropDown(jslogContext).track({ click: true }).toString(),
    );
  }
  get jslogContext() {
...
async function loadApplicableRegisteredProviders(target) {
  ...
  for (const providerRegistration of registeredProviders) {
    if (
      !Root.Runtime.Runtime.isDescriptorEnabled({ experiment: providerRegistration.experiment })
    ) {
      continue;
    }
