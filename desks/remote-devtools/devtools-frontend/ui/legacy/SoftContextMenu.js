...
export class SoftContextMenu {
  ...
  show(document, anchorBox) {
    ...
    this.glassPane.setPointerEventsBehavior(
      this.parentMenu
        ? 'PierceGlassPane' /* PointerEventsBehavior.PIERCE_GLASS_PANE */
    ...
    this.glassPane.setAnchorBehavior(
      this.parentMenu
        ? 'PreferRight' /* AnchorBehavior.PREFER_RIGHT */
        : 'PreferBottom' /* AnchorBehavior.PREFER_BOTTOM */,
    );
    ...
    this.contextMenuElement.setAttribute(
      'jslog',
      `${VisualLogging.menu().track({ resize: true }).parent('mapped').track({
  ...
  createMenuItem(item, menuContainsCheckbox) {
    ...
    menuItemElement.createChild('span', 'soft-context-menu-shortcut').textContent =
      item.shortcut || '';
    ...
    if (item.type === 'checkbox') {
      ...
      if (item.shortcut) {
        accessibleName = i18nString(UIStrings.sSS, {
          PH1: String(item.label),
          PH2: item.shortcut,
          PH3: checkedState,
        });
      } else {
    ...
    if (item.element?.className === 'new-badge') {
      accessibleName = i18nString(UIStrings.sS, {
        PH1: String(item.label),
        PH2: i18nString(UIStrings.newFeature),
      });
    }
  ...
  createSubMenu(item, menuContainsCheckbox) {
    ...
    // If the menu contains a checkbox, add checkbox space in front of the label to align the items
    if (menuContainsCheckbox) {
      const checkMarkElement = createIcon(
        'checkmark',
        'checkmark soft-context-menu-item-checkmark',
      );
      menuItemElement.appendChild(checkMarkElement);
    }
    createTextChild(menuItemElement, item.label || '');
    ARIAUtils.setExpanded(menuItemElement, false);
    const subMenuArrowElement = createIcon(
  ...
  highlightMenuItem(menuItemElement, scheduleSubMenu) {
    ...
    if (this.highlightedMenuItemElement) {
      ...
      const detailsForElement = this.detailsForElementMap.get(this.highlightedMenuItemElement);
      if (
        detailsForElement?.customElement &&
      ...
      if (scheduleSubMenu && detailsForElement?.subItems && !detailsForElement.subMenuTimer) {
        detailsForElement.subMenuTimer = globalThis.setTimeout(
  ...
  highlightPrevious() {
    let menuItemElement = this.highlightedMenuItemElement
      ...
      : this.contextMenuElement
        ...
        : null;
    let menuItemDetails = menuItemElement
      ? this.detailsForElementMap.get(menuItemElement)
      : undefined;
    while (
      menuItemElement &&
      menuItemDetails &&
      ...
      menuItemDetails = menuItemElement
        ? this.detailsForElementMap.get(menuItemElement)
        : undefined;
    }
    if (menuItemElement) {
    ...
    switch (keyboardEvent.key) {
      ...
      case 'ArrowRight': {
        ...
        if (detailsForElement?.customElement?.classList.contains('location-menu')) {
          detailsForElement.customElement.dispatchEvent(
