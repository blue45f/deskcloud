...
export class Toolbar extends HTMLElement {
  ...
  static createLongPressActionButton(action, toggledOptions, untoggledOptions) {
    ...
    function showOptions() {
      ...
      const hostButtonPosition = button.element
        .boxInWindow()
        .relativeToElement(GlassPane.container(document));
      const topNotBottom =
        hostButtonPosition.y + buttonHeight * buttons.length <
      ...
      if (topNotBottom) {
        ...
        optionsBar.style.top =
          hostButtonPosition.y - buttonHeight * (buttons.length - 1) - 6 + 'px';
      }
  ...
  static createActionButton(actionOrActionId, options = {}) {
    const action =
      typeof actionOrActionId === 'string'
        ? ActionRegistry.instance().getAction(actionOrActionId)
        : actionOrActionId;
...
export class ToolbarItemWithCompactLayout extends ToolbarItem {
  setCompactLayout(enable) {
    this.dispatchEventToListeners(
      'CompactLayoutUpdated' /* ToolbarItemWithCompactLayoutEvents.COMPACT_LAYOUT_UPDATED */,
      enable,
    );
  }
}
...
export class ToolbarInput extends ToolbarItem {
  ...
  constructor(
    ...
    this.prompt.addEventListener(
      'TextChanged' /* TextPromptEvents.TEXT_CHANGED */,
    ...
    clearButton.setAttribute(
      'jslog',
      `${VisualLogging.action('clear').track({ click: true }).parent('mapped')}`,
    );
  ...
  onKeydownCallback(event) {
    if (event.key === 'Enter' && this.prompt.text()) {
      this.dispatchEventToListeners(
        'EnterPressed' /* ToolbarInput.Event.ENTER_PRESSED */,
        this.prompt.text(),
      );
    }
  ...
  onChangeCallback() {
    this.updateEmptyStyles();
    this.dispatchEventToListeners(
      'TextChanged' /* ToolbarInput.Event.TEXT_CHANGED */,
      this.prompt.text(),
    );
  }
  updateEmptyStyles() {
  ...
  connectedCallback() {
    ...
    if (isFilter) {
      this.item = new ToolbarFilter(
        placeholder,
        /* growFactor=*/ undefined,
        /* shrinkFactor=*/ undefined,
        tooltip,
        this.datalist ? this.#onAutocomplete.bind(this) : undefined,
        /* dynamicCompletions=*/ undefined,
        jslogContext || 'filter',
        this,
  ...
  constructor(title, glyph, toggledGlyph, jslogContext, toggleOnClick) {
    ...
    if (jslogContext) {
      this.element.setAttribute(
        'jslog',
        `${VisualLogging.toggle().track({ click: true }).context(jslogContext)}`,
      );
    }
...
export class ToolbarMenuButton extends ToolbarItem {
  ...
  mouseDown(event) {
    ...
    if (!this.triggerTimeoutId) {
      this.triggerTimeoutId = globalThis.setTimeout(
  ...
  trigger(event) {
    ...
    const horizontalPosition = this.isIconDropdown
...
export class ToolbarSettingToggle extends ToolbarToggle {
  ...
  settingChanged() {
    ...
    const toggleAnnouncement = toggled
      ? i18nString(UIStrings.pressed)
      : i18nString(UIStrings.notPressed);
...
export class ToolbarCheckbox extends ToolbarItem {
  ...
  constructor(text, tooltip, listener, jslogContext) {
    ...
    const checkboxLabel = CheckboxLabel.create(
      text,
      undefined,
      undefined,
      jslogContext,
...
function getRegisteredToolbarItems() {
  return registeredToolbarItems.filter((item) =>
