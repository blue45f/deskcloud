...
export class FilterBar extends Common.ObjectWrapper.eventMixin(HBox) {
  ...
  constructor(name, visibleByDefault) {
    ...
    this.stateSetting = Common.Settings.Settings.instance().createSetting(
      'filter-bar-' + name + '-toggled',
      Boolean(visibleByDefault),
    );
    this.#filterButton = new ToolbarSettingToggle(
      this.stateSetting,
      'filter',
  ...
  addFilter(filter) {
    ...
    filter.addEventListener(
      'FilterChanged' /* FilterUIEvents.FILTER_CHANGED */,
      this.filterChanged,
      this,
    );
...
export class TextFilterUI extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    this.#filter = new ToolbarFilter(
      undefined,
      1,
      1,
      i18nString(UIStrings.egSmalldUrlacomb),
      this.completions.bind(this),
    );
    filterToolbar.appendToolbarItem(this.#filter);
    this.#filter.addEventListener('TextChanged' /* ToolbarInput.Event.TEXT_CHANGED */, () =>
      this.valueChanged(),
    );
...
export class NamedBitSetFilterUIElement extends HTMLElement {
  ...
  set options(options) {
    ...
    if (
      this.#options.items.toString() === options.items.toString() &&
      this.#options.setting === options.setting
    ) {
      return;
    }
  ...
  getOrCreateNamedBitSetFilterUI() {
    ...
    namedBitSetFilterUI.addEventListener(
      'FilterChanged' /* FilterUIEvents.FILTER_CHANGED */,
...
export class NamedBitSetFilterUI extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(items, setting) {
    ...
    Tooltip.install(
      this.filtersElement,
      i18nString(UIStrings.sclickToSelectMultipleTypes, {
  ...
  addBit(name, label, jslogContext, title) {
    ...
    typeFilterElement.setAttribute(
      'jslog',
      `${VisualLogging.item(jslogContext).track({ click: true })}`,
    );
