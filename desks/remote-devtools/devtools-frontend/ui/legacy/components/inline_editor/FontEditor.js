...
const str_ = i18n.i18n.registerUIStrings(
  'ui/legacy/components/inline_editor/FontEditor.ts',
  UIStrings,
);
...
export class FontEditor extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(propertyMap) {
    ...
    this.fontSelectorSection.createChild('h2', 'font-section-header').textContent = i18nString(
      UIStrings.fontFamily,
    );
    this.fontSelectors = [];
    this.fontsList = null;
    ...
    cssPropertySection.createChild('h2', 'font-section-header').textContent = i18nString(
      UIStrings.cssProperties,
    );
    // The regexes only handle valid property values as invalid values are not passed into the property map.
    const fontSizePropertyInfo = this.getPropertyInfo(
      'font-size',
      FontEditorUtils.FontSizeStaticParams.regex,
    );
    const lineHeightPropertyInfo = this.getPropertyInfo(
      'line-height',
      FontEditorUtils.LineHeightStaticParams.regex,
    );
    const fontWeightPropertyInfo = this.getPropertyInfo(
      'font-weight',
      FontEditorUtils.FontWeightStaticParams.regex,
    );
    const letterSpacingPropertyInfo = this.getPropertyInfo(
      'letter-spacing',
      FontEditorUtils.LetterSpacingStaticParams.regex,
    );
    ...
    for (const fontFamilyValue of computedFontArray) {
      if (fontFamilyValue.includes(',')) {
        ...
        fonts.forEach((element) => {
          if (
            array.findIndex(
              (item) => item.toLowerCase() === element.trim().toLowerCase().replace(/"/g, "'"),
            ) === -1
          ) {
        ...
        array.findIndex(
          (item) => item.toLowerCase() === fontFamilyValue.toLowerCase().replace('"', "'"),
        ) === -1
      ) {
  ...
  async createFontSelector(value, isPrimary) {
    ...
    const selectorField = this.fontSelectorSection.createChild(
      'div',
  ...
  deleteFontSelector(index, isGlobalValue) {
    ...
    if (fontSelectorObject.input.value === '' && !isGlobalValue) {
      UI.ARIAUtils.LiveAnnouncer.alert(
        i18nString(UIStrings.thereIsNoValueToDeleteAtIndexS, { PH1: index }),
      );
      return;
    }
    if (isPrimary) {
    ...
    if (fontSelectorObject.input.parentNode) {
      ...
      UI.ARIAUtils.LiveAnnouncer.alert(
        i18nString(UIStrings.fontSelectorDeletedAtIndexS, { PH1: index }),
      );
    }
  ...
  createSelector(field, label, options, currentValue, jslogContext) {
    ...
    selectInput.setAttribute(
      'jslog',
      `${VisualLogging.dropDown(jslogContext).track({ click: true, change: true })}`,
    );
    ...
    const deleteButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.deleteS, { PH1: label }),
      'bin',
      undefined,
      'delete',
    );
...
class FontPropertyInputs {
  ...
  createTextBoxInput(field, jslogContext) {
    ...
    UI.ARIAUtils.setLabel(
      textBoxInput,
      i18nString(UIStrings.sTextInput, { PH1: this.propertyName }),
    );
    return textBoxInput;
  }
  ...
  onTextBoxInput(event) {
    ...
    if (target) {
      ...
      if (
        this.staticParams.regex.test(valueString) ||
        (value === '' && !target.validationMessage.length)
      ) {
  ...
  createTypeToggle(field, jslogContext) {
    ...
    UI.UIUtils.setTitle(
      displaySwitcher,
      i18nString(UIStrings.sToggleInputType, { PH1: this.propertyName }),
    );
    ...
    displaySwitcher.setAttribute(
      'jslog',
      `${VisualLogging.toggle(jslogContext).track({ click: true })}`,
    );
  }
  ...
  setInputUnits(multiplier, newUnit) {
    ...
    if (this.textBoxInput.value) {
      hasValue = true;
      newValue = parseFloat(
        (parseFloat(this.textBoxInput.value) * multiplier).toFixed(roundingPrecision),
      );
    }
