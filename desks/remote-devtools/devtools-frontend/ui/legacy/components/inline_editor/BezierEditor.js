...
export class BezierEditor extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(model) {
    ...
    // Curve UI
    this.debouncedStartPreviewAnimation = Common.Debouncer.debounce(
      this.startPreviewAnimation.bind(this),
      PREVIEW_ANIMATION_DEBOUNCE_DELAY,
    );
    ...
    const minus = this.createPresetModifyIcon(
      this.header,
      'bezier-preset-minus',
    ...
    minus.setAttribute(
      'jslog',
      `${VisualLogging.action('bezier.prev-preset').track({ click: true })}`,
    );
    const plus = this.createPresetModifyIcon(
      this.header,
      'bezier-preset-plus',
    ...
    plus.setAttribute(
      'jslog',
      `${VisualLogging.action('bezier.next-preset').track({ click: true })}`,
    );
  ...
  onchange() {
    ...
    this.dispatchEventToListeners(
      'BezierChanged' /* Events.BEZIER_CHANGED */,
      this.model.asCSSText(),
    );
  }
  updateUI() {
    const labelText = this.selectedCategory
  ...
  createPresetModifyIcon(parentElement, className, drawPath) {
    const icon = UI.UIUtils.createSVGChild(
      parentElement,
      'svg',
      'bezier-preset-modify ' + className,
    );
  ...
  presetModifyClicked(intensify, _event) {
    ...
    this.selectedCategory.presetIndex =
      (this.selectedCategory.presetIndex + (intensify ? 1 : -1) + length) % length;
