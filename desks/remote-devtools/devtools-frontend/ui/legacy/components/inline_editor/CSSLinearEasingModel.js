...
function consumeLinearStop(cursor, referenceText) {
  ...
  return {
    ...
    lengthA: percentages[0]
      ? Number(percentages[0].substring(0, percentages[0].length - 1))
      : undefined,
    lengthB: percentages[1]
      ? Number(percentages[1].substring(0, percentages[1].length - 1))
      : undefined,
  };
}
...
export class CSSLinearEasingModel {
  ...
  static parse(text) {
    ...
    for (let i = 1; i < points.length; i++) {
      if (isNaN(points[i].input)) {
        ...
        points[i].input =
          points[i - 1].input +
          (points[upperIndex].input - points[i - 1].input) / (upperIndex - (i - 1));
      }
    }
  ...
  asCSSText() {
    const args = this.#points
      .map(
        (point) =>
          `${numberFormatter.format(point.output)} ${numberFormatter.format(point.input)}%`,
      )
      .join(', ');
