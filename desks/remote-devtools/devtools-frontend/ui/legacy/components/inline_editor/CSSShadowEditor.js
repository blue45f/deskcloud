...
const str_ = i18n.i18n.registerUIStrings(
  'ui/legacy/components/inline_editor/CSSShadowEditor.ts',
  UIStrings,
);
...
export class CSSShadowEditor extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor() {
    ...
    this.typeField.createChild('label', 'shadow-editor-label').textContent = i18nString(
      UIStrings.type,
    );
    ...
    const blurField = this.contentElement.createChild(
      'div',
    ...
    this.spreadInput = this.createTextInput(
      this.spreadField,
  ...
  createSlider(field, jslogContext) {
    ...
    slider.setAttribute(
      'jslog',
      `${VisualLogging.slider().track({ click: true, drag: true }).context(jslogContext)}`,
    );
  ...
  handleValueModification(event) {
    ...
    const modifiedValue = UI.UIUtils.createReplacementString(
      target.value,
      event,
      customNumberHandler,
    );
  ...
  onTextBlur() {
    ...
    let length = !this.changedElement.value.trim()
      ? CSSLength.zero()
      : CSSLength.parse(this.changedElement.value);
    if (!length) {
  ...
  onSliderInput(event) {
    if (event.currentTarget === this.blurSlider) {
      ...
      this.model.setSpreadRadius(
        new CSSLength(
          Number(this.spreadSlider.value),
          this.model.spreadRadius().unit || defaultUnit,
        ),
      );
      this.spreadInput.value = this.model.spreadRadius().asCSSText();
  ...
  dragStart(event) {
    ...
    const clickedPoint = new Geometry.Point(
      event.x - this.canvasOrigin.x,
      event.y - this.canvasOrigin.y,
    );
    const thumbPoint = this.sliderThumbPosition();
  ...
  onCanvasArrowKey(event) {
    ...
    if (shiftX) {
      ...
      const newAmount = Platform.NumberUtilities.clamp(
        offsetX.amount + shiftX,
        -maxRange,
        maxRange,
      );
