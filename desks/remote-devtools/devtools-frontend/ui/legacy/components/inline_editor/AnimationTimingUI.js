...
class BezierCurveUI {
  ...
  dragStart(event) {
    ...
    const firstControlPointIsCloser =
      this.#controlPosition.distanceTo(this.#bezier.controlPoints[0]) <
...
class LinearEasingPresentation {
  ...
  draw(linearEasingModel, svg) {
    ...
    for (let i = 1; i < positions.length; i++) {
      ...
      line.setAttribute(
        'd',
        `M ${startingPoint.x} ${startingPoint.y} L ${position.x} ${position.y}`,
      );
...
class LinearEasingUI {
  ...
  #handleLineClick(event, lineIndex) {
    const newPoint = this.#presentation.positionToTimingPoint({
      x: event.offsetX,
      y: event.offsetY,
    });
  ...
  #updatePointPosition(mouseX, mouseY) {
    ...
    this.#model.setPoint(
      ...
      this.#presentation.positionToTimingPoint(newPoint),
    );
  }
  #dragMove(event) {
...
export class AnimationTimingUI {
  ...
  constructor({ model, onChange }) {
    ...
    if (this.#model instanceof Geometry.CubicBezier) {
      this.#bezierCurveUI = new BezierCurveUI({
        bezier: this.#model,
        container: this.#bezierContainer,
        onBezierChange: this.#onChange,
      });
  ...
  draw() {
    this.#linearEasingContainer.classList.toggle(
      'hidden',
      !(this.#model instanceof CSSLinearEasingModel),
    );
    this.#bezierContainer.classList.toggle(
      'hidden',
      !(this.#model instanceof Geometry.CubicBezier),
    );
    if (this.#bezierCurveUI) {
