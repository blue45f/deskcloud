...
const fontWeightRangeMap = new Map([['', { min: 100, max: 700, step: 100 }]]);
...
export async function generateComputedFontArray() {
  const modelArray = SDK.TargetManager.TargetManager.instance().models(
    CssOverviewModule.CSSOverviewModel.CSSOverviewModel,
  );
  if (modelArray) {
