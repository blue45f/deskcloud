  ...
  switch (node.nodeType) {
    case Node.ELEMENT_NODE:
      if (node instanceof Element) {
        ...
        return closeTag
          ? hasNonTextChildren(node) || node.textContent
            ? '</' + tag + '>'
            : ''
          : `${'<' + tag}${attributes(node)
              .map(
                (attributeNode) =>
                  `${'\xA0'}${attributeNode.name}${'="'}${attributeNode.value}${'"'}`,
              )
              .toArray()
              .join('')}${
    ...
    case Node.PROCESSING_INSTRUCTION_NODE:
      return node.nodeValue && !closeTag
...
function htmlView(treeNode) {
  ...
  switch (node.nodeType) {
    case Node.ELEMENT_NODE:
      if (node instanceof Element) {
        const tag = node.tagName;
        return html`<span part="shadow-xml-view-tag">${'<' + tag}</span>${attributes(node).map(
            (attributeNode) =>
              html`<span part="shadow-xml-view-tag">${'\xA0'}</span>
          ...
          <span ?hidden=${treeNode.expanded}
            >${hasNonTextChildren(node)
              ? html`<span part="shadow-xml-view-tag">${'>'}</span>
    ...
    case Node.TEXT_NODE:
      return node.nodeValue
        ? html`<span part="shadow-xml-view-text">${node.nodeValue}</span>`
        : Lit.nothing;
    case Node.CDATA_SECTION_NODE:
      return node.nodeValue
        ? html`<span part="shadow-xml-view-cdata">${'<![CDATA['}</span>
            <span part="shadow-xml-view-text">${node.nodeValue}</span>
            <span part="shadow-xml-view-cdata">${']]>'}</span>`
        : Lit.nothing;
    case Node.PROCESSING_INSTRUCTION_NODE:
      return node.nodeValue
        ? html`<span part="shadow-xml-view-processing-instruction"
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  function subtree(treeNode) {
    ...
    // clang-format off
    return html` ${children.map((child) => layOutNode(child))}
...
export class XMLTreeViewNode {
  ...
  children() {
    if (!this.#children) {
      this.#children = children(this.node)
        .map((node) => new XMLTreeViewNode(node))
        .toArray();
    }
...
export class XMLView extends UI.Widget.Widget {
  ...
  performUpdate() {
    if (this.#treeViewModel) {
      ...
      this.#view(
        {
          xml: this.#treeViewModel.root,
          onExpand,
          search: this.#search,
          jumpToNextSearchResult: this.#nextJump,
        },
        {},
        this.contentElement,
      );
  ...
  performSearch(searchConfig, shouldJump, jumpBackwards) {
    ...
    this.#search.search(this.#treeViewModel.root, jumpBackwards ?? false, (node, closeTag) =>
      node
        .match(regex, closeTag)
        .map((match, matchIndexInNode) => ({
          node,
          matchIndexInNode,
          isPostOrderMatch: closeTag,
          match,
        }))
        .toArray(),
    );
