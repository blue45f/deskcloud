...
const str_ = i18n.i18n.registerUIStrings(
  'ui/legacy/components/source_frame/JSONView.ts',
  UIStrings,
);
...
export class JSONView extends UI.Widget.VBox {
  ...
  jumpToMatch(index) {
    ...
    if (newFocusElement) {
      ...
      newFocusElement.setSearchRegex(
        this.searchRegex,
        Highlighting.highlightedCurrentSearchResultClassName,
      );
  ...
  onSearchCanceled() {
    ...
    let element;
    for (
      element = this.treeOutline.rootElement();
      element;
  ...
  jumpToNextSearchResult() {
    ...
    const newIndex = Platform.NumberUtilities.mod(
