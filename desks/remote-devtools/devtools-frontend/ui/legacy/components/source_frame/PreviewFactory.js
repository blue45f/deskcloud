...
const str_ = i18n.i18n.registerUIStrings(
  'ui/legacy/components/source_frame/PreviewFactory.ts',
  UIStrings,
);
  ...
  static async createPreview(provider, mimeType) {
    ...
    if (TextUtils.ContentData.ContentData.isError(contentData)) {
      return new UI.EmptyWidget.EmptyWidget(
        i18nString(UIStrings.failedToLoadData),
        contentData.error,
      );
    }
    ...
    const highlighterType =
      mimeType.replace(/;.*/, '') /* remove charset */ ||
      provider.contentType().canonicalMimeType();
