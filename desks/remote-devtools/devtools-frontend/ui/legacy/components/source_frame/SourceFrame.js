...
const str_ = i18n.i18n.registerUIStrings(
  'ui/legacy/components/source_frame/SourceFrame.ts',
  UIStrings,
);
...
export class SourceFrameImpl extends Common.ObjectWrapper.eventMixin(UI.View.SimpleView) {
  ...
  constructor(lazyContent, options = {}) {
    ...
    this.prettyToggle = new UI.Toolbar.ToolbarToggle(
      i18nString(UIStrings.prettyPrint),
      'brackets',
      undefined,
      'pretty-print',
    );
  ...
  editorConfiguration(doc) {
    return [
      CodeMirror.EditorView.updateListener.of((update) =>
        this.dispatchEventToListeners('EditorUpdate' /* Events.EDITOR_UPDATE */, update),
      ),
      ...
      CodeMirror.lineNumbers({
        domEventHandlers: {
          contextmenu: (_view, block, event) => this.onLineGutterContextMenu(block.from, event),
        },
      }),
      CodeMirror.EditorView.updateListener.of((update) => {
      ...
      this.wasmDisassemblyInternal
        ? markNonBreakableLines(this.wasmDisassemblyInternal)
        : nonBreakableLines,
  ...
  async showSelfXssWarning() {
    ...
    const allowPasting = await PanelCommon.TypeToAllowDialog.show({
      ...
      inputPlaceholder: i18nString(UIStrings.typeAllowPasting, {
  ...
  async setPretty(value) {
    ...
    const startPos = textEditor.toLineColumn(selection.from),
      endPos = textEditor.toLineColumn(selection.to);
    let newSelection;
    if (this.prettyInternal) {
      const content =
        this.rawContent instanceof CodeMirror.Text
  ...
  getLineNumberFormatter() {
    ...
    if (this.wasmDisassemblyInternal) {
      ...
      const lastBytecodeOffset = disassembly.lineNumberToBytecodeOffset(
        disassembly.lineNumbers - 1,
      );
      const bytecodeOffsetDigits = lastBytecodeOffset.toString(16).length + 1;
      formatNumber = (lineNumber) => {
        const bytecodeOffset = disassembly.lineNumberToBytecodeOffset(
          Math.min(disassembly.lineNumbers, lineNumber) - 1,
        );
    ...
    return formatNumber
      ? [CodeMirror.lineNumbers({ formatNumber }), LINE_NUMBER_FORMATTER.of(formatNumber)]
      : [];
  }
  updateLineNumberFormatter() {
    this.textEditor.dispatch({
      effects: config.lineNumbers.reconfigure(this.getLineNumberFormatter()),
    });
  ...
  revealPosition(position, shouldHighlight) {
    ...
    if (typeof position === 'number') {
      ...
      this.positionToReveal = {
        to: { lineNumber, columnNumber: columnNumber ?? 0 },
        shouldHighlight,
      };
    } else {
  ...
  #revealPositionIfNeeded() {
    ...
    const fromLocation = from
      ? this.uiLocationToEditorLocation(from.lineNumber, from.columnNumber)
      : undefined;
    const { textEditor } = this;
    textEditor.revealPosition(
      textEditor.createSelection(toLocation, fromLocation),
      shouldHighlight,
    );
    this.positionToReveal = null;
  }
  ...
  #scrollToLineIfNeeded() {
    if (this.lineToScrollTo !== null) {
      if (this.loaded && this.isShowing()) {
        ...
        textEditor.dispatch({
          effects: CodeMirror.EditorView.scrollIntoView(position, { y: 'start', yMargin: 0 }),
        });
        this.lineToScrollTo = null;
      }
    }
  ...
  async getLanguageSupport(content) {
    ...
    return [
      ...
      CodeMirror.javascript.javascriptLanguage.data.of({
        autocomplete: CodeMirror.completeAnyWord,
      }),
    ];
  }
  ...
  performSearch(searchConfig, shouldJump, jumpBackwards) {
    ...
    if (this.loaded) {
      ...
      this.delayedFindSearchMatches = this.doFindSearchMatches.bind(
        this,
        searchConfig,
        shouldJump,
        Boolean(jumpBackwards),
      );
    }
    void this.ensureContentLoaded();
  }
  resetCurrentSearchResultIndex() {
    ...
    if (currentActiveSearch?.currentRange) {
      editor.dispatch({
        effects: setActiveSearch.of(new ActiveSearch(currentActiveSearch.regexp, null)),
      });
    }
  }
  resetSearch() {
  ...
  replaceSelectionWith(_searchConfig, replacement) {
    ...
    const insert = this.searchRegex?.fromQuery
      ? range.insertPlaceholders(replacement)
      : replacement;
    const editor = this.textEditor;
    const changes = editor.state.changes({ from: range.from, to: range.to, insert });
    editor.dispatch({
      changes,
      selection: { anchor: changes.mapPos(editor.state.selection.main.to, 1) },
  ...
  replaceAllWith(searchConfig, replacement) {
    ...
    const changes = ranges.map((match) => ({
      from: match.from,
      to: match.to,
      insert: isRegExp ? match.insertPlaceholders(replacement) : replacement,
    }));
  ...
  updateSourcePosition() {
    ...
    if (selection.ranges.length > 1) {
      this.sourcePosition.setText(
        i18nString(UIStrings.dSelectionRegions, { PH1: selection.ranges.length }),
      );
      return;
    }
    ...
    if (main.empty) {
      ...
      if (this.wasmDisassemblyInternal) {
        ...
        this.sourcePosition.setText(
          i18nString(UIStrings.bytecodePositionXs, {
            PH1: bytecodeOffset.toString(16).padStart(bytecodeOffsetDigits, '0'),
          }),
        );
      } else {
        this.sourcePosition.setText(
          i18nString(UIStrings.lineSColumnS, { PH1: location[0] + 1, PH2: location[1] + 1 }),
        );
      }
    } else {
      const startLine = state.doc.lineAt(main.from),
        endLine = state.doc.lineAt(main.to);
      if (startLine.number === endLine.number) {
        this.sourcePosition.setText(
          i18nString(UIStrings.dCharactersSelected, { PH1: main.to - main.from }),
        );
      } else {
        this.sourcePosition.setText(
          i18nString(UIStrings.dLinesDCharactersSelected, {
            PH1: endLine.number - startLine.number + 1,
            PH2: main.to - main.from,
          }),
        );
      }
    }
...
class ActiveSearch {
  ...
  map(change) {
    return change.empty || !this.currentRange
      ? this
      : new ActiveSearch(this.regexp, {
          from: change.mapPos(this.currentRange.from),
          to: change.mapPos(this.currentRange.to),
        });
  }
  static eq(a, b) {
    return Boolean(
      ...
      (a &&
        b &&
        a.currentRange?.from === b.currentRange?.from &&
        a.currentRange?.to === b.currentRange?.to &&
        a.regexp.regex.source === b.regexp.regex.source &&
        a.regexp.regex.flags === b.regexp.regex.flags),
    );
  }
}
const setActiveSearch = CodeMirror.StateEffect.define({
...
const activeSearchState = CodeMirror.StateField.define({
  ...
  update(state, tr) {
    return tr.effects.reduce(
      (state, effect) => (effect.is(setActiveSearch) ? effect.value : state),
      state?.map(tr.changes) ?? null,
    );
  },
});
const searchMatchDeco = CodeMirror.Decoration.mark({ class: 'cm-searchMatch' });
const currentSearchMatchDeco = CodeMirror.Decoration.mark({
  class: 'cm-searchMatch cm-searchMatch-selected',
});
const searchHighlighter = CodeMirror.ViewPlugin.fromClass(
  class {
    decorations;
    constructor(view) {
    ...
    update(update) {
      ...
      if (
        !ActiveSearch.eq(active, update.startState.field(activeSearchState)) ||
          ...
          if (part !== '\n') {
            ...
            for (;;) {
              ...
              if (match[0].length) {
                ...
                const current =
                  active.currentRange?.from === start && active.currentRange.to === end;
...
const sourceFrameInfobarState = CodeMirror.StateField.define({
  ...
  provide: (field) =>
    CodeMirror.showPanel.computeN([field], (state) =>
      state
        .field(field)
