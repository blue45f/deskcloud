...
const str_ = i18n.i18n.registerUIStrings(
  'ui/legacy/components/object_ui/CustomPreviewComponent.ts',
  UIStrings,
);
  ...
  async loadBody() {
    ...
    if (customPreview.bodyGetterId) {
      const bodyJsonML = await this.object.callFunctionJSON(
        ...
        [{ objectId: customPreview.bodyGetterId }],
      );
      if (bodyJsonML === null) {
...
export class CustomPreviewComponent {
  ...
  contextMenuEventFired(event) {
    ...
    if (this.customPreviewSection) {
      contextMenu
        .revealSection()
        .appendItem(i18nString(UIStrings.showAsJavascriptObject), this.disassemble.bind(this), {
  ...
  disassemble() {
    if (this.element.shadowRoot) {
      ...
      this.element.shadowRoot.appendChild(
        ObjectPropertiesSection.defaultObjectPresentation(this.object),
      );
    }
  }
}
