...
export class RemoteObjectPreviewFormatter {
  static objectPropertyComparator(a, b) {
    ...
    function sortValue(property) {
      ...
      if (
        property.name === '[[GeneratorState]]' /* InternalName.GENERATOR_STATE */ ||
        property.name === '[[PrimitiveValue]]' /* InternalName.PRIMITIVE_VALUE */ ||
        property.name === '[[WeakRefTarget]]' /* InternalName.WEAK_REF_TARGET */
      ) {
        return 3;
      }
      if (
        property.type !== 'function' /* Protocol.Runtime.PropertyPreviewType.Function */ &&
  ...
  renderObjectPreview(preview, includeNullOrUndefined = true) {
    ...
    if (
      preview.type !== 'object' /* Protocol.Runtime.ObjectPreviewType.Object */ ||
    ...
    const isArrayOrTypedArray =
      preview.subtype === 'array' /* Protocol.Runtime.ObjectPreviewSubtype.Array */ ||
    ...
    const items = Array.from(
      preview.entries
        ? this.renderEntries(preview)
        : isArrayOrTypedArray
          ? this.renderArrayProperties(preview)
    ...
    const renderEntry = (entry) =>
      html`${entry.key &&
      html`${this.renderPropertyPreview(
        entry.key.type,
        entry.key.subtype,
        undefined,
    ...
    const renderItem = ({ name, entry, value, placeholder }, index) =>
      html`${index > 0 ? ', ' : ''}${placeholder !== undefined
        ? renderPlaceholder(placeholder)
        : nothing}${name !== undefined ? renderName(name) : nothing}${name !== undefined && value
        ? ': '
        : ''}${value ? renderValue(value) : nothing}${entry ? renderEntry(entry) : nothing}`;
    // clang-format on
    return html`${objectDescription.length > 0
        ? html`<span class="object-description">${objectDescription + '\xA0'}</span>`
        : nothing}<span class="object-properties-preview"
        >${isArrayOrTypedArray ? '[' : '{'}${repeat(items, renderItem)}${preview.overflow
          ? html`<span>${items.length > 0 ? ',\xA0…' : '…'}</span>`
          : ''}
    ...
    for (let i = 0; i < properties.length; ++i) {
      ...
      if (
        ...
        (property.type === 'undefined' ||
          (property.type === 'object' && property.subtype === 'null'))
      ) {
        continue;
      }
      // Internal properties are given special formatting, e.g. Promises `<rejected>: 123`.
      if (
        preview.subtype === 'promise' /* Protocol.Runtime.ObjectPreviewSubtype.Promise */ &&
        name === '[[PromiseState]]' /* InternalName.PROMISE_STATE */
      ) {
        const promiseResult =
          properties.at(i + 1)?.name === '[[PromiseResult]]' /* InternalName.PROMISE_RESULT */
            ? properties.at(i + 1)
            : undefined;
        if (promiseResult) {
          i++;
        ...
        yield {
          name: '<' + property.value + '>',
          value: property.value !== 'pending' ? promiseResult : undefined,
        };
      } else if (
        preview.subtype === 'generator' &&
        name === '[[GeneratorState]]' /* InternalName.GENERATOR_STATE */
      ) {
  ...
  *renderArrayProperties(preview) {
    const arrayLength = SDK.RemoteObject.RemoteObject.arrayLength(preview);
    const indexProperties = preview.properties
      .filter((p) => toArrayIndex(p.name) !== -1)
      .sort(arrayEntryComparator);
    const otherProperties = preview.properties
      .filter((p) => toArrayIndex(p.name) === -1)
      .sort(RemoteObjectPreviewFormatter.objectPropertyComparator);
    ...
    const renderGap = (count) => ({
      placeholder:
        count !== 1 ? i18nString(UIStrings.emptyD, { PH1: count }) : i18nString(UIStrings.empty),
    });
  ...
  *renderEntries(preview) {
  ...
  renderPropertyPreview(type, subtype, className, description) {
    const title =
      type === 'accessor'
        ? i18nString(UIStrings.thePropertyIsComputedWithAGetter)
        : type === 'object' && !subtype
          ? description
          : undefined;
    const abbreviateFullQualifiedClassName = (description) => {
      ...
      for (let i = 0; i < abbreviatedDescription.length - 1; ++i) {
        abbreviatedDescription[i] = Platform.StringUtilities.trimMiddle(
          abbreviatedDescription[i],
          3,
        );
      }
    ...
    const preview = () =>
      type === 'accessor'
        ...
        : type === 'function'
          ? '\u0192'
          : type === 'object' && subtype === 'trustedtype' && className
            ? renderTrustedType(description ?? '', className)
            : type === 'object' && subtype === 'node' && description
              ? renderNodeTitle(description)
              : type === 'string'
                ? Platform.StringUtilities.formatAsJSLiteral(description ?? '')
                : type === 'object' && !subtype
                  ? abbreviateFullQualifiedClassName(description ?? '')
                  : description;
    return html`<span class="object-value-${subtype || type}" title=${ifDefined(title)}
      >${preview()}</span
    >`;
  }
...
export function renderNodeTitle(nodeTitle) {
  ...
  return html`<span class="webkit-html-tag-name">${match[1]}</span>${match[2] &&
    html`<span class="webkit-html-attribute-value">${match[2]}</span>`}${match[3] &&
    html`<span class="webkit-html-attribute-name">${match[3]}</span>`}`;
}
export function renderTrustedType(description, className) {
  return html`${className}
    <span class="object-value-string">"${description.replace(/\n/g, '\u21B5')}"</span>`;
}
