...
export class ViewportDataGrid extends Common.ObjectWrapper.eventMixin(DataGridImpl) {
  ...
  updateStripesClass(startsWithOdd) {
    ...
    this.element.classList.toggle(
      'striped-data-grid-starts-with-odd',
      startsWithOdd && this.isStriped,
    );
  }
  ...
  update() {
    ...
    if (visibleNodes.length) {
      ...
      if (
        ...
        index !== -1 &&
        Boolean(index % 2) !== this.firstVisibleIsStriped
      ) {
        offset += 1;
      }
    }
    ...
    const contentFits =
      viewportState.contentHeight <= clientHeight &&
...
export class ViewportDataGridNode extends DataGridNode {
  ...
  nodeSelfHeight() {
    ...
    const height =
      firstVisibleRow?.classList.contains('data-grid-data-grid-node') &&
      firstVisibleRow.clientHeight;
  ...
  collapse() {
    ...
    this.dataGrid?.dispatchEventToListeners(
      'CollapsedNode' /* DataGridEvents.COLLAPSED_NODE */,
      this,
    );
  }
  expand() {
