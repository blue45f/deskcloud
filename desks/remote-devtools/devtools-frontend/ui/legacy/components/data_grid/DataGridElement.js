...
class DataGridElement extends UI.UIUtils.HTMLElementWithLightDOMTemplate {
  ...
  constructor() {
    ...
    this.#shadowRoot = UI.UIUtils.createShadowRootWithCoreStyles(this, {
      delegatesFocus: true,
      cssFile: dataGridStyles,
    });
    this.#shadowRoot.appendChild(this.#dataGrid.element);
    this.#dataGrid.addEventListener('SelectedNode' /* DataGridEvents.SELECTED_NODE */, (e) =>
      e.data.configElement.dispatchEvent(new CustomEvent('select')),
    );
    this.#dataGrid.addEventListener('DeselectedNode' /* DataGridEvents.DESELECTED_NODE */, () =>
      this.dispatchEvent(new CustomEvent('deselect')),
    );
    this.#dataGrid.addEventListener('OpenedNode' /* DataGridEvents.OPENED_NODE */, (e) =>
      e.data.configElement.dispatchEvent(new CustomEvent('open')),
    );
    this.#dataGrid.addEventListener('ExpandedNode' /* DataGridEvents.EXPANDED_NODE */, (e) =>
      e.data.configElement.dispatchEvent(new CustomEvent('expand')),
    );
    this.#dataGrid.addEventListener('CollapsedNode' /* DataGridEvents.COLLAPSED_NODE */, (e) =>
      e.data.configElement.dispatchEvent(new CustomEvent('collapse')),
    );
    this.#dataGrid.addEventListener('SortingChanged' /* DataGridEvents.SORTING_CHANGED */, () =>
      this.dispatchEvent(
        new CustomEvent('sort', {
          detail: {
            columnId: this.#dataGrid.sortColumnId(),
            ascending: this.#dataGrid.isSortOrderAscending(),
          },
        }),
      ),
    );
            ...
            () => {
              ...
              this.#dataGrid.setColumnsVisibility(
                new Set(
                  this.#columns
                    .map(({ id }) => id)
                    .filter((column) => !this.#hiddenColumns.has(column)),
                ),
              );
            },
  ...
  #updateColumns() {
    ...
    for (const column of this.templateRoot.querySelectorAll('th[id]') || []) {
      ...
      const titleDOMFragment = column.firstElementChild
      ...
      const dataType =
        column.getAttribute('type') === 'boolean'
      ...
      const sort =
        column.getAttribute('sort') === 'descending'
          ? Order.Descending
          : column.getAttribute('sort') === 'ascending'
            ? Order.Ascending
            : undefined;
    ...
    const visibleColumns = new Set(
      this.#columns.map(({ id }) => id).filter((id) => !this.#hiddenColumns.has(id)),
    );
    if (visibleColumns.size) {
      this.#dataGrid.setColumnsVisibility(visibleColumns);
    }
    this.#dataGrid.setEditCallback(
      hasEditableColumn ? this.#editCallback.bind(this) : undefined,
      INTERNAL_TOKEN,
    );
  ...
  onChange(mutationList) {
    ...
    if (this.#dataGrid.sortColumnId() !== null && !hadAddedNodes) {
      this.#dataGrid.dispatchEventToListeners(
        'SortingChanged' /* DataGridEvents.SORTING_CHANGED */,
      );
    }
  }
  #editCallback(node, columnId, valueBeforeEditing, newText, moveDirection) {
    ...
    node.configElement.dispatchEvent(
      new CustomEvent('edit', { detail: { columnId, valueBeforeEditing, newText } }),
    );
  }
  #deleteCallback(node) {
...
class DataGridElementNode extends SortableDataGridNode {
  ...
  #updateData() {
    ...
    for (let i = 0; i < this.#dataGridElement.columns.length; ++i) {
      ...
      if (column.dataType === 'Boolean' /* DataType.BOOLEAN */) {
        this.data[column.id] =
          hasBooleanAttribute(cell, 'data-value') || cell.textContent === 'true';
      } else {
  ...
  #onRowMouseEvent(event) {
    ...
    if (targetInConfigRow instanceof HTMLElement) {
      targetInConfigRow?.dispatchEvent(
        new MouseEvent(event.type, { bubbles: true, composed: true }),
      );
    }
  }
  createCells(element) {
...
export const ifExpanded = Lit.Directive.directive(
  class extends Lit.Directive.Directive {
    #partInfo;
