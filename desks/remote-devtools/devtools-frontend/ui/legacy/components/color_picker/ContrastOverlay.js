...
export class ContrastOverlay {
  ...
  constructor(contrastInfo, colorElement) {
    ...
    this.visible = false;
    this.contrastRatioSVG = UI.UIUtils.createSVGChild(
      colorElement,
      'svg',
    ...
    this.contrastInfo.addEventListener(
      'ContrastInfoUpdated' /* Events.CONTRAST_INFO_UPDATED */,
  ...
  update() {
    ...
    if (
      Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.APCA) &&
  ...
  async drawContrastRatioLines() {
    for (const [level, element] of this.contrastRatioLines) {
      const path = this.contrastRatioLineBuilder.drawContrastRatioLine(
        this.width,
        this.height,
        level,
      );
...
export class ContrastRatioLineBuilder {
  ...
  drawContrastRatioLine(width, height, level) {
    ...
    const requiredContrast = isAPCA
      ? this.contrastInfo.contrastRatioAPCAThreshold()
      : this.contrastInfo.contrastRatioThreshold(level);
    ...
    if (
      isAPCA &&
      Math.abs(
        Math.round(Common.ColorUtils.contrastRatioByLuminanceAPCA(desiredLuminance, bgLuminance)),
      ) < requiredContrast
    ) {
      return null;
    }
    ...
    for (s = 0; s < 1 + dS; s += dS) {
      ...
      const v = Common.Color.approachColorValue(
        candidateHSVA,
        V,
        desiredLuminance,
        candidateLuminance,
