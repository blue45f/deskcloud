...
const str_ = i18n.i18n.registerUIStrings(
  'ui/legacy/components/color_picker/ContrastDetails.ts',
  UIStrings,
);
...
export class ContrastDetails extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(
    ...
    this.contrastValueBubble = contrastValueRowContents.createChild(
      'span',
    ...
    this.contrastValueBubbleIcons.push(
      this.contrastValueBubble.appendChild(createIcon('checkmark')),
    );
    this.contrastValueBubbleIcons.push(
      this.contrastValueBubble.appendChild(createIcon('check-double')),
    );
    this.contrastValueBubbleIcons.push(this.contrastValueBubble.appendChild(createIcon('clear')));
    this.contrastValueBubbleIcons.forEach((button) =>
    ...
    this.bgColorPickerButton.addEventListener(
      'Click' /* UI.Toolbar.ToolbarButton.Events.CLICK */,
      this.#toggleBackgroundColorPicker.bind(this, undefined, true),
    );
    ...
    this.contrastInfo.addEventListener(
      'ContrastInfoUpdated' /* ContrastInfoEvents.CONTRAST_INFO_UPDATED */,
  ...
  update() {
    ...
    const isAPCAEnabled = Root.Runtime.experiments.isEnabled(
    ...
    if (isAPCAEnabled) {
      ...
      const passesAPCA =
        apcaContrastRatio && apcaThreshold ? Math.abs(apcaContrastRatio) >= apcaThreshold : false;
      ...
      if (apcaThreshold !== null) {
        this.contrastPassFailAPCA.createChild('span').textContent =
          `: ${apcaThreshold.toFixed(2)}%`;
      }
      if (passesAPCA) {
    ...
    this.contrastPassFailAA.createChild('span').textContent = i18nString(
      UIStrings.placeholderWithColon,
      { PH1: aa.toFixed(1) },
    );
    if (this.passesAA) {
      const iconCheckmark = createIconCheckmark();
    ...
    this.contrastPassFailAAA.createChild('span').textContent = i18nString(
      UIStrings.placeholderWithColon,
      { PH1: aaa.toFixed(1) },
    );
    if (passesAAA) {
    ...
    [labelAA, labelAAA].forEach((e) =>
      e.addEventListener('click', () => ContrastDetails.showHelp()),
    );
  ...
  #toggleBackgroundColorPicker(enabled, shouldTriggerEvent = true) {
    ...
    if (shouldTriggerEvent) {
      this.dispatchEventToListeners(
        'BackgroundColorPickerWillBeToggled' /* Events.BACKGROUND_COLOR_PICKER_WILL_BE_TOGGLED */,
        enabled,
      );
    }
...
export class Swatch {
  ...
  setColors(fgColor, bgColor) {
    ...
    this.swatchElement.classList.toggle(
      'swatch-inner-white',
      bgColor.as('hsl' /* Common.Color.Format.HSL */).l > 0.9,
    );
  }
}
