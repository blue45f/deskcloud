...
export class FormatPickerContextMenu {
  ...
  async show(e, onSelect) {
    ...
    const colorFunctionSection = menu
      .section('color-function')
      .appendSubMenuItem('color()', false, 'color')
      .section();
    ...
    for (const format of [...legacyFormats, ...modernFormats]) {
      ...
      const section = legacyFormats.includes(format)
        ? legacySection
        : newColor instanceof Common.Color.ColorFunction
          ? colorFunctionSection
          : wideSection;
