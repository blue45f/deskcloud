...
export class BrickBreaker extends HTMLElement {
  ...
  constructor(timelineFlameChart) {
    ...
    this.#scorePanel.style.borderImage =
      'linear-gradient(' +
      this.#currentPalette.mediumDarker +
      ',' +
      this.#currentPalette.dark +
      ') 1';
    this.initButton();
  }
  initButton() {
    ...
    button.style.boxShadow =
      ...
      this.#currentPalette.mediumDarker +
      ' 3px 3px, ' +
      this.#currentPalette.mediumLighter +
      ' 5px 5px';
  ...
  #setUpNewGame() {
    this.#resetCanvas();
    this.#deltaMultiplier = Math.max(
      0.1,
      (this.offsetHeight - this.#minScreenHeight) / this.#screenHeightDiff,
    );
    ...
    this.#gameViewportOffset =
      this.#trackTimelineOffset +
      this.timelineFlameChart.getCanvas().getBoundingClientRect().top -
      this.timelineFlameChart.getScrollOffset();
    requestAnimationFrame(() =>
      this.#animateFlameChartTopPositioning(trackData.top, trackData.height),
    );
  }
  ...
  #drawPaddle() {
    ...
    this.#ctx.rect(
      this.#paddleX,
      this.offsetHeight - PADDLE_HEIGHT,
      this.#paddleLength,
      PADDLE_HEIGHT,
    );
    this.#ctx.fillStyle = gradient;
  ...
  #patchBrokenBricks() {
    ...
    for (const brick of this.#brokenBricks.values()) {
      ...
      this.#ctx.fillStyle = ThemeSupport.ThemeSupport.instance().getComputedValue(
  ...
  #draw() {
    ...
    this.#deltaVectorLength =
      (MIN_DELTA +
        ((MAX_DELTA - MIN_DELTA) * this.#brokenBricks.size) / this.#visibleEntries.size) *
      this.#deltaMultiplier;
    this.#paddleLength =
      MAX_PADDLE_LENGTH -
      ((MAX_PADDLE_LENGTH - MIN_PADDLE_LENGTH) * this.#brokenBricks.size) /
        this.#visibleEntries.size;
    if (
      this.#ballX + this.#ballDx > this.offsetWidth - BALL_RADIUS ||
      this.#ballX + this.#ballDx < BALL_RADIUS
    ) {
    ...
    if (this.#ballY + this.#ballDy < BALL_RADIUS) {
      ...
      // void.
      if (
        this.#ballX > this.#paddleX - BALL_RADIUS &&
        ...
        const paddleLenghtPortion =
          ((roundedBallX - this.#paddleX) * this.#deltaVectorLength * 2) / this.#paddleLength;
        this.#ballDx = -this.#deltaVectorLength + paddleLenghtPortion;
        // Solve for this.ballDy given the above equation and bounce up.
        this.#ballDy = -Math.sqrt(
          2 * Math.pow(this.#deltaVectorLength, 2) - Math.pow(this.#ballDx, 2),
        );
      } else {
  ...
  #brickCollisionDetection() {
    ...
    const entryIndexTop = this.timelineFlameChart.coordinatesToEntryIndex(
      this.#ballX,
      ballYRelativeToGame + BALL_RADIUS,
    );
    const entryIndexBottom = this.timelineFlameChart.coordinatesToEntryIndex(
      this.#ballX,
      ballYRelativeToGame - BALL_RADIUS,
    );
    const entryIndexRight = this.timelineFlameChart.coordinatesToEntryIndex(
      this.#ballX + BALL_RADIUS,
      ballYRelativeToGame,
    );
    const entryIndexLeft = this.timelineFlameChart.coordinatesToEntryIndex(
      this.#ballX - BALL_RADIUS,
      ballYRelativeToGame,
    );
    ...
    if (
      entryIndexTop > -1 &&
      !this.#brokenBricks.has(entryIndexTop) &&
      this.#visibleEntries.has(entryIndexTop)
    ) {
      this.#ballDy = -this.#ballDy;
    ...
    if (
      entryIndexBottom > -1 &&
      !this.#brokenBricks.has(entryIndexBottom) &&
    ...
    if (
      entryIndexRight > -1 &&
      !this.#brokenBricks.has(entryIndexRight) &&
      this.#visibleEntries.has(entryIndexRight)
    ) {
      this.#ballDx = -this.#ballDx;
    ...
    if (
      entryIndexLeft > -1 &&
      !this.#brokenBricks.has(entryIndexLeft) &&
      this.#visibleEntries.has(entryIndexLeft)
    ) {
      this.#ballDx = -this.#ballDx;
    ...
    const diagonalIndexes = [
      entryIndexRightTop,
      entryIndexLeftTop,
      entryIndexRightBottom,
      entryIndexLeftBottom,
    ];
  ...
  #party() {
    ...
    const drawConfetti = () => {
      for (let i = 0; i < columnCount * rowCount; i++) {
        ...
        confettiContainerElement.append(
          this.#createConfettiElement(
            (i % columnCount) * xSpacing + randomOffset(),
            (i % rowCount) * ySpacing + randomOffset(),
          ),
        );
        timeoutIDs.push(
          globalThis.setTimeout(() => this.append(confettiContainerElement), Math.random() * 100),
        );
        timeoutIDs.push(
  ...
  #createConfettiElement(x, y) {
    ...
    confettiElement.style.setProperty(
      '--rotation',
      this.#random(-maxRotation * 360, maxRotation * 360) + 'deg',
    );
