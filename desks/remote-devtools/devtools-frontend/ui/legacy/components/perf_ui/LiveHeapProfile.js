...
export class LiveHeapProfile {
  ...
  constructor() {
    ...
    this.setting.addChangeListener((event) =>
      event.data ? this.startProfiling() : this.stopProfiling(),
    );
    if (this.setting.get()) {
  ...
  async startProfiling() {
    ...
    do {
      ...
      await Promise.race([
        new Promise((r) =>
          globalThis.setTimeout(r, Host.InspectorFrontendHost.isUnderTest() ? 10 : 5000),
        ),
        new Promise((r) => {
