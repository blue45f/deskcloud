...
export class Window extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(parentElement, dividersLabelBarElement, calculator) {
    ...
    this.breadcrumbButtonContainerElement = parentElement.createChild(
      'div',
  ...
  handleKeyboardResizing(event, moveRightResizer) {
    ...
    if (keyboardEvent.key === 'ArrowLeft' || keyboardEvent.key === 'ArrowRight') {
      ...
      const newPos = this.getNewResizerPosition(
        target.offsetLeft,
        increment,
  ...
  setWindowRatio(windowLeftRatio, windowRightRatio) {
    ...
    if (this.calculator) {
      this.dispatchEventToListeners(
        'WindowChangedWithPosition' /* Events.WINDOW_CHANGED_WITH_POSITION */,
        this.calculateWindowValue(),
      );
    }
  ...
  #createBreadcrumb() {
    this.dispatchEventToListeners(
      'BreadcrumbAdded' /* Events.BREADCRUMB_ADDED */,
      this.calculateWindowValue(),
    );
  }
  updateCurtains() {
    ...
    if (this.calculator) {
      ...
      this.updateResizeElementPercentageLabels(
        leftResizerPercLeftOffsetString,
        rightResizerPercLeftOffsetString,
      );
    }
  ...
  setWindowPosition(startPixel, endPixel) {
    ...
    const windowLeft =
      typeof startPixel === 'number' ? startPixel / clientWidth : this.windowLeftRatio;
    const windowRight =
      typeof endPixel === 'number' ? endPixel / clientWidth : this.windowRightRatio;
...
export class WindowSelector {
  ...
  close(position) {
    ...
    return this.startPosition < position
      ? { start: this.startPosition, end: position }
