...
export class ChartViewport extends UI.Widget.VBox {
  ...
  onMouseWheel(wheelEvent) {
    const navigation = Common.Settings.Settings.instance()
      .moduleSetting('flamechart-selected-navigation')
      .get();
    ...
    if (navigation === 'classic') {
      if (wheelEvent.shiftKey) {
        ...
        // Pan left/right on trackpad horizontal scroll
      ...
      const isCtrlOrCmd =
        UI.KeyboardShortcut.KeyboardShortcut.eventHasCtrlEquivalentKey(wheelEvent);
      if (wheelEvent.shiftKey) {
        // Pan left/right
  ...
  rangeSelectionDragging(event) {
    const x = Platform.NumberUtilities.clamp(
      event.pageX + this.selectionOffsetShiftX,
      0,
      this.offsetWidth,
    );
  ...
  updateCursorPosition(e) {
    ...
    const shouldShowCursor =
      this.#config.enableCursorElement && mouseEvent.shiftKey && !mouseEvent.metaKey;
  ...
  timeToPosition(time) {
    return Math.floor(
      ((time - this.visibleLeftTime) / (this.visibleRightTime - this.visibleLeftTime)) *
        this.offsetWidth,
    );
  }
  timeToPixel() {
  ...
  handleZoomPanScrollKeys(keyboardEvent) {
    ...
    if (
      UI.KeyboardShortcut.KeyboardShortcut.hasAtLeastOneModifier(keyboardEvent) &&
  ...
  setWindowTimes(startTime, endTime, animate) {
    ...
    if (
      !animate ||
      this.visibleLeftTime === 0 ||
      this.visibleRightTime === Infinity ||
    ...
    this.cancelWindowTimesAnimation = UI.UIUtils.animateFunction(
      ...
      [
        { from: this.visibleLeftTime, to: startTime },
        { from: this.visibleRightTime, to: endTime },
      ],
      100,
      () => {
