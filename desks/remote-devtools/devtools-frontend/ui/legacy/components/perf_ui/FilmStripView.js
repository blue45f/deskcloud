...
const str_ = i18n.i18n.registerUIStrings(
  'ui/legacy/components/perf_ui/FilmStripView.ts',
  UIStrings,
);
...
export class FilmStripView extends Common.ObjectWrapper.eventMixin(UI.Widget.HBox) {
  ...
  createFrameElement(frame) {
    ...
    element.setAttribute(
      'jslog',
      `${VisualLogging.preview('film-strip').track({ click: true, dblclick: true })}`,
    );
    element.setAttribute(
      'aria-label',
      i18nString(UIStrings.screenshotForSSelectToView, { PH1: frameTime }),
    );
    ...
    element.addEventListener(
      'mousedown',
      this.onMouseEvent.bind(this, 'FrameSelected' /* Events.FRAME_SELECTED */, time),
      false,
    );
    element.addEventListener(
      'mouseenter',
      this.onMouseEvent.bind(this, 'FrameEnter' /* Events.FRAME_ENTER */, time),
      false,
    );
    element.addEventListener(
      'mouseout',
      this.onMouseEvent.bind(this, 'FrameExit' /* Events.FRAME_EXIT */, time),
      false,
    );
    ...
    const imgData = Trace.Handlers.ModelHandlers.Screenshots.screenshotImageDataUri(
