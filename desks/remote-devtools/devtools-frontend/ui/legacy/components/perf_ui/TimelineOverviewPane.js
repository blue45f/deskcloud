...
export class TimelineOverviewPane extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(prefix) {
    ...
    this.overviewGrid.addEventListener(
      'WindowChangedWithPosition' /* OverviewGridEvents.WINDOW_CHANGED_WITH_POSITION */,
      this.onWindowChanged,
      this,
    );
    this.overviewGrid.addEventListener(
      'BreadcrumbAdded' /* OverviewGridEvents.BREADCRUMB_ADDED */,
    ...
    this.#dimHighlightSVG = UI.UIUtils.createSVGChild(
      this.element,
      'svg',
  ...
  onMouseMove(event) {
    ...
    if (
      Trace.Types.Timing.Milli(timeWindow.rawStartValue) <= timeInMilliSeconds &&
      ...
      this.dispatchEventToListeners('OverviewPaneMouseMove' /* Events.OVERVIEW_PANE_MOUSE_MOVE */, {
        timeInMicroSeconds,
      });
    } else {
      this.dispatchEventToListeners(
        'OverviewPaneMouseLeave' /* Events.OVERVIEW_PANE_MOUSE_LEAVE */,
      );
    }
    if (!this.hasPointer) {
  ...
  async buildOverviewInfo() {
    ...
    const elements = await Promise.all(
      this.overviewControls.map((control) => control.overviewInfoPromise(x)),
    );
  ...
  wasShown() {
    super.wasShown();
    const start =
      TraceBounds.TraceBounds.BoundsManager.instance().state()?.milli.minimapTraceBounds.min;
    const end =
      TraceBounds.TraceBounds.BoundsManager.instance().state()?.milli.minimapTraceBounds.max;
  ...
  setBounds(minimumBoundary, maximumBoundary) {
    if (
      minimumBoundary === this.overviewCalculator.minimumBoundary() &&
  ...
  #dimMarkers(highlightBounds) {
    for (const time of this.markers.keys()) {
      ...
      const dim =
        highlightBounds &&
        !Trace.Helpers.Timing.timestampIsInBounds(highlightBounds, timeInMicroSeconds);
  ...
  updateMarkers() {
    ...
    for (const time of this.markers.keys()) {
      ...
      const position = Math.round(
        this.overviewCalculator.computePosition(Trace.Types.Timing.Milli(time)),
      );
  ...
  onBreadcrumbAdded() {
    this.dispatchEventToListeners(
      'OverviewPaneBreadcrumbAdded' /* Events.OVERVIEW_PANE_BREADCRUMB_ADDED */,
      {
  ...
  onWindowChanged(event) {
    ...
    this.windowStartTime = Trace.Types.Timing.Milli(
      event.data.rawStartValue === this.overviewCalculator.minimumBoundary()
        ? 0
        : event.data.rawStartValue,
    );
    this.windowEndTime = Trace.Types.Timing.Milli(
      event.data.rawEndValue === this.overviewCalculator.maximumBoundary()
        ? Infinity
    ...
    this.dispatchEventToListeners(
      'OverviewPaneWindowChanged' /* Events.OVERVIEW_PANE_WINDOW_CHANGED */,
      windowTimes,
    );
  }
  ...
  updateWindow() {
    ...
    const left =
      haveRecords && this.windowStartTime
        ? Math.min((this.windowStartTime - absoluteMin) / timeSpan, 1)
        : 0;
    const right =
      haveRecords && this.windowEndTime < Infinity
        ? (this.windowEndTime - absoluteMin) / timeSpan
        : 1;
  ...
  #initializeDimHighlightSVG() {
    ...
    desaturateRect.setAttribute(
      'fill',
      ThemeSupport.ThemeSupport.instance().getComputedValue('--color-background'),
    );
    ...
    const bracketColor = ThemeSupport.ThemeSupport.instance().getComputedValue(
  ...
  highlightBounds(bounds, withBracket) {
    const left = this.overviewCalculator.computePosition(
      Trace.Helpers.Timing.microToMilli(bounds.min),
    );
    const right = this.overviewCalculator.computePosition(
      Trace.Helpers.Timing.microToMilli(bounds.max),
    );
...
export class OverviewInfo {
  ...
  constructor(anchor) {
    ...
    this.glassPane.setPointerEventsBehavior(
      'PierceContents' /* UI.GlassPane.PointerEventsBehavior.PIERCE_CONTENTS */,
    );
    this.glassPane.setMarginBehavior(
      'DefaultMargin' /* UI.GlassPane.MarginBehavior.DEFAULT_MARGIN */,
    );
