...
export class Performance {
  ...
  appendLegacyCPUProfile(profile, target) {
    ...
    const sampleDuration =
      (profile.profileEndTime - profile.profileStartTime) / profile.totalHitCount;
          ...
          for (const [lineNumber, lineData] of lineToDataMap) {
            ...
            for (const [columnNumber, data] of lineData) {
              ...
              if (target) {
                ...
                if (rawLocation) {
                  pending.push(
                    workspaceBinding.rawLocationToUILocation(rawLocation).then((uiLocation) => {
                      if (!uiLocation) {
                        return;
                      }
                      ...
                      if (uiLocation.uiSourceCode.contentType().isFromSourceMap()) {
                        ...
                        const uiSourceCode = script
                          ? workspaceBinding.uiSourceCodeForScript(script)
                          : null;
                        if (uiSourceCode) {
                          this.addLineColumnData(
                            decorationsBySource,
