...
// arrow-down.svg
const moveDownIconPath =
  'M9.25 3V14.125L7.062 11.938L6 13L10 17L14 13L12.938 11.938L10.75 14.125V3H9.25Z';
// eye-crossed.svg
const hideIconPath =
...
export class FlameChart extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(dataProvider, flameChartDelegate, optionalConfig = {}) {
    ...
    this.popoverElement =
      optionalConfig.tooltipElement ||
      this.viewportElement.createChild('div', 'flame-chart-entry-info');
  ...
  canvasBoundingClientRect() {
    ...
    if (
      ...
      this.#canvasBoundingClientRect.width > 0 &&
  ...
  enableDimming(entryIndices, inclusive, outline) {
    ...
    this.dimShouldOutlineUndimmedEntries = Array.isArray(outline)
      ? this.#createTypedIndexArray(outline, true)
      : outline;
    this.draw();
  }
  disableDimming() {
  ...
  timelineData(rebuild) {
    ...
    if (
      timelineData !== this.rawTimelineData ||
  ...
  onMouseMove(mouseEvent) {
    ...
    const timeMilliSeconds = Trace.Types.Timing.Milli(
      this.chartViewport.pixelToTime(mouseEvent.offsetX),
    );
    ...
    const { groupIndex, hoverType } = this.coordinatesToGroupIndexAndHoverType(
      mouseEvent.offsetX,
      mouseEvent.offsetY,
    );
    switch (hoverType) {
  ...
  updateHighlight() {
    ...
    this.viewportElement.style.cursor = this.dataProvider.canJumpToEntry(entryIndex)
      ? 'pointer'
      : 'default';
  ...
  #updatePopoverForEntry(entryIndex) {
    ...
    if (
      entryIndex === this.lastPopoverState.entryIndex &&
    ...
    const popoverElement =
      isMouseOverRevealChildrenArrow && group
  ...
  updatePopoverOffset() {
    ...
    if (this.#searchResultEntryIndex !== null) {
      ...
      const { x: canvasViewportOffsetX, y: canvasViewportOffsetY } =
    ...
    const parentWidth = this.popoverElement.parentElement
      ? this.popoverElement.parentElement.clientWidth
      : 0;
    const parentHeight = this.popoverElement.parentElement
      ? this.popoverElement.parentElement.clientHeight
      : 0;
  ...
  #onDblClick(mouseEvent) {
    this.focus();
    const { groupIndex } = this.coordinatesToGroupIndexAndHoverType(
      mouseEvent.offsetX,
      mouseEvent.offsetY,
    );
    /**
    ...
    if (this.highlightedEntryIndex !== -1) {
      ...
      this.dispatchEventToListeners(
        'EntryLabelAnnotationAdded' /* Events.ENTRY_LABEL_ANNOTATION_ADDED */,
    ...
    switch (hoverType) {
      ...
      case 'OUTSIDE_TRACKS' /* HoverType.OUTSIDE_TRACKS */: {
        ...
        if (mouseEvent.shiftKey && this.highlightedEntryIndex !== -1 && timelineData) {
          ...
          this.dispatchEventToListeners(
            'EntryInvoked' /* Events.ENTRY_INVOKED */,
  ...
  toggleGroupExpand(groupIndex) {
    ...
    this.expandGroup(
      groupIndex,
      !this.rawTimelineData.groups[groupIndex].expanded /* setExpanded */,
    );
  }
  ...
  expandGroup(groupIndex, setExpanded = true, propagatedExpand = false) {
    ...
    if (!propagatedExpand) {
      const groupName = groups[groupIndex].name;
      const content = group.expanded
        ? i18nString(UIStrings.sExpanded, { PH1: groupName })
  ...
  onContextMenu(event) {
    ...
    // does.
    if (
      hoverType === 'INSIDE_TRACK_HEADER' /* HoverType.INSIDE_TRACK_HEADER */ &&
      this.#hasTrackConfigurationMode()
    ) {
    ...
    const entryIndexToUse = isFakedFromKeyboardPress
    ...
    this.contextMenu =
      this.dataProvider.customizedContextMenu?.(event, this.selectedEntryIndex, groupIndex) ??
    ...
    annotationSection.appendItem(
      i18nString(UIStrings.labelEntry),
      () => {
        this.dispatchEventToListeners(
          'EntryLabelAnnotationAdded' /* Events.ENTRY_LABEL_ANNOTATION_ADDED */,
    ...
    annotationSection.appendItem(
      i18nString(UIStrings.linkEntries),
      () => {
        this.dispatchEventToListeners(
          'EntriesLinkAnnotationCreated' /* Events.ENTRIES_LINK_ANNOTATION_CREATED */,
  ...
  #triggerContextMenuFromKeyPress() {
    ...
    const event = new MouseEvent('contextmenu', {
      clientX: x,
      clientY: y,
      detail: KEYBOARD_FAKED_CONTEXT_MENU_DETAIL,
    });
  ...
  drawTrackOnCanvas(trackName, context, minWidth) {
    ...
    const entryIndexIsInTrack = (index) => {
      ...
      return (
        timelineData.entryLevels[index] >= startLevel &&
        timelineData.entryLevels[index] < endLevel &&
        barWidth > minWidth
      );
    };
  ...
  selectFirstEntryInCurrentGroup() {
    ...
    if (
      this.keyboardFocusedGroup < allGroups.length - 1 &&
    ...
    if (eventName) {
      UI.ARIAUtils.LiveAnnouncer.alert(
        i18nString(UIStrings.eventSelectedFromGroup, {
          PH1: eventName,
          PH2: group.name,
        }),
      );
  ...
  getGroupIndexToSelect(offset) {
    ...
    do {
      ...
      groupWithSubNestingLevel =
        this.keyboardFocusedGroup !== -1 &&
        allGroups[groupIndexToSelect].style.nestingLevel >
          allGroups[this.keyboardFocusedGroup].style.nestingLevel;
    } while (
      groupIndexToSelect > 0 &&
      groupIndexToSelect < allGroups.length - 1 &&
  ...
  handleSelectionNavigation(event) {
    ...
    if (keyboardEvent.keyCode === keys.Left.code || keyboardEvent.keyCode === keys.Right.code) {
      ...
      let indexOnLevel = Platform.ArrayUtilities.lowerBound(
        levelIndexes,
        this.selectedEntryIndex,
        (a, b) => a - b,
      );
      ...
      if (indexOnLevel >= 0 && indexOnLevel < levelIndexes.length) {
        this.dispatchEventToListeners(
          'EntrySelected' /* Events.ENTRY_SELECTED */,
    ...
    if (keyboardEvent.keyCode === keys.Up.code || keyboardEvent.keyCode === keys.Down.code) {
      ...
      const entryTime =
        timelineData.entryStartTimes[this.selectedEntryIndex] +
        timelineData.entryTotalTimes[this.selectedEntryIndex] / 2;
      const levelIndexes = this.timelineLevels ? this.timelineLevels[level] : [];
      let indexOnLevel =
        Platform.ArrayUtilities.upperBound(levelIndexes, entryTime, timeComparator) - 1;
      if (!entriesIntersect(this.selectedEntryIndex, levelIndexes[indexOnLevel])) {
        ++indexOnLevel;
        if (
          indexOnLevel >= levelIndexes.length ||
  ...
  isMouseOverRevealChildrenArrow(x, index) {
    ...
    if (
      !this.entryHasDecoration(
        index,
        'HIDDEN_DESCENDANTS_ARROW' /* FlameChartDecorationType.HIDDEN_DESCENDANTS_ARROW */,
      )
    ) {
      return false;
    }
  ...
  entryIndexToCoordinates(entryIndex) {
    ...
    const x =
      this.chartViewport.timeToPosition(timelineData.entryStartTimes[entryIndex]) +
      canvasViewportOffsetX;
    const y =
      this.visibleLevelOffsets[timelineData.entryLevels[entryIndex]] -
      this.chartViewport.scrollOffset() +
    ...
    if (this.#groupTreeRoot) {
      ...
      for (let i = 0; i < sortedGroupIndexes.length; i++) {
        ...
        if (y >= this.groupOffsets[groupIndex] && y < this.groupOffsets[nextIndex]) {
          ...
          const headerRight =
            HEADER_LEFT_PADDING +
            (this.#inTrackConfigEditMode ? EDIT_MODE_TOTAL_ICON_WIDTH : 0) +
            this.labelWidthForGroup(context, groups[groupIndex]);
          context.restore();
          const mouseInHeaderRow =
            y >= this.groupOffsets[groupIndex] &&
            y < this.groupOffsets[groupIndex] + groups[groupIndex].style.height;
          if (this.#inTrackConfigEditMode) {
            if (mouseInHeaderRow) {
              if (UP_ICON_LEFT <= x && x < UP_ICON_LEFT + EDIT_ICON_WIDTH) {
                return {
                  groupIndex,
                  hoverType: 'TRACK_CONFIG_UP_BUTTON' /* HoverType.TRACK_CONFIG_UP_BUTTON */,
                };
              }
              if (DOWN_ICON_LEFT <= x && x < DOWN_ICON_LEFT + EDIT_ICON_WIDTH) {
                return {
                  groupIndex,
                  hoverType: 'TRACK_CONFIG_DOWN_BUTTON' /* HoverType.TRACK_CONFIG_DOWN_BUTTON */,
                };
              }
              if (HIDE_ICON_LEFT <= x && x < HIDE_ICON_LEFT + EDIT_ICON_WIDTH) {
                return {
                  groupIndex,
                  hoverType: groups[groupIndex].hidden
                    ? 'TRACK_CONFIG_SHOW_BUTTON' /* HoverType.TRACK_CONFIG_SHOW_BUTTON */
  ...
  enterTrackConfigurationMode() {
    ...
    this.dispatchEventToListeners(
      'TracksReorderStateChange' /* Events.TRACKS_REORDER_STATE_CHANGED */,
      true,
    );
    this.updateLevelPositions();
    this.draw();
  ...
  draw() {
    ...
    this.dispatchEventToListeners('LatestDrawDimensions' /* Events.LATEST_DRAW_DIMENSIONS */, {
      ...
      traceWindow: Trace.Helpers.Timing.traceWindowFromMilliSeconds(
    ...
    const allIndexes = Array.from(drawBatches.values())
      .map((x) => x.indexes)
      .flat();
    ...
    const drawAdjustedTime = (time) => {
      ...
      if (hasNextNavStartTime) {
        ...
        const nextNavStartTimeStartTimestamp = Trace.Helpers.Timing.microToMilli(
          nextNavStartTime.ts,
        );
  ...
  #drawBatchEvents(context, timelineData, color, indexes, outline) {
    ...
    if (outline) {
      ...
      const nearBlack =
        ThemeSupport.ThemeSupport.instance().getComputedValue('--ref-palette-neutral10');
      ...
      for (const decoration of decorationsForEvent) {
        switch (decoration.type) {
          case 'CANDY' /* FlameChartDecorationType.CANDY */: {
            ...
            context.fillStyle = this.#shouldDimEvent(entryIndex)
              ? this.candyStripePatternGray
              : this.candyStripePattern;
            context.fill();
            context.restore();
            break;
          }
  ...
  getDrawBatches(context, timelineData) {
    ...
    for (let level = 0; level < this.dataProvider.maxStackDepth(); ++level) {
      ...
      // skipping it.
      if (
        this.levelToOffset(level) + this.levelHeight(level) < top ||
  ...
  drawGroupHeaders(width, height) {
    ...
    context.translate(0, -top);
    context.font = this.#font;
    context.fillStyle = ThemeSupport.ThemeSupport.instance().getComputedValue(
    ...
    context.strokeStyle = ThemeSupport.ThemeSupport.instance().getComputedValue(
    ...
    this.forEachGroupInViewport((offset, index, group) => {
      ...
      const endLevel =
        nextGroup < groups.length
          ...
          : this.dataProvider.maxStackDepth();
    ...
    this.forEachGroupInViewport((offset, groupIndex, group) => {
      ...
      context.fillStyle =
        this.#inTrackConfigEditMode && group.hidden
      ...
      const titleStart =
        iconsWidth +
        EXPANSION_ARROW_INDENT * (group.style.nestingLevel + 1) +
        ARROW_SIDE / 2 +
      ...
      if (group.subtitle) {
        ...
        context.fillText(
          group.subtitle,
          titleStart + titleMetrics.width + PADDING_BETWEEN_TITLE_AND_SUBTITLE,
          y - 1,
        );
        context.font = this.#font;
      }
    ...
    this.forEachGroupInViewport((offset, index, group, _isFirst, groupHeight) => {
      if (this.isGroupFocused(index)) {
        ...
        context.fillRect(
          ...
          offset - lineWidth,
          lineWidth,
          groupHeight - group.style.padding + 2 * lineWidth,
        );
  ...
  drawEventTitles(context, timelineData, titleIndices, canvasWidth) {
    ...
    for (let i = 0; i < titleIndices.length; ++i) {
      ...
      if (text?.length) {
        ...
        const maxBarWidth =
          hasArrowDecoration && barWidth > barHeight * 2
            ? barWidth - textPadding - this.barHeight
  ...
  forEachGroup(callback) {
    ...
    for (let i = 0; i < groups.length; ++i) {
      ...
      const thisGroupVisible =
        !group.hidden && parentGroupVisible && (!this.isGroupCollapsible(i) || group.expanded);
      groupStack.push({
        nestingLevel: group.style.nestingLevel,
        visible: Boolean(thisGroupVisible),
      });
      if (!this.#groupTreeRoot) {
        return;
      }
  ...
  drawFlowEvents(context, timelineData) {
    ...
    context.scale(ratio, ratio);
    context.translate(0, -top);
    const arrowColor = ThemeSupport.ThemeSupport.instance().getComputedValue(
    ...
    for (let i = 0; i < td.initiatorsData.length; ++i) {
      ...
      const initiatorEndTime =
        entryStartTimes[initiatorsData.initiatorIndex] +
        entryTotalTimes[initiatorsData.initiatorIndex];
      ...
      if (initiatorsData.isInitiatorHidden) {
        const { circleEndX } = this.drawCircleAroundCollapseArrow(
          initiatorsData.initiatorIndex,
          context,
          timelineData,
        );
      ...
      if (initiatorsData.isEntryHidden) {
        const { circleStartX } = this.drawCircleAroundCollapseArrow(
          initiatorsData.eventIndex,
          context,
          timelineData,
        );
  ...
  drawCircleAroundCollapseArrow(entryIndex, context, timelineData) {
    ...
    context.restore();
    return {
      circleStartX: triangleCenterX - circleRadius,
      circleEndX: triangleCenterX + circleRadius,
    };
  }
  /**
  ...
  processTimelineData(timelineData) {
    ...
    for (let i = 0; i < groups.length; ++i) {
      ...
      const matchingConfig = this.#persistedGroupConfig?.find(
        (c) => c.trackName === groups[i].name,
      );
      // Priority:
    ...
    if (this.selectedGroupIndex === -1) {
      this.selectedGroupIndex = timelineData.selectedGroup
        ? groups.indexOf(timelineData.selectedGroup)
        : -1;
    }
  ...
  #traverseGroupTreeAndUpdateLevelPositionsForTheGroup(
    ...
    if (groupNode.index >= 0) {
      ...
      if (
        (this.#inTrackConfigEditMode && groups[groupNode.index].name) ||
        (!groups[groupNode.index].hidden &&
          parentGroupIsVisible &&
          !groups[groupNode.index].style.shareHeaderLine)
      ) {
    ...
    if (groupNode.index < 0) {
      ...
      const thisGroupIsExpanded = !(
        this.isGroupCollapsible(groupNode.index) && !groups[groupNode.index].expanded
      );
    ...
    for (const child of groupNode.children) {
      ...
      // For edit mode, we will show all the groups whose name are not empty.
      if (
        (this.#inTrackConfigEditMode && groups[child.index].name) ||
        (thisGroupLevelsAreVisible &&
          !groups[child.index]?.hidden &&
          child !== groupNode.children[0])
      ) {
  ...
  isGroupCollapsible(index) {
    ...
    if (style.collapsible === 2 /* GroupCollapsibleState.IF_MULTI_ROW */) {
      const nextRowStartLevel = isLastGroup
        ? this.dataProvider.maxStackDepth()
    ...
    const nextGroupLevel = isLastGroup
      ? this.dataProvider.maxStackDepth()
      : groups[index + 1].startLevel;
  ...
  groupIsLastVisibleTopLevel(groupIndex) {
    ...
    const group = this.rawTimelineData.groups[groupIndex];
    const visibleTopLevelGroupNumber = this.#groupTreeRoot?.children.filter(
      (track) => !this.rawTimelineData?.groups[track.index].hidden,
    ).length;
  ...
  timeToPositionClipped(time) {
    return Platform.NumberUtilities.clamp(
      this.chartViewport.timeToPosition(time),
      0,
      this.offsetWidth,
    );
  }
