...
const str_ = i18n.i18n.registerUIStrings(
  'ui/legacy/components/quick_open/FilteredListWidget.ts',
  UIStrings,
);
...
export class FilteredListWidget extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(provider, promptHistory, queryChangedCallback) {
    ...
    this.inputBoxElement.data = {
      ariaLabel: i18nString(UIStrings.quickOpenPrompt),
      prefix: '',
      suggestion: '',
    };
    ...
    this.list = new UI.ListControl.ListControl(
      this.items,
      this,
      UI.ListControl.ListMode.EqualHeightItems,
    );
    ...
    UI.ARIAUtils.setAutocomplete(
      ...
      'list' /* UI.ARIAUtils.AutocompleteInteractionModel.LIST */,
    );
  ...
  onMouseMove(event) {
    ...
    const text = Array.from(children)
      ...
      .map((e) =>
        e.classList.contains('new-badge') ? i18nString(UIStrings.newFeature) : e.textContent,
      )
      .join();
    if (text) {
      UI.ARIAUtils.LiveAnnouncer.alert(
        i18nString(UIStrings.sItemSOfS, {
          PH1: text,
  ...
  tabKeyPressed() {
    ...
    for (let i = this.promptHistory.length - 1; i >= 0; i--) {
      if (
        this.promptHistory[i] !== userEnteredText &&
        this.promptHistory[i].startsWith(userEnteredText)
      ) {
  ...
  filterItems() {
    ...
    const maxWorkItems = Platform.NumberUtilities.clamp(
      10,
      500,
      (this.provider.itemCount() / 10) | 0,
    );
    ...
    function scoreItems(fromIndex) {
      ...
      this.refreshListWithCurrentResult = this.refreshList.bind(
        this,
        bestItems,
        overflowItems,
        filteredItems,
    ...
    switch (keyboardEvent.key) {
      case Platform.KeyboardUtilities.ENTER_KEY:
        if (!keyboardEvent.isComposing) {
          // Ignore ENTER to confirm selection in an IME
