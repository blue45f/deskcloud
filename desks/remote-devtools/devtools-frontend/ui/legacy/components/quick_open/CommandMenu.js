...
export class CommandMenu {
      ...
      executeHandler: () => {
        ...
        if (setting.name === 'emulate-page-focus') {
          Host.userMetrics.actionTaken(
            Host.UserMetrics.Action.ToggleEmulateFocusedPageFromCommandMenu,
          );
        }
        if (reloadRequired) {
  ...
  static createActionCommand(options) {
    ...
    if (!category) {
      throw new Error(
        `Creating '${action.title()}' action command failed. Action has no category.`,
      );
    }
  ...
  static createRevealViewCommand(options) {
    ...
    if (!category) {
      throw new Error(
        `Creating '${title}' reveal view command failed. Reveal view has no category.`,
      );
    }
    let panelOrDrawer = undefined;
...
export class CommandMenuProvider extends Provider {
  ...
  attach() {
    ...
    for (const command of allCommands) {
      ...
      if (
        this.commands.find(
          ({ title, category }) => title === command.title && category === command.category,
        )
      ) {
        continue;
      }
  ...
  itemScoreAt(itemIndex, query) {
    ...
    let score = Diff.Diff.DiffWrapper.characterScore(
      query.toLowerCase(),
  ...
  renderItem(itemIndex, query) {
    const command = this.commands[itemIndex];
    const badge = command.featurePromotionId
      ? UI.UIUtils.maybeCreateNewBadge(command.featurePromotionId)
      : undefined;
    const deprecationWarning = command.deprecationWarning;
    // clang-format off
    return html` <devtools-icon name=${categoryIcons[command.category]}></devtools-icon>
      <div>
        <devtools-highlight
          type="markup"
          ranges=${FilteredListWidget.getHighlightRanges(command.title, query, true)}
        >
          ${command.title}
        </devtools-highlight>
        ${badge ?? nothing}
        ...
        ${deprecationWarning
          ? html` <span class="deprecated-tag" title=${deprecationWarning}>
