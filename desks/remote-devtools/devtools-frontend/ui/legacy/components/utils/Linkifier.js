...
export class Linkifier extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static setLinkDecorator(linkDecorator) {
    ...
    linkDecorator.addEventListener(
      'LinkIconChanged' /* LinkDecorator.Events.LINK_ICON_CHANGED */,
      onLinkIconChanged,
    );
  ...
  static bindBreakpoint(anchor, uiLocation) {
    ...
    const breakpoint =
      Breakpoints.BreakpointManager.BreakpointManager.instance().findBreakpoint(uiLocation);
    if (breakpoint) {
  ...
  maybeLinkifyScriptLocation(target, scriptId, sourceURL, lineNumber, options) {
    ...
    const updateDelegate = async (liveLocation) => {
      ...
      this.dispatchEventToListeners(
        'liveLocationUpdated' /* Events.LIVE_LOCATION_UPDATED */,
        liveLocation,
      );
    };
  ...
  linkifyScriptLocation(target, scriptId, sourceURL, lineNumber, options) {
    const scriptLink = this.maybeLinkifyScriptLocation(
      target,
      scriptId,
      sourceURL,
      lineNumber,
  ...
  static renderLink(text, className, options = {}) {
    const { maxLength, title, href, preventClick, tabStop, bypassURLTrimming, jslogContext } =
      options;
    const classes = {
      'devtools-link': true,
  ...
  static setTrimmedText(link, text, maxLength) {
    ...
    function splitMiddle(string, maxLength) {
      ...
      if (
        typeof codePointAtLeftIndex !== 'undefined' &&
        leftIndex > 0 &&
        codePointAtLeftIndex >= 0x10000
      ) {
        leftIndex--;
      }
      return [
        string.substring(0, leftIndex),
        string.substring(leftIndex, rightIndex),
        string.substring(rightIndex),
      ];
    }
  }
  ...
  static appendHiddenText(link, string) {
    const ellipsisNode = UI.UIUtils.createTextChild(
      link.createChild('span', 'devtools-link-ellipsis'),
      '…',
    );
  ...
  static linkHandlerSetting() {
    if (!linkHandlerSettingInstance) {
      linkHandlerSettingInstance = Common.Settings.Settings.instance().createSetting(
        'open-link-handler',
        i18nString(UIStrings.auto),
      );
    }
  ...
  static registerLinkHandler(registration) {
    for (const origin of linkHandlers.keys()) {
      ...
      if (existingHandler?.scheme === registration.scheme) {
        const schemeString = registration.scheme
  ...
  static linkActions(info) {
    ...
    if (info.uiLocation) {
      ...
      uiLocation = uiSourceCode
        ? uiSourceCode.uiLocation(info.lineNumber || 0, info.columnNumber || 0)
        : null;
    }
    ...
    if (revealable) {
      ...
      result.push({
        section: 'reveal',
        title: destination
          ? i18nString(UIStrings.revealInS, { PH1: destination })
          : i18nString(UIStrings.reveal),
        jslogContext: 'reveal',
  ...
  appendApplicableItems(_event, contextMenu, target) {
    ...
    for (const action of actions) {
      contextMenu
        ...
        .appendItem(action.title, action.handler, { jslogContext: action.jslogContext });
    }
  }
}
