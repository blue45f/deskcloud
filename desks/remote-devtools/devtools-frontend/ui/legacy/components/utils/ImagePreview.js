...
export class ImagePreview {
  static async build(
    originalImageURL,
    showDimensions,
    options = { align: 'center' /* Align.CENTER */ },
  ) {
    ...
    return await new Promise((resolve) => {
      ...
      function buildContent() {
        ...
        const renderedWidth = precomputedFeatures
          ? precomputedFeatures.renderedWidth
          : intrinsicWidth;
        const renderedHeight = precomputedFeatures
          ? precomputedFeatures.renderedHeight
          : intrinsicHeight;
        if (showDimensions) {
          const renderedRow = container.createChild('tr', 'row');
          renderedRow.createChild('td', `title ${align}`).textContent = i18nString(
            UIStrings.renderedSize,
          );
          renderedRow.createChild('td', 'description').textContent =
            `${renderedWidth} × ${renderedHeight} px`;
          const aspectRatioRow = container.createChild('tr', 'row');
          aspectRatioRow.createChild('td', `title ${align}`).textContent = i18nString(
            UIStrings.renderedAspectRatio,
          );
          ...
          if (renderedHeight !== intrinsicHeight || renderedWidth !== intrinsicWidth) {
            ...
            intrinsicRow.createChild('td', `title ${align}`).textContent = i18nString(
              UIStrings.intrinsicSize,
            );
            intrinsicRow.createChild('td', 'description').textContent =
              `${intrinsicWidth} × ${intrinsicHeight} px`;
            const intrinsicAspectRatioRow = container.createChild('tr', 'row');
            intrinsicAspectRatioRow.createChild('td', `title ${align}`).textContent = i18nString(
              UIStrings.intrinsicAspectRatio,
            );
        ...
        if (!options.hideFileData) {
          ...
          originalRow.createChild('td', `title ${align}`).textContent = i18nString(
            UIStrings.currentSource,
          );
          const sourceText = Platform.StringUtilities.trimMiddle(imageURL, 100);
          const sourceLink = originalRow
            .createChild('td', 'description description-link')
            .createChild('span', 'source-link');
  ...
  static defaultAltTextForImageURL(url) {
    ...
    const imageSourceText = parsedImageURL.isValid
      ? parsedImageURL.displayName
      : i18nString(UIStrings.unknownSource);
