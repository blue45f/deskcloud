...
export class ShortcutRegistry {
  ...
  keysForAction(actionId) {
    const keys = [...this.actionToShortcut.get(actionId)].flatMap((shortcut) =>
      shortcut.descriptors.map((descriptor) => descriptor.key),
    );
    return keys;
  }
  ...
  keyAndModifiersForAction(actionId) {
    for (const keys of this.keysForAction(actionId)) {
      ...
      if (key) {
        return {
          key,
          modifier: KeyboardShortcut.modifierValueToModifier(modifiers) || Modifiers.None,
        };
      }
    }
    return undefined;
  ...
  getShortcutListener(handlers) {
    const shortcuts = Object.keys(handlers).flatMap((action) => [
      ...this.actionToShortcut.get(action),
    ]);
    ...
    return (event) => {
      ...
      const keyMap = this.activePrefixKey
        ? allowlistKeyMap.getNode(this.activePrefixKey.key())
        : allowlistKeyMap;
      if (!keyMap) {
        return;
      }
  ...
  async handleKey(key, domKey, event, handlers) {
    ...
    if (
      (!hasHandlersOrPrefixKey && isPossiblyInputKey()) ||
      !maybeHasActions ||
    ...
    function isPossiblyInputKey() {
      ...
      if (
        (keyModifiers & (modifiers.Ctrl.value | modifiers.Alt.value)) ===
        (modifiers.Ctrl.value | modifiers.Alt.value)
      ) {
        return Host.Platform.isWin();
      }
      return (
        !hasModifier(modifiers.Ctrl.value) &&
        !hasModifier(modifiers.Alt.value) &&
  ...
  registerUserShortcut(shortcut) {
    for (const otherShortcut of this.disabledDefaultShortcutsForAction.get(shortcut.action)) {
      if (
        otherShortcut.descriptorsMatch(shortcut.descriptors) &&
  ...
  removeShortcut(shortcut) {
    if (
      shortcut.type === 'DefaultShortcut' /* Type.DEFAULT_SHORTCUT */ ||
      shortcut.type === 'KeybindSetShortcut' /* Type.KEYBIND_SET_SHORTCUT */
    ) {
  ...
  registerBindings() {
    ...
    for (const userShortcut of userShortcuts) {
      ...
      if (shortcut.type === 'DisabledDefault' /* Type.DISABLED_DEFAULT */) {
        ...
        if (ForwardedActions.has(shortcut.action)) {
          forwardedKeys.push(
            ...shortcut.descriptors.map((descriptor) =>
              KeyboardShortcut.keyCodeAndModifiersFromKey(descriptor.key),
            ),
          );
        }
      ...
      for (let i = 0; bindings && i < bindings.length; ++i) {
        ...
        if (shortcutDescriptors.length > 0) {
          ...
          if (ForwardedActions.has(actionId)) {
            forwardedKeys.push(
              ...shortcutDescriptors.map((shortcut) =>
                KeyboardShortcut.keyCodeAndModifiersFromKey(shortcut.key),
              ),
            );
          }
          if (!keybindSets) {
            ...
            this.registerShortcut(
              new KeyboardShortcut(
                shortcutDescriptors,
                actionId,
                'DefaultShortcut' /* Type.DEFAULT_SHORTCUT */,
              ),
            );
          } else {
            ...
            this.registerShortcut(
              new KeyboardShortcut(
                shortcutDescriptors,
                actionId,
                'KeybindSetShortcut' /* Type.KEYBIND_SET_SHORTCUT */,
                new Set(keybindSets),
              ),
            );
          }
    ...
    Host.InspectorFrontendHost.InspectorFrontendHostInstance.setWhitelistedShortcuts(
      JSON.stringify(forwardedKeys),
    );
