...
export class TabbedPane extends Common.ObjectWrapper.eventMixin(VBox) {
  ...
  constructor(element) {
    ...
    this.headerContentsElement = this.#headerElement.createChild(
      'div',
    ...
    ZoomManager.instance().addEventListener(
      'ZoomChanged' /* ZoomManagerEvents.ZOOM_CHANGED */,
      this.zoomChanged,
      this,
    );
    this.makeTabSlider();
    if (Annotations.AnnotationRepository.annotationsEnabled()) {
      Annotations.AnnotationRepository.instance().addEventListener(
        'AnnotationAdded' /* Annotations.Events.ANNOTATION_ADDED */,
        this.#onUpdateAnnotations,
        this,
      );
      Annotations.AnnotationRepository.instance().addEventListener(
        'AnnotationDeleted' /* Annotations.Events.ANNOTATION_DELETED */,
        this.#onUpdateAnnotations,
        this,
      );
      Annotations.AnnotationRepository.instance().addEventListener(
        'AllAnnotationsDeleted' /* Annotations.Events.ALL_ANNOTATIONS_DELETED */,
        this.#onUpdateAnnotations,
        this,
      );
    }
  ...
  appendTab(
    ...
    const tab = new TabbedPaneTab(
      this,
      id,
      tabTitle,
      closeable,
      Boolean(isPreviewFeature),
      view,
      tabTooltip,
      jslogContext,
    );
  ...
  calculateConstraints() {
    ...
    const minContentConstraints = new Geometry.Constraints(
      new Geometry.Size(0, 0),
      new Geometry.Size(50, 50),
    );
  ...
  updateTabAnnotationIcons() {
    ...
    for (const tab of this.tabs) {
      ...
      const showTabAnnotationIcon =
        annotations.getAnnotationDataByType(primaryType).length > 0 ||
  ...
  performUpdate() {
    ...
    if (!this.#tabs.length) {
      ...
      if (this.placeholderElement && !this.placeholderContainerElement) {
        this.placeholderContainerElement = this.#contentElement.createChild(
          'div',
  ...
  createDropDownButton() {
    ...
    dropDownContainer.setAttribute(
      'jslog',
      `${VisualLogging.dropDown('more-tabs').track({ click: true })}`,
    );
    ...
    dropDownContainer.addEventListener('mousedown', (event) => {
      ...
      this.triggerDropDownTimeout = globalThis.setTimeout(
        this.dropDownClicked.bind(this, event),
        200,
      );
    });
  ...
  dropDownClicked(event) {
    ...
    for (const tab of this.#tabs) {
      ...
      if (this.numberOfTabsShown() === 0 && this.tabsHistory[0] === tab) {
        menu
          .defaultSection()
          .appendCheckboxItem(tab.title, this.dropDownMenuItemSelected.bind(this, tab), {
            checked: true,
            jslogContext: tab.jslogContext,
          });
      } else {
        menu
          .defaultSection()
          .appendItem(tab.title, this.dropDownMenuItemSelected.bind(this, tab), {
            jslogContext: tab.jslogContext,
          });
      }
    }
    void menu
      .show()
      .then(() => ARIAUtils.setExpanded(this.dropDownButton, menu.isHostedMenuOpen()));
  }
  ...
  updateTabsDropDown() {
    const tabsToShowIndexes = this.tabsToShowIndexes(
      this.#tabs,
      this.tabsHistory,
      this.totalWidth(),
  ...
  updateWidths() {
    ...
    const maxWidth = this.shrinkableTabs
  ...
  tabsToShowIndexes(tabsOrdered, tabsHistory, totalWidth, measuredDropDownButtonWidth) {
    ...
    if (this.lastSelectedOverflowTab !== undefined) {
      tabsToLookAt.unshift(
        tabsToLookAt.splice(tabsToLookAt.indexOf(this.lastSelectedOverflowTab), 1)[0],
      );
    }
  ...
  updateTabSlider() {
    ...
    const sliderWidth = this.currentTab.shown
...
export class TabbedPaneTab {
  ...
  isCloseIconClicked(element) {
    return (
      element?.classList.contains('tabbed-pane-close-button') ||
      element?.parentElement?.classList.contains('tabbed-pane-close-button') ||
      false
    );
  }
  tabKeyDown(ev) {
  ...
  tabContextMenu(event) {
    ...
    if (this.closeable) {
      contextMenu
        .defaultSection()
        .appendItem(i18nString(UIStrings.close), close.bind(this), { jslogContext: 'close' });
      contextMenu
        .defaultSection()
        .appendItem(i18nString(UIStrings.closeOthers), closeOthers.bind(this), {
          jslogContext: 'close-others',
        });
      contextMenu
        .defaultSection()
        .appendItem(i18nString(UIStrings.closeTabsToTheRight), closeToTheRight.bind(this), {
          jslogContext: 'close-tabs-to-the-right',
        });
      contextMenu
        .defaultSection()
        .appendItem(i18nString(UIStrings.closeAll), closeAll.bind(this), {
    ...
    if (tabIndex > 0) {
      contextMenu
        .defaultSection()
        .appendItem(i18nString(UIStrings.moveTabLeft), moveTabBackward.bind(this, tabIndex), {
    ...
    if (tabIndex < this.tabbedPane.tabsElement.childNodes.length - 1) {
      contextMenu
        .defaultSection()
        .appendItem(i18nString(UIStrings.moveTabRight), moveTabForward.bind(this, tabIndex), {
  ...
  tabDragging(event) {
    ...
    for (let i = 0; i < tabElements.length; ++i) {
      ...
      const intersects =
        tabElement.offsetLeft + tabElement.clientWidth > this.#tabElement.offsetLeft &&
    ...
    registerWidgetConfig(
      ...
      widgetConfig((element) => {
        ...
        widget.addEventListener(Events.TabSelected, () => {
          ...
          this.dispatchEvent(
            new CustomEvent('select', { detail: { tabId: widget.selectedTabId } }),
          );
        });
