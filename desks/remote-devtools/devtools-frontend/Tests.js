...
(function createTestSuite(window) {
  ...
  TestSuite.prototype.setupLegacyFilesForTest = async function () {
    ...
    [Common, HostModule, Root, SDK, UI, Workspace] = await Promise.all([
    ...
    globalThis.Host = {};
    globalThis.Host.InspectorFrontendHost =
      HostModule.InspectorFrontendHost.InspectorFrontendHostInstance;
  ...
  TestSuite.prototype.testPauseWhenLoadingDevTools = function () {
    const debuggerModel = SDK.TargetManager.TargetManager.instance()
      .primaryPageTarget()
  ...
  TestSuite.prototype.testNetworkSize = function () {
    ...
    this.addSniffer(
      SDK.NetworkManager.NetworkDispatcher.prototype,
  ...
  TestSuite.prototype.testNetworkSyncSize = function () {
    ...
    test.evaluateInConsole_(
      'let xhr = new XMLHttpRequest(); xhr.open("GET", "chunked", false); xhr.send(null);',
      function () {},
    );
  ...
  TestSuite.prototype.testNetworkRawHeadersText = function () {
    ...
    function finishRequest(request, finishTime) {
      ...
      test.assertEquals(
        112,
        request.responseHeadersText.substring(index).length,
  ...
  TestSuite.prototype.testPushTimes = function (url) {
    ...
    function finishRequest(request, finishTime) {
      ...
      test.assertTrue(
        typeof request.timing.pushEnd === 'number',
        `pushEnd is invalid: ${request.timing.pushEnd}`,
      );
      test.assertTrue(
        request.timing.pushStart < request.startTime,
        'pushStart should be before startTime',
      );
      if (request.url().endsWith('?pushUseNullEndTime')) {
        test.assertTrue(
          request.timing.pushEnd === 0,
          `pushEnd should be 0 but is ${request.timing.pushEnd}`,
        );
      } else {
        test.assertTrue(
        ...
        test.assertTrue(
          request.timing.pushEnd < request.endTime,
          'pushEnd should be before endTime',
        );
        test.assertTrue(
          request.startTime < request.timing.pushEnd,
          'pushEnd should be after startTime',
        );
      }
  ...
  TestSuite.prototype.testPauseInSharedWorkerInitialization2 = function () {
    ...
    function callback() {
      const debuggerModel = SDK.TargetManager.TargetManager.instance().models(
  ...
  TestSuite.prototype.testDeviceMetricsOverrides = function () {
    ...
    function step1() {
      testOverrides(
        ...
        { width: 1200, height: 1000, deviceScaleFactor: 1 },
        step2,
      );
    }
    ...
    function step3() {
      testOverrides(
        ...
        { width: 1200, height: 1000, deviceScaleFactor: 3 },
        step4,
      );
    }
  ...
  TestSuite.prototype.testKeyEventUnhandled = function () {
    function onKeyEventUnhandledKeyDown(event) {
      ...
      Host.InspectorFrontendHost.events.removeEventListener(
        Host.InspectorFrontendHostAPI.Events.KeyEventUnhandled,
        onKeyEventUnhandledKeyDown,
        this,
      );
      Host.InspectorFrontendHost.events.addEventListener(
        Host.InspectorFrontendHostAPI.Events.KeyEventUnhandled,
        onKeyEventUnhandledKeyUp,
        this,
      );
      SDK.TargetManager.TargetManager.instance()
        .primaryPageTarget()
  ...
  TestSuite.prototype.testDispatchKeyEventDoesNotCrash = function () {
    SDK.TargetManager.TargetManager.instance()
      .primaryPageTarget()
      ?.inputAgent()
      .invoke_dispatchKeyEvent({ type: 'rawKeyDown', windowsVirtualKeyCode: 0x23, key: 'End' });
    SDK.TargetManager.TargetManager.instance()
      .primaryPageTarget()
      ?.inputAgent()
      .invoke_dispatchKeyEvent({ type: 'keyUp', windowsVirtualKeyCode: 0x23, key: 'End' });
  };
  ...
  TestSuite.prototype.testSettings = function () {
    ...
    function createSettings() {
      const localSetting = Common.Settings.Settings.instance().createLocalSetting(
        'local',
        undefined,
      );
  ...
  TestSuite.prototype.testInspectedElementIs = async function (nodeName) {
    ...
    if (!Elements.ElementsPanel.ElementsPanel.firstInspectElementNodeNameForTest) {
      await new Promise((f) =>
        this.addSniffer(
          ...
          'firstInspectElementCompletedForTest',
          f,
        ),
      );
    }
    this.assertEquals(
      nodeName,
      Elements.ElementsPanel.ElementsPanel.firstInspectElementNodeNameForTest,
    );
    this.releaseControl();
  };
  ...
  TestSuite.prototype.testCreateBrowserContext = async function (url) {
    ...
    SDK.TargetManager.TargetManager.instance().observeTargets({
      targetAdded: () => {},
      targetRemoved: (target) => removedTargets.push(target),
    });
    await Promise.all([
  ...
  async function createIsolatedTarget(url, opt_browserContextIds) {
    ...
    const target = SDK.TargetManager.TargetManager.instance()
      .targets()
  ...
  TestSuite.prototype.testInputDispatchEventsToOOPIF = async function () {
    ...
    const runtimeAgent = SDK.TargetManager.TargetManager.instance()
      .primaryPageTarget()
      ?.runtimeAgent();
    ...
    parentFrameOutput =
      'Event type: mousedown button: 0 x: 10 y: 10 Event type: mouseup button: 0 x: 10 y: 20';
    this.assertEquals(
      parentFrameOutput,
      await takeLogs(SDK.TargetManager.TargetManager.instance().targets()[0]),
    );
    childFrameOutput =
      'Event type: mousedown button: 0 x: 30 y: 40 Event type: mouseup button: 0 x: 30 y: 50';
    this.assertEquals(
      childFrameOutput,
      await takeLogs(SDK.TargetManager.TargetManager.instance().targets()[1]),
    );
    ...
    await inputAgent.invoke_dispatchTouchEvent({
      type: 'touchStart',
  ...
  TestSuite.prototype.testLoadResourceForFrontend = async function (baseURL, fileURL) {
    ...
    await testCase(
      baseURL + 'echoheader?x-devtools-test',
      { 'x-devtools-test': 'Foo' },
      200,
      ['cache-control'],
      'Foo',
    ...
    await testCase(
      baseURL + 'set-header?pragma:%20no-cache',
      undefined,
      200,
      ['pragma'],
      'pragma: no-cache',
  ...
  TestSuite.prototype.testExtensionWebSocketUserAgentOverride = async function (websocketPort) {
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      ...
      onRequestUpdated.bind(this),
    );

    this.evaluateInConsole_(`new WebSocket('ws://127.0.0.1:${websocketPort}')`, () => {});
  };

  TestSuite.prototype.testExtensionWebSocketOfflineNetworkConditions = async function (
    websocketPort,
  ) {
    SDK.NetworkManager.MultitargetNetworkManager.instance().requestConditions.conditionsEnabled = true;
    SDK.NetworkManager.MultitargetNetworkManager.instance().setNetworkConditions(
    ...
    this.evaluateInConsole_(
      `new WebSocket('ws://127.0.0.1:${websocketPort}/echo-with-no-extension')`,
      () => {},
    );
  };
  ...
  TestSuite.prototype.testDoesNotCrashOnSourceMapsFromUnknownScheme = function () {
    this.evaluateInConsole_(
      'console.log(1) //# sourceMappingURL=invalid-scheme://source.map',
      () => {},
    );
  };
  ...
  TestSuite.prototype.evaluateInConsole_ = function (code, callback) {
    function innerEvaluate() {
      ...
      this.addSniffer(
        Console.ConsoleView.prototype,
        'consoleMessageAddedForTest',
  ...
  TestSuite.prototype._waitForTargets = function (n, callback) {
    ...
    function checkTargets() {
      if (SDK.TargetManager.TargetManager.instance().targets().length >= n) {
        ...
        this.addSniffer(
          SDK.TargetManager.TargetManager.prototype,
          'createTarget',
          checkTargets.bind(this),
        );
      }
    }
  };
  ...
  TestSuite.prototype._waitForExecutionContexts = function (n, callback) {
    const runtimeModel = SDK.TargetManager.TargetManager.instance()
      .primaryPageTarget()
    ...
    function checkForExecutionContexts() {
      if (runtimeModel.executionContexts().length >= n) {
        ...
        this.addSniffer(
          SDK.RuntimeModel.RuntimeModel.prototype,
