    ...
    if (data.type === 'main-window-alive') {
      ...
      if (pendingActivationSessionId) {
        this.#syncChannel.postMessage({
          type: 'select-tab',
          sessionId: pendingActivationSessionId,
        });
  ...
  setNode(node) {
    if (this.#node) {
      this.#node
        ...
        .overlayModel()
        .removeEventListener(
          'InspectPanelShowRequested' /* SDK.OverlayModel.Events.INSPECT_PANEL_SHOW_REQUESTED */,
    ...
    const nodeDescriptionElement = document.querySelector(
      ...
      for await (const result of this.#agent.run(query, { selected: this.#nodeContext })) {
        switch (result.type) {
          ...
          case 'error' /* ResponseType.ERROR */:
            ...
            this.#syncChannel.postMessage({
              type: 'update-last-message',
              text: this.#formatError(result.error),
              sessionId: this.#backendNodeId,
            });
            break;
        ...
        async () => {
          ...
          if (overlayModel) {
            overlayModel.addEventListener(
              'InspectedElementWindowRestored' /* SDK.OverlayModel.Events.INSPECTED_ELEMENT_WINDOW_RESTORED */,
