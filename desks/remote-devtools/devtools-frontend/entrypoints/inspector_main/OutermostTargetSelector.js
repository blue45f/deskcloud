...
const str_ = i18n.i18n.registerUIStrings(
  'entrypoints/inspector_main/OutermostTargetSelector.ts',
  UIStrings,
);
...
export class OutermostTargetSelector {
  ...
  constructor() {
    ...
    this.listItems.addEventListener('ItemsReplaced' /* UI.ListModel.Events.ITEMS_REPLACED */, () =>
  ...
  createElementForItem(item) {
    ...
    const shadowRoot = UI.UIUtils.createShadowRootWithCoreStyles(element, {
      cssFile: outermostTargetSelectorStyles,
    });
    const title = shadowRoot.createChild('div', 'title');
    UI.UIUtils.createTextChild(
      title,
      Platform.StringUtilities.trimEndWithMaxLength(this.titleFor(item), 100),
    );
  ...
  itemSelected(item) {
    const title = item
      ? i18nString(UIStrings.targetS, { PH1: this.titleFor(item) })
      : i18nString(UIStrings.targetNotSelected);
    this.#toolbarItem.setTitle(title);
    if (
      item &&
      item !== UI.Context.Context.instance().flavor(SDK.Target.Target)?.outermostTarget()
    ) {
