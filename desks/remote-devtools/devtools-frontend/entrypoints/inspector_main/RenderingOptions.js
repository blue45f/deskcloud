...
const str_ = i18n.i18n.registerUIStrings(
  'entrypoints/inspector_main/RenderingOptions.ts',
  UIStrings,
);
...
export class RenderingOptionsView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.element.setAttribute(
      'jslog',
      `${VisualLogging.panel('rendering').track({ resize: true })}`,
    );
    this.#appendCheckbox(
    ...
    const jpegXlFormatDisabledSetting =
      Common.Settings.Settings.instance().moduleSetting('jpeg-xl-format-disabled');
    ...
    switch (actionId) {
      case 'rendering.toggle-prefers-color-scheme': {
        ...
        const current = options.findIndex(
          (x) => x === emulatedCSSMediaFeaturePrefersColorSchemeSetting.get() || '',
        );
