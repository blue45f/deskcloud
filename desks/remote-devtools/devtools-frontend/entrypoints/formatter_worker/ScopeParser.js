...
export class Scope {
  ...
  finalizeToParent(isFunctionScope) {
    ...
    for (const [name, defUse] of this.variables.entries()) {
      if (
        defUse.definitionKind === 0 /* DefinitionKind.NONE */ ||
...
export class ScopeVariableAnalysis {
    ...
    switch (node.type) {
      ...
      case 'ArrowFunctionExpression': {
        this.#pushScope(
          node.start,
          node.end,
          4 /* ScopeKind.ARROW_FUNCTION */,
          undefined,
          mappingLocationsForArrowFunctions(node, this.#sourceText),
        );
        node.params.forEach(
          this.#processNodeAsDefinition.bind(this, 2 /* DefinitionKind.VAR */, false),
        );
      ...
      case 'VariableDeclaration': {
        const definitionKind =
          node.kind === 'var' ? 2 /* DefinitionKind.VAR */ : 1; /* DefinitionKind.LET */
      ...
      case 'FunctionDeclaration':
        ...
        node.params.forEach(
          this.#processNodeAsDefinition.bind(this, 1 /* DefinitionKind.LET */, false),
        );
  ...
  #addVariable(
    name,
    offset,
    definitionKind = 0 /* DefinitionKind.NONE */,
    isShorthandAssignmentProperty = false,
  ) {
...
function mappingLocationsForArrowFunctions(node, sourceText) {
  ...
  const searchArrowStartPos = node.params.length
