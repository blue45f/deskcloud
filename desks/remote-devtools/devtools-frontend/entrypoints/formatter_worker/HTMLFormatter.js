...
function scriptTagIsJSON(element) {
  ...
  return ['application/json', 'importmap', 'speculationrules'].includes(type.trim());
}
...
export class HTMLModel {
  ...
  constructor(text) {
    ...
    this.#document.closeTag = new Tag(
      'document',
      text.length,
      text.length,
      new Map(),
