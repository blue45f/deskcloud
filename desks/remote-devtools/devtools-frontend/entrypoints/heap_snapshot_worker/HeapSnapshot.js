...
export class HeapSnapshotNode {
  ...
  dominatorIndex() {
    ...
    return (
      this.snapshot.dominatorsTree[this.nodeIndex / this.snapshot.nodeFieldCount] * nodeFieldCount
    );
  }
  edges() {
  ...
  reportProblem(error) {
    ...
    if (this.#dispatcher) {
      this.#dispatcher.sendEvent(
        HeapSnapshotModel.HeapSnapshotModel.HeapSnapshotProgressEvent.BrokenSnapshot,
        error,
      );
    }
  }
...
export class HeapSnapshot {
  ...
  constructor(profile, progress) {
    ...
    this.#edgeNamesThatAreNotWeakMaps = Platform.TypedArrayUtilities.createBitVector(
      this.strings.length,
    );
  }
  ...
  buildEdgeIndexes() {
    ...
    for (let nodeOrdinal = 0, edgeIndex = 0; nodeOrdinal < nodeCount; ++nodeOrdinal) {
      ...
      edgeIndex +=
        nodes.getValue(nodeOrdinal * nodeFieldCount + nodeEdgeCountOffset) * edgeFieldsCount;
    }
  }
  static buildRetainers(inputs) {
    const { edgeToNodeOrdinals, firstEdgeIndexes, nodeCount, edgeFieldsCount, nodeFieldCount } =
      inputs;
      ...
      case 'objectsRetainedByEventHandlers': {
        ...
        for (let i = 0; i < this.nodeCount; ++i) {
          ...
          if (node.rawName() === 'V8EventListener') {
            ...
            for (
              let childEdgeIt = callbackNode.edges();
              childEdgeIt.hasNext();
              childEdgeIt.next()
            ) {
  ...
  applyInterfaceDefinitions(definitions) {
    ...
    for (let it = this.allNodes(); it.hasNext(); it.next()) {
      ...
      let classIndex =
        match === initialMatch ? node.rawNameIndex() : this.#interfaceNames.get(match.name);
  ...
  propagateDOMState() {
    ...
    const processNode = function (snapshot, nodeOrdinal, newState) {
      ...
      if (
        snapshot.nodes.getValue(nodeIndex + snapshot.nodeTypeOffset) !== snapshot.nodeNativeType
      ) {
    ...
    const propagateState = function (snapshot, parentNodeOrdinal, newState) {
      snapshot.iterateFilteredChildren(
        parentNodeOrdinal,
        (edgeType) =>
          ![snapshot.edgeHiddenType, snapshot.edgeInvisibleType, snapshot.edgeWeakType].includes(
            edgeType,
          ),
  ...
  buildSamples() {
    ...
    this.#samples = new HeapSnapshotModel.HeapSnapshotModel.Samples(
      timestamps,
      lastAssignedIds,
      sizeForRange,
    );
...
export class HeapSnapshotItemProvider {
  ...
  serializeItemsRange(begin, end) {
    ...
    return new HeapSnapshotModel.HeapSnapshotModel.ItemsRange(
      begin,
      end,
  ...
  #plainObjectName() {
    ...
    while (edgeIndexFromStart <= edgeIndexFromEnd) {
      ...
      if (
        ...
        categoryNameStart.length + categoryNameEnd.length + formatted.length > 100
      ) {
        break;
      }
      if (nextFromEnd) {
