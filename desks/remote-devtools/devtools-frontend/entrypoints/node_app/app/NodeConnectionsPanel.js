...
const str_ = i18n.i18n.registerUIStrings(
  'entrypoints/node_app/app/NodeConnectionsPanel.ts',
  UIStrings,
);
...
export class NodeConnectionsPanel extends UI.Panel.Panel {
  ...
  constructor() {
    ...
    this.#networkDiscoveryView = new NodeConnectionsView((config) => {
      ...
      Host.InspectorFrontendHost.InspectorFrontendHostInstance.setDevicesDiscoveryConfig(
        this.#config,
      );
    });
...
export class NodeConnectionsView extends UI.Widget.VBox {
  ...
  constructor(callback) {
    ...
    networkDiscoveryFooter.appendChild(
      uiI18n.getFormatLocalizedString(str_, UIStrings.specifyNetworkEndpointAnd, {
  ...
  #createEditor() {
    ...
    const input = editor.createInput(
      'address',
      'text',
      i18nString(UIStrings.networkAddressEgLocalhost),
      addressValidator,
    );
    fields
      .createChild('div', 'network-discovery-value network-discovery-address')
      .appendChild(input);
    return editor;
