...
let RecorderController = class RecorderController extends LitElement {
  ...
  #disableRecorderImportWarningSetting = Common.Settings.Settings.instance().createSetting(
    ...
    false,
    'Synced' /* Common.Settings.SettingStorageType.SYNCED */,
  );
  ...
  constructor() {
    ...
    const textEditorIndent = Common.Settings.Settings.instance()
    ...
    extensionManager.addEventListener(
      'extensionsUpdated' /* Extensions.ExtensionManager.Events.EXTENSIONS_UPDATED */,
      (event) => {
  ...
  #updateExtensions(extensions) {
    this.extensionConverters = extensions
      .filter((extension) => extension.getCapabilities().includes('export'))
      .map((extension, idx) => {
        return new Converters.ExtensionConverter.ExtensionConverter(idx, extension);
      });
    this.replayExtensions = extensions.filter((extension) =>
      extension.getCapabilities().includes('replay'),
    );
  }
    ...
    for (let i = navigateStepIdx - 1; i >= 0; i--) {
      ...
      if (!settings.networkConditionsSettings && step.type === 'emulateNetworkConditions') {
        ...
        for (const preset of [
          ...
          if (
            SDK.NetworkManager.networkConditionsEqual(
            ...
            settings.networkConditionsSettings.title =
              preset.title instanceof Function ? preset.title() : preset.title;
  ...
  async #onPlayViaExtension(extension) {
    ...
    const promise = pluginManager.once(
      'showViewRequested' /* PublicExtensions.RecorderPluginManager.Events.SHOW_VIEW_REQUESTED */,
    );
    ...
    Host.userMetrics.recordingReplayStarted(
      3 /* Host.UserMetrics.RecordingReplayStarted.REPLAY_VIA_EXTENSION */,
    );
  }
  async #onPlayRecording(event) {
    ...
    Host.userMetrics.recordingReplayStarted(
      event.targetPanel !== 'chrome-recorder' /* Components.RecordingView.TargetPanel.DEFAULT */
    ...
    this.recordingPlayer = new Models.RecordingPlayer.RecordingPlayer(this.currentRecording.flow, {
      speed: event.speed,
      breakpointIndexes: this.#stepBreakpointIndexes,
    });
    const withPerformanceTrace =
      event.targetPanel === 'timeline'; /* Components.RecordingView.TargetPanel.PERFORMANCE_PANEL */
    const sectionsWithScreenshot = new Set();
    this.recordingPlayer.addEventListener(
      'Step' /* Models.RecordingPlayer.Events.STEP */,
      async ({ data: { step, resolve } }) => {
    ...
    this.recordingPlayer.addEventListener(
      'Continue' /* Models.RecordingPlayer.Events.CONTINUE */,
      () => {
        this.#replayState.isPausedOnBreakpoint = false;
        this.requestUpdate();
      },
    ...
    this.recordingPlayer.addEventListener(
      'Error' /* Models.RecordingPlayer.Events.ERROR */,
      ({ data: error }) => {
        ...
        if (errorMessage.startsWith('could not find element')) {
          Host.userMetrics.recordingReplayFinished(
            2 /* Host.UserMetrics.RecordingReplayFinished.TIMEOUT_ERROR_SELECTORS */,
          );
        } else if (errorMessage.startsWith('waiting for target failed')) {
          Host.userMetrics.recordingReplayFinished(
            3 /* Host.UserMetrics.RecordingReplayFinished.TIMEOUT_ERROR_TARGET */,
          );
        } else {
          Host.userMetrics.recordingReplayFinished(
            4 /* Host.UserMetrics.RecordingReplayFinished.OTHER_ERROR */,
          );
        }
    ...
    this.recordingPlayer.addEventListener('Done' /* Models.RecordingPlayer.Events.DONE */, () => {
      ...
      this.dispatchEvent(new Events.ReplayFinishedEvent());
      Host.userMetrics.recordingReplayFinished(
        1 /* Host.UserMetrics.RecordingReplayFinished.SUCCESS */,
      );
    });
    ...
    switch (event.targetPanel) {
      case 'timeline' /* Components.RecordingView.TargetPanel.PERFORMANCE_PANEL */:
        performanceTracing = new Tracing.PerformanceTracing.PerformanceTracing(
          this.#getMainTarget(),
          {
            tracingBufferUsage() {},
    ...
    if (performanceTracing) {
      ...
      if (
        event.targetPanel ===
        'timeline' /* Components.RecordingView.TargetPanel.PERFORMANCE_PANEL */
      ) {
  ...
  async #handleRecordingChanged(event) {
    ...
    const recording = {
      ...
      flow: {
        ...
        steps: this.currentRecording.flow.steps.map((step) =>
          step === event.currentStep ? event.newStep : step,
        ),
      },
    };
    this.#setCurrentRecording(
      await this.#storage.upsertRecording(recording.flow, recording.storageName),
  ...
  async #handleStepAdded(event) {
    ...
    const indexToInsertAt =
      currentIndex +
      (position === 'before' /* Components.StepView.AddStepPosition.BEFORE */ ? 0 : 1);
    ...
    this.#stepBreakpointIndexes = new Set(
      [...this.#stepBreakpointIndexes.values()].map((breakpointIndex) => {
  ...
  async #handleRecordingTitleChanged(title) {
    ...
    this.#setCurrentRecording(
      await this.#storage.upsertRecording(flow, this.currentRecording.storageName),
    );
  }
  async #handleStepRemoved(event) {
    ...
    this.#stepBreakpointIndexes = new Set(
      [...this.#stepBreakpointIndexes.values()]
        .map((breakpointIndex) => {
  ...
  async #onNetworkConditionsChanged(data) {
    ...
    const navigateIdx = this.currentRecording.flow.steps.findIndex(
  ...
  async #onRecordingStarted(data) {
    ...
    this.currentRecordingSession = new Models.RecordingSession.RecordingSession(
      this.#getMainTarget(),
      {
        title: data.name,
        selectorAttribute: data.selectorAttribute,
        selectorTypesToRecord: data.selectorTypesToRecord.length
          ? data.selectorTypesToRecord
          : Object.values(Models.Schema.SelectorType),
      },
    );
    this.#setCurrentRecording(
      await this.#storage.upsertRecording(this.currentRecordingSession.cloneUserFlow()),
    );
    ...
    this.currentRecordingSession.addEventListener(
      'recordingupdated' /* Models.RecordingSession.Events.RECORDING_UPDATED */,
      async ({ data }) => {
        if (!this.currentRecording) {
          throw new Error('No current recording found');
        }
        this.#setCurrentRecording(
          await this.#storage.upsertRecording(data, this.currentRecording.storageName),
        );
    ...
    this.currentRecordingSession.addEventListener(
      'recordingstopped' /* Models.RecordingSession.Events.RECORDING_STOPPED */,
      async ({ data }) => {
        if (!this.currentRecording) {
          throw new Error('No current recording found');
        }
        Host.userMetrics.keyboardShortcutFired(
          'chrome-recorder.start-recording' /* Actions.RecorderActions.START_RECORDING */,
        );
        this.#setCurrentRecording(
          await this.#storage.upsertRecording(data, this.currentRecording.storageName),
        );
  ...
  async #onRecordingSelected(event) {
    const storageName =
      event instanceof Components.RecordingListView.OpenRecordingEvent ||
  ...
  async #handleAddAssertionEvent() {
    ...
    this.#recordingView?.contentElement
  ...
  async #acknowledgeImportNotice() {
    ...
    if (
      ...
      Root.Runtime.Runtime.queryParam('disableSelfXssWarnings') ||
    ...
    const result = await PanelCommon.TypeToAllowDialog.show({
      ...
      inputPlaceholder: i18nString(UIStrings.typeAllowImporting, {
        PH1: i18nString(UIStrings.allowImporting),
      }),
    });
    if (result) {
  ...
  async #onPlayRecordingByName(event) {
    ...
    await this.#onPlayRecording({
      ...
      speed: this.#recorderSettings.speed,
    });
  }
  ...
  handleActions(actionId) {
    ...
    switch (actionId) {
      ...
      case 'chrome-recorder.start-recording' /* Actions.RecorderActions.START_RECORDING */:
        if (
          this.currentPage !== 'CreateRecordingPage' /* Pages.CREATE_RECORDING_PAGE */ &&
          !this.isRecording
        ) {
          this.#shortcutHelper.handleShortcut(
            this.#onRecordingStarted.bind(this, {
              ...
              selectorAttribute: this.#recorderSettings.selectorAttribute
  ...
  isActionPossible(actionId) {
    switch (actionId) {
      ...
      case 'chrome-recorder.replay-recording' /* Actions.RecorderActions.REPLAY_RECORDING */:
        return (
          this.currentPage === 'RecordingPage' /* Pages.RECORDING_PAGE */ &&
  ...
  #getShortcutsInfo() {
    const getBindingForAction = (action) => {
      ...
      const shortcutsWithSplitBindings = shortcuts.map((shortcut) =>
        shortcut
          .title()
          .split(/[\s+]+/)
          .map((word) => {
    ...
    return [
      ...
      },
      {
        ...
        rows: getBindingForAction(
          'chrome-recorder.replay-recording' /* Actions.RecorderActions.REPLAY_RECORDING */,
        ),
      },
      {
        title: i18nString(UIStrings.copyShortcut),
        rows: Host.Platform.isMac()
          ? [[{ key: '⌘' }, { key: 'C' }]]
          : [[{ key: 'Ctrl' }, { key: 'C' }]],
      },
      {
        ...
        rows: getBindingForAction(
          'chrome-recorder.toggle-code-view' /* Actions.RecorderActions.TOGGLE_CODE_VIEW */,
        ),
      },
    ];
  }
    ...
    return html`
      <div class="empty-state" jslog=${VisualLogging.section().context('start-view')}>
        ...
        <devtools-button
          ...
          jslogContext=${
            'chrome-recorder.create-recording' /* Actions.RecorderActions.CREATE_RECORDING */
          }
          @click=${this.#onCreateNewRecording}
          >${i18nString(UIStrings.createRecording)}</devtools-button
        >
      </div>
    `;
    // clang-format on
  ...
  render() {
    const recordings = this.#storage.getRecordings();
    const selectValue = this.currentRecording
      ? this.currentRecording.storageName
      : this.currentPage;
    // clang-format off
    const values = [
        ...
        <div class="header" jslog=${VisualLogging.toolbar()}>
          ...
          >
            ${Lit.Directives.repeat(
              ...
              (item) => item.value,
              (item) => {
                return html`<option .selected=${item.selected} value=${item.value}>
                  ${item.name}
                </option>`;
              },
            )}
                ...
                return html`
                  <devtools-menu-item
                    ...
                    jslog=${VisualLogging.item(
                      `converter-${Platform.StringUtilities.toKebabCase(converter.getId())}`,
                    ).track({ click: true })}
                  >
...
__decorate([state()], RecorderController.prototype, 'currentRecordingSession', void 0);
__decorate([state()], RecorderController.prototype, 'currentRecording', void 0);
__decorate([state()], RecorderController.prototype, 'currentStep', void 0);
__decorate([state()], RecorderController.prototype, 'recordingError', void 0);
__decorate([state()], RecorderController.prototype, 'isRecording', void 0);
__decorate([state()], RecorderController.prototype, 'isToggling', void 0);
__decorate([state()], RecorderController.prototype, 'recordingPlayer', void 0);
__decorate([state()], RecorderController.prototype, 'lastReplayResult', void 0);
__decorate([state()], RecorderController.prototype, 'currentPage', void 0);
__decorate([state()], RecorderController.prototype, 'previousPage', void 0);
__decorate([state()], RecorderController.prototype, 'sections', void 0);
__decorate([state()], RecorderController.prototype, 'settings', void 0);
__decorate([state()], RecorderController.prototype, 'importError', void 0);
__decorate([state()], RecorderController.prototype, 'exportMenuExpanded', void 0);
__decorate([state()], RecorderController.prototype, 'extensionConverters', void 0);
__decorate([state()], RecorderController.prototype, 'replayExtensions', void 0);
__decorate([state()], RecorderController.prototype, 'viewDescriptor', void 0);
