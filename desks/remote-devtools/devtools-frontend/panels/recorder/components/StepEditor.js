...
const attributesByType = deepFreeze({
  [Models.Schema.StepType.Click]: {
    ...
    optional: ['assertedEvents', 'button', 'deviceType', 'duration', 'frame', 'target', 'timeout'],
  },
...
export class EditorState {
  ...
  static async defaultByAttribute(_state, attribute) {
    return await this.#puppeteer.run((puppeteer) => {
      switch (attribute) {
        case 'assertedEvents': {
          return immutableDeepAssign(
            defaultValuesByAttribute.assertedEvents,
            new ArrayAssignments({
              0: {
        ...
        case 'height': {
          return (
            puppeteer.page.evaluate(() => visualViewport.height) || defaultValuesByAttribute.height
          );
        }
        case 'width': {
          return (
            puppeteer.page.evaluate(() => visualViewport.width) || defaultValuesByAttribute.width
          );
        }
        default: {
...
let StepEditor = class StepEditor extends LitElement {
  ...
  #handleSelectorPicked = (data) => {
    this.#commit(
      immutableDeepAssign(this.state, {
        target: data.target,
        frame: data.frame,
  ...
  #handleKeyDownEvent = (event) => {
    ...
    if (
      event.target instanceof SuggestionInput.SuggestionInput.SuggestionInput &&
      event.key === 'Enter'
    ) {
      event.preventDefault();
      event.stopPropagation();
  ...
  #renderTypeRow(editable) {
    ...
    return html`<div
      class="row attribute"
      data-attribute="type"
      jslog=${VisualLogging.treeItem('type').track({ resize: true })}
    >
  ...
  #renderRow(attribute) {
    ...
    return html`<div
      class="row attribute"
      data-attribute=${attribute}
      jslog=${VisualLogging.treeItem(Platform.StringUtilities.toKebabCase(attribute)).track({
        resize: true,
      })}
    >
  ...
  #renderFrameRow() {
    ...
    return html`
      <div
        class="attribute"
        data-attribute="frame"
        jslog=${VisualLogging.treeItem('frame').track({ resize: true })}
      >
        <div class="row">
  ...
  #renderSelectorsRow() {
    ...
    return html`<div
      ...
      jslog=${VisualLogging.treeItem('selectors')}
    >
      <div class="row">
      ...
      ${this.state.selectors.map((selector, index, selectors) => {
        return html`<div class="padded row" data-selector-path=${index}>
          ...
          ${selector.map((part, partIndex, parts) => {
            return html`<div class="double padded row" data-selector-path="${index}.${partIndex}">
              <devtools-suggestion-input
  ...
  #renderAssertedEvents() {
    ...
    return html`<div
      class="attribute"
      data-attribute="assertedEvents"
      jslog=${VisualLogging.treeItem('asserted-events')}
    >
      <div class="row">
  ...
  #renderAttributesRow() {
    ...
    return html`<div
      ...
      jslog=${VisualLogging.treeItem('attributes')}
    >
      <div class="row">
  ...
  #renderAddRowButtons() {
    ...
    return [...attributes.optional]
      .filter((attr) => this.state[attr] === undefined)
      .map((attr) => {
        // clang-format off
        return html`<devtools-button
  ...
  render() {
    ...
    const result = html`
      ...
      <div class="wrapper" jslog=${VisualLogging.tree('step-editor')}>
        ...
        ${this.#renderFrameRow()} ${this.#renderSelectorsRow()} ${this.#renderRow('deviceType')}
        ${this.#renderRow('button')} ${this.#renderRow('url')} ${this.#renderRow('x')}
        ${this.#renderRow('y')} ${this.#renderRow('offsetX')} ${this.#renderRow('offsetY')}
        ${this.#renderRow('value')} ${this.#renderRow('key')} ${this.#renderRow('operator')}
        ${this.#renderRow('count')} ${this.#renderRow('expression')} ${this.#renderRow('duration')}
        ${this.#renderAssertedEvents()} ${this.#renderRow('timeout')} ${this.#renderRow('width')}
        ...
        ${this.#renderRow('upload')} ${this.#renderRow('latency')} ${this.#renderRow('name')}
        ${this.#renderRow('parameters')} ${this.#renderRow('visible')}
        ${this.#renderRow('properties')} ${this.#renderAttributesRow()}
        ${this.error
          ? html`
              <div class="error">
          ...
          : undefined}
        ${!this.disabled
          ? html`<div class="row-buttons wrapped gap row regular-font no-margin">
...
__decorate([state()], StepEditor.prototype, 'state', void 0);
__decorate([state()], StepEditor.prototype, 'error', void 0);
__decorate([property({ type: Boolean })], StepEditor.prototype, 'isTypeEditable', void 0);
__decorate([property({ type: Boolean })], StepEditor.prototype, 'disabled', void 0);
StepEditor = __decorate([customElement('devtools-recorder-step-editor')], StepEditor);
export { StepEditor };
