...
function getStepTypeTitle(input) {
  if (input.section) {
    return input.section.title
      ? input.section.title
      : html`<span class="fallback">(No Title)</span>`;
  }
  if (!input.step) {
      ...
      <div>
        ...
        >
          <svg slot="icon" width="24" height="24" class="icon">
            ...
            <path
              @click=${input.onBreakpointClick}
              jslog=${VisualLogging.action('breakpoint').track({ click: true })}
              class="breakpoint-icon"
              d="M2.5 5.5H17.7098L21.4241 12L17.7098 18.5H2.5V5.5Z"
            />
          </svg>
          <div class="summary">
            <div
              class="title-container ${isExpandable ? 'action' : ''}"
          ...
          ${input.error && html` <div class="error" role="alert">${input.error.message}</div> `}
        </devtools-widget>
      </div>
    `,
    target,
...
export class StepView extends UI.Widget.Widget {
  ...
  set state(state) {
    ...
    this.performUpdate();
    if (
      this.#viewInput.state !== prevState &&
      this.#viewInput.state === 'current' &&
      !this.#viewInput.isVisible
    ) {
    ...
    switch (event.itemValue) {
      case 'add-step-before': {
        ...
        this.contentElement.dispatchEvent(
          new AddStep(stepOrSection, 'before' /* AddStepPosition.BEFORE */),
        );
        break;
      }
      case 'add-step-after': {
        ...
        this.contentElement.dispatchEvent(
          new AddStep(stepOrSection, 'after' /* AddStepPosition.AFTER */),
        );
        break;
      }
      case 'remove-step': {
  ...
  #populateStepContextMenu(contextMenu) {
    ...
    const preferredCopyAction = copyActions.find(
      (item) =>
        item.id === COPY_ACTION_PREFIX + this.#viewInput.recorderSettings?.preferredCopyFormat,
    );
    if (preferredCopyAction) {
      contextMenu.section('copy').appendItem(
        preferredCopyAction.label,
        () => {
    ...
    if (copyActions.length) {
      const copyAs = contextMenu
        .section('copy')
        .appendSubMenuItem(i18nString(UIStrings.copyAs), false, 'copy');
      for (const item of copyActions) {
        if (item === preferredCopyAction) {
          continue;
        }
        copyAs.section(item.group).appendItem(
          item.label,
          () => {
            this.#handleStepAction(new Menus.Menu.MenuItemSelectedEvent(item.id));
          },
