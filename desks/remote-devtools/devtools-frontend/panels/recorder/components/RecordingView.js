...
function renderSettings({
  ...
  if (!replaySettingsExpanded) {
    ...
    replaySettingsFragments.push(
      html`<div class="editable-setting">
        <label class="wrapping-label" @click=${onSelectMenuLabelClick}>
          ...
          >
            ${networkConditionPresets.map(
              (condition) =>
                html` <option
                  jslog=${VisualLogging.item(
                    Platform.StringUtilities.toKebabCase(condition.i18nTitleKey || ''),
                  )}
    ...
    <div class="settings-row">
      <div class="settings-container">
        ...
        >
          ...
          ${isEditable
            ? html`<devtools-icon class="chevron" name="triangle-down"> </devtools-icon>`
            : ''}
        </div>
...
function renderTimelineArea(input, output) {
  ...
  return html`
    ...
    >
      <div slot="main">${renderSections(input)}</div>
      <div slot="sidebar" jslog=${VisualLogging.pane('source-code').track({ resize: true })}>
        ${input.showCodeView
          ? html` <div class="section-toolbar" jslog=${VisualLogging.toolbar()}>
  ...
  return html`
    <div class="text-editor" jslog=${VisualLogging.textField().track({ change: true })}>
      <devtools-text-editor
        .state=${input.editorState}
        ${Lit.Directives.ref((editor) => {
...
function renderScreenshot(section) {
  ...
  return html`
    <img
      ...
      alt=${i18nString(UIStrings.screenshotForSection)}
    />
  `;
  // clang-format on
}
function renderReplayOrAbortButton(input) {
  if (input.replayState.isPlaying) {
    return html` <devtools-button
      .jslogContext=${'abort-replay'}
      @click=${input.onAbortReplay}
      .iconName=${'pause'}
  ...
  return html`
    <div class="sections">
      ${!input.showCodeView
        ? html`<div class="section-toolbar">
            <devtools-button
              ...
              jslog=${VisualLogging.toggleSubpane(
                'chrome-recorder.toggle-code-view' /* Actions.RecorderActions.TOGGLE_CODE_VIEW */,
              ).track({ click: true })}
            >
              ${i18nString(UIStrings.showCode)}
      ...
      ${input.sections.map(
        (section, i) => html`
          <div class="section">
            <div class="screenshot-wrapper">${renderScreenshot(section)}</div>
            <div class="content">
              <div class="steps">
                ${widget(StepView, {
                  section,
                  ...
                  error:
                    input.getSectionState(section) === 'error' /* State.ERROR */
                    ...
                    <devtools-widget
                      ...
                      ${widget(StepView, {
                        ...
                        isLastSection:
                          ...
                          input.recording.steps[input.recording.steps.length - 1] === step,
                ...
                ${!input.recordingTogglingInProgress &&
                input.isRecording &&
                i === input.sections.length - 1
                  ? html`<devtools-button
                      ...
                      >${i18nString(UIStrings.addAssertion)}</devtools-button
                    >`
                  : undefined}
...
export const DEFAULT_VIEW = (input, output, target) => {
  const classNames = {
    ...
    'was-successful':
      input.lastReplayResult === 'Success' /* Models.RecordingPlayer.ReplayResult.SUCCESS */,
    'was-failure':
      input.lastReplayResult === 'Failure' /* Models.RecordingPlayer.ReplayResult.FAILURE */,
  };
  const footerButtonTitle = input.recordingTogglingInProgress
    ? i18nString(UIStrings.recordingIsBeingStopped)
      ...
      <div @click=${input.onWrapperClick} class=${Lit.Directives.classMap(classNames)}>
        <div class="recording-view main">
          ...
          ${input.extensionDescriptor
            ? html` <devtools-recorder-extension-view
                .descriptor=${input.extensionDescriptor}
              ></devtools-recorder-extension-view>`
            : html` ${renderSettings(input)} ${renderTimelineArea(input, output)} `}
          ${input.isRecording
...
export class RecordingView extends UI.Widget.Widget {
  ...
  set recorderSettings(settings) {
    ...
    this.#converterId =
      this.recorderSettings?.preferredCopyFormat ?? this.#builtInConverters[0]?.getId();
  ...
  performUpdate() {
    const converter =
      [...(this.builtInConverters || []), ...(this.extensionConverters || [])].find(
        (converter) => converter.getId() === this.#converterId,
  ...
  #onStepClick(stepOrSection) {
    const selectedStep =
      'type' in stepOrSection ? stepOrSection : stepOrSection.causingStep || null;
  ...
  #onNetworkConditionsChange(event) {
    ...
    if (throttlingMenu instanceof HTMLSelectElement) {
      ...
      this.networkConditionsChanged?.(
        preset?.i18nTitleKey === SDK.NetworkManager.NoThrottlingConditions.i18nTitleKey
          ? undefined
          : preset,
      );
    }
  ...
  async #copyCurrentSelection(step) {
    let converter = [...this.builtInConverters, ...this.extensionConverters].find(
  ...
  async #onCopy(event) {
    ...
    Host.userMetrics.keyboardShortcutFired(
      'chrome-recorder.copy-recording-or-step' /* Actions.RecorderActions.COPY_RECORDING_OR_STEP */,
    );
  }
  ...
  #convertToCode = async () => {
    ...
    const languageSupport = mediaType
      ? await CodeHighlighter.CodeHighlighter.languageFromMIME(mediaType)
      : null;
