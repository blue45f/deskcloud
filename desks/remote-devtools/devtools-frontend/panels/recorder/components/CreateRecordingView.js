...
const {
  html,
  Directives: { ref, createRef, repeat },
} = Lit;
const UIStrings = {
  /**
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  const selectorTypeToLabel = new Map([
    ...
    [Models.Schema.SelectorType.XPath, i18nString(UIStrings.selectorTypeXPath)],
    [Models.Schema.SelectorType.Pierce, i18nString(UIStrings.selectorTypePierce)],
  ]);
  // clang-format off
  Lit.render(
    html`
      ...
      <div class="wrapper" jslog=${VisualLogging.section('create-recording-view')}>
        ...
        <label class="row-label" for="selector-attribute">
          ...
          <devtools-link
            class="link"
            href="https://g.co/devtools/recorder#selector"
            title=${i18nString(UIStrings.learnMore)}
            .jslogContext=${'recorder-selector-help'}
          >
            <devtools-icon name="help"> </devtools-icon>
          </devtools-link>
        </label>
        <input
              ...
              <label class="checkbox-label selector-type">
                <input
                  ...
                  jslog=${VisualLogging.toggle()
                    .track({ click: true })
                    .context(`selector-${item.selectorType}`)}
...
export class CreateRecordingView extends UI.Widget.Widget {
  ...
  startRecording() {
    ...
    const selectorTypesToRecord = this.#selectorTypes
      .filter((item) => item.checked)
      .map((item) => item.selectorType);
    if (
      !selectorTypesToRecord.includes(Models.Schema.SelectorType.CSS) &&
