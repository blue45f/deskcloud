...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  Lit.render(
    html` <style>
      ...
      <div class="select-button" title=${ifDefined(actionTitle)}>
        <label>
          ${groups.length > 1
            ? html` <div class="groups-label">
                ${groups
                  .map((group) => {
                    return group.name;
                  })
                      ...
                      return html`
                        <option
                          ...
                          jslog=${VisualLogging.item(
                            Platform.StringUtilities.toKebabCase(item.value),
                          ).track({ click: true })}
                        >
...
export class ReplaySection extends UI.Widget.Widget {
  ...
  #onStartReplay() {
    ...
    if (value?.startsWith(REPLAY_EXTENSION_PREFIX)) {
      ...
      this.onStartReplay(
        this.#settings ? this.#settings.speed : 'normal' /* PlayRecordingSpeed.NORMAL */,
      );
    }
    this.performUpdate();
  }
