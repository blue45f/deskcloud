...
const createShortcuts = (descriptors) => {
  ...
  for (const shortcut of descriptors) {
    for (const key of shortcut) {
      ...
      const { keyCode, modifiers } =
        UI.KeyboardShortcut.KeyboardShortcut.keyCodeAndModifiersFromKey(key);
...
export class RecordingSession extends Common.ObjectWrapper.ObjectWrapper {
  ...
  async #appendInitialSteps() {
    ...
    if (history) {
      ...
      this.#lastNavigationHistoryByTarget.set(
        this.#target.id(),
        ...
        assertedEvents: [
          {
            ...
            url: mainFrame.url,
            title: await this.#getDocumentTitle(this.#target),
          },
        ],
      });
    }
  ...
  #dispatchRecordingUpdate() {
    ...
    this.#updateTimeout = setTimeout(() => {
      ...
      this.dispatchEventToListeners(
        'recordingupdated' /* Events.RECORDING_UPDATED */,
        structuredClone(this.#userFlow),
      );
  ...
  #handleBeforeUnload(context, sdkTarget) {
    ...
    if (
      lastStep &&
      !lastStep.assertedEvents?.find((event) => event.type === AssertedEventType.Navigation)
    ) {
  ...
  #handleStopShortcutBinding(event) {
    ...
    this.dispatchEventToListeners(
      'recordingstopped' /* Events.RECORDING_STOPPED */,
      structuredClone(this.#userFlow),
    );
  }
  ...
  #handleAddStepBinding(target, event) {
    ...
    if (!frameId) {
      throw new Error(
        'No execution context found for the binding call + ' + JSON.stringify(event.data),
      );
    }
  ...
  async #addBindings(target) {
    ...
    this.#runtimeEventDescriptors.set(target, [
    ...
    await Promise.all(
      Object.values(RecorderBinding).map((name) =>
        runtimeModel.addBinding({ name, executionContextName: Util.DEVTOOLS_RECORDER_WORLD_NAME }),
      ),
    );
  }
  async #removeBindings(target) {
    await Promise.all(
      Object.values(RecorderBinding).map((name) =>
        target.runtimeAgent().invoke_removeBinding({ name }),
      ),
    );
  ...
  async #handlePageNavigation(resourceTreeModel, target) {
    ...
    if (unrelatedNavigationTypes.has(entry.transitionType) || lastHistory.includes(entry.id)) {
      ...
      this.#replaceUnloadWithNavigation(target, {
        type: AssertedEventType.Navigation,
        url: entry.url,
        title: entry.title,
      });
  ...
  async #handleTargetInfoChanged(event) {
    ...
    if (event.event.data.type === 'iframe') {
      this.#replaceUnloadWithNavigation(target, {
        ...
        title: await this.#getDocumentTitle(target),
      });
