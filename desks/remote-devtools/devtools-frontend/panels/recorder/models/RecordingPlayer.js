...
function isPageTarget(target) {
  ...
  return (
    Common.ParsedURL.schemeIs(target.url, 'devtools:') ||
    target.type === 'page' ||
...
export class RecordingPlayer extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static async connectPuppeteer() {
    ...
    const { sessionId } = await rootTarget
      .targetAgent()
      .invoke_attachToTarget({ targetId: rootTargetId, flatten: true });
  ...
  async play() {
    ...
    class ExtensionWithBreak extends PuppeteerReplay.PuppeteerRunnerExtension {
      ...
      async beforeEachStep(step, flow) {
        ...
        const shouldStopAtCurrentStep =
          player.steppingOver || player.breakpointIndexes.has(currentStepIndex);
        const shouldWaitForSpeed =
          step.type !== 'setViewport' && step.type !== 'navigate' && !player.aborted;
      ...
      async runStep(step, flow) {
        ...
        if (
          Common.ParsedURL.schemeIs(page?.url(), 'devtools:') &&
