...
export class EventListenersView extends UI.Widget.VBox {
  ...
  constructor(element) {
    ...
    this.emptyHolder.createChild('span', 'gray-info-message').textContent = i18nString(
      UIStrings.noEventListeners,
    );
  ...
  addObject(object) {
    ...
    async function markInternalEventListeners() {
      ...
      return await frameworkEventListenersObject.internalHandlers
        .object()
  ...
  showFrameworkListeners(showFramework, showPassive, showBlocking) {
    ...
    for (const eventType of eventTypes) {
      ...
      for (const listenerElement of eventType.children()) {
        ...
        if (
          listenerOrigin ===
            'FrameworkUser' /* SDK.DOMDebuggerModel.EventListener.Origin.FRAMEWORK_USER */ &&
          !showFramework
        ) {
          hidden = true;
        }
        if (
          listenerOrigin ===
            'Framework' /* SDK.DOMDebuggerModel.EventListener.Origin.FRAMEWORK */ &&
          showFramework
        ) {
          hidden = true;
        }
...
export class EventListenersTreeElement extends UI.TreeOutline.TreeElement {
  ...
  addObjectEventListener(eventListener, object) {
    const treeElement = new ObjectEventListenerBar(
      eventListener,
      object,
      this.linkifier,
      this.changeCallback,
...
export class ObjectEventListenerBar extends UI.TreeOutline.TreeElement {
  ...
  async onpopulate() {
    ...
    properties.push(
      new ObjectUI.ObjectPropertiesSection.ObjectTreeNode(
        runtimeModel.createRemotePropertyFromPrimitiveValue(
          'useCapture',
  ...
  setTitle(object, linkifier) {
    ...
    if (this.#eventListener.isScrollBlockingType() && this.#eventListener.canTogglePassive()) {
      ...
      passiveButton.setAttribute(
        'jslog',
        `${VisualLogging.action('passive').track({ click: true })}`,
      );
    ...
    this.listItemElement.addEventListener('contextmenu', (event) => {
      ...
      menu
        .defaultSection()
        .appendItem(i18nString(UIStrings.deleteEventListener), this.removeListener.bind(this), {
          disabled: !this.#eventListener.canRemove(),
          jslogContext: 'delete-event-listener',
        });
      menu
        .defaultSection()
        .appendCheckboxItem(i18nString(UIStrings.passive), this.togglePassiveListener.bind(this), {
