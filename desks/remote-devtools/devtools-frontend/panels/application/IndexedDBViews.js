...
export class IDBDatabaseView extends ApplicationComponents.StorageMetadataView.StorageMetadataView {
  ...
  async renderReportContent() {
    ...
    return html`
      ${await super.renderReportContent()} ${this.key(i18nString(UIStrings.version))}
      ...
      <devtools-report-section>
        ...
        >
          ${i18nString(UIStrings.deleteDatabase)} </devtools-button
        >&nbsp;
        <devtools-button
  ...
  update(database) {
    ...
    const bucketInfo = this.model
      .target()
      .model(SDK.StorageBucketsModel.StorageBucketsModel)
      ?.getBucketByName(
        database.databaseId.storageBucket.storageKey,
        database.databaseId.storageBucket.name,
      );
    if (bucketInfo) {
...
export class IDBDataView extends UI.View.SimpleView {
  ...
  constructor(model, databaseId, objectStore, index, refreshObjectStoreCallback) {
    ...
    this.clearButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.clearObjectStore),
      'clear',
    );
    ...
    this.clearButton.element.setAttribute(
      'jslog',
      `${VisualLogging.action('clear-all').track({ click: true })}`,
    );
    ...
    if (this.isIndex) {
      columns.push({
        ...
        titleDOMFragment: this.keyColumnHeaderFragment(
          i18nString(UIStrings.primaryKey),
  ...
  keyPathStringFragment(keyPathString) {
    ...
    const keyPathSpan = keyPathStringFragment.createChild(
      'span',
  ...
  createEditorToolbar() {
    ...
    this.pageBackButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.showPreviousPage),
      'triangle-left',
      undefined,
      'prev-page',
    );
    this.pageBackButton.addEventListener(
      'Click' /* UI.Toolbar.ToolbarButton.Events.CLICK */,
      this.pageBackButtonClicked,
      this,
    );
    editorToolbar.appendToolbarItem(this.pageBackButton);
    this.pageForwardButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.showNextPage),
      'triangle-right',
      undefined,
      'next-page',
    );
    ...
    this.keyInput.addEventListener(
      ...
      this.updateData.bind(this, false),
    );
  ...
  async resolveArrayKey(key) {
    ...
    const propertyPromises = properties
      .filter((property) => !isNaN(Number(property.name)))
      .map(async (property) => {
  ...
  async deleteButtonClicked(node) {
    ...
    await this.model.deleteEntries(
      this.databaseId,
      this.objectStore.name,
      globalThis.IDBKeyRange.only(keyValue),
    );
