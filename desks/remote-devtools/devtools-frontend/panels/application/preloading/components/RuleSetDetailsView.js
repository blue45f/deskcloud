...
const codeMirrorJsonType =
  await CodeHighlighter.CodeHighlighter.languageFromMIME('application/json');
export const DEFAULT_VIEW = (input, _output, target) => {
  // clang-format off
  render(
    html`
      <style>
        ${ruleSetDetailsViewStyles}
      </style>
      <style>
        ${UI.inspectorCommonStyles}
      </style>
      ${input
        ? html` <div class="ruleset-header-container">
              <div class="ruleset-header" id="ruleset-url">${input.url}</div>
              ${input.errorMessage
                ? html`
                    <div class="ruleset-header">
                      <devtools-icon name="cross-circle" class="medium"> </devtools-icon>
