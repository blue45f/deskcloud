...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/preloading/components/RuleSetGrid.ts',
  UIStrings,
);
        ...
        <devtools-data-grid striped>
          <table>
            <tr>
              ...
              <th id="status" weight="80" sortable>${i18nString(UIStrings.status)}</th>
            </tr>
            ${rows.map(({ ruleSet, preloadsStatusSummary }) => {
              ...
              return html`
                <tr @select=${() => input.onSelect(ruleSet.id)}>
                  <td>
                    ${revealInElements || revealInNetwork
                      ? html` <button
                          class="link"
                          role="link"
                          @click=${() => {
                          ...
                          jslog=${VisualLogging.action(
                            revealInElements ? 'reveal-in-elements' : 'reveal-in-network',
                          ).track({ click: true })}
                        >
                          <devtools-icon
                            name=${revealInElements ? 'code-circle' : 'arrow-up-down-circle'}
                            class="medium"
                            style=${styleMap({
                              color: 'var(--icon-link)',
                              'vertical-align': 'sub',
                  ...
                  <td>
                    ${ruleSet.errorType !== undefined
                      ? html` <span style=${styleMap({ color: 'var(--sys-color-error)' })}>
                          ${i18nString(UIStrings.errors, { errorCount: 1 })}
                        </span>`
                      : ''}
                    ${ruleSet.errorType !==
                      'SourceIsNotJsonObject' /* Protocol.Preload.RuleSetErrorType.SourceIsNotJsonObject */ &&
                    ruleSet.errorType !==
                      'InvalidRulesetLevelTag' /* Protocol.Preload.RuleSetErrorType.InvalidRulesetLevelTag */
                      ? html` <button
                          class="link"
                          role="link"
...
export class RuleSetGrid extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  #revealAttemptViewWithFilter(ruleSet) {
    void Common.Revealer.reveal(
      new PreloadingHelper.PreloadingForward.AttemptViewWithFilter(ruleSet.id),
    );
  }
}
