...
function renderSpeculativeLoadingStatusForThisPageSections({
  ...
  if (kind === 'PrefetchFailed' /* UsedKind.PREFETCH_FAILED */) {
    ...
    kind ===
      'DowngradedPrerenderToPrefetchAndUsed' /* UsedKind.DOWNGRADED_PRERENDER_TO_PREFETCH_AND_USED */
  ) {
  ...
  return html` <devtools-report-section-header>
    ...
    <devtools-report-section>
      <div>
        <div class="status-badge-container">${renderBadge(badge)}</div>
        <div>${basicMessage}</div>
      </div>
    </devtools-report-section>

    ${maybeFailureReasonMessage !== undefined
      ? html` <devtools-report-section-header>
            ${i18nString(UIStrings.detailsFailureReason)}
          </devtools-report-section-header>
          <devtools-report-section> ${maybeFailureReasonMessage} </devtools-report-section>`
      : nothing}
    ${mismatchedData ? renderMismatchedSections(mismatchedData) : nothing}
    ${attemptWithMismatchedHeaders
      ? renderMismatchedHTTPHeadersSections(attemptWithMismatchedHeaders)
      : nothing}`;
  // clang-format on
}
function renderMismatchedSections(data) {
  ...
  return html` <devtools-report-section-header>
    ...
    <devtools-report-section>
      <devtools-link class="link devtools-link" href=${data.pageURL} jslogcontext="current-url"
...
function renderMismatchedHTTPHeadersSections(attempt) {
  ...
  return html` <devtools-report-section-header>
    ...
    <devtools-report-section>
      <style>
        ${preloadingGridStyles}
      </style>
      <div class="preloading-container">
        <devtools-data-grid striped inline>
          <table>
            <tr>
              <th id="header-name" weight="30" sortable>${i18nString(UIStrings.headerName)}</th>
            ...
            ${(attempt.mismatchedHeaders ?? []).map(
              (mismatchedHeaders) => html`
                <tr>
...
function renderSpeculationsInitiatedByThisPageSummarySections({
  ...
  return html` <devtools-report-section-header>
    ...
    <devtools-report-section>
      <div>
        <div class="status-badge-container">${badges.map(renderBadge)}</div>
  ...
  render(
    html` <style>
      ...
      <devtools-report>
        ...
        ${renderSpeculationsInitiatedByThisPageSummarySections(
          input.speculationsInitiatedSummaryData,
        )}
...
export class UsedPreloadingView extends UI.Widget.VBox {
  ...
  #getSpeculativeLoadingStatusForThisPageData() {
    ...
    const prefetch = forThisPage.filter(
      (attempt) =>
        attempt.key.action === 'Prefetch' /* Protocol.Preload.SpeculationAction.Prefetch */,
    )[0];
    ...
    if (
      prerenderLike?.status === 'Failure' /* SDK.PreloadingModel.PreloadingStatus.FAILURE */ &&
      ...
      prerenderLike?.status === 'Success' /* SDK.PreloadingModel.PreloadingStatus.SUCCESS */
    ) {
  ...
  #getMismatchedData(kind) {
    if (
      kind !== 'NoPreloads' /* UsedKind.NO_PRELOADS */ ||
      this.#data.previousAttempts.length === 0
    ) {
      return undefined;
    }
  ...
  #getSpeculationsInitiatedByThisPageSummaryData() {
    ...
    const notTriggeredCount =
      count.get('NotTriggered' /* SDK.PreloadingModel.PreloadingStatus.NOT_TRIGGERED */) ?? 0;
    const readyCount = count.get('Ready' /* SDK.PreloadingModel.PreloadingStatus.READY */) ?? 0;
    const failureCount =
      count.get('Failure' /* SDK.PreloadingModel.PreloadingStatus.FAILURE */) ?? 0;
    const inProgressCount =
      (count.get('Pending' /* SDK.PreloadingModel.PreloadingStatus.PENDING */) ?? 0) +
    ...
    if (notTriggeredCount > 0) {
      badges.push({
        type: 'neutral',
        message: i18nString(UIStrings.badgeNotTriggeredWithCount, { n: notTriggeredCount }),
      });
    }
    if (inProgressCount > 0) {
      badges.push({
        type: 'neutral',
        message: i18nString(UIStrings.badgeInProgressWithCount, { n: inProgressCount }),
      });
    }
    if (readyCount > 0) {
    ...
    const revealAttemptViewWithFilter = () => {
      void Common.Revealer.reveal(
        new PreloadingHelper.PreloadingForward.AttemptViewWithFilter(null),
      );
    };
