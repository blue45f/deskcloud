...
import {
  ...
  prerenderFailureReason,
  ruleSetLocationShort,
} from './PreloadingString.js';
const { html } = Lit;
const UIStrings = {
  /**
...
export class PreloadingDetailsReportView extends LegacyWrapper.LegacyWrapper.WrappableComponent {
    ...
    await RenderCoordinator.write('PreloadingDetailsReportView render', () => {
      if (this.#data === null) {
        ...
        Lit.render(
          html`
            <style>
              ${preloadingDetailsReportViewStyles}
            </style>
            <style>
            ...
            <div class="empty-state">
              ...
              <span class="empty-state-description"
                >${i18nString(UIStrings.selectAnElementForMoreDetails)}</span
              >
            </div>
          `,
          this.#shadow,
      ...
      const isFallbackToPrefetch =
        pipeline.getPrerender()?.status ===
          'Failure' /* SDK.PreloadingModel.PreloadingStatus.FAILURE */ &&
        (pipeline.getPrefetch()?.status ===
        ...
        html`
          ...
          >
            <devtools-report-section-header
              >${i18nString(UIStrings.detailsDetailedInformation)}</devtools-report-section-header
            >

            ${this.#url()} ${this.#action(isFallbackToPrefetch)}
            ${this.#status(isFallbackToPrefetch)} ${this.#targetHint()}
            ${this.#maybePrefetchFailureReason()} ${this.#maybePrerenderFailureReason()}
  ...
  #url() {
    ...
    let value;
    if (
      attempt.action === 'Prefetch' /* Protocol.Preload.SpeculationAction.Prefetch */ &&
      ...
      value = html`
        <devtools-request-link-icon
          .data=${{
            affectedRequest,
            requestResolver:
              this.#data.requestResolver || new Logs.RequestResolver.RequestResolver(),
            displayURL: true,
            urlToDisplay: url,
          }}
        >
  ...
  #action(isFallbackToPrefetch) {
    ...
    (() => {
      ...
      const prerenderTarget = SDK.TargetManager.TargetManager.instance()
        .targets()
        .find(
          (child) =>
            child.targetInfo()?.subtype === 'prerender' && child.inspectedURL() === attempt.key.url,
        );
    ...
    return html`
      ...
      <devtools-report-value>
        <div class="text-ellipsis" title="">${action} ${maybeFallback} ${maybeInspectButton}</div>
      </devtools-report-value>
    `;
    // clang-format on
  }
  #status(isFallbackToPrefetch) {
    assertNotNullOrUndefined(this.#data);
    const attempt = this.#data.pipeline.getOriginallyTriggered();
    const detailedStatus = isFallbackToPrefetch
      ? i18nString(UIStrings.detailedStatusFallbackToPrefetch)
      : PreloadingUIUtils.detailedStatus(attempt);
    return html`
      <devtools-report-key>${i18nString(UIStrings.detailsStatus)}</devtools-report-key>
      <devtools-report-value> ${detailedStatus} </devtools-report-value>
    `;
  }
  #maybePrefetchFailureReason() {
    ...
    return html`
      ...
      <devtools-report-value> ${failureDescription} </devtools-report-value>
    `;
  }
  #targetHint() {
    assertNotNullOrUndefined(this.#data);
    const attempt = this.#data.pipeline.getOriginallyTriggered();
    const hasTargetHint =
      this.#isPrerenderLike(attempt.action) && attempt.key.targetHint !== undefined;
    if (!hasTargetHint) {
      return Lit.nothing;
    }
    return html`
  ...
  #maybePrerenderFailureReason() {
    ...
    return html`
      <devtools-report-key>${i18nString(UIStrings.detailsFailureReason)}</devtools-report-key>
      <devtools-report-value> ${failureReason} </devtools-report-value>
    `;
  }
