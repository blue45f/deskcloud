...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/preloading/components/PreloadingGrid.ts',
  UIStrings,
);
...
export const PRELOADING_GRID_DEFAULT_VIEW = (input, _output, target) => {
  ...
  const securityOrigin =
    pageURL === '' ? null : new Common.ParsedURL.ParsedURL(pageURL).securityOrigin();
        ...
        <devtools-data-grid striped>
          <table>
            ...
            ${rows.map((row) => {
              ...
              const hasWarning =
                prerenderStatus === 'Failure' /* PreloadingStatus.FAILURE */ &&
                (prefetchStatus === 'Ready' /* PreloadingStatus.READY */ ||
                  prefetchStatus === 'Success') /* PreloadingStatus.SUCCESS */;
              const hasError =
                row.pipeline.getOriginallyTriggered().status ===
                'Failure'; /* PreloadingStatus.FAILURE */
              return html`<tr @select=${() => input.onSelect?.({ rowId: row.id })}>
                ...
                <td>
                  ${row.ruleSets.length === 0
                    ? ''
                    : ruleSetTagOrLocationShort(row.ruleSets[0], pageURL)}
                </td>
                <td data-value=${sortOrder(attempt)}>
                  <div
                    style=${styleMap({
                      color: hasWarning
                        ? 'var(--sys-color-orange-bright)'
                        : hasError
                          ? 'var(--sys-color-error)'
                  ...
                  >
                    ${hasError || hasWarning
                      ...
                      : ''}
                    ${hasWarning
                      ? i18nString(UIStrings.prefetchFallbackReady)
                      : composedStatus(attempt, row.statusCode)}
                  </div>
                </td>
              </tr>`;
            })}
