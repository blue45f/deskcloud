...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  if (input.header !== null) {
    ...
    // clang-format off
    template = html` <style>
        ${preloadingDisabledInfobarStyles}
      </style>
      <div id="container">
        ...
        <devtools-button-dialog
          ...
          jslog=${VisualLogging.dialog('preloading-disabled').track({
            resize: true,
            keydown: 'Escape',
          })}
        >
            ...
            <devtools-report>
              ${input.warnings.map(({ key, valueId, placeholders = {} }) => {
                const value = i18nTemplate(
                  ...
                  valueId,
                  Object.fromEntries(
                    Object.entries(placeholders).map(([key, { title, href }]) => [
                      key,
                      html`<devtools-link href=${href}>${title}</devtools-link>`,
                    ]),
                  ),
                );
                return html`
...
export class PreloadingDisabledInfobar extends UI.Widget.VBox {
  ...
  performUpdate() {
    ...
    if (this.#disabledByPreference || this.#disabledByDataSaver || this.#disabledByBatterySaver) {
      ...
      this.#disabledByHoldbackPrefetchSpeculationRules ||
