...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html` <devtools-data-grid striped inline>
      <table>
        <tr>
          <th id="url" weight="40" sortable>${i18nString(UIStrings.url)}</th>
          <th id="action" weight="15" sortable>${i18nString(UIStrings.action)}</th>
          <th id="status" weight="15" sortable>${i18nString(UIStrings.status)}</th>
        </tr>
        ${input.rows
          .map((row) => ({
            row,
          ...
          .map(
            ({ row }) => html`
              <tr>
                <td>
                  <div>
                    ${charDiff(row.url, input.pageURL).map((diffOp) => {
                      const s = diffOp[1];
                      switch (diffOp[0]) {
                        ...
                        case Diff.Diff.Operation.Delete:
                          return html`<span style=${styleMap({ color: 'var(--sys-color-error)' })}
                            >${s}</span
                          >`;
...
export class MismatchedPreloadingGrid extends UI.Widget.Widget {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    super(element, {
      classes: ['devtools-resources-mismatched-preloading-grid'],
      useShadowDom: true,
    });
    this.#view = view;
  }
