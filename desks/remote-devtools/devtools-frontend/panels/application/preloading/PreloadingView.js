...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/preloading/PreloadingView.ts',
  UIStrings,
);
...
export class PreloadingRuleSetView extends UI.Widget.VBox {
  ...
  ruleSetDetailsRef;
  shouldPrettyPrint = Common.Settings.Settings.instance()
  ...
  constructor(model) {
    ...
    this.model = model;
    SDK.TargetManager.TargetManager.instance().addScopeChangeListener(
      this.onScopeChange.bind(this),
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.PreloadingModel.PreloadingModel,
      'ModelUpdated' /* SDK.PreloadingModel.Events.MODEL_UPDATED */,
      this.render,
      this,
      { scoped: true },
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.PreloadingModel.PreloadingModel,
      'WarningsUpdated' /* SDK.PreloadingModel.Events.WARNINGS_UPDATED */,
      (e) => {
    ...
    this.ruleSetGrid.addEventListener(
      'select' /* PreloadingComponents.RuleSetGrid.Events.SELECT */,
    ...
    render(
      html` <div class="empty-state">
        ...
        <devtools-split-view sidebar-position="second">
          <div slot="main" ${ref(this.ruleSetGridContainerRef)}></div>
          <div slot="sidebar" jslog=${VisualLogging.section('rule-set-details')}>
            <devtools-widget
              ${widget(PreloadingComponents.RuleSetDetailsView.RuleSetDetailsView, {
                ruleSet: this.getRuleSet(),
                shouldPrettyPrint: this.shouldPrettyPrint,
              })}
              ${ref(this.ruleSetDetailsRef)}
            ></devtools-widget>
          </div>
        </devtools-split-view>
        <div class="pretty-print-button" style="border-top: 1px solid var(--sys-color-divider)">
          <devtools-button
            ...
            jslog=${VisualLogging.action()
              .track({ click: true })
              .context('preloading-status-panel-pretty-print')}
          ></devtools-button>
        </div>`,
      this.contentElement,
      { host: this },
  ...
  onScopeChange() {
    const model = SDK.TargetManager.TargetManager.instance()
      .scopeTarget()
...
export function applyFilterText(filterText, rows) {
  ...
  const isKeyWithNoValue =
    (lastTerm.key === undefined || lastTerm.key === null) &&
    FILTER_KEYS.some((key) => lastTerm.text === `${key}:`);
  if (isKeyWithNoValue) {
    query.pop();
  }
  if (query.length === 0) {
    ...
    return query.every((term) => {
      ...
      switch (key) {
        ...
        case undefined:
          ...
          return (
            url.includes(searchText) || action.includes(searchText) || status.includes(searchText)
          );
        default:
          return false;
      }
...
export class PreloadingAttemptView extends UI.Widget.VBox {
    ...
    render(
      html` <div class="empty-state">
        ...
        <devtools-split-view
          ...
          ${UI.Widget.widgetRef(UI.SplitWidget.SplitWidget, (w) => {
            this.hsplit = w;
          })}
        >
  ...
  updatePreloadingDetails() {
    ...
    if (preloadingAttempt === null) {
      ...
      const ruleSets = preloadingAttempt.ruleSetIds
        .map((id) => this.model.getRuleSetById(id))
        .filter((x) => x !== null);
  ...
  render() {
    ...
    const rows = this.model
      .getRepresentativePreloadingAttempts(filteringRuleSetId)
      .map(({ id, value }) => {
        const attempt = value;
...
class PreloadingRuleSetSelector {
  ...
  onModelUpdated() {
    ...
    const newSelected =
      selected === null || !items.includes(selected) ? AllRuleSetRootId : selected;
  ...
  subtitleFor(id) {
    const convertedId = this.translateItemIdToRuleSetId(id);
    const countsByStatus = this.model.getPreloadCountsByRuleSetId().get(convertedId) || new Map();
    return (
      PreloadingUIUtils.preloadsStatusSummary(countsByStatus) ||
      `(${i18nString(UIStrings.noRuleSets)})`
    );
  }
  // Method for UI.SoftDropDown.Delegate<Protocol.Preload.RuleSetId|typeof AllRuleSetRootId>
  createElementForItem(id) {
    const element = document.createElement('div');
    const shadowRoot = UI.UIUtils.createShadowRootWithCoreStyles(element, {
      cssFile: preloadingViewDropDownStyles,
    });
    const title = shadowRoot.createChild('div', 'title');
    UI.UIUtils.createTextChild(
      title,
      Platform.StringUtilities.trimEndWithMaxLength(this.titleFor(id), 100),
    );
