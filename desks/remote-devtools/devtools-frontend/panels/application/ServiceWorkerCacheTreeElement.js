...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/ServiceWorkerCacheTreeElement.ts',
  UIStrings,
);
...
export class ServiceWorkerCacheTreeElement extends ExpandableApplicationPanelTreeElement {
  ...
  handleContextMenuEvent(event) {
    ...
    contextMenu
      .defaultSection()
      .appendItem(i18nString(UIStrings.refreshCaches), this.refreshCaches.bind(this), {
  ...
  serviceWorkerCacheModelAdded(model) {
    ...
    model.addEventListener(
      'CacheAdded' /* SDK.ServiceWorkerCacheModel.Events.CACHE_ADDED */,
      this.cacheAdded,
      this,
    );
    model.addEventListener(
      'CacheRemoved' /* SDK.ServiceWorkerCacheModel.Events.CACHE_REMOVED */,
      this.cacheRemoved,
      this,
    );
  }
  ...
  addCache(model, cache) {
    if (this.cacheInTree(cache)) {
      const swCacheTreeElement = new SWCacheTreeElement(
        this.resourcesPanel,
        model,
        cache,
...
export class SWCacheTreeElement extends ApplicationPanelTreeElement {
  ...
  handleContextMenuEvent(event) {
    ...
    contextMenu
      .defaultSection()
      .appendItem(i18nString(UIStrings.delete), this.clearCache.bind(this), {
        jslogContext: 'delete',
      });
    void contextMenu.show();
  }
