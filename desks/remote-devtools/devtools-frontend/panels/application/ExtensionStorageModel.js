...
export class ExtensionStorageModel extends SDK.SDKModel.SDKModel {
      ...
      storage
        ...
        .then(() => {
          ...
          this.dispatchEventToListeners(
            'ExtensionStorageAdded' /* Events.EXTENSION_STORAGE_ADDED */,
            storage,
          );
        })
        .catch(() => {
  ...
  #removeExtension(id) {
    ...
    for (const [key, storage] of storages) {
      ...
      storages.delete(key);
      this.dispatchEventToListeners(
        'ExtensionStorageRemoved' /* Events.EXTENSION_STORAGE_REMOVED */,
        storage,
      );
    }
  ...
  #executionContextDestroyed(context) {
    ...
    if (extensionId) {
      ...
      if (
        this.#runtimeModel
          ?.executionContexts()
          .some((c) => this.#extensionIdForContext(c) === extensionId)
      ) {
        return;
      }
...
SDK.SDKModel.SDKModel.register(ExtensionStorageModel, {
  capabilities: 4 /* SDK.Target.Capability.JS */,
  autostart: false,
});
