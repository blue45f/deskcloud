...
export function filterToolCalls(toolCalls, filterState) {
  ...
  if (statusTypes) {
    filtered = filtered.filter((call) => {
      ...
      if (
        success &&
        call.result?.status === 'Success' /* Protocol.WebMCP.InvocationStatus.Success */
      ) {
        return true;
      }
  ...
  if (filterState.text) {
    ...
    filtered = filtered.filter((call) => {
      return (
        regex.test(call.tool.name) ||
        regex.test(call.input) ||
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  const isFilterActive =
    Boolean(input.filters.text) ||
    Boolean(input.filters.toolTypes) ||
    Boolean(input.filters.statusTypes);
  const iconName = (call) => {
  ...
  render(
    html`
      ...
      <style>
        ${UI.FilterBar.filterStyles}
      </style>
      <devtools-split-view
        class="webmcp-view"
        direction="row"
        sidebar-position="second"
        name="webmcp-split-view"
      >
        <div slot="main" class="call-log">
          <div class="webmcp-toolbar-container" role="toolbar" jslog=${VisualLogging.toolbar()}>
            <devtools-toolbar class="webmcp-toolbar" role="presentation" wrappable>
              <devtools-button
                title=${i18nString(UIStrings.clearLog)}
                .iconName=${'clear'}
                .variant=${'toolbar' /* Buttons.Button.Variant.TOOLBAR */}
                @click=${input.onClearLogClick}
              ></devtools-button>
              ...
              <devtools-button
                title=${i18nString(UIStrings.clearFilters)}
          ...
          ${input.toolCalls.length > 0
                  ...
                  >
                    <devtools-data-grid
                      ...
                      .template=${html` <table>
                        ...
                        <tr>
                          <th id="name" weight="20">${i18nString(UIStrings.name)}</th>
                ...
                <div class="webmcp-toolbar-container" role="toolbar">
                  <devtools-toolbar class="webmcp-toolbar" role="presentation" wrappable>
                    <span class="toolbar-text"
                      >${i18nString(UIStrings.totalCalls, { PH1: stats.total })}</span
                    >
                    <div class="toolbar-divider"></div>
                    <span class="toolbar-text status-error-text"
                      >${i18nString(UIStrings.failed, { PH1: stats.failed })}</span
                    >
                    <div class="toolbar-divider"></div>
                    ...
                    <div class="toolbar-divider"></div>
                    <span class="toolbar-text"
                      >${i18nString(UIStrings.inProgressCount, { PH1: stats.inProgress })}</span
                    >
                  </devtools-toolbar>
                </div>
              `
            : html`
                ${UI.Widget.widget(UI.EmptyWidget.EmptyWidget, {
                  header: i18nString(UIStrings.noCallsPlaceholderTitle),
        ...
        >
          <div slot="main" class="tool-list">
            ...
            ${tools.length === 0
              ? html`
                  ${UI.Widget.widget(UI.EmptyWidget.EmptyWidget, {
                    header: i18nString(UIStrings.noToolsPlaceholderTitle),
                  ...
                  <devtools-list>
                    ${tools.map((tool) => {
                      const toolStats = calculateToolStats(
                        input.toolCalls.filter((c) => c.tool === tool),
                      );
...
export class WebMCPView extends UI.Widget.VBox {
  ...
  static createFilterButtons(onToolTypesClick, onStatusTypesClick) {
    ...
    return {
      toolTypes: createButton(
        i18nString(UIStrings.toolTypes),
        onToolTypesClick,
        'webmcp.tool-types',
      ),
      statusTypes: createButton(
        i18nString(UIStrings.statusTypes),
        onStatusTypesClick,
        'webmcp.status-types',
      ),
    };
  ...
  #showToolTypesContextMenu(contextMenu) {
    ...
    contextMenu
      .defaultSection()
      .appendCheckboxItem(i18nString(UIStrings.imperative), () => toggle('imperative'), {
        checked: this.#filterState.toolTypes?.imperative ?? false,
        jslogContext: 'webmcp.imperative',
      });
    contextMenu
      .defaultSection()
      .appendCheckboxItem(i18nString(UIStrings.declarative), () => toggle('declarative'), {
        checked: this.#filterState.toolTypes?.declarative ?? false,
  ...
  #showStatusTypesContextMenu(contextMenu) {
    ...
    contextMenu
      .defaultSection()
      .appendCheckboxItem(i18nString(UIStrings.success), () => toggle('success'), {
        checked: this.#filterState.statusTypes?.['success'] ?? false,
        jslogContext: 'webmcp.success',
      });
    contextMenu
      .defaultSection()
      .appendCheckboxItem(i18nString(UIStrings.error), () => toggle('error'), {
        checked: this.#filterState.statusTypes?.['error'] ?? false,
        jslogContext: 'webmcp.error',
      });
    contextMenu
      .defaultSection()
      .appendCheckboxItem(i18nString(UIStrings.pending), () => toggle('pending'), {
        checked: this.#filterState.statusTypes?.['pending'] ?? false,
        jslogContext: 'webmcp.pending',
      });
  }
  #webMCPModelAdded(model) {
    model.addEventListener(
      'ToolsAdded' /* WebMCP.WebMCPModel.Events.TOOLS_ADDED */,
      this.requestUpdate,
      this,
    );
    model.addEventListener(
      'ToolsRemoved' /* WebMCP.WebMCPModel.Events.TOOLS_REMOVED */,
      this.requestUpdate,
      this,
    );
    model.addEventListener(
      'ToolInvoked' /* WebMCP.WebMCPModel.Events.TOOL_INVOKED */,
      this.requestUpdate,
      this,
    );
    model.addEventListener(
      'ToolResponded' /* WebMCP.WebMCPModel.Events.TOOL_RESPONDED */,
      this.requestUpdate,
      this,
    );
  }
  ...
  #handleClearLogClick = () => {
    const models = SDK.TargetManager.TargetManager.instance().models(
      WebMCP.WebMCPModel.WebMCPModel,
    );
  ...
  #handleFilterChange = (filters) => {
    ...
    const toolTypesCount = this.#filterState.toolTypes
      ? Object.values(this.#filterState.toolTypes).filter(Boolean).length
      : 0;
    this.#filterButtons.toolTypes.setCount(toolTypesCount);
    const statusTypesCount = this.#filterState.statusTypes
      ? Object.values(this.#filterState.statusTypes).filter(Boolean).length
      : 0;
...
export const PAYLOAD_DEFAULT_VIEW = (input, output, target) => {
  ...
  const createPayload = (parsedInput) => {
    ...
    const section = new ObjectUI.ObjectPropertiesSection.RootElement(
      new ObjectUI.ObjectPropertiesSection.ObjectTree(object, {
        readOnly: true,
    ...
    return html`<devtools-tree
      .template=${html`
        <style>
          ${ObjectUI.ObjectPropertiesSection.objectValueStyles}
        </style>
        <style>
          ${ObjectUI.ObjectPropertiesSection.objectPropertiesSectionStyles}
        </style>
        <ul role="tree">
          ...
          ? html`
              ...
              <div class="value">
                ${Components.Linkifier.Linkifier.linkifyRevealable(
                  tool.frame,
                  tool.frame.displayName(),
                )}
              </div>
            `
          : nothing}
        ${origin instanceof SDK.DOMModel.DOMNode
          ? html` <div class="label">Origin</div>
