...
export class StorageView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.includeThirdPartyCookiesSetting = Common.Settings.Settings.instance().createSetting(
      'clear-storage-include-third-party-cookies',
      false,
    );
    const clearButtonSection = this.reportView
      .appendSection('', 'clear-storage-button')
      .appendRow();
    ...
    this.quotaOverrideCheckbox = UI.UIUtils.CheckboxLabel.create(
      i18nString(UIStrings.simulateCustomStorage),
      false,
    );
    ...
    this.appendItem(
      storage,
      i18nString(UIStrings.localAndSessionStorage),
      'local_storage' /* Protocol.Storage.StorageType.Local_storage */,
    );
    this.appendItem(
      storage,
      i18nString(UIStrings.indexDB),
  ...
  updateOrigin(mainOrigin, unreachableMainOrigin) {
    ...
    if (unreachableMainOrigin) {
      ...
      this.reportView.setSubtitle(
        i18nString(UIStrings.sFailedToLoad, { PH1: unreachableMainOrigin }),
      );
    } else {
  ...
  async applyQuotaOverrideFromInputField() {
    ...
    if (quota >= cutoff) {
      this.quotaOverrideErrorMessage.textContent = i18nString(UIStrings.numberMustBeSmaller, {
    ...
    await this.target
      .storageAgent()
      .invoke_overrideQuotaForOrigin({ origin: this.securityOrigin, quotaSize: quotaInBytes });
  }
  ...
  static clear(
    ...
    void target
      .storageAgent()
      .invoke_clearDataForStorageKey({ storageKey, storageTypes: selectedStorageTypes.join(',') });
    ...
    if (
      originForCookies &&
      (set.has('cookies' /* Protocol.Storage.StorageType.Cookies */) || hasAll)
    ) {
      void target
        .storageAgent()
  ...
  async performUpdate() {
    ...
    const response = await this.target
      .storageAgent()
      .invoke_getUsageAndQuota({ origin: securityOrigin });
    this.quotaRow.textContent = '';
    ...
    const formattedQuotaAsString = i18nString(UIStrings.storageWithCustomMarker, {
      PH1: quotaAsString,
    });
    ...
    const element = uiI18n.getFormatLocalizedString(str_, UIStrings.storageQuotaUsed, {
      PH1: usageAsString,
      PH2: quota,
    });
...
export class ActionDelegate {
  ...
  handleClear(includeThirdPartyCookies) {
    ...
    resourceTreeModel.getMainStorageKey().then(
      (storageKey) => {
        StorageView.clear(
          target,
          storageKey,
