...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/StorageBucketsTreeElement.ts',
  UIStrings,
);
...
export class StorageBucketsTreeParentElement extends ExpandableApplicationPanelTreeElement {
  ...
  initialize() {
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.StorageBucketsModel.StorageBucketsModel,
      'BucketChanged' /* SDK.StorageBucketsModel.Events.BUCKET_CHANGED */,
      this.bucketChanged,
      this,
    );
  ...
  addBucketTreeElement(model, bucketInfo) {
    ...
    const singleBucketTreeElement = new StorageBucketsTreeElement(
      this.resourcesPanel,
      model,
      bucketInfo,
    );
  ...
  getBucketTreeElement(model, { bucket: { storageKey, name } }) {
    for (const bucketTreeElement of this.bucketTreeElements) {
      if (
        bucketTreeElement.model === model &&
        bucketTreeElement.bucketInfo.bucket.storageKey === storageKey &&
...
export class StorageBucketsTreeElement extends ExpandableApplicationPanelTreeElement {
  ...
  initialize() {
    ...
    const serviceWorkerCacheTreeElement = new ServiceWorkerCacheTreeElement(
      this.resourcesPanel,
      bucket,
    );
  ...
  onselect(selectedByUser) {
    ...
    if (!this.view) {
      this.view = LegacyWrapper.LegacyWrapper.legacyWrapper(
        UI.Widget.Widget,
        new StorageMetadataView.StorageMetadataView(),
      );
