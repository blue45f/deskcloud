...
export class ServiceWorkersView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.currentWorkersView = new UI.ReportView.ReportView(
    ...
    this.currentWorkersView.element.setAttribute(
      'jslog',
      `${VisualLogging.section('this-origin')}`,
    );
    ...
    const othersSection = othersView.appendSection(
    ...
    const forceUpdate = new UI.Toolbar.ToolbarSettingCheckbox(
      ...
      i18nString(UIStrings.onPageReloadForceTheService),
    );
    this.toolbar.appendToolbarItem(forceUpdate);
    const bypassServiceWorkerSetting = Common.Settings.Settings.instance().createSetting(
  ...
  modelAdded(serviceWorkerManager) {
    if (
      serviceWorkerManager.target() !==
      SDK.TargetManager.TargetManager.instance().primaryPageTarget()
    ) {
      return;
    }
    this.manager = serviceWorkerManager;
    this.securityOriginManager = serviceWorkerManager
      .target()
      .model(SDK.SecurityOriginManager.SecurityOriginManager);
    ...
    this.eventListeners.set(serviceWorkerManager, [
      ...
      this.manager.addEventListener(
        'RegistrationDeleted' /* SDK.ServiceWorkerManager.Events.REGISTRATION_DELETED */,
  ...
  updateSectionVisibility() {
    ...
    for (const section of this.sections.values()) {
      if (
        section.section.parentWidget() === this.currentWorkersView ||
  ...
  gcRegistrations() {
    ...
    for (const registration of this.manager.registrations().values()) {
      if (
        !securityOrigins.has(registration.securityOrigin) &&
        !this.isRegistrationVisible(registration)
      ) {
        continue;
      }
...
export class Section {
  ...
  constructor(manager, section, registration) {
    ...
    this.syncTagNameSetting = Common.Settings.Settings.instance().createLocalSetting(
      'sync-tag-name',
      'test-tag-from-devtools',
    );
    this.periodicSyncTagNameSetting = Common.Settings.Settings.instance().createLocalSetting(
    ...
    this.deleteButton = UI.UIUtils.createTextButton(
  ...
  createSyncNotificationField(label, initialValue, placeholder, callback, jslogContext) {
    const form = this.wrapWidget(this.section.appendField(label)).createChild(
      'form',
    ...
    editor.setAttribute(
      'jslog',
      `${VisualLogging.textField().track({ change: true }).context(jslogContext)}`,
    );
  ...
  updateClientsField(version) {
    ...
    this.section.setFieldVisible(
      i18nString(UIStrings.clients),
      Boolean(version.controlledClients.length),
    );
  ...
  updateSourceField(version) {
    ...
    if (version.scriptResponseTime !== undefined) {
      this.sourceField.createChild('div', 'report-field-value-subtitle').textContent = i18nString(
        UIStrings.receivedS,
  ...
  update() {
    ...
    const title = this.registration.isDeleted
      ? i18nString(UIStrings.sDeleted, { PH1: scopeURL })
      : scopeURL;
    ...
    if (active) {
      ...
      const localizedRunningStatus =
        SDK.ServiceWorkerManager.ServiceWorkerVersion.RunningStatus[
          active.currentState.runningStatus
        ]();
    ...
    if (waiting) {
      ...
      const skipWaitingButton = UI.UIUtils.createTextButton(
      ...
      if (waiting.scriptResponseTime !== undefined) {
        waitingEntry.createChild('div', 'service-worker-subtitle').textContent = i18nString(
          UIStrings.receivedS,
    ...
    if (installing) {
      ...
      if (installing.scriptResponseTime !== undefined) {
        installingEntry.createChild('div', 'service-worker-subtitle').textContent = i18nString(
          UIStrings.receivedS,
          {
  ...
  createUpdateCycleField() {
    this.updateCycleField = this.wrapWidget(
      this.section.appendField(i18nString(UIStrings.updateCycle)),
    );
