...
function renderOriginTrialTree(originTrial) {
  ...
  return html` <li role="treeitem">
    ...
    ${originTrial.tokensWithStatus.length > 1
      ? html` <devtools-adorner class="badge-secondary">
...
function renderTokenDetails(token) {
  return html` <li role="treeitem">${widget(OriginTrialTokenRows, { data: token })}</li>`;
}
function renderTokenDetailsNodes(token) {
  // clang-format off
  return html` ${renderTokenDetails(token)} ${renderRawTokenTextNode(token.rawTokenText)} `;
  // clang-format on
}
function renderRawTokenTextNode(tokenText) {
  ...
  return html` <li role="treeitem">
    ...
    <ul role="group">
      <li role="treeitem">
        <div style="overflow-wrap: break-word;">${tokenText}</div>
      </li>
    </ul>
  </li>`;
  // clang-format on
...
const ROWS_DEFAULT_VIEW = (input, _output, target) => {
  const success =
    input.tokenWithStatus.status === 'Success'; /* Protocol.Page.OriginTrialTokenStatus.Success */
  // clang-format off
  render(
    html` <style>
...
export class OriginTrialTokenRows extends UI.Widget.Widget {
  ...
  #parsedTokenDetails = [];
  #dateFormatter = new Intl.DateTimeFormat(i18n.DevToolsLocale.DevToolsLocale.instance().locale, {
    dateStyle: 'long',
    timeStyle: 'long',
  });
  ...
  #setTokenFields() {
    ...
    this.#parsedTokenDetails = [
      {
        ...
        value: {
          ...
          hasError:
            ...
            'WrongOrigin' /* Protocol.Page.OriginTrialTokenStatus.WrongOrigin */,
        },
      },
      {
        ...
        value: {
          ...
          hasError:
            this.#tokenWithStatus.status ===
            'Expired' /* Protocol.Page.OriginTrialTokenStatus.Expired */,
        },
      },
      {
    ...
    if (
      this.#tokenWithStatus.status ===
      'UnknownTrial' /* Protocol.Page.OriginTrialTokenStatus.UnknownTrial */
    ) {
      this.#parsedTokenDetails = [
        {
