...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html`
      <style>
        ${frameDetailsReportViewStyles}
      </style>
      <devtools-report
        .data=${{ reportTitle: input.frame.displayName() }}
        jslog=${VisualLogging.pane('frames')}
      >
        ${renderDocumentSection(input)} ${renderIsolationSection(input)}
        ${renderApiAvailabilitySection(input.frame)} ${renderOriginTrial(input.trials)}
        ${input.permissionsPolicies
          ...
          : nothing}
        ${input.protocolMonitorExperimentEnabled
          ? renderAdditionalInfoSection(input.frame)
          : nothing}
      </devtools-report>
    `,
    target,
...
function renderOriginTrial(trials) {
  ...
  return html` <devtools-report-section-header>
    ...
    <devtools-widget class="span-cols" ${widget(OriginTrialTreeView, { data })}> </devtools-widget>
...
function renderDocumentSection(input) {
  ...
  // clang-format off
  return html` <devtools-report-section-header
      >${i18nString(UIStrings.document)}</devtools-report-section-header
    >
    <devtools-report-key>${i18nString(UIStrings.url)}</devtools-report-key>
    <devtools-report-value>
      <div class="inline-items">
        ${!input.frame?.unreachableUrl()
          ? renderSourcesLinkForURL(input.onRevealInSources)
          : nothing}
    ...
    ${maybeRenderOrigin(input.frame?.securityOrigin)} ${renderOwnerElement(input.linkTargetDOMNode)}
    ...
    ${maybeRenderCreatorAdScriptAncestry(
      input.frame?.adFrameType(),
      input.target,
      input.adScriptAncestry,
    )}
    <devtools-report-divider></devtools-report-divider>`;
  // clang-format on
}
...
function maybeRenderAdStatus(adFrameType, adFrameStatus) {
  ...
  // Disabled until https://crbug.com/1079231 is fixed.
  // clang-format off
  return html` <devtools-report-key>${i18nString(UIStrings.adStatus)}</devtools-report-key>
...
function maybeRenderCreatorAdScriptAncestry(adFrameType, target, adScriptAncestry) {
  ...
  return html`
    ...
    <devtools-report-value
      class="creator-ad-script-ancestry-list"
      jslog=${VisualLogging.section('creator-ad-script-ancestry')}
    >
      <devtools-expandable-list
        .data=${{ rows, title: i18nString(UIStrings.creatorAdScriptAncestry) }}
      >
      </devtools-expandable-list>
    </devtools-report-value>
    ${shouldRenderFilterlistRule
      ? html`
          <devtools-report-key
            >${i18nString(UIStrings.rootScriptFilterlistRule)}</devtools-report-key
          >
          <devtools-report-value jslog=${VisualLogging.section('root-script-filterlist-rule')}
            >${adScriptAncestry.rootScriptFilterlistRule}</devtools-report-value
          >
        `
      : nothing}
  `;
...
function renderIsolationSection(input) {
  ...
  return html`
    <devtools-report-section-header
      >${i18nString(UIStrings.securityIsolation)}</devtools-report-section-header
    >
    <devtools-report-key>${i18nString(UIStrings.secureContext)}</devtools-report-key>
    <devtools-report-value>
      ${input.frame.isSecureContext()
        ? i18nString(UIStrings.yes)
        : i18nString(UIStrings.no)} ${maybeRenderSecureContextExplanation(input.frame)}
    </devtools-report-value>
  ...
  if (info) {
    return html`
      ${maybeRenderCrossOriginStatus(
        ...
        'None' /* Protocol.Network.CrossOriginEmbedderPolicyValue.None */,
      )}
      ${maybeRenderCrossOriginStatus(
        info.coop,
        i18n.i18n.lockedString('Cross-Origin Opener Policy (COOP)'),
        'UnsafeNone' /* Protocol.Network.CrossOriginOpenerPolicyValue.UnsafeNone */,
      )}
...
function maybeRenderCrossOriginStatus(info, policyName, noneValue) {
  ...
  return html`
    ...
    <devtools-report-value>
      ...
      ${endpoint
        ? html`<span class="inline-name">${i18nString(UIStrings.reportingTo)}</span>${endpoint}`
        : nothing}
    </devtools-report-value>
  `;
}
  ...
  return html`
    <devtools-report-key>
      ${cspInfo.isEnforced
        ...
        : html` ${i18n.i18n.lockedString('Content-Security-Policy-Report-Only')}
            <devtools-button
              .iconName=${'help'}
              class="help-button"
    ...
    <devtools-report-value>
      ${cspInfo.source === 'HTTP' /* Protocol.Network.ContentSecurityPolicySource.HTTP */
        ? i18n.i18n.lockedString('HTTP header')
        : i18n.i18n.lockedString('Meta tag')}
      ${renderEffectiveDirectives(cspInfo.effectiveDirectives)}
    </devtools-report-value>
    ${divider
      ? html`<devtools-report-divider class="subsection-divider"></devtools-report-divider>`
      : nothing}
  `;
  // clang-format on
}
function renderCSPSection(cspInfos) {
  ...
  return html`
    ...
    ${cspInfos?.length
      ? cspInfos.map((cspInfo, index) => renderSingleCSP(cspInfo, index < cspInfos?.length - 1))
      : html`
          <devtools-report-key>
            ${i18n.i18n.lockedString('Content-Security-Policy')}
          </devtools-report-key>
          <devtools-report-value> ${i18nString(UIStrings.none)} </devtools-report-value>
        `}
  `;
  // clang-format on
}
function renderApiAvailabilitySection(frame) {
  ...
  return html` <devtools-report-section-header>
    ...
    ${renderSharedArrayBufferAvailability(frame)} ${renderMeasureMemoryAvailability(frame)}
    <devtools-report-divider></devtools-report-divider>`;
  // clang-format on
}
  ...
  if (frame) {
    ...
    if (features) {
      ...
      const availabilityText = sabTransferAvailable
        ...
        : sabAvailable
          ? i18nString(UIStrings.availableNotTransferable)
      ...
      const tooltipText = sabTransferAvailable
        ...
        : sabAvailable
          ? i18nString(UIStrings.sharedarraybufferConstructorIsAvailable)
          : '';
      ...
      return html`
        ...
        <devtools-report-value title=${tooltipText}>
          ${availabilityText} ${renderHint(frame)}
        </devtools-report-value>
      `;
    }
  }
...
function renderMeasureMemoryAvailability(frame) {
  if (frame) {
    ...
    const availabilityText = measureMemoryAvailable
      ? i18nString(UIStrings.available)
      : i18nString(UIStrings.unavailable);
    const tooltipText = measureMemoryAvailable
      ? i18nString(UIStrings.thePerformanceAPI)
    ...
    return html`
      ...
      <devtools-report-value>
        <span title=${tooltipText}>${availabilityText}</span> <devtools-link
          class="link"
          href="https://web.dev/monitor-total-page-memory-usage/"
          jslogcontext="learn-more.monitor-memory-usage"
...
function renderAdditionalInfoSection(frame) {
  ...
  return html`
    <devtools-report-section-header title=${i18nString(UIStrings.thisAdditionalDebugging)}
...
export class FrameDetailsReportView extends UI.Widget.Widget {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    ...
    this.#protocolMonitorExperimentEnabled = Root.Runtime.experiments.isEnabled(
