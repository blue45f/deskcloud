...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/ServiceWorkerCacheViews.ts',
  UIStrings,
);
...
export class ServiceWorkerCacheView extends UI.View.SimpleView {
  ...
  constructor(model, cache) {
    ...
    entryPathFilterBox.addEventListener(
  ...
  wasShown() {
    ...
    this.model.addEventListener(
      'CacheStorageContentUpdated' /* SDK.ServiceWorkerCacheModel.Events.CACHE_STORAGE_CONTENT_UPDATED */,
  ...
  createDataGrid() {
    const columns = [
      ...
      {
        id: 'content-type',
        title: i18n.i18n.lockedString('Content-Type'),
        weight: 1,
        sortable: true,
      },
      {
      ...
      {
        id: 'vary-header',
        title: i18n.i18n.lockedString('Vary Header'),
        weight: 1,
        sortable: true,
      },
    ];
  ...
  createRequest(entry) {
    ...
    let header = entry.responseHeaders.find(
    ...
    if (!resourceType) {
      resourceType =
        Common.ResourceType.ResourceType.fromURL(entry.requestURL) ||
  ...
  async requestContent(request) {
    const response = await this.cache.requestCachedResponse(
      request.url(),
...
export class DataGridNode extends DataGrid.DataGrid.DataGridNode {
  ...
  constructor(number, request, responseType) {
    ...
    this.varyHeader =
      request.responseHeaders.find((header) => header.name.toLowerCase() === 'vary')?.value || '';
  }
  createCell(columnId) {
...
export class RequestView extends UI.Widget.VBox {
  ...
  constructor(request) {
    ...
    this.tabbedPane.element.setAttribute(
      'jslog',
      `${VisualLogging.section('network-item-preview')}`,
    );
