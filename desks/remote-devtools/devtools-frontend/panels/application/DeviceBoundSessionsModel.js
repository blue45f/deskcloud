...
export class DeviceBoundSessionsModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  modelAdded(networkManager) {
    networkManager.addEventListener(
      SDK.NetworkManager.Events.DeviceBoundSessionsAdded,
      this.#onSessionsSet,
      this,
    );
    networkManager.addEventListener(
  ...
  addVisibleSite(site) {
    ...
    this.dispatchEventToListeners(
      'ADD_VISIBLE_SITE' /* DeviceBoundSessionModelEvents.ADD_VISIBLE_SITE */,
      { site },
    );
  }
  clearVisibleSites() {
    ...
    this.dispatchEventToListeners(
      'CLEAR_VISIBLE_SITES' /* DeviceBoundSessionModelEvents.CLEAR_VISIBLE_SITES */,
    );
  }
  clearEvents() {
    if (this.getPreserveLogSetting().get()) {
    ...
    this.dispatchEventToListeners('CLEAR_EVENTS' /* DeviceBoundSessionModelEvents.CLEAR_EVENTS */, {
      emptySessions,
      emptySites,
      noLongerFailedSessions,
    });
  }
  isSiteVisible(site) {
  ...
  #onSessionsSet({ data: sessions }) {
    for (const session of sessions) {
      const sessionAndEvents = this.#ensureSiteAndSessionInitialized(
        session.key.site,
        session.key.id,
      );
    ...
    this.dispatchEventToListeners(
      'INITIALIZE_SESSIONS' /* DeviceBoundSessionModelEvents.INITIALIZE_SESSIONS */,
      { sessions },
    );
  }
  ...
  #onEventOccurred({ data: event }) {
    ...
    const newSession =
      event.creationEventDetails?.newSession || event.refreshEventDetails?.newSession;
    if (newSession) {
