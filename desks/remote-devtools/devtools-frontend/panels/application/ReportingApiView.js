    ...
    render(
      html`
        ...
        <devtools-split-view
          ...
          sidebar-initial-size="150"
          jslog=${VisualLogging.pane('reporting-api')}
        >
          ${input.hasReports
            ? html`
                <devtools-split-view
    ...
    render(
      html`
        <devtools-widget
          ...
          jslog=${VisualLogging.pane('reporting-api-empty')}
        ></devtools-widget>
      `,
      target,
    );
...
export class ReportingApiView extends UI.Widget.VBox {
  ...
  modelAdded(networkManager) {
    if (
      networkManager.target() !== SDK.TargetManager.TargetManager.instance().primaryPageTarget()
    ) {
      return;
    }
