...
function renderSectionHeader(text, output) {
  // clang-format off
  return html` <devtools-report-section-header
    ${ref((e) => {
...
function renderErrors(warnings, manifestErrors, imageErrors, output) {
  // clang-format off
  return html` ${renderSectionHeader(i18nString(UIStrings.errorsAndWarnings), output)}
...
function renderIdentity(identityData, onCopy, output) {
  ...
  return html`${renderSectionHeader(i18nString(UIStrings.identity), output)}
    <div class="report-section" jslog=${VisualLogging.section('identity')}>
      ...
      ${appId && recommendedId
        ? html` <devtools-report-key aria-label="App Id"
              >${i18nString(UIStrings.computedAppId)}</devtools-report-key
            >
            <devtools-report-value jslog=${VisualLogging.section('identity')}>
              ...
              <devtools-icon
                class="inline-icon"
                name="help"
                title=${i18nString(UIStrings.appIdExplainer)}
                jslog=${VisualLogging.action('help').track({ hover: true })}
              >
              </devtools-icon>
              <devtools-link
                href="https://developer.chrome.com/blog/pwa-manifest-id/"
  ...
  return html`${renderSectionHeader(i18nString(UIStrings.presentation), output)}
    <div class="report-section" jslog=${VisualLogging.section('presentation')}>
      ...
      <devtools-report-value>
        ${completeStartUrl
          ? (() => {
              const link = linkifyURL(completeStartUrl, {
                text: startUrl,
                tabStop: true,
                jslogContext: 'start-url',
              });
...
function renderProtocolHandlers(data, output) {
  ...
  return html`${renderSectionHeader(i18nString(UIStrings.protocolHandlers), output)}
    <div class="report-row">
      <devtools-widget
        ${widget(ApplicationComponents.ProtocolHandlersView.ProtocolHandlersView, {
          protocolHandlers: data.protocolHandlers,
          manifestLink: data.manifestLink,
        })}
...
function renderImage(imageSrc, imageUrl, naturalWidth) {
  ...
  return html` <div class="image-wrapper">
    <img
      src=${imageSrc}
      alt=${i18nString(UIStrings.imageFromS, { PH1: imageUrl })}
      width=${naturalWidth}
    />
  </div>`;
  // clang-format on
  ...
  return html`${renderSectionHeader(i18nString(UIStrings.icons), output)}
    <div class="report-section" jslog=${VisualLogging.section('icons')}>
      <div class="report-row">
        <devtools-checkbox
          class="mask-checkbox"
          jslog=${VisualLogging.toggle('show-minimal-safe-area-for-maskable-icons').track({
            change: true,
          })}
      ...
      <div class="report-row">
        ${i18nTemplate(str_, UIStrings.needHelpReadOurS, {
          PH1: html` <devtools-link href="https://web.dev/maskable-icon/" jslogcontext="learn-more">
...
function renderShortcuts(data) {
  // clang-format off
  return html`${data.shortcuts.map(
    (shortcut, index) =>
      html` ${renderSectionHeader(i18nString(UIStrings.shortcutS, { PH1: index + 1 }))}
        <div class="report-section" jslog=${VisualLogging.section('shortcuts')}>
          ...
          <devtools-report-value>
            ${linkifyURL(shortcut.shortcutUrl, {
              text: shortcut.url,
              tabStop: true,
              jslogContext: 'shortcut',
            })}
          </devtools-report-value>
          ${Array.from(shortcut.icons).map(
            ([title, images]) => html`
              <devtools-report-key>${title}</devtools-report-key>
              <devtools-report-value>
                ${images
...
function renderScreenshots(data) {
  // clang-format off
  return html`${data.screenshots.map(
    ({ screenshot, processedImage }, index) => html`
      ...
      <div class="report-section" jslog=${VisualLogging.section('screenshots')}>
        ...
        ${screenshot.platform
          ...
          : nothing}
        ${'imageSrc' in processedImage
          ? html` <devtools-report-key>${processedImage.title}</devtools-report-key>
              <devtools-report-value>
                ${renderImage(
...
function renderInstallability(installabilityErrors) {
  ...
  ${getInstallabilityErrorMessages(installabilityErrors).map(
    (content) => html` <div class="report-row">${content}</div> `,
  )}`;
}
function renderWindowControlsSection(
  ...
  // clang-format off
  return html` ${renderSectionHeader(i18nString(UIStrings.windowControlsOverlay), output)}
    <div class="report-section" jslog=${VisualLogging.section('window-controls-overlay')}>
      ...
      <div class="report-row">
        ${i18nTemplate(str_, UIStrings.wcoNeedHelpReadMore, {
...
function renderWindowControls(selectedPlatform, onSelectOs, onToggleWcoToolbar) {
  ...
  return html`<div class="report-row">
    <devtools-checkbox
      @click=${(event) => onToggleWcoToolbar(event.target.checked)}
    ...
    >
      <option
        value=${'Windows' /* SDK.OverlayModel.EmulatedOSType.WINDOWS */}
        jslog=${VisualLogging.item('windows').track({ click: true })}
      >
        Windows
      </option>
      <option
        value=${'Mac' /* SDK.OverlayModel.EmulatedOSType.MAC */}
        jslog=${VisualLogging.item('macos').track({ click: true })}
      >
        macOS
      </option>
      <option
        value=${'Linux' /* SDK.OverlayModel.EmulatedOSType.LINUX */}
  ...
  render(
    html` <style>
      ...
      ${isEmpty
        ...
        : html` <devtools-report
            .data=${{ reportTitle: i18nString(UIStrings.appManifest), reportUrl: url }}
          >
            ...
            ${iconsData && onToggleIconMasked
              ? renderIcons(iconsData, Boolean(maskedIcons), onToggleIconMasked, output)
              : nothing}
            ${windowControlsData && output
...
export class AppManifestView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  targetAdded(target) {
    ...
    this.registeredListeners = [
      ...
      this.serviceWorkerManager.addEventListener(
        'RegistrationUpdated' /* SDK.ServiceWorkerManager.Events.REGISTRATION_UPDATED */,
        () => {
  ...
  async performUpdate() {
    ...
    const onSelectOs = this.overlayModel
      ...
      : undefined;
    const onToggleWcoToolbar = this.overlayModel
      ? (enabled) => this.onToggleWcoToolbar(enabled)
      : undefined;
    const onCopyId = recommendedId
      ? () => {
          UI.ARIAUtils.LiveAnnouncer.alert(
            i18nString(UIStrings.copiedToClipboard, { PH1: recommendedId }),
          );
    ...
    for (const size of rawSizeArray) {
      ...
      if (match) {
        ...
        imageResourceErrors.push(
          i18nString(UIStrings.sSShouldSpecifyItsSizeAs, { PH1: resourceName, PH2: imageUrl }),
        );
      }
    }
    return parsedSizes;
  ...
  async processImageResource(
    baseUrl,
    imageResource, // eslint-disable-line @typescript-eslint/no-explicit-any
    isScreenshot,
  ) {
    ...
    if (!result) {
      imageResourceErrors.push(
        i18nString(UIStrings.sSFailedToLoad, { PH1: resourceName, PH2: imageUrl }),
      );
    ...
    if (!imageResource.sizes) {
      imageResourceErrors.push(
        i18nString(UIStrings.sSDoesNotSpecifyItsSizeInThe, { PH1: resourceName, PH2: imageUrl }),
      );
    } else {
      ...
      for (const size of sizes) {
        ...
        if (error) {
          ...
          if (width < 320 || height < 320) {
            ...
            imageResourceErrors.push(
              i18nString(UIStrings.sSWidthDoesNotComplyWithRatioRequirement, {
                PH1: resourceName,
                PH2: imageUrl,
              }),
            );
            ...
            imageResourceErrors.push(
              i18nString(UIStrings.sSHeightDoesNotComplyWithRatioRequirement, {
                PH1: resourceName,
                PH2: imageUrl,
              }),
            );
    ...
    const purpose =
      typeof imageResource['purpose'] === 'string' ? imageResource['purpose'].toLowerCase() : '';
    ...
    for (const shortcut of shortcuts) {
      ...
      for (const shortcutIcon of shortcutIcons) {
        ...
        if (!hasShortcutIconLargeEnough && shortcutIcon.sizes) {
          ...
          if (
            shortcutIconSize &&
            Number(shortcutIconSize[1]) >= 96 &&
            Number(shortcutIconSize[2]) >= 96
          ) {
      ...
      if (!hasShortcutIconLargeEnough) {
        imageErrors.push(
          i18nString(UIStrings.shortcutSShouldIncludeAXPixel, { PH1: shortcutIndex }),
        );
      }
      shortcutIndex++;
    }
  ...
  async processScreenshots(parsedManifest, url) {
    ...
    for (const screenshot of screenshots) {
      ...
      if (screenshot.form_factor && 'naturalWidth' in result) {
        ...
        const formFactorFirstScreenshotDimensions = formFactorScreenshotDimensions.get(
          screenshot.form_factor,
        );
  ...
  processPresentation(parsedManifest, url) {
    ...
    const themeColor = themeColorString
      ? (Common.Color.parse(themeColorString) ?? Common.Color.parse('white'))
      : null;
    ...
    const backgroundColor = backgroundColorString
      ? (Common.Color.parse(backgroundColorString) ?? Common.Color.parse('white'))
      : null;
