...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/ServiceWorkerUpdateCycleView.ts',
  UIStrings,
);
...
export class ServiceWorkerUpdateCycleView {
  ...
  calculateServiceWorkerUpdateRanges() {
    ...
    function rangesForVersion(version) {
      ...
      while (state) {
        ...
        if (
          state.status ===
          'activated' /* Protocol.ServiceWorker.ServiceWorkerVersionStatus.Activated */
        ) {
          ...
          state.status ===
          'installed' /* Protocol.ServiceWorker.ServiceWorkerVersionStatus.Installed */
        ) {
  ...
  createTimingTableHead() {
    const serverHeader = this.tableElement.createChild(
      'tr',
      'service-worker-update-timing-table-header',
    );
    UI.UIUtils.createTextChild(serverHeader.createChild('td'), i18nString(UIStrings.version));
    UI.UIUtils.createTextChild(
      serverHeader.createChild('td'),
      i18nString(UIStrings.updateActivity),
    );
  ...
  updateTimingTable(timeRanges) {
    ...
    for (const range of timeRangeArray) {
      ...
      tr.setAttribute(
        'jslog',
        `${VisualLogging.treeItem('update-timeline').track({
          click: true,
          resize: true,
      ...
      timingBarVersionElement.setAttribute(
        'jslog',
        `${VisualLogging.expand('timing-info').track({ click: true })}`,
      );
      ...
      const barContainer = tr
        .createChild('td')
        .createChild('div', 'service-worker-update-timing-row');
      const bar = barContainer.createChild(
        'span',
        'service-worker-update-timing-bar ' + phaseName.toLowerCase(),
      );
      bar.style.left = left + '%';
  ...
  constructUpdateDetails(tr, range) {
    const startRow = this.tableElement.createChild(
      'tr',
    ...
    UI.UIUtils.createTextChild(
      startTimeItem.createChild('span'),
      i18nString(UIStrings.startTimeS, { PH1: startTime }),
    );
    startRow.tabIndex = 0;
    ...
    UI.UIUtils.createTextChild(
      endTimeItem.createChild('span'),
      i18nString(UIStrings.endTimeS, { PH1: endTime }),
    );
    endRow.tabIndex = 0;
  ...
  onKeydown(event, startRow, endRow) {
    ...
    if (
      (!expanded && keyboardEvent.key === 'ArrowRight') ||
      (expanded && keyboardEvent.key === 'ArrowLeft')
    ) {
      this.toggle(startRow, endRow, target, expanded);
      event.preventDefault();
      return;
