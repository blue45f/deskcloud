...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/components/PermissionsPolicySection.ts',
  UIStrings,
);
...
function renderAllowed(allowed) {
  ...
  return html` <devtools-report-key>${i18nString(UIStrings.allowedFeatures)}</devtools-report-key>
    <devtools-report-value
      >${allowed.map(({ feature }) => feature).join(', ')}</devtools-report-value
    >`;
}
function renderDisallowed(data, showDetails, onToggleShowDetails, onRevealDOMNode, onRevealHeader) {
  ...
  if (!showDetails) {
    // clang-format off
    return html` <devtools-report-key
        >${i18nString(UIStrings.disabledFeatures)}</devtools-report-key
      >
      <devtools-report-value>
  ...
  const featureRows = data.map(({ policy, blockReason, linkTargetDOMNode, linkTargetRequest }) => {
    ...
    // Disabled until https://crbug.com/1079231 is fixed.
    // clang-format off
    return html` <div class="permissions-row">
      <div>
        <devtools-icon class="allowed-icon extra-large" name="cross-circle"> </devtools-icon>
      </div>
  ...
  // clang-format off
  return html` <devtools-report-key>${i18nString(UIStrings.disabledFeatures)}</devtools-report-key>
...
const DEFAULT_VIEW = (input, output, target) => {
  ...
  render(
    html` <style>
        ${permissionsPolicySectionStyles}
      </style>
...
export class PermissionsPolicySection extends UI.Widget.Widget {
  ...
  async performUpdate() {
    ...
    const disallowed = policies
      .filter((p) => !p.allowed)
      .sort((a, b) => a.feature.localeCompare(b.feature));
    const disallowedData = this.#showDetails
      ? await Promise.all(
          disallowed.map(async (policy) => {
            const frame = policy.locator
              ? frameManager.getFrame(policy.locator.frameId)
              : undefined;
            const blockReason = policy.locator?.blockReason;
            const linkTargetDOMNode = await ((blockReason ===
              'IframeAttribute' /* Protocol.Page.PermissionsPolicyBlockReason.IframeAttribute */ &&
              frame?.getOwnerDOMNodeOrDocument()) ||
              undefined);
            const resource = frame?.resourceForURL(frame.url);
            const linkTargetRequest =
              (blockReason === 'Header' /* Protocol.Page.PermissionsPolicyBlockReason.Header */ &&
                resource?.request) ||
              undefined;
