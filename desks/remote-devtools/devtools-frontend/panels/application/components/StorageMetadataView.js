...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/components/StorageMetadataView.ts',
  UIStrings,
);
...
export class StorageMetadataView extends LegacyWrapper.LegacyWrapper.WrappableComponent {
    ...
    return RenderCoordinator.write('StorageMetadataView render', async () => {
      ...
      render(
        html` <style>
          ...
          <devtools-report
            .data=${{ reportTitle: this.getTitle() ?? i18nString(UIStrings.loading) }}
          >
  ...
  async renderReportContent() {
    ...
    const origin = this.#storageKey.origin;
    const ancestorChainHasCrossSite = Boolean(
      this.#storageKey.components.get(
    ...
    const thirdPartyReason = ancestorChainHasCrossSite
      ? i18nString(UIStrings.yesBecauseAncestorChainHasCrossSite)
      : hasNonce
        ? i18nString(UIStrings.yesBecauseKeyIsOpaque)
        : topLevelSiteIsOpaque
          ? i18nString(UIStrings.yesBecauseTopLevelIsOpaque)
          : topLevelSite && origin !== topLevelSite
            ? i18nString(UIStrings.yesBecauseOriginNotInTopLevelSite)
            : null;
    const isIframeOrEmbedded = topLevelSite && origin !== topLevelSite;
    // Disabled until https://crbug.com/1079231 is fixed.
    // clang-format off
    return html` ${isIframeOrEmbedded
    ...
    ${thirdPartyReason
      ? html` ${this.key(i18nString(UIStrings.isThirdParty))}${this.value(thirdPartyReason)}`
      : nothing}
  ...
  #renderStorageBucketInfo() {
    ...
    if (!this.#showOnlyBucket) {
      if (isDefault) {
        return html` ${this.key(i18nString(UIStrings.bucketName))}
        ${this.value(html`<span class="default-bucket">default</span>`)}`;
      }
      return html` ${this.key(i18nString(UIStrings.bucketName))} ${this.value(name)}`;
    }
    // clang-format off
    return html` ${this.key(i18nString(UIStrings.bucketName))}
    ...
    ${this.key(i18nString(UIStrings.durability))} ${this.value(durability)}
    ${this.key(i18nString(UIStrings.quota))} ${this.value(i18n.ByteUtilities.bytesToString(quota))}
    ${this.key(i18nString(UIStrings.expiration))} ${this.value(this.#getExpirationString())}`;
  }
  #getExpirationString() {
    if (!this.#storageBucket) {
  ...
  #renderBucketControls() {
    // clang-format off
    return html` <devtools-report-divider></devtools-report-divider>
