...
const renderForceRunButton = (input) => {
  ...
  return html`
    ...
    >
      ${isMitigationRunning
        ? html` ${i18nString(UIStrings.runningMitigations)}`
        : `
  ...
  if (input.trackingSites.length === 0) {
    ...
    return html`
      <devtools-report-section>
        ${input.screenStatus === 'Running' /* ScreenStatusType.RUNNING */
          ? html` ${i18nString(UIStrings.checkingPotentialTrackers)}`
          : `
  ...
  return html`
    <devtools-report-section>
      <devtools-data-grid striped inline>
        <table>
          <tr>
            <th id="sites" weight="10" sortable>${i18nString(UIStrings.stateDeletedFor)}</th>
          </tr>
          ${input.trackingSites.map(
            (site) =>
              html` <tr>
                <td>${site}</td>
              </tr>`,
...
const renderMainFrameInformation = (input) => {
  ...
  if (input.screenStatus === 'Disabled' /* ScreenStatusType.DISABLED */) {
    // clang-format off
    return html`
      <devtools-report-section> ${i18nString(UIStrings.featureDisabled)} </devtools-report-section>
    `;
    // clang-format on
  }
  // clang-format off
  return html`
    <devtools-report-section> ${renderForceRunButton(input)} </devtools-report-section>
    ${renderDeletedSitesOrNoSitesMessage(input)}
    <devtools-report-divider> </devtools-report-divider>
    <devtools-report-section>
      <devtools-link
        href="https://privacycg.github.io/nav-tracking-mitigations/#bounce-tracking-mitigations"
        class="link"
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  Lit.render(
    html`
      <style>
        ${bounceTrackingMitigationsViewStyles}
      </style>
      <style>
        ${UI.inspectorCommonStyles}
      </style>
      <devtools-report
        .data=${{ reportTitle: i18nString(UIStrings.bounceTrackingMitigationsTitle) }}
  ...
  constructor(element, view = DEFAULT_VIEW) {
    ...
    if (!mainTarget) {
      ...
      void mainTarget
        ...
        .then((state) => {
          this.#screenStatus = state.featureEnabled
            ? 'Result' /* ScreenStatusType.RESULT */
            : 'Disabled' /* ScreenStatusType.DISABLED */;
          this.requestUpdate();
        });
    }
  }
