...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/components/ProtocolHandlersView.ts',
  UIStrings,
);
...
function renderStatusMessage(protocolHandlers, manifestLink) {
  const statusString =
    protocolHandlers.length > 0 ? UIStrings.protocolDetected : UIStrings.protocolNotDetected;
  // clang-format off
  return html` <div class="protocol-handlers-row status">
    <devtools-icon
      class="inline-icon"
      name=${protocolHandlers.length > 0 ? 'check-circle' : 'info'}
    >
    </devtools-icon>
    ${uiI18n.getFormatLocalizedStringTemplate(str_, statusString, {
      PH1: html`
        <devtools-link href=${manifestLink} jslogcontext="manifest"
          >${i18nString(UIStrings.manifest)}</devtools-link
        >
      `,
    })}
  </div>`;
...
function renderProtocolTest(
  protocolHandlers,
  queryInputState,
  protocolSelectHandler,
  queryInputChangeHandler,
  testProtocolClickHandler,
) {
  if (protocolHandlers.length === 0) {
    return nothing;
  }
  // clang-format off
  return html` <div class="protocol-handlers-row">
    <select
      class="protocol-select"
      @change=${protocolSelectHandler}
      ...
      ${protocolHandlers
        ...
        .map(
          ({ protocol }) =>
            html` <option
              value=${protocol}
              jslog=${VisualLogging.item(protocol).track({ click: true })}
            >
              ${protocol}://
            </option>`,
        )}
    ...
    <input
      ...
      type="text"
      @change=${queryInputChangeHandler}
      aria-label=${i18nString(UIStrings.textboxLabel)}
      placeholder=${i18nString(UIStrings.textboxPlaceholder)}
    />
    <devtools-button
      .variant=${'primary' /* Buttons.Button.Variant.PRIMARY */}
      @click=${testProtocolClickHandler}
    >
  ...
  render(
    html`
      ...
      <div class="protocol-handlers-row">
        ${i18nTemplate(UIStrings.needHelpReadOur, {
          PH1: html` <devtools-link
            href=${PROTOCOL_DOCUMENT_URL}
            class="devtools-link"
            autofocus
