...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/components/SharedStorageMetadataView.ts',
  UIStrings,
);
...
export class SharedStorageMetadataView extends StorageMetadataView {
  ...
  async renderReportContent() {
    ...
    // clang-format off
    return html` <style>
        ${sharedStorageMetadataViewStyles}
      </style>
      ${await super.renderReportContent()} ${this.key(i18nString(UIStrings.creation))}
      ${this.value(this.#renderDateForCreationTime())} ${this.key(i18nString(UIStrings.numEntries))}
      ${this.value(String(this.#length))} ${this.key(i18nString(UIStrings.numBytesUsed))}
      ${this.value(String(this.#bytesUsed))}
      ${this.key(
        html`<span class="entropy-budget"
          >${i18nString(UIStrings.entropyBudget)}<devtools-icon
            name="info"
            title=${i18nString(UIStrings.budgetExplanation)}
          ></devtools-icon
        ></span>`,
      )}
      ${this.value(
        html`<span class="entropy-budget"
          >${this.#remainingBudget}${this.#renderResetBudgetButton()}</span
        >`,
      )}`;
    // clang-format on
  }
  ...
  #renderResetBudgetButton() {
    ...
    return html`
      <devtools-button
        ...
        @click=${this.#resetBudget.bind(this)}
      ></devtools-button>
    `;
    // clang-format on
  }
