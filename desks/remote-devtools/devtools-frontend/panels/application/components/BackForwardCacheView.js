...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/components/BackForwardCacheView.ts',
  UIStrings,
);
...
function renderMainFrameInformation(
  ...
  if (!frame) {
    // clang-format of
    return html` <devtools-report-key> ${i18nString(UIStrings.mainFrame)} </devtools-report-key>
      <devtools-report-value> ${i18nString(UIStrings.unavailable)} </devtools-report-value>`;
    // clang-format on
  }
  ...
  // clang-format off
  return html` ${renderBackForwardCacheStatus(frame.backForwardCacheDetails.restoredFromCache)}
...
function maybeRenderFrameTree(frameTreeData) {
  ...
  function renderFrameTreeNode(node) {
    // clang-format off
    return html` <li role="treeitem" class="text-ellipsis">
      ${node.iconName
        ? html`
            <devtools-icon
              class="inline-icon extra-large"
              .name=${node.iconName}
  ...
  // clang-format off
  return html` <devtools-report-key jslog=${VisualLogging.section('frames')}
      >${i18nString(UIStrings.framesTitle)}</devtools-report-key
    >
...
function renderBackForwardCacheStatus(status) {
  switch (status) {
    case true:
      // clang-format off
      return html` <devtools-report-section autofocus tabindex="-1">
    ...
    case false:
      ...
      return html` <devtools-report-section autofocus tabindex="-1">
        <div class="status">
          <devtools-icon class="inline-icon extra-large" name="clear"> </devtools-icon>
        </div>
...
function maybeRenderExplanations(explanations, explanationTree, reasonToFramesMap) {
  ...
  const pageSupportNeeded = explanations.filter(
    (explanation) =>
      explanation.type ===
      'PageSupportNeeded' /* Protocol.Page.BackForwardCacheNotRestoredReasonType.PageSupportNeeded */,
  );
  const supportPending = explanations.filter(
    (explanation) =>
      explanation.type ===
      'SupportPending' /* Protocol.Page.BackForwardCacheNotRestoredReasonType.SupportPending */,
  );
  const circumstantial = explanations.filter(
    (explanation) =>
      explanation.type ===
      'Circumstantial' /* Protocol.Page.BackForwardCacheNotRestoredReasonType.Circumstantial */,
  );
  // Disabled until https://crbug.com/1079231 is fixed.
  // clang-format off
  return html` ${renderExplanations(
    i18nString(UIStrings.pageSupportNeeded),
    i18nString(UIStrings.pageSupportNeededExplanation),
    pageSupportNeeded,
    reasonToFramesMap,
  )}
  ${renderExplanations(
    i18nString(UIStrings.supportPending),
    i18nString(UIStrings.supportPendingExplanation),
    supportPending,
    reasonToFramesMap,
  )}
  ${renderExplanations(
    i18nString(UIStrings.circumstantial),
    i18nString(UIStrings.circumstantialExplanation),
    circumstantial,
    reasonToFramesMap,
  )}`;
  // clang-format on
...
function renderExplanations(category, explainerText, explanations, reasonToFramesMap) {
  // Disabled until https://crbug.com/1079231 is fixed.
  // clang-format off
  return html` ${explanations.length > 0
    ? html`
        ...
        ${explanations.map((explanation) =>
          renderReason(explanation, reasonToFramesMap.get(explanation.reason)),
        )}
      `
    : nothing}`;
  // clang-format on
...
function renderFramesPerReason(frames) {
    ...
    ...frames.map(
      (url) =>
        html`<div
          class="text-ellipsis"
          title=${url}
          jslog=${VisualLogging.treeItem().track({ resize: true })}
        >
          ${url}
        </div>`,
    ),
  ...
  return html`
    <div class="details-list" jslog=${VisualLogging.tree('frames-per-issue')}>
...
function maybeRenderDeepLinkToUnload(explanation) {
  if (
    explanation.reason ===
      'UnloadHandlerExistsInMainFrame' /* Protocol.Page.BackForwardCacheNotRestoredReason.UnloadHandlerExistsInMainFrame */ ||
    explanation.reason ===
      'UnloadHandlerExistsInSubFrame' /* Protocol.Page.BackForwardCacheNotRestoredReason.UnloadHandlerExistsInSubFrame */
  ) {
    return html` <devtools-link
      href="https://web.dev/bfcache/#never-use-the-unload-event"
      class="link"
...
function maybeRenderJavaScriptDetails(details) {
  ...
  rows.push(
    ...details.map(
      (detail) =>
        html` ${widget(Components.Linkifier.ScriptLocationLink, {
...
function renderReason(explanation, frames) {
  // clang-format off
  return html` <devtools-report-section>
      ${explanation.reason in NotRestoredReasonDescription
        ? html` <div class="circled-exclamation-icon">
              <devtools-icon
                class="inline-icon medium"
            ...
            <div>
              ...
              ${maybeRenderDeepLinkToUnload(explanation)} ${maybeRenderReasonContext(explanation)}
            </div>`
        : nothing}
    </devtools-report-section>
    <div class="gray-text">${explanation.reason}</div>
    ${maybeRenderJavaScriptDetails(explanation.details)} ${renderFramesPerReason(frames)}`;
  // clang-format on
}
const DEFAULT_VIEW = (input, output, target) => {
  ...
  render(
    html`
      <style>
        ${backForwardCacheViewStyles}
      </style>
      <devtools-report
        .data=${{ reportTitle: i18nString(UIStrings.backForwardCacheTitle) }}
        jslog=${VisualLogging.pane('back-forward-cache')}
      >
        ${renderMainFrameInformation(
          input.frame,
          input.frameTreeData,
          input.reasonToFramesMap,
          input.screenStatus,
          input.navigateAwayAndBack,
        )}
      </devtools-report>
    `,
    target,
...
export class BackForwardCacheView extends UI.Widget.Widget {
  ...
  async #waitAndGoBackInHistory(delay) {
    ...
    if (historyResults.currentIndex === this.#historyIndex) {
      ...
      resourceTreeModel.navigateToHistoryEntry(
        historyResults.entries[historyResults.currentIndex - 1],
      );
    }
  }
