...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/components/SharedStorageAccessGrid.ts',
  UIStrings,
);
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html` <style>
        ${sharedStorageAccessGridStyles}
      </style>
      ${input.events.length === 0
        ? html` <div class="empty-state" jslog=${VisualLogging.section().context('empty-view')}>
        ...
        : html` <div jslog=${VisualLogging.section('events-table')}>
            <span class="heading">${i18nString(UIStrings.sharedStorage)}</span>
            <devtools-icon
              class="info-icon medium"
              name="info"
            ...
            <devtools-data-grid striped inline>
              <table>
                <thead>
                  <tr>
                    <th id="event-time" weight="10" sortable>${i18nString(UIStrings.eventTime)}</th>
