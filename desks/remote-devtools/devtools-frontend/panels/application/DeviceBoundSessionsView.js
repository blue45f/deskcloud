...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/DeviceBoundSessionsView.ts',
  UIStrings,
);
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  const toolbarHtml = preserveLogSetting
    ? html`
        <devtools-toolbar class="device-bound-sessions-toolbar">
          <devtools-checkbox
            title=${i18nString(UIStrings.doNotClearLogOnPageReload)}
            ${UI.UIUtils.bindToSetting(preserveLogSetting)}
            >${i18nString(UIStrings.preserveLog)}</devtools-checkbox
          >
        </devtools-toolbar>
      `
    : nothing;
  if (!sessionAndEvents) {
    ...
    render(
      html`
        ...
        <style>
          ${deviceBoundSessionsViewStyles}
        </style>
        ${toolbarHtml}
        <devtools-widget
          ${widget(UI.EmptyWidget.EmptyWidget, { header: defaultTitle, text: defaultDescription })}
          jslog=${VisualLogging.pane('device-bound-sessions-empty')}
        ></devtools-widget>
      `,
      target,
    );
  ...
  if (sessionAndEvents.session) {
    ...
    sessionDetailsHtml = html` <devtools-report>
        ...
        <devtools-report-value
          >${new Date(
            sessionAndEvents.session.expiryDate * 1000,
          ).toLocaleString()}</devtools-report-value
        >
        <devtools-report-key>${i18nString(UIStrings.cachedChallenge)}</devtools-report-key>
        <devtools-report-value
          >${sessionAndEvents.session.cachedChallenge || ''}</devtools-report-value
        >
        <devtools-report-key>${i18nString(UIStrings.allowedRefreshInitiators)}</devtools-report-key>
        <devtools-report-value
          >${sessionAndEvents.session.allowedRefreshInitiators.join(', ')}</devtools-report-value
        >
        <devtools-report-section-header role="heading" aria-level="2"
      ...
      ${inclusionRules.urlRules.length > 0
        ? html`
            <div class="device-bound-session-grid-wrapper">
              <devtools-data-grid
                class="device-bound-session-url-rules-grid"
                striped
                inline
                name=${i18nString(UIStrings.scope)}
              >
                <table>
                  <thead>
                    <tr>
      ...
      >
      ${cookieCravings.length > 0
        ? html`
            <div class="device-bound-session-grid-wrapper">
              <devtools-data-grid
                class="device-bound-session-cookie-cravings-grid"
                striped
                inline
                name=${i18nString(UIStrings.cookieCravings)}
              >
                <table>
                  <thead>
                    <tr>
                      ...
                      <th id="secure" type="boolean" align="center" sortable>
                        ${i18n.i18n.lockedString('Secure')}
                      </th>
                      <th id="http-only" type="boolean" align="center" sortable>
                        ${i18n.i18n.lockedString('HttpOnly')}
                      </th>
  ...
  const eventsHtml = html` <devtools-report-section-header role="heading" aria-level="2"
    ...
    ${events.length > 0 && onEventRowSelected
      ? html`
          <div class="device-bound-session-grid-wrapper">
            <devtools-data-grid
              class="device-bound-session-events-grid"
              striped
              inline
              name=${i18nString(UIStrings.events)}
              ${Directives.ref((el) => {
        ...
        `
      : html`<div class="device-bound-session-no-events-wrapper">
          ${i18nString(UIStrings.noEvents)}
        </div>`}`;
  const failedRequestDetailsGetter = (failedRequest) => {
    ...
    // clang-format off
    return html`${failedRequest.requestUrl &&
    html` <devtools-report-key>${i18nString(UIStrings.failedRequestUrl)}</devtools-report-key>
      <devtools-report-value>${failedRequest.requestUrl}</devtools-report-value>`}
    ${failedRequest.netError &&
    html` <devtools-report-key>${i18nString(UIStrings.failedRequestNetError)}</devtools-report-key>
      <devtools-report-value>${failedRequest.netError}</devtools-report-value>`}
    ${failedRequest.responseError !== undefined
      ? html` <devtools-report-key
            >${i18nString(UIStrings.failedRequestResponseCode)}</devtools-report-key
          >
          <devtools-report-value>${failedRequest.responseError}</devtools-report-value>`
      : nothing}
    ${failedRequest.responseErrorBody &&
    html` <devtools-report-key
        >${i18nString(UIStrings.failedRequestResponseBody)}</devtools-report-key
      >
      <devtools-report-value>
  ...
  const creationEventDetails =
    ...
    html`
      ...
      <devtools-report-value
        >${fetchResultToString(
          selectedEvent.creationEventDetails.fetchResult,
        )}</devtools-report-value
      >
  ...
  const refreshEventDetails =
    ...
    html`
      ...
      <devtools-report-value
        >${refreshResultToString(
          selectedEvent.refreshEventDetails.refreshResult,
        )}</devtools-report-value
      >
      <devtools-report-key>${i18nString(UIStrings.causedAnyRequestDeferrals)}</devtools-report-key>
      <devtools-report-value
        >${boolToString(
          !selectedEvent.refreshEventDetails.wasFullyProactiveRefresh,
        )}</devtools-report-value
      >
  ...
  const challengeEventDetails =
    ...
    html`
      ...
      <devtools-report-value
        >${challengeResultToString(
          selectedEvent.challengeEventDetails.challengeResult,
        )}</devtools-report-value
      >
      <devtools-report-key>${i18nString(UIStrings.challenge)}</devtools-report-key>
      <devtools-report-value
        >${selectedEvent.challengeEventDetails.challenge}</devtools-report-value
      >
    `;
  const terminationEventDetails =
    ...
    html`
      ...
      <devtools-report-value
        >${deletionReasonToString(
          selectedEvent.terminationEventDetails.deletionReason,
        )}</devtools-report-value
      >
    `;
  const eventDetailsContentHtml = selectedEvent
    ? html`
        <devtools-report>
          ...
          <devtools-report-value
            >${succeededToString(selectedEvent.succeeded)}</devtools-report-value
          >
          ${creationEventDetails} ${refreshEventDetails} ${challengeEventDetails}
          ${terminationEventDetails}
        </devtools-report>
      `
    : html`<div class="device-bound-session-no-event-details">
        ${i18nString(UIStrings.selectEventToViewDetails)}
      </div>`;
  ...
  render(
    html` <style>
      ...
      <devtools-split-view sidebar-position="second">
        <div
          slot="main"
          class="device-bound-session-view-wrapper"
          role="region"
          aria-label=${i18nString(UIStrings.sessionDetails)}
        >
          ${sessionDetailsHtml || nothing} ${eventsHtml}
        </div>
        <div
          slot="sidebar"
          class="device-bound-session-sidebar"
          role="region"
          aria-label=${i18nString(UIStrings.eventDetails)}
        >
          ${eventDetailsHtml}
        </div>
      </devtools-split-view>`,
  ...
  #attachModel(model) {
    if (this.#model) {
      this.#model.removeEventListener(
        'EVENT_OCCURRED' /* DeviceBoundSessionModelEvents.EVENT_OCCURRED */,
        this.performUpdate,
        this,
      );
      this.#model.removeEventListener(
        'CLEAR_EVENTS' /* DeviceBoundSessionModelEvents.CLEAR_EVENTS */,
        this.performUpdate,
        this,
      );
    }
