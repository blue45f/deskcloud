...
export class SharedStorageItemsView extends KeyValueStorageItemsView {
  ...
  async refreshItems() {
    ...
    this.sharedStorageItemsDispatcher.dispatchEventToListeners(
      'ItemsRefreshed' /* SharedStorageItemsDispatcher.Events.ITEMS_REFRESHED */,
    );
  }
  async deleteAllItems() {
    if (!this.toolbar?.hasFilter()) {
      ...
      await this.refreshItems();
      this.sharedStorageItemsDispatcher.dispatchEventToListeners(
        'ItemsCleared' /* SharedStorageItemsDispatcher.Events.ITEMS_CLEARED */,
      );
    ...
    await this.refreshItems();
    this.sharedStorageItemsDispatcher.dispatchEventToListeners(
      'FilteredItemsCleared' /* SharedStorageItemsDispatcher.Events.FILTERED_ITEMS_CLEARED */,
    );
  ...
  async setItem(key, value) {
    ...
    await this.refreshItems();
    this.sharedStorageItemsDispatcher.dispatchEventToListeners(
      'ItemEdited' /* SharedStorageItemsDispatcher.Events.ITEM_EDITED */,
    );
  ...
  #showSharedStorageItems(items) {
    if (this.toolbar) {
      const filteredList = items.filter(
        (item) => this.toolbar?.filterRegex?.test(`${item.key} ${item.value}`) ?? true,
      );
  ...
  async removeItem(key) {
    ...
    await this.refreshItems();
    this.sharedStorageItemsDispatcher.dispatchEventToListeners(
      'ItemDeleted' /* SharedStorageItemsDispatcher.Events.ITEM_DELETED */,
      { key },
    );
