  ...
  render(
    html`
      ...
      <div class="vbox flex-auto" jslog=${VisualLogging.pane('crash-report-context')}>
        <devtools-toolbar
          class="crash-report-context-toolbar"
          role="toolbar"
        ...
        >
          ...
          <devtools-toolbar-input
            ...
            @change=${(e) => input.onFilterChanged(e)}
            class="flex-auto"
          >
          </devtools-toolbar-input>
        </devtools-toolbar>
...
Frame ID: ${frame.frameId}"
                          >${frame.displayName}</span
                        >
                      </div>
                      <div class="grid-container">
                        <devtools-widget
                          ${widget(
                            ApplicationComponents.CrashReportContextGrid.CrashReportContextGrid,
                            {
                              data: {
...
export class CrashReportContextView extends UI.Widget.VBox {
  ...
  async performUpdate() {
    const models = SDK.TargetManager.TargetManager.instance().models(
      SDK.CrashReportContextModel.CrashReportContextModel,
    );
    ...
    this.#view(
      {
        ...
        onFilterChanged: (e) => {
          ...
          const textFilterRegExp = text
            ...
            : null;
          if (textFilterRegExp) {
            this.#filters = [{ key: 'key,value', regex: textFilterRegExp, negative: false }];
          } else {
            this.#filters = [];
          }
          this.requestUpdate();
