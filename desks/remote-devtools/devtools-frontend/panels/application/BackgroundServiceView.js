...
export class BackgroundServiceView extends UI.Widget.VBox {
  ...
  constructor(serviceName, model) {
    ...
    this.serviceWorkerManager = this.model
      .target()
      .model(SDK.ServiceWorkerManager.ServiceWorkerManager);
    ...
    this.storageKeyManager.addEventListener(
      'MainStorageKeyChanged' /* SDK.StorageKeyManager.Events.MAIN_STORAGE_KEY_CHANGED */,
      () => this.onStorageKeyChanged(),
    );
    this.recordAction = UI.ActionRegistry.ActionRegistry.instance().getAction(
    ...
    this.previewPanel.element.setAttribute(
      'jslog',
      `${VisualLogging.pane('preview').track({ resize: true })}`,
    );
  ...
  updateRecordButtonTooltip() {
    const buttonTooltip = this.recordButton.isToggled()
      ? i18nString(UIStrings.stopRecordingEvents)
  ...
  showPreview(dataNode) {
    ...
    if (this.dataGrid.rootNode().children.length) {
      ...
      const recordShortcuts = UI.ShortcutRegistry.ShortcutRegistry.instance().shortcutsForAction(
        'background-service.toggle-recording',
      )[0];
      emptyWidget = new UI.EmptyWidget.EmptyWidget(
      ...
      const button = UI.UIUtils.createTextButton(
        ...
        { jslogContext: 'start-recording', variant: 'tonal' /* Buttons.Button.Variant.TONAL */ },
      );
...
export class EventDataNode extends DataGrid.DataGrid.DataGridNode {
  ...
  constructor(data, eventMetadata) {
    super(data);
    this.eventMetadata = eventMetadata.sort((m1, m2) =>
      Platform.StringUtilities.compare(m1.key, m2.key),
    );
  }
  createPreview() {
    ...
    for (const entry of this.eventMetadata) {
      ...
      if (entry.value) {
        div.createChild('div', 'background-service-metadata-value source-code').textContent =
          entry.value;
      } else {
        div.createChild(
          'div',
          'background-service-metadata-value background-service-empty-value',
        ).textContent = i18nString(UIStrings.empty);
      }
    ...
    if (!preview.element.children.length) {
      ...
      ).textContent = i18nString(UIStrings.noMetadataForThisEvent);
      preview.element.appendChild(div);
    }
    return preview;
  }
