...
export class IndexedDBModel extends SDK.SDKModel.SDKModel {
  ...
  enable() {
    ...
    if (this.storageBucketModel) {
      this.storageBucketModel.addEventListener(
        'BucketAdded' /* SDK.StorageBucketsModel.Events.BUCKET_ADDED */,
        this.storageBucketAdded,
        this,
      );
      this.storageBucketModel.addEventListener(
        'BucketRemoved' /* SDK.StorageBucketsModel.Events.BUCKET_REMOVED */,
  ...
  clearForStorageKey(storageKey) {
    ...
    for (const [storageBucketName] of this.databaseNamesByStorageKeyAndBucket.get(storageKey) ||
      []) {
      const storageBucket = this.storageBucketModel?.getBucketByName(
        storageKey,
        storageBucketName,
      )?.bucket;
      if (storageBucket) {
  ...
  async refreshDatabaseNames() {
    for (const [storageKey] of this.databaseNamesByStorageKeyAndBucket) {
      const storageBucketNames =
        this.databaseNamesByStorageKeyAndBucket.get(storageKey)?.keys() || [];
  ...
  updateStorageKeyDatabaseNames(storageBucket, databaseNames) {
    ...
    const newDatabases = new Set(
      databaseNames.map((databaseName) => new DatabaseId(storageBucket, databaseName)),
    );
  ...
  async loadDatabaseNamesByStorageBucket(storageBucket) {
    const { storageKey } = storageBucket;
    const { databaseNames } = await this.indexedDBAgent.invoke_requestDatabaseNames({
      storageBucket,
    });
    if (!databaseNames) {
      return [];
  ...
  async loadDatabase(databaseId, entriesUpdated) {
    ...
    if (
      !this.databaseNamesByStorageKeyAndBucket
        .get(databaseId.storageBucket.storageKey)
    ...
    for (const objectStore of databaseWithObjectStores.objectStores) {
      ...
      const objectStoreModel = new ObjectStore(
        objectStore.name,
...
SDK.SDKModel.SDKModel.register(IndexedDBModel, {
...
export class DatabaseId {
  ...
  equals(databaseId) {
    return (
      ...
      this.storageBucket.name === databaseId.storageBucket.name &&
