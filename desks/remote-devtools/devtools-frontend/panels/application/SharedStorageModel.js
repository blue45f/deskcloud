...
export class SharedStorageForOrigin extends Common.ObjectWrapper.ObjectWrapper {
  ...
  async getMetadata() {
    return await this.#model.storageAgent
  ...
  async deleteEntry(key) {
    await this.#model.storageAgent.invoke_deleteSharedStorageEntry({
      ownerOrigin: this.securityOrigin,
      key,
    });
  }
  async clear() {
    await this.#model.storageAgent.invoke_clearSharedStorageEntries({
      ownerOrigin: this.securityOrigin,
    });
  }
  async resetBudget() {
    await this.#model.storageAgent.invoke_resetSharedStorageBudget({
      ownerOrigin: this.securityOrigin,
    });
  }
}
export class SharedStorageModel extends SDK.SDKModel.SDKModel {
  ...
  #maybeAddOrigin(securityOrigin) {
    ...
    if (
      !parsedSecurityOrigin.isValid ||
      parsedSecurityOrigin.scheme === 'data' ||
      parsedSecurityOrigin.scheme === 'about' ||
      parsedSecurityOrigin.scheme === 'javascript'
    ) {
      return;
    }
  ...
  #removeOrigin(securityOrigin) {
    ...
    this.dispatchEventToListeners(
      'SharedStorageRemoved' /* Events.SHARED_STORAGE_REMOVED */,
      storage,
    );
  }
  storages() {
  ...
  sharedStorageAccessed(event) {
    if (this.isChangeEvent(event)) {
      ...
      if (sharedStorage) {
        ...
        sharedStorage.dispatchEventToListeners(
          'SharedStorageChanged' /* SharedStorageForOrigin.Events.SHARED_STORAGE_CHANGED */,
          eventData,
        );
      } else {
...
SDK.SDKModel.SDKModel.register(SharedStorageModel, {
