...
const str_ = i18n.i18n.registerUIStrings(
  'panels/application/InterestGroupStorageView.ts',
  UIStrings,
);
...
export class InterestGroupStorageView extends UI.SplitWidget.SplitWidget {
  ...
  constructor(detailsGetter) {
    ...
    this.noDisplayView = new UI.EmptyWidget.EmptyWidget(
      i18nString(UIStrings.noValueSelected),
      i18nString(UIStrings.clickToDisplayBody),
    );
    this.noDataView = new UI.EmptyWidget.EmptyWidget(
      i18nString(UIStrings.noDataAvailable),
      i18nString(UIStrings.noDataDescription),
    );
  ...
  async onFocus(event) {
    ...
    if (
      eventType !== 'additionalBid' /* Protocol.Storage.InterestGroupAccessType.AdditionalBid */ &&
      eventType !==
    ...
    if (details) {
      ...
      if (jsonView) {
        jsonView.contentElement.setAttribute(
          'jslog',
          `${VisualLogging.pane('details').track({ resize: true })}`,
        );
