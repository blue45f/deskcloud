...
const DEFAULT_VIEW = (input, output, target) => {
  ...
  render(
    html` <devtools-widget
        ...
        class=${classMap({ suspended: input.suspended })}
      ></devtools-widget>
      <div class="perfmon-chart-container ${classMap({ suspended: input.suspended })}">
        <canvas
          tabindex="-1"
          aria-label=${i18nString(UIStrings.graphsDisplayingARealtimeViewOf)}
          .width=${Math.round(input.width * globalThis.devicePixelRatio)}
          .height=${input.height}
          style="height:${input.height / globalThis.devicePixelRatio}px"
          ${ref((e) => {
            if (e) {
              const canvas = e;
      ...
      ${input.suspended
        ? html` <div class="perfmon-chart-suspend-overlay fill">
...
export class PerformanceMonitorImpl extends UI.Widget.HBox {
  ...
  drawHorizontalGrid(ctx) {
    ...
    const lightGray = ThemeSupport.ThemeSupport.instance().getComputedValue(
      '--color-background-inverted-opacity-2',
    );
    ctx.font = '10px ' + Host.Platform.fontFamily();
    ctx.fillStyle = ThemeSupport.ThemeSupport.instance().getComputedValue(
  ...
  drawChart(ctx, chartInfo, height) {
    ...
    const backgroundColor = Common.Color.parse(
    ...
    if (backgroundColor) {
      for (const path of paths.reverse()) {
        ...
        ctx.fillStyle =
          backgroundColor.blendWith(parsedColor.setAlpha(0.2).asLegacyColor()).asString() || '';
        ctx.fill(path.path);
  ...
  drawVerticalGrid(ctx, height, max, info) {
    ...
    ctx.strokeStyle = ThemeSupport.ThemeSupport.instance().getComputedValue(
      '--color-background-inverted-opacity-2',
    );
    ctx.stroke();
  ...
  createChartInfos() {
    ...
    return [
        ...
        metrics: [
          {
            ...
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-cpu-task-duration',
              elementForStyles,
            ),
          },
          {
            name: 'ScriptDuration',
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-cpu-script-duration',
              elementForStyles,
            ),
          },
          {
            name: 'LayoutDuration',
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-cpu-layout-duration',
              elementForStyles,
            ),
          },
          {
      ...
      },
      {
        ...
        metrics: [
          {
            ...
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-jsheap-total-size',
              elementForStyles,
            ),
          },
          {
            name: 'JSHeapUsedSize',
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-jsheap-used-size',
              elementForStyles,
            ),
          },
        ],
        ...
        color: themeSupport.getComputedValue(
          '--override-color-perf-monitor-jsheap',
          elementForStyles,
        ),
      },
      {
        ...
        metrics: [
          {
            name: 'Nodes',
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-dom-nodes',
              elementForStyles,
            ),
          },
        ],
        ...
        metrics: [
          {
            ...
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-js-event-listeners',
              elementForStyles,
            ),
          },
        ],
        ...
        metrics: [
          {
            name: 'Documents',
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-documents',
              elementForStyles,
            ),
          },
        ],
        ...
        metrics: [
          {
            name: 'Frames',
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-document-frames',
              elementForStyles,
            ),
          },
        ],
        ...
        metrics: [
          {
            name: 'LayoutCount',
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-layout-count',
              elementForStyles,
            ),
          },
        ],
        ...
        metrics: [
          {
            ...
            color: themeSupport.getComputedValue(
              '--override-color-perf-monitor-recalc-style-count',
              elementForStyles,
            ),
          },
        ],
...
const CONTROL_PANE_DEFAULT_VIEW = (input, _output, target) => {
  render(
    input.chartsInfo.map((chartInfo) => {
      ...
      return renderMetricIndicator(chartInfo, active, value, (e) =>
        input.onCheckboxChange(chartName, e),
      );
    }),
    target,
  );
...
function renderMetricIndicator(info, active, value, onCheckboxChange) {
  ...
  return html`
    <div
      class="perfmon-indicator ${active ? 'active' : ''}"
    ...
    >
      <devtools-checkbox .checked=${active} @change=${onCheckboxChange} .jslogContext=${chartName}
        >${info.title}</devtools-checkbox
      >
