...
export class Event {
  ...
  constructor(
    timelineData,
    eventHandlers,
    eventProperties = { color: undefined, duration: undefined, level: 0, name: '', startTime: 0 },
  ) {
  ...
  decorate(htmlElement) {
    ...
    if (this.#live) {
      ...
      htmlElement.createChild('span').textContent =
        `Duration: ${startTimeReadable} - ${durationReadable}`;
    } else {
...
export class TickingFlameChart extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.dataProvider = new TickingFlameChartDataProvider(
      this.bounds,
      this.updateMaxTime.bind(this),
    );
    // Delegate doesn't do much for now.
    this.delegate = new TickingFlameChartDelegate();
    // Chart settings.
    this.chartGroupExpansionSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.chart =
      ...
      // @ts-expect-error
      new PerfUI.FlameChart.FlameChart(
        this.dataProvider,
        this.delegate,
...
class TickingFlameChartDataProvider {
  ...
  startEvent(properties) {
    ...
    if (properties['level'] > this.maxLevel) {
      throw new Error(
        `level ${properties['level']} is above the maximum allowed of ${this.maxLevel}`,
      );
    }
