...
const MEDIA_PLAYER_EXPLANATION_URL =
  'https://developer.chrome.com/docs/devtools/media-panel#hide-show';
...
export class MainView extends UI.Panel.PanelWithSidebar {
  ...
  constructor(downloadStore = new PlayerDataDownloadManager()) {
    ...
    this.#placeholder = new UI.EmptyWidget.EmptyWidget(
      i18nString(UIStrings.noMediaPlayer),
      UIStrings.mediaPlayerDescription,
    );
  ...
  addEventListeners(mediaModel) {
    ...
    mediaModel.addEventListener(
      'PlayerPropertiesChanged' /* Events.PLAYER_PROPERTIES_CHANGED */,
      this.propertiesChanged,
      this,
    );
    mediaModel.addEventListener(
      'PlayerEventsAdded' /* Events.PLAYER_EVENTS_ADDED */,
      this.eventsAdded,
      this,
    );
    mediaModel.addEventListener(
      'PlayerMessagesLogged' /* Events.PLAYER_MESSAGES_LOGGED */,
      this.messagesLogged,
      this,
    );
    mediaModel.addEventListener(
      'PlayerErrorsRaised' /* Events.PLAYER_ERRORS_RAISED */,
      this.errorsRaised,
      this,
    );
    mediaModel.addEventListener(
      'PlayerCreated' /* Events.PLAYER_CREATED */,
      this.playerCreated,
      this,
    );
  }
  ...
  exportPlayerData(playerID) {
    ...
    const uriContent =
      'data:application/octet-stream,' + encodeURIComponent(JSON.stringify(dump, null, 2));
