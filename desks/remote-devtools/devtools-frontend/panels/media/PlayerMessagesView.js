...
class MessageLevelSelector {
  ...
  createElementForItem(item) {
    ...
    const shadowRoot = UI.UIUtils.createShadowRootWithCoreStyles(element, {
      cssFile: playerMessagesViewStyles,
    });
...
export class PlayerMessagesView extends UI.Widget.VBox {
  ...
  createDropdown() {
    ...
    const dropDown = new UI.SoftDropDown.SoftDropDown(
      items,
      this.messageLevelSelector,
      'log-level',
    );
  ...
  createFilterInput() {
    const filterInput = new UI.Toolbar.ToolbarFilter(
      i18nString(UIStrings.filterByLogMessages),
      1,
      1,
    );
  ...
  addMessage(message) {
    const container = this.bodyPanel.createChild(
      'div',
      'media-messages-message-container media-message-' + message.level,
    );
    ...
    return html`
      <div class="status-error-box">
        <div class="status-error-field-labeled">
          <span class="status-error-field-label">${i18nString(UIStrings.errorGroupLabel)}</span>
          <span>${error.errorType}</span>
        </div>
        <div class="status-error-field-labeled">
          <span class="status-error-field-label">${i18nString(UIStrings.errorCodeLabel)}</span>
