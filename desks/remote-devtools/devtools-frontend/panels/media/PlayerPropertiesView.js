...
export class PropertyRenderer extends UI.Widget.VBox {
  ...
  changeNestedContents(value) {
    if (value === null || Object.keys(value).length === 0) {
      ...
      const jsonWrapperElement = new SourceFrame.JSONView.JSONView(
        new SourceFrame.JSONView.ParsedJSON(value, '', ''),
        true,
      );
...
export class PlayerPropertiesView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.videoDecoderTabs = new DecoderTrackMenu(
      TrackTypeLocalized.Video(),
      this.videoDecoderProperties,
    );
    this.videoDecoderTabs.show(this.contentElement);
    this.audioDecoderTabs = new DecoderTrackMenu(
      TrackTypeLocalized.Audio(),
      this.audioDecoderProperties,
    );
  ...
  populateAttributesAndElements() {
    ...
    const fileSize = new FormattedPropertyRenderer(
      i18nString(UIStrings.fileSize),
      this.formatFileSize,
    );
    ...
    this.attributeMap.set(
      'kIsSingleOrigin' /* PlayerPropertyKeys.IS_SINGLE_ORIGIN */,
      singleOrigin,
    );
    const rangeHeaders = new PropertyRenderer(i18nString(UIStrings.rangeHeaderSupport));
    this.mediaElements.push(rangeHeaders);
    this.attributeMap.set(
      'kIsRangeHeaderSupported' /* PlayerPropertyKeys.IS_RANGE_HEADER_SUPPORTED */,
      rangeHeaders,
    );
    ...
    this.attributeMap.set(
      'kVideoPlaybackRoughness' /* PlayerPropertyKeys.VIDEO_PLAYBACK_ROUGHNESS */,
      roughness,
    );
    const freezingScore = new PropertyRenderer(i18nString(UIStrings.videoFreezingScore));
    this.mediaElements.push(freezingScore);
    this.attributeMap.set(
      'kVideoPlaybackFreezing' /* PlayerPropertyKeys.VIDEO_PLAYBACK_FREEZING */,
      freezingScore,
    );
    ...
    const hlsBufferedRanges = new FormattedPropertyRenderer(
      i18nString(UIStrings.hlsBufferedRanges),
      this.formatBufferedRanges,
    );
    this.mediaElements.push(hlsBufferedRanges);
    this.attributeMap.set(
      'kHlsBufferedRanges' /* PlayerPropertyKeys.HLS_BUFFERED_RANGES */,
      hlsBufferedRanges,
    );
    /* Video Decoder Properties */
    const decoderName = new DefaultPropertyRenderer(
      i18nString(UIStrings.decoderName),
      i18nString(UIStrings.noDecoder),
    );
    this.videoDecoderElements.push(decoderName);
    this.attributeMap.set(
      'kVideoDecoderName' /* PlayerPropertyKeys.VIDEO_DECODER_NAME */,
      decoderName,
    );
    ...
    this.attributeMap.set(
      'kIsPlatformVideoDecoder' /* PlayerPropertyKeys.IS_PLATFORM_VIDEO_DECODER */,
      videoPlatformDecoder,
    );
    const encoderName = new DefaultPropertyRenderer(
      i18nString(UIStrings.encoderName),
      i18nString(UIStrings.noEncoder),
    );
    this.videoDecoderElements.push(encoderName);
    this.attributeMap.set(
      'kVideoEncoderName' /* PlayerPropertyKeys.VIDEO_ENCODER_NAME */,
      encoderName,
    );
    ...
    this.attributeMap.set(
      'kIsPlatformVideoEncoder' /* PlayerPropertyKeys.IS_PLATFORM_VIDEO_ENCODER */,
      videoPlatformEncoder,
    );
    const videoDDS = new PropertyRenderer(i18nString(UIStrings.decryptingDemuxer));
    this.videoDecoderElements.push(videoDDS);
    this.attributeMap.set(
      'kIsVideoDecryptingDemuxerStream' /* PlayerPropertyKeys.IS_VIDEO_DECRYPTION_DEMUXER_STREAM */,
      videoDDS,
    );
    ...
    const audioDecoder = new DefaultPropertyRenderer(
      i18nString(UIStrings.decoderName),
      i18nString(UIStrings.noDecoder),
    );
    this.audioDecoderElements.push(audioDecoder);
    this.attributeMap.set(
      'kAudioDecoderName' /* PlayerPropertyKeys.AUDIO_DECODER_NAME */,
      audioDecoder,
    );
    ...
    this.attributeMap.set(
      'kIsPlatformAudioDecoder' /* PlayerPropertyKeys.IS_PLATFORM_AUDIO_DECODER */,
    ...
    this.attributeMap.set(
      'kIsAudioDecryptingDemuxerStream' /* PlayerPropertyKeys.IS_AUDIO_DECRYPTING_DEMUXER_STREAM */,
      audioDDS,
    );
