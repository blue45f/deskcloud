      ...
      .template=${html`
        <ul role="tree">
          ${results.map(
            (searchResult) =>
              html` <li
                @expand=${(e) => onExpand(searchResult, e)}
                role="treeitem"
                class="search-result"
                ?open=${expandedResults.has(searchResult)}
              >
                <style>
                  ${searchResultsPaneStyles}
                </style>
...
const renderSearchMatches = (searchResult, matches, onSelectMatch, onShowMoreMatches) => {
  ...
  // clang-format off
  return html` ${visibleMatches.map(
    ({ lineContent, matchRanges, resultLabel }, i) =>
      html` <li
        role="treeitem"
        class="search-match"
        @click=${() => onSelectMatch(searchResult, i)}
        @keydown=${(event) => {
          if (event.key === 'Enter') {
      ...
      >
        <button
          class="devtools-link text-button link-style search-match-link"
          jslog="Link; context: search-match; track: click"
          role="link"
          tabindex="0"
          @click=${() => void Common.Revealer.reveal(searchResult.matchRevealable(i))}
        >
          <span
            class="search-match-line-number"
            aria-label=${typeof resultLabel === 'number' && !isNaN(resultLabel)
              ? i18nString(UIStrings.lineS, { PH1: resultLabel })
              : resultLabel}
          >
            ${resultLabel}
          </span>
          <span
            class="search-match-content"
            aria-label="${lineContent} line"
  ...
  ${matchesLeftCount > 0
    ? html` <li
        ...
        @click=${() => onShowMoreMatches(searchResult)}
      >
...
export class SearchResultsPane extends UI.Widget.VBox {
  ...
  #appendSearchMatches(searchResult, fromIndex, toIndex) {
    ...
    for (let i = fromIndex; i < toIndex; ++i) {
      ...
      if (column !== undefined && matchLength !== undefined) {
        const { matchRange, lineSegment } = lineSegmentForMatch(
          lineContent,
          new TextUtils.TextRange.SourceRange(column, matchLength),
        );
        ...
        ({ lineSegment: lineContent, matchRanges } = lineSegmentForMultipleMatches(
          lineContent,
          matchRanges,
        ));
      }
  ...
  performUpdate() {
    if (this.#resultsUpdated) {
      ...
      for (const searchResult of this.#searchResults) {
        if (
          matchesExpandedCount < matchesExpandedByDefault &&
          !this.#expandedResults.has(searchResult)
        ) {
          this.#expandedResults.add(searchResult);
...
export function lineSegmentForMatch(lineContent, range, optionsArg = DEFAULT_OPTS) {
  ...
  const actuallyRemovedWhitespaceLength = Math.min(
    range.offset,
    potentiallyRemovedWhitespaceLength,
  );
  // Apply {options.prefixLength} and {options.maxLength}.
  const lineSegmentBegin = Math.max(
    actuallyRemovedWhitespaceLength,
    range.offset - options.prefixLength,
  );
...
function lineSegmentForMultipleMatches(lineContent, ranges) {
  ...
  if (trimBy) {
    matchRanges = matchRanges.map(
      (range) => new TextUtils.TextRange.SourceRange(range.offset - trimBy + 1, range.length),
    );
