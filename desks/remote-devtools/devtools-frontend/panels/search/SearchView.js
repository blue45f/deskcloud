...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  if (!query) {
    ...
    text = i18nString(UIStrings.typeAndPressSToSearch, {
    ...
    html` <style>
      ...
      <div class="search-drawer-header" @keydown=${onPanelKeyDown}>
          ...
          <div class="toolbar-item-search">
            ...
            <devtools-button
              ...
              .data=${{
                ...
                toggled: isRegex,
                title: isRegex
                  ? i18nString(UIStrings.disableRegularExpression)
            ...
            <devtools-button
              ...
              .data=${{
                ...
                toggled: matchCase,
                title: matchCase
                  ? i18nString(UIStrings.disableCaseSensitive)
        ...
        <devtools-toolbar class="search-toolbar" jslog=${VisualLogging.toolbar()}>
          ...
          ></devtools-button>
          <devtools-button
            title=${i18nString(UIStrings.clear)}
            @click=${onClearSearch}
            .data=${{
              variant: 'toolbar' /* Buttons.Button.Variant.TOOLBAR */,
              iconName: 'clear',
      ...
      <div class="search-results" @keydown=${onPanelKeyDown}>
        ${searchResults.length
          ? html`<devtools-widget
              ${widget(SearchResultsPane, { searchResults, searchConfig })}
...
export class SearchView extends UI.Widget.VBox {
  ...
  #updateSearchResultsMessage() {
    if (this.#searchMatchesCount && this.#searchResultsCount) {
      if (this.#searchMatchesCount === 1 && this.#nonEmptySearchResultsCount === 1) {
        ...
        this.#searchResultsMessage = i18nString(UIStrings.foundDMatchingLinesInFile, {
          PH1: this.#searchMatchesCount,
        });
      } else {
        this.#searchResultsMessage = i18nString(UIStrings.foundDMatchingLinesInDFiles, {
          PH1: this.#searchMatchesCount,
  ...
  #searchFinished(finished) {
    this.#searchMessage = finished
      ? i18nString(UIStrings.searchFinished)
      : i18nString(UIStrings.searchInterrupted);
    this.requestUpdate();
  }
  focus() {
  ...
  #load() {
    const searchConfig = Workspace.SearchConfig.SearchConfig.fromPlainObject(
      this.#advancedSearchConfig.get(),
    );
