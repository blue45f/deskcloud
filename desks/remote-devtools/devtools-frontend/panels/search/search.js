...
var $ = { matchesCountS: 'Matches Count {PH1}', lineS: 'Line {PH1}', showDMore: 'Show {PH1} more' },
  ...
  z = (i, e, t) => {
    let {
        ...
        expandedResults: c,
        onSelectMatch: g,
        onExpandSearchResult: l,
        onShowMoreMatches: n,
      } = i,
      d = (h, { detail: { expanded: p } }) => {
        p ? (c.add(h), l(h)) : c.delete(h);
      };
    ee(
      y`
  ...
  oe = (i, e, t, o) => {
    let s = e.get(i) ?? [],
      c = i.matchesCount() - s.length;
    return y`
      ${s.map(
        ({ lineContent: g, matchRanges: l, resultLabel: n }, d) => y`
  ...
  S = class extends C.Widget.VBox {
    ...
    set searchResults(e) {
      if (this.#t !== e) {
        if (this.#t.length !== e.length) this.#n = !0;
        else if (this.#t.length === e.length) {
        ...
        this.#n && ((this.#t = e), this.requestUpdate());
      }
    }
    get searchConfig() {
      return this.#o;
    ...
    showAllMatches() {
      for (let e of this.#t) {
        let t = this.#s.get(e)?.length ?? 0;
        (this.#h(e, t, e.matchesCount()), this.#r.add(e));
      }
      this.requestUpdate();
    }
    collapseAllResults() {
      ((this.#r = new WeakSet()), this.requestUpdate());
    }
    #i(e) {
      let t = Math.min(e.matchesCount(), H);
      (this.#h(e, 0, t), this.requestUpdate());
    }
    #h(e, t, o) {
      if (!this.#o) return;
      let s = this.#o.queries(),
        c = [];
      for (let l = 0; l < s.length; ++l)
        c.push(q.StringUtilities.createSearchRegex(s[l], !this.#o.ignoreCase(), this.#o.isRegex()));
      let g = this.#s.get(e) ?? [];
      if ((this.#s.set(e, g), !(g.length >= o)))
        for (let l = t; l < o; ++l) {
          let n = e.matchLineContent(l),
            d = [],
            h = e.matchColumn(l),
            p = e.matchLength(l);
          if (h !== void 0 && p !== void 0) {
            let { matchRange: m, lineSegment: U } = V(n, new w.TextRange.SourceRange(h, p));
            ((n = U), (d = [m]));
          } else {
            n = n.trim();
            for (let m = 0; m < c.length; ++m) d = d.concat(this.#l(n, c[m]));
            ({ lineSegment: n, matchRanges: d } = re(n, d));
          }
          let v = e.matchLabel(l);
          g.push({ lineContent: n, matchRanges: d, resultLabel: v });
        }
    }
    performUpdate() {
      if (this.#n) {
        let e = 0;
        for (let t of this.#t) this.#r.has(t) && (e += this.#s.get(t)?.length ?? 0);
        for (let t of this.#t)
          e < W &&
            !this.#r.has(t) &&
            (this.#r.add(t), this.#i(t), (e += this.#s.get(t)?.length ?? 0));
        this.#n = !1;
      }
      this.#a(
        {
          results: this.#t,
          matches: this.#s,
          expandedResults: this.#r,
          onSelectMatch: (e, t) => {
            P.Revealer.reveal(e.matchRevealable(t));
          },
          onExpandSearchResult: this.#i.bind(this),
          onShowMoreMatches: this.#m.bind(this),
        },
        {},
        this.contentElement,
      );
    ...
    #l(e, t) {
      ...
      let o,
        s = [];
      for (; t.lastIndex < e.length && (o = t.exec(e)); )
        s.push(new w.TextRange.SourceRange(o.index, o[0].length));
      return s;
    }
    #m(e) {
      let t = this.#s.get(e)?.length ?? 0;
      (this.#h(e, t, e.matchesCount()), this.requestUpdate());
    }
  },
  W = 200,
  H = 20,
  K = { prefixLength: 25, maxLength: 1e3 };
function V(i, e, t = K) {
  let o = { ...K, ...t },
    s = i.trimStart(),
    c = i.length - s.length,
    g = Math.min(e.offset, c),
    l = Math.max(g, e.offset - o.prefixLength),
    n = Math.min(i.length, l + o.maxLength),
    d = l > g ? '\u2026' : '',
    h = d + i.substring(l, n),
    p = e.offset - l + d.length,
    v = Math.min(e.length, h.length - p),
    m = new w.TextRange.SourceRange(p, v);
  return { lineSegment: h, matchRange: m };
}
function re(i, e) {
  let t = 0,
    o = e;
  o.length > 0 && o[0].offset > 20 && (t = 15);
  let s = i.substring(t, 1e3 + t);
  return (
    t &&
      ((o = o.map((c) => new w.TextRange.SourceRange(c.offset - t + 1, c.length))),
      (s = '\u2026' + s)),
    { lineSegment: s, matchRanges: o }
  );
}
var ae = {};
var Q = {};
F(Q, { DEFAULT_VIEW: () => _, SearchView: () => E });
import './../../ui/legacy/legacy.js';
import './../../ui/kit/kit.js';
import * as b from './../../core/common/common.js';
import * as O from './../../core/host/host.js';
...
var r = {
  ...
  le = A.i18n.registerUIStrings('panels/search/SearchView.ts', r),
  a = A.i18n.getLocalizedString.bind(void 0, le),
  { ref: ce, live: de } = ne,
  ...
  _ = (i, e, t) => {
    ...
    (o
      ? h
        ? (M = a(r.searching))
        : n.length || ((M = a(r.noMatchesFound)), (I = a(r.nothingMatchedTheQuery)))
      : ((M = a(r.noSearchResult)),
        (I = a(r.typeAndPressSToSearch, {
          PH1: u.KeyboardShortcut.KeyboardShortcut.shortcutToString(u.KeyboardShortcut.Keys.Enter),
        }))),
      he(
        T`
        ...
        <div class="search-container">
          <div class="toolbar-item-search">
            ...
            <input type="text"
                ...
                ${ce((x) => {
                  e.focusSearchInput = () => {
                    x instanceof HTMLInputElement && (x.focus(), x.select());
                  };
                })}>
      ...
      <div class="search-results" @keydown=${m}>
        ${
          n.length
            ? T`<devtools-widget ${B(S, { searchResults: n, searchConfig: g })}
            ${ge(S, (x) => {
              ((e.showAllMatches = () => void x.showAllMatches()),
                (e.collapseAllResults = () => void x.collapseAllResults()));
            })}>
            </devtools-widget>`
            : B(u.EmptyWidget.EmptyWidget, { header: M, text: I })
        }
      </div>
  ...
  E = class extends u.Widget.VBox {
    ...
    #y = [];
    constructor(e, t = _) {
      (super({ jslog: `${f.panel('search').track({ resize: !0 })}`, useShadowDom: !0 }),
        (this.#o = t),
        ...
        (this.#p = null),
        (this.#e = null),
        (this.#S = b.Settings.Settings.instance().createLocalSetting(
          e + '-search-config',
          new k.SearchConfig.SearchConfig('', !0, !1).toPlainObject(),
        )),
        this.performUpdate(),
        this.#H(),
        this.performUpdate(),
    ...
    performUpdate() {
      let e = {
          ...
          isRegex: this.#x,
          searchConfig: this.#b,
          searchMessage: this.#g,
          searchResults: this.#y.filter((s) => s.matchesCount()),
          searchResultsMessage: this.#u,
          progress: this.#e,
          onQueryChange: (s) => {
            this.#d = s;
          },
          onQueryKeyDown: this.#z.bind(this),
          onPanelKeyDown: this.#W.bind(this),
          onClearSearchInput: this.#k.bind(this),
          onToggleRegex: this.#L.bind(this),
          onToggleMatchCase: this.#T.bind(this),
          onRefresh: this.#v.bind(this),
          onClearSearch: this.#V.bind(this),
        },
        t = this,
        o = {
          set focusSearchInput(s) {
            t.#t = s;
          },
          set showAllMatches(s) {
            t.#n = s;
          },
          set collapseAllResults(s) {
            t.#r = s;
          },
        };
      this.#o(e, o, this.contentElement);
    }
    #L() {
      ((this.#x = !this.#x), this.performUpdate());
    }
    #T() {
      ((this.#f = !this.#f), this.performUpdate());
    }
    #w() {
      return new k.SearchConfig.SearchConfig(this.#d, !this.#f, this.#x);
    }
    toggle(e, t) {
      ((this.#d = e),
        this.requestUpdate(),
    ...
    #C() {
      this.#c = this.createScope();
    }
    #M() {
      if (!this.#e) return;
      let e = !this.#e.canceled;
      if (
        ((this.#e = null),
        (this.#s = !1),
        (this.#g = e ? '' : a(r.indexingInterrupted)),
        e || (this.#p = null),
        this.performUpdate(),
        !this.#p)
      )
        return;
      let t = this.#p;
      ((this.#p = null), this.#F(t));
    }
    #$() {
      ((this.#s = !0),
        this.#e && (this.#e.done = !0),
        (this.#e = new b.Progress.ProgressProxy(
          new b.Progress.Progress(),
          this.#M.bind(this),
          this.requestUpdate.bind(this),
        )),
        (this.#g = a(r.indexing)),
        this.performUpdate(),
        this.#c && this.#c.performIndexing(this.#e));
    }
    #k() {
      ((this.#d = ''), this.requestUpdate(), this.#P(), this.focus());
    }
    #E(e, t) {
      if (!(e !== this.#a || !this.#e)) {
        if (this.#e?.canceled) {
        ...
        (this.#y.push(t), this.#K(t), this.requestUpdate());
      }
    }
    #A(e, t) {
      e !== this.#a ||
        !this.#e ||
        ((this.#e = null), this.#q(t), u.ARIAUtils.LiveAnnouncer.alert(this.#g + ' ' + this.#u));
    }
    #F(e) {
      ((this.#b = e),
        this.#e && (this.#e.done = !0),
        ...
        this.#D(),
        this.#c &&
          this.#c.performSearch(
            e,
            this.#e,
            this.#E.bind(this, this.#a),
            this.#A.bind(this, this.#a),
          ));
    }
    #U() {
      (this.#I(), (this.#y = []), (this.#g = ''), (this.#u = ''), this.performUpdate());
    }
    #I() {
      (this.#e && !this.#s && (this.#e.canceled = !0), this.#c && this.#c.stopSearch());
    }
    #D() {
      ((this.#i = 0),
        (this.#h = 0),
        (this.#y = []),
        (this.#l = 0),
        this.#m || (this.#m = new u.EmptyWidget.EmptyWidget(a(r.searching), '')),
        (this.#g = a(r.searching)),
        this.performUpdate(),
        this.#R());
    }
    #R() {
      (this.#i && this.#h
        ? this.#i === 1 && this.#l === 1
          ? (this.#u = a(r.foundMatchingLineInFile))
          : this.#i > 1 && this.#l === 1
            ? (this.#u = a(r.foundDMatchingLinesInFile, { PH1: this.#i }))
            : (this.#u = a(r.foundDMatchingLinesInDFiles, { PH1: this.#i, PH2: this.#l }))
        : (this.#u = ''),
        this.performUpdate());
    }
    #K(e) {
      let t = e.matchesCount();
      ((this.#i += t), this.#h++, t && this.#l++, this.#R());
    }
    #q(e) {
      ((this.#g = a(e ? r.searchFinished : r.searchInterrupted)), this.requestUpdate());
    }
    focus() {
      this.#t();
    }
    willHide() {
      (super.willHide(), this.#I());
    }
    #z(e) {
      switch ((this.#P(), e.keyCode)) {
        case u.KeyboardShortcut.Keys.Enter.code:
          this.#v();
          break;
      }
    }
    #W(e) {
      let t = O.Platform.isMac(),
        o = t && e.metaKey && !e.ctrlKey && e.altKey && e.code === 'BracketRight',
        s = !t && e.ctrlKey && !e.metaKey && e.shiftKey && e.code === 'BracketRight',
        c = t && e.metaKey && !e.ctrlKey && e.altKey && e.code === 'BracketLeft',
        g = !t && e.ctrlKey && !e.metaKey && e.shiftKey && e.code === 'BracketLeft';
      o || s
        ? (this.#n(), f.logKeyDown(e.currentTarget, e, 'show-all-matches'))
        : (c || g) && (this.#r(), f.logKeyDown(e.currentTarget, e, 'collapse-all-results'));
    }
    #P() {
      this.#S.set(this.#w().toPlainObject());
    }
    #H() {
      let e = k.SearchConfig.SearchConfig.fromPlainObject(this.#S.get());
      ((this.#d = e.query()),
        (this.#f = !e.ignoreCase()),
        (this.#x = e.isRegex()),
        this.requestUpdate());
    ...
    #v() {
      let e = this.#w();
      e.query()?.length && (this.#U(), ++this.#a, this.#C(), this.#s || this.#$(), (this.#p = e));
    }
    #V() {
      (this.#U(), this.#k());
    }
  };
export { j as SearchResultsPane, ae as SearchScope, Q as SearchView };
