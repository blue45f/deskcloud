...
const REPORT_URL =
  'https://support.google.com/legal/troubleshooter/1114905?hl=en#ts=1115658%2C13380504';
...
function maybeRenderSources(
  ...
  return html`
    <ol class="sources-list">
      ${directCitationUrls.map(
        (url, index) => html`
          <li>
            <devtools-link
              href=${url}
              class=${Directives.classMap({
                link: true,
                highlighted: index === highlightedCitationIndex,
              })}
...
function maybeRenderRelatedContent(relatedUrls, directCitationUrls) {
  ...
  // clang-format off
  return html`
    ${directCitationUrls.length
      ? html`<h3>${i18nString(UIStrings.relatedContent)}</h3>`
      : Lit.nothing}
...
function renderInsightSourcesList(sources, isPageReloadRecommended) {
  // clang-format off
  return html` <div class="insight-sources">
    <ul>
      ${Directives.repeat(
        sources,
        (item) => item.value,
        (item) => {
          return html`<li>
            <devtools-link
              class="link"
              title="${localizeType(item.type)} ${i18nString(UIStrings.opensInNewTab)}"
              href="data:text/plain;charset=utf-8,${encodeURIComponent(item.value)}"
              .jslogContext=${'source-' + item.type}
            >
      ...
      ${isPageReloadRecommended
        ? html`<li class="source-disclaimer">
...
function renderInsight(
  insight,
  { renderer, disableAnimations, areReferenceDetailsOpen, highlightedCitationIndex, callbacks },
  output,
) {
  // clang-format off
  return html` ${insight.validMarkdown
      ? html`<devtools-markdown-view
          .data=${{ tokens: insight.tokens, renderer, animationEnabled: !disableAnimations }}
        >
        </devtools-markdown-view>`
      : insight.explanation}
    ${insight.timedOut
      ? html`<p class="error-message">${i18nString(UIStrings.timedOut)}</p>`
      : Lit.nothing}
    ${isSearchRagResponse(insight.metadata)
      ? html`
          <details
            class="references"
    ...
    <div class="buttons">${renderSearchButton(callbacks.onSearch)}</div>`;
  // clang-format on
}
...
function renderConsentReminder(noLogging) {
  // clang-format off
  return html` <h3>Things to consider</h3>
    <div class="reminder-items">
      <div>
        <devtools-icon name="google" class="medium"> </devtools-icon>
      </div>
      <div>
        The console message, associated stack trace, related source code, and the associated network
        headers are sent to Google to generate explanations.
        ${noLogging
      ...
      <div>
        <devtools-icon name="policy" class="medium"> </devtools-icon>
      </div>
      <div>
      ...
      <div>
        <devtools-icon name="warning" class="medium"> </devtools-icon>
      </div>
      <div>
        <devtools-link
...
function renderSettingIsNotTrue(onEnableInsightsInSettingsLink) {
  ...
  >
    ${i18nString(UIStrings.settingsLink)}
  </button>`;
  return html` <div class="badge">
      <devtools-icon name="lightbulb-spark" class="medium"> </devtools-icon>
    </div>
    <div>
...
function renderNotLoggedIn() {
  return renderError(
    Root.Runtime.hostConfig.isOffTheRecord
      ? i18nString(UIStrings.notAvailableInIncognitoMode)
...
function renderDisclaimer(noLogging, onDisclaimerSettingsLink) {
  ...
  return html`<span>
    ...
    <button
      class="link"
      role="link"
      @click=${onDisclaimerSettingsLink}
      jslog=${VisualLogging.action('open-ai-settings').track({ click: true })}
    >
      Open settings
    </button>
    or
    <devtools-link href=${LEARN_MORE_URL} class="link" jslogcontext="learn-more">
      learn more
    </devtools-link>
  </span>`;
  // clang-format on
...
function renderDisclaimerFooter(noLogging, onDisclaimerSettingsLink) {
  // clang-format off
  return html` <div class="disclaimer">
...
function renderSignInFooter(onGoToSignIn) {
  ...
  // clang-format off
  return html` <div class="filler"></div>
    <div>
      <devtools-button
        @click=${onGoToSignIn}
...
function renderHeaderIcon() {
  // clang-format off
  return html` <div class="header-icon-container">
    <devtools-icon name="lightbulb-spark" class="large"> </devtools-icon>
  </div>`;
  // clang-format on
}
function renderHeader({ headerText, showIcon = false, showSpinner = false, onClose }, headerRef) {
  ...
  return html`
    <header>
      ...
      <div class="filler">
        <h2 tabindex="-1" ${Directives.ref(headerRef)}>${headerText}</h2>
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  switch (state.type) {
    case 'loading' /* State.LOADING */:
      header = renderHeader(
        { headerText: i18nString(UIStrings.generating), onClose },
        output.headerRef,
      );
      main = renderLoading();
      break;
    ...
    case 'consent-reminder' /* State.CONSENT_REMINDER */:
      header = renderHeader(
        { headerText: 'Understand console messages with AI', onClose, showIcon: true },
        output.headerRef,
      );
      mainClasses['reminder-container'] = true;
      main = renderConsentReminder(noLogging);
      footer = renderConsentReminderFooter(
        callbacks.onReminderSettingsLink,
        callbacks.onConsentReminderConfirmed,
      );
      break;
    ...
    case 'sync-is-paused' /* State.SYNC_IS_PAUSED */:
      header = renderHeader(
        { headerText: i18nString(UIStrings.signInToUse), onClose },
        output.headerRef,
      );
    ...
    case 'offline' /* State.OFFLINE */:
      header = renderHeader(
        { headerText: i18nString(UIStrings.offlineHeader), onClose },
        output.headerRef,
      );
    ...
    html`
      ...
      >
        <div class="animation-wrapper">
          ${header}
          <main jslog=${jslog} class=${Directives.classMap(mainClasses)}>${main}</main>
          ${footer
            ? html`<footer jslog=${VisualLogging.section('footer')}>${footer}</footer>`
            : Lit.nothing}
        </div>
      </div>
    `,
...
export class ConsoleInsight extends UI.Widget.Widget {
  static async create(promptBuilder, aidaClient) {
    ...
    return html`<devtools-widget
      class="devtools-console-insight"
      ${widget(
  ...
  constructor(promptBuilder, aidaClient, aidaPreconditions, element, view = DEFAULT_VIEW) {
    ...
    this.#renderer = new MarkdownView.MarkdownView.MarkdownInsightRenderer(
      this.#citationClickHandler.bind(this),
    );
  ...
  #getOnboardingCompletedSetting() {
    return Common.Settings.Settings.instance().createLocalSetting(
  ...
  wasShown() {
    super.wasShown();
    this.focus();
    this.#consoleInsightsEnabledSetting?.addChangeListener(
      this.#onConsoleInsightsSettingChanged,
      this,
    );
    const blockedByAge = Root.Runtime.hostConfig.aidaAvailability?.blockedByAge === true;
    if (
      this.#state.type === 'loading' /* State.LOADING */ &&
    ...
    if (
      this.#state.type !== 'insight' /* State.INSIGHT */ &&
      this.#state.type !== 'error' /* State.ERROR */
    ) {
      this.#state = this.#getStateFromAidaAvailability();
    }
  ...
  willHide() {
    super.willHide();
    this.#consoleInsightsEnabledSetting?.removeChangeListener(
      this.#onConsoleInsightsSettingChanged,
      this,
    );
  ...
  #onConsoleInsightsSettingChanged() {
    ...
    if (
      this.#state.type === 'setting-is-not-true' /* State.SETTING_IS_NOT_TRUE */ &&
      this.#consoleInsightsEnabledSetting?.getIfNotDisabled() === true
    ) {
      this.#transitionTo({
        type: 'loading' /* State.LOADING */,
  ...
  #onRating(isPositive) {
    ...
    return this.#aidaClient.registerClientEvent({
      ...
      do_conversation_client_event: {
        user_feedback: {
          sentiment: this.#selectedRating
            ? 'POSITIVE' /* Host.AidaClient.Rating.POSITIVE */
            : 'NEGATIVE' /* Host.AidaClient.Rating.NEGATIVE */,
        },
      },
    });
  }
    ...
    for (const [index, citation] of sortedCitations.entries()) {
      ...
      if (result && citation.uri) {
        explanationWithCitations =
          explanationWithCitations.slice(0, result.index) +
          `[^${sortedCitations.length - index}]` +
  ...
  #deriveRelatedUrls(directCitationUrls, metadata) {
    ...
    const relatedUrls =
      metadata.factualityMetadata.facts
    ...
    const trainingDataUrls =
      metadata.attributionMetadata?.citations
        .filter(
          (citation) =>
            citation.sourceType === Host.AidaClient.CitationSourceType.TRAINING_DATA &&
    ...
    const dedupedTrainingDataUrls = [
      ...new Set(
        trainingDataUrls.filter(
          (url) => !relatedUrls.includes(url) && !directCitationUrls.includes(url),
        ),
      ),
    ];
  ...
  async #generateInsight() {
    try {
      for await (const {
        ...
        const { explanationWithCitations, directCitationUrls } = this.#insertCitations(
          explanation,
          metadata,
        );
      ...
      if (
        err.message === 'doAidaConversation timed out' &&
        this.#state.type === 'insight' /* State.INSIGHT */
      ) {
