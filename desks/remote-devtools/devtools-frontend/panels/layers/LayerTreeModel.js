...
SDK.SDKModel.SDKModel.register(LayerTreeModel, {
...
export class AgentLayer {
  ...
  async requestCompositingReasons() {
    const reasons = await this.layerTreeModel.layerTreeAgent.invoke_compositingReasons({
      layerId: this.id(),
    });
  ...
  snapshots() {
    const promise = this.layerTreeModel.paintProfilerModel
  ...
  calculateTransformToViewport(parentTransform) {
    const offsetMatrix = new WebKitCSSMatrix().translate(
      this.layerPayload.offsetX,
      this.layerPayload.offsetY,
    );
    ...
    if (this.layerPayload.transform) {
      ...
      const anchorMatrix = new WebKitCSSMatrix().translate(
        -anchorPoint.x,
        -anchorPoint.y,
        -anchorPoint.z,
      );
      matrix = anchorMatrix
        .inverse()
        .multiply(transformMatrix.multiply(anchorMatrix.multiply(matrix)));
    }
  ...
  calculateQuad(parentTransform) {
    ...
    this.#quad = [];
    const vertices = this.createVertexArrayForRect(
      this.layerPayload.width,
      this.layerPayload.height,
    );
    for (let i = 0; i < 4; ++i) {
