...
export class ScreencastView extends UI.Widget.VBox {
  ...
  initialize() {
    ...
    this.shortcuts[
      UI.KeyboardShortcut.KeyboardShortcut.makeKey('l', UI.KeyboardShortcut.Modifiers.Ctrl.value)
  ...
  async startCasting() {
    ...
    this.screencastOperationId = await this.screenCaptureModel.startScreencast(
      'jpeg' /* Protocol.Page.StartScreencastRequestFormat.Jpeg */,
      80,
  ...
  screencastFrame(base64Data, metadata) {
    this.imageElement.onload = () => {
      ...
      this.viewportElement.style.width =
        metadata.deviceWidth * this.screenZoom + bordersSize + 'px';
      this.viewportElement.style.height =
        metadata.deviceHeight * this.screenZoom + bordersSize + 'px';
  ...
  createNavigationBar() {
    ...
    if (this.resourceTreeModel) {
      ...
      this.touchInputToggle.addEventListener(
        'click',
        this.#toggleTouchEmulation.bind(this, true),
        false,
      );
      this.mouseInputToggle.addEventListener(
        'click',
        this.#toggleTouchEmulation.bind(this, false),
        false,
      );
