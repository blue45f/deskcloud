...
export class ThrottlingManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    this.currentNetworkThrottlingConditionKeySetting =
      SDK.NetworkManager.activeNetworkThrottlingKeySetting();
  ...
  #getCurrentNetworkConditions() {
    ...
    const custom = this.customNetworkConditionsSetting
      .get()
      .find((conditions) => conditions.key === activeKey);
  ...
  createOfflineToolbarCheckbox() {
    ...
    checkbox.element.setAttribute(
      'jslog',
      `${VisualLogging.toggle('disconnect-from-network').track({ click: true })}`,
    );
    SDK.NetworkManager.MultitargetNetworkManager.instance().addEventListener(
      'ConditionsChanged' /* SDK.NetworkManager.MultitargetNetworkManager.Events.CONDITIONS_CHANGED */,
  ...
  createMobileThrottlingButton() {
    ...
    function appendItems(contextMenu) {
      for (let index = 0; index < options.length; ++index) {
        ...
        if (
          conditions.title === ThrottlingPresets.getCustomConditions().title &&
  ...
  onCPUThrottlingRateChangedOnSDK(rate) {
    ...
    const index = this.cpuThrottlingOptions.indexOf(
  ...
  createSaveDataOverrideSelector(className) {
    ...
    const handler = (e) => {
      ...
      switch (select.selectedOptions.item(0)) {
        case reset:
          for (const emulationModel of SDK.TargetManager.TargetManager.instance().models(
            ...
            void this.#emulationQueue.push(
              emulationModel.setDataSaverOverride(
                'unset' /* SDK.EmulationModel.DataSaverOverride.UNSET */,
              ),
            );
          }
          break;
        case enable:
          for (const emulationModel of SDK.TargetManager.TargetManager.instance().models(
            ...
            void this.#emulationQueue.push(
              emulationModel.setDataSaverOverride(
                'enabled' /* SDK.EmulationModel.DataSaverOverride.ENABLED */,
              ),
            );
          }
          break;
        case disable:
          for (const emulationModel of SDK.TargetManager.TargetManager.instance().models(
            ...
            void this.#emulationQueue.push(
              emulationModel.setDataSaverOverride(
                'disabled' /* SDK.EmulationModel.DataSaverOverride.DISABLED */,
              ),
            );
          }
          break;
      ...
      this.dispatchEventToListeners(
        'SaveDataOverrideChanged' /* ThrottlingManager.Events.SAVE_DATA_OVERRIDE_CHANGED */,
        select.selectedIndex,
      );
    };
    const select = new UI.Toolbar.ToolbarComboBox(
      handler,
      i18nString(UIStrings.saveDataSettingTooltip),
      className,
    );
  ...
  createHardwareConcurrencySelector() {
    const numericInput = new UI.Toolbar.ToolbarItem(
      UI.UIUtils.createInput('devtools-text-input', 'number', 'hardware-concurrency'),
    );
    ...
    const reset = new UI.Toolbar.ToolbarButton(
      'Reset concurrency',
      'undo',
      undefined,
    ...
    void this.cpuThrottlingManager.getHardwareConcurrency().then((defaultValue) => {
      ...
      checkbox.addEventListener('change', () => {
        ...
        setHardwareConcurrency(
          this.hardwareConcurrencyOverrideEnabled ? Number(inputElement.value) : defaultValue,
        );
      });
