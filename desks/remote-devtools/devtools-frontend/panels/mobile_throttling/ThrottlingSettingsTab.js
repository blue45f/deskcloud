...
const str_ = i18n.i18n.registerUIStrings(
  'panels/mobile_throttling/ThrottlingSettingsTab.ts',
  UIStrings,
);
...
export class CPUThrottlingCard {
  ...
  constructor() {
    this.setting = Common.Settings.Settings.instance().createSetting(
      ...
      {},
      'Global' /* Common.Settings.SettingStorageType.GLOBAL */,
    );
  ...
  updateState() {
    ...
    if (this.state === 'cta') {
      ...
      this.calibrateButton.textContent = hasCalibrated
        ? i18nString(UIStrings.recalibrate)
        : i18nString(UIStrings.calibrate);
      if (!hasCalibrated) {
      ...
      this.textEl.append(
        this.createTextWithIcon(i18nString(UIStrings.calibrationConfirmationPrompt), 'info'),
      );
...
export class ThrottlingSettingsTab extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const settingsContent = this.contentElement
  ...
  renderItem(conditions, _editable) {
    ...
    element.createChild('div', 'conditions-list-separator');
    element.createChild('div', 'conditions-list-text').textContent = i18nString(UIStrings.dms, {
      PH1: conditions.latency,
    });
    element.createChild('div', 'conditions-list-separator');
    element.createChild('div', 'conditions-list-text').textContent = percentText(
      conditions.packetLoss ?? 0,
    );
    element.createChild('div', 'conditions-list-separator');
    element.createChild('div', 'conditions-list-text').textContent = String(
      conditions.packetQueueLength ?? 0,
    );
    element.createChild('div', 'conditions-list-separator');
    element.createChild('div', 'conditions-list-text').textContent = conditions.packetReordering
      ? i18nString(UIStrings.on)
      : i18nString(UIStrings.off);
    return element;
  }
  ...
  beginEdit(conditions) {
    ...
    editor.control('download').value =
      conditions.download <= 0 ? '' : String(conditions.download / (1000 / 8));
    editor.control('upload').value =
      conditions.upload <= 0 ? '' : String(conditions.upload / (1000 / 8));
    ...
    editor.control('packetQueueLength').value = conditions.packetQueueLength
      ? String(conditions.packetQueueLength)
      : '';
  ...
  createEditor() {
    ...
    function titleValidator(_item, _index, input) {
      ...
      if (!valid) {
        const errorMessage = i18nString(UIStrings.profileNameCharactersLengthMust, {
          PH1: maxLength,
        });
    ...
    function throughputValidator(_item, _index, input) {
      ...
      const valid =
        !Number.isNaN(parsedValue) && parsedValue >= minThroughput && parsedValue <= maxThroughput;
      if (!valid) {
        const errorMessage = i18nString(UIStrings.sMustBeANumberBetweenSkbsToSkbs, {
          PH1: String(throughput),
          PH2: minThroughput,
          PH3: maxThroughput,
        });
        return { valid, errorMessage };
    ...
    function latencyValidator(_item, _index, input) {
      ...
      const parsedValue = Number(value);
      const valid =
        Number.isInteger(parsedValue) && parsedValue >= minLatency && parsedValue <= maxLatency;
      if (!valid) {
        const errorMessage = i18nString(UIStrings.latencyMustBeAnIntegerBetweenSms, {
          PH1: minLatency,
          PH2: maxLatency,
        });
        return { valid, errorMessage };
    ...
    function packetLossValidator(_item, _index, input) {
      ...
      if (!valid) {
        const errorMessage = i18nString(UIStrings.packetLossMustBeAnIntegerBetweenSpct, {
          PH1: minPacketLoss,
          PH2: maxPacketLoss,
        });
        return { valid, errorMessage };
    ...
    function packetQueueLengthValidator(_item, _index, input) {
      ...
      if (!valid) {
        const errorMessage = i18nString(
          UIStrings.packetQueueLengthMustBeAnIntegerGreaterOrEqualToZero,
        );
        return { valid, errorMessage };
      }
      return {
