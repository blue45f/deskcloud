...
const str_ = i18n.i18n.registerUIStrings(
  'panels/mobile_throttling/MobileThrottlingSelector.ts',
  UIStrings,
);
...
export class MobileThrottlingSelector {
  ...
  populateOptions() {
    ...
    const presetsGroup = {
      ...
      items: ThrottlingPresets.getMobilePresets(),
    };
    const advancedGroup = {
      title: i18nString(UIStrings.advanced),
  ...
  conditionsChanged() {
    ...
    for (let index = 0; index < this.options.length; ++index) {
      ...
      if (
        option &&
        'network' in option &&
        option.network === networkConditions &&
    ...
    for (let index = 0; index < this.options.length; ++index) {
      ...
      if (
        item &&
        item.title === customConditions.title &&
        item.description === customConditions.description
      ) {
        this.selectCallback(index);
        return;
      }
