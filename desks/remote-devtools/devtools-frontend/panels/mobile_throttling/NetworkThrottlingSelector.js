...
const str_ = i18n.i18n.registerUIStrings(
  'panels/mobile_throttling/NetworkThrottlingSelector.ts',
  UIStrings,
);
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  const jslog = (group, condition) =>
    `${VisualLogging.item(
      Platform.StringUtilities.toKebabCase(
  ...
  render(
    ...
    >
            ...
            ${group.items.map(
              (condition) =>
                html`<option
                  ...
                  aria-label=${i18nString(UIStrings.sS, {
                    PH1: group.title,
                    PH2: title(condition),
                  })}
      ...
      <optgroup label=${input.customConditionsGroup.title}>
        ${input.customConditionsGroup.items.map(
          (condition) =>
            html`<option
              ${Directives.ref((option) => option && optionsMap.set(option, condition))}
              ?selected=${selectedConditions &&
              ...
              value=${title(condition)}
              aria-label=${i18nString(UIStrings.sS, {
                PH1: input.customConditionsGroup.title,
                PH2: title(condition),
              })}
...
export class NetworkThrottlingSelect extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static createForGlobalConditions(element, title) {
    ...
    select.addEventListener(
      'conditionsChanged' /* Events.CONDITIONS_CHANGED */,
      (ev) =>
        !('block' in ev.data) &&
  ...
  constructor(element, options = {}, view = DEFAULT_VIEW) {
    super();
    SDK.NetworkManager.customUserNetworkConditionsSetting().addChangeListener(
      this.#performUpdate,
      this,
    );
  ...
  #performUpdate() {
    ...
    switch (this.#variant) {
      ...
      case 'individual-request-conditions' /* NetworkThrottlingSelect.Variant.INDIVIDUAL_REQUEST_CONDITIONS */:
        throttlingGroups.push(
          {
            title: i18nString(UIStrings.blockingGroup),
            items: [SDK.NetworkManager.BlockingConditions],
          },
          {
            title: i18nString(UIStrings.presets),
            items: [
    ...
    const customConditionsGroup = {
      title: i18nString(UIStrings.custom),
      items: customNetworkConditions,
    };
    const viewInput = {
...
export class NetworkThrottlingSelectorWidget extends UI.Widget.VBox {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    ...
    this.#select.addEventListener('conditionsChanged' /* Events.CONDITIONS_CHANGED */, ({ data }) =>
      this.#conditionsChangedHandler?.(data),
    );
  }
  get disabled() {
