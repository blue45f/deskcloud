  ...
  return html`
    <details open class=${classes}>
      <summary>
        <div class="summary-left">
          ...
          <button
            ...
            jslog=${VisualLogging.action('jump-to-file')}
            @click=${() => onFileNameClick(fileUrl)}
          >
            ${fileName}
          </button>
        </div>
      ...
      <div class="diff-view-container">
        <devtools-diff-view .data=${{ diff, mimeType }}> </devtools-diff-view>
      </div>
    </details>
  `;
  // clang-format on
  ...
  Lit.render(
    html`
      <div class="combined-diff-view">
        ${input.singleDiffViewInputs.map((singleDiffViewInput) =>
          renderSingleDiffView(singleDiffViewInput),
        )}
      </div>
    `,
    target,
...
export class CombinedDiffView extends UI.Widget.Widget {
  ...
  wasShown() {
    ...
    this.#workspaceDiff?.addEventListener(
      'ModifiedStatusChanged' /* WorkspaceDiff.WorkspaceDiff.Events.MODIFIED_STATUS_CHANGED */,
  ...
  #onFileNameClick(fileUrl) {
    const uiSourceCode = this.#modifiedUISourceCodes.find(
      (uiSourceCode) => uiSourceCode.url() === fileUrl,
    );
  ...
  async #initializeModifiedUISourceCodes() {
    ...
    const nowNonModifiedUISourceCodes = currentModifiedUISourceCodes.filter(
      (uiSourceCode) => !nextModifiedUISourceCodes.includes(uiSourceCode),
    );
    nowNonModifiedUISourceCodes.forEach((nonModifiedUISourceCode) =>
      this.#workspaceDiff?.unsubscribeFromDiffChange(
    ...
    const newlyModifiedUISourceCodes = nextModifiedUISourceCodes.filter(
      (uiSourceCode) => !currentModifiedUISourceCodes.includes(uiSourceCode),
    );
    newlyModifiedUISourceCodes.forEach((modifiedUISourceCode) =>
  ...
  async performUpdate() {
    const uiSourceCodeAndDiffs = (
      await Promise.all(
        this.#modifiedUISourceCodes.map(async (modifiedUISourceCode) => {
