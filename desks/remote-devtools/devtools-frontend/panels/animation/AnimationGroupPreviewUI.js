...
const DEFAULT_VIEW = (input, output, target) => {
  ...
  const renderAnimationLines = () => {
    const timeToPixelRatio =
      100 / Math.max(input.animationGroupDuration, MIN_ANIMATION_GROUP_DURATION);
    ...
    const lines = input.animations.map((animation, index) => {
      ...
      const yPoint = Math.floor(
        (VIEW_BOX_HEIGHT / Math.max(6, input.animations.length)) * index + 1,
      );
      ...
      <div class="animation-group-preview-ui">
        <button
          jslog=${VisualLogging.item(
            `animations.buffer-preview${input.isScrollDrivenAnimationGroup ? '-sda' : ''}`,
        ...
        >
          ...
          <devtools-icon
            name=${input.isScrollDrivenAnimationGroup ? 'mouse' : 'watch'}
            class="preview-icon"
          ></devtools-icon>
          <div
            class="animation-buffer-preview-animation"
            ${ref((el) => {
              if (el instanceof HTMLElement) {
                output.replay = () => {
                  el.animate(
