...
export class AnimationUI {
  ...
  constructor(animation, timeline, parentElement) {
    ...
    this.#activeIntervalGroup.setAttribute(
      'jslog',
      `${VisualLogging.animationClip().track({ drag: true })}`,
    );
    if (!this.#animation.viewOrScrollTimeline()) {
      UI.UIUtils.installDragHandle(
        this.#activeIntervalGroup,
        this.mouseDown.bind(this, 'AnimationDrag' /* Events.ANIMATION_DRAG */, null),
      ...
      AnimationUI.installDragHandleKeyboard(
        this.#activeIntervalGroup,
        this.keydownMove.bind(this, 'AnimationDrag' /* Events.ANIMATION_DRAG */, null),
      );
    }
  ...
  drawDelayLine(parentElement) {
    ...
    this.#delayLine.setAttribute(
      'x2',
      (this.delayOrStartTime() * this.#timeline.pixelTimeRatio() + margin).toFixed(2),
    );
    ...
    this.#endDelayLine.setAttribute(
      ...
      forwardsFill
        ...
        : (this.#animation.source().endDelay() * this.#timeline.pixelTimeRatio() + margin).toFixed(
            2,
          ),
    );
  }
  drawPoint(iteration, parentElement, x, keyframeIndex, attachEvents) {
    if (this.#cachedElements[iteration].keyframePoints[keyframeIndex]) {
      this.#cachedElements[iteration].keyframePoints[keyframeIndex].setAttribute(
        'cx',
        x.toFixed(2),
      );
      return;
    ...
    circle.setAttribute(
      'jslog',
      `${VisualLogging.controlPoint('animations.keyframe').track({ drag: true })}`,
    );
    circle.tabIndex = 0;
    UI.ARIAUtils.setLabel(
      circle,
      keyframeIndex <= 0
        ? i18nString(UIStrings.animationEndpointSlider)
    ...
    if (!this.animation().viewOrScrollTimeline()) {
      ...
      AnimationUI.installDragHandleKeyboard(
        circle,
        this.keydownMove.bind(this, eventType, keyframeIndex),
      );
    }
  }
  renderKeyframe(iteration, keyframeIndex, parentElement, leftDistance, width, easing) {
    ...
    if (!cache[keyframeIndex]) {
      const svg = bezier
        ? UI.UIUtils.createSVGChild(parentElement, 'path', 'animation-keyframe')
  ...
  redraw() {
    ...
    this.#activeIntervalGroup.style.transform =
      'translateX(' +
      (this.delayOrStartTime() * this.#timeline.pixelTimeRatio()).toFixed(2) +
      'px)';
    this.#nameElement.style.transform =
      'translateX(' +
      (
        Math.max(this.delayOrStartTime(), 0) * this.#timeline.pixelTimeRatio() +
        Options.AnimationMargin
      ).toFixed(2) +
      'px)';
    this.#nameElement.style.width =
      (this.duration() * this.#timeline.pixelTimeRatio()).toFixed(2) + 'px';
    ...
    if (!this.#tailGroup) {
      this.#tailGroup = UI.UIUtils.createSVGChild(
        this.#activeIntervalGroup,
        'g',
    ...
    for (
      iteration = 1;
      iteration < this.#animation.source().iterations() &&
  ...
  renderIteration(parentElement, iteration) {
    ...
    group.style.transform =
      'translateX(' +
      (iteration * this.duration() * this.#timeline.pixelTimeRatio()).toFixed(2) +
      'px)';
    this.drawAnimationLine(iteration, group);
    if (this.#keyframes && this.#keyframes.length > 1) {
      for (let i = 0; i < this.#keyframes.length - 1; i++) {
        const leftDistance =
          this.offset(i) * this.duration() * this.#timeline.pixelTimeRatio() +
          Options.AnimationMargin;
        const width =
          this.duration() * (this.offset(i + 1) - this.offset(i)) * this.#timeline.pixelTimeRatio();
  ...
  delayOrStartTime() {
    ...
    if (
      this.#mouseEventType === 'AnimationDrag' /* Events.ANIMATION_DRAG */ ||
  ...
  offset(i) {
    ...
    if (
      ...
      i === this.#keyframeMoved
    ) {
      console.assert(
        i > 0 && i < this.#keyframes.length - 1,
  ...
  mouseMove(event) {
    const mouseEvent = event;
    this.setMovementAndRedraw(
      (mouseEvent.clientX - (this.#downMouseX || 0)) / this.#timeline.pixelTimeRatio(),
    );
  }
  ...
  mouseUp(event) {
    ...
    if (this.#mouseEventType === 'KeyframeMove' /* Events.KEYFRAME_MOVE */) {
      if (
        this.#keyframes &&
        this.#keyframeMoved !== null &&
        typeof this.#keyframeMoved !== 'undefined'
      ) {
