...
const DEFAULT_TOOLBAR_VIEW = (input, output, target) => {
  const renderPlaybackRateControl = () => {
    const focusNextPlaybackRateButton = (eventTarget, focusPrevious) => {
      ...
      const nextPlaybackRateButton = target.querySelector(
        `[data-playback-rate="${nextPlaybackRate}"]`,
      );
    ...
    return html`
      <div
        class="animation-playback-rate-control"
        role="listbox"
        aria-label=${i18nString(UIStrings.playbackRates)}
        @keydown=${handleKeyDown}
      >
        ${GlobalPlaybackRates.map((playbackRate) => {
          const isSelected = input.selectedPlaybackRate === playbackRate;
          const textContent = playbackRate
            ? i18nString(UIStrings.playbackRatePlaceholder, { PH1: playbackRate * 100 })
            : i18nString(UIStrings.pause);
          return html`
            <button
              jslog=${VisualLogging.action()
                .context(`animations.playback-rate-${playbackRate * 100}`)
                .track({
                  click: true,
  ...
  render(
    html`
      <div
        class="animation-timeline-toolbar-container"
        role="toolbar"
...
export class AnimationTimeline extends UI.Widget.VBox {
  ...
  constructor(toolbarView = DEFAULT_TOOLBAR_VIEW) {
    ...
    this.#timelineControlsResizer = this.contentElement.createChild(
      'div',
    ...
    const emptyBufferHint = this.contentElement.createChild(
      'div',
  ...
  #setupTimelineControlsResizer() {
    ...
    UI.UIUtils.installDragHandle(
      ...
      (ev) => {
        ...
        this.#timelineControlsWidth = Math.min(
          Math.max(newWidth, MIN_TIMELINE_CONTROLS_WIDTH),
          MAX_TIMELINE_CONTROLS_WIDTH,
        );
        resizeOriginX = ev.clientX;
        this.element.style.setProperty(
          '--timeline-controls-width',
          this.#timelineControlsWidth + 'px',
        );
        this.onResize();
      },
      () => {
  ...
  createScrubber() {
    ...
    this.#timelineScrubberLine = this.#timelineScrubber.createChild(
      'div',
    ...
    this.#toolbarView(
      {
        ...
        onTogglePauseAllClick: () => {
          ...
          Host.userMetrics.actionTaken(
            this.#allPaused
              ? Host.UserMetrics.Action.AnimationsPaused
              : Host.UserMetrics.Action.AnimationsResumed,
          );
  ...
  createHeader() {
    ...
    this.#controlButton.addEventListener(
      ...
      this.controlButtonToggle.bind(this),
    );
  ...
  updateControlButton() {
    ...
    this.#controlButton.setEnabled(
      Boolean(this.#selectedGroup) &&
        this.hasAnimationGroupActiveNodes() &&
        !this.#selectedGroup?.isScrollDriven(),
    );
    if (this.#selectedGroup && this.#selectedGroup.paused()) {
      ...
      !this.#scrubberPlayer ||
      !this.#scrubberPlayer.currentTime ||
      typeof this.#scrubberPlayer.currentTime !== 'number' ||
  ...
  effectivePlaybackRate() {
    return this.#allPaused || (this.#selectedGroup && this.#selectedGroup.paused())
      ? 0
      : this.#playbackRate;
  }
  togglePause(pause) {
  ...
  animationGroupUpdated({ data: group }) {
    void this.#animationGroupUpdatedThrottler.schedule(async () => {
      ...
      if (group.isScrollDriven()) {
        ...
        if (animationNode) {
          const scrollRange =
            group.scrollOrientation() === 'vertical' /* Protocol.DOM.ScrollOrientation.Vertical */
  ...
  addAnimationGroup(group) {
    ...
    while (this.#groupBuffer.length > bufferSize) {
      const toDiscard = this.#groupBuffer.splice(
        this.#groupBuffer[0] === this.#selectedGroup ? 1 : 0,
        1,
      );
  ...
  removeAnimationGroup(group) {
    ...
    const nextGroup =
      currentGroupIndex >= this.#groupBuffer.length
  ...
  setCurrentTimeText(time) {
    ...
    this.#currentTime.textContent = this.#selectedGroup?.isScrollDriven()
      ? `${time.toFixed(0)}px`
      : i18n.TimeUtilities.millisToString(time);
  }
  async selectAnimationGroup(group) {
    ...
    if (group.isScrollDriven()) {
      ...
      this.#scrollListenerId = await animationNode.addScrollEventListener(
  ...
  renderGrid() {
    ...
    for (let time = 0; time < this.duration(); time += gridSize) {
      ...
      if (lastDraw === undefined || gridWidth - lastDraw > 50) {
        lastDraw = gridWidth;
        const label = UI.UIUtils.createSVGChild(
          this.#grid,
          'text',
          'animation-timeline-grid-label',
        );
        label.textContent = isScrollDriven
          ? `${time.toFixed(0)}px`
          : i18n.TimeUtilities.millisToString(time);
  ...
  render(timestamp) {
    while (
      ...
      (!timestamp || globalThis.performance.now() - timestamp < 50)
    ) {
  ...
  onResize() {
    this.#cachedTimelineWidth =
      Math.max(0, this.contentElement.offsetWidth - this.#timelineControlsWidth) || 0;
    this.scheduleRedraw();
    if (this.#scrubberPlayer) {
      this.syncScrubber();
    }
  ...
  updateScrubber(_timestamp) {
    ...
    if (
      this.#scrubberPlayer.playState.toString() === 'pending' ||
      this.#scrubberPlayer.playState === 'running'
    ) {
      this.element.window().requestAnimationFrame(this.updateScrubber.bind(this));
    }
  }
  scrubberDragStart(event) {
    ...
    if (this.#selectedGroup.isScrollDriven()) {
      ...
      this.#animationGroupPausedBeforeScrub =
        this.#selectedGroup.paused() ||
        (typeof currentTime === 'number' && currentTime >= this.duration());
  ...
  async updateScrollOffsetOnPage(offset) {
    ...
    if (
      this.#selectedGroup?.scrollOrientation() ===
      'vertical' /* Protocol.DOM.ScrollOrientation.Vertical */
    ) {
  ...
  scrubberDragMove(event) {
    ...
    const currentTime = Math.max(
      0,
      Math.min((this.#originalScrubberTime || 0) + delta / this.pixelTimeRatio(), this.duration()),
    );
    if (this.#scrubberPlayer) {
      this.#scrubberPlayer.currentTime = currentTime;
    } else {
  ...
  #scrubberCurrentTime() {
    return typeof this.#scrubberPlayer?.currentTime === 'number'
      ? this.#scrubberPlayer.currentTime
      : 0;
  }
...
export class NodeUI {
  ...
  constructor(_animationEffect) {
    ...
    this.#description.setAttribute(
      'jslog',
      `${VisualLogging.tableCell('description').track({ resize: true })}`,
    );
    this.#timelineElement = this.element.createChild('div', 'animation-node-timeline');
    this.#timelineElement.setAttribute(
      'jslog',
      `${VisualLogging.tableCell('timeline').track({ resize: true })}`,
    );
