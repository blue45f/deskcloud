...
export class LighthousePanel extends UI.Panel.Panel {
  ...
  async handleCompleteRun(overrides) {
    try {
      ...
      UI.Context.Context.instance().setFlavor(
        ActiveLighthouseReport,
        new ActiveLighthouseReport(lhr),
      );
      return { report: lhr };
    } catch (err) {
      this.handleError(err);
  ...
  renderToolbar() {
    const lighthouseToolbarContainer = this.element.createChild(
      'div',
  ...
  beforePrint() {
    ...
    this.statusView.renderText(
      i18nString(UIStrings.printing),
      i18nString(UIStrings.thePrintPopupWindowIsOpenPlease),
    );
  }
  afterPrint() {
    this.statusView.hide();
  ...
  renderReport(lighthouseResult, artifacts) {
    ...
    const reportContainer = LighthouseReportRenderer.renderLighthouseReport(
      lighthouseResult,
      artifacts,
      {
