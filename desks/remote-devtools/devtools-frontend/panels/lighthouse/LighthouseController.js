...
class LighthouseRun {
  ...
  async setupEmulationAndProtocolConnection() {
    ...
    this.emulationStateBefore = {
      ...
      network: {
        conditions: SDK.NetworkManager.MultitargetNetworkManager.instance().networkConditions(),
      },
    };
...
export class LighthouseController extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(protocolService) {
    super();
    this.protocolService = protocolService;
    protocolService.registerStatusCallback((message) =>
      this.dispatchEventToListeners(Events.AuditProgressChanged, { message }),
    );
    ...
    const javaScriptDisabledSetting =
      Common.Settings.Settings.instance().moduleSetting('java-script-disabled');
  ...
  async hasImportantResourcesNotCleared() {
    const clearStorageSetting = getRuntimeSettings().find(
      (runtimeSetting) => runtimeSetting.setting.name === 'lighthouse.clear-storage',
    );
    ...
    if (locations.length === 1) {
      return i18nString(UIStrings.thereMayBeStoredDataAffectingSingular, {
        PH1: String(locations[0]),
      });
    }
    if (locations.length > 1) {
      return i18nString(UIStrings.thereMayBeStoredDataAffectingLoadingPlural, {
        PH1: locations.join(', '),
      });
    }
    return '';
