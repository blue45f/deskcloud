          ...
          <div class="lighthouse-status-text" role="status">
            ${bugReport
              ? html`
                  ...
                  ${bugReport.knownBugPattern
                    ? html` <p>${i18nString(UIStrings.tryToNavigateToTheUrlInAFresh)}</p> `
        ...
        <div class="lighthouse-action-buttons">
          <devtools-button
            ...
            style=${Lit.Directives.styleMap({
              visibility: cancelButtonVisible ? 'visible' : 'hidden',
            })}
...
export class StatusView {
  ...
  render() {
    if (!this.dialogRoot) {
      this.dialogRoot = UI.UIUtils.createShadowRootWithCoreStyles(this.dialog.contentElement, {
        cssFile: lighthouseDialogStyles,
      });
  ...
  getStatusHeader() {
    ...
    if (this.isAIControlled) {
      return pageHost
        ? i18nString(UIStrings.aiAuditingS, { PH1: pageHost })
        : i18nString(UIStrings.aiAuditingYourWebPage);
    }
    return pageHost
      ? i18nString(UIStrings.auditingS, { PH1: pageHost })
  ...
  getMessageForPhase(phase) {
    ...
    const deviceTypeSetting = getRuntimeSettings().find(
      (item) => item.setting.name === 'lighthouse.device-type',
    );
    const throttleSetting = getRuntimeSettings().find(
      (item) => item.setting.name === 'lighthouse.throttling',
    );
