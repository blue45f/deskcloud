...
export class ProtocolService {
  ...
  async attach() {
    ...
    resourceTreeModel.addEventListener(
      SDK.ResourceTreeModel.Events.JavaScriptDialogOpening,
      dialogHandler,
    );
  ...
  initWorker() {
    this.lighthouseWorkerPromise = new Promise((resolve) => {
      ...
      const remoteBaseSearchParam = new URL(globalThis.location.href).searchParams.get(
        'remoteBase',
      );
  ...
  sendProtocolMessage(message) {
    ...
    void this.connection?.send(method, params, sessionId).then((response) => {
      const message =
        'result' in response
          ? { id, sessionId, result: response.result }
          : { id, sessionId, error: response.error };
