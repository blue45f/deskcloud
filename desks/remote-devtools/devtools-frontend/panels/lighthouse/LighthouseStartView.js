...
export class StartView extends UI.Widget.Widget {
  ...
  populateRuntimeSettingAsToolbarDropdown(settingName, toolbar) {
    ...
    const options =
      runtimeSetting.options?.map((option) => ({ label: option.label(), value: option.value })) ||
      [];
    runtimeSetting.setting.setTitle(runtimeSetting.title());
  ...
  populateFormControls(deviceTypeFormElements, categoryFormElements, mode) {
    ...
    this.populateRuntimeSettingAsRadio(
      'lighthouse.device-type',
      i18nString(UIStrings.device),
      deviceTypeFormElements,
    );
  ...
  render() {
    ...
    this.populateRuntimeSettingAsToolbarCheckbox(
  ...
  populateStartButton(mode) {
    ...
    const startButtonContainer = this.contentElement.querySelector(
