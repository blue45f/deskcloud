...
const DEFAULT_VIEW = (input, output, target) => {
  const makeTreeNode = (parentId) => (layer) => {
    ...
    return {
      ...
      id,
      children: async () =>
        subLayers.sort((layer1, layer2) => layer1.order - layer2.order).map(makeTreeNode(id)),
    };
  };
  ...
  const template = html`
    ...
    <div class="layers-widget">
      ...
      <devtools-tree-outline ${ref(captureTreeOutline)} .data=${data}></devtools-tree-outline>
    </div>
  `;
  ...
  #onDOMNodeChanged(event) {
    ...
    if (this.#node) {
      this.#node
        .domModel()
        .cssModel()
        .removeEventListener(SDK.CSSModel.Events.StyleSheetChanged, this.requestUpdate, this);
    }
    ...
    if (this.#node) {
      this.#node
        .domModel()
        .cssModel()
        .addEventListener(SDK.CSSModel.Events.StyleSheetChanged, this.requestUpdate, this);
    }
    if (this.isShowing()) {
      this.requestUpdate();
    }
...
export class ButtonProvider {
  button;
  constructor() {
    this.button = new UI.Toolbar.ToolbarToggle(
      i18nString(UIStrings.toggleCSSLayers),
      'layers',
      'layers-filled',
    );
    ...
    this.button.element.setAttribute(
      'jslog',
      `${VisualLogging.toggleSubpane('css-layers').track({ click: true })}`,
    );
  }
  static instance(opts = { forceNew: null }) {
    const { forceNew } = opts;
