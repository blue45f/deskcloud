...
export class BezierPopoverIcon {
  ...
  iconClick(event) {
    ...
    const model =
      InlineEditor.AnimationTimingModel.AnimationTimingModel.parse(this.bezierText.innerText) ||
      InlineEditor.AnimationTimingModel.LINEAR_BEZIER;
    this.bezierEditor = new InlineEditor.BezierEditor.BezierEditor(model);
    this.bezierEditor.addEventListener(
      'BezierChanged' /* InlineEditor.BezierEditor.Events.BEZIER_CHANGED */,
      this.boundBezierChanged,
    );
  ...
  onPopoverHidden(commitEdit) {
    ...
    if (this.bezierEditor) {
      this.bezierEditor.removeEventListener(
        'BezierChanged' /* InlineEditor.BezierEditor.Events.BEZIER_CHANGED */,
        this.boundBezierChanged,
      );
    }
    this.bezierEditor = undefined;
    const propertyText = commitEdit
      ? this.treeElement.renderedPropertyText()
...
export class ColorSwatchPopoverIcon extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(treeElement, swatchPopoverHelper, swatch) {
    ...
    this.swatch.addEventListener(
      InlineEditor.ColorSwatch.ClickEvent.eventName,
  ...
  showPopover() {
    ...
    this.spectrum.addEventListener(
      'SizeChanged' /* ColorPicker.Spectrum.Events.SIZE_CHANGED */,
      this.spectrumResized,
      this,
    );
    this.spectrum.addEventListener(
      'ColorChanged' /* ColorPicker.Spectrum.Events.COLOR_CHANGED */,
      this.boundSpectrumChanged,
    );
  ...
  async spectrumChanged(event) {
    const getColor = (colorText) => {
      ...
      const customProperty =
        this.spectrum?.colorName()?.startsWith('--') && `var(${this.spectrum.colorName()})`;
      if (!color || !customProperty) {
        return color;
      }
      if (
        color.is('hex' /* Common.Color.Format.HEX */) ||
        color.is('hexa' /* Common.Color.Format.HEXA */) ||
        color.is('rgb' /* Common.Color.Format.RGB */) ||
        color.is('rgba' /* Common.Color.Format.RGBA */)
      ) {
      ...
      if (
        color.is('srgb' /* Common.Color.Format.SRGB */) ||
        color.is('srgb-linear' /* Common.Color.Format.SRGB_LINEAR */) ||
        color.is('display-p3' /* Common.Color.Format.DISPLAY_P3 */) ||
        color.is('a98-rgb' /* Common.Color.Format.A98_RGB */) ||
        color.is('prophoto-rgb' /* Common.Color.Format.PROPHOTO_RGB */) ||
        color.is('rec2020' /* Common.Color.Format.REC_2020 */) ||
        color.is('xyz' /* Common.Color.Format.XYZ */) ||
        color.is('xyz-d50' /* Common.Color.Format.XYZ_D50 */) ||
  ...
  onPopoverHidden(commitEdit) {
    ...
    if (this.spectrum) {
      this.spectrum.removeEventListener(
        'ColorChanged' /* ColorPicker.Spectrum.Events.COLOR_CHANGED */,
        this.boundSpectrumChanged,
      );
    }
...
export class ShadowSwatchPopoverHelper extends Common.ObjectWrapper.ObjectWrapper {
  ...
  showPopover() {
    ...
    this.cssShadowEditor.addEventListener(
      'ShadowChanged' /* InlineEditor.CSSShadowEditor.Events.SHADOW_CHANGED */,
...
export class FontEditorSectionManager {
  ...
  async showPopover(iconElement, stylesContainer) {
    ...
    this.fontEditor.addEventListener(
      'FontChanged' /* InlineEditor.FontEditor.Events.FONT_CHANGED */,
      this.boundFontChanged,
    );
    this.fontEditor.addEventListener(
      'FontEditorResized' /* InlineEditor.FontEditor.Events.FONT_EDITOR_RESIZED */,
      this.boundResized,
    );
  ...
  onPopoverHidden() {
    ...
    this.section.onpopulate();
    if (this.fontEditor) {
      this.fontEditor.removeEventListener(
        'FontChanged' /* InlineEditor.FontEditor.Events.FONT_CHANGED */,
        this.boundFontChanged,
      );
    }
