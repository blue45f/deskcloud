...
function defaultView(input, output, target) {
  ...
  // clang-format off
  render(
    html` <div role="dialog" class="css-value-trace monospace" @keydown=${onKeyDown}>
      ${substitutions.map(
        (line) =>
      ...
      ${firstEvaluation && intermediateEvaluations.length === 0
        ...
        : html` <details @toggle=${input.onToggle} ?hidden=${hiddenSummary}>
      ...
      ${finalResult
        ? html` <span class="trace-line-icon" aria-label="is equal to" ?hidden=${singleResult}
              >↳</span
            >
