...
export class AccessibilityTreeView extends UI.Widget.VBox {
      ...
      if (deferredNode) {
        deferredNode.resolve((domNode) => {
          if (domNode) {
            ...
            void ElementsPanel.instance().revealAndSelectNode(domNode, {
              showPanel: true,
              focusNode: true,
              highlightInOverlay: true,
            });
          }
        });
      }
  ...
  async loadSubTreeIntoAccessibilityModel(selectedNode) {
    ...
    const inspectedAXNode = ancestors.find(
      (node) => node.backendDOMNodeId() === selectedNode.backendNodeId(),
    );
    if (!inspectedAXNode) {
      return;
    }
    ...
    await this.accessibilityTreeComponent.focusNodeId(
      AccessibilityTreeUtils.getNodeId(inspectedAXNode),
    );
  }
  ...
  async selectedNodeChanged(inspectedNode) {
    ...
    if (
      inspectedNode.ownerDocument &&
      (inspectedNode.nodeName() === 'HTML' || inspectedNode.nodeName() === 'BODY')
    ) {
  ...
  modelAdded(model) {
    model.addEventListener(
      'TreeUpdated' /* SDK.AccessibilityModel.Events.TREE_UPDATED */,
      this.treeUpdated,
      this,
    );
  }
