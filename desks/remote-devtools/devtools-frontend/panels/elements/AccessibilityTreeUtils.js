...
export async function getNodeAndAncestorsFromDOMNode(domNode) {
  ...
  while (frameId !== outermostFrameId) {
    const node = await SDK.FrameManager.FrameManager.instance()
      .getFrame(frameId)
...
async function getChildren(node) {
  ...
  return await node
    .accessibilityModel()
    .requestAXChildren(node.id(), node.getFrameId() || undefined);
}
  ...
  return [
    {
      ...
      children: async () => {
        ...
        const childTreeNodes = await Promise.all(
          childNodes.map((childNode) => sdkNodeToAXTreeNodes(childNode)),
        );
...
export function accessibilityNodeRenderer(node) {
  ...
  return html`<devtools-accessibility-tree-node
    .data=${{
      name,
      role,
      ignored,
