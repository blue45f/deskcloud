  ...
  *#nodeRangesHitByMouseEvent(e) {
  ...
  #onExit() {
    ...
    if (this.#activeHighlights.length > 0) {
      this.#activeHighlights[this.#activeHighlights.length - 1].forEach((range) =>
        this.#registry.add(range),
      );
    }
  }
...
export class TracingContext {
  ...
  constructor(
    highlighting,
    expandPercentagesInShorthands,
    initialLonghandOffset = 0,
    matchedResult,
  ) {
    this.#highlighting = highlighting;
    this.#hasMoreSubstitutions =
      matchedResult?.hasMatches(
        SDK.CSSPropertyParserMatchers.VariableMatch,
  ...
  applyEvaluation(children, evaluation) {
    if (
      ...
      children.some((child) => child.#appliedEvaluations >= this.#evaluationCount)
    ) {
  ...
  substitution(root = null) {
    ...
    child.#longhandOffset =
      this.#longhandOffset +
      (root?.context.matchedResult.getComputedLonghandName(root?.match.node) ?? 0);
    child.#propertyName = this.propertyName;
    return child;
  }
  cachedParsedValue(declaration, matchedStyles, computedStyles) {
    ...
    if (
      cachedValue?.matchedStyles === matchedStyles &&
      cachedValue?.computedStyles === computedStyles
    ) {
...
export class RenderingContext {
  ...
  getComputedLonghandName(node) {
    ...
    const longhands = SDK.CSSMetadata.cssMetadata().getLonghands(
...
export class Renderer extends SDK.CSSPropertyParser.TreeWalker {
  ...
  static render(nodeOrNodes, context) {
    ...
    const renderers = nodeOrNodes.map((node) =>
  ...
  static renderValueElement(property, matchedResult, renderers, tracing, signal) {
    ...
    UI.ARIAUtils.setLabel(
      valueElement,
      i18nString(UIStrings.cssPropertyValue, { PH1: property.value }),
    );
    valueElement.className = 'value';
    valueElement.tabIndex = -1;
    const { nodes, cssControls } = this.renderValueNodes(
      property,
      matchedResult,
      renderers,
      tracing,
  ...
  static renderValueNodes(property, matchedResult, renderers, tracing, signal) {
    ...
    const context = new RenderingContext(
      matchedResult.ast,
      property instanceof SDK.CSSProperty.CSSProperty ? property : null,
      rendererMap,
      matchedResult,
      undefined,
      {},
