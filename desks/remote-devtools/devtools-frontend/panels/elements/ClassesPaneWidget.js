...
export class ClassesPaneWidget extends UI.Widget.Widget {
  ...
  constructor() {
    ...
    this.prompt.addEventListener(
      'TextChanged' /* UI.TextPrompt.Events.TEXT_CHANGED */,
      this.onTextChanged,
      this,
    );
  ...
  onKeyDown(event) {
    ...
    const announcementString =
      classNames.length > 1
        ? i18nString(UIStrings.classesSAdded, { PH1: joinClassString })
  ...
  toggleClass(node, className, enabled) {
    ...
    ButtonProvider.instance()
      .item()
      .setChecked([...classes.values()].includes(true));
  }
  ...
  async flushPendingClasses() {
    ...
    for (const node of this.pendingNodeClasses.keys()) {
      ...
      const promise = node
        .setAttributeValuePromise('class', this.pendingNodeClasses.get(node))
...
export class ClassNamePrompt extends UI.TextPrompt.TextPrompt {
  ...
  async getClassNames(selectedNode) {
    ...
    const domPromise = selectedNode
      .domModel()
      .classNamesPromise(ownerDocumentId)
      .then((classes) => {
        for (const className of classes) {
          completions.add(className);
        }
