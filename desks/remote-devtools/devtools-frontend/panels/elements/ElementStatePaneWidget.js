...
export const DEFAULT_VIEW = (input, _output, target) => {
  const createElementStateCheckbox = (state) => {
    // clang-format off
    return html` <div id=${state.state}>
      <devtools-checkbox
        class="small"
        @click=${input.onStateCheckboxClicked}
        jslog=${VisualLogging.toggle(state.state).track({ change: true })}
  ...
  render(
    html` <style>
        ${elementStatePaneWidgetStyles}
      </style>
      <div class="styles-element-state-pane" jslog=${VisualLogging.pane('element-states')}>
        <div class="page-state-checkbox">
          <devtools-checkbox
            class="small"
            title=${i18nString(UIStrings.emulatesAFocusedPage)}
        ...
        <details
          ...
          ?hidden=${input.states
            .filter(({ type }) => type === 'specific')
            .every((state) => state.hidden)}
        >
...
export class ElementStatePaneWidget extends UI.Widget.Widget {
  ...
  constructor(view = DEFAULT_VIEW) {
    ...
    this.#states.set(SpecificPseudoStates.ENABLED, {
      state: SpecificPseudoStates.ENABLED,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.DISABLED, {
      state: SpecificPseudoStates.DISABLED,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.VALID, {
      state: SpecificPseudoStates.VALID,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.INVALID, {
      state: SpecificPseudoStates.INVALID,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.USER_VALID, {
      state: SpecificPseudoStates.USER_VALID,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.USER_INVALID, {
      state: SpecificPseudoStates.USER_INVALID,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.REQUIRED, {
      state: SpecificPseudoStates.REQUIRED,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.OPTIONAL, {
      state: SpecificPseudoStates.OPTIONAL,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.READ_ONLY, {
      state: SpecificPseudoStates.READ_ONLY,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.READ_WRITE, {
      state: SpecificPseudoStates.READ_WRITE,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.IN_RANGE, {
      state: SpecificPseudoStates.IN_RANGE,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.OUT_OF_RANGE, {
      state: SpecificPseudoStates.OUT_OF_RANGE,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.VISITED, {
      state: SpecificPseudoStates.VISITED,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.LINK, {
      state: SpecificPseudoStates.LINK,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.CHECKED, {
      state: SpecificPseudoStates.CHECKED,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.INDETERMINATE, {
      state: SpecificPseudoStates.INDETERMINATE,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.PLACEHOLDER_SHOWN, {
      state: SpecificPseudoStates.PLACEHOLDER_SHOWN,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.AUTOFILL, {
      state: SpecificPseudoStates.AUTOFILL,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.OPEN, {
      state: SpecificPseudoStates.OPEN,
      type: 'specific',
    });
    this.#states.set(SpecificPseudoStates.TARGET_CURRENT, {
      state: SpecificPseudoStates.TARGET_CURRENT,
      type: 'specific',
    });
  ...
  updateModel(cssModel) {
    ...
    if (this.#cssModel) {
      this.#cssModel.removeEventListener(
        SDK.CSSModel.Events.PseudoStateForced,
        this.requestUpdate,
        this,
      );
    }
  ...
  async performUpdate() {
    ...
    ButtonProvider.instance()
      .item()
      .setToggled([...this.#states.values()].some((input) => input.checked));
    const viewInput = {
...
export class ButtonProvider {
  ...
  constructor() {
    ...
    this.button.element.setAttribute(
      'jslog',
      `${VisualLogging.toggleSubpane('element-states').track({ click: true })}`,
    );
  ...
  clicked() {
    ElementsPanel.instance().showToolbarPane(
      !this.view.isShowing() ? this.view : null,
      this.button,
    );
  }
  item() {
