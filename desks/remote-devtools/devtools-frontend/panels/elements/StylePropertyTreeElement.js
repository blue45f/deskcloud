...
// clang-format off
export class EnvFunctionRenderer extends rendererBase(
  ...
  render(match, context) {
    ...
    render(
      html`${func}(<span class=${valueClass}>${match.varName}</span>${fallbackNodes
          ? html`,
              <span class=${fallbackClass}>${Renderer.render(fallbackNodes, context).nodes}</span>`
          : nothing})`,
      span,
      { host: span },
    );
...
// clang-format off
export class FlexGridRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.FlexGridGridLanesMatch,
) {
  // clang-format on
  #treeElement;
  #stylesContainer;
  ...
  render(match, context) {
    const children = Renderer.render(
      ASTUtils.siblings(ASTUtils.declValue(match.node)),
      context,
    ).nodes;
    ...
    button.setAttribute(
      'jslog',
      `${VisualLogging.showStyleEditor().track({ click: true }).context(match.layoutType)}`,
    );
    ...
    if (
      helper.isShowing(StyleEditorWidget.instance()) &&
      StyleEditorWidget.instance().getTriggerKey() === key
    ) {
...
// clang-format off
export class CSSWideKeywordRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.CSSWideKeywordMatch,
) {
  // clang-format on
  #treeElement;
  #stylesContainer;
  ...
  render(match, context) {
    ...
    swatch.data = {
      text: match.text,
      tooltip: resolvedProperty
        ? undefined
        : { title: i18nString(UIStrings.sIsNotDefined, { PH1: match.text }) },
      isDefined: Boolean(resolvedProperty),
      onLinkActivate: () =>
        resolvedProperty && this.#stylesContainer.jumpToDeclaration(resolvedProperty),
      jslogContext: 'css-wide-keyword-link',
    };
    if (
      SDK.CSSMetadata.cssMetadata().isColorAwareProperty(resolvedProperty.name) ||
      ...
      if (color) {
        return [
          new ColorRenderer(this.#stylesContainer, this.#treeElement).renderColorSwatch(
            color,
            swatch,
          ),
          swatch,
...
export class VariableRenderer extends rendererBase(SDK.CSSPropertyParserMatchers.VariableMatch) {
  ...
  render(match, context) {
    ...
    if (substitution) {
      if (declaration?.declaration) {
        const { nodes, cssControls } = Renderer.renderValueNodes(
          ...
          substitution.cachedParsedValue(
        ...
        cssControls.forEach((value, key) =>
          value.forEach((control) => context.addControl(key, control)),
        );
        return nodes;
      }
    ...
    const tooltipContents = this.#stylesContainer.getVariablePopoverContents(
      this.#matchedStyles,
      match.name,
      variableValue ?? null,
    );
    ...
    render(
      html`
        ...
        ${tooltipId
          ? html`
              <devtools-tooltip id=${tooltipId} variant="rich" jslogContext="elements.css-var">
                ${tooltipContents}
              </devtools-tooltip>
            `
          : ''}
    ...
    const colorSwatch = new ColorRenderer(
      this.#stylesContainer,
      this.#treeElement,
    ).renderColorSwatch(color, varSwatch);
    context.addControl('color', colorSwatch);
    if (fromFallback) {
      renderedFallback?.cssControls.get('color')?.forEach((innerSwatch) =>
        innerSwatch.addEventListener(InlineEditor.ColorSwatch.ColorChangedEvent.eventName, (ev) => {
  ...
  #handleVarDefinitionActivate(variable) {
    ...
    if (typeof variable === 'string') {
      this.#stylesContainer.jumpToProperty(variable) ||
        this.#stylesContainer.jumpToProperty(
          'initial-value',
          variable,
...
export class AttributeRenderer extends rendererBase(SDK.CSSPropertyParserMatchers.AttributeMatch) {
  ...
  render(match, context) {
    ...
    const tooltipId = attributeMissing
      ? undefined
      : this.#treeElement?.getTooltipId('custom-attribute');
    const tooltip = tooltipId ? { tooltipId } : undefined;
    // clang-format off
    render(
      html` <span
        data-title=${computedValue || ''}
        jslog=${VisualLogging.link('css-variable').track({ click: true, hover: true })}
        >${attrCall ?? 'attr'}(<devtools-link-swatch
          ...
          .data=${{
            ...
            isDefined: true,
            onLinkActivate: () =>
              this.#handleAttributeActivate(
                this.#matchedStyles.originatingNodeForStyle(match.style),
                match.name,
              ),
          }}
        ></devtools-link-swatch
        >${tooltipId
          ? html` <devtools-tooltip id=${tooltipId} variant="rich" jslogContext="elements.css-var"
              >${JSON.stringify(rawValue)}</devtools-tooltip
            >`
          : nothing}${match.type
          ? html` <span class=${typeClass}>${match.type}</span>`
          : nothing}${renderedFallback
          ? html`, <span class=${fallbackClass}>${renderedFallback.nodes}</span>`
          : nothing})</span
      >`,
      varSwatch,
    );
...
// clang-format off
export class LinearGradientRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.LinearGradientMatch,
) {
  // clang-format on
  render(match, context) {
    ...
    if (angle instanceof InlineEditor.CSSAngle.CSSAngle) {
      ...
      const angleNode = args[0]?.find(
        (node) =>
          context.matchedResult.getMatch(node) instanceof SDK.CSSPropertyParserMatchers.AngleMatch,
      );
      ...
      if (angleMatch) {
        angle.addEventListener(InlineEditor.InlineEditorUtils.ValueChangedEvent.eventName, (ev) => {
          angle.updateProperty(
            context.matchedResult.getComputedText(match.node, (match) =>
              match === angleMatch ? ev.data.value : null,
            ),
          );
        });
      }
...
// clang-format off
export class RelativeColorChannelRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.RelativeColorChannelMatch,
) {
  // clang-format on
  #treeElement;
  ...
  render(match, context) {
    ...
    const evaluation = context.tracing?.applyEvaluation([], () => ({
      placeholder: [document.createTextNode(value.toFixed(3))],
    }));
    if (evaluation) {
      return evaluation;
    }
...
export class ColorRenderer extends rendererBase(SDK.CSSPropertyParserMatchers.ColorMatch) {
  ...
  #getValueChild(match, context) {
    ...
    const childTracingContexts =
      context.tracing?.evaluation([args], { match, context }) ?? undefined;
    ...
    render(
      html`${this.#treeElement?.getTracingTooltip(
        ...
        this.#treeElement.getComputedStyles() ?? new Map(),
  ...
  render(match, context) {
    ...
    if (match.node.name === 'CallExpression' && childTracingContexts) {
      const evaluation = context.tracing?.applyEvaluation(childTracingContexts, () => {
        ...
        const swatch = new ColorRenderer(this.#stylesContainer, null).renderColorSwatch(
    ...
    if (
      cssControls &&
      match.node.name === 'CallExpression' &&
      ...
      if (angle instanceof InlineEditor.CSSAngle.CSSAngle) {
        ...
        angle.addEventListener(InlineEditor.InlineEditorUtils.ValueChangedEvent.eventName, (ev) => {
          ...
          if (
            color.is('hsl' /* Common.Color.Format.HSL */) ||
            color.is('hsla' /* Common.Color.Format.HSLA */)
          ) {
            swatch.setColor(new Common.Color.HSL(hue, color.s, color.l, color.alpha));
          } else if (
            color.is('hwb' /* Common.Color.Format.HWB */) ||
            color.is('hwba' /* Common.Color.Format.HWBA */)
          ) {
  ...
  renderColorSwatch(color, valueChild) {
    ...
    const tooltip = editable
      ? i18nString(UIStrings.openColorPickerS, { PH1: shiftClickMessage })
      : '';
    ...
    if (this.#treeElement?.editable()) {
      ...
      swatch.addEventListener(
        InlineEditor.ColorSwatch.ColorFormatChangedEvent.eventName,
        onColorFormatChanged,
      );
      const swatchIcon = new ColorSwatchPopoverIcon(
        treeElement,
        treeElement.stylesContainer().swatchPopoverHelper(),
        swatch,
      );
      swatchIcon.addEventListener(
        'colorchanged' /* ColorSwatchPopoverIconEvents.COLOR_CHANGED */,
        (ev) => {
  ...
  async #addColorContrastInfo(swatchIcon) {
    ...
    const contrastInfo = new ColorPicker.ContrastInfo.ContrastInfo(
      await cssModel.getBackgroundColors(node.id),
    );
    swatchIcon.setContrastInfo(contrastInfo);
  }
}
// clang-format off
export class LightDarkColorRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.LightDarkColorMatch,
) {
  // clang-format on
  #treeElement;
  #stylesContainer;
  ...
  render(match, context) {
    ...
    const color = Common.Color.parse(
      context.matchedResult.getComputedTextRange(
        match.light[0],
        match.light[match.light.length - 1],
      ),
    );
    if (!color) {
      return [content];
    ...
    const colorSwatch = new ColorRenderer(
      this.#stylesContainer,
      this.#treeElement,
    ).renderColorSwatch(undefined, content);
    context.addControl('color', colorSwatch);
    void this.applyColorScheme(
      match,
      context,
      colorSwatch,
      light,
  ...
  async applyColorScheme(match, context, colorSwatch, light, dark, lightControls, darkControls) {
    ...
    const activeColorSwatches = (activeColor === match.light ? lightControls : darkControls).get(
      'color',
    );
    ...
    const colorText = context.matchedResult.getComputedTextRange(
      activeColor[0],
      activeColor[activeColor.length - 1],
    );
  ...
  async #activeColor(match) {
    const activeColorSchemes =
      this.#matchedStyles
        .resolveProperty('color-scheme', match.style)
...
export class ColorMixRenderer extends rendererBase(SDK.CSSPropertyParserMatchers.ColorMixMatch) {
  ...
  render(match, context) {
    const hookUpColorArg = (node, onChange) => {
      if (
        node instanceof InlineEditor.ColorMixSwatch.ColorMixSwatch ||
        node instanceof InlineEditor.ColorSwatch.ColorSwatch
      ) {
        if (node instanceof InlineEditor.ColorSwatch.ColorSwatch) {
          node.addEventListener(InlineEditor.ColorSwatch.ColorChangedEvent.eventName, (ev) =>
            onChange(ev.data.color.getAuthoredText() ?? ev.data.color.asString()),
          );
        } else {
          node.addEventListener(InlineEditor.ColorMixSwatch.ColorMixChangedEvent.eventName, (ev) =>
    ...
    const childTracingContexts = context.tracing?.evaluation(
      [match.space, match.color1, match.color2],
      { match, context },
    );
    const childRenderingContexts = childTracingContexts?.map((ctx) =>
      ctx.renderingContext(context),
    ...
    render(
      html`${this.#treeElement?.getTracingTooltip(
        ...
        this.#computedStyleExtraFields,
        context,
      ) ?? 'color-mix'}(${Renderer.render(match.space, childRenderingContexts[0]).nodes},
    ...
    if (
      context.matchedResult.hasUnresolvedSubstitutions(match.node) ||
      color1Controls.length !== 1 ||
      color2Controls.length !== 1
    ) {
      return [contentChild];
    }
    const space = match.space
      .map((space) => context.matchedResult.getComputedText(space))
      .join(' ');
    const color1Text = match.color1
      .map((color) => context.matchedResult.getComputedText(color))
      .join(' ');
    const color2Text = match.color2
      .map((color) => context.matchedResult.getComputedText(color))
      .join(' ');
    ...
    if (nodeId !== undefined && childTracingContexts) {
      const evaluation = context.tracing?.applyEvaluation(childTracingContexts, () => {
        ...
        const asyncEvalCallback = async () => {
          const results = await this.#stylesContainer
            .cssModel()
            ?.resolveValues(undefined, nodeId, colorMixText);
          if (context.signal?.aborted) {
            return false;
          }
          if (results) {
    ...
    if (
      !hookUpColorArg(color1Controls[0], (text) => swatch.setFirstColor(text)) ||
...
// clang-format off
export class ContrastColorRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.ContrastColorMatch,
) {
  // clang-format on
  #treeElement;
  #stylesContainer;
  ...
  render(match, context) {
    ...
    const colorSwatch = new ColorRenderer(
      this.#stylesContainer,
      this.#treeElement,
    ).renderColorSwatch(color, content);
    context.addControl('color', colorSwatch);
    return [colorSwatch, content];
  }
}
...
export class AngleRenderer extends rendererBase(SDK.CSSPropertyParserMatchers.AngleMatch) {
  ...
  render(match, context) {
    ...
    cssAngle.setAttribute(
      'jslog',
      `${VisualLogging.showStyleEditor().track({ click: true }).context('css-angle')}`,
    );
    const valueElement = document.createElement('span');
    valueElement.textContent = angleText;
    cssAngle.data = {
      angleText,
      containingPane: this.#treeElement
        .stylesContainer()
...
// clang-format off
export class LinkableNameRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.LinkableNameMatch,
) {
  // clang-format on
  #matchedStyles;
  #stylesContainer;
    ...
    switch (match.propertyName) {
      ...
      case 'animation-name' /* SDK.CSSPropertyParserMatchers.LinkableNameProperties.ANIMATION_NAME */:
        return {
          ...
          ruleBlock: '@keyframes',
          isDefined: Boolean(
            this.#matchedStyles.keyframes().find((kf) => kf.name().text === match.text),
          ),
        };
      case 'font-palette' /* SDK.CSSPropertyParserMatchers.LinkableNameProperties.FONT_PALETTE */:
        return {
          ...
          isDefined: Boolean(
            this.#matchedStyles
              .atRules()
              .find((ar) => ar.type() === 'font-palette-values' && ar.name()?.text === match.text),
          ),
        };
      ...
      case 'position-try-fallbacks' /* SDK.CSSPropertyParserMatchers.LinkableNameProperties.POSITION_TRY_FALLBACKS */:
        return {
          ...
          isDefined: Boolean(
            this.#matchedStyles.positionTryRules().find((pt) => pt.name().text === match.text),
          ),
        };
    }
  }
  render(match) {
    ...
    swatch.data = {
      ...
      onLinkActivate: () => {
        ...
        if (
          match.propertyName ===
          'font-palette' /* SDK.CSSPropertyParserMatchers.LinkableNameProperties.FONT_PALETTE */
        ) {
    ...
    if (
      match.propertyName ===
        'animation' /* SDK.CSSPropertyParserMatchers.LinkableNameProperties.ANIMATION */ ||
      match.propertyName ===
        'animation-name' /* SDK.CSSPropertyParserMatchers.LinkableNameProperties.ANIMATION_NAME */
    ) {
      ...
      if (node) {
        ...
        void animationModel
          ?.getAnimationGroupForAnimation(match.text, node.id)
          .then((maybeAnimationGroup) => {
            ...
            icon.setAttribute(
              'jslog',
              `${VisualLogging.link('open-in-animations-panel').track({ click: true })}`,
            );
...
export class BezierRenderer extends rendererBase(SDK.CSSPropertyParserMatchers.BezierMatch) {
  ...
  render(match, context) {
    const nodes =
      match.node.name === 'CallExpression'
        ? Renderer.render(ASTUtils.children(match.node), context).nodes
        : [document.createTextNode(match.text)];
    if (
      !this.#treeElement?.editable() ||
...
export class ShadowModel {
  ...
  isBoxShadow() {
    return (
      this.#shadowType === 'boxShadow' /* SDK.CSSPropertyParserMatchers.ShadowType.BOX_SHADOW */
    );
  }
  inset() {
    return Boolean(
      this.#properties.find(
        (property) => property.propertyType === 'inset' /* ShadowPropertyType.INSET */,
      ),
    );
  }
  #length(lengthType) {
    return (
      this.#properties.find((property) => property.propertyType === lengthType)?.length ??
  ...
  #expandPropertyIfNeeded(property) {
    if (this.#needsExpansion(property)) {
      ...
      this.#properties
        .filter((property) => property.source === source)
        .forEach((property) => {
    ...
    if (property) {
      ...
      const insertionIdx =
        ...
        this.#properties.findLastIndex(
          (property) =>
            property.propertyType === 'y' /* ShadowPropertyType.Y */ ||
            (propertyType === 'spread' /* ShadowPropertyType.SPREAD */ &&
              property.propertyType === 'blur') /* ShadowPropertyType.BLUR */,
        );
      if (
        insertionIdx > 0 &&
        insertionIdx < this.#properties.length &&
      ...
      this.#properties.splice(insertionIdx, 0, {
    ...
    for (const property of this.#properties) {
      if (!property.source || property.source !== previousSource) {
        ...
        if (property.source) {
          ...
          span.append(
            ...Renderer.render(property.value, property.expansionContext ?? this.#context).nodes,
          );
        }
      }
...
export class ShadowRenderer extends rendererBase(SDK.CSSPropertyParserMatchers.ShadowMatch) {
  ...
  shadowModel(shadow, shadowType, context) {
    ...
    for (let item = queue.shift(); item; item = queue.shift()) {
      ...
      if (value.name === 'NumberLiteral') {
        ...
        const computedValueAst = SDK.CSSPropertyParser.tokenizeDeclaration(
          '--property',
          computedValue,
        );
        if (!computedValueAst) {
        ...
        queue.unshift(
          ...ASTUtils.siblings(ASTUtils.declValue(matches.ast.tree)).map((matchedNode) => ({
            ...
            expansionContext: new RenderingContext(
              computedValueAst,
              null,
              context.renderers,
              matches,
        ...
        if (value.name === 'ValueName' && text.toLowerCase() === 'inset') {
          if (
            shadowType ===
              'textShadow' /* SDK.CSSPropertyParserMatchers.ShadowType.TEXT_SHADOW */ ||
            properties.find(
              ({ propertyType }) => propertyType === 'inset' /* ShadowPropertyType.INSET */,
            )
          ) {
            return null;
          }
          ...
          if (
            properties.find(
              ({ propertyType }) => propertyType === 'color' /* ShadowPropertyType.COLOR */,
            )
          ) {
            return null;
          }
  ...
  render(match, context) {
    ...
    for (const shadow of shadows) {
      ...
      swatch.setAttribute(
        'jslog',
        `${VisualLogging.showStyleEditor('css-shadow').track({ click: true })}`,
      );
      ...
      if (isImportant) {
        result.push(
          ...[document.createTextNode(' '), ...Renderer.render(isImportant, context).nodes],
        );
      }
    }
    return result;
...
// clang-format off
export class GridTemplateRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.GridTemplateMatch,
) {
  // clang-format on
  render(match, context) {
    ...
    const template = html`
      ${match.lines.map((line) => {
        ...
        return html` <span class="styles-clipboard-only">${indent.repeat(2)}</span>
          ${lines}`;
      })}
    `;
...
async function resolveValues(stylesContainer, propertyName, match, context, ...values) {
  ...
  propertyName =
    context.tracing?.propertyName ?? context.matchedResult.ast.propertyName ?? propertyName;
  if (
    SHORTHANDS_FOR_PERCENTAGES.has(propertyName) &&
    (context.tracing?.expandPercentagesInShorthands ??
      context.matchedResult.getLonghandValuesCount() > 1)
  ) {
  ...
  return (
    (await stylesContainer.cssModel()?.resolveValues(propertyName, nodeId, ...values)) ??
...
export class LengthRenderer extends rendererBase(SDK.CSSPropertyParserMatchers.LengthMatch) {
  ...
  async #applyEvaluation(valueElement, match, context) {
    const pixelValue = await resolveValues(
      this.#stylesContainer,
      this.#propertyName,
      match,
      context,
  ...
  #getTooltip(valueElement, match, context) {
    ...
    tooltip.addEventListener('beforetoggle', () => this.getTooltipValue(tooltip, match, context), {
      once: true,
    });
    return tooltip;
  }
...
// clang-format off
export class BaseFunctionRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.BaseFunctionMatch,
) {
  // clang-format on
  #stylesContainer;
  #matchedStyles;
  ...
  render(match, context) {
    ...
    render(
      html`${this.#treeElement?.getTracingTooltip(
        ...
        this.#computedStyleExtraFields,
        context,
      ) ?? match.func}(${renderedArgs
        .map((arg, idx) => (idx === 0 ? [arg] : [html`, `, arg]))
        .flat()})`,
      span,
    );
    if (childTracingContexts) {
      const evaluation = context.tracing?.applyEvaluation(childTracingContexts, () => ({
        placeholder: [span],
        asyncEvalCallback: () => this.applyEvaluation(span, match, context),
      }));
      if (evaluation) {
        return evaluation;
      }
    } else if (
      match instanceof SDK.CSSPropertyParserMatchers.MathFunctionMatch &&
      !match.isArithmeticFunctionCall()
    ) {
  ...
  async applyEvaluation(span, match, context) {
    const value = context.matchedResult.getComputedText(match.node, (match) => {
      if (match instanceof SDK.CSSPropertyParserMatchers.RelativeColorChannelMatch) {
        const relativeColor =
          context.findParent(match.node, SDK.CSSPropertyParserMatchers.ColorMatch)?.relativeColor ??
          null;
        return (relativeColor && match.getColorChannelValue(relativeColor)?.toFixed(3)) ?? null;
      }
      return null;
    });
    const evaled = await resolveValues(
      this.#stylesContainer,
      this.#propertyName,
      match,
      context,
  ...
  async applyMathFunction(renderedArgs, match, context) {
    ...
    const values = match.args.map((arg) =>
      context.matchedResult.getComputedTextRange(arg[0], arg[arg.length - 1]),
    );
    values.unshift(context.matchedResult.getComputedText(match.node));
    const evaledArgs = await resolveValues(
      this.#stylesContainer,
      this.#propertyName,
      match,
      context,
...
// clang-format off
export class AnchorFunctionRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.AnchorFunctionMatch,
) {
  // clang-format on
  #stylesContainer;
  static async decorateAnchorForAnchorLink(stylesContainer, container, { identifier, needsSpace }) {
    ...
    const anchorNode =
      (await stylesContainer.node()?.getAnchorBySpecifier(identifier)) ?? undefined;
  ...
  render(match, context) {
    ...
    if (match.node.name === 'VariableName') {
      ...
      void AnchorFunctionRenderer.decorateAnchorForAnchorLink(this.#stylesContainer, content, {
...
// clang-format off
export class PositionAnchorRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.PositionAnchorMatch,
) {
  #stylesContainer;
  // clang-format on
  constructor(stylesContainer) {
...
// clang-format off
export class PositionTryRenderer extends rendererBase(
  SDK.CSSPropertyParserMatchers.PositionTryMatch,
) {
  #matchedStyles;
  // clang-format on
...
export class StylePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  constructor({
    ...
    this.property.addEventListener(
      'localValueUpdated' /* SDK.CSSProperty.Events.LOCAL_VALUE_UPDATED */,
      this.updateTitle,
      this,
    );
  }
  onunbind() {
    this.property.removeEventListener(
      'localValueUpdated' /* SDK.CSSProperty.Events.LOCAL_VALUE_UPDATED */,
      this.updateTitle,
      this,
    );
  ...
  async gridNames() {
    ...
    for (let node = this.#stylesContainer.node()?.parentNode; node; node = node?.parentNode) {
      ...
      const getNames = (propertyName, predicate) => {
        ...
        return SDK.CSSPropertyParser.TreeSearch.findAll(ast, predicate).map((node) =>
          ast.text(node),
        );
      };
      if (SDK.CSSMetadata.cssMetadata().isGridAreaNameAwareProperty(this.name)) {
        return new Set(
          getNames('grid-template-areas', (node) => node.name === 'StringLiteral')?.flatMap((row) =>
            row
      ...
      if (SDK.CSSMetadata.cssMetadata().isGridColumnNameAwareProperty(this.name)) {
        return new Set(
          getNames(
            ...
            (node) => node.name === 'ValueName' && node.parent?.name === 'BracketedValue',
          ),
        );
      }
      return new Set(
  ...
  getLonghand() {
    return this.parent instanceof StylePropertyTreeElement && this.parent.isShorthand
      ? this.parent
      : null;
  }
  editable() {
  ...
  updateFilter() {
    ...
    const matches =
      regex !== null && (regex.test(this.property.name) || regex.test(this.property.value));
  ...
  async #getLonghandProperties() {
    ...
    const parsedLonghands = await this.#stylesContainer
      .cssModel()
      ?.agent.invoke_getLonghandProperties({
        shorthandName: this.property.name,
        value: parsedProperty,
      });
    if (!parsedLonghands || parsedLonghands.getError()) {
      return staticLonghandProperties;
    }
    return parsedLonghands.longhandProperties.map((p) =>
      SDK.CSSProperty.CSSProperty.parsePayload(this.style, -1, p),
    );
  }
  async onpopulate() {
    if (!this.#gridNames) {
    ...
    for (const property of longhandProperties) {
      ...
      overloaded =
        this.#matchedStyles.propertyState(property) ===
        'Overloaded' /* SDK.CSSMatchedStyles.PropertyState.OVERLOADED */;
      const leadingProperty = leadingProperties.find(
        (property) => property.name === name && property.activeInStyle(),
      );
      if (leadingProperty) {
        overloaded = true;
      }
  ...
  #computeCSSExpression(style, text) {
    ...
    const decl = SDK.CSSPropertyParser.ASTUtils.siblings(
      SDK.CSSPropertyParser.ASTUtils.declValue(matching.ast.tree),
    );
  ...
  updateTitle() {
    this.lastComputedValue = this.#computeCSSExpression(
      this.property.ownerStyle,
      this.property.value,
    );
    this.#updateTitle();
  }
  #updateTitle() {
    ...
    const renderers = this.property.parsedOk
      ...
      : [];
    if (
      Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.FONT_EDITOR) &&
    ...
    this.valueElement = Renderer.renderValueElement(
      this.property,
      matchedResult,
      renderers,
    ).valueElement;
    ...
    if (
      ...
      tooltip.onbeforetoggle = (e) => {
        if (e.newState === 'open') {
          contents.value = this.#matchedStyles.computeCSSVariable(
            this.style,
            this.property.name,
          )?.value;
        }
      ...
      tooltip.onbeforetoggle = (event) => {
        ...
        tooltip.appendChild(
          new ElementsComponents.CSSPropertyDocsView.CSSPropertyDocsView(cssProperty),
        );
      };
      this.listItemElement.appendChild(tooltip);
    }
    ...
    if (this.property.parsedOk) {
      ...
      const invalidPropertyValue = SDK.CSSMetadata.cssMetadata().isCSSPropertyName(
        this.property.name,
      );
    ...
    if (this.property.parsedOk && this.parent?.root) {
      ...
      enabledCheckboxElement.setAttribute(
        'jslog',
        `${VisualLogging.toggle().track({ click: true })}`,
      );
  ...
  #getLinkableFunction(functionName, matchedStyles) {
    ...
    swatch.data = {
      ...
      tooltip: isDefined
        ? undefined
        : { title: i18nString(UIStrings.sIsNotDefined, { PH1: functionName }) },
      isDefined,
      onLinkActivate: () => {
  ...
  getTracingTooltip(
    ...
    // clang-format off
    return html` <span tabIndex="-1" class="tracing-anchor" aria-details=${tooltipId}
        >${functionName.startsWith('--')
          ? this.#getLinkableFunction(functionName, matchedStyles)
          : functionName}</span
      >
      <devtools-tooltip
        id=${tooltipId}
  ...
  getTooltipId(key) {
    ...
    const propertyCount = this.style
      .allProperties()
      .reduce(
        (value, property, index) =>
          index < ownIndex &&
          (property.name === this.name || property.name === propertyNameForCounting)
            ? value + 1
            : value,
        0,
      );
  ...
  updateAuthoringHint() {
    ...
    for (const validator of cssRuleValidatorsMap.get(propertyName) || []) {
      const hint = validator.getHint(
        propertyName,
        this.computedStyles || undefined,
        this.parentsComputedStyles || undefined,
  ...
  updateAnimationOverrideHint() {
    ...
    if (
      ...
      UI.ViewManager.ViewManager.instance().isViewVisible('animations')
    ) {
      return;
    }
    const wrapper = document.createElement('span');
  ...
  mouseUp(event) {
    ...
    selectedElement =
      selectedElement.enclosingNodeOrSelfWithClass('webkit-css-property') ||
  ...
  handleContextMenuEvent(context, event) {
    ...
    contextMenu
      .defaultSection()
      .appendItem(i18nString(UIStrings.openInSourcesPanel), revealCallback, {
  ...
  createCopyContextMenu(event) {
    ...
    contextMenu.headerSection().appendItem(
      i18nString(UIStrings.copyDeclaration),
      () => {
    ...
    contextMenu.headerSection().appendItem(
      i18nString(UIStrings.copyProperty),
      () => {
    ...
    contextMenu.clipboardSection().appendItem(
      i18nString(UIStrings.copyAllDeclarations),
      () => {
    ...
    contextMenu.footerSection().appendItem(
      i18nString(UIStrings.viewComputedValue),
      () => {
  ...
  copyAllCssDeclarationAsJs() {
    ...
    const cssDeclarationsAsJsProperties = leadingProperties
      .filter((property) => !property.disabled)
      .map(getCssDeclarationAsJavascriptProperty);
    Host.InspectorFrontendHost.InspectorFrontendHostInstance.copyText(
      cssDeclarationsAsJsProperties.join(',\n'),
    );
  }
  navigateToSource(element, omitFocus) {
    ...
    const uiLocation =
      Bindings.CSSWorkspaceBinding.CSSWorkspaceBinding.instance().propertyUILocation(
        this.property,
        propertyNameClicked,
      );
    if (uiLocation) {
  ...
  startEditingValue() {
    ...
    if (SDK.CSSMetadata.cssMetadata().isGridAreaDefiningProperty(this.name)) {
      const splitResult = TextUtils.TextUtils.Utils.splitStringByRegexes(this.value, [
  ...
  #startEditing(context) {
    ...
    this.prompt = new CSSPropertyPrompt(
      this,
      context.isEditingName,
      Array.from(this.#gridNames ?? []),
    );
    ...
    const proxyElement = this.prompt.attachAndStartEditing(
      selectedElement,
    ...
    proxyElement.addEventListener(
      'keypress',
      this.editingNameValueKeyPress.bind(this, context),
      false,
    );
    if (context.isEditingName) {
      ...
      proxyElement.addEventListener(
        'contextmenu',
        this.handleContextMenuEvent.bind(this, context),
        false,
      );
    }
  ...
  editingNameValueKeyPress(context, event) {
    ...
    const isFieldInputTerminated = context.isEditingName
      ? keyChar === ':'
      : keyChar === ';' &&
        selectionLeftOffset !== null &&
        StylePropertyTreeElement.shouldCommitValueSemicolon(
          target.textContent || '',
          selectionLeftOffset,
        );
  ...
  async editingCommitted(userInput, context, moveDirection) {
    ...
    const shouldCommitNewProperty =
      ...
      (isPropertySplitPaste ||
        ...
        nameValueEntered);
    if (
      ((userInput !== context.previousContent || isDirtyViaPaste) && !this.newProperty) ||
      ...
      if (nameValueEntered) {
        ...
        blankInput ||
        (this.newProperty &&
          Platform.StringUtilities.isWhitespace(this.valueElement.textContent || ''))
      ) {
        propertyText = '';
  ...
  applyStyleText(styleText, majorChange, property) {
    ...
    return this.applyStyleThrottler.schedule(
      this.innerApplyStyleText.bind(this, styleText, majorChange, property),
    );
  }
  async innerApplyStyleText(styleText, majorChange, property) {
    ...
    if (hasBeenEditedIncrementally && majorChange && !success) {
      majorChange = false;
      success = await this.property.setText(
  ...
  renderActiveAiSuggestion(activeAiSuggestion) {
    ...
    if (isEditingName) {
      ...
      const currentSuggestedText = isEditingName
        ? activeAiSuggestion.name
        : activeAiSuggestion.value;
