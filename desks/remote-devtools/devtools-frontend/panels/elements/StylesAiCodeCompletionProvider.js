...
export class StylesAiCodeCompletionProvider {
  ...
  async triggerAiCodeCompletion(text, cursorPosition, isEditingName, cssProperty, cssModel) {
    ...
    const propertyStartOffset = contentText.offsetFromPosition(
      propertyRange.startLine,
      propertyRange.startColumn,
    );
    const propertyEndOffset = contentText.offsetFromPosition(
      propertyRange.endLine,
      propertyRange.endColumn,
    );
    let prefix = content.substring(0, propertyStartOffset);
    if (!isEditingName) {
      const nameRange = cssProperty.nameRange();
      if (nameRange) {
        const nameEndOffset = contentText.offsetFromPosition(
          nameRange.endLine,
          nameRange.endColumn,
        );
  ...
  async #requestAidaSuggestion(prefix, suffix, cursorPositionAtRequest) {
    ...
    try {
      const completionResponse = await this.#aiCodeCompletion?.completeCode(
        ...
        'CSS' /* Host.AidaClient.AidaInferenceLanguage.CSS */,
      );
  ...
  async #generateSampleForRequest(response, suffix) {
    ...
    const suggestionText =
      TextEditor.AiCodeCompletionProvider.AiCodeCompletionProvider.trimSuggestionOverlap(
  ...
  #pickSampleFromResponse(response) {
    ...
    return (
      response.generatedSamples.find((sample) =>
        sample.generationString.startsWith(completionHint),
      ) ?? null
    );
  }
  clearCache() {
