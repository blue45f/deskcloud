...
const str_ = i18n.i18n.registerUIStrings(
  'panels/elements/components/ElementsBreadcrumbs.ts',
  UIStrings,
);
...
export class ElementsBreadcrumbs extends HTMLElement {
  ...
  #renderOverflowButton(direction, disabled) {
    ...
    const tooltipString =
      direction === 'left' ? i18nString(UIStrings.scrollLeft) : i18nString(UIStrings.scrollRight);
    // clang-format off
    return html`
      <button
        class=${buttonStyles}
      ...
      >
        <devtools-icon
          ...
          style="width: var(--sys-size-6); height: 10px;"
        >
        </devtools-icon>
      </button>
    `;
  ...
  #render() {
    ...
    Lit.render(
      html`
        <style>
          ${elementsBreadcrumbsStyles}
        </style>
        <nav
          class="crumbs"
          aria-label=${i18nString(UIStrings.breadcrumbs)}
          jslog=${VisualLogging.elementsBreadcrumbs()}
        >
          ...
          <div class="crumbs-window" @scroll=${this.#onCrumbsWindowScroll}>
            <ul class="crumbs-scroll-container">
              ${crumbs.map((crumb) => {
                ...
                return html` <li
                  class=${Lit.Directives.classMap(crumbClasses)}
                ...
                >
                  ...
                  >
                    <devtools-node-text
                      data-node-title=${crumb.title.main}
                      .data=${{
