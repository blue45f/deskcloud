...
const writingModesAffectingFlexDirection = new Set(['tb', 'tb-rl', 'vertical-lr', 'vertical-rl']);
...
export function getPhysicalDirections(computedStyles) {
  ...
  if (isVertical) {
    return extendWithReverseDirections({
      row: isRtl
        ? 'bottom-to-top' /* PhysicalDirection.BOTTOM_TO_TOP */
        : 'top-to-bottom' /* PhysicalDirection.TOP_TO_BOTTOM */,
      column:
        writingMode === 'vertical-lr'
          ? 'left-to-right' /* PhysicalDirection.LEFT_TO_RIGHT */
          : 'right-to-left' /* PhysicalDirection.RIGHT_TO_LEFT */,
    });
  }
  return extendWithReverseDirections({
    row: isRtl
...
export function rotateAlignContentIcon(iconName, direction) {
  return {
    iconName,
    rotate:
      direction === 'right-to-left' /* PhysicalDirection.RIGHT_TO_LEFT */
        ? 90
        : direction === 'left-to-right' /* PhysicalDirection.LEFT_TO_RIGHT */
          ? -90
          : 0,
    scaleX: 1,
    scaleY: 1,
...
export function rotateJustifyContentIcon(iconName, direction) {
  return {
    iconName,
    rotate:
      direction === 'top-to-bottom' /* PhysicalDirection.TOP_TO_BOTTOM */
        ? 90
        : direction === 'bottom-to-top' /* PhysicalDirection.BOTTOM_TO_TOP */
          ? -90
          : 0,
...
function flexJustifyContentIcon(iconName) {
  function getIcon(computedStyles) {
    const directions = getPhysicalDirections(computedStyles);
    return rotateJustifyContentIcon(
      iconName,
      directions[computedStyles.get('flex-direction') || 'row'],
    );
  }
  return getIcon;
}
