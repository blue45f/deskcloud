...
const str_ = i18n.i18n.registerUIStrings(
  'panels/elements/components/StylePropertyEditor.ts',
  UIStrings,
);
...
export class StylePropertyEditor extends HTMLElement {
  ...
  #renderProperty(prop) {
    ...
    return html`<div class="row">
      <div class="property">
        <span class="property-name">${prop.propertyName}</span>:
        <span class=${classes}>${shownValue}</span>
      </div>
      <div class="buttons">
        ${prop.propertyValues.map((value) =>
          this.#renderButton(value, prop.propertyName, value === authoredValue),
        )}
      </div>
    </div>`;
  }
  #renderGridAutoFlowProperty(prop, shownValue, classes) {
    ...
    const isDense =
      authoredValue === 'dense' ||
      authoredValue === 'row dense' ||
      authoredValue === 'column dense';
  ...
  #renderButton(propertyValue, propertyName, selected = false) {
    ...
    const title = selected
      ? i18nString(UIStrings.deselectButton, values)
      : i18nString(UIStrings.selectButton, values);
    return html`
      <button
        title=${title}
        class=${classes}
      ...
      >
        <devtools-icon style=${transform} name=${iconInfo.iconName}> </devtools-icon>
      </button>
    `;
  }
...
export const GridEditableProperties = [
  ...
  },
  {
    ...
    propertyValues: ['center', 'start', 'end', 'stretch'],
  },
];
