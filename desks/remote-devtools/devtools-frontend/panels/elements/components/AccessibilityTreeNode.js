...
const str_ = i18n.i18n.registerUIStrings(
  'panels/elements/components/AccessibilityTreeNode.ts',
  UIStrings,
);
...
export class AccessibilityTreeNode extends HTMLElement {
  #shadow = UI.UIUtils.createShadowRootWithCoreStyles(this, {
    cssFile: accessibilityTreeNodeStyles,
  });
  #ignored = true;
  #name = '';
  #role = '';
  ...
  async #render() {
    ...
    const properties = this.#properties.map(({ name, value }) =>
      isPrintable(value.type)
        ? html` <span class="attribute-name">${name}</span>:&nbsp;<span class="attribute-value"
              >${value.value}</span
            >`
        : nothing,
    );
    const content = this.#ignored
      ? html`<span>${i18nString(UIStrings.ignored)}</span>`
      : html`${role}&nbsp;${name}${properties}`;
