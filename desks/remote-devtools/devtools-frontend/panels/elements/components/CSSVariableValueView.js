...
const str_ = i18n.i18n.registerUIStrings(
  'panels/elements/components/CSSVariableValueView.ts',
  UIStrings,
);
...
export class CSSVariableParserError extends HTMLElement {
  ...
  #render(details) {
    ...
    render(
      html` <style>
          ${cssVariableValueViewStyles}
        </style>
        <div class="variable-value-popup-wrapper">
          ${i18nTemplate(UIStrings.invalidPropertyValue, { type })} ${getLinkSection(details)}
        </div>`,
      this.#shadow,
      {
        host: this,
...
export class CSSVariableValueView extends HTMLElement {
  ...
  #render() {
    ...
    const registrationView = this.details
      ? html` <hr class="divider" />
          <div class="registered-property-popup-wrapper">
            <div class="monospace">
              <div>
                <span class="css-property">syntax:</span> ${this.details.registration.syntax()}
              </div>
              <div>
                <span class="css-property">inherits:</span> ${this.details.registration.inherits()}
              </div>
              ${initialValue
                ? html`<div><span class="css-property">initial-value:</span> ${initialValue}</div>`
                : ''}
            </div>
    ...
    render(
      html`<style>
        ...
        <div class="variable-value-popup-wrapper">${valueText}</div>
        ${registrationView} `,
      this.#shadow,
      {
        host: this,
