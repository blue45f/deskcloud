...
const str_ = i18n.i18n.registerUIStrings(
  'panels/elements/components/CSSPropertyDocsView.ts',
  UIStrings,
);
...
const browserIdToNameAndPlatform = new Map([
  ...
  [
    'FFA' /* BrowserId.FFA */,
    { name: 'Firefox', platform: 'Android' /* BrowserPlatform.ANDROID */ },
  ],
...
function formatBrowserList(browserNames) {
  ...
  return formatter.format(
    browserNames
      .entries()
      .map(([name, platforms]) =>
        platforms.length !== 1 || platforms[0] === 'desktop' /* BrowserPlatform.DESKTOP */
          ? name
...
const getBaselineText = (baseline, browsers) => {
  if (baseline.status === 'false' /* BaselineStatus.LIMITED */) {
    ...
    if (missingBrowsers) {
      return i18nString(UIStrings.limitedAvailabilityInBrowsers, {
        PH1: formatBrowserList(missingBrowsers),
      });
    }
    return i18nString(UIStrings.limitedAvailability);
  }
  if (baseline.status === 'low' /* BaselineStatus.LOW */) {
    return i18nString(UIStrings.newlyAvailableSince, {
      PH1: formatBaselineDate(baseline.baseline_low_date),
    });
  }
  return i18nString(UIStrings.widelyAvailableSince, {
...
export class CSSPropertyDocsView extends HTMLElement {
  ...
  #render() {
    ...
    render(
      html`
        <style>
          ${CSSPropertyDocsViewStyles}
        </style>
        <div class="docs-popup-wrapper">
          ${description ? html` <div id="description">${description}</div> ` : nothing}
          ${baseline
            ? html`
                <div id="baseline" class="docs-popup-section">
                  <img
                    ...
                    role="presentation"
                  />
                  <span> ${getBaselineText(baseline, browsers)} </span>
                </div>
              `
            : nothing}
          ${link
