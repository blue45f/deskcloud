...
const gridNodesToElements = (nodes) => {
  return nodes.map((node) => {
    ...
    return {
      ...layoutElement,
      color:
        node.domModel().overlayModel().colorOfGridInPersistentOverlay(nodeId) ||
...
const flexContainerNodesToElements = (nodes) => {
  return nodes.map((node) => {
    ...
    return {
      ...layoutElement,
      color:
        node.domModel().overlayModel().colorOfFlexInPersistentOverlay(nodeId) ||
        'var(--sys-color-inverse-surface)',
...
const DEFAULT_VIEW = (input, output, target) => {
  ...
  const renderElement = (element) =>
    html`<div class="element" jslog=${VisualLogging.item().track({ resize: true })}>
      <devtools-checkbox
        data-element="true"
  ...
  render(
    html` <div
      ...
      jslog=${VisualLogging.pane('layout').track({ resize: true })}
    >
      <style>
        ${layoutPaneStyles}
      </style>
                      ...
                      (opt) =>
                        html`<option
                          ...
                          jslog=${VisualLogging.item(
                            Platform.StringUtilities.toKebabCase(opt.value),
                          ).track({
                            click: true,
                          })}
                        >
        ...
        ? html` <details open>
            ...
            ${input.flexContainerElements
              ? html`<div
                  ...
                  jslog=${VisualLogging.section('flexbox-overlays')}
                >
                  <h3 class="content-section-title">
                  ...
                  ${input.flexContainerElements.length
                    ? html`<div class="elements">
                        ${input.flexContainerElements.map(renderElement)}
                      </div>`
                    : ''}
                </div>`
              : ''}
...
export class LayoutPane extends UI.Widget.Widget {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    ...
    this.#uaShadowDOMSetting =
      Common.Settings.Settings.instance().moduleSetting('show-ua-shadow-dom');
    this.#domModels = [];
    this.#view = view;
  }
  static instance() {
  ...
  modelAdded(domModel) {
    ...
    overlayModel.addEventListener(
      'PersistentGridOverlayStateChanged' /* SDK.OverlayModel.Events.PERSISTENT_GRID_OVERLAY_STATE_CHANGED */,
      this.requestUpdate,
      this,
    );
    overlayModel.addEventListener(
      'PersistentFlexContainerOverlayStateChanged' /* SDK.OverlayModel.Events.PERSISTENT_FLEX_CONTAINER_OVERLAY_STATE_CHANGED */,
      this.requestUpdate,
      this,
    );
