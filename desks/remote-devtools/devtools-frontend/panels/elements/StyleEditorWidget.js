  ...
  async render() {
    ...
    this.editor.data = {
      authoredProperties: this.section
        ? getAuthoredStyles(this.section, this.editor.getEditableProperties())
        : new Map(),
      computedProperties: this.stylesContainer
        ? await fetchComputedStyles(this.stylesContainer)
        : new Map(),
    };
  }
  async performUpdate() {
...
function ensureTreeElementForProperty(section, propertyName) {
  const target = section.propertiesTreeOutline
    .rootElement()
    .children()
    .find(
