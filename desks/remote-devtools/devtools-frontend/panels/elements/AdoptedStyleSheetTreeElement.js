  ...
  static createContents(header, treeElement) {
    ...
    if (linkText) {
      documentElement.appendChild(
        Components.Linkifier.Linkifier.linkifyURL(linkText, {
          text: linkText,
          preventClick: true,
...
export class AdoptedStyleSheetContentsTreeElement extends UI.TreeOutline.TreeElement {
  ...
  onbind() {
    this.styleSheetHeader
      .cssModel()
      .addEventListener(SDK.CSSModel.Events.StyleSheetChanged, this.onStyleSheetChanged, this);
  ...
  onunbind() {
    ...
    this.styleSheetHeader
      .cssModel()
      .removeEventListener(SDK.CSSModel.Events.StyleSheetChanged, this.onStyleSheetChanged, this);
  }
  async onpopulate() {
    ...
    if (!TextUtils.ContentData.ContentData.isError(data) && data.isTextContent) {
      ...
      const newNode = this.listItemElement.createChild(
        'span',
        'webkit-html-text-node webkit-html-css-node',
      );
      newNode.setAttribute(
        'jslog',
        `${VisualLogging.value('css-text-node').track({ change: true, dblclick: true })}`,
      );
  ...
  async startEditing(target) {
    ...
    textNode.textContent =
      TextUtils.ContentData.ContentData.isError(data) || !data.isTextContent ? '' : data.text;
  ...
  async editingCommitted(element, newText, oldText) {
    ...
    if (newText !== oldText) {
      await this.styleSheetHeader
        .cssModel()
        .setStyleSheetText(this.styleSheetHeader.id, newText, false);
    }
    this.editingCancelled();
  }
  editingCancelled() {
