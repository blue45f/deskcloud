...
const str_ = i18n.i18n.registerUIStrings(
  'panels/browser_debugger/XHRBreakpointsSidebarPane.ts',
  UIStrings,
);
...
export class XHRBreakpointsSidebarPane extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.#emptyElement.addEventListener(
      'contextmenu',
      this.emptyElementContextMenu.bind(this),
      true,
    );
  ...
  emptyElementContextMenu(event) {
    ...
    contextMenu
      .defaultSection()
      .appendItem(i18nString(UIStrings.addBreakpoint), this.addButtonClicked.bind(this), {
  ...
  async addButtonClicked() {
    ...
    inputElementContainer.setAttribute(
      'jslog',
      `${VisualLogging.value('condition').track({ change: true })}`,
    );
  ...
  createElementForItem(item) {
    ...
    const enabled =
      SDK.DOMDebuggerModel.DOMDebuggerManager.instance().xhrBreakpoints().get(item) || false;
    ...
    const title = item
      ? i18nString(UIStrings.urlContainsS, { PH1: item })
  ...
  contextMenu(breakKeyword, event) {
    ...
    contextMenu
      .defaultSection()
      .appendItem(i18nString(UIStrings.removeBreakpoint), removeBreakpoint.bind(this), {
        jslogContext: 'sources.remove-xhr-fetch-breakpoint',
      });
    contextMenu
      .defaultSection()
      .appendItem(removeAllTitle, removeAllBreakpoints.bind(this), {
  ...
  labelClicked(breakKeyword) {
    ...
    const commit = (inputElement, newText, _oldText, element) => {
      ...
      if (element) {
        ...
        const checkboxElement = breakpointEntryElement
          ? breakpointEntryToCheckbox.get(breakpointEntryElement)
          : undefined;
        if (checkboxElement) {
