...
const str_ = i18n.i18n.registerUIStrings(
  'panels/browser_debugger/DOMBreakpointsSidebarPane.ts',
  UIStrings,
);
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html`
      <style>
        ${domBreakpointsSidebarPaneStyles}
      </style>
      <div
        class="dom-breakpoints-container"
        jslog=${VisualLogging.section('sources.dom-breakpoints').track({ resize: true })}
      >
        ${hasBreakpoints
          ? html`<ul class="breakpoint-list" aria-label=${i18nString(UIStrings.domBreakpointsList)}>
              ${input.breakpoints.map((item) => {
                const { breakpoint } = item;
                const checkedStateText = breakpoint.enabled
                  ? i18nString(UIStrings.checked)
                  : i18nString(UIStrings.unchecked);
                const linkified = PanelsCommon.DOMLinkifier.Linkifier.instance().linkify(
                  breakpoint.node,
                  { preventKeyboardFocus: true, tooltip: undefined },
                );
                const linkifiedText = breakpoint.node.simpleSelector();
                return html` <li
                  class=${`breakpoint-entry ${item.isHighlighted ? 'breakpoint-hit' : ''}`}
                  ...
                  aria-label=${i18nString(UIStrings.sSS, {
                    PH1: item.label,
                    PH2: linkifiedText,
                    PH3: checkedStateText,
                  })}
                  aria-description=${item.isHighlighted
                    ? i18nString(UIStrings.sBreakpointHit, { PH1: checkedStateText })
                    : checkedStateText}
                  jslog=${VisualLogging.domBreakpoint()
                    .context(breakpoint.type)
                    .track({ keydown: 'ArrowUp|ArrowDown|PageUp|PageDown' })}
                >
                  <devtools-checkbox
                    ...
                    aria-description=${Directives.ifDefined(
                      item.isHighlighted ? i18nString(UIStrings.breakpointHit) : undefined,
                    )}
...
export class DOMBreakpointsSidebarPane extends UI.Widget.VBox {
  ...
  constructor(view = DEFAULT_VIEW) {
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      ...
      'DOMBreakpointAdded' /* SDK.DOMDebuggerModel.Events.DOM_BREAKPOINT_ADDED */,
      this.breakpointAdded,
      this,
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.DOMDebuggerModel.DOMDebuggerModel,
      'DOMBreakpointToggled' /* SDK.DOMDebuggerModel.Events.DOM_BREAKPOINT_TOGGLED */,
      this.breakpointToggled,
      this,
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.DOMDebuggerModel.DOMDebuggerModel,
      'DOMBreakpointsRemoved' /* SDK.DOMDebuggerModel.Events.DOM_BREAKPOINTS_REMOVED */,
  ...
  onBreakpointContextMenu(breakpoint, event) {
    ...
    contextMenu.defaultSection().appendItem(
      i18nString(UIStrings.removeAllDomBreakpoints),
      () => {
  ...
  update() {
    ...
    if (
      !details?.auxData ||
      details.reason !== 'DOM' /* Protocol.Debugger.PausedEventReason.DOM */
    ) {
      return;
    }
...
export class ContextMenuProvider {
  appendApplicableItems(_event, contextMenu, node) {
    ...
    function toggleBreakpoint(type) {
      ...
      if (domDebuggerModel.hasDOMBreakpoint(node, type)) {
        ...
        UI.ARIAUtils.LiveAnnouncer.alert(
          `${i18nString(UIStrings.breakpointRemoved)}: ${labelString}`,
        );
      } else {
    ...
    const breakpointsMenu = contextMenu
      .debugSection()
      .appendSubMenuItem(i18nString(UIStrings.breakOn), false, 'break-on');
    ...
    for (const type of Object.values(allBreakpointTypes)) {
      ...
      if (label) {
        breakpointsMenu
          .defaultSection()
          .appendCheckboxItem(label(), toggleBreakpoint.bind(null, type), {
