...
const str_ = i18n.i18n.registerUIStrings(
  'panels/browser_debugger/CategorizedBreakpointsSidebarPane.ts',
  UIStrings,
);
...
export const DEFAULT_VIEW = (input, output, target) => {
  const shouldExpandCategory = (breakpoints) =>
    Boolean(input.filterText) ||
    (input.highlightedItem && breakpoints.includes(input.highlightedItem)) ||
    breakpoints.some((breakpoint) => breakpoint.enabled());
  const filterRegex = input.filterText
    ? new RegExp(Platform.StringUtilities.escapeForRegExp(input.filterText), 'i')
    : null;
  const filter = (breakpoint) =>
    !filterRegex ||
    Boolean(
      Sources.CategorizedBreakpointL10n.getLocalizedBreakpointName(breakpoint.name).match(
        filterRegex,
      ),
    ) ||
    breakpoint === input.highlightedItem;
  const filteredCategories = input.sortedCategoryNames
    .values()
    .map((category) => {
  ...
  const onCheckboxClicked = (event, target) => {
    ...
    if (target instanceof SDK.CategorizedBreakpoint.CategorizedBreakpoint) {
      ...
      input.categories
        .get(target)
        ?.forEach((breakpoint) => input.onBreakpointChange(breakpoint, enabled));
    }
  };
  ...
  render(
    // clang-format off
    html` <devtools-toolbar jslog=${VisualLogging.toolbar()}>
      ...
      <devtools-tree
        autofocus
        @keydown=${onKeyDown}
        .template=${html` <ul role="tree">
          ${filteredCategories.map(
            ([category, breakpoints]) =>
              html` <li
                @select=${() => input.onItemSelected(category)}
              ...
              >
                <style>
                  ${categorizedBreakpointsSidebarPaneStyles}
                </style>
                <devtools-checkbox
                  class="small"
                  tabIndex="-1"
                ...
                <ul role="group">
                  ${ifExpanded(
                    html`${breakpoints.map(
                      (breakpoint) =>
                        html` <li
                          @select=${() => input.onItemSelected(breakpoint)}
                          role="treeitem"
                          aria-checked=${breakpoint.enabled()}
                          jslog-context=${Platform.StringUtilities.toKebabCase(breakpoint.name)}
                        >
                          <div
                            ?hidden=${breakpoint !== input.highlightedItem}
                            class="breakpoint-hit-marker"
                          ></div>
                          <devtools-checkbox
                            class=${classes(breakpoint)}
                            tabIndex="-1"
                            title=${Sources.CategorizedBreakpointL10n.getLocalizedBreakpointName(
                              breakpoint.name,
                            )}
                            ?checked=${breakpoint.enabled()}
                            aria-description=${breakpoint === input.highlightedItem
                              ? i18nString(UIStrings.breakpointHit)
                              : Lit.nothing}
                            @change=${(e) => onCheckboxClicked(e, breakpoint)}
                            >${Sources.CategorizedBreakpointL10n.getLocalizedBreakpointName(
                              breakpoint.name,
                            )}</devtools-checkbox
                          >
                        </li>`,
...
const LOCALIZED_CATEGORIES = {
  ['animation' /* SDK.CategorizedBreakpoint.Category.ANIMATION */]: i18nLazyString(
    UIStrings.animation,
  ),
  ['auction-worklet' /* SDK.CategorizedBreakpoint.Category.AUCTION_WORKLET */]: i18nLazyString(
    UIStrings.auctionWorklet,
  ),
  ['canvas' /* SDK.CategorizedBreakpoint.Category.CANVAS */]: i18nLazyString(UIStrings.canvas),
  ['clipboard' /* SDK.CategorizedBreakpoint.Category.CLIPBOARD */]: i18nLazyString(
    UIStrings.clipboard,
  ),
  ...
  ['dom-mutation' /* SDK.CategorizedBreakpoint.Category.DOM_MUTATION */]: i18nLazyString(
    UIStrings.domMutation,
  ),
  ['drag-drop' /* SDK.CategorizedBreakpoint.Category.DRAG_DROP */]: i18nLazyString(
    UIStrings.dragDrop,
  ),
  ['geolocation' /* SDK.CategorizedBreakpoint.Category.GEOLOCATION */]: i18nLazyString(
    UIStrings.geolocation,
  ),
  ['keyboard' /* SDK.CategorizedBreakpoint.Category.KEYBOARD */]: i18nLazyString(
    UIStrings.keyboard,
  ),
  ...
  ['notification' /* SDK.CategorizedBreakpoint.Category.NOTIFICATION */]: i18nLazyString(
    UIStrings.notification,
  ),
  ['parse' /* SDK.CategorizedBreakpoint.Category.PARSE */]: i18nLazyString(UIStrings.parse),
  ['picture-in-picture' /* SDK.CategorizedBreakpoint.Category.PICTURE_IN_PICTURE */]:
  ...
  ['shared-storage-worklet' /* SDK.CategorizedBreakpoint.Category.SHARED_STORAGE_WORKLET */]:
  ...
  ['trusted-type-violation' /* SDK.CategorizedBreakpoint.Category.TRUSTED_TYPE_VIOLATION */]:
    i18nLazyString(UIStrings.trustedTypeViolations),
  ['web-audio' /* SDK.CategorizedBreakpoint.Category.WEB_AUDIO */]: i18nLazyString(
    UIStrings.webaudio,
  ),
