...
export class JSONEditor extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  wasShown() {
    ...
    targetManager.addEventListener(
      'AvailableTargetsChanged' /* SDK.TargetManager.Events.AVAILABLE_TARGETS_CHANGED */,
  ...
  #convertArrayParameter(key, value, schema) {
    ...
    const nestedType = isTypePrimitive(typeRef)
      ? undefined
      : {
          optional: true,
          type: 'object' /* ParameterType.OBJECT */,
  ...
  #getChildByPath(pathArray) {
    ...
    for (let i = 0; i < pathArray.length; i++) {
      ...
      if (
        parameter?.type === 'array' /* ParameterType.ARRAY */ ||
        parameter?.type === 'object' /* ParameterType.OBJECT */
      ) {
        if (parameter.value) {
  ...
  #createNestedParameter(type, name) {
    if (type.type === 'object' /* ParameterType.OBJECT */) {
      ...
      const nestedTypes = this.typesByName.get(typeRef) ?? [];
      const nestedValue = nestedTypes.map((nestedType) =>
        this.#createNestedParameter(nestedType, nestedType.name),
      );
      return {
        type: 'object' /* ParameterType.OBJECT */,
        name,
  ...
  #handleDeleteParameter(parameter, parentParameter) {
    ...
    parentParameter.value.splice(
      parentParameter.value.findIndex((p) => p === parameter),
      1,
    );
    ...
    <div class="row attribute padded">
      ...
      >
        ${input.targets.map(
          (target) =>
            html` <option
              jslog=${VisualLogging.item('target').track({ click: true, resize: true })}
              value=${target.id()}
              ?selected=${target.id() === input.targetId}
            >
...
function renderWarningIcon() {
  return html`<devtools-icon name="warning-filled" class="warning-icon small"> </devtools-icon>`;
}
/**
  ...
  return html`
    <ul>
      ${repeat(parameters, (parameter) => {
        const parameterId = parentParameter
          ? `${parentParameterId}` + '.' + `${parameter.name}`
          : parameter.name;
        const subparameters =
          parameter.type === 'array' /* ParameterType.ARRAY */ ||
          parameter.type === 'object' /* ParameterType.OBJECT */
        ...
        const isParentArray =
          parentParameter && parentParameter.type === 'array'; /* ParameterType.ARRAY */
        const isParentObject =
          parentParameter && parentParameter.type === 'object'; /* ParameterType.OBJECT */
        ...
        const hasTypeRef =
          isObject && parameter.typeRef && input.typesByName.get(parameter.typeRef) !== undefined;
        ...
        const hasOptions =
          parameter.type === 'string' /* ParameterType.STRING */ ||
          parameter.type === 'boolean'; /* ParameterType.BOOLEAN */
        const canClearParameter =
          (isArray && !isParamValueUndefined && parameter.value?.length !== 0) ||
          (isObject && !isParamValueUndefined);
        ...
        return html`
          <li class="row">
                ...
                ${hasNoKeys
                  ? html`<devtools-suggestion-input
                      ...
                      .placeholder=${parameter.value === ''
                        ? EMPTY_STRING
                        : `<${defaultValueByType.get(parameter.type)}>`}
            ...
            <div class="row-icons">
              ...
              ${isPrimitive &&
              !hasNoKeys &&
              (!isParamValueUndefined || !isParamOptional) &&
              !isParentArray
                ? html` <!-- @ts-ignore -->
                    <devtools-suggestion-input
                      data-paramId=${parameterId}
              ...
              ${isParentArray
                ? html` <!-- If the parameter is an object we don't want to display the input field we just want the delete button-->
                    ${!isObject
                      ? html` <!-- @ts-ignore -->
                          <devtools-suggestion-input
                            data-paramId=${parameterId}
    ...
    >
      ...
      <devtools-toolbar class="protocol-monitor-sidebar-toolbar">
        <devtools-button
          ...
          @click=${input.onCopyToClipboard}
        ></devtools-button>
        <div class="toolbar-spacer"></div>
        <devtools-button
          title=${Host.Platform.isMac()
            ? i18nString(UIStrings.sendCommandCmdEnter)
            : i18nString(UIStrings.sendCommandCtrlEnter)}
          .iconName=${'send'}
          jslogContext="protocol-monitor.send-command"
          .variant=${'primary_toolbar' /* Buttons.Button.Variant.PRIMARY_TOOLBAR */}
          @click=${input.onCommandSend}
        ></devtools-button>
      </devtools-toolbar>
    </div>`,
    target,
