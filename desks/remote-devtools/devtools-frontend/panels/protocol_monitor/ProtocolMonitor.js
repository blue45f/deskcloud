          ...
          <div slot="main" class="vbox protocol-monitor-main">
            ...
            <devtools-split-view direction="column" sidebar-position="second"
                    ...
                    ${input.messages.map(
                      (message) =>
                        ...
                        >
                          ${'id' in message
                            ? html` <td title="sent">
                                <devtools-icon
                                  name="arrow-up-down"
                                  class="medium"
                                  style="color: var(--icon-request-response);"
                                >
                                </devtools-icon>
                              </td>`
                            : html` <td title="received">
                                <devtools-icon
                                  name="arrow-down"
                                  class="medium"
                                  style="color: var(--icon-request);"
                                >
                                </devtools-icon>
                              </td>`}
                          ...
                          <td>
                            ${message.params
                              ? html`<code>${JSON.stringify(message.params)}</code>`
                              : ''}
                          </td>
                          <td>
                            ${message.result
                              ? html`<code>${JSON.stringify(message.result)}</code>`
                              : message.error
                                ? html`<code>${JSON.stringify(message.error)}</code>`
                                : 'id' in message
                                  ? '(pending)'
                                  : ''}
                          </td>
                          <td data-value=${message.elapsedTime || 0}>
                            ${!('id' in message)
                              ? ''
                              : message.elapsedTime
                                ? i18nString(UIStrings.sMs, { PH1: String(message.elapsedTime) })
                                : '(pending)'}
                          </td>
                          <td data-value=${message.requestTime}>
                            ${i18nString(UIStrings.sMs, { PH1: String(message.requestTime) })}
                          </td>
              ...
              <devtools-widget ${widget(InfoWidget, {
                ...
                type: !input.selectedMessage
                  ? undefined
                  : 'id' in input?.selectedMessage
                    ? 'sent'
                    : 'received',
              })}
            ...
            <devtools-toolbar class="protocol-monitor-bottom-toolbar"
              ...
              <select class="target-selector"
                ...
                ${input.targets.map(
                  (target) =>
                    html` <option
                      jslog=${VisualLogging.item('target').track({ click: true })}
                      value=${target.id()}
                      ?selected=${target.id() === input.selectedTargetId}
                    >
...
export class ProtocolMonitorImpl extends UI.Panel.Panel {
  ...
  constructor(view = DEFAULT_VIEW) {
    ...
    this.performUpdate();
    this.#editorWidget.addEventListener(
      'submiteditor' /* JSONEditorEvents.SUBMIT_EDITOR */,
      (event) => {
  ...
  #populateContextMenu(message, menu) {
    ...
    menu.footerSection().appendItem(
      i18nString(UIStrings.documentation),
      () => {
  ...
  messageReceived(message) {
    ...
    const target =
      message.sessionId !== undefined ? this.#targetsBySessionId.get(message.sessionId) : undefined;
    this.#messages.push({
