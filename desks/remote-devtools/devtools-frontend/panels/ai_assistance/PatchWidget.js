...
const DEFAULT_VIEW = (input, output, target) => {
  ...
  function renderHeader() {
    if (input.savedToDisk) {
      return html`
        ...
        <span class="header-text"> ${lockedString(UIStringsNotTranslate.savedToDisk)} </span>
      `;
    }
    if (input.patchSuggestionState === PatchSuggestionState.SUCCESS) {
      return html`
        ...
        <span class="header-text"> ${lockedString(`File changes in ${input.projectName}`)} </span>
    ...
    return html`
      ...
      <span class="header-text"> ${lockedString(UIStringsNotTranslate.unsavedChanges)} </span>
      <devtools-icon class="arrow" name="chevron-down"></devtools-icon>
    `;
  }
  function renderContent() {
    if (
      (!input.changeSummary && input.patchSuggestionState === PatchSuggestionState.INITIAL) ||
      input.savedToDisk
    ) {
      return nothing;
    }
    ...
    return html`<devtools-code-block
      ...
      ${input.patchSuggestionState === PatchSuggestionState.ERROR
        ? html`<div class="error-container">
            <devtools-icon name="cross-circle-filled"></devtools-icon>${lockedString(
              UIStringsNotTranslate.genericErrorMessage,
            )}
            ${renderSourcesLink()}
          </div>`
        : nothing}`;
  ...
  function renderFooter() {
    ...
    if (input.patchSuggestionState === PatchSuggestionState.SUCCESS) {
      return html`
        <div class="footer">
          <div class="left-side">
            <devtools-link
              class="link disclaimer-link"
              href="https://support.google.com/legal/answer/13505487"
              jslogcontext="code-disclaimer"
            >
    ...
    const iconName =
      input.projectType === SelectedProjectType.AUTOMATIC_DISCONNECTED
        ? 'folder-off'
        : input.projectType === SelectedProjectType.AUTOMATIC_CONNECTED
          ? 'folder-asterisk'
          : 'folder';
    return html` <div class="footer">
      ...
      <div class="apply-to-workspace-container" aria-live="polite">
        ${input.patchSuggestionState === PatchSuggestionState.LOADING
          ? html`
              <div
                ...
                jslog=${VisualLogging.section('patch-widget.apply-to-workspace-loading')}
              >
                <devtools-spinner></devtools-spinner>
                <span> ${lockedString(UIStringsNotTranslate.applyingToWorkspace)} </span>
              </div>
            `
          : html`
        ...
        ></devtools-button>
        <devtools-tooltip id="info-tooltip" variant="rich">
  ...
  const template = input.savedToDisk
    ? html` <div class="change-summary saved-to-disk" role="status" aria-live="polite">
        <div class="header-container">${renderHeader()}</div>
      </div>`
    : html`
...
export class PatchWidget extends UI.Widget.Widget {
  ...
  #aiPatchingFreCompletedSetting = Common.Settings.Settings.instance().createSetting(
    'ai-assistance-patching-fre-completed',
    false,
  );
  #projectIdSetting = Common.Settings.Settings.instance().createSetting(
  ...
  #shouldShowChangeButton() {
    const automaticFileSystemProject = this.#automaticFileSystem
      ? this.#workspace.projectForFileSystemRoot(this.#automaticFileSystem.root)
      : null;
    const regularProjects = this.#workspace
  ...
  #getSelectedProjectType(projectPath) {
    if (this.#automaticFileSystem && this.#automaticFileSystem.root === projectPath) {
      return this.#project
        ? SelectedProjectType.AUTOMATIC_CONNECTED
        : SelectedProjectType.AUTOMATIC_DISCONNECTED;
    }
  ...
  #onProjectAdded(event) {
    ...
    if (
      this.#applyToDisconnectedAutomaticWorkspace &&
  ...
  async #applyPatchAndUpdateUI() {
    ...
    if (
      response?.type === 'answer' /* AiAssistanceModel.AiAgent.ResponseType.ANSWER */ &&
      hasChanges
    ) {
      this.#patchSuggestionState = PatchSuggestionState.SUCCESS;
    } else if (
      response?.type === 'error' /* AiAssistanceModel.AiAgent.ResponseType.ERROR */ &&
...
${processedFiles.map((filename) => `* ${filename}`).join('\n')}`;
  ...
  async #applyPatch(changeSummary) {
    ...
    const { responses, processedFiles } = await agent.applyChanges(changeSummary, {
      signal: this.#applyPatchAbortController.signal,
    });
    return {
...
globalThis.aiAssistanceTestPatchPrompt = async (projectName, changeSummary, expectedChanges) => {
  if (!isAiAssistancePatchingEnabled()) {
    return;
  }
