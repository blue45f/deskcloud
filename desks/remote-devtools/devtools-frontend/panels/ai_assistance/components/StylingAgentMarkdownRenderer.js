...
export class StylingAgentMarkdownRenderer extends MarkdownRendererWithCodeBlock {
  ...
  #renderTableFromJson(data) {
    ...
    return html`
      <table style="width: 100%;">
        <thead>
          <tr>
            ${headers.map(
              (header) =>
                html`<th style="text-align: left;">${header === 'NodeId' ? '' : header}</th>`,
            )}
          </tr>
        </thead>
        <tbody>
      ...
      <br />
      <div>
        To investigate these problems, please click one of the provided links (above), to set as
        context, and ask me further questions about the problem.
      </div>
    `;
  }
  templateForToken(token) {
    ...
    if (token.type === 'link' && token.href.startsWith('#')) {
      ...
      if (nodeId) {
        return html`<span
          >${until(
            this.#linkifyNode(nodeId, token.text).then((node) => node || token.text),
            token.text,
          )}</span
        >`;
      }
  ...
  #renderSingleLink(nodeId) {
    ...
    return html`<span
      >${until(
        this.#linkifyNode(nodeId, label).then((node) => node || label),
        label,
      )}</span
    >`;
  }
  async #linkifyNode(backendNodeId, label) {
    ...
    const linkedNode = PanelsCommon.DOMLinkifier.Linkifier.instance().linkify(node, {
      textContent: label,
    });
    return linkedNode;
  }
