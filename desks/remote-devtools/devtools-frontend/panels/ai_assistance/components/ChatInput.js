...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  const renderRelevantDataDisclaimer = (tooltipId) => {
    ...
    return html`
      <div class=${classes}>
        ...
        >
          ${lockedString('Relevant data')}</button
  ...
  Lit.render(
    html`
      ...
      ${input.isReadOnly
        ? html` <div class="chat-readonly-container" jslog=${VisualLogging.section('read-only')}>
        ...
        : html` <form class="input-form" @submit=${input.onSubmit}>
              ...
              ${input.multimodalInputEnabled && input.imageInput && !input.isTextInputDisabled
                ? html` <div class="image-input-container">
                    ...
                    ${input.imageInput.isLoading
                      ...
                      : html` <img
                          src="data:${input.imageInput.mimeType};base64, ${input.imageInput.data}"
                          alt="Image input"
                        />`}
                  </div>`
                : Lit.nothing}
              ...
              <div class="chat-input-actions">
                          ...
                          ${input.context instanceof AiAssistanceModel.StylingAgent.NodeContext
                            ? html` <devtools-widget
                                ...
                                ${widget(PanelsCommon.DOMLinkifier.DOMNodeLink, {
                                  ...
                                  options: {
                                    disabled: !input.isContextSelected,
                                    hiddenClassList: input.context
                                      .getItem()
                                      .classNames()
                                      .filter((className) =>
                                        className.startsWith(
                                          AiAssistanceModel.Injected.AI_ASSISTANCE_CSS_CLASS_NAME,
                                        ),
                                      ),
                              ...
                              ></devtools-widget>`
                            : html` ${input.context instanceof
                                AiAssistanceModel.NetworkAgent.RequestContext
                                  ? PanelUtils.PanelUtils.getIconForNetworkRequest(
                                      input.context.getItem(),
                                    )
                                  : input.context instanceof AiAssistanceModel.FileAgent.FileContext
                                    ? PanelUtils.PanelUtils.getIconForSourceFile(
                                        input.context.getItem(),
                                      )
                                    : input.context instanceof
                                        AiAssistanceModel.AccessibilityAgent.AccessibilityContext
                                      ? html`<devtools-icon
                                          class="icon"
                                          name="performance"
                                          title="Lighthouse"
                                        ></devtools-icon>`
                                      : input.context instanceof
                                          AiAssistanceModel.PerformanceAgent.PerformanceTraceContext
                                        ? html`<devtools-icon
                                            class="icon"
                                            name="performance"
                                            title="Performance"
                                ...
                                >`}
                          ${input.isContextSelected && input.onContextRemoved
                            ? html` <devtools-button
                                ...
                                .variant=${'icon' /* Buttons.Button.Variant.ICON */}
                                @click=${input.onContextRemoved}
                              ></devtools-button>`
                            : Lit.nothing}
                          ${!input.isContextSelected && input.onContextAdd
                            ? html` <devtools-button
                ...
                <div class="chat-input-actions-right">
                  ...
                  ${input.multimodalInputEnabled && !input.blockedByCrossOrigin
                    ? html` ${input.uploadImageInputEnabled
                          ? html` <devtools-button
                        ...
                        <devtools-button
                          class="chat-input-button"
                          aria-label=${lockedString(
                            UIStringsNotTranslate.takeScreenshotButtonTitle,
                          )}
                    ...
                    : input.blockedByCrossOrigin
                      ...
                      : html` <devtools-button
                          ...
                          .data=${{
                            ...
                            size: 'REGULAR' /* Buttons.Button.Size.REGULAR */,
                            disabled:
                              input.isTextInputDisabled ||
                              input.isTextInputEmpty ||
                              input.imageInput?.isLoading,
                            iconName: 'send',
...
export class ChatInput extends UI.Widget.Widget {
  ...
  async #handleTakeScreenshot() {
    ...
    const bytes = await model.captureScreenshot(
      ...
      SCREENSHOT_QUALITY,
      'fromViewport' /* SDK.ScreenCaptureModel.ScreenshotMode.FROM_VIEWPORT */,
    );
  ...
  async #handleLoadImage(file) {
    ...
    try {
      ...
      this.#imageInput = {
        ...
        mimeType: file.type,
        inputType:
          'uploaded-image' /* AiAssistanceModel.AiAgent.MultimodalInputType.UPLOADED_IMAGE */,
      };
    } catch {
      this.#imageInput = undefined;
      Snackbars.Snackbar.Snackbar.show({
  ...
  onImageUpload = (ev) => {
    ev.stopPropagation();
    const fileSelector = UI.UIUtils.createFileSelectorElement(
