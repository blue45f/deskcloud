 ...
 * ```
 **/
export class MarkdownRendererWithCodeBlock
  extends MarkdownView.MarkdownView.MarkdownInsightRenderer
{
  ...
  #renderLink(href, fallbackText) {
    if (href.startsWith('#req-')) {
      const request = Logs.NetworkLog.NetworkLog.instance()
        .requests()
        .find((req) => req.requestId() === href.substring(5));
      if (request) {
    ...
    if (href.startsWith('#file-')) {
      const file =
        AiAssistanceModel.ContextSelectionAgent.ContextSelectionAgent.getUISourceCodes().find(
          (file) =>
            AiAssistanceModel.ContextSelectionAgent.ContextSelectionAgent.uiSourceCodeId.get(
              file,
