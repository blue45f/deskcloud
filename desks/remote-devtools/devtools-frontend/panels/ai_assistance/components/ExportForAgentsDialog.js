...
const str_ = i18n.i18n.registerUIStrings(
  'panels/ai_assistance/components/ExportForAgentsDialog.ts',
  UIStrings,
);
const i18nString = i18n.i18n.getLocalizedString.bind(undefined, str_);
export const DEFAULT_VIEW = (input, _output, target) => {
  const isPrompt = input.state.activeType === 'prompt'; /* StateType.PROMPT */
  const buttonText = isPrompt
    ? i18nString(UIStrings.copyToClipboard)
    : i18nString(UIStrings.saveAsMarkdown);
  const exportText =
    isPrompt && input.state.isPromptLoading
      ? i18nString(UIStrings.generatingSummary)
      : isPrompt
        ? input.state.promptText
        : input.state.conversationText;
  // clang-format off
  render(
    html`
      <style>
      ...
      <div class="export-for-agents-dialog">
        <header>
          <h2 tabindex="-1">${i18nString(UIStrings.exportForAgents)}</h2>
        </header>
...
export class ExportForAgentsDialog extends UI.Widget.VBox {
  ...
  static show({ promptText, markdownText, onConversationSaveAs }) {
    ...
    const exportDialog = new ExportForAgentsDialog({
      dialog,
      promptText,
      markdownText,
      onConversationSaveAs,
