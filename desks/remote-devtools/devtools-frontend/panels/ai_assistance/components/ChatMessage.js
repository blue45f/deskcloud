...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  if (message.entity === 'user' /* ChatMessageEntity.USER */) {
    const imageInput =
      message.imageInput && 'inlineData' in message.imageInput
              ...
              <div class="message-name">
                <h2>
                  ${AiAssistanceModel.AiUtils.isGeminiBranding()
                    ? lockedString(UIStringsNotTranslate.gemini)
                    : lockedString(UIStringsNotTranslate.ai)}
                </h2>
              </div>
            </div>`}
        ...
        <div class="answer-body-wrapper">
          ${Lit.Directives.repeat(
            ...
            (part, index) => {
              ...
              if (part.type === 'answer') {
                return html`<p>
                  ${renderTextAsMarkdown(part.text, input.markdownRenderer, {
                    animate:
                      !input.isReadOnly && input.isLoading && isLastPart && input.isLastMessage,
                  })}
                </p>`;
              }
              if (part.type === 'widget') {
                return html`${Lit.Directives.until(
                  renderWidgets(part.widgets, { wrapperClass: 'main-widgets-wrapper' }),
                )}`;
              }
          ...
          ${renderError(message)}
          ${input.isLastMessage && hasAiV2 && !input.isLoading && input.changeSummary
            ? html`
                <devtools-code-block
...
function renderStepDetails({ step, markdownRenderer, isLast }) {
  const sideEffects =
    isLast && step.requestApproval ? renderSideEffectConfirmationUi(step) : Lit.nothing;
  const thought = step.thought
    ? html`<p>${renderTextAsMarkdown(step.thought, markdownRenderer)}</p>`
    : Lit.nothing;
  // clang-format off
...
function renderWalkthroughSidebarButton(input, steps) {
  ...
  const isExpanded =
    walkthrough.isExpanded && input.message === input.walkthrough.activeSidebarMessage;
  const title = isExpanded
    ? walkthroughCloseTitle({ hasWidgets: hasOneStepWithWidget })
    : walkthroughTitle({
        isLoading: input.isLoading,
  ...
  // loading.
  const variant =
    hasOneStepWithWidget && !input.isLoading
      ? 'tonal' /* Buttons.Button.Variant.TONAL */
      : 'text'; /* Buttons.Button.Variant.TEXT */
  const icon = AiAssistanceModel.AiUtils.getIconName();
  // clang-format off
  return html`
    <div class="walkthrough-toggle-container ${hasOneStepWithWidget ? 'has-widgets' : ''}">
      ...
      <devtools-button
        ...
        .jslogContext=${walkthrough.isExpanded
      ...
      >
        ${title}<devtools-icon
          class="chevron"
          .name=${isExpanded ? 'cross' : 'chevron-right'}
        ></devtools-icon>
      </devtools-button>
    </div>
  `;
...
function renderWalkthroughUI(input, steps) {
  ...
  // ChatView to open it.
  const openWalkThroughSidebarButton = !input.walkthrough.isInlined
    ? renderWalkthroughSidebarButton(input, steps)
    : Lit.nothing;
  ...
  // clang-format on
  // clang-format off
  const walkthroughInline = input.walkthrough.isInlined
    ? html`
      ...
      `
    : Lit.nothing;
  return html` ${openWalkThroughSidebarButton} ${walkthroughInline} ${sideEffectStepsUI} `;
  // clang-format on
}
    ...
    >
      <summary>
        <div class="summary">
          ...
          <devtools-icon class="arrow" name="chevron-down"></devtools-icon>
        </div>
      </summary>
...
async function makeComputedStyleWidget(widgetData) {
  ...
  const styles = new ComputedStyle.ComputedStyleModel.ComputedStyle(
    domNodeForId,
    widgetData.data.computedStyles,
  );
  // clang-format off
  const renderedWidget = html`<devtools-widget
    class="computed-styles-widget"
    ${widget(Elements.ComputedStyleWidget.ComputedStyleWidget, {
      nodeStyle: styles,
...
async function makeCoreWebVitalsWidget(widgetData) {
  // clang-format off
  const renderedWidget = html`<devtools-widget
    class="core-vitals-widget"
    ${widget(TimelineComponents.CWVMetrics.CWVMetrics, {
      ...
      ${response.title
        ? html`
            <div class="widget-header">
              ...
              <div class="widget-reveal-container">${revealButton}</div>
            </div>
          `
        : Lit.nothing}
      ${response.renderedWidget
        ? html` <div class="widget-content-container">${response.renderedWidget}</div>`
        : Lit.nothing}
      ${!response.title
        ? html` <div class="widget-reveal-container">${revealButton}</div> `
        : Lit.nothing}
    </div>
  `;
  // clang-format on
      ...
      <div class="network-request-header">
        <div class="network-request-icon">
          ${resourceType.isImage()
            ? html`<img src=${networkRequest.imageUrl ?? networkRequest.url} alt=${filename} />`
            : html`<devtools-icon
                name=${iconName}
...
function renderSideEffectConfirmationUi(step) {
  ...
  >
    ${step.requestApproval.description
      ? html`<p>${step.requestApproval.description}</p>`
      : Lit.nothing}
...
function renderError(message) {
  if (message.error) {
    ...
    return html`<p class="error" jslog=${VisualLogging.section('error')}>
...
function renderImageChatMessage(inlineData) {
  ...
  return html`<devtools-link
    class="image-link"
    title=${UIStringsNotTranslate.openImageInNewTab}
    href=${imageUrl}
  >
    ...
    <div class=${rowClasses}>
        ...
        ${input.showRateButtons
          ? html`
              <devtools-button
                ...
                @click=${() =>
                  input.onRatingClick('POSITIVE' /* Host.AidaClient.Rating.POSITIVE */)}
              ></devtools-button>
              <devtools-button
                .data=${{
                  variant: 'icon' /* Buttons.Button.Variant.ICON */,
                ...
                @click=${() =>
                  input.onRatingClick('NEGATIVE' /* Host.AidaClient.Rating.NEGATIVE */)}
              ></devtools-button>
        ...
        ${aiAssistanceV2
          ...
          : html`
              ...
              <devtools-button
                ...
                @click=${input.onCopyResponseClick}
              ></devtools-button>
            `}
        ${input.onExportClick && aiAssistanceV2 && input.isLastMessage
          ? html`
              <devtools-button
      ...
      ${input.suggestions
        ? html`<div class="suggestions-container">
            <div
              class="scroll-button-container left hidden"
              ${ref((element) => {
            ...
            <div
              class="suggestions-scroll-container"
              @scroll=${input.onSuggestionsScrollOrResize}
              ${ref((element) => {
            ...
            <div
              class="scroll-button-container right hidden"
              ${ref((element) => {
...
export class ChatMessage extends UI.Widget.Widget {
    ...
    this.#view(
      {
        ...
        showRateButtons:
          this.message.entity === 'model' /* ChatMessageEntity.MODEL */ && !!this.message.rpcId,
        suggestions:
          this.isLastMessage &&
          this.message.entity === 'model' /* ChatMessageEntity.MODEL */ &&
          !this.isReadOnly &&
  ...
  #evaluateSuggestionsLayout = () => {
    ...
    const shouldShowRightButton =
      suggestionsScrollContainer.scrollLeft +
        suggestionsScrollContainer.offsetWidth +
        SCROLL_ROUNDING_OFFSET <
    ...
    if (this.#currentRating === rating) {
      ...
      if (this.message.entity === 'model' /* ChatMessageEntity.MODEL */ && this.message.rpcId) {
        this.onFeedbackSubmit(
          this.message.rpcId,
          'SENTIMENT_UNSPECIFIED' /* Host.AidaClient.Rating.SENTIMENT_UNSPECIFIED */,
        );
      }
      void this.performUpdate();
      return;
...
async function makeTimelineRangeSummaryWidget(widgetData) {
  ...
  if (track === 'main') {
    ...
    const mainTrack = mainDataProvider
      .timelineData()
      .groups.find((group) => group.name.startsWith('Main \u2014 '));
    if (mainTrack) {
  ...
  thirdPartyTree.activeSelection = Timeline.TimelineSelection.selectionFromRangeMicroSeconds(
    bounds.min,
    bounds.max,
  );
  thirdPartyTree.refreshTree(true);
  // clang-format off
  const template = html` <devtools-widget
    ${widget(TimelineComponents.TimelineRangeSummaryView.TimelineRangeSummaryView, {
      data: {
        ...
        thirdPartyTreeTemplate: html`${widget(
          Timeline.ThirdPartyTreeView.ThirdPartyTreeViewWidget,
          {
            maxRows: 10,
            model: {
