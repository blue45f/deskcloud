...
const str_ = i18n.i18n.registerUIStrings(
  'panels/ai_assistance/components/WalkthroughView.ts',
  UIStrings,
);
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  render(
    html` <style>
      ...
      ${input.isInlined
        ? renderInlineWalkthrough(input, stepsOutput, allSteps)
...
export class WalkthroughView extends UI.Widget.Widget {
  ...
  #handleScroll = (ev) => {
    ...
    if (this.#isProgrammaticScroll) {
      ...
      const isAtBottom =
        ev.target.scrollTop + ev.target.clientHeight + SCROLL_ROUND_OFFSET >=
