...
const str_ = i18n.i18n.registerUIStrings(
  'panels/ai_assistance/components/DisabledWidget.ts',
  UIStrings,
);
...
function renderConsentViewContents(hostConfig) {
  ...
  settingsLink.setAttribute(
    'jslog',
    `${VisualLogging.action('open-ai-settings').track({ click: true })}`,
  );
  ...
  if (hostConfig.devToolsAiAssistancePerformanceAgent?.enabled) {
    ...
    consentViewContents = uiI18n.getFormatLocalizedString(
      str_,
      UIStrings.turnOnForStylesRequestsAndFiles,
      { PH1: settingsLink },
    );
  } else if (hostConfig.devToolsAiAssistanceNetworkAgent?.enabled) {
    consentViewContents = uiI18n.getFormatLocalizedString(
      str_,
      UIStrings.turnOnForStylesAndRequests,
      { PH1: settingsLink },
    );
  } else {
    consentViewContents = uiI18n.getFormatLocalizedString(str_, UIStrings.turnOnForStyles, {
      PH1: settingsLink,
    });
  }
