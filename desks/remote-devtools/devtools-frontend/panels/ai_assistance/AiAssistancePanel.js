...
function getMarkdownRenderer(conversation) {
  ...
  if (context instanceof AiAssistanceModel.PerformanceAgent.PerformanceTraceContext) {
    ...
    conversation?.type ===
    'drjones-performance-full' /* AiAssistanceModel.AiHistoryStorage.ConversationType.PERFORMANCE */
  ) {
    ...
    conversation?.type ===
      'freestyler' /* AiAssistanceModel.AiHistoryStorage.ConversationType.STYLING */ &&
    SDK.TargetManager.TargetManager.instance().primaryPageTarget()?.model(SDK.DOMModel.DOMModel)
  ) {
    const domModel = SDK.TargetManager.TargetManager.instance()
      .primaryPageTarget()
    ...
    conversation?.type ===
    'accessibility' /* AiAssistanceModel.AiHistoryStorage.ConversationType.ACCESSIBILITY */
  ) {
    const domModel = SDK.TargetManager.TargetManager.instance()
      .primaryPageTarget()
      ?.model(SDK.DOMModel.DOMModel);
      ...
      <devtools-toolbar class="freestyler-left-toolbar" role="presentation">
        ${input.showChatActions
          ? html`<devtools-button
              ...
              ></devtools-menu-button>`
          : Lit.nothing}
        ${input.showActiveConversationActions
          ? html` <devtools-button
...
function defaultView(input, output, target) {
  ...
  if (
    Root.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled ||
    Greendev.Prototypes.instance().isEnabled('breakpointDebuggerAgent')
  ) {
    const shouldShowWalkthrough =
      input.state === 'chat-view' /* ViewState.CHAT_VIEW */ && input.props.walkthrough.isExpanded;
    /**
      ...
      html`
        ...
        <div class="ai-assistance-view-container">
          <devtools-split-view
            ...
            sidebar-visibility=${shouldShowWalkthrough && !input.props.walkthrough.isInlined
              ? 'visible'
              : 'hidden'}
            sidebar-initial-size=${WALKTHROUGH_SIDEBAR_INITIAL_WIDTH}
          >
            <div slot="main" class="main-view">${renderState()}</div>
...
export class AiAssistancePanel extends UI.Panel.Panel {
  ...
  constructor(view = defaultView, { aidaClient, aidaAvailability }) {
    ...
    if (UI.ActionRegistry.ActionRegistry.instance().hasAction('elements.toggle-element-search')) {
      this.#toggleSearchElementAction = UI.ActionRegistry.ActionRegistry.instance().getAction(
        'elements.toggle-element-search',
      );
    }
    AiAssistanceModel.AiHistoryStorage.AiHistoryStorage.instance().addEventListener(
      'AiHistoryDeleted' /* AiAssistanceModel.AiHistoryStorage.Events.HISTORY_DELETED */,
  ...
  async #getPanelViewInput() {
    ...
    if (
      ...
      !this.#aiAssistanceEnabledSetting?.getIfNotDisabled() ||
      blockedByAge
    ) {
      return {
    ...
    if (this.#conversation) {
      ...
      if (
        isAiAssistanceContextSelectionAgentEnabled() &&
      ...
      return {
        ...
        props: {
           ...
           */
          context:
            this.#conversation.selectedContext ??
            this.#getConversationContext(this.#getDefaultConversationType()),
          ...
          multimodalInputEnabled:
            isAiAssistanceMultimodalInputEnabled() &&
          ...
          uploadImageInputEnabled:
            isAiAssistanceMultimodalUploadInputEnabled() &&
            this.#conversation.type ===
              'freestyler' /* AiAssistanceModel.AiHistoryStorage.ConversationType.STYLING */,
          markdownRenderer,
          ...
          generateConversationSummary: async (markdown) => {
            if (!this.#conversationSummaryAgent) {
              this.#conversationSummaryAgent =
                new AiAssistanceModel.ConversationSummaryAgent.ConversationSummaryAgent({
          ...
          onContextRemoved: isAiAssistanceContextSelectionAgentEnabled()
            ? this.#handleContextRemoved.bind(this)
            : null,
          onContextAdd,
          walkthrough: {
  ...
  #updateWalkthroughResponsiveness() {
    ...
    if (isNarrow) {
      ...
      this.#walkthrough.inlineExpandedMessages = this.#walkthrough.activeSidebarMessage
        ? [this.#walkthrough.activeSidebarMessage]
        : [];
    } else {
      // Inline -> Wide: the last walkthrough that the user opened stays expanded
      this.#walkthrough.activeSidebarMessage =
        this.#walkthrough.inlineExpandedMessages.at(-1) ?? null;
    }
    this.requestUpdate();
  }
  ...
  #toggleWalkthrough(isOpen, message) {
    if (isOpen) {
      // If we are opening a walkthrough, ensure it's in our list of expanded messages.
      this.#openWalkthrough(message);
      return;
    }
    // If we are closing a walkthrough, remove it from the list of expanded messages.
    this.#walkthrough.inlineExpandedMessages = this.#walkthrough.inlineExpandedMessages.filter(
      (m) => m !== message,
    );
  ...
  #bindTimelineTraceListener() {
    const timelinePanel = UI.Context.Context.instance().flavor(
    ...
    this.#timelinePanelInstance?.removeEventListener(
      'IsViewingTrace' /* TimelinePanel.TimelinePanel.Events.IS_VIEWING_TRACE */,
      this.requestUpdate,
      this,
    );
  ...
  #getDefaultConversationType() {
    ...
    if (isElementsPanelVisible && hostConfig.devToolsFreestyler?.enabled) {
      ...
      targetConversationType =
        'drjones-network-request' /* AiAssistanceModel.AiHistoryStorage.ConversationType.NETWORK */;
    } else if (
      isSourcesPanelVisible &&
      this.#conversation?.type ===
        'breakpoint' /* AiAssistanceModel.AiHistoryStorage.ConversationType.BREAKPOINT */
    ) {
      targetConversationType =
        'breakpoint' /* AiAssistanceModel.AiHistoryStorage.ConversationType.BREAKPOINT */;
    } else if (isSourcesPanelVisible && hostConfig.devToolsAiAssistanceFileAgent?.enabled) {
      targetConversationType =
        'drjones-file' /* AiAssistanceModel.AiHistoryStorage.ConversationType.FILE */;
    } else if (
      isPerformancePanelVisible &&
      hostConfig.devToolsAiAssistancePerformanceAgent?.enabled
    ) {
      targetConversationType =
        'drjones-performance-full' /* AiAssistanceModel.AiHistoryStorage.ConversationType.PERFORMANCE */;
    } else if (
      isLighthousePanelVisible &&
      hostConfig.devToolsAiAssistanceAccessibilityAgent?.enabled
    ) {
      targetConversationType =
        'accessibility' /* AiAssistanceModel.AiHistoryStorage.ConversationType.ACCESSIBILITY */;
    }
  ...
  async handleBreakpointConversation(uiLocation, errorMsg) {
    ...
    const prompt = errorMsg
      ? `debug the error "${errorMsg}" using breakpoint debugging agent`
  ...
  wasShown() {
    ...
    this.#selectedElement = createNodeContext(
      selectedElementFilter(UI.Context.Context.instance().flavor(SDK.DOMModel.DOMNode)),
    );
    this.#selectedRequest = createRequestContext(
      UI.Context.Context.instance().flavor(SDK.NetworkRequest.NetworkRequest),
    );
    this.#selectedPerformanceTrace = createPerformanceTraceContext(
      UI.Context.Context.instance().flavor(AiAssistanceModel.AIContext.AgentFocus),
    );
    this.#selectedFile = createFileContext(
      UI.Context.Context.instance().flavor(Workspace.UISourceCode.UISourceCode),
    );
    this.#selectedBreakpoint = createBreakpointContext(
      UI.Context.Context.instance().flavor(Workspace.UISourceCode.UILocation),
    );
    ...
    Host.AidaClient.HostConfigTracker.instance().addEventListener(
      'aidaAvailabilityChanged' /* Host.AidaClient.Events.AIDA_AVAILABILITY_CHANGED */,
      this.#handleAidaAvailabilityChange,
    );
    this.#toggleSearchElementAction?.addEventListener(
      'Toggled' /* UI.ActionRegistration.Events.TOGGLED */,
      this.requestUpdate,
      this,
    );
    UI.Context.Context.instance().addFlavorChangeListener(
      SDK.DOMModel.DOMNode,
      this.#handleDOMNodeFlavorChange,
    );
    UI.Context.Context.instance().addFlavorChangeListener(
  ...
  #handleNetworkRequestFlavorChange = (ev) => {
    ...
    if (Boolean(ev.data)) {
      const calculator =
        NetworkPanel.NetworkPanel.NetworkPanel.instance().networkLogView.timeCalculator();
      this.#selectedRequest = new AiAssistanceModel.NetworkAgent.RequestContext(
        ev.data,
        calculator,
      );
    } else {
  ...
  #handlePerformanceTraceFlavorChange = (ev) => {
    ...
    this.#selectedPerformanceTrace = Boolean(ev.data)
      ? new AiAssistanceModel.PerformanceAgent.PerformanceTraceContext(ev.data)
      : null;
    this.#updateConversationState(this.#conversation);
  };
  ...
  #handleBreakpointFlavorChange = (ev) => {
    ...
    this.#selectedBreakpoint = new AiAssistanceModel.BreakpointDebuggerAgent.BreakpointContext(
      newBreakpoint,
    );
    this.#updateConversationState(this.#conversation);
  };
  ...
  #shouldShowChatActions() {
    ...
    if (
      this.#aidaAvailability ===
        'no-account-email' /* Host.AidaClient.AidaAccessPreconditions.NO_ACCOUNT_EMAIL */ ||
      this.#aidaAvailability ===
        'sync-is-paused' /* Host.AidaClient.AidaAccessPreconditions.SYNC_IS_PAUSED */
    ) {
      return false;
    }
    return true;
  ...
  #getChatInputPlaceholder() {
    ...
    switch (this.#conversation.type) {
      ...
      case 'drjones-performance-full' /* AiAssistanceModel.AiHistoryStorage.ConversationType.PERFORMANCE */: {
        const perfPanel = UI.Context.Context.instance().flavor(
          TimelinePanel.TimelinePanel.TimelinePanel,
        );
  ...
  #getDisclaimerText() {
    ...
    const noLogging =
      Root.Runtime.hostConfig.aidaAvailability?.enterprisePolicyValue ===
    ...
    switch (this.#conversation.type) {
      ...
      case 'drjones-performance-full' /* AiAssistanceModel.AiHistoryStorage.ConversationType.PERFORMANCE */:
        if (noLogging) {
          return lockedString(
            UIStringsNotTranslate.inputDisclaimerForPerformanceEnterpriseNoLogging,
          );
        }
      ...
      case 'accessibility' /* AiAssistanceModel.AiHistoryStorage.ConversationType.ACCESSIBILITY */:
        if (noLogging) {
          return lockedString(
            UIStringsNotTranslate.inputDisclaimerForAccessibilityEnterpriseNoLogging,
          );
        }
  ...
  async handleAction(actionId, opts) {
    ...
    switch (actionId) {
      case 'freestyler.elements-floating-button': {
        Host.userMetrics.actionTaken(
          Host.UserMetrics.Action.AiAssistanceOpenedFromElementsPanelFloatingButton,
        );
        targetConversationType =
          'freestyler' /* AiAssistanceModel.AiHistoryStorage.ConversationType.STYLING */;
        break;
      ...
      case 'drjones.network-floating-button': {
        Host.userMetrics.actionTaken(
          Host.UserMetrics.Action.AiAssistanceOpenedFromNetworkPanelFloatingButton,
        );
        targetConversationType =
          'drjones-network-request' /* AiAssistanceModel.AiHistoryStorage.ConversationType.NETWORK */;
        break;
      ...
      case 'drjones.performance-panel-context': {
        Host.userMetrics.actionTaken(
          Host.UserMetrics.Action.AiAssistanceOpenedFromPerformancePanelCallTree,
        );
        targetConversationType =
          'drjones-performance-full' /* AiAssistanceModel.AiHistoryStorage.ConversationType.PERFORMANCE */;
        break;
      ...
      case 'drjones.sources-floating-button': {
        Host.userMetrics.actionTaken(
          Host.UserMetrics.Action.AiAssistanceOpenedFromSourcesPanelFloatingButton,
        );
        targetConversationType =
          'drjones-file' /* AiAssistanceModel.AiHistoryStorage.ConversationType.FILE */;
        break;
    ...
    if (
      !this.#conversation ||
      this.#conversation.type !== targetConversationType ||
  ...
  #populateHistoryMenu(contextMenu) {
    const historicalConversations = AiAssistanceModel.AiHistoryStorage.AiHistoryStorage.instance()
      .getHistory()
      .map((serializedConversation) =>
        AiAssistanceModel.AiConversation.AiConversation.fromSerializedConversation(
    ...
    for (const conversation of historicalConversations.reverse()) {
      ...
      contextMenu.defaultSection().appendCheckboxItem(
        ...
        {
          checked: this.#conversation?.id === conversation.id,
          jslogContext: 'freestyler.history-item',
        },
      );
    }
  ...
  #onDeleteClicked() {
    ...
    void AiAssistanceModel.AiHistoryStorage.AiHistoryStorage.instance().deleteHistoryEntry(
      this.#conversation.id,
    );
    this.#updateConversationState();
  ...
  async #handleInspectElement() {
    ...
    const result = new Promise((resolve) => {
      ...
      this.#runAbortController.signal.addEventListener(
        'abort',
        () => {
          resolve(null);
          removeListeners();
  ...
  async #startConversation(text, imageInput, multimodalInputType) {
    ...
    const greenDevEmulationEnabled =
      Greendev.Prototypes.instance().isEnabled('emulationCapabilities');
    let multimodalInput;
      ...
      for await (const data of items) {
        ...
        switch (data.type) {
          case 'user-query' /* AiAssistanceModel.AiAgent.ResponseType.USER_QUERY */: {
            ...
            const isSidebarWalkthroughOpen =
              this.#walkthrough.isExpanded && !this.#walkthrough.isInlined;
            if (
              isSidebarWalkthroughOpen ||
              (Greendev.Prototypes.instance().isEnabled('breakpointDebuggerAgent') &&
                this.#conversation?.type ===
                  'breakpoint') /* AiAssistanceModel.AiHistoryStorage.ConversationType.BREAKPOINT */
            ) {
          ...
          case 'answer' /* AiAssistanceModel.AiAgent.ResponseType.ANSWER */: {
            ...
            if (systemMessage.parts.length > 1) {
              ...
              if (
                firstPart.type === 'step' &&
                firstPart.step.isLoading &&
                !firstPart.step.thought &&
                !firstPart.step.code &&
                !firstPart.step.contextDetails
              ) {
        ...
        if (!this.#conversation?.isReadOnly) {
          ...
          if (
            data.type === 'context' /* AiAssistanceModel.AiAgent.ResponseType.CONTEXT */ ||
            ...
            case 'answer' /* AiAssistanceModel.AiAgent.ResponseType.ANSWER */: {
              if (!data.complete && !announcedAnswerLoading) {
                ...
                UI.ARIAUtils.LiveAnnouncer.status(
                  lockedString(UIStringsNotTranslate.answerLoading),
                );
  ...
  for (const part of message.parts) {
    if (part.type === 'answer') {
      ...
      if (step.contextDetails) {
        contentParts.push(
          AiAssistanceModel.AiConversation.generateContextDetailsMarkdown(step.contextDetails),
        );
      }
      if (step.thought) {
