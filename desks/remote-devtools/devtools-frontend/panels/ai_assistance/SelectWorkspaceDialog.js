  ...
  render(
    html`
      ...
      <div class="main-content">
        ...
        ${input.showAutomaticWorkspaceNudge
          ? html`
      ...
      ${hasFolders
        ? html`
            <ul
              role="listbox"
              aria-label=${lockedString(UIStringsNotTranslate.selectFolder)}
              aria-activedescendant=${input.folders.length > 0
                ? `option-${input.selectedIndex}`
                : ''}
            >
      ...
      <div class="buttons">
        <devtools-button
          ...
          >${lockedString(UIStringsNotTranslate.cancel)}</devtools-button
        >
        <devtools-button
          ...
          >${lockedString(UIStringsNotTranslate.addFolder)}</devtools-button
        >
        ${hasFolders
          ? html`
              <devtools-button
                ...
                >${lockedString(UIStringsNotTranslate.select)}</devtools-button
              >
            `
          : nothing}
      </div>
...
export class SelectWorkspaceDialog extends UI.Widget.VBox {
  ...
  constructor(options, view) {
    ...
    if (options.currentProject) {
      this.#selectedIndex = Math.max(
        0,
        this.#folders.findIndex((folder) => folder.project === options.currentProject),
      );
    }
  ...
  performUpdate() {
    ...
    const viewInput = {
      ...
      selectProjectRootText: noLogging
      ...
      showAutomaticWorkspaceNudge:
        this.#automaticFileSystemManager.automaticFileSystem === null &&
  ...
  #updateProjectsAndFolders() {
    ...
    for (const project of projects) {
      ...
      if (
        automaticFileSystem &&
        project === this.#workspace.projectForFileSystemRoot(automaticFileSystem.root)
      ) {
  ...
  #onProjectAdded(event) {
    ...
    if (
      automaticFileSystem &&
      addedProject === this.#workspace.projectForFileSystemRoot(automaticFileSystem.root)
    ) {
      this.#dialog.hide();
  ...
  #onProjectRemoved() {
    const selectedProject =
      this.#selectedIndex >= 0 && this.#selectedIndex < this.#folders.length
    ...
    if (selectedProject) {
      ...
      this.#selectedIndex =
        projectIndex === -1
          ? Math.min(this.#folders.length - 1, this.#selectedIndex)
          : projectIndex;
    } else {
  ...
  static show(onProjectSelected, currentProject) {
    ...
    dialog.setSizeBehavior(
      'SetExactWidthMaxHeight' /* UI.GlassPane.SizeBehavior.SET_EXACT_WIDTH_MAX_HEIGHT */,
    );
    dialog.setDimmed(true);
    new SelectWorkspaceDialog({ dialog, onProjectSelected, currentProject }).show(
