...
var J = (t, e) => {
  for (var s in e) Pi(t, s, { get: e[s], enumerable: !0 });
};
import './../../ui/kit/kit.js';
import * as F from './../../core/common/common.js';
import * as w from './../../core/host/host.js';
import * as At from './../../core/i18n/i18n.js';
import * as Li from './../../core/platform/platform.js';
import * as R from './../../core/root/root.js';
import * as u from './../../core/sdk/sdk.js';
import * as g from './../../models/ai_assistance/ai_assistance.js';
import * as Xt from './../../models/annotations/annotations.js';
import * as St from './../../models/badges/badges.js';
import * as pe from './../../models/greendev/greendev.js';
import * as Te from './../../models/workspace/workspace.js';
import './../../ui/components/buttons/buttons.js';
import * as Mi from './../../ui/components/snackbars/snackbars.js';
import * as Ri from './../../ui/helpers/helpers.js';
import * as h from './../../ui/legacy/legacy.js';
import * as O from './../../ui/lit/lit.js';
import * as Ye from './../../ui/visual_logging/visual_logging.js';
import * as Ke from './../lighthouse/lighthouse.js';
import * as Ei from './../network/forward/forward.js';
import * as me from './../network/network.js';
import * as Ie from './../timeline/timeline.js';
var es = `.toolbar-container{display:flex;flex-wrap:wrap;background-color:var(--sys-color-cdt-base-container);border-bottom:1px solid var(--sys-color-divider);flex:0 0 auto;justify-content:space-between}.ai-assistance-view-container{display:flex;flex-direction:column;width:100%;height:100%;align-items:center;overflow:hidden;& .fill-panel{width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center}devtools-split-view{width:100%;height:100%}}.toolbar-feedback-link{color:var(--sys-color-primary);margin:0 var(--sys-size-3);height:auto;font-size:var(--sys-typescale-body4-size)}
/*# sourceURL=${import.meta.resolve('././aiAssistancePanel.css')} */`;
import * as ue from './../../core/sdk/sdk.js';
import * as Je from './../../ui/lit/lit.js';
import * as $t from './../common/common.js';
import * as ts from './../../core/common/common.js';
import * as ss from './../../core/platform/platform.js';
import * as It from './../../models/ai_assistance/ai_assistance.js';
import * as is from './../../models/logs/logs.js';
import * as os from './../../ui/components/markdown_view/markdown_view.js';
import * as Di from './../../ui/lit/lit.js';
var { html: Tt } = Di,
  V = class extends os.MarkdownView.MarkdownInsightRenderer {
    #o(e, s) {
      return Tt`<devtools-link @click=${(i) => {
        (i.preventDefault(), i.stopPropagation(), ts.Revealer.reveal(e));
      }}>${ss.StringUtilities.trimEndWithMaxLength(s, 100)}</devtools-link>`;
    }
    #t(e, s) {
      if (e.startsWith('#req-')) {
        let i = is.NetworkLog.NetworkLog.instance()
          .requests()
          .find((o) => o.requestId() === e.substring(5));
        return i ? this.#o(i, i.url()) : Tt`${s}`;
      }
      if (e.startsWith('#file-')) {
        let i = It.ContextSelectionAgent.ContextSelectionAgent.getUISourceCodes().find(
          (o) =>
            It.ContextSelectionAgent.ContextSelectionAgent.uiSourceCodeId.get(o) ===
            Number(e.substring(6)),
        );
        return i ? this.#o(i, i.name()) : Tt`${s}`;
      }
      return null;
    }
    templateForToken(e) {
      if (e.type === 'link') {
        let s = this.#t(e.href, e.text);
        if (s) return s;
      }
      if (e.type === 'code') {
        let s = e.text.split(`
`);
        s[0]?.trim() === 'css' &&
          ((e.lang = 'css'),
          (e.text = s.slice(1).join(`
`)));
      }
      if (e.type === 'codespan') {
        let s = e.text.match(/^\[(.*)\]\((.+)\)$/);
        if (s?.[2]) {
          let i = this.#t(s[2], s[1]);
          if (i) return i;
        }
      }
      return super.templateForToken(e);
    }
  };
var { html: ns } = Je.StaticHtml,
  { until: as } = Je.Directives,
  Qe = class extends V {
    mainFrameId;
    constructor(e = '') {
      (super(), (this.mainFrameId = e));
    }
    templateForToken(e) {
      if (e.type === 'link' && e.href.startsWith('#')) {
        if (e.href.startsWith('#path-')) {
          let i = e.href.replace('#path-', '');
          return ns`<span>${as(
            this.#t(i, e.text).then((o) => o || e.text),
            e.text,
          )}</span>`;
        }
        let s;
        if (
          (e.href.startsWith('#node-')
            ? (s = Number(e.href.replace('#node-', '')))
            : e.href.startsWith('#') && (s = Number(e.href.replace('#', ''))),
          s)
        )
          return ns`<span>${as(
            this.#o(s, e.text).then((i) => i || e.text),
            e.text,
          )}</span>`;
      }
      return super.templateForToken(e);
    ...
    async #o(e, s) {
      if (e === void 0) return;
      let o = ue.TargetManager.TargetManager.instance()
        .primaryPageTarget()
        ?.model(ue.DOMModel.DOMModel);
      if (!o) return;
      let r = (await o.pushNodesByBackendIdsToFrontend(new Set([e])))?.get(e);
      return !r || r.frameId() !== this.mainFrameId
        ? void 0
        : $t.DOMLinkifier.Linkifier.instance().linkify(r, { textContent: s });
    }
    async #t(e, s) {
      let o = ue.TargetManager.TargetManager.instance()
        .primaryPageTarget()
        ?.model(ue.DOMModel.DOMModel);
      if (!o) return;
      let n = await o.pushNodeByPathToFrontend(e);
      if (!n) return;
      let r = o.nodeForId(n);
      return r ? $t.DOMLinkifier.Linkifier.instance().linkify(r, { textContent: s }) : void 0;
    }
  };
import './../../ui/components/spinners/spinners.js';
import * as ft from './../../core/host/host.js';
import * as ei from './../../core/i18n/i18n.js';
import * as ti from './../../core/root/root.js';
import * as Bt from './../../models/ai_assistance/ai_assistance.js';
import './../../ui/components/buttons/buttons.js';
import * as si from './../../ui/legacy/legacy.js';
import { Directives as Ao, html as ke, nothing as So, render as To } from './../../ui/lit/lit.js';
var fs = {};
J(fs, {
  PatchSuggestionState: () => $,
  PatchWidget: () => Re,
  isAiAssistancePatchingEnabled: () => de,
});
...
import * as gs from './../../core/i18n/i18n.js';
import * as ms from './../../core/platform/platform.js';
import * as st from './../../core/root/root.js';
import * as Pe from './../../models/ai_assistance/ai_assistance.js';
import * as ps from './../../models/greendev/greendev.js';
import * as K from './../../models/persistence/persistence.js';
import * as G from './../../models/workspace/workspace.js';
import * as Rt from './../../models/workspace_diff/workspace_diff.js';
import './../../ui/components/buttons/buttons.js';
import * as ve from './../../ui/legacy/legacy.js';
import { Directives as tt, html as L, nothing as Z, render as Ni } from './../../ui/lit/lit.js';
import * as fe from './../../ui/visual_logging/visual_logging.js';
import * as us from './../changes/changes.js';
import * as it from './../common/common.js';
import './../../ui/kit/kit.js';
import * as Xe from './../../core/common/common.js';
import * as ls from './../../core/host/host.js';
import * as cs from './../../core/i18n/i18n.js';
import * as Mt from './../../core/root/root.js';
import * as ds from './../../models/geometry/geometry.js';
import * as X from './../../models/persistence/persistence.js';
import * as le from './../../models/workspace/workspace.js';
import './../../ui/components/buttons/buttons.js';
import * as et from './../../ui/legacy/legacy.js';
import { html as Me, nothing as Lt, render as Fi } from './../../ui/lit/lit.js';
var rs = `@scope to (devtools-widget > *){:scope{width:100%;box-shadow:none}.dialog-header{margin:var(--sys-size-6) var(--sys-size-8) var(--sys-size-5);font:var(--sys-typescale-headline5)}.buttons{margin:var(--sys-size-6) var(--sys-size-8) var(--sys-size-8);display:flex;justify-content:flex-start;gap:var(--sys-size-5)}.main-content{color:var(--sys-color-on-surface-subtle);margin:0 var(--sys-size-8);line-height:18px}.add-folder-button{margin-left:auto}ul{list-style-type:none;padding:0;margin:var(--sys-size-6) 0 var(--sys-size-4) 0;max-height:var(--sys-size-20);overflow-y:auto}li{display:flex;align-items:center;color:var(--sys-color-on-surface-subtle);border-radius:0 var(--sys-shape-corner-full) var(--sys-shape-corner-full) 0;height:var(--sys-size-10);margin:0 var(--sys-size-8);padding-left:var(--sys-size-9)}li:hover, li.selected{background-color:var(--sys-color-state-hover-on-subtle)}li:focus{background-color:var(--app-color-navigation-drawer-background-selected)}.folder-icon{color:var(--icon-file-default);margin-right:var(--sys-size-4)}li.selected .folder-icon{color:var(--icon-file-authored)}.select-project-root{margin-bottom:var(--sys-size-6)}.theme-with-dark-background, :host-context(.theme-with-dark-background){li:focus{color:var(--app-color-navigation-drawer-label-selected);background-color:var(--app-color-navigation-drawer-background-selected);& .folder-icon{color:var(--app-color-navigation-drawer-label-selected)}}}.ellipsis{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}
...
var N = {
  ...
  B = cs.i18n.lockedString,
  Ui = (t, e, s) => {
    let i = t.folders.length > 0;
    Fi(
      Me`
      <style>${rs}</style>
  ...
  Ze = class t extends et.Widget.VBox {
    #o;
    #t = le.Workspace.WorkspaceImpl.instance();
    #s = 0;
    #a;
    #n;
    #r = X.AutomaticFileSystemManager.AutomaticFileSystemManager.instance();
    #i = [];
    constructor(e, s) {
      (super(),
        (this.#a = e.onProjectSelected),
        (this.#n = e.dialog),
        this.#g(),
        e.currentProject &&
          (this.#s = Math.max(
            0,
            this.#i.findIndex((i) => i.project === e.currentProject),
          )),
        (this.#o = s ?? Ui),
        this.requestUpdate(),
    ...
    wasShown() {
      (super.wasShown(),
        this.#t.addEventListener(le.Workspace.Events.ProjectAdded, this.#h, this),
        this.#t.addEventListener(le.Workspace.Events.ProjectRemoved, this.#d, this));
    }
    willHide() {
      (super.willHide(),
        this.#t.removeEventListener(le.Workspace.Events.ProjectAdded, this.#h, this),
        this.#t.removeEventListener(le.Workspace.Events.ProjectRemoved, this.#d, this));
    }
    #m(e) {
      switch (e.key) {
        case 'ArrowDown': {
          (e.preventDefault(), (this.#s = Math.min(this.#s + 1, this.#i.length - 1)));
          let s = this.contentElement.querySelectorAll('li')[this.#s];
        ...
        case 'ArrowUp': {
          (e.preventDefault(), (this.#s = Math.max(this.#s - 1, 0)));
          let s = this.contentElement.querySelectorAll('li')[this.#s];
          (s?.scrollIntoView({ block: 'nearest', inline: 'nearest' }),
            s?.focus({ preventScroll: !0 }),
            this.requestUpdate());
    ...
    #e() {
      let e = this.#i[this.#s];
      e.project ? (this.#n.hide(), this.#a(e.project)) : this.#c();
    }
    performUpdate() {
      let e =
          Mt.Runtime.hostConfig.aidaAvailability?.enterprisePolicyValue ===
          Mt.Runtime.GenAiEnterprisePolicyValue.ALLOW_WITHOUT_LOGGING,
        s = {
          folders: this.#i,
          selectedIndex: this.#s,
          selectProjectRootText: B(e ? N.selectProjectRootNoLogging : N.selectProjectRoot),
          showAutomaticWorkspaceNudge:
            this.#r.automaticFileSystem === null && this.#r.availability === 'available',
          onProjectSelected: (i) => {
            ((this.#s = i), this.requestUpdate());
          },
          onSelectButtonClick: this.#e.bind(this),
          onCancelButtonClick: () => {
            this.#n.hide();
          },
          ...
          onListItemKeyDown: this.#m.bind(this),
        };
      this.#o(s, void 0, this.contentElement);
    }
    async #p() {
      (await X.IsolatedFileSystemManager.IsolatedFileSystemManager.instance().addFileSystem(),
        this.contentElement
    ...
    async #c() {
      (await this.#r.connectAutomaticFileSystem(!0)) || this.#n.hide();
    }
    #g() {
      this.#i = [];
      let e = this.#r.automaticFileSystem;
      e &&
        this.#i.push({
          name: Xe.ParsedURL.ParsedURL.extractName(e.root),
          path: e.root,
          automaticFileSystem: e,
        });
      let s = this.#t
        .projectsForType(le.Workspace.projectTypes.FileSystem)
        .filter(
          (i) =>
            i instanceof X.FileSystemWorkspaceBinding.FileSystem &&
            i.fileSystem().type() === X.PlatformFileSystem.PlatformFileSystemType.WORKSPACE_PROJECT,
        );
      for (let i of s) {
        if (e && i === this.#t.projectForFileSystemRoot(e.root)) {
          this.#i[0].project = i;
          continue;
        }
        this.#i.push({
          name: Xe.ParsedURL.ParsedURL.encodedPathToRawPathString(i.displayName()),
          path: Xe.ParsedURL.ParsedURL.urlToRawPathString(i.id(), ls.Platform.isWin()),
          project: i,
        });
      }
    }
    #h(e) {
      let s = e.data,
        i = this.#r.automaticFileSystem;
      if (i && s === this.#t.projectForFileSystemRoot(i.root)) {
        (this.#n.hide(), this.#a(s));
        return;
      }
      this.#g();
      let o = this.#i.findIndex((n) => n.project === s);
      (o !== -1 && (this.#s = o),
        this.requestUpdate(),
        this.updateComplete.then(() => {
    ...
    #d() {
      let e = this.#s >= 0 && this.#s < this.#i.length ? this.#i[this.#s].project : null;
      if ((this.#g(), e)) {
        let s = this.#i.findIndex((i) => i.project === e);
        this.#s = s === -1 ? Math.min(this.#i.length - 1, this.#s) : s;
      } else this.#s = 0;
      this.requestUpdate();
    }
    static show(e, s) {
      let i = new et.Dialog.Dialog('select-workspace');
      (i.setAriaLabel(N.selectFolderAccessibleLabel),
        i.setMaxContentSize(new ds.Size(384, 340)),
        i.setSizeBehavior('SetExactWidthMaxHeight'),
        i.setDimmed(!0),
        new t({ dialog: i, onProjectSelected: e, currentProject: s }).show(i.contentElement),
        i.show());
    }
  };
var C = {
    ...
    applyToWorkspaceTooltip:
      'Source code from the selected folder is sent to Google to generate code suggestions.',
  ...
  A = gs.i18n.lockedString,
  zi = 'https://support.google.com/legal/answer/13505487',
  { widget: hs } = ve.Widget,
  $;
(function (t) {
  ((t.INITIAL = 'initial'), (t.LOADING = 'loading'), (t.SUCCESS = 'success'), (t.ERROR = 'error'));
})($ || ($ = {}));
var ee;
(function (t) {
  ((t.NONE = 'none'),
    (t.REGULAR = 'regular'),
    (t.AUTOMATIC_DISCONNECTED = 'automaticDisconnected'),
    (t.AUTOMATIC_CONNECTED = 'automaticConnected'));
})(ee || (ee = {}));
var ji = (t, e, s) => {
    if (!t.changeSummary && t.patchSuggestionState === $.INITIAL) return;
    ((e.changeRef = e.changeRef ?? tt.createRef()),
      (e.summaryRef = e.summaryRef ?? tt.createRef()));
    function i() {
      return t.sources
        ? L`<devtools-link
          class="link"
    ...
    function o() {
      return t.savedToDisk
        ? L`
          ...
          `
        : t.patchSuggestionState === $.SUCCESS
          ? L`
    ...
    function n() {
      return (!t.changeSummary && t.patchSuggestionState === $.INITIAL) || t.savedToDisk
        ? Z
        : t.patchSuggestionState === $.SUCCESS
          ? L`${hs(us.CombinedDiffView.CombinedDiffView, { workspaceDiff: t.workspaceDiff, ignoredUrls: ['inspector://'] })}`
          : L`<devtools-code-block
          .code=${t.changeSummary ?? ''}
          .codeLang=${'css'}
          .displayNotice=${!0}
        ></devtools-code-block>
        ${
          t.patchSuggestionState === $.ERROR
    ...
    function r(f) {
      return !ps.Prototypes.instance().isEnabled('copyToGemini') || !f
        ? Z
        : L`<devtools-widget class="copy-to-prompt"
      ${hs(it.CopyChangesToPrompt, { workspaceDiff: t.workspaceDiff, patchAgentCSSChange: f })}></devtools-widget>`;
    }
    function c() {
      if (t.savedToDisk) return Z;
      if (t.patchSuggestionState === $.SUCCESS)
        return L`
          <div class="footer">
            <div class="left-side">
          ...
          `;
      let f =
        t.projectType === ee.AUTOMATIC_DISCONNECTED
          ? 'folder-off'
          : t.projectType === ee.AUTOMATIC_CONNECTED
            ? 'folder-asterisk'
            : 'folder';
      return L`
        <div class="footer">
          ...
          <div class="apply-to-workspace-container" aria-live="polite">
            ${
              t.patchSuggestionState === $.LOADING
                ? L`
  ...
  Re = class extends ve.Widget.Widget {
    changeSummary = '';
    changeManager;
    #o = ce.Settings.Settings.instance().createSetting('ai-assistance-patching-fre-completed', !1);
    #t = ce.Settings.Settings.instance().createSetting(
    ...
    #p;
    #c = $.INITIAL;
    #g = Rt.WorkspaceDiff.workspaceDiff();
    #h = G.Workspace.WorkspaceImpl.instance();
    #d = K.AutomaticFileSystemManager.AutomaticFileSystemManager.instance().automaticFileSystem;
    #u = !1;
    #f = null;
    constructor(e, s = ji, i) {
      (super(e),
        (this.#n = i?.aidaClient ?? new Ee.AidaClient.AidaClient()),
        (this.#p =
          st.Runtime.hostConfig.aidaAvailability?.enterprisePolicyValue ===
          st.Runtime.GenAiEnterprisePolicyValue.ALLOW_WITHOUT_LOGGING),
        (this.#s = s),
        this.requestUpdate());
    }
    #w() {
      ve.ViewManager.ViewManager.instance().showView('chrome-ai');
    }
    #v() {
      return this.#i
        ? {
            projectName: ce.ParsedURL.ParsedURL.encodedPathToRawPathString(this.#i.displayName()),
            projectPath: ce.ParsedURL.ParsedURL.urlToRawPathString(
              this.#i.id(),
              Ee.Platform.isWin(),
            ),
          }
        : this.#d
          ? {
              projectName: ce.ParsedURL.ParsedURL.extractName(this.#d.root),
              projectPath: this.#d.root,
            }
          : { projectName: '', projectPath: ms.DevToolsPath.EmptyRawPathString };
    }
    #b() {
      let e = this.#d ? this.#h.projectForFileSystemRoot(this.#d.root) : null;
      return (
        this.#h
          .projectsForType(G.Workspace.projectTypes.FileSystem)
          .filter(
            (i) =>
              i instanceof K.FileSystemWorkspaceBinding.FileSystem &&
              i.fileSystem().type() ===
                K.PlatformFileSystem.PlatformFileSystemType.WORKSPACE_PROJECT,
          )
          .filter((i) => i !== e).length > 0
      );
    }
    #C(e) {
      return this.#d && this.#d.root === e
        ? this.#i
          ? ee.AUTOMATIC_CONNECTED
          : ee.AUTOMATIC_DISCONNECTED
        : this.#i
          ? ee.NONE
          : ee.REGULAR;
    }
    performUpdate() {
      let { projectName: e, projectPath: s } = this.#v();
      this.#s(
        {
          workspaceDiff: this.#g,
          changeSummary: this.changeSummary,
          patchSuggestionState: this.#c,
          sources: this.#m,
          projectName: e,
          projectPath: s,
          projectType: this.#C(s),
          savedToDisk: this.#e,
          applyToWorkspaceTooltipText: this.#p
            ? A(C.applyToWorkspaceTooltipNoLogging)
            : A(C.applyToWorkspaceTooltip),
          onLearnMoreTooltipClick: this.#w.bind(this),
          onApplyToWorkspace: this.#F.bind(this),
          onCancel: () => {
            this.#r?.abort();
          },
          onDiscard: this.#S.bind(this),
          onSaveAll: this.#T.bind(this),
          onChangeWorkspaceClick: this.#b() ? this.#A.bind(this, { applyPatch: !1 }) : void 0,
        },
        this.#a,
        this.contentElement,
      );
    ...
    wasShown() {
      (super.wasShown(),
        this.#M(),
        de() &&
          (this.#h.addEventListener(G.Workspace.Events.ProjectAdded, this.#R, this),
          this.#h.addEventListener(G.Workspace.Events.ProjectRemoved, this.#E, this)));
    }
    willHide() {
      (super.willHide(),
        (this.#u = !1),
        de() &&
          (this.#h.removeEventListener(G.Workspace.Events.ProjectAdded, this.#R, this),
          this.#h.removeEventListener(G.Workspace.Events.ProjectRemoved, this.#E, this)));
    }
    async #l() {
      if (this.#o.get()) return !0;
      let s = Pe.AiUtils.getIconName(),
        i = await it.FreDialog.show({
          header: { iconName: s, text: A(C.freDisclaimerHeader) },
          reminderItems: [
            { iconName: 'psychiatry', content: A(C.freDisclaimerTextAiWontAlwaysGetItRight) },
            {
              iconName: 'google',
              content: this.#p
                ? A(C.freDisclaimerTextPrivacyNoLogging)
                : A(C.freDisclaimerTextPrivacy),
            },
            {
              iconName: 'warning',
              content: L`<devtools-link
            href=${zi}
            class="link devtools-link"
            jslogcontext="code-snippets-explainer.patch-widget"
          >${A(C.freDisclaimerTextUseWithCaution)}</devtools-link>`,
            },
          ],
          onLearnMoreClick: () => {
            ve.ViewManager.ViewManager.instance().showView('chrome-ai');
          },
          ariaLabel: A(C.freDisclaimerHeader),
          learnMoreButtonText: A(C.learnMore),
        });
      return (i && this.#o.set(!0), i);
    }
    #M() {
      let e = this.#d
        ? this.#h.projectForFileSystemRoot(this.#d.root)
        : this.#h.project(this.#t.get());
      (e ? (this.#i = e) : ((this.#i = void 0), this.#t.set('')), this.requestUpdate());
    }
    #R(e) {
      let s = e.data;
      this.#u && this.#d && s === this.#h.projectForFileSystemRoot(this.#d.root)
        ? ((this.#u = !1), (this.#i = s), this.#k())
        : this.#i === void 0 && this.#M();
    }
    #E() {
      this.#i &&
        !this.#h.project(this.#i.id()) &&
        (this.#t.set(''), (this.#i = void 0), this.requestUpdate());
    }
    #A(e = { applyPatch: !1 }) {
      let s = (i) => {
        ((this.#i = i),
          ...
          e.applyPatch
            ...
            : (this.requestUpdate(),
              this.updateComplete.then(() => {
                this.contentElement
                  ?.querySelector('.apply-to-workspace-container devtools-button')
                  ?.shadowRoot?.querySelector('button')
                  ?.focus();
              })));
      };
    ...
    async #F() {
      !de() ||
        ...
        (this.#i
          ...
          : this.#d
            ? ((this.#u = !0),
              await K.AutomaticFileSystemManager.AutomaticFileSystemManager.instance().connectAutomaticFileSystem(
                !0,
              ))
            : this.#A({ applyPatch: !0 }));
    }
    get #I() {
      return this.#g.modifiedUISourceCodes().filter((e) => !e.url().startsWith('inspector://'));
    }
    async #k() {
      let e = this.changeSummary;
      if (!e) throw new Error('Change summary does not exist');
      ((this.#c = $.LOADING), (this.#f = null), this.requestUpdate());
      let { response: s, processedFiles: i } = await this.#$(e);
      s && 'rpcId' in s && s.rpcId && (this.#f = s.rpcId);
      let o = this.#I.length > 0;
      (s?.type === 'answer' && o
        ? (this.#c = $.SUCCESS)
        : s?.type === 'error' && s.error === 'abort'
          ? (this.#c = $.INITIAL)
          : (this.#c = $.ERROR),
        (this.#m = `Filenames in ${this.#i?.displayName()}.
Files:
${i.map((n) => `* ${n}`).join(`
`)}`),
        this.requestUpdate(),
        this.#c === $.SUCCESS &&
          this.updateComplete.then(() => {
            this.#a.summaryRef?.value?.focus();
          }));
    }
    #S() {
      for (let e of this.#I) e.resetWorkingCopy();
      ((this.#c = $.INITIAL),
        (this.#m = void 0),
        this.changeManager?.popStashedChanges(),
        this.#x('NEGATIVE'),
        this.requestUpdate(),
        this.updateComplete.then(() => {
          this.#a.changeRef?.value?.focus();
        }));
    }
    #T() {
      for (let e of this.#I) e.commitWorkingCopy();
      (this.changeManager?.stashChanges().then(() => {
...
var {
    html: I,
    Directives: { createRef: Wi, ref: Oi },
  } = M,
  { widget: Vi } = te.Widget,
  De = {
    inputTextAriaDescription:
      'You can also use one of the suggested prompts above to start your conversation',
  ...
  Bi = nt.i18n.registerUIStrings('panels/ai_assistance/components/ChatInput.ts', De),
  ot = nt.i18n.getLocalizedString.bind(void 0, Bi),
  b = nt.i18n.lockedString,
  qi = 80,
  Hi = 'image/jpeg',
  ys = 100,
  _i = 'relevant-data-link-chat',
  Gi = 'relevant-data-link-footer';
function ws(t) {
  return t instanceof E.FileAgent.FileContext
    ? b(k.removeContextFile)
    : t instanceof E.StylingAgent.NodeContext
      ? b(k.removeContextElement)
      : t instanceof E.NetworkAgent.RequestContext
        ? b(k.removeContextRequest)
        : t instanceof E.PerformanceAgent.PerformanceTraceContext
          ? b(k.removeContextPerfInsight)
          : b(k.removeContext);
}
var ks = (t, e, s) => {
    let i = M.Directives.classMap({
        'chat-input-container': !0,
        'single-line-layout': !t.context,
        disabled: t.isTextInputDisabled,
      }),
      o = (n) => {
        let r = M.Directives.classMap({
          'chat-input-disclaimer': !0,
          'hide-divider': !t.isLoading && t.blockedByCrossOrigin,
        });
        return I`
      <div class=${r}>
        <button
          ...
          @click=${(c) => {
            (c.preventDefault(), te.ViewManager.ViewManager.instance().showView('chrome-ai'));
          }}
        ...
        ><div class="info-tooltip-container">
          ...
          <button
            ...
            jslog=${se.link('open-ai-settings').track({ click: !0 })}
            @click=${() => {
              te.ViewManager.ViewManager.instance().showView('chrome-ai');
            }}>${ot(De.learnAbout)}
          </button>
        ...
        <form class="input-form" @submit=${t.onSubmit}>
          <div class=${i}>
            ...
            <textarea
              ...
              @drop=${t.onImageDrop}
              @input=${(n) => {
                t.onTextInputChange(n.target.value);
              }}
                        ...
                        ${
                          t.context instanceof E.StylingAgent.NodeContext
                            ? I`
                              <devtools-widget
                                class="title"
                                ${Vi(bs.DOMLinkifier.DOMNodeLink, {
                                  node: t.context.getItem(),
                                  options: {
                                    disabled: !t.isContextSelected,
                                    hiddenClassList: t.context
                                      .getItem()
                                      .classNames()
                                      .filter((n) =>
                                        n.startsWith(E.Injected.AI_ASSISTANCE_CSS_CLASS_NAME),
                                      ),
                                    ariaDescription: ot(De.revealContextDescription),
                                  },
                                })}
                              ></devtools-widget>`
                            : I`
                          ${t.context instanceof E.NetworkAgent.RequestContext ? Et.PanelUtils.getIconForNetworkRequest(t.context.getItem()) : t.context instanceof E.FileAgent.FileContext ? Et.PanelUtils.getIconForSourceFile(t.context.getItem()) : t.context instanceof E.AccessibilityAgent.AccessibilityContext ? I`<devtools-icon class="icon" name="performance" title="Lighthouse"></devtools-icon>` : t.context instanceof E.PerformanceAgent.PerformanceTraceContext ? I`<devtools-icon class="icon" name="performance" title="Performance"></devtools-icon>` : M.nothing}
                            <span
                              ...
                              @keydown=${(n) => {
                                (n.key === 'Enter' || n.key === ' ') && t.onContextClick();
                              }}
                          ...
                          t.isContextSelected && t.onContextRemoved
                            ? I`
                                  <devtools-button
                                    ...
                                    .variant=${'icon'}
                                    @click=${t.onContextRemoved}></devtools-button>`
                            : M.nothing
                        }
                      ${
                        !t.isContextSelected && t.onContextAdd
                          ? I`
                                    <devtools-button
                                      ...
                                      .variant=${'icon'}
                                      @click=${t.onContextAdd}></devtools-button>`
                          : M.nothing
                      }
                      </div>
                    </div>`
  ...
  ye = class extends te.Widget.Widget {
    isLoading = !1;
    blockedByCrossOrigin = !1;
    isTextInputDisabled = !1;
    inputPlaceholder = '';
    context = null;
    isContextSelected = !1;
    inspectElementToggled = !1;
    disclaimerText = '';
    conversationType = 'freestyler';
    multimodalInputEnabled = !1;
    uploadImageInputEnabled = !1;
    isReadOnly = !1;
    #o = Wi();
    #t;
    setInputValue(e) {
      (this.#o.value && (this.#o.value.value = e), this.performUpdate());
    }
    #s() {
      return !this.#o.value?.value?.trim();
    }
    onTextSubmit = () => {};
    onContextClick = () => {};
    ...
    onContextAdd = null;
    async #a() {
      let e = z.TargetManager.TargetManager.instance().primaryPageTarget();
      if (!e) throw new Error('Could not find main target');
      let s = e.model(z.ScreenCaptureModel.ScreenCaptureModel);
      if (!s) throw new Error('Could not find model');
      let i = setTimeout(() => {
          ((this.#t = { isLoading: !0 }), this.performUpdate());
        }, ys),
        o = await s.captureScreenshot('jpeg', qi, 'fromViewport');
      (clearTimeout(i),
        o
          ? ((this.#t = { isLoading: !1, data: o, mimeType: Hi, inputType: 'screenshot' }),
            this.performUpdate(),
            this.updateComplete.then(() => {
              this.focusTextInput();
            }))
          : ((this.#t = void 0),
            this.performUpdate(),
            Pt.Snackbar.Snackbar.show({ message: b(k.screenshotFailureMessage) })));
    }
    targetAdded(e) {}
    targetRemoved(e) {}
    #n() {
    ...
    #r(e, s) {
      if (this.conversationType !== 'freestyler') return;
      let i = e?.files;
      if (!i || i.length === 0) return;
      let o = Array.from(i).find((n) => n.type.startsWith('image/'));
      o && (s.preventDefault(), this.#p(o));
    }
    #i = (e) => {
      this.#r(e.clipboardData, e);
    };
    #m = (e) => {
      this.conversationType === 'freestyler' && e.preventDefault();
    };
    #e = (e) => {
      this.#r(e.dataTransfer, e);
    };
    async #p(e) {
      let s = setTimeout(() => {
        ((this.#t = { isLoading: !0 }), this.performUpdate());
      ...
      try {
        let i = new FileReader(),
          o = await new Promise((c, a) => {
            ((i.onload = () => {
              typeof i.result == 'string'
                ? c(i.result)
                : a(new Error('FileReader result was not a string.'));
            }),
              i.readAsDataURL(e));
          }),
          n = o.indexOf(','),
          r = o.substring(n + 1);
        this.#t = { isLoading: !1, data: r, mimeType: e.type, inputType: 'uploaded-image' };
      } catch {
        ((this.#t = void 0),
          Pt.Snackbar.Snackbar.show({ message: b(k.uploadImageFailureMessage) }));
      }
      (clearTimeout(s),
        this.performUpdate(),
        this.updateComplete.then(() => {
    ...
    #g() {
      ((this.#t = void 0), this.performUpdate());
    }
    performUpdate() {
      this.#c(
        {
          ...
          onTakeScreenshot: this.#a.bind(this),
          onRemoveImageInput: this.#n.bind(this),
          onSubmit: this.onSubmit,
          onTextAreaKeyDown: this.onTextAreaKeyDown,
          onCancel: this.onCancel,
          onImageUpload: this.onImageUpload,
          onImageDragOver: this.#m,
          onImageDrop: this.#e,
    ...
    onSubmit = (e) => {
      if ((e.preventDefault(), this.#t?.isLoading)) return;
      let s =
        !this.#t?.isLoading && this.#t?.data
          ? { inlineData: { data: this.#t.data, mimeType: this.#t.mimeType } }
          : void 0;
      (this.onTextSubmit(this.#o.value?.value ?? '', s, this.#t?.inputType),
        (this.#t = void 0),
        this.setInputValue(''));
    };
    onTextAreaKeyDown = (e) => {
      if (
        ...
        if ((e.preventDefault(), !e.target?.value || this.#t?.isLoading)) return;
        let s =
          !this.#t?.isLoading && this.#t?.data
            ? { inlineData: { data: this.#t.data, mimeType: this.#t.mimeType } }
            : void 0;
        (this.onTextSubmit(e.target.value, s, this.#t?.inputType),
          (this.#t = void 0),
          this.setInputValue(''));
      }
    };
    onCancel = (e) => {
      (e.preventDefault(), this.isLoading && this.onCancelClick());
    };
    onImageUpload = (e) => {
      (e.stopPropagation(),
        te.UIUtils.createFileSelectorElement(this.#p.bind(this), '.jpeg,.jpg,.png').click());
    };
  };
var Bs = {};
J(Bs, {
...
var at = rt.i18n.lockedString,
  ...
  Qi = rt.i18n.registerUIStrings('panels/ai_assistance/components/WalkthroughView.ts', oe),
  Dt = rt.i18n.getLocalizedString.bind(void 0, Qi);
function lt(t) {
  return t.isLoading
    ? Ue(t.lastStep)
    : t.hasWidgets
      ? at(oe.showAgentWalkthrough)
      : at(oe.showThinking);
}
function ct(t) {
  return t.isInlined
    ? Dt(oe.title)
    : t.hasWidgets
      ? at(oe.hideAgentWalkthrough)
      : at(oe.hideThinking);
}
function Ji(t, e, s) {
  let i = s.at(-1);
  if (!t.isInlined || !i) return we.nothing;
  function o(c) {
    let a = c.target.open;
    t.message && (a ? t.onOpen(t.message) : t.onToggle(a, t.message));
  }
  let n = s.some((c) => c.widgets?.length),
    r = Is.AiUtils.getIconName();
  return ie`
    ...
    <div class="walkthrough-view">
      <div class="walkthrough-header">
         ...
         <devtools-button
          ...
          @click=${() => {
            t.message && t.onToggle(!1, t.message);
          }}
        ></devtools-button>
      </div>
      ${e}
...
var Ms = (t, e, s) => {
    let i = t.message?.parts.filter((r) => r.type === 'step')?.map((r) => r.step) ?? [],
      o = i.filter((r) => !r.requestApproval),
      n =
        o.length > 0
          ? ie`
    <div class="steps-container" @scroll=${t.handleScroll} ${Ss((r) => {
      e.scrollContainer = r;
    })}>
      <div class="steps-scroll-content" ${Ss((r) => {
        e.stepsContainer = r;
      })}>
        ${o.map(
          (r, c) => ie`
  ...
  he = class extends Ls.Widget.Widget {
    #o;
    #t = null;
    #s = !1;
    #a = null;
    ...
    #p = !1;
    #c = {};
    #g = new ResizeObserver(() => this.#u());
    #h = 0;
    constructor(e, s = Ms) {
      (super(e), (this.#o = s));
    }
    wasShown() {
      (super.wasShown(), this.#d());
    }
    willHide() {
      (super.willHide(), this.#g.disconnect());
    }
    #d() {
      this.#c.stepsContainer && this.#g.observe(this.#c.stepsContainer);
    }
    #u() {
      let e = this.#c.stepsContainer?.offsetWidth ?? 0;
      if (e !== this.#h) {
        this.#h = e;
        return;
      }
      !this.#e || !this.#s || this.scrollToBottom();
    }
    scrollToBottom() {
      this.#c.stepsContainer &&
        ((this.#p = !0),
        globalThis.requestAnimationFrame(() => {
          let e = this.#c.stepsContainer?.lastElementChild;
    ...
    #f = (e) => {
      if (!(!e.target || !(e.target instanceof HTMLElement))) {
        if (this.#p) {
          e.target.scrollTop + e.target.clientHeight + Ts >= e.target.scrollHeight &&
            (this.#p = !1);
          return;
        }
        this.#e = e.target.scrollTop + e.target.clientHeight + Ts >= e.target.scrollHeight;
      }
    };
    set isLoading(e) {
...
var {
    html: l,
    Directives: { ref: dt, ifDefined: Es },
  } = d,
  p = gt.i18n.lockedString,
  { widget: j } = zt.Widget,
  eo = 'https://crbug.com/364805393',
  Ps = 1,
  ht = 11,
  m = {
    ...
    maxStepsError:
      'Seems like I am stuck with the investigation. It would be better if you start over.',
  ...
  Ws = (t, e, s) => {
    let i = !!mt.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled,
      o = t.message;
    if (o.entity === 'user') {
      let a =
          o.imageInput && 'inlineData' in o.imageInput ? wo(o.imageInput.inlineData) : d.nothing,
        f = d.Directives.classMap({
          'chat-message': !0,
          query: !0,
          'is-last-message': t.isLastMessage,
          'is-first-message': t.isFirstMessage,
          'ai-v2': i,
        }),
        x = d.Directives.classMap({ 'user-query-wrapper': i });
      d.render(
        l`
    ...
    let n = o.parts.filter((a) => a.type === 'step').map((a) => a.step),
      r = ne.AiUtils.getIconName(),
      c = d.Directives.classMap({
        'chat-message': !0,
        answer: !0,
        'is-last-message': t.isLastMessage,
        'is-first-message': t.isFirstMessage,
    ...
    <section class=${c} jslog=${Y.section('answer')}>
      ...
      <div class="answer-body-wrapper">
        ${d.Directives.repeat(
          ...
          (a, f) => f,
          (a, f) => {
            let x = f === o.parts.length - 1;
            return a.type === 'answer'
              ? l`<p>${Ut(a.text, t.markdownRenderer, { animate: !t.isReadOnly && t.isLoading && x && t.isLastMessage })}</p>`
              : a.type === 'widget'
                ? l`${d.Directives.until(Vs(a.widgets, { wrapperClass: 'main-widgets-wrapper' }))}`
                : !i && a.type === 'step'
                  ? Ne({
                      step: a.step,
                      isLoading: t.isLoading,
                      markdownRenderer: t.markdownRenderer,
                      isLast: x,
                    })
                  : d.nothing;
          },
        ...
        ${yo(o)}
        ${
          t.isLastMessage && i && !t.isLoading && t.changeSummary
            ? l`
          <devtools-code-block
            .code=${t.changeSummary}
            .codeLang=${'css'}
...
function Ut(t, e, { animate: s, ref: i } = {}) {
  let o = [];
  try {
    o = Ns.Marked.lexer(t);
    for (let n of o) e.renderToken(n);
  } catch {
    return l`${t}`;
  }
  return l`<devtools-markdown-view
    .data=${{ tokens: o, renderer: e, animationEnabled: s }}
    ${i ? dt(i) : d.nothing}>
  </devtools-markdown-view>`;
}
function Ue(t) {
  return t.title ?? `${p(m.investigating)}\u2026`;
}
function to(t) {
  let e = t.requestApproval ? l`<span class="paused">${p(m.paused)}: </span>` : d.nothing;
  return l`<span class="title">${e}${Ue(t)}</span>`;
}
function so(t) {
  if (!t.code && !t.output) return d.nothing;
  let e = t.output && !t.canceled ? p(m.codeExecuted) : p(m.codeToExecute),
    s = t.code
      ? l`<div class="action-result">
      <devtools-code-block
        .code=${t.code.trim()}
        .codeLang=${'js'}
        .displayLimit=${ht}
    ...
    ></devtools-code-block>
      ...
      : d.nothing;
  return l`<div class="step-code">${s}${i}</div>`;
}
function io({ step: t, markdownRenderer: e, isLast: s }) {
  let i = s && t.requestApproval ? vo(t) : d.nothing,
    o = t.thought ? l`<p>${Ut(t.thought, e)}</p>` : d.nothing,
    n = t.contextDetails
      ? l`${d.Directives.repeat(
          t.contextDetails,
          (r) => l`<div class="context-details">
      <devtools-code-block
        .code=${r.text}
        .codeLang=${r.codeLang || ''}
        .displayLimit=${ht}
      ...
      : d.nothing;
  return l`<div class="step-details">
    ${o}
    ${so(t)}
    ${i}
    ${n}
...
function oo(t, e) {
  let { message: s, walkthrough: i } = t,
    o = e.at(-1);
  if (i.isInlined || !o) return d.nothing;
  let n = e.some((x) => x.widgets?.length),
    r = i.isExpanded && t.message === t.walkthrough.activeSidebarMessage,
    c = r ? ct({ hasWidgets: n }) : lt({ isLoading: t.isLoading, hasWidgets: n, lastStep: o }),
    a = n && !t.isLoading ? 'tonal' : 'text',
    f = ne.AiUtils.getIconName();
  return l`
    <div class="walkthrough-toggle-container ${n ? 'has-widgets' : ''}">
      ...
      <devtools-button
        ...
        @click=${() => {
          i.activeSidebarMessage === t.message && i.isExpanded ? i.onToggle(!1, s) : i.onOpen(s);
        }}
>
...
function no(t, e) {
  if (!e.at(-1)) return d.nothing;
  let i = e.filter((a) => a.requestApproval),
    o = t.walkthrough.isInlined ? d.nothing : oo(t, e),
    n = t.walkthrough.isInlined
      ? t.walkthrough.inlineExpandedMessages.includes(t.message)
      : t.walkthrough.isExpanded && t.walkthrough.activeSidebarMessage === t.message,
    r =
      i.length > 0
        ? i.map(
            (a) => l`
...
function ao({ step: t, isLoading: e, isLast: s }) {
  if (e && s && !t.requestApproval) return l`<devtools-spinner></devtools-spinner>`;
  let i = 'checkmark',
    o = p(m.completed),
    n = 'button';
  return (
    s && t.requestApproval
      ? ((n = void 0), (o = void 0), (i = 'pause-circle'))
      : t.canceled && ((o = p(m.canceled)), (i = 'cross')),
    l`<devtools-icon
      class="indicator"
      role=${Es(n)}
      aria-label=${Es(o)}
...
function Ne({ step: t, isLoading: e, markdownRenderer: s, isLast: i }) {
  let o = d.Directives.classMap({
    step: !0,
    empty: !t.thought && !t.code && !t.contextDetails && !t.requestApproval,
    paused: !!t.requestApproval,
    canceled: !!t.canceled,
  });
  return l`
    <details class=${o}
...
async function Os(t) {
  let e = Ds.get(t);
  if (e) return e;
  let s = be.TargetManager.TargetManager.instance().primaryPageTarget();
  if (!s) return null;
  let o = await new be.DOMModel.DeferredDOMNode(s, t).resolvePromise();
  return (o && Ds.set(t, o), o);
}
async function ro(t) {
  let e = await Os(t.data.backendNodeId);
  if (!e) return null;
  let s = new Us.ComputedStyleModel.ComputedStyle(e, t.data.computedStyles);
  return {
    renderedWidget: l`<devtools-widget
      class="computed-styles-widget" ${j(ae.ComputedStyleWidget.ComputedStyleWidget, { nodeStyle: s, matchedStyles: t.data.matchedCascade, propertyTraces: null, allowUserControl: !1, filterText: new RegExp(t.data.properties.join('|'), 'i'), enableNarrowViewResizing: !1 })}></devtools-widget>`,
    revealable: new ae.ElementsPanel.NodeComputedStyles(e),
    title: l`<devtools-widget
...
async function lo(t) {
  return {
    renderedWidget: l`<devtools-widget class="core-vitals-widget" ${j(pt.CWVMetrics.CWVMetrics, { data: t.data, skipBottomBorder: !0 })}>
  </devtools-widget>`,
    revealable: new je.Helpers.RevealableCoreVitals(t.data.insightSetKey),
    title: p(m.coreVitals),
  };
}
async function co(t) {
  let e = await Os(t.data.backendNodeId);
...
async function ho(t) {
  let e = t.data.lcpData;
  return e
    ? {
        renderedWidget: l`<devtools-widget
    class="lcp-breakdown-widget"
    ${j(js.LCPBreakdown.LCPBreakdown, { model: e, minimal: !0 })}></devtools-widget>`,
        revealable: new je.Helpers.RevealableInsight(e),
        title: p(m.lcpBreakdown),
      }
    : null;
}
async function go(t) {
  let e = ne.AIQueries.AIQueries.mainThreadActivityBottomUp(t.data.bounds, t.data.parsedTrace);
  if (!e) return null;
  let s = e.events,
    i = ge.Helpers.Timing.microToMilli(t.data.bounds.min),
    o = ge.Helpers.Timing.microToMilli(t.data.bounds.max);
  return {
    renderedWidget: l`<devtools-widget
      class="bottom-up-timeline-tree-widget"
      ${j(q.TimelineTreeView.BottomUpTimelineTreeView, { selectedEvents: s, parsedTrace: t.data.parsedTrace, startTime: i, endTime: o, compactMode: !0, maxLinkLength: 15, maxRows: 10 })}></devtools-widget>`,
    revealable: new je.Helpers.RevealableBottomUpProfile(t.data.bounds),
    title: p(m.bottomUpTree),
  };
}
function mo(t) {
  if (t === null) return d.nothing;
  function e() {
    t !== null && xe.Revealer.reveal(t?.revealable);
  }
  let s = d.Directives.classMap({
      'widget-and-revealer-container': !0,
      'revealer-only': t.renderedWidget === null,
    }),
    i = l`
...
async function po(t) {
  return {
    renderedWidget: null,
    title: null,
    revealable: new q.TimelinePanel.ParsedTraceRevealable(t.data.parsedTrace),
    customRevealTitle: p(m.revealTrace),
  };
}
function uo(t) {
  let e = t.url.split('/').pop() || t.url,
    s = gt.ByteUtilities.bytesToString(t.size),
    i = xe.ResourceType.resourceTypes[t.resourceType],
    { iconName: o, color: n } = Zi.iconDataForResourceType(i);
  return l`
...
async function fo(t) {
  let e = t.data.root;
  if (!(e instanceof be.DOMModel.DOMNodeSnapshot)) return null;
  let s = t.data.networkRequest;
  return {
    renderedWidget: l`
    ${s ? uo(s) : d.nothing}
  ...
  `,
    revealable: new be.DOMModel.DeferredDOMNode(e.domModel().target(), e.backendNodeId()),
    title: p(m.lcpElement),
  };
}
async function Vs(t, e = {}) {
  if (!mt.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled || !t || t.length === 0)
    return d.nothing;
    ...
    t.map(async (i) => {
      ...
      switch (i.name) {
        ...
        case 'BOTTOM_UP_TREE':
          ...
          break;
        default:
          Fs.assertNever(i, 'Unknown AiWidget name');
      }
      return mo(o);
    }),
  );
  return e.wrapperClass ? l`<div class=${e.wrapperClass}>${s}</div>` : l`${s}`;
}
function vo(t) {
  ...
  if (t.error) {
    ...
    switch (t.error) {
      ...
      case 'max-steps':
        ...
        break;
      case 'cross-origin':
        e = m.crossOriginError;
        break;
      case 'abort':
        return l`<p class="aborted" jslog=${Y.section('aborted')}>${p(m.stoppedResponse)}</p>`;
    }
    return l`<p class="error" jslog=${Y.section('error')}>${p(e)}</p>`;
  }
  return d.nothing;
}
function wo(t) {
  if (t.data === ne.AiConversation.NOT_FOUND_IMAGE_DATA)
    return l`<div class="unavailable-image" title=${m.imageUnavailable}>
      <devtools-icon name='file-image'></devtools-icon>
    </div>`;
  let e = `data:${t.mimeType};base64,${t.data}`;
  return l`<devtools-link
...
function bo(t, e) {
  let s = mt.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled,
    i = d.Directives.classMap({ 'ai-assistance-feedback-row': !0, 'not-v2': !s });
  return l`
    <div class=${i}>
      <div class="action-buttons">
        ...
        ${
          t.onExportClick && s && t.isLastMessage
            ? l`
          <devtools-button
      ...
      ${
        t.suggestions
          ? l`<div class="suggestions-container">
        <div class="scroll-button-container left hidden" ${dt((o) => {
          e.suggestionsLeftScrollButtonContainer = o;
        })}>
          <devtools-button
            class='scroll-button'
        ...
        <div class="suggestions-scroll-container" @scroll=${t.onSuggestionsScrollOrResize} ${dt(
          (o) => {
            e.suggestionsScrollContainer = o;
          },
        )}>
          ${t.suggestions.map(
            (o) => l`<devtools-button
        ...
        <div class="scroll-button-container right hidden" ${dt((o) => {
          e.suggestionsRightScrollButtonContainer = o;
        })}>
          <devtools-button
            class='scroll-button'
...
var ze = class extends zt.Widget.Widget {
  message = { entity: 'user', text: '' };
  isLoading = !1;
  isReadOnly = !1;
  canShowFeedbackForm = !1;
  isLastMessage = !1;
  isFirstMessage = !1;
  markdownRenderer;
  onSuggestionClick = () => {};
  ...
  #o = new ResizeObserver(() => this.#g());
  #t = new xe.Throttler.Throttler(100);
  #s = '';
  #a;
  #n = !1;
  #r = !0;
  ...
  wasShown() {
    (super.wasShown(), this.performUpdate(), this.#c());
  }
  performUpdate() {
    (this.#i(
      {
        ...
        onRatingClick: this.#d.bind(this),
        onReportClick: () => zs.openInNewTab(eo),
        onCopyResponseClick: () => {
          this.message.entity === 'model' && this.onCopyResponseClick(this.message);
        },
        onExportClick: this.onExportClick,
        scrollSuggestionsScrollContainer: this.#h.bind(this),
        onSuggestionsScrollOrResize: this.#g.bind(this),
        onSubmit: this.#f.bind(this),
        onClose: this.#u.bind(this),
        onInputChange: this.#p.bind(this),
        isSubmitButtonDisabled: this.#r,
        showActions: !(this.isLastMessage && this.isLoading),
        showRateButtons: this.message.entity === 'model' && !!this.message.rpcId,
        suggestions:
          this.isLastMessage &&
          this.message.entity === 'model' &&
          !this.isReadOnly &&
          this.message.parts.at(-1)?.type === 'answer'
            ? this.message.parts.at(-1).suggestions
            : void 0,
        currentRating: this.#a,
        isShowingFeedbackForm: this.#n,
      ...
      this.#m.suggestionsScrollContainer &&
        !this.#e &&
        (this.#o.observe(this.#m.suggestionsScrollContainer), (this.#e = !0)));
  }
  #p(e) {
    this.#s = e;
    let s = !e;
    s !== this.#r && ((this.#r = s), this.performUpdate());
  }
  #c = () => {
    let e = this.#m.suggestionsScrollContainer,
      s = this.#m.suggestionsLeftScrollButtonContainer,
      i = this.#m.suggestionsRightScrollButtonContainer;
    if (!e || !s || !i) return;
    let o = e.scrollLeft > Ps,
      n = e.scrollLeft + e.offsetWidth + Ps < e.scrollWidth;
    (s.classList.toggle('hidden', !o), i.classList.toggle('hidden', !n));
  };
  willHide() {
    (super.willHide(), this.#o.disconnect(), (this.#e = !1));
  }
  #g() {
    this.#t.schedule(() => (this.#c(), Promise.resolve()));
  }
  #h(e) {
    let s = this.#m.suggestionsScrollContainer;
    s &&
      s.scroll({
        top: 0,
        left: e === 'left' ? s.scrollLeft - s.clientWidth : s.scrollLeft + s.clientWidth,
        behavior: 'smooth',
      });
  }
  #d(e) {
    if (this.#a === e) {
      ((this.#a = void 0),
        ...
        this.message.entity === 'model' &&
          this.message.rpcId &&
          this.onFeedbackSubmit(this.message.rpcId, 'SENTIMENT_UNSPECIFIED'),
        this.performUpdate());
      return;
    }
    ((this.#a = e),
      (this.#n = this.canShowFeedbackForm),
      this.message.entity === 'model' &&
        this.message.rpcId &&
        this.onFeedbackSubmit(this.message.rpcId, e),
      this.performUpdate());
  }
  #u() {
    ((this.#n = !1), (this.#r = !0), this.performUpdate());
  }
  #f(e) {
    e.preventDefault();
    let s = this.#s;
    !this.#a ||
      ...
      (this.message.entity === 'model' &&
        this.message.rpcId &&
        this.onFeedbackSubmit(this.message.rpcId, this.#a, s),
      (this.#n = !1),
      (this.#r = !0),
      this.performUpdate());
  }
...
async function xo(t) {
  let { bounds: e, parsedTrace: s, track: i } = t.data,
    o = [];
  if (i === 'main') {
    let x = q.TimelinePanel.TimelinePanel.instance().getFlameChart().getMainDataProvider(),
      U = x.timelineData().groups.find(($e) => $e.name.startsWith('Main \u2014 '));
    U && (o = x.groupTreeEvents(U) ?? []);
  }
  let n = Array.from(o);
  n.sort((f, x) => f.ts - x.ts);
  let r = new q.ThirdPartyTreeView.ThirdPartyTreeViewWidget(),
    c = new ge.EntityMapper.EntityMapper(s);
  return (
    (r.model = { selectedEvents: n, parsedTrace: s, entityMapper: c }),
    (r.activeSelection = q.TimelineSelection.selectionFromRangeMicroSeconds(e.min, e.max)),
    r.refreshTree(!0),
    {
      renderedWidget: l`
    <devtools-widget
...
var { html: _s, render: ko } = Wt,
  ...
  Co = jt.i18n.registerUIStrings('panels/ai_assistance/components/ExportForAgentsDialog.ts', W),
  H = jt.i18n.getLocalizedString.bind(void 0, Co),
  Ys = (t, e, s) => {
    let i = t.state.activeType === 'prompt',
      o = H(i ? W.copyToClipboard : W.saveAsMarkdown),
      n =
        i && t.state.isPromptLoading
          ? H(W.generatingSummary)
          : i
            ? t.state.promptText
            : t.state.conversationText;
    ko(
      _s`
    <style>${Hs}</style>
  ...
  We = class t extends ut.Widget.VBox {
    #o;
    #t;
    #s;
    #a;
    constructor(e, s = Ys) {
      (super(),
        ...
        (this.#s = {
          ...
          conversationText: e.markdownText,
          isPromptLoading: typeof e.promptText != 'string',
        }),
        (this.#a = e.onConversationSaveAs),
        (this.#o = s),
        typeof e.promptText != 'string' &&
          e.promptText.then((i) => {
            ((this.#s.promptText = i), (this.#s.isPromptLoading = !1), this.requestUpdate());
          }),
        this.requestUpdate());
    }
    #n = (e) => {
      ((this.#s.activeType = e), this.requestUpdate());
    };
    performUpdate() {
      let e,
        s = '';
      switch (this.#s.activeType) {
        case 'prompt':
          ((s = 'ai-export-for-agents.copy-to-clipboard'),
            (e = (o) => {
              (o.preventDefault(),
                Gs.InspectorFrontendHost.InspectorFrontendHostInstance.copyText(this.#s.promptText),
                Ks.Snackbar.Snackbar.show({ message: H(W.copiedToClipboard) }),
                this.#t.hide());
            }));
          break;
        case 'conversation':
          ((s = 'ai-export-for-agents.save-as-markdown'),
            (e = () => {
              (this.#t.hide(), this.#a());
            }));
          break;
      }
      let i = { onButtonClick: e, state: this.#s, onStateChange: this.#n, jslogContext: s };
      this.#o(i, void 0, this.contentElement);
    }
    static show({ promptText: e, markdownText: s, onConversationSaveAs: i }) {
      let o = new ut.Dialog.Dialog();
      (o.setAriaLabel(H(W.exportForAgents)),
        o.setOutsideClickCallback((r) => {
          (r.consume(!0), o.hide());
        }),
        o.addCloseButton(),
        o.setSizeBehavior('MeasureContent'),
        o.setDimmed(!0));
      let n = new t({ dialog: o, promptText: e, markdownText: s, onConversationSaveAs: i });
      (n.show(o.contentElement),
        n.updateComplete.then(() => {
          o.show();
        }));
    }
  };
var { ref: Ot, repeat: Io, classMap: Js } = Ao,
  { widget: Vt } = si.Widget,
  Xs = { emptyStateText: 'How can I help you?', emptyStateTextGemini: 'Where should we start?' },
  Zs = ei.i18n.lockedString,
  $o = 1,
  Lo = (t, e, s) => {
    let i = !!ti.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled,
      o = Js({ 'chat-ui': !0, gemini: Bt.AiUtils.isGeminiBranding(), 'ai-v2': i }),
      n = Js({ 'chat-input-widget': !0, sticky: !t.isReadOnly }),
      r = !i && !t.isLoading;
    To(
      ke`
      <style>${qs}</style>
      <div class=${o}>
        <main @scroll=${t.handleScroll} ${Ot((c) => {
          e.mainElement = c;
        })}>
          ${
            t.messages.length > 0
          ...
          `
            ...
            <div class="empty-state-container">
              ...
              <div class="empty-state-content">
                ${t.emptyStateSuggestions.map(
                  ({ title: c, jslogContext: a }) => ke`<devtools-button
                    class="suggestion"
  ...
  Oe = class extends HTMLElement {
    ...
    #a;
    #n = {};
    #r = new ResizeObserver(() => this.#c());
    #i = !0;
    #m = !1;
    #e;
    #p = null;
    constructor(e, s = Lo) {
      (super(), (this.#s = e), (this.#e = s));
    }
    set props(e) {
      ((this.#s = e), this.#v());
    }
    connectedCallback() {
      (this.#v(), this.#a && this.#r.observe(this.#a));
    }
    disconnectedCallback() {
      this.#r.disconnect();
    }
    focusTextInput() {
      let e = this.#o.querySelector('.chat-input');
      e && e.focus();
    }
    restoreScrollPosition() {
      this.#t !== void 0 && this.#n.mainElement && this.#g(this.#t);
    }
    scrollToBottom() {
      this.#n.mainElement && this.#g(this.#n.mainElement.scrollHeight);
    }
    #c() {
      this.#i && this.#n.mainElement && this.#i && this.#g(this.#n.mainElement.scrollHeight);
    }
    #g(e) {
      this.#n.mainElement && ((this.#t = e), (this.#m = !0), (this.#n.mainElement.scrollTop = e));
    }
    #h = (e) => {
      ((this.#a = e), e ? this.#r.observe(e) : ((this.#i = !0), this.#r.disconnect()));
    };
    #d = (e) => {
      if (!(!e.target || !(e.target instanceof HTMLElement))) {
        if (this.#m) {
        ...
        ((this.#t = e.target.scrollTop),
          (this.#i = e.target.scrollTop + e.target.clientHeight + $o > e.target.scrollHeight));
      }
    };
    #u = (e) => {
      (this.#n.input?.getWidget()?.setInputValue(e),
        this.#v(),
        this.focusTextInput(),
        ft.userMetrics.actionTaken(ft.UserMetrics.Action.AiAssistanceDynamicSuggestionClicked));
    };
    async #f() {
      if (this.#p?.markdown === this.#s.conversationMarkdown) return this.#p.summary;
      try {
        let e = await this.#s.generateConversationSummary(this.#s.conversationMarkdown);
        return ((this.#p = { markdown: this.#s.conversationMarkdown, summary: e }), e);
      } catch (e) {
        return (console.error(e), 'Failed to generate summary.');
      }
    }
    async #w() {
      let e = this.#f();
      We.show({
        promptText: e,
        markdownText: this.#s.conversationMarkdown,
        onConversationSaveAs: this.#s.onExportConversation ?? (async () => {}),
      });
    }
    #v() {
      this.#e(
        {
          ...this.#s,
          handleScroll: this.#d,
          handleSuggestionClick: this.#u,
          handleMessageContainerRef: this.#h,
          exportForAgentsClick: this.#w.bind(this),
        },
        this.#n,
        this.#o,
      );
...
var Q = {
    ...
    turnOnForStylesRequestsAndFiles:
      'Turn on {PH1} to get help with styles, network requests, and files',
  ...
  Be = qt.i18n.registerUIStrings('panels/ai_assistance/components/DisabledWidget.ts', Q),
  vt = qt.i18n.getLocalizedString.bind(void 0, Be);
function Ro(t) {
  switch (t) {
    case 'no-account-email':
    case 'sync-is-paused':
      return qe`${vt(Q.notLoggedIn)}`;
    case 'no-internet':
      return qe`${vt(Q.offline)}`;
  }
}
function Eo(t) {
  if (t.isOffTheRecord) return qe`${vt(Q.notAvailableInIncognitoMode)}`;
  let e = document.createElement('span');
  ((e.textContent = vt(Q.settingsLink)),
    e.classList.add('link'),
    Ce.ARIAUtils.markAsLink(e),
    e.addEventListener('click', () => {
      Ce.ViewManager.ViewManager.instance().showView('chrome-ai');
    }),
    e.setAttribute('jslog', `${ni.action('open-ai-settings').track({ click: !0 })}`));
  let s;
  return (
    t.devToolsAiAssistancePerformanceAgent?.enabled
      ? (s = Ve.getFormatLocalizedString(Be, Q.turnOnForStylesRequestsPerformanceAndFiles, {
          PH1: e,
        }))
      : t.devToolsAiAssistanceFileAgent?.enabled
        ? (s = Ve.getFormatLocalizedString(Be, Q.turnOnForStylesRequestsAndFiles, { PH1: e }))
        : t.devToolsAiAssistanceNetworkAgent?.enabled
          ? (s = Ve.getFormatLocalizedString(Be, Q.turnOnForStylesAndRequests, { PH1: e }))
          : (s = Ve.getFormatLocalizedString(Be, Q.turnOnForStyles, { PH1: e })),
    qe`${s}`
  );
}
var ai = (t, e, s) => {
  ...
  He = class extends Ce.Widget.Widget {
    ...
    performUpdate() {
      let e = oi.Runtime.hostConfig;
      this.#o({ aidaAvailability: this.aidaAvailability, hostConfig: e }, {}, this.contentElement);
    }
  };
var pi = {};
J(pi, { DEFAULT_VIEW: () => mi, ExploreWidget: () => _e });
import * as hi from './../../core/i18n/i18n.js';
import * as gi from './../../core/root/root.js';
import * as P from './../../ui/legacy/legacy.js';
import { html as Ht, render as Po } from './../../ui/lit/lit.js';
import * as _t from './../../ui/visual_logging/visual_logging.js';
var li = `@scope to (devtools-widget > *){.ai-assistance-explore-container{&,
...
var ci = { Explore: 'Explore AI assistance', learnAbout: 'Learn about AI in DevTools' },
  di = hi.i18n.lockedString,
  mi = (t, e, s) => {
    function i(o) {
      return Ht`Open
     <button
        ...
        <div class="header">
          ...
          <p>
            ...
            <button
              ...
              @click=${() => {
                P.ViewManager.ViewManager.instance().showView('chrome-ai');
              }}
            >${di(ci.learnAbout)}
            </button>
          </p>
  ...
  _e = class extends P.Widget.Widget {
    #o;
    constructor(e, s = mi) {
      (super(e), (this.#o = s));
    }
    ...
    performUpdate() {
      let e = gi.Runtime.hostConfig,
        s = [];
      (e.devToolsFreestyler?.enabled &&
        P.ViewManager.ViewManager.instance().hasView('elements') &&
        s.push({
          icon: 'brush-2',
          heading: 'CSS styles',
          jslogContext: 'open-elements-panel',
          onClick: () => {
            P.ViewManager.ViewManager.instance().showView('elements');
          },
          panelName: 'Elements',
          text: 'to ask about CSS styles',
        }),
        e.devToolsAiAssistanceNetworkAgent?.enabled &&
          P.ViewManager.ViewManager.instance().hasView('network') &&
          s.push({
            icon: 'arrow-up-down',
            heading: 'Network',
            jslogContext: 'open-network-panel',
            onClick: () => {
              P.ViewManager.ViewManager.instance().showView('network');
            },
            panelName: 'Network',
            text: "to ask about a request's details",
          }),
        e.devToolsAiAssistanceFileAgent?.enabled &&
          P.ViewManager.ViewManager.instance().hasView('sources') &&
          s.push({
            icon: 'document',
            heading: 'Files',
            jslogContext: 'open-sources-panel',
            onClick: () => {
              P.ViewManager.ViewManager.instance().showView('sources');
            },
            panelName: 'Sources',
            text: "to ask about a file's content",
          }),
        e.devToolsAiAssistancePerformanceAgent?.enabled &&
          P.ViewManager.ViewManager.instance().hasView('timeline') &&
          s.push({
            icon: 'performance',
            heading: 'Performance',
            jslogContext: 'open-performance-panel',
            onClick: () => {
              P.ViewManager.ViewManager.instance().showView('timeline');
            },
            panelName: 'Performance',
            text: 'to ask about a trace item',
          }),
        this.#o({ featureCards: s }, {}, this.contentElement));
    }
  };
import * as ui from './../../core/common/common.js';
import * as Ae from './../../core/sdk/sdk.js';
import * as fi from './../../models/trace/trace.js';
import * as yt from './../../ui/lit/lit.js';
import * as vi from './../common/common.js';
var { html: Gt } = yt.StaticHtml,
  { until: Do } = yt.Directives,
  Ge = class extends V {
    mainFrameId;
    lookupEvent;
    constructor(e = '', s = () => null) {
      (super(), (this.mainFrameId = e), (this.lookupEvent = s));
    }
    templateForToken(e) {
      if (e.type === 'link' && e.href.startsWith('#')) {
        if (e.href.startsWith('#node-')) {
          let n = Number(e.href.replace('#node-', ''));
          return Gt`<span>${Do(
            this.#o(n, e.text).then((r) => r || e.text),
            e.text,
          )}</span>`;
        }
        let s = this.lookupEvent(e.href.slice(1));
        if (!s) return Gt`${e.text}`;
        let i = e.text,
          o = '';
        return (
          fi.Types.Events.isSyntheticNetworkRequest(s)
            ? (o = s.args.data.url)
            : (i += ` (${s.name})`),
          Gt`<a href="#" draggable=false .title=${o} @click=${(n) => {
            (n.stopPropagation(), ui.Revealer.reveal(new Ae.TraceObject.RevealableEvent(s)));
          }}>${i}</a>`
        );
      }
      return super.templateForToken(e);
    ...
    async #o(e, s) {
      if (e === void 0) return;
      let o = Ae.TargetManager.TargetManager.instance()
        .primaryPageTarget()
        ?.model(Ae.DOMModel.DOMModel);
      if (!o) return;
      let r = (await o.pushNodesByBackendIdsToFrontend(new Set([e])))?.get(e);
      return !r || r.frameId() !== this.mainFrameId
        ? void 0
        : vi.DOMLinkifier.Linkifier.instance().linkify(r, { textContent: s });
    }
  };
import * as bt from './../../core/sdk/sdk.js';
import * as wi from './../../third_party/marked/marked.js';
import * as xt from './../../ui/lit/lit.js';
import * as bi from './../common/common.js';
var { html: D } = xt.StaticHtml,
  { until: yi } = xt.Directives,
  wt = class t extends V {
    mainFrameId;
    constructor(e = '') {
      (super(), (this.mainFrameId = e));
    }
    #o(e) {
      ...
      if (!['Problem', 'Element', 'NodeId', 'Details'].every((n) => s.includes(n))) return null;
      let o = s.indexOf('Problem');
      if (o > -1) {
        let n = s.splice(o, 1);
        s.unshift(...n);
      }
    ...
    templateForToken(e) {
      ...
      if (e.type === 'link' && e.href.startsWith('#')) {
        ...
        if (
          ...
          return D`<span>${yi(
            this.#n(s, e.text).then((i) => i || e.text),
            e.text,
          )}</span>`;
      }
      return super.templateForToken(e);
    ...
    #t(e) {
      ...
      o?.classList.contains('details-row') &&
        (o.style.display === 'none'
          ? ((o.style.display = 'table-row'), (s.textContent = 'Hide'))
          : ((o.style.display = 'none'), (s.textContent = 'Details')));
    }
    #s(e) {
      if (e.indexOf(',') === -1) {
        let i = Number(e);
        return isNaN(i) ? D`${e}` : this.#a(i);
      }
      let s = e
        .split(',')
        .map((i) => i.trim())
      ...
      return D`${s.map((i) => {
        let o = Number(i);
        return isNaN(o) ? D`<div>${i}</div>` : D`<div>${this.#a(o)}</div>`;
      })}`;
    }
    #a(e) {
      let s = 'link';
      return D`<span>${yi(
        this.#n(e, s).then((i) => i || s),
        s,
      )}</span>`;
    }
    async #n(e, s) {
      if (e === void 0) return;
      let o = bt.TargetManager.TargetManager.instance()
        .primaryPageTarget()
        ?.model(bt.DOMModel.DOMModel);
      if (!o) return;
      let r = (await o.pushNodesByBackendIdsToFrontend(new Set([e])))?.get(e);
      return !r || r.frameId() !== this.mainFrameId
        ? void 0
        : bi.DOMLinkifier.Linkifier.instance().linkify(r, { textContent: s });
    }
  };
var Ci = {};
J(Ci, { saveToDisk: () => Yt });
import * as xi from './../../core/platform/platform.js';
import * as ki from './../../models/text_utils/text_utils.js';
import * as Kt from './../../models/workspace/workspace.js';
async function Yt(t) {
  let e = t.getConversationMarkdown(),
    s = new ki.ContentData.ContentData(e, !1, 'text/markdown'),
    i = xi.StringUtilities.toSnakeCase(t.title || ''),
    o = 'devtools_',
    n = '.md',
    r = 64 - o.length - n.length,
    c = i || 'conversation';
  c.length > r && (c = c.substring(0, r));
  let a = `${o}${c}${n}`;
  (await Kt.FileManager.FileManager.instance().save(a, s, !0),
    Kt.FileManager.FileManager.instance().close(a));
}
var { html: _ } = O,
  { widget: Qt } = h.Widget,
  Fo = 'https://crbug.com/364805393',
  Uo = 'https://developer.chrome.com/docs/devtools/ai-assistance',
  No = 700,
  zo = 400,
  S = {
    newChat: 'New chat',
  ...
  v = {
    ...
    inputPlaceholderForPerformanceWithNoRecording:
  ...
  jo = At.i18n.registerUIStrings('panels/ai_assistance/AiAssistancePanel.ts', S),
  T = At.i18n.getLocalizedString.bind(void 0, jo),
  y = At.i18n.lockedString;
function kt(t) {
  return t &&
    (pe.Prototypes.instance().isEnabled('emulationCapabilities') ||
      t.nodeType() === Node.ELEMENT_NODE)
    ? t
    : null;
}
async function Wo(t) {
  let e = t?.selectedContext;
  if (e) {
    let s = await e.getSuggestions();
    if (s) return s;
  }
  if (!t?.type || t.isReadOnly) return [];
  switch (t.type) {
    case 'freestyler':
      return [
    ...
    case 'accessibility':
      return [
        {
          title: 'What are the accessibility issues on this page?',
          jslogContext: 'accessibility-default',
        },
        {
          title: 'How can I fix these accessibility issues?',
          jslogContext: 'accessibility-default',
        },
        {
          title: 'What does this Lighthouse report say about accessibility?',
          jslogContext: 'accessibility-default',
        },
      ];
    ...
    default:
      Li.assertNever(t.type, 'Unknown conversation type');
  }
}
function Oo(t) {
  let e = t?.selectedContext;
  if (e instanceof g.PerformanceAgent.PerformanceTraceContext) {
    if (!e.external) {
      let s = e.getItem();
      return new Ge(s.parsedTrace.data.Meta.mainFrameId, s.lookupEvent.bind(s));
    }
  } else {
    if (t?.type === 'drjones-performance-full') return new Ge();
    if (
      pe.Prototypes.instance().isEnabled('emulationCapabilities') &&
      t?.type === 'freestyler' &&
      u.TargetManager.TargetManager.instance().primaryPageTarget()?.model(u.DOMModel.DOMModel)
    ) {
      let o = u.TargetManager.TargetManager.instance()
        .primaryPageTarget()
        ?.model(u.DOMModel.DOMModel)
        ?.target()
        .model(u.ResourceTreeModel.ResourceTreeModel)?.mainFrame?.id;
      return new wt(o);
    } else if (t?.type === 'accessibility') {
      let o = u.TargetManager.TargetManager.instance()
        .primaryPageTarget()
        ?.model(u.DOMModel.DOMModel)
        ?.target()
...
function Ai(t) {
  let e = !!R.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled;
  return _`
    <div class="toolbar-container" role="toolbar" jslog=${Ye.toolbar()}>
      ...
      ${
        ...
        ${
          t.showActiveConversationActions
            ? _`
          <devtools-button
...
function Si(t, e, s) {
  function i() {
    switch (t.state) {
      case 'chat-view':
        return _`<devtools-ai-chat-view
          .props=${t.props}
          ${O.Directives.ref((o) => {
            !o || !(o instanceof Oe) || (e.chatView = o);
          })}
        ></devtools-ai-chat-view>`;
      case 'explore-view':
        return _`<devtools-widget class="fill-panel" ${Qt(_e)}>
                    </devtools-widget>`;
      case 'disabled-view':
        return _`<devtools-widget class="fill-panel" ${Qt(He, t.props)}>
                    </devtools-widget>`;
    }
  }
  if (
    R.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled ||
    pe.Prototypes.instance().isEnabled('breakpointDebuggerAgent')
  ) {
    let o = t.state === 'chat-view' && t.props.walkthrough.isExpanded,
      n = !1;
    if (t.state === 'chat-view') {
      let r = t.props.messages.at(-1);
      r && t.props.walkthrough.activeSidebarMessage === r && (n = !0);
    }
    O.render(
      _`
      ${Ai(t)}
        ...
        >
          ...
          <div slot="sidebar" class="sidebar-view">
            ${
              o
                ? _`
              <devtools-widget ${Qt(he, { message: t.props.walkthrough.activeSidebarMessage, isLoading: t.props.isLoading && n, markdownRenderer: t.props.markdownRenderer, onToggle: t.props.walkthrough.onToggle })}></devtools-widget>`
                : O.nothing
            }
          </div>
        </devtools-split-view>
...
function Ti(t) {
  return t ? new g.StylingAgent.NodeContext(t) : null;
}
function Vo(t) {
  return t ? new g.FileAgent.FileContext(t) : null;
}
function Bo(t) {
  return t ? new g.BreakpointDebuggerAgent.BreakpointContext(t) : null;
}
function Ii(t) {
  return t ? new g.AccessibilityAgent.AccessibilityContext(t.report) : null;
}
function qo(t) {
  if (!t) return null;
  let e = me.NetworkPanel.NetworkPanel.instance().networkLogView.timeCalculator();
  return new g.NetworkAgent.RequestContext(t, e);
}
function Ho(t) {
  return t ? new g.PerformanceAgent.PerformanceTraceContext(t) : null;
}
var Jt,
  Ct = class t extends h.Panel.Panel {
    view;
    ...
    #n = Ko();
    #r;
    #i = new g.ChangeManager.ChangeManager();
    #m = new F.Mutex.Mutex();
    #e;
    #p = null;
    #c = null;
    ...
    #l = { isInlined: !1, isExpanded: !1, activeSidebarMessage: null, inlineExpandedMessages: [] };
    constructor(e = Si, { aidaClient: s, aidaAvailability: i }) {
      (super(t.panelName),
        (this.view = e),
        this.registerRequiredCSS(es),
        (this.#r = this.#I()),
        (this.#t = s),
        (this.#v = i),
        h.ActionRegistry.ActionRegistry.instance().hasAction('elements.toggle-element-search') &&
          (this.#o = h.ActionRegistry.ActionRegistry.instance().getAction(
            'elements.toggle-element-search',
          )),
        g.AiHistoryStorage.AiHistoryStorage.instance().addEventListener(
          'AiHistoryDeleted',
          this.#le,
          this,
        ));
    ...
    #M() {
      return {
        isLoading: this.#w,
        showChatActions: this.#Z(),
        showActiveConversationActions: !!(this.#e && !this.#e.isEmpty),
        onNewChatClick: this.#N.bind(this),
        populateHistoryMenu: this.#re.bind(this),
        onDeleteClick: this.#ce.bind(this),
        onExportConversationClick: this.#_.bind(this),
        onHelpClick: () => {
          Ri.openInNewTab(Uo);
        },
        onSettingsClick: () => {
          h.ViewManager.ViewManager.instance().showView('chrome-ai');
        },
      };
    }
    async #R() {
      let e = R.Runtime.hostConfig.aidaAvailability?.blockedByAge === !0;
      if (this.#v !== 'available' || !this.#r?.getIfNotDisabled() || e)
        return { state: 'disabled-view', props: { aidaAvailability: this.#v } };
      if (this.#e) {
        let s = await Wo(this.#e),
          i = Oo(this.#e),
          o = null;
        return (
          Se() && this.#L(this.#x()) && (o = this.#ne.bind(this)),
          {
            state: 'chat-view',
            props: {
              blockedByCrossOrigin: this.#e.isBlockedByOrigin,
              isLoading: this.#w,
              messages: this.#f,
              context: this.#e.selectedContext ?? this.#L(this.#x()),
              isContextSelected: !!this.#e.selectedContext,
              conversationType: this.#e.type,
              isReadOnly: this.#e.isReadOnly ?? !1,
              changeSummary: this.#Y(),
              inspectElementToggled: this.#o?.toggled() ?? !1,
              canShowFeedbackForm: this.#n,
              multimodalInputEnabled: Zt() && this.#e.type === 'freestyler',
              isTextInputDisabled: this.#X(),
              emptyStateSuggestions: s,
              inputPlaceholder: this.#ee(),
              disclaimerText: this.#te(),
              onExportConversation: this.#_.bind(this),
              changeManager: this.#i,
              uploadImageInputEnabled: Go() && this.#e.type === 'freestyler',
              markdownRenderer: i,
              conversationMarkdown: this.#e.getConversationMarkdown(),
              generateConversationSummary: async (n) => (
                this.#s ||
                  (this.#s = new g.ConversationSummaryAgent.ConversationSummaryAgent({
                    aidaClient: this.#t,
                    serverSideLoggingEnabled: this.#n,
                  })),
                await this.#s.summarizeConversation(n)
              ),
              onTextSubmit: async (n, r, c) => {
                (w.userMetrics.actionTaken(w.UserMetrics.Action.AiAssistanceQuerySubmitted),
                  await this.#j(n, r, c));
              },
              onInspectElementClick: this.#J.bind(this),
              onFeedbackSubmit: this.#se.bind(this),
              onCancelClick: this.#z.bind(this),
              onContextClick: this.#ie.bind(this),
              onNewConversation: this.#N.bind(this),
              onCopyResponseClick: this.#Q.bind(this),
              onContextRemoved: Se() ? this.#oe.bind(this) : null,
              onContextAdd: o,
              walkthrough: {
                onToggle: this.#F.bind(this),
                onOpen: this.#A.bind(this),
                isExpanded: this.#l.isExpanded,
                isInlined: this.#l.isInlined,
                activeSidebarMessage: this.#l.activeSidebarMessage,
                inlineExpandedMessages: this.#l.inlineExpandedMessages,
              },
            },
          }
        );
      }
      return { state: 'explore-view' };
    }
    onResize() {
      (super.onResize(), R.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled && this.#E());
    }
    #E() {
      let e = this.contentElement.offsetWidth < No;
      if (e !== this.#l.isInlined) {
        if (((this.#l.isInlined = e), !this.#l.isExpanded)) {
          ((this.#l.activeSidebarMessage = null),
            (this.#l.inlineExpandedMessages = []),
            this.requestUpdate());
          return;
        }
        (e
          ? (this.#l.inlineExpandedMessages = this.#l.activeSidebarMessage
              ? [this.#l.activeSidebarMessage]
              : [])
          : (this.#l.activeSidebarMessage = this.#l.inlineExpandedMessages.at(-1) ?? null),
          this.requestUpdate());
      }
    }
    #A(e) {
      (this.#l.inlineExpandedMessages.includes(e) || this.#l.inlineExpandedMessages.push(e),
        (this.#l.activeSidebarMessage = e),
        (this.#l.isExpanded = !0),
        this.requestUpdate());
    }
    #F(e, s) {
      ...
      ((this.#l.inlineExpandedMessages = this.#l.inlineExpandedMessages.filter((i) => i !== s)),
        this.#l.isInlined
          ? ((this.#l.isExpanded = this.#l.inlineExpandedMessages.length > 0),
            this.#l.activeSidebarMessage === s &&
              (this.#l.activeSidebarMessage = this.#l.inlineExpandedMessages.at(-1) ?? null))
          : ((this.#l.isExpanded = !1), (this.#l.activeSidebarMessage = null)),
        this.requestUpdate());
    }
    #I() {
      try {
        return F.Settings.moduleSetting('ai-assistance-enabled');
      } catch {
        return;
      }
    }
    static async instance(e = { forceNew: null }) {
      let { forceNew: s } = e;
      if (!Jt || s) {
        let i = new w.AidaClient.AidaClient(),
          o = await w.AidaClient.AidaClient.checkAccessPreconditions();
        Jt = new t(Si, { aidaClient: i, aidaAvailability: o });
      }
      return Jt;
    }
    #k() {
      let e = h.Context.Context.instance().flavor(Ie.TimelinePanel.TimelinePanel);
      e !== this.#b &&
        (this.#b?.removeEventListener('IsViewingTrace', this.requestUpdate, this),
        (this.#b = e),
        this.#b && this.#b.addEventListener('IsViewingTrace', this.requestUpdate, this));
    }
    async #S() {
      return await Ie.TimelinePanel.TimelinePanel.executeRecordAndReload();
    }
    async #T(e) {
      return await Ke.LighthousePanel.LighthousePanel.executeLighthouseRecording({
        isAIControlled: !0,
        ...e,
      });
    }
    #x() {
      let { hostConfig: e } = R.Runtime,
        s = h.ViewManager.ViewManager.instance(),
        i = s.isViewVisible('elements'),
        o = s.isViewVisible('network'),
        n = s.isViewVisible('sources'),
        r = s.isViewVisible('timeline'),
        c = s.isViewVisible('lighthouse'),
        a;
      return (
        i && e.devToolsFreestyler?.enabled
          ? (a = 'freestyler')
          : o && e.devToolsAiAssistanceNetworkAgent?.enabled
            ? (a = 'drjones-network-request')
            : n && this.#e?.type === 'breakpoint'
              ? (a = 'breakpoint')
              : n && e.devToolsAiAssistanceFileAgent?.enabled
                ? (a = 'drjones-file')
                : r && e.devToolsAiAssistancePerformanceAgent?.enabled
                  ? (a = 'drjones-performance-full')
                  : c && e.devToolsAiAssistanceAccessibilityAgent?.enabled && (a = 'accessibility'),
        Se() && !a ? 'none' : a
      );
    }
    #$() {
      ...
      let s = e
        ? new g.AiConversation.AiConversation({
            type: e,
            data: [],
            isReadOnly: !1,
            aidaClient: this.#t,
            changeManager: this.#i,
            isExternal: !1,
            performanceRecordAndReload: this.#S.bind(this),
            onInspectElement: this.#D.bind(this),
            networkTimeCalculator:
              me.NetworkPanel.NetworkPanel.instance().networkLogView.timeCalculator(),
            lighthouseRecording: this.#T.bind(this),
          })
        : void 0;
      this.#y(s);
    }
    #y(e) {
      if (this.#e !== e) {
        if ((this.#z(), (this.#f = []), (this.#w = !1), this.#e?.archiveConversation(), !e)) {
          let s = this.#x();
          s &&
            (e = new g.AiConversation.AiConversation({
              type: s,
              data: [],
              isReadOnly: !1,
              aidaClient: this.#t,
      ...
      if (this.#e)
        if (this.#e.isEmpty && Se()) {
          let s = this.#L(this.#x());
          this.#e.setContext(s);
        } else {
          let s = this.#L(this.#e.type);
          (s || !Se()) && this.#e.setContext(s);
        }
      this.requestUpdate();
    }
    async handleBreakpointConversation(e, s) {
      let i = new g.BreakpointDebuggerAgent.BreakpointContext(e);
      this.#d = i;
      let o = new g.AiConversation.AiConversation({
        type: 'breakpoint',
        data: [],
        isReadOnly: !1,
        aidaClient: this.#t,
      ...
      (this.#y(o),
        this.#e?.setContext(i),
        this.requestUpdate(),
        await h.ViewManager.ViewManager.instance().showView(t.panelName));
      let n = s
        ? `debug the error "${s}" using breakpoint debugging agent`
        : 'debug the error using breakpoint debugging agent';
      await this.#j(n);
    }
    wasShown() {
      (super.wasShown(),
        this.#a.chatView?.restoreScrollPosition(),
        this.#a.chatView?.focusTextInput(),
        this.#U(),
        (this.#c = Ti(kt(h.Context.Context.instance().flavor(u.DOMModel.DOMNode)))),
        (this.#h = qo(h.Context.Context.instance().flavor(u.NetworkRequest.NetworkRequest))),
        (this.#g = Ho(h.Context.Context.instance().flavor(g.AIContext.AgentFocus))),
        (this.#p = Vo(h.Context.Context.instance().flavor(Te.UISourceCode.UISourceCode))),
        (this.#d = Bo(h.Context.Context.instance().flavor(Te.UISourceCode.UILocation))),
        (this.#u = Ii(
          h.Context.Context.instance().flavor(Ke.LighthousePanel.ActiveLighthouseReport),
        )),
        this.#y(this.#e),
        this.#r?.addChangeListener(this.requestUpdate, this),
        w.AidaClient.HostConfigTracker.instance().addEventListener(
          'aidaAvailabilityChanged',
          this.#U,
        ),
        this.#o?.addEventListener('Toggled', this.requestUpdate, this),
        h.Context.Context.instance().addFlavorChangeListener(u.DOMModel.DOMNode, this.#W),
        h.Context.Context.instance().addFlavorChangeListener(
          u.NetworkRequest.NetworkRequest,
          this.#O,
        ),
        h.Context.Context.instance().addFlavorChangeListener(g.AIContext.AgentFocus, this.#V),
        h.Context.Context.instance().addFlavorChangeListener(Te.UISourceCode.UISourceCode, this.#B),
        h.Context.Context.instance().addFlavorChangeListener(Te.UISourceCode.UILocation, this.#K),
        h.Context.Context.instance().addFlavorChangeListener(
          Ke.LighthousePanel.ActiveLighthouseReport,
          this.#q,
        ),
        h.ViewManager.ViewManager.instance().addEventListener(
        ...
        u.TargetManager.TargetManager.instance().addModelListener(
          u.DOMModel.DOMModel,
          u.DOMModel.Events.AttrModified,
          this.#P,
          this,
        ),
        u.TargetManager.TargetManager.instance().addModelListener(
          u.DOMModel.DOMModel,
          u.DOMModel.Events.AttrRemoved,
          this.#P,
          this,
        ),
        h.Context.Context.instance().addFlavorChangeListener(
          Ie.TimelinePanel.TimelinePanel,
          this.#k,
          this,
        ),
        this.#k(),
        this.#$(),
        w.userMetrics.actionTaken(w.UserMetrics.Action.AiAssistancePanelOpened));
    }
    willHide() {
      (super.willHide(),
        this.#r?.removeChangeListener(this.requestUpdate, this),
        w.AidaClient.HostConfigTracker.instance().removeEventListener(
          'aidaAvailabilityChanged',
          this.#U,
        ),
        this.#o?.removeEventListener('Toggled', this.requestUpdate, this),
        h.Context.Context.instance().removeFlavorChangeListener(u.DOMModel.DOMNode, this.#W),
        h.Context.Context.instance().removeFlavorChangeListener(
          u.NetworkRequest.NetworkRequest,
          this.#O,
        ),
        h.Context.Context.instance().removeFlavorChangeListener(g.AIContext.AgentFocus, this.#V),
        h.Context.Context.instance().removeFlavorChangeListener(
          Te.UISourceCode.UISourceCode,
          this.#B,
        ),
        h.Context.Context.instance().removeFlavorChangeListener(
          Ke.LighthousePanel.ActiveLighthouseReport,
        ...
        h.ViewManager.ViewManager.instance().removeEventListener(
          'ViewVisibilityChanged',
          this.#$,
          this,
        ),
    ...
    #U = async () => {
      let e = await w.AidaClient.AidaClient.checkAccessPreconditions();
      e !== this.#v && ((this.#v = e), this.requestUpdate());
    };
    #W = (e) => {
      this.#c?.getItem() !== e.data && ((this.#c = Ti(kt(e.data))), this.#y(this.#e));
    };
    #P = (e) => {
      this.#c?.getItem() === e.data.node &&
        (e.data.name === 'class' || e.data.name === 'id') &&
        this.requestUpdate();
    };
    #O = (e) => {
      if (this.#h?.getItem() !== e.data) {
        if (e.data) {
          let s = me.NetworkPanel.NetworkPanel.instance().networkLogView.timeCalculator();
          this.#h = new g.NetworkAgent.RequestContext(e.data, s);
        } else this.#h = null;
        this.#y(this.#e);
      }
    };
    #V = (e) => {
      this.#g?.getItem() !== e.data &&
        ((this.#g = e.data ? new g.PerformanceAgent.PerformanceTraceContext(e.data) : null),
        this.#y(this.#e));
    };
    #B = (e) => {
      let s = e.data;
      !s ||
        this.#p?.getItem() === s ||
        ((this.#p = new g.FileAgent.FileContext(e.data)), this.#y(this.#e));
    };
    #K = (e) => {
      let s = e.data;
      !s ||
        this.#d?.getItem() === s ||
        ((this.#d = new g.BreakpointDebuggerAgent.BreakpointContext(s)), this.#y(this.#e));
    };
    #q = (e) => {
      let s = e.data;
      this.#u?.getItem() !== s?.report && ((this.#u = Ii(s)), this.#y(this.#e));
    };
    #Y() {
      if (!de() || !this.#e || this.#e?.isReadOnly) return;
      let e = !!R.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled;
      return this.#i.formatChangesForPatching(this.#e.id, !e);
    }
    async performUpdate() {
      let e = { ...this.#M(), ...(await this.#R()) };
      this.view(e, this.#a, this.contentElement);
    }
    #Q(e) {
      let s = _o(e);
      s &&
        (w.InspectorFrontendHost.InspectorFrontendHostInstance.copyText(s),
        Mi.Snackbar.Snackbar.show({ message: T(S.responseCopiedToClipboard) }));
    }
    #J() {
      (h.Context.Context.instance().setFlavor(
        F.ReturnToPanel.ReturnToPanelFlavor,
        new F.ReturnToPanel.ReturnToPanelFlavor(this.panelName),
      ),
        this.#o?.execute());
    }
    #X() {
      return !!(
        (this.#e && this.#e.isBlockedByOrigin) ||
        !this.#e ||
        (!this.#e.selectedContext && !Se())
      );
    }
    #Z() {
      let e = this.#r?.getIfNotDisabled(),
        s = R.Runtime.hostConfig.aidaAvailability?.blockedByAge === !0;
      return !(!e || s || this.#v === 'no-account-email' || this.#v === 'sync-is-paused');
    }
    #ee() {
      if (!this.#e) return T(S.followTheSteps);
      if (this.#e && this.#e.isBlockedByOrigin) return y(v.crossOriginError);
      switch (this.#e.type) {
        case 'freestyler':
          return this.#e.selectedContext
            ? y(v.inputPlaceholderForStyling)
            : y(v.inputPlaceholderForStylingNoContext);
        case 'drjones-file':
          return this.#e.selectedContext
            ? y(v.inputPlaceholderForFile)
            : y(v.inputPlaceholderForFileNoContext);
        case 'drjones-network-request':
          return this.#e.selectedContext
            ? y(v.inputPlaceholderForNetwork)
            : y(v.inputPlaceholderForNetworkNoContext);
        case 'drjones-performance-full':
          return h.Context.Context.instance()
            .flavor(Ie.TimelinePanel.TimelinePanel)
            ?.hasActiveTrace()
            ? this.#e.selectedContext
              ? y(v.inputPlaceholderForPerformanceTrace)
              : y(v.inputPlaceholderForPerformanceTraceNoContext)
            : y(v.inputPlaceholderForPerformanceWithNoRecording);
        case 'breakpoint':
          return y(v.inputPlaceholderForNoContext);
        case 'accessibility':
          return this.#e.selectedContext
            ? y(v.inputPlaceholderForAccessibility)
            : y(v.inputPlaceholderForAccessibilityNoContext);
        case 'none':
          return g.AiUtils.isGeminiBranding()
            ? y(v.inputPlaceholderForNoContextBranded)
            : y(v.inputPlaceholderForNoContext);
      }
    }
    #te() {
      if (!this.#e || this.#e.isReadOnly) return T(S.inputDisclaimerForEmptyState);
      let e =
        R.Runtime.hostConfig.aidaAvailability?.enterprisePolicyValue ===
        R.Runtime.GenAiEnterprisePolicyValue.ALLOW_WITHOUT_LOGGING;
      switch (this.#e.type) {
        case 'freestyler':
          return y(
            e ? v.inputDisclaimerForStylingEnterpriseNoLogging : v.inputDisclaimerForStyling,
          );
        case 'drjones-file':
          return y(e ? v.inputDisclaimerForFileEnterpriseNoLogging : v.inputDisclaimerForFile);
        case 'drjones-network-request':
          return y(
            e ? v.inputDisclaimerForNetworkEnterpriseNoLogging : v.inputDisclaimerForNetwork,
          );
        case 'drjones-performance-full':
          return y(
            e
              ? v.inputDisclaimerForPerformanceEnterpriseNoLogging
              : v.inputDisclaimerForPerformance,
          );
        case 'accessibility':
          return y(
            e
              ? v.inputDisclaimerForAccessibilityEnterpriseNoLogging
              : v.inputDisclaimerForAccessibility,
          );
        case 'breakpoint':
        case 'none':
          return y(
            e ? v.inputDisclaimerForNoContextEnterpriseNoLogging : v.inputDisclaimerForNoContext,
          );
      }
    }
    #se(e, s, i) {
      this.#t.registerClientEvent({
        corresponding_aida_rpc_global_id: e,
        disable_user_content_logging: !this.#n,
        do_conversation_client_event: {
          user_feedback: { sentiment: s, user_input: { comment: i } },
        },
      });
    }
    #ie() {
      if (!this.#e) return;
      let e = this.#e.selectedContext;
      if (e instanceof g.NetworkAgent.RequestContext) {
        let s = Ei.UIRequestLocation.UIRequestLocation.tab(e.getItem(), 'headers-component');
        return F.Revealer.reveal(s);
      }
      if (e instanceof g.FileAgent.FileContext)
        return F.Revealer.reveal(e.getItem().uiLocation(0, 0));
      if (e instanceof g.PerformanceAgent.PerformanceTraceContext) {
        let s = e.getItem();
        if (s.callTree) {
          let i = s.callTree.selectedNode?.event ?? s.callTree.rootNode.event,
            o = new u.TraceObject.RevealableEvent(i);
          return F.Revealer.reveal(o);
        }
        if (s.insight) return F.Revealer.reveal(s.insight);
      }
    }
    #oe() {
      (this.#e?.setContext(null), this.requestUpdate());
    }
    #ne() {
      (this.#e?.setContext(this.#L(this.#x())), this.requestUpdate());
    }
    #ae() {
      let e = !!R.Runtime.hostConfig.aidaAvailability?.enabled,
        s = !!R.Runtime.hostConfig.aidaAvailability?.blockedByAge,
        i = this.#v === 'available',
        o = !!this.#r?.getIfNotDisabled();
      return e && i && o && !s;
    }
    async handleAction(e, s) {
      ...
      switch (e) {
        case 'freestyler.elements-floating-button': {
          (w.userMetrics.actionTaken(
            w.UserMetrics.Action.AiAssistanceOpenedFromElementsPanelFloatingButton,
          ),
            (i = 'freestyler'));
          break;
        }
        case 'freestyler.element-panel-context': {
          (w.userMetrics.actionTaken(w.UserMetrics.Action.AiAssistanceOpenedFromElementsPanel),
            (i = 'freestyler'));
          break;
        }
        case 'drjones.network-floating-button': {
          (w.userMetrics.actionTaken(
            w.UserMetrics.Action.AiAssistanceOpenedFromNetworkPanelFloatingButton,
          ),
            (i = 'drjones-network-request'));
          break;
        }
        case 'drjones.network-panel-context': {
          (w.userMetrics.actionTaken(w.UserMetrics.Action.AiAssistanceOpenedFromNetworkPanel),
            (i = 'drjones-network-request'));
          break;
        }
        case 'drjones.performance-panel-context': {
          (w.userMetrics.actionTaken(
            w.UserMetrics.Action.AiAssistanceOpenedFromPerformancePanelCallTree,
          ),
            (i = 'drjones-performance-full'));
          break;
        }
        case 'drjones.sources-floating-button': {
          (w.userMetrics.actionTaken(
            w.UserMetrics.Action.AiAssistanceOpenedFromSourcesPanelFloatingButton,
          ),
            (i = 'drjones-file'));
          break;
        }
        case 'drjones.sources-panel-context': {
          (w.userMetrics.actionTaken(w.UserMetrics.Action.AiAssistanceOpenedFromSourcesPanel),
            (i = 'drjones-file'));
          break;
        }
      }
      if (!i) return;
      let o = this.#e;
      ((!this.#e || this.#e.type !== i || this.#e.isEmpty) &&
        (o = new g.AiConversation.AiConversation({
          type: i,
          data: [],
          isReadOnly: !1,
          aidaClient: this.#t,
      ...
      if (n && typeof n == 'string') {
        if (!this.#ae()) return;
        (w.userMetrics.actionTaken(w.UserMetrics.Action.AiAssistanceQuerySubmitted),
          this.#e && this.#e.isBlockedByOrigin && this.#N(),
          await this.#j(n));
      } else this.#a.chatView?.focusTextInput();
    }
    #re(e) {
      let s = g.AiHistoryStorage.AiHistoryStorage.instance()
        .getHistory()
        .map((o) => g.AiConversation.AiConversation.fromSerializedConversation(o));
      for (let o of s.reverse())
        o.isEmpty ||
          !o.title ||
          e.defaultSection().appendCheckboxItem(
            o.title,
            () => {
              this.#de(o);
            },
            { checked: this.#e?.id === o.id, jslogContext: 'freestyler.history-item' },
          );
      let i = e.defaultSection().items.length === 0;
      (i && e.defaultSection().appendItem(T(S.noPastConversations), () => {}, { disabled: !0 }),
        e.footerSection().appendItem(
          T(S.clearChatHistory),
          () => {
            g.AiHistoryStorage.AiHistoryStorage.instance().deleteAll();
          },
          { disabled: i },
        ));
    }
    ...
    #ce() {
      this.#e &&
        (this.#H(),
        g.AiHistoryStorage.AiHistoryStorage.instance().deleteHistoryEntry(this.#e.id),
        this.#y(),
        h.ARIAUtils.LiveAnnouncer.alert(T(S.chatDeleted)));
    }
    async #_() {
      if (this.#e) return await Yt(this.#e);
    }
    async #de(e) {
      this.#e?.id !== e.id && (this.#y(e), await this.#G(e.history));
    }
    #N() {
      (this.#y(),
        this.#H(),
        h.ARIAUtils.LiveAnnouncer.alert(T(S.newChatCreated)),
        Xt.AnnotationRepository.annotationsEnabled() &&
          Xt.AnnotationRepository.instance().deleteAllAnnotations());
    }
    #z() {
      (this.#C.abort(), (this.#C = new AbortController()));
    }
    #L(e) {
      switch (e) {
        case 'freestyler':
    ...
    #he = (e) => {
      (e instanceof g.FileAgent.FileContext
        ? (this.#p = e)
        : e instanceof g.StylingAgent.NodeContext
          ? (this.#c = e)
          : e instanceof g.NetworkAgent.RequestContext
            ? (this.#h = e)
            : e instanceof g.PerformanceAgent.PerformanceTraceContext
              ? (this.#g = e)
              : e instanceof g.BreakpointDebuggerAgent.BreakpointContext
                ? (this.#d = e)
                : e instanceof g.AccessibilityAgent.AccessibilityContext && (this.#u = e),
        Ye.logFunctionCall(`context-change-${this.#e?.type}`),
        this.requestUpdate());
    };
    async #D() {
      ...
      let e = new Promise((s) => {
        let i = (r) => {
            r.data && (s(kt(r.data)), n());
          },
          o = (r) => {
            r.data ||
              globalThis.setTimeout(() => {
                (s(kt(h.Context.Context.instance().flavor(u.DOMModel.DOMNode))), n());
              }, 50);
          },
          n = () => {
            (h.Context.Context.instance().removeFlavorChangeListener(u.DOMModel.DOMNode, i),
              this.#o?.removeEventListener('Toggled', o));
          };
        (h.Context.Context.instance().addFlavorChangeListener(u.DOMModel.DOMNode, i),
          this.#o?.addEventListener('Toggled', o),
          this.#C.signal.addEventListener(
            'abort',
            () => {
              (s(null), n());
            },
    ...
    async #j(e, s, i) {
      ...
      let o = this.#C.signal;
      this.#e.isEmpty &&
        St.UserBadges.instance().recordAction(St.BadgeAction.STARTED_AI_CONVERSATION);
      let n = pe.Prototypes.instance().isEnabled('emulationCapabilities'),
        r,
        c = this.#e.getPendingMultimodalInput();
      (n && c ? (r = c) : Zt() && s && i && (r = { input: s, id: crypto.randomUUID(), type: i }),
        Ye.logFunctionCall(`start-conversation-${this.#e.type}`, 'ui'),
        await this.#G(this.#e.run(e, { signal: o, multimodalInput: r })));
    }
    async #G(e) {
      let s = await this.#m.acquire();
      try {
        let n = function () {
            let a = i.parts.at(-1);
            (a?.type === 'step' && a.step === o) || i.parts.push({ type: 'step', step: o });
          },
          i = { entity: 'model', parts: [] },
          o = { isLoading: !0 };
        this.#w = !0;
        let r = !1,
          c = !1;
        for await (let a of e) {
          switch (((o.requestApproval = void 0), a.type)) {
            case 'user-query': {
              (this.#f.push({ entity: 'user', text: a.query, imageInput: a.imageInput }),
                (i = { entity: 'model', parts: [] }),
                this.#f.push(i),
                ((this.#l.isExpanded && !this.#l.isInlined) ||
                  (pe.Prototypes.instance().isEnabled('breakpointDebuggerAgent') &&
                    this.#e?.type === 'breakpoint')) &&
                  this.#A(i));
              break;
            }
            case 'querying': {
              ((o = { isLoading: !0 }), i.parts.length || n());
              break;
            }
            case 'context': {
              ((o.title = y(v.analyzingData)),
                (o.contextDetails = a.details),
                (o.widgets = a.widgets),
                (o.isLoading = !1),
                n());
              break;
            ...
            case 'thought': {
              ((o.isLoading = !1), (o.thought = a.thought), n());
              break;
            }
            case 'suggestions': {
              let f = i.parts.at(-1);
              f?.type === 'answer'
                ? (f.suggestions = a.suggestions)
                : i.parts.push({ type: 'answer', text: '', suggestions: a.suggestions });
              break;
            }
            case 'side-effect': {
              ((o.isLoading = !1),
                ...
                (o.requestApproval = {
                  ...
                  onAnswer: (f) => {
                    (a.confirm(f), (o.requestApproval = void 0), this.requestUpdate());
                  },
                }),
                n());
              break;
            ...
            case 'answer': {
              ...
              let f = i.parts.at(-1);
              if (f?.type === 'answer')
                ((f.text = a.text), a.suggestions && (f.suggestions = a.suggestions));
              else {
                let x = { type: 'answer', text: a.text };
                (a.suggestions && (x.suggestions = a.suggestions), i.parts.push(x));
              }
              if (
                (a.widgets &&
                  R.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled &&
                  i.parts.push({ type: 'widget', widgets: a.widgets }),
                i.parts.length > 1)
              ) {
                let x = i.parts[0];
                x.type === 'step' &&
            ...
            case 'error': {
              ...
              if (f?.type === 'step') {
                let x = f.step;
                a.error === 'abort' ? (x.canceled = !0) : x.isLoading && i.parts.pop();
              }
              a.error === 'block' && i.parts.at(-1)?.type === 'answer' && i.parts.pop();
              break;
            }
            case 'context-change': {
              (this.#he(a.context),
          ...
          if (!this.#e?.isReadOnly)
            switch (
              (this.requestUpdate(),
              (a.type === 'context' || a.type === 'side-effect') &&
                this.#a.chatView?.scrollToBottom(),
              a.type)
            ) {
              case 'context':
                h.ARIAUtils.LiveAnnouncer.status(y(v.analyzingData));
                break;
              case 'answer':
                !a.complete && !r
                  ? ((r = !0), h.ARIAUtils.LiveAnnouncer.status(y(v.answerLoading)))
                  : a.complete &&
                    !c &&
                    ((c = !0), h.ARIAUtils.LiveAnnouncer.status(y(v.answerReady)));
            }
        }
        ((this.#w = !1), this.requestUpdate());
      } finally {
        s();
      }
    }
...
${s.text}`);
      ...
      let i = s.step;
      (i.title && e.push(`### ${i.title}`),
        i.contextDetails &&
          e.push(g.AiConversation.generateContextDetailsMarkdown(i.contextDetails)),
        i.thought && e.push(i.thought),
        i.code &&
  ...
  handleAction(e, s, i) {
    switch (s) {
      ...
      case 'drjones.sources-panel-context':
        return (
          (async () => {
            let o = h.ViewManager.ViewManager.instance().view(Ct.panelName);
            if (!o) return;
            await h.ViewManager.ViewManager.instance().showView(Ct.panelName);
            let n = h.InspectorView.InspectorView.instance().totalSize() / 4;
            (h.InspectorView.InspectorView.instance().drawerSize() < n &&
              h.InspectorView.InspectorView.instance().setDrawerSize(n),
              (await o.widget()).handleAction(s, i));
          })(),
          !0
        );
    }
...
function Go() {
  return Zt() && !!R.Runtime.hostConfig.devToolsFreestyler?.multimodalUploadInput;
}
function Zt() {
  return !!R.Runtime.hostConfig.devToolsFreestyler?.multimodal;
}
function Se() {
  return !!R.Runtime.hostConfig.devToolsAiAssistanceContextSelectionAgent?.enabled;
}
function Ko() {
  return !R.Runtime.hostConfig.aidaAvailability?.disallowLogging;
}
export {
  Qe as AccessibilityAgentMarkdownRenderer,
  $i as ActionDelegate,
  Ct as AiAssistancePanel,
  Cs as ChatInput,
  Bs as ChatMessage,
  Oe as ChatView,
  ri as DisabledWidget,
  pi as ExploreWidget,
  Ci as ExportConversation,
  Qs as ExportForAgentsDialog,
  V as MarkdownRendererWithCodeBlock,
  fs as PatchWidget,
  Ui as SELECT_WORKSPACE_DIALOG_DEFAULT_VIEW,
  Ze as SelectWorkspaceDialog,
  Rs as WalkthroughView,
  _o as getResponseMarkdown,
};
