...
function i18nAiBrandedString(gemini, assistance) {
  ...
  return () =>
    Root.Runtime.hostConfig.devToolsGeminiRebranding?.enabled
      ? i18nString(gemini)
      : i18nString(assistance);
}
...
function isNetworkAgentFeatureAvailable(config) {
  return (
    (config?.aidaAvailability?.enabled && config?.devToolsAiAssistanceNetworkAgent?.enabled) ===
    true
  );
}
function isPerformanceAgentFeatureAvailable(config) {
  return (
    (config?.aidaAvailability?.enabled && config?.devToolsAiAssistancePerformanceAgent?.enabled) ===
    true
  );
}
function isFileAgentFeatureAvailable(config) {
  return (
    (config?.aidaAvailability?.enabled && config?.devToolsAiAssistanceFileAgent?.enabled) === true
  );
}
function isAnyFeatureAvailable(config) {
  return (
    isStylingAgentFeatureAvailable(config) ||
    isNetworkAgentFeatureAvailable(config) ||
    isPerformanceAgentFeatureAvailable(config) ||
    isFileAgentFeatureAvailable(config)
  );
}
...
UI.ActionRegistration.registerActionExtension({
  ...
  condition: (config) =>
    isAnyFeatureAvailable(config) && !isPolicyRestricted(config) && !isGeoRestricted(config),
});
UI.ActionRegistration.registerActionExtension({
...
UI.ActionRegistration.registerActionExtension({
  ...
  condition: (config) =>
    isFileAgentFeatureAvailable(config) && !isPolicyRestricted(config) && !isGeoRestricted(config),
});
UI.ActionRegistration.registerActionExtension({
