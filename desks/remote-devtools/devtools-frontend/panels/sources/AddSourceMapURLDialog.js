...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html` <style>
      ...
      <input
        class="harmony-input add-source-map"
        spellcheck="false"
        type="text"
      ...
      <devtools-button
        ...
        >${i18nString(UIStrings.add)}</devtools-button
      >`,
    target,
  );
  // clang-format on
...
export class AddDebugInfoURLDialog extends UI.Widget.HBox {
  ...
  static createAddSourceMapURLDialog(callback) {
    return new AddDebugInfoURLDialog(
      i18nString(UIStrings.sourceMapUrl),
      'add-source-map-url',
      callback,
    );
  }
  static createAddDWARFSymbolsURLDialog(callback) {
    return new AddDebugInfoURLDialog(
      i18nString(UIStrings.debugInfoUrl),
      'add-debug-info-url',
      callback,
    );
  }
