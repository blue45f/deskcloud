...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  render(
    html`
      <style>
        ${breakpointEditDialogStyles}
      </style>
      <div class="dialog-header">
        <devtools-toolbar class="source-frame-breakpoint-toolbar"
          >Line ${input.editorLineNumber + 1}:
          <select
            class="type-selector"
            title=${input.breakpointType ===
          ...
          >
            <option
              value=${
                'REGULAR_BREAKPOINT' /* SDK.DebuggerModel.BreakpointType.REGULAR_BREAKPOINT */
              }
            >
            ...
            <option
              ...
              .selected=${
                input.breakpointType === 'LOGPOINT' /* SDK.DebuggerModel.BreakpointType.LOGPOINT */
              }
            >
      ...
      <div class="link-wrapper">
        ...
        <devtools-link
          class="devtools-link"
          href="https://goo.gle/devtools-loc"
...
export class BreakpointEditDialog extends UI.Widget.Widget {
  ...
  #getEditorState() {
    ...
    const history = () =>
      this.#editor &&
      new TextEditor.TextEditorHistory.TextEditorHistory(this.#editor, this.#history);
    const autocomplete = (context) => history()?.historyCompletions(context) ?? null;
    const historyBack = (force) =>
      history()?.moveHistory(-1 /* Direction.BACKWARD */, force) ?? false;
    const historyForward = (force) =>
      history()?.moveHistory(1 /* Direction.FORWARD */, force) ?? false;
    const finishIfComplete = (view) => {
      void TextEditor.JavaScript.isExpressionComplete(view.state.doc.toString()).then(
        (complete) => {
          if (complete) {
  ...
  finishEditing(committed, condition) {
    ...
    const isLogpoint =
      this.breakpointType === 'LOGPOINT'; /* SDK.DebuggerModel.BreakpointType.LOGPOINT */
