...
export class SourcesView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor() {
    ...
    this.#searchableView = new UI.SearchableView.SearchableView(
      this,
      this,
    ...
    this.editorContainer.addEventListener(
      'EditorSelected' /* TabbedEditorContainerEvents.EDITOR_SELECTED */,
      this.editorSelected,
      this,
    );
    this.editorContainer.addEventListener(
      'EditorClosed' /* TabbedEditorContainerEvents.EDITOR_CLOSED */,
      this.editorClosed,
      this,
    );
  ...
  placeholderElement() {
    ...
    for (const shortcut of shortcuts) {
      const shortcutKeys = UI.ShortcutRegistry.ShortcutRegistry.instance().shortcutsForAction(
        shortcut.actionId,
      );
      ...
      if (shortcutKeys?.[0]) {
        ...
        const keys = shortcutKeys[0].descriptors.flatMap((descriptor) =>
          descriptor.name.split(separator),
        );
        keys.forEach((key) => {
  ...
  createSourceView(uiSourceCode) {
    ...
    if (
      ...
      uiSourceCode.mimeType().startsWith('image/')
    ) {
      sourceView = new SourceFrame.ImageView.ImageView(uiSourceCode.mimeType(), uiSourceCode);
    } else if (
      contentType === Common.ResourceType.resourceTypes.Font ||
      uiSourceCode.mimeType().includes('font')
    ) {
  ...
  #uiSourceCodeTitleChanged(event) {
    ...
    if (widget) {
      if (
        this.#sourceViewTypeForWidget(widget) !== this.#sourceViewTypeForUISourceCode(uiSourceCode)
      ) {
  ...
  editorSelected(event) {
    const previousSourceFrame =
      event.data.previousView instanceof UISourceCodeFrame ? event.data.previousView : null;
    if (previousSourceFrame) {
      previousSourceFrame.setSearchableView(null);
    }
    const currentSourceFrame =
      event.data.currentView instanceof UISourceCodeFrame ? event.data.currentView : null;
  ...
  updateToolbarChangedListener() {
    ...
    this.toolbarChangedListener = sourceFrame.addEventListener(
      'ToolbarItemsChanged' /* UISourceCodeFrameEvents.TOOLBAR_ITEMS_CHANGED */,
      this.updateScriptViewToolbarItems,
      this,
    );
  }
