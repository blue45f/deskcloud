...
export class ResourceOriginPlugin extends Plugin {
  ...
  rightToolbarItems() {
    ...
    if (this.uiSourceCode.contentType().isFromSourceMap()) {
      ...
      for (const originURL of Bindings.SASSSourceMapping.SASSSourceMapping.uiSourceOrigin(
        this.uiSourceCode,
      )) {
      ...
      return [
        new UI.Toolbar.ToolbarItem(
          uiI18n.getFormatLocalizedString(str_, UIStrings.fromS, { PH1: element }),
        ),
      ];
    }
    ...
    for (const script of debuggerWorkspaceBinding.scriptsForUISourceCode(this.uiSourceCode)) {
      if (script.originStackTrace?.callFrames.length) {
        const link = this.#linkifier.linkifyStackTraceTopFrame(
          script.debuggerModel.target(),
          script.originStackTrace,
        );
        return [
          new UI.Toolbar.ToolbarItem(
            uiI18n.getFormatLocalizedString(str_, UIStrings.fromS, { PH1: link }),
          ),
        ];
      }
    }
