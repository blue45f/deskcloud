...
export class NavigatorView extends UI.Widget.VBox {
  ...
  constructor(jslogContext, enableAuthoredGrouping) {
    ...
    this.navigatorGroupByFolderSetting = Common.Settings.Settings.instance().moduleSetting(
    ...
    this.navigatorJustMyCodeSetting =
      Common.Settings.Settings.instance().moduleSetting('navigator-just-my-code');
    this.navigatorJustMyCodeSetting.addChangeListener(this.groupingChanged.bind(this));
    if (enableAuthoredGrouping) {
      this.navigatorGroupByAuthoredSetting = Common.Settings.Settings.instance().moduleSetting(
    ...
    Persistence.NetworkPersistenceManager.NetworkPersistenceManager.instance().addEventListener(
      'RequestsForHeaderOverridesFileChanged' /* Persistence.NetworkPersistenceManager.Events.REQUEST_FOR_HEADER_OVERRIDES_FILE_CHANGED */,
      this.#onRequestsForHeaderOverridesFileChanged,
      this,
    );
    SDK.TargetManager.TargetManager.instance().addEventListener(
      'NameChanged' /* SDK.TargetManager.Events.NAME_CHANGED */,
    ...
    Bindings.NetworkProject.NetworkProjectManager.instance().addEventListener(
      'FrameAttributionAdded' /* Bindings.NetworkProject.Events.FRAME_ATTRIBUTION_ADDED */,
      this.frameAttributionAdded,
      this,
    );
    Bindings.NetworkProject.NetworkProjectManager.instance().addEventListener(
      'FrameAttributionRemoved' /* Bindings.NetworkProject.Events.FRAME_ATTRIBUTION_REMOVED */,
  ...
  static appendSearchItem(contextMenu, path) {
    const searchLabel = path
      ? i18nString(UIStrings.searchInFolder)
      : i18nString(UIStrings.searchInAllFiles);
    const searchSources = new SearchSources(path && `file:${path}`);
    contextMenu
      .viewSection()
      .appendItem(searchLabel, () => Common.Revealer.reveal(searchSources), {
  ...
  setPlaceholder(placeholder) {
    ...
    this.scriptsTree.addEventListener(
  ...
  frameAttributionRemoved(event) {
    ...
    const node = Array.from(this.uiSourceCodeNodes.get(uiSourceCode)).find(
      (node) => node.frame() === removedFrame,
    );
    if (node) {
  ...
  addUISourceCodeNode(uiSourceCode, frame) {
    ...
    if (uiSourceCode.project().type() === Workspace.Workspace.projectTypes.FileSystem) {
      path = Persistence.FileSystemWorkspaceBinding.FileSystemWorkspaceBinding.relativePath(
        uiSourceCode,
      ).slice(0, -1);
    } else {
  ...
  projectAdded(project) {
    ...
    if (
      ...
      !FILE_SYSTEM_TYPES.includes(project.type()) ||
      Snippets.ScriptSnippetFileSystem.isSnippetsProject(project) ||
      rootOrDeployed.child(project.id())
    ) {
      return;
    }
    const type =
      project instanceof Persistence.AutomaticFileSystemWorkspaceBinding.FileSystem ||
      (project instanceof Persistence.FileSystemWorkspaceBinding.FileSystem &&
        project.fileSystem().automatic)
        ? Types.AutomaticFileSystem
        : Types.FileSystem;
    rootOrDeployed.appendChild(
      new NavigatorGroupTreeNode(this, project, project.id(), type, project.displayName()),
    );
    this.selectDefaultTreeNode();
  }
  ...
  computeUniqueFileSystemProjectNames() {
    const fileSystemProjects = this.#workspace.projectsForType(
      Workspace.Workspace.projectTypes.FileSystem,
    );
  ...
  removeProject(project) {
    ...
    if (
      project.type() !== Workspace.Workspace.projectTypes.ConnectableFileSystem &&
  ...
  folderNodeId(project, target, frame, projectOrigin, isFromSourceMap, path) {
    const projectId =
      project.type() === Workspace.Workspace.projectTypes.FileSystem ? project.id() : '';
  ...
  folderNode(uiSourceCode, project, target, frame, projectOrigin, path, fromSourceMap) {
    ...
    const parentNode = this.folderNode(
      uiSourceCode,
      project,
      target,
      frame,
    ...
    folderNode = new NavigatorFolderTreeNode(
      this,
      project,
      folderId,
      type,
  ...
  frameNode(project, target, frame, isAuthored) {
    ...
    this.frameNode(
      project,
      parentFrame ? parentFrame.resourceTreeModel().target() : target,
      parentFrame,
      isAuthored,
  ...
  computeProjectDisplayName(target, projectOrigin) {
    ...
    const prettyURL = parsedURL.isValid
  ...
  removeUISourceCodeNode(node) {
    ...
    while (currentNode) {
      ...
      if (
        (parentNode === this.rootNode || parentNode === this.deployedNode) &&
        project.type() === Workspace.Workspace.projectTypes.FileSystem
      ) {
        break;
      }
      if (
        !(
          currentNode instanceof NavigatorGroupTreeNode ||
  ...
  handleFileContextMenu(event, node) {
    ...
    if (project.type() === Workspace.Workspace.projectTypes.FileSystem) {
      contextMenu
        .editSection()
        .appendItem(i18nString(UIStrings.rename), this.handleContextMenuRename.bind(this, node), {
          jslogContext: 'rename',
        });
      contextMenu
        .editSection()
  ...
  handleFolderContextMenu(event, node) {
    ...
    if (project.type() === Workspace.Workspace.projectTypes.FileSystem) {
      ...
      for (const {
        ...
        callback,
        jslogContext,
      } of Workspace.IgnoreListManager.IgnoreListManager.instance().getIgnoreListFolderContextMenuItems(
        url,
        options,
      )) {
    ...
    if (project.type() === Workspace.Workspace.projectTypes.ConnectableFileSystem) {
      ...
      if (automaticFileSystem?.state === 'disconnected') {
        contextMenu.defaultSection().appendItem(
          i18nString(UIStrings.connectFolderToWorkspace),
          async () => {
    ...
    if (project.type() === Workspace.Workspace.projectTypes.FileSystem) {
      if (
        ...
        if (node instanceof NavigatorGroupTreeNode) {
          contextMenu.defaultSection().appendItem(
            i18nString(UIStrings.removeFolderFromWorkspace),
            async () => {
              const header = i18nString(UIStrings.areYouSureYouWantToRemoveThis, {
                PH1: node.title,
              });
              const shouldRemove = await UI.UIUtils.ConfirmDialog.show(
        ...
        contextMenu
          .defaultSection()
          .appendItem(i18nString(UIStrings.delete), this.handleDeleteFolder.bind(this, node), {
            jslogContext: 'delete',
          });
      }
    }
...
export class NavigatorSourceTreeElement extends UI.TreeOutline.TreeElement {
  ...
  constructor(navigatorView, uiSourceCode, title, node) {
    ...
    this.titleElement.setAttribute(
      'jslog',
      `${VisualLogging.value('title').track({ change: true })}`,
    );
  }
  updateIcon() {
  ...
  selectOnMouseDown(event) {
    ...
    globalThis.setTimeout(rename.bind(this), 300);
    function rename() {
...
export class NavigatorUISourceCodeTreeNode extends NavigatorTreeNode {
  ...
  treeNode() {
    ...
    this.treeElement = new NavigatorSourceTreeElement(
      this.navigatorView,
      this.#uiSourceCode,
      '',
      this,
...
export class NavigatorFolderTreeNode extends NavigatorTreeNode {
  ...
  updateTitle() {
    ...
    const hasMappedFiles =
      Persistence.Persistence.PersistenceImpl.instance().filePathHasBindings(
...
export class NavigatorGroupTreeNode extends NavigatorTreeNode {
  ...
  treeNode() {
    ...
    const expandable = !(
      this.project instanceof Persistence.AutomaticFileSystemWorkspaceBinding.FileSystem
    );
    this.treeElement = new NavigatorFolderTreeElement(
      this.navigatorView,
      this.type,
      this.title,
      this.hoverCallback,
  ...
  updateTitle() {
    if (
      !this.treeElement ||
      !this.project ||
      this.project.type() !== Workspace.Workspace.projectTypes.FileSystem
    ) {
      return;
    }
