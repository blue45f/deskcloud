...
function findPropertyAt(node, pos) {
  ...
  for (let cur = node; cur; cur = cur.parent) {
    if (cur.name === 'StyleSheet' || cur.name === 'Styles' || cur.name === 'CallExpression') {
      ...
      const name = cur.getChild('PropertyName'),
        colon = cur.getChild(':');
...
async function specificCssCompletion(cx, uiSourceCode, cssModel) {
  ...
  if (property) {
    const propertyValues = SDK.CSSMetadata.cssMetadata().getPropertyValues(
      cx.state.sliceDoc(property.from, property.to),
    );
    return {
...
class ColorSwatchWidget extends CodeMirror.WidgetType {
  ...
  eq(other) {
    return (
      this.#color.equal(other.#color) && this.#text === other.#text && this.#from === other.#from
    );
  }
  toDOM(view) {
...
function createCSSTooltip(active) {
  return {
    ...
    create(view) {
      ...
      widget.element.addEventListener(
        'focusout',
        (event) => {
...
const cssTooltipState = CodeMirror.StateField.define({
  ...
  provide: (field) =>
    CodeMirror.showTooltip.from(field, (active) => active && createCSSTooltip(active)),
});
  ...
  findColorsAndCurves(
    ...
    (pos, curve, text) => {
      builder.add(
        pos,
        pos,
        CodeMirror.Decoration.widget({ widget: new CurveSwatchWidget(curve, text) }),
      );
    },
  );
  return builder.finish();
}
const cssSwatchPlugin = CodeMirror.ViewPlugin.fromClass(
  class {
    decorations;
    constructor(view) {
    ...
    update(update) {
      if (update.viewportChanged || update.docChanged) {
        this.decorations = computeSwatchDeco(
          update.state,
...
export class CSSPlugin extends Plugin {
  ...
  #cssCompletion() {
    ...
    return CodeMirror.autocompletion({
      override: [
        async (cx) => {
          return await ((await specificCssCompletion(cx, uiSourceCode, cssModel)) ||
  ...
  populateTextAreaContextMenu(contextMenu) {
    ...
    if (
      ...
      contextMenu
        .debugSection()
        .appendItem(addSourceMapURLLabel, () => addSourceMapURL(cssModel, url), {
