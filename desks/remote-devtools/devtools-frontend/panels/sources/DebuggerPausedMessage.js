...
const DEFAULT_VIEW = (input, _output, target) => {
  render(
    html`
      <style>
        ${debuggerPausedMessageStyles}
      </style>
      <div aria-live="polite" ?hidden=${!input}>
        ${input
          ? html` <div class="paused-status ${input.errorLike ? 'error-reason' : ''}">
              <span>
                <div class="status-main">
                  <devtools-icon
                    name=${input.errorLike ? 'cross-circle-filled' : 'info'}
                    class="medium"
                  ></devtools-icon>
                  ${input.mainText}
                </div>
                ${input.subText || input.domBreakpointData
                  ? html`
                      <div
                        class="status-sub monospace"
                        title=${ifDefined(input.title ?? input.subText)}
                      >
                        ${input.domBreakpointData
                          ? domBreakpointSubtext(input.domBreakpointData)
                          : input.subText}
                      </div>
                    `
                  : nothing}
...
export class DebuggerPausedMessage extends UI.Widget.Widget {
  ...
  static async createDOMBreakpointHitMessageDetails(details) {
    const domDebuggerModel = details.debuggerModel
      .target()
    ...
    return {
      mainText: i18nString(UIStrings.pausedOnS, {
  ...
  static #findEventNameForUi(detailsAuxData) {
    ...
    let breakpoint =
      SDK.EventBreakpointsModel.EventBreakpointsManager.instance().resolveEventListenerBreakpoint(
        detailsAuxData,
      );
    if (breakpoint) {
    ...
    breakpoint =
      SDK.DOMDebuggerModel.DOMDebuggerManager.instance().resolveEventListenerBreakpoint(
        detailsAuxData,
      );
  ...
  async render(details, debuggerWorkspaceBinding, breakpointManager) {
    ...
    const errorLike =
      details.reason === 'exception' /* Protocol.Debugger.PausedEventReason.Exception */ ||
      details.reason ===
        'promiseRejection' /* Protocol.Debugger.PausedEventReason.PromiseRejection */ ||
    ...
    if (details.reason === 'DOM' /* Protocol.Debugger.PausedEventReason.DOM */) {
      ...
      details.reason === 'EventListener' /* Protocol.Debugger.PausedEventReason.EventListener */
    ) {
      const eventNameForUI = DebuggerPausedMessage.#findEventNameForUi(details.auxData);
      this.#viewInput = {
        mainText: i18nString(UIStrings.pausedOnEventListener),
        subText: eventNameForUI,
        errorLike,
      };
    } else if (details.reason === 'XHR' /* Protocol.Debugger.PausedEventReason.XHR */) {
      const auxData = details.auxData;
      this.#viewInput = {
        mainText: i18nString(UIStrings.pausedOnXhrOrFetch),
        subText: auxData.url || '',
        errorLike,
      };
      ...
      details.reason === 'debugCommand' /* Protocol.Debugger.PausedEventReason.DebugCommand */
    ) {
      ...
      this.#viewInput = {
        mainText: i18nString(UIStrings.pausedBeforePotentialOutofmemory),
        errorLike,
      };
    } else if (
      details.reason === 'CSPViolation' /* Protocol.Debugger.PausedEventReason.CSPViolation */ &&
      ...
      const defaultText = breakpoint
        ? i18nString(UIStrings.pausedOnBreakpoint)
        : i18nString(UIStrings.debuggerPaused);
