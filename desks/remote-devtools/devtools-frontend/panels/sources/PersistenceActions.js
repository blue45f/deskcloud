...
export class ContextMenuProvider {
  appendApplicableItems(_event, contextMenu, contentProvider) {
    async function saveAs() {
      ...
      if (maybeScript?.isWasm()) {
        try {
          ...
          console.error(
            `Unable to convert WASM byte code for ${url} to base64. Not saving to disk`,
            e.stack,
          );
        ...
        if (TextUtils.ContentData.ContentData.isError(contentDataOrError)) {
          ...
          Common.Console.Console.instance().error(
            i18nString(UIStrings.saveFailed),
            /* show=*/ false,
          );
          return;
        }
    ...
    if (contentProvider.contentType().isDocumentOrScriptOrStyleSheet()) {
      contextMenu
        .saveSection()
        .appendItem(i18nString(UIStrings.saveAs), saveAs, { jslogContext: 'save-as' });
    } else if (
      contentProvider instanceof SDK.Resource.Resource &&
      contentProvider.contentType().isImage()
    ) {
      contextMenu
        .saveSection()
        .appendItem(i18nString(UIStrings.saveImage), saveImage, { jslogContext: 'save-image' });
    }
    ...
    const binding =
      uiSourceCode && Persistence.Persistence.PersistenceImpl.instance().binding(uiSourceCode);
    ...
    if (uiSourceCode && networkPersistenceManager.isUISourceCodeOverridable(uiSourceCode)) {
      if (!uiSourceCode.contentType().isFromSourceMap()) {
        ...
        if (deployedUiSourceCode) {
          disabled = false;
          handler = this.redirectOverrideToDeployedUiSourceCode.bind(
            this,
            deployedUiSourceCode,
            uiSourceCode,
          );
        }
      }
    ...
    contextMenu
      .overrideSection()
      .appendItem(i18nString(UIStrings.overrideContent), handler, {
        disabled,
        jslogContext: 'override-content',
      });
    if (contentProvider instanceof SDK.NetworkRequest.NetworkRequest) {
      contextMenu.overrideSection().appendItem(
        i18nString(UIStrings.showOverrides),
        async () => {
          await UI.ViewManager.ViewManager.instance().showView('navigator-overrides');
          Host.userMetrics.actionTaken(
            Host.UserMetrics.Action.ShowAllOverridesFromNetworkContextMenu,
          );
        },
  ...
  async redirectOverrideToDeployedUiSourceCode(deployedUiSourceCode, originalUiSourceCode) {
    Host.userMetrics.actionTaken(
      Host.UserMetrics.Action.OverrideContentContextMenuSourceMappedWarning,
    );
    ...
    const shouldJumpToDeployedFile = await UI.UIUtils.ConfirmDialog.show(
      ...
      i18nString(UIStrings.overrideSourceMappedFileWarning, { PH1: deployedName }),
      undefined,
      { jslogContext: 'override-source-mapped-file-warning' },
    );
    if (shouldJumpToDeployedFile) {
      Host.userMetrics.actionTaken(
        Host.UserMetrics.Action.OverrideContentContextMenuRedirectToDeployed,
      );
  ...
  getDeployedUiSourceCode(uiSourceCode) {
    ...
    const [deployedStylesUrl] =
      Bindings.SASSSourceMapping.SASSSourceMapping.uiSourceOrigin(uiSourceCode);
    if (!deployedStylesUrl) {
      return null;
    }
    const deployedUiSourceCode =
      Workspace.Workspace.WorkspaceImpl.instance().uiSourceCodeForURL(deployedStylesUrl) ||
