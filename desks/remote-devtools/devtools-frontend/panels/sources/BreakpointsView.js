...
import {
  ...
  getDifferentiatingPathMap,
} from './BreakpointsViewUtils.js';
const {
  html,
  render,
  Directives: { ifDefined, repeat, classMap, live },
} = Lit;
const UIStrings = {
  /**
...
export class BreakpointsSidebarController {
  ...
  constructor(breakpointManager, settings) {
    this.#collapsedFilesSettings = Common.Settings.Settings.instance().createSetting(
      'collapsed-files',
      [],
    );
  ...
  static instance(
    { forceNew, breakpointManager, settings } = {
      forceNew: null,
  ...
  breakpointsRemoved(breakpointItems) {
    const locations = breakpointItems.flatMap((breakpointItem) =>
      this.#getLocationsForBreakpointItem(breakpointItem),
    );
  ...
  async jumpToSource(breakpointItem) {
    const uiLocations = this.#getLocationsForBreakpointItem(breakpointItem).map(
      (location) => location.uiLocation,
    );
    let uiLocation;
  ...
  async getUpdatedBreakpointViewData() {
    ...
    for (let idx = 0; idx < locationsGroupedById.length; idx++) {
      ...
      const codeSnippet =
        contentData instanceof TextUtils.WasmDisassembly.WasmDisassembly
          ? (contentData.lines[
              contentData.bytecodeOffsetToLineNumber(uiLocation.columnNumber ?? 0)
            ] ?? '')
      ...
      if (group) {
        ...
        const editable = this.#breakpointManager.supportsConditionalBreakpoints(
  ...
  #onBreakpointRemoved(event) {
    ...
    if (this.#collapsedFiles.has(breakpoint.url())) {
      const locations =
        Breakpoints.BreakpointManager.BreakpointManager.instance().allBreakpointLocations();
      const otherBreakpointsOnSameFileExist = locations.some(
        (location) => location.breakpoint.url() === breakpoint.url(),
      );
  ...
  #getBreakpointTypeAndDetails(locations) {
    const breakpointWithCondition = locations.find((location) =>
      Boolean(location.breakpoint.condition()),
    );
  ...
  #getBreakpointLocations() {
    const locations = this.#breakpointManager
      .allBreakpointLocations()
      .filter(
        (breakpointLocation) =>
          breakpointLocation.uiLocation.uiSourceCode.project().type() !==
    ...
    for (const location of locations) {
      if (
        location.breakpoint !== lastBreakpoint ||
        (lastLocation && location.uiLocation.compareTo(lastLocation))
      ) {
  ...
  #getBreakpointState(locations) {
    ...
    if (hasEnabled) {
      status = hasDisabled
        ? 'INDETERMINATE' /* BreakpointStatus.INDETERMINATE */
        : 'ENABLED' /* BreakpointStatus.ENABLED */;
    } else {
    ...
    html` <style>
      ...
      <div jslog=${VisualLogging.section('sources.js-breakpoints')} id="devtools-breakpoint-view">
        ...
        >
          <label class="checkbox-label">
            <input
              ...
              tabindex="-1"
              class="small"
              ?checked=${input.pauseOnUncaughtExceptions}
              @change=${input.onPauseOnUncaughtExceptionsStateChanged}
              jslog=${VisualLogging.toggle('pause-uncaught').track({ change: true })}
            />
            <span>${i18nString(UIStrings.pauseOnUncaughtExceptions)}</span>
          </label>
        </div>
        <div
          class="pause-on-caught-exceptions"
          tabindex="-1"
          @click=${input.clickHandler}
          @keydown=${input.keyDownHandler}
          role="checkbox"
        ...
        >
          <label class="checkbox-label">
            <input
              data-pause-on-caught-checkbox
              type="checkbox"
              class="small"
              tabindex="-1"
              ?checked=${input.pauseOnCaughtExceptions}
              @change=${input.onPauseOnCaughtExceptionsStateChanged.bind(this)}
              jslog=${VisualLogging.toggle('pause-on-caught-exception').track({ change: true })}
            />
        ...
        <div role="tree">
          ${repeat(
            ...
            (group, groupIndex) =>
              html` <details
                class=${classMap({ active: input.breakpointsActive })}
              ...
              >
                <summary
                  @contextmenu=${input.groupContextMenuHandler.bind(undefined, group)}
                  tabindex="-1"
                  @keydown=${input.keyDownHandler}
                  @click=${input.clickHandler}
                >
                  <span class="group-header" aria-hidden="true">
                    <span class="group-icon-or-disable">
                      ...
                      <input
                        class="group-checkbox small"
                        type="checkbox"
                        aria-label=""
                        .checked=${group.breakpointItems.some(
                          (item) => item.status === 'ENABLED' /* BreakpointStatus.ENABLED */,
                        )}
                  ...
                  <span class="group-hover-actions">
                    <button
                      ...
                      @click=${input.removeAllBreakpointsInFileClickHandler.bind(
                        undefined,
                  ...
                  (item, itemIndex) =>
                    html` <div
                      class=${classMap({
                        ...
                        logpoint:
                          item.type === 'LOGPOINT' /* SDK.DebuggerModel.BreakpointType.LOGPOINT */,
                      })}
                      ...
                      role="treeitem"
                      tabindex="-1"
                      @contextmenu=${input.itemContextMenuHandler.bind(
                        undefined,
                        item,
                        group.editable,
                      )}
                    ...
                    >
                      <label class="checkbox-label">
                        ...
                        <input
                          ...
                          class="small"
                          ?indeterminate=${
                            item.status === 'INDETERMINATE' /* BreakpointStatus.INDETERMINATE */
                          }
                      ...
                      <span
                        class="code-snippet"
                        @click=${input.itemSnippetClickHandler.bind(undefined, item)}
                        title=${ifDefined(input.itemDetails.get(item.id)?.codeSnippetTooltip)}
                        jslog=${VisualLogging.action('sources.jump-to-breakpoint').track({
                          click: true,
                        })}
                        >${input.itemDetails.get(item.id)?.codeSnippet}</span
                      >
                      <span class="breakpoint-item-location-or-actions">
                        ${group.editable
                          ? html` <button
                              data-edit-breakpoint
                              @click=${input.itemEditClickHandler.bind(undefined, item)}
                              title=${item.type ===
                              'LOGPOINT' /* SDK.DebuggerModel.BreakpointType.LOGPOINT */
                                ? i18nString(UIStrings.editLogpoint)
                                : i18nString(UIStrings.editCondition)}
                              jslog=${VisualLogging.action('edit-breakpoint').track({
                                click: true,
                              })}
                            >
...
export class BreakpointsView extends UI.Widget.VBox {
    ...
    for (const group of data.groups) {
      ...
      for (const item of group.breakpointItems) {
        this.#breakpointItemDetails.set(item.id, {
          ...
          codeSnippet: Platform.StringUtilities.trimEndWithMaxLength(
            item.codeSnippet,
            MAX_SNIPPET_LENGTH,
          ),
  ...
  performUpdate() {
    const input = {
      ...
      onPauseOnUncaughtExceptionsStateChanged:
        this.#onPauseOnUncaughtExceptionsStateChanged.bind(this),
      ...
      removeAllBreakpointsInFileClickHandler:
        this.#removeAllBreakpointsInFileClickHandler.bind(this),
  ...
  #groupCheckboxToggled(group, event) {
    ...
    const updatedStatus = element.checked
      ? 'ENABLED' /* BreakpointStatus.ENABLED */
      : 'DISABLED'; /* BreakpointStatus.DISABLED */
  ...
  async #handleHomeOrEndKey(key) {
    ...
    if (key === 'End') {
      ...
      if (lastGroup.expanded) {
        const lastBreakpointItem = this.contentElement.querySelector(
          '[data-last-group] > [data-last-breakpoint]',
        );
        return await this.#setSelected(lastBreakpointItem);
      }
      const lastGroupSummaryElement = this.contentElement.querySelector(
        '[data-last-group] > summary',
      );
  ...
  #onBreakpointGroupContextMenu(event, breakpointGroup) {
    ...
    menu.defaultSection().appendItem(
      i18nString(UIStrings.removeAllBreakpointsInFile),
      () => {
        Host.userMetrics.actionTaken(
          Host.UserMetrics.Action.BreakpointsInFileRemovedFromContextMenu,
        );
    ...
    menu.defaultSection().appendItem(
      i18nString(UIStrings.removeOtherBreakpoints),
      () => {
    ...
    menu.defaultSection().appendItem(
      i18nString(UIStrings.removeAllBreakpoints),
      () => {
        const breakpointItems = this.#breakpointGroups
          .map(({ breakpointItems }) => breakpointItems)
          .flat();
        void this.#controller.breakpointsRemoved(breakpointItems);
      },
    ...
    const notEnabledItems = breakpointItems.filter(
      (breakpointItem) => breakpointItem.status !== 'ENABLED' /* BreakpointStatus.ENABLED */,
    );
    menu.debugSection().appendItem(
      i18nString(UIStrings.enableAllBreakpointsInFile),
      () => {
        Host.userMetrics.actionTaken(
          Host.UserMetrics.Action.BreakpointsInFileEnabledDisabledFromContextMenu,
        );
    ...
    const notDisabledItems = breakpointItems.filter(
      (breakpointItem) => breakpointItem.status !== 'DISABLED' /* BreakpointStatus.DISABLED */,
    );
    menu.debugSection().appendItem(
      i18nString(UIStrings.disableAllBreakpointsInFile),
      () => {
        Host.userMetrics.actionTaken(
          Host.UserMetrics.Action.BreakpointsInFileEnabledDisabledFromContextMenu,
        );
  ...
  #onBreakpointEntryContextMenu(event, breakpointItem, editable) {
    ...
    const menu = new UI.ContextMenu.ContextMenu(event);
    const editBreakpointText =
      breakpointItem.type === 'LOGPOINT' /* SDK.DebuggerModel.BreakpointType.LOGPOINT */
        ? i18nString(UIStrings.editLogpoint)
        : i18nString(UIStrings.editCondition);
    menu.revealSection().appendItem(
      i18nString(UIStrings.revealLocation),
      () => {
