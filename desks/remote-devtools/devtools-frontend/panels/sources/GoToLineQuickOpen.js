...
export class GoToLineQuickOpen extends QuickOpen.FilteredListWidget.Provider {
  ...
  renderItem(itemIndex, _query) {
    // clang-format off
    return html` <devtools-icon name="colon"></devtools-icon>
      <div>
    ...
    if (!position) {
      ...
      if (disassembly) {
        ...
        this.#goToLineStrings.push(
          i18nString(UIStrings.currentPositionXsTypeAnOffset, {
      ...
      const linesCount =
        sourceFrame.editorLocationToUILocation(editorState.doc.lines - 1).lineNumber + 1;
      this.#goToLineStrings.push(
        i18nString(UIStrings.currentLineSTypeALineNumber, { PH1: linesCount }),
      );
      return;
    }
    if (sourceFrame?.wasmDisassembly) {
      this.#goToLineStrings.push(
        i18nString(UIStrings.goToOffsetXs, { PH1: (position.column - 1).toString(16) }),
      );
      return;
    }
    if (position.column && position.column > 1) {
      this.#goToLineStrings.push(
        i18nString(UIStrings.goToLineSAndColumnS, { PH1: position.line, PH2: position.column }),
      );
      return;
    }
