...
const str_ = i18n.i18n.registerUIStrings(
  'panels/sources/InplaceFormatterEditorAction.ts',
  UIStrings,
);
...
export class InplaceFormatterEditorAction {
  ...
  getOrCreateButton(sourcesView) {
    ...
    this.sourcesView.addEventListener(
      'EditorClosed' /* Events.EDITOR_CLOSED */,
      this.editorClosed.bind(this),
    );
    ...
    if (uiSourceCode.isDirty()) {
      ...
      void uiSourceCode
        ...
        .then((contentDataOrError) =>
          TextUtils.ContentData.ContentData.textOr(contentDataOrError, ''),
        )
        .then((content) => {
  ...
  async contentLoaded(uiSourceCode, sourceFrame, content) {
    const { formattedContent, formattedMapping } = await Formatter.ScriptFormatter.format(
      ...
      sourceFrame.contentType,
      content,
    );
    if (uiSourceCode.workingCopy() === formattedContent) {
      return;
    }
    const selection = sourceFrame.textEditor.toLineColumn(
      sourceFrame.textEditor.state.selection.main.head,
    );
