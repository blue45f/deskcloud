...
export class UISourceCodeFrame extends Common.ObjectWrapper.eventMixin(
  ...
  setUISourceCode(uiSourceCode) {
    const loaded = uiSourceCode.contentLoaded()
      ? Promise.resolve()
  ...
  initializeUISourceCode() {
    ...
    const isEditable = Persistence.Persistence.PersistenceImpl.instance().hasEditableContent(
      this.#uiSourceCode,
    );
  ...
  createMessage(origin) {
    const { lineNumber, columnNumber } = this.uiLocationToEditorLocation(
      origin.lineNumber(),
      origin.columnNumber(),
    );
  ...
  allMessages() {
    const origins =
      this.#persistenceBinding !== null
        ? [
            ...this.#persistenceBinding.network.messages(),
            ...this.#persistenceBinding.fileSystem.messages(),
          ]
  ...
  reloadPlugins() {
    ...
    editor.dispatch({
      effects: pluginCompartment.reconfigure(
        this.plugins.map((plugin) => plugin.editorExtension()),
      ),
    });
    for (const plugin of this.plugins) {
  ...
  onMessageAdded(event) {
    const { editor } = this.textEditor,
      shownMessages = editor.state.field(showRowMessages, false);
    if (shownMessages) {
  ...
  getErrorPopoverContent(event) {
    ...
    const anchorElement =
      eventTarget.enclosingNodeOrSelfWithClass('cm-messageIcon-error') ||
    ...
    const messages = row.filter(
      (msg) =>
        (msg.level() === 'Issue') /* Workspace.UISourceCode.Message.Level.ISSUE */ === issues,
    );
    if (!messages.length) {
      return null;
    }
    const anchor = anchorElement
      ? anchorElement.boxInWindow()
...
function getIconDataForMessage(message) {
  if (message.origin instanceof IssuesManager.SourceFrameIssuesManager.IssueMessage) {
    return {
      iconName: IssueCounter.IssueCounter.getIssueKindIconName(message.origin.getIssueKind()),
    };
  }
...
class MessageWidget extends CodeMirror.WidgetType {
  ...
  toDOM() {
    ...
    const nonIssues = this.messages.filter(
      (msg) => msg.level() !== 'Issue' /* Workspace.UISourceCode.Message.Level.ISSUE */,
    );
    if (nonIssues.length) {
    ...
    const issue = this.messages.find(
      (m) => m.level() === 'Issue' /* Workspace.UISourceCode.Message.Level.ISSUE */,
    );
    if (issue) {
  ...
  static create(messages, doc) {
    ...
    for (const row of messages.rows) {
      ...
      builder.add(
        line.to,
        line.to,
        CodeMirror.Decoration.widget({ side: 1, widget: new MessageWidget(row) }),
      );
    }
...
const showRowMessages = CodeMirror.StateField.define({
  ...
  provide: (field) =>
    CodeMirror.Prec.lowest(
      CodeMirror.EditorView.decorations.from(field, (value) => value.decorations),
    ),
});
...
function renderMessage(message, count) {
  ...
  if (count === 1) {
    ...
    const repeatCountElement = element.createChild(
      'dt-small-bubble',
...
function rowMessages(initialMessages) {
  return [
    showRowMessages.init((state) =>
      RowMessageDecorations.create(RowMessages.create(initialMessages), state.doc),
    ),
    rowMessageTheme,
  ];
}
