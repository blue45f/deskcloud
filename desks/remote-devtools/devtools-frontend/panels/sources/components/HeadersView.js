...
export class HeadersViewComponent extends HTMLElement {
  ...
  #onClick(event) {
    ...
    if (target.matches('.add-header')) {
      this.#headerOverrides[blockIndex].headers.splice(headerIndex + 1, 0, {
        name: this.#generateNextHeaderName(this.#headerOverrides[blockIndex].headers),
      ...
      this.#headerOverrides.push({
        applyTo: '*',
        headers: [{ name: getDefaultHeaderName(1), value: DEFAULT_HEADER_VALUE }],
      });
  ...
  #isDeletable(blockIndex, headerIndex) {
    const isOnlyDefaultHeader =
      headerIndex === 0 &&
      this.#headerOverrides[blockIndex].headers.length === 1 &&
  ...
  #render() {
    ...
    if (this.#focusElement) {
      ...
      if (this.#focusElement.headerIndex) {
        focusElement = this.#shadow.querySelector(
          `[data-block-index="${this.#focusElement.blockIndex}"][data-header-index="${this.#focusElement.headerIndex}"] .header-name`,
        );
      } else {
        focusElement = this.#shadow.querySelector(
          `[data-block-index="${this.#focusElement.blockIndex}"] .apply-to`,
        );
      }
      if (focusElement) {
        focusElement.focus();
  ...
  #renderApplyToRow(pattern, blockIndex) {
    ...
    return html`
      <div
        ...
        jslog=${VisualLogging.treeItem(pattern === '*' ? pattern : undefined).track({
          resize: true,
        })}
      >
  ...
  #renderHeaderRow(header, blockIndex, headerIndex) {
    ...
    return html`
      <div
        ...
        jslog=${VisualLogging.treeItem(header.name)
          .parent('headers-editor-row-parent')
          .track({ resize: true })}
      >
  ...
  #renderEditable(value, className, isKey) {
    ...
    return html`<span
      jslog=${jslog.track({ change: true, keydown: 'Enter|Escape|Tab', click: true })}
      contenteditable="true"
      class="editable ${className}"
      tabindex="0"
      .innerText=${Lit.Directives.live(value)}
    ></span>`;
    // clang-format on
  }
}
