    ...
    switch (cursor.name) {
      ...
      case 'VariableDefinition': {
        if (cursor.matchContext(['ClassDeclaration'])) {
          ...
          cursor.matchContext(['AssignmentExpression', 'MemberExpression']) ||
          cursor.matchContext(['VariableDeclaration'])
        ) {
          let title = state.sliceDoc(cursor.from, cursor.to);
          const { lineNumber, columnNumber } = toLineColumn(cursor.from);
          let hasEquals = false;
      ...
      case 'App': {
        if (
          cursor.firstChild() &&
          cursor.nextSibling() &&
          state.sliceDoc(cursor.from, cursor.to) === 'module'
        ) {
          ...
          do {
            if (cursor.name === 'App' && cursor.firstChild()) {
              if (
                cursor.nextSibling() &&
                state.sliceDoc(cursor.from, cursor.to) === 'func' &&
                cursor.nextSibling() &&
                ...
                while (cursor.nextSibling()) {
                  if (cursor.name === 'App' && cursor.firstChild()) {
                    if (
                      cursor.nextSibling() &&
                      state.sliceDoc(cursor.from, cursor.to) === 'param'
                    ) {
                      if (cursor.nextSibling() && cursor.name === 'Identifier') {
...
export class OutlineQuickOpen extends QuickOpen.FilteredListWidget.Provider {
  ...
  attach() {
    ...
    if (sourceFrame) {
      this.active = true;
      this.items = outline(sourceFrame.textEditor.state).map(
        ({ title, subtitle, lineNumber, columnNumber }) => {
          ({ lineNumber, columnNumber } = sourceFrame.editorLocationToUILocation(
            lineNumber,
            columnNumber,
          ));
  ...
  renderItem(itemIndex, query) {
    ...
    const highlightRanges = QuickOpen.FilteredListWidget.FilteredListWidget.getHighlightRanges(
      title,
      query,
      true,
    );
    // clang-format off
    return html` <devtools-icon name="deployed"></devtools-icon>
      <div>
        <devtools-highlight type="markup" ranges=${highlightRanges}>${title}</devtools-highlight>
      </div>
  ...
  selectItem(itemIndex, _promptValue) {
    ...
    sourceFrame.revealPosition(
      { lineNumber: item.lineNumber, columnNumber: item.columnNumber },
      true,
    );
  }
  currentSourceFrame() {
