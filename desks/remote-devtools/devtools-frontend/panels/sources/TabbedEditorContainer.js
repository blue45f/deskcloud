...
export class TabbedEditorContainer extends Common.ObjectWrapper.ObjectWrapper {
  ...
  onBindingCreated(event) {
    ...
    if (!fileSystemTabId) {
      ...
      if (networkView instanceof UISourceCodeFrame) {
        ...
        this.restoreEditorProperties(
          fileSystemTabView,
          currentSelectionRange,
          currentScrollLineNumber,
        );
      }
    }
  ...
  showFile(uiSourceCode) {
    ...
    if (
      frame?.currentSourceFrame()?.contentSet &&
  ...
  addViewListeners() {
    if (
      !this.currentView ||
      !(this.currentView instanceof SourceFrame.SourceFrame.SourceFrameImpl)
    ) {
      return;
    }
    this.currentView.addEventListener(
      'EditorUpdate' /* SourceFrame.SourceFrame.Events.EDITOR_UPDATE */,
      this.onEditorUpdate,
      this,
    );
    this.currentView.addEventListener(
      'EditorScroll' /* SourceFrame.SourceFrame.Events.EDITOR_SCROLL */,
  ...
  onScrollChanged() {
    if (this.currentView instanceof SourceFrame.SourceFrame.SourceFrameImpl) {
      ...
      if (this.#currentFile) {
        ...
        const topBlock = editor.lineBlockAtHeight(
  ...
  onEditorUpdate({ data: update }) {
    if (update.docChanged || update.selectionSet) {
      ...
      const lineFrom = update.state.doc.lineAt(main.from),
        lineTo = update.state.doc.lineAt(main.to);
      ...
      this.previouslyViewedFilesSetting.set(this.history.toObject());
      if (this.#currentFile) {
        PanelCommon.ExtensionServer.ExtensionServer.instance().sourceSelectionChanged(
  ...
  #showFile(uiSourceCode, userGesture) {
    ...
    try {
      ...
      const tabId =
        this.tabIds.get(canonicalSourceCode) ||
        this.appendFileTab(canonicalSourceCode, userGesture);
    ...
    if (
      this.currentView instanceof UISourceCodeFrame &&
      this.currentView.uiSourceCode() !== uiSourceCode
    ) {
  ...
  titleForFile(uiSourceCode) {
    ...
    let title = Platform.StringUtilities.trimMiddle(
      uiSourceCode.displayName(true),
  ...
  maybeCloseTab(id, nextTabId) {
    ...
    if (
      !shouldPrompt ||
      confirm(i18nString(UIStrings.areYouSureYouWantToCloseUnsaved, { PH1: uiSourceCode.name() }))
    ) {
  ...
  addUISourceCode(uiSourceCode) {
    ...
    const currentProjectIsSnippets = Snippets.ScriptSnippetFileSystem.isSnippetsUISourceCode(
      this.#currentFile,
    );
    const addedProjectIsSnippets =
      Snippets.ScriptSnippetFileSystem.isSnippetsUISourceCode(uiSourceCode);
    if (
      this.history.index(historyItemKey(this.#currentFile)) &&
      currentProjectIsSnippets &&
  ...
  tooltipForFile(uiSourceCode) {
    uiSourceCode =
      Persistence.Persistence.PersistenceImpl.instance().network(uiSourceCode) || uiSourceCode;
  ...
  restoreEditorProperties(editorView, selection, firstLineNumber) {
    const sourceFrame =
      editorView instanceof SourceFrame.SourceFrame.SourceFrameImpl ? editorView : null;
    if (!sourceFrame) {
      return;
    }
    if (selection) {
  ...
  tabClosed(event) {
    ...
    if (
      this.#currentFile &&
      this.#currentFile.canonicalScriptId() === uiSourceCode?.canonicalScriptId()
    ) {
      this.removeViewListeners();
  ...
  updateFileTitle(uiSourceCode) {
    ...
    if (tabId) {
      ...
      if (uiSourceCode.loadError()) {
        ...
        const icon =
          PanelCommon.PersistenceUtils.PersistenceUtils.iconForUISourceCode(uiSourceCode);
        this.tabbedPane.setTrailingTabIcon(tabId, icon);
      }
    }
  }
...
export class HistoryItem {
  ...
  static fromObject(serializedHistoryItem) {
    const resourceType = Common.ResourceType.ResourceType.fromName(
      serializedHistoryItem.resourceTypeName,
    );
