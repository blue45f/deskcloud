...
UI.ActionRegistration.registerActionExtension({
  ...
  contextTypes() {
    return maybeRetrieveContextTypes((Sources) => [
  ...
  bindings: [
    {
      shortcut: 'F8',
      keybindSets: ['devToolsDefault' /* UI.ActionRegistration.KeybindSet.DEVTOOLS_DEFAULT */],
    },
    {
...
UI.ActionRegistration.registerActionExtension({
  ...
  toggledIconClass:
    'breakpoint-crossed-filled' /* UI.ActionRegistration.IconClass.BREAKPOINT_CROSSED_FILLED */,
  toggleable: true,
  async loadActionDelegate() {
    const Sources = await loadSourcesModule();
    return new Sources.SourcesPanel.ActionDelegate();
...
Common.Revealer.registerRevealer({
  contextTypes() {
    return [Workspace.UISourceCode.UILocationRange];
  },
  destination: Common.Revealer.RevealerDestination.SOURCES_PANEL,
  async loadRevealer() {
    const Sources = await loadSourcesModule();
