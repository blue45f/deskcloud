...
const domNodeIsPauseOnExceptionsNode = (domNode) => {
  return (
    domNode.getAttribute('data-first-pause') !== null ||
    domNode.getAttribute('data-last-pause') !== null
  );
};
...
function findFirstDifferingSegmentIndex(splitUrls) {
  ...
  for (
    let segmentIndex = 0;
    segmentIndex < firstUrl.length && firstDifferingIndex === -1;
    ++segmentIndex
  ) {
...
function findDifferentiatingPath(url, allUrls, startIndex) {
  ...
  for (let segmentIndex = startIndex; segmentIndex < url.length; ++segmentIndex) {
    ...
    remainingUrlsToDifferentiate = remainingUrlsToDifferentiate.filter(
      (url) => url.length > segmentIndex && url[segmentIndex] === segment,
    );
...
function populateDifferentiatingPathMap(urls, urlToDifferentiator) {
  ...
  console.assert(
    new Set(urlToDifferentiator.values()).size === urls.length,
    'Differentiators should be unique.',
  );
}
