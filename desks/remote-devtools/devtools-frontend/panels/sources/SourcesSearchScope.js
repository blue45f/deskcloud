...
export class SourcesSearchScope {
  ...
  performIndexing(progress) {
    ...
    for (let i = 0; i < projects.length; ++i) {
      ...
      const projectProgress = compositeProgress.createSubProgress(
  ...
  projects() {
    ...
    return Workspace.Workspace.WorkspaceImpl.instance()
      ...
      .filter((project) => {
        ...
        if (
          !searchInAnonymousAndContentScripts &&
          project.type() === Workspace.Workspace.projectTypes.ContentScripts
        ) {
          return false;
        }
        if (
  ...
  performSearch(searchConfig, progress, searchResultCallback, searchFinishedCallback) {
    ...
    const findMatchingFilesProgress = new Common.Progress.CompositeProgress(
      compositeProgress.createSubProgress(),
    );
    for (const project of this.projects()) {
      const weight = [...project.uiSourceCodes()].length;
      const findMatchingFilesInProjectProgress =
      ...
      const promise = project
        ...
        .then(
          this.processMatchingFilesForProject.bind(
            this,
            this.searchId,
            project,
            searchConfig,
  ...
  processMatchingFilesForProject(
    searchId,
    project,
    searchConfig,
    filesMatchingFileQuery,
