        ...
        ${input.threads.map(
          (thread) => html`
            ...
            >
              ...
              <div class="thread-item-paused-state">
                ${thread.paused ? i18nString(UIStrings.paused) : ''}
              </div>
              ${thread.selected
                ? html`<devtools-icon
                    name="large-arrow-right-filled"
                    class="selected-thread-icon"
                  ></devtools-icon>`
                : nothing}
            </button>
          `,
...
export class ThreadsSidebarPane extends UI.Widget.VBox {
  ...
  static shouldBeShown() {
    return (
      SDK.TargetManager.TargetManager.instance().models(SDK.DebuggerModel.DebuggerModel).length >= 2
    );
  }
  wasShown() {
    super.wasShown();
  ...
  modelAdded(debuggerModel) {
    ...
    debuggerModel
      .runtimeModel()
      .addEventListener(SDK.RuntimeModel.Events.ExecutionContextChanged, this.requestUpdate, this);
