...
const str_ = i18n.i18n.registerUIStrings(
  'panels/sources/FilteredUISourceCodeListProvider.ts',
  UIStrings,
);
...
export class FilteredUISourceCodeListProvider extends QuickOpen.FilteredListWidget.Provider {
  ...
  itemScoreAt(itemIndex, query) {
    ...
    return (
      score + multiplier * (contentTypeBonus + this.scorer.calculateScore(fullDisplayName, null))
    );
  }
  renderItem(itemIndex, query) {
    ...
    // clang-format off
    return html` <style>
        ${filteredUISourceCodeListProviderStyles}
      </style>
      <div
        class="filtered-ui-source-code-list-item
      ...
      >
        ...
        <devtools-highlight
          ...
          class="filtered-ui-source-code-subtitle"
          title=${tooltipText}
        >
  ...
  renderSubtitleElement(text) {
    ...
    // clang-format off
    return html` <div class="first-part">${text.substring(0, splitPosition)}</div>
