...
export class NetworkNavigatorView extends NavigatorView {
  ...
  acceptProject(project) {
    return (
      project.type() === Workspace.Workspace.projectTypes.Network &&
...
export class FilesNavigatorView extends NavigatorView {
  ...
  constructor() {
    ...
    const placeholder = new UI.EmptyWidget.EmptyWidget(
      i18nString(UIStrings.noWorkspace),
    ...
    this.#automaticFileSystemNudge = uiI18n.getFormatLocalizedString(
      str_,
      UIStrings.automaticWorkspaceNudge,
      { PH1: link },
    );
  ...
  wasShown() {
    ...
    this.#eventListeners = [
      this.#automaticFileSystemManager.addEventListener(
        'AutomaticFileSystemChanged' /* Persistence.AutomaticFileSystemManager.Events.AUTOMATIC_FILE_SYSTEM_CHANGED */,
        this.#automaticFileSystemChanged,
        this,
      ),
      this.#automaticFileSystemManager.addEventListener(
        'AvailabilityChanged' /* Persistence.AutomaticFileSystemManager.Events.AVAILABILITY_CHANGED */,
        this.#availabilityChanged,
        this,
      ),
    ];
    this.#automaticFileSystemChanged({
      data: this.#automaticFileSystemManager.automaticFileSystem,
    });
  }
  willHide() {
  ...
  #availabilityChanged(event) {
    ...
    this.#automaticFileSystemNudge.hidden =
      automaticFileSystem !== null || availability !== 'available';
  }
}
...
export class OverridesNavigatorView extends NavigatorView {
  ...
  constructor() {
    ...
    Persistence.NetworkPersistenceManager.NetworkPersistenceManager.instance().addEventListener(
      'ProjectChanged' /* Persistence.NetworkPersistenceManager.Events.PROJECT_CHANGED */,
  ...
  updateUI() {
    ...
    if (project) {
      ...
      const clearButton = new UI.Toolbar.ToolbarButton(
        i18nString(UIStrings.clearConfiguration),
        'clear',
      );
...
export class SnippetsNavigatorView extends NavigatorView {
  constructor() {
    ...
    const placeholder = new UI.EmptyWidget.EmptyWidget(
      i18nString(UIStrings.noSnippets),
  ...
  handleFileContextMenu(event, node) {
    ...
    contextMenu
      .editSection()
      .appendItem(i18nString(UIStrings.rename), () => this.rename(node, false), {
        jslogContext: 'rename',
      });
    contextMenu
      .editSection()
    ...
    contextMenu
      .saveSection()
      .appendItem(i18nString(UIStrings.saveAs), this.handleSaveAs.bind(this, uiSourceCode), {
