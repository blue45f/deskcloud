...
export class DebuggerPlugin extends Plugin {
  ...
  constructor(uiSourceCode, transformer) {
    ...
    this.loader.addEventListener(
      'Update' /* SDK.PageResourceLoader.Events.UPDATE */,
      ...
      CodeMirror.lineNumbers({
        domEventHandlers: {
          click: (view, block, event) =>
            this.handleGutterClick(view.state.doc.lineAt(block.from), event),
        },
      }),
      breakpointMarkers,
      TextEditor.ExecutionPositionHighlighter.positionHighlighter(
        'cm-executionLine',
        'cm-executionToken',
      ),
  ...
  #openEditDialogForLine(line, isLogpoint) {
    ...
    const breakpoint =
      this.breakpoints.find((b) => b.position >= line.from && b.position <= line.to)?.breakpoint ||
      null;
  ...
  editorInitialized(editor) {
    ...
    // instance
    this.editor = editor;
    computeNonBreakableLines(editor.state, this.transformer, this.uiSourceCode).then(
      (linePositions) => {
        if (linePositions.length) {
          editor.dispatch({
            effects: SourceFrame.SourceFrame.addNonBreakableLines.of(linePositions),
          });
        }
      },
      console.error,
  ...
  showIgnoreListInfobarIfNeeded() {
    ...
    const infobar = new UI.Infobar.Infobar(
      'warning' /* UI.Infobar.Type.WARNING */,
  ...
  attachInfobar(bar) {
    if (this.editor) {
      this.editor.dispatch({
        effects: SourceFrame.SourceFrame.addSourceFrameInfobar.of({ element: bar.element }),
      });
    }
  }
  removeInfobar(bar) {
    if (this.editor && bar) {
      this.editor.dispatch({
        effects: SourceFrame.SourceFrame.removeSourceFrameInfobar.of({ element: bar.element }),
      });
    }
  }
  ...
  editBreakpointLocation({ breakpoint, uiLocation }) {
    const { lineNumber } = this.transformer.uiLocationToEditorLocation(
        ...
        () =>
          breakpoints.forEach((breakpoint) => {
            Host.userMetrics.actionTaken(
              Host.UserMetrics.Action.BreakpointRemovedFromGutterContextMenu,
            );
  ...
  populateTextAreaContextMenu(contextMenu) {
    function addSourceMapURL(scriptFile) {
      const dialog = AddDebugInfoURLDialog.createAddSourceMapURLDialog(
    ...
    function addDebugInfoURL(scriptFile) {
      const dialog = AddDebugInfoURLDialog.createAddDWARFSymbolsURLDialog(
        addDebugInfoURLDialogCallback.bind(this, scriptFile),
      );
      dialog.show();
    }
    ...
    if (
      this.uiSourceCode.project().type() === Workspace.Workspace.projectTypes.Network &&
      Common.Settings.Settings.instance().moduleSetting('js-source-maps-enabled').get() &&
      !Workspace.IgnoreListManager.IgnoreListManager.instance().isUserIgnoreListedURL(
      ...
      if (this.scriptFileForDebuggerModel.size) {
        ...
        contextMenu
          .debugSection()
          .appendItem(addSourceMapURLLabel, addSourceMapURL.bind(null, scriptFile), {
            jslogContext: 'add-source-map',
          });
        if (
          scriptFile.script?.isWasm() &&
  ...
  getPopoverRequest(event) {
    ...
    const debuggableFrame = UI.Context.Context.instance().flavor(
      StackTrace.StackTrace.DebuggableFrameFlavor,
    );
    if (!debuggableFrame) {
      return null;
    ...
    if (
      !positionCoords ||
      event.clientY < positionCoords.top ||
      event.clientY > positionCoords.bottom ||
      event.clientX < positionCoords.left - 30 ||
      event.clientX > positionCoords.right + 30
    ) {
      return null;
    }
    if (
      event.clientX < positionCoords.left &&
      textPosition > editor.state.doc.lineAt(textPosition).from
    ) {
      textPosition -= 1;
    }
    const highlightRange = computePopoverHighlightRange(
      editor.state,
    ...
    return {
      ...
      show: async (popover) => {
        ...
        if (selectedCallFrame.script.isJavaScript()) {
          const nameMap =
            await SourceMapScopes.NamesResolver.allVariablesInCallFrame(selectedCallFrame);
          try {
            resolvedText =
              await Formatter.FormatterWorkerPool.formatterWorkerPool().javaScriptSubstitute(
                evaluationText,
                nameMap,
              );
          } catch {}
        ...
        const potentiallyUpdatedCallFrame = UI.Context.Context.instance().flavor(
          StackTrace.StackTrace.DebuggableFrameFlavor,
        );
        ...
        const decoration = CodeMirror.Decoration.set(
          evalExpressionMark.range(highlightRange.from, highlightRange.to),
        );
  ...
  editBreakpointCondition(breakpointEditRequest) {
    ...
    const isRepeatedEditRequest =
      this.#activeBreakpointEditRequest &&
    ...
    ((dialog.oldCondition = oldCondition),
      (dialog.breakpointType = isLogpointForDialog
        ? 'LOGPOINT' /* SDK.DebuggerModel.BreakpointType.LOGPOINT */
    ...
    editor.dispatch({
      effects: CodeMirror.StateEffect.appendConfig.of(
        compartment.of(
          CodeMirror.EditorView.decorations.of(
            CodeMirror.Decoration.set([
  ...
  async updateValueDecorations() {
    ...
    if (decorations || this.editor.state.field(valueDecorations.field).size) {
      this.editor.dispatch({
        effects: valueDecorations.update.of(decorations || CodeMirror.Decoration.none),
      });
    }
  }
  ...
  async computeValueDecorations() {
    ...
    const functionOffsetPromise = this.#rawLocationToEditorOffset(
      callFrame.functionLocation(),
      url,
    );
    const executionOffsetPromise = this.#rawLocationToEditorOffset(callFrame.location(), url);
    const [functionOffset, executionOffset] = await Promise.all([
    ...
    if (
      functionOffset >= executionOffset ||
      executionOffset - functionOffset > MAX_CODE_SIZE_FOR_VALUE_DECORATIONS
    ) {
      return null;
    }
    ...
    for (const [line, names] of variablesByLine) {
      ...
      let newNames = prevLine
        ? Array.from(names).filter((n) => prevLine.get(n[0]) !== n[1])
        : Array.from(names);
      if (!newNames.length) {
        continue;
      }
      ...
      decorations.push(
        CodeMirror.Decoration.widget({ widget: new ValueDecoration(newNames), side: 1 }).range(
          this.editor.state.doc.line(line + 1).to,
        ),
      );
    }
  ...
  async showContinueToLocations() {
    ...
    for (const location of locations.reverse()) {
      ...
      if (syntaxNode.firstChild || syntaxNode.from < line.from || syntaxNode.to > line.to) {
        // Only use leaf nodes within the line
        continue;
      }
      if (syntaxNode.name === '.') {
      ...
      this.continueToLocations.push({
        ...
        to: syntaxNode.to,
        async: false,
        click: () => location.continueToLocation(),
      });
      if (location.type === 'call' /* Protocol.Debugger.BreakLocationType.Call */) {
        previousCallLine = editorLocation.lineNumber;
      }
      const identifierName = validKeyword
        ? ''
        : line.text.slice(syntaxNode.from - line.from, syntaxNode.to - line.from);
      let asyncCall = null;
      ...
      if (syntaxType === 'new') {
        ...
        if (
          ...
          state.sliceDoc(callee.from, callee.to) === 'Worker'
        ) {
          asyncCall = syntaxNode.parent;
        }
      }
      if (
        asyncCall &&
        (asyncCall.name === 'CallExpression' || asyncCall.name === 'NewExpression') &&
        location.type === 'call' /* Protocol.Debugger.BreakLocationType.Call */
      ) {
  ...
  fetchBreakpoints() {
    ...
    const { editor } = this;
    const breakpointLocations = this.breakpointManager.breakpointLocationsForUISourceCode(
      this.uiSourceCode,
    );
  ...
  lineBreakpoints(line) {
    return this.breakpoints
      .filter((b) => b.position >= line.from && b.position <= line.to)
      .map((b) => b.breakpoint);
  }
  ...
  async computeBreakpointDecoration(state, breakpoints) {
    ...
    for (const { position, breakpoint } of breakpoints) {
      ...
      if (breakpoint.enabled() && forThisLine.every((b) => !b.enabled())) {
        ...
        possibleBreakpointRequests.push(
          this.linePossibleBreakpoints(line).then((locations) =>
            addPossibleBreakpoints(line, locations),
          ),
        );
      }
    ...
    for (const [lineStart, lineBreakpoints] of breakpointsByLine) {
      ...
      gutterMarkers.push(
        new BreakpointGutterMarker(gutterClass, lineStart, main.condition()).range(lineStart),
      );
    }
    ...
    for (const [linePos, inlineMarkers] of inlineMarkersByLine) {
      if (inlineMarkers.length > 1) {
        for (const { column, breakpoint } of inlineMarkers) {
          ...
          decorations.push(
            CodeMirror.Decoration.widget({ widget: marker, side: -1 }).range(linePos + column),
          );
        }
      }
    }
    return {
      content: CodeMirror.Decoration.set(decorations, true),
      gutter: CodeMirror.RangeSet.of(gutterMarkers, true),
    };
  }
  ...
  async restoreBreakpointsAfterEditing() {
    ...
    this.breakpoints = [];
    await Promise.all(
      breakpoints.map(async (description) => {
        ...
        const uiLocation = this.transformer.editorLocationToUILocation(
  ...
  async refreshBreakpoints() {
    if (this.editor) {
      ...
      if (
        this.editor &&
        this.breakpoints === forBreakpoints &&
  ...
  onInlineBreakpointMarkerContextMenu(event, breakpoint) {
    ...
    if (
      !SourceFrame.SourceFrame.isBreakableLine(editor.state, line) ||
      // Editing breakpoints only make sense for conditional breakpoints
      // and logpoints.
      !Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().supportsConditionalBreakpoints(
        this.uiSourceCode,
  ...
  updateScriptFile(debuggerModel) {
    ...
    if (oldScriptFile) {
      oldScriptFile.removeEventListener(
        'DidMergeToVM' /* Bindings.ResourceScriptMapping.ResourceScriptFile.Events.DID_MERGE_TO_VM */,
        this.didMergeToVM,
        this,
      );
      oldScriptFile.removeEventListener(
        'DidDivergeFromVM' /* Bindings.ResourceScriptMapping.ResourceScriptFile.Events.DID_DIVERGE_FROM_VM */,
    ...
    void newScriptFile.missingSymbolFiles().then((resources) => {
      if (resources) {
        const details = i18nString(UIStrings.debugInfoNotFound, {
  ...
  updateMissingDebugInfoInfobar(warning) {
    ...
    this.missingDebugInfoBar = UI.Infobar.Infobar.create(
      'error' /* UI.Infobar.Type.ERROR */,
      warning.details,
      [],
      undefined,
      'missing-debug-info',
    ...
    for (const resource of warning.resources) {
      const detailsRow = this.missingDebugInfoBar?.createDetailsRowMessage(
        i18nString(UIStrings.debugFileNotFound, {
          PH1: Common.ParsedURL.ParsedURL.extractName(resource.resourceUrl),
        }),
      );
      if (detailsRow) {
        const pageResourceKey = SDK.PageResourceLoader.PageResourceLoader.makeExtensionKey(
        ...
        if (
          ...
          const showRequest = UI.UIUtils.createTextButton(
            i18nString(UIStrings.showRequest),
            () => {
      ...
      if (url) {
        ...
        if (resolvedUrl) {
          const resource = resourceMap.get(
            SDK.PageResourceLoader.PageResourceLoader.makeKey(
              resolvedUrl,
  ...
  showSourceMapInfobarIfNeeded() {
    ...
    if (!resource) {
      ...
      this.sourceMapInfobar.createDetailsRowMessage(
        i18nString(UIStrings.associatedFilesShouldBeAdded),
      );
      this.sourceMapInfobar.createDetailsRowMessage(
        i18nString(UIStrings.associatedFilesAreAvailable, {
          PH1: String(
      ...
      if (resource.errorMessage) {
        this.sourceMapInfobar.createDetailsRowMessage(
          i18nString(UIStrings.errorLoading, {
            PH1: Platform.StringUtilities.trimMiddle(
              resource.url,
              UI.UIUtils.MaxLengthForDisplayedURLs,
            ),
  ...
  async callFrameChanged() {
    const frameFlavor = UI.Context.Context.instance().flavor(
      StackTrace.StackTrace.DebuggableFrameFlavor,
    );
    if (
      frameFlavor?.frame.uiSourceCode?.canonicalScriptId() === this.uiSourceCode.canonicalScriptId()
    ) {
      const uiLocation = new Workspace.UISourceCode.UILocation(
      ...
      if (frameFlavor.frame.missingDebugInfo) {
        this.updateMissingDebugInfoInfobar(
          convertMissingDebugInfo(
            frameFlavor.frame.missingDebugInfo,
            frameFlavor.sdkFrame.functionName,
          ),
        );
      } else {
        this.updateMissingDebugInfoInfobar(null);
  ...
  setExecutionLocation(executionLocation) {
    ...
    if (executionLocation) {
      const editorLocation = this.transformer.uiLocationToEditorLocation(
        ...
        executionLocation.columnNumber,
      );
  ...
  dispose() {
    ...
    Workspace.IgnoreListManager.IgnoreListManager.instance().removeChangeListener(
      this.ignoreListCallback,
    );
...
function muteGutterMarkers(markers, doc) {
  ...
  markers.between(0, doc.length, (from, _to, marker) => {
    ...
    newMarkers.push(
      new BreakpointGutterMarker(
        className,
        from,
        marker instanceof BreakpointGutterMarker ? marker.condition : undefined,
      ).range(from),
    );
  });
  ...
  update(deco, tr) {
    ...
    for (const effect of tr.effects) {
      if (effect.is(setBreakpointDeco)) {
        ...
        deco = {
          content: CodeMirror.RangeSet.empty,
...
const asyncContinueToMark = CodeMirror.Decoration.mark({
...
// execution line.
const markIfContinueTo = CodeMirror.EditorView.contentAttributes.compute(
...
class ValueDecoration extends CodeMirror.WidgetType {
  ...
  eq(other) {
    return (
      this.pairs.length === other.pairs.length &&
...
export async function computeScopeMappings(callFrame, rawLocationToEditorOffset) {
  ...
  for (const scope of callFrame.scopeChain()) {
    ...
    const { properties } = await SourceMapScopes.NamesResolver.resolveScopeInObject(
      scope,
    ...
    case 'application/wasm': {
      ...
      for (let parent = node.parent; parent; parent = parent.parent) {
        if (parent.name === 'App') {
          ...
          const opName =
            firstChild?.name === 'Keyword' && state.sliceDoc(firstChild.from, firstChild.to);
