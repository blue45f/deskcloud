...
export class CoveragePlugin extends Plugin {
  ...
  getCoverageManager() {
    return this.uiSourceCode.getDecorationData(
      'coverage' /* Workspace.UISourceCode.DecoratorType.COVERAGE */,
    );
  }
  ...
  startDecoUpdate(editor) {
    ...
    void (
      manager
        ? manager.usageByLine(this.uiSourceCode, this.#editorLines(editor))
        : Promise.resolve([])
    ).then((usageByLine) => {
  ...
  #editorLines(editor) {
    ...
    for (let n = 1; n <= editor.state.doc.lines; ++n) {
      ...
      const { lineNumber: startLine, columnNumber: startColumn } =
        this.#transformer.editorLocationToUILocation(n - 1, 0);
...
const coverageState = CodeMirror.StateField.define({
  ...
  update(markers, tr) {
    return tr.effects.reduce((markers, effect) => {
      return effect.is(setCoverageState)
        ? markersFromCoverageData(effect.value, tr.state)
        : markers;
