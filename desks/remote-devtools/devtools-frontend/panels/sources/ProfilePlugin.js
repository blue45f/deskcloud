...
function markersFromProfileData(map, state, type) {
  const markerType =
    type === 'performance' /* Workspace.UISourceCode.DecoratorType.PERFORMANCE */
      ? PerformanceMarker
      : MemoryMarker;
  const markers = [];
...
const makeLineLevelProfilePlugin = (type) =>
  class ProfilePlugin extends Plugin {
      ...
      this.field = CodeMirror.StateField.define({
        ...
        update: (markers, tr) => {
          return tr.effects.reduce((markers, effect) => {
            return effect.is(this.updateEffect)
              ? markersFromProfileData(effect.value, tr.state, type)
              : markers;
    ...
    getLineMap() {
      ...
      return Workspace.UISourceCode.createMappedProfileData(
        uiSourceCodeProfileMap,
        (line, column) => {
    ...
    editorExtension() {
      ...
      return this.compartment.of(
        !map
          ? []
          : [
              this.field.init((state) => markersFromProfileData(map, state, type)),
              this.gutter,
              theme,
            ],
      );
...
export const MemoryProfilePlugin = makeLineLevelProfilePlugin(
  'memory' /* Workspace.UISourceCode.DecoratorType.MEMORY */,
);
