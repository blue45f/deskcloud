...
export class ProfileHeader extends Common.ObjectWrapper.ObjectWrapper {
  ...
  updateStatus(subtitle, wait) {
    this.dispatchEventToListeners(
      'UpdateStatus' /* Events.UPDATE_STATUS */,
      new StatusUpdate(subtitle, wait),
    );
  }
  removeTempFile() {
    if (this.tempFile) {
...
export class ProfileType extends Common.ObjectWrapper.ObjectWrapper {
  ...
  addProfile(profile) {
    ...
    this.dispatchEventToListeners(
      'add-profile-header' /* ProfileEvents.ADD_PROFILE_HEADER */,
      profile,
    );
  }
  ...
  disposeProfile(profile) {
    this.dispatchEventToListeners(
      'remove-profile-header' /* ProfileEvents.REMOVE_PROFILE_HEADER */,
      profile,
    );
    profile.dispose();
