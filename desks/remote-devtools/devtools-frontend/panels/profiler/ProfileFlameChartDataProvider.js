...
export class ProfileFlameChartDataProvider {
  ...
  entryColor(entryIndex) {
    ...
    return this.#colorGenerator.colorForID(
...
export class ProfileFlameChart extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(searchableView, dataProvider) {
    ...
    this.mainPane.addEventListener(
    ...
    this.mainPane.addEventListener(
      'CanvasFocused' /* PerfUI.FlameChart.Events.CANVAS_FOCUSED */,
      this.onEntrySelected,
      this,
    );
    this.overviewPane.addEventListener(
      'WindowChanged' /* OverviewPaneEvents.WINDOW_CHANGED */,
  ...
  performSearch(searchConfig, _shouldJump, jumpBackwards) {
    const matcher = Platform.StringUtilities.createPlainTextSearchRegex(
      searchConfig.query,
      searchConfig.caseSensitive ? '' : 'i',
    );
    const selectedEntryIndex =
      this.searchResultIndex !== -1 ? this.searchResults[this.searchResultIndex] : -1;
    this.searchResults = [];
...
export class OverviewPane extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(dataProvider) {
    ...
    this.overviewGrid = new PerfUI.OverviewGrid.OverviewGrid(
    ...
    this.overviewGrid.addEventListener(
      'WindowChangedWithPosition' /* PerfUI.OverviewGrid.Events.WINDOW_CHANGED_WITH_POSITION */,
      this.onWindowChanged,
      this,
    );
  }
  ...
  selectRange(timeLeft, timeRight) {
    ...
    this.overviewGrid.setWindowRatio(
  ...
  drawOverviewCanvas() {
    ...
    context.lineTo(
      -lineWidth,
      Math.round(canvasHeight - drawData[0] * yScaleFactor - offsetFromBottom),
    );
    let value = 0;
  ...
  calculateDrawData(width) {
    ...
    for (let entryIndex = 0; entryIndex < length; ++entryIndex) {
      ...
      const finish = Math.floor(
        (entryStartTimes[entryIndex] - minimumBoundary + entryTotalTimes[entryIndex]) * scaleFactor,
      );
