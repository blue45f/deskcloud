...
export class HeapTimelineOverview extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  drawOverviewCanvas(width, height) {
    ...
    if (gridValue) {
      ...
      context.font =
        '10px ' + globalThis.getComputedStyle(this.element, null).getPropertyValue('font-family');
  ...
  updateGrid() {
    ...
    const minIndex = Platform.ArrayUtilities.lowerBound(
      timestamps,
      timeLeft,
      Platform.ArrayUtilities.DEFAULT_COMPARATOR,
    );
    const maxIndex = Platform.ArrayUtilities.upperBound(
      timestamps,
      timeRight,
      Platform.ArrayUtilities.DEFAULT_COMPARATOR,
    );
    ...
    this.dispatchEventToListeners('IdsRangeChanged' /* Events.IDS_RANGE_CHANGED */, {
      minId,
      maxId,
      size,
    });
...
export class SmoothScale {
  ...
  nextScale(target) {
    ...
    if (this.currentScale) {
      ...
      this.currentScale *= Platform.NumberUtilities.clamp(
        scaleChange,
        1 / maxChangePerDelta,
        maxChangePerDelta,
      );
