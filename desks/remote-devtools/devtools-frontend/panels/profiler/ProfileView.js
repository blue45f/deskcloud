...
export class ProfileView extends UI.View.SimpleView {
  ...
  constructor() {
    ...
    this.dataGrid.addEventListener(
      ...
      this.nodeSelected.bind(this, true),
    );
    this.dataGrid.addEventListener(
      'DeselectedNode' /* DataGrid.DataGrid.Events.DESELECTED_NODE */,
      this.nodeSelected.bind(this, false),
    );
    ...
    this.focusButton.addEventListener(
    ...
    this.excludeButton.addEventListener(
      'Click' /* UI.Toolbar.ToolbarButton.Events.CLICK */,
      this.excludeClicked,
      this,
    );
  ...
  initialize(nodeFormatter) {
    ...
    this.viewType = Common.Settings.Settings.instance().createSetting(
      'profile-view',
    ...
    const options = new Map(
      viewTypes.map((type) => [
        type,
        this.viewSelectComboBox.createOption(optionNames.get(type), type),
      ]),
    );
  ...
  sortProfile() {
    ...
    this.profileDataGridTree.sort(
      ProfileDataGridTree.propertyComparator(sortProperty, sortAscending),
      false,
    );
    this.refresh();
  }
...
export class WritableProfileHeader extends ProfileHeader {
  ...
  onChunkTransferred(_reader) {
    if (this.jsonifiedProfile) {
      ...
      this.updateStatus(
        i18nString(UIStrings.loadingD, {
          PH1: i18n.ByteUtilities.bytesToString(this.jsonifiedProfile.length),
        }),
      );
    }
  }
  onError(reader) {
    ...
    if (error) {
      this.updateStatus(
        i18nString(UIStrings.fileSReadErrorS, { PH1: reader.fileName(), PH2: error.message }),
      );
    }
  }
  async write(text) {
  ...
  async loadFromFile(file) {
    ...
    const fileReader = new Bindings.FileUtils.ChunkedFileReader(
      file,
      10000000,
