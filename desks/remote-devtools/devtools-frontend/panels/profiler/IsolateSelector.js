...
export class IsolateSelector extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    UI.Tooltip.Tooltip.install(
      this.totalValueDiv,
      i18nString(UIStrings.totalPageJsHeapSizeAcrossAllVm),
    );
  ...
  wasShown() {
    ...
    SDK.IsolateManager.IsolateManager.instance().addEventListener(
      'MemoryChanged' /* SDK.IsolateManager.Events.MEMORY_CHANGED */,
  ...
  isolateAdded(isolate) {
    ...
    const index =
      item.model().target() === SDK.TargetManager.TargetManager.instance().primaryPageTarget()
        ? 0
        : this.items.length;
  ...
  updateTitle() {
    ...
    for (const model of this.isolate.models()) {
      ...
      const title = target.decorateLabel(
        domain && !isPrimaryPageTarget
          ? `${domain}: ${name}`
          : name || domain || i18nString(UIStrings.empty),
      );
