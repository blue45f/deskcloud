...
export class ProfileLauncherView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(profilesPanel) {
    ...
    this.selectedProfileTypeSetting = Common.Settings.Settings.instance().createSetting(
    ...
    const isolateSelectorElement = this.#contentElement.createChild(
      'div',
      'vbox profile-isolate-selector-block',
    );
    isolateSelectorElement.createChild('h1').textContent = i18nString(
      UIStrings.selectJavascriptVmInstance,
    );
    const isolateSelector = new IsolateSelector();
    const isolateSelectorElementChild = isolateSelectorElement.createChild(
      'div',
      'vbox profile-launcher-target-list',
    );
  ...
  loadButtonClicked() {
    const loadFromFileAction =
      UI.ActionRegistry.ActionRegistry.instance().getAction('profiler.load-from-file');
  ...
  updateControls() {
    ...
    UI.Tooltip.Tooltip.install(
      this.controlButton,
      this.recordButtonEnabled ? '' : UI.UIUtils.anotherProfilerActiveLabel(),
    );
  ...
  addProfileType(profileType) {
    const { radio, label } = UI.UIUtils.createRadioButton(
      'profile-type',
      profileType.name,
    ...
    const headerText =
      this.typeIdToOptionElementAndProfileType.size > 1
        ? i18nString(UIStrings.selectProfilingType)
        : profileType.name;
  ...
  profileTypeChanged(profileType) {
    ...
    this.dispatchEventToListeners(
      'ProfileTypeSelected' /* Events.PROFILE_TYPE_SELECTED */,
      profileType,
    );
    this.isInstantProfile = profileType.isInstantProfile();
    this.isEnabled = profileType.isEnabled();
