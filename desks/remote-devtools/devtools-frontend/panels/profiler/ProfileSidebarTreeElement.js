...
export class ProfileSidebarTreeElement extends UI.TreeOutline.TreeElement {
  ...
  constructor(dataDisplayDelegate, profile, className) {
    ...
    this.titlesElement.setAttribute(
      'jslog',
      `${VisualLogging.value('title').track({ dblclick: true, change: true })}`,
    );
    ...
    this.menuElement.setAttribute(
      'jslog',
      `${VisualLogging.dropDown('profile-options').track({ click: true })}`,
    );
    ...
    profile.addEventListener(
      'UpdateStatus' /* ProfileHeaderEvents.UPDATE_STATUS */,
      this.updateStatus,
      this,
    );
    this.editing = null;
  ...
  updateStatus(event) {
    ...
    if (statusUpdate.subtitle !== null) {
      this.subtitleElement.textContent =
        statusUpdate.subtitle.length > 0 ? `(${statusUpdate.subtitle})` : '';
      this.titlesElement.classList.toggle('no-subtitle', !statusUpdate.subtitle);
      UI.ARIAUtils.setLabel(
        this.listItemElement,
        `${this.profile.title}, ${statusUpdate.subtitle}`,
      );
    }
