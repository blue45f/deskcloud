...
export class HeapSnapshotGridNode extends Common.ObjectWrapper.eventMixin(
  ...
  createValueCell(columnId) {
    ...
    if (dataGrid.snapshot && dataGrid.snapshot.totalSize !== 0) {
      ...
      if (percent) {
        render(
          html` <div class="profile-multiple-values">
  ...
  populateChildren(fromPosition, toPosition) {
    return new Promise((resolve) => {
      ...
      function serializeNextChunk(toPosition) {
        ...
        const end = Math.min(
          firstNotSerializedPosition + this.dataGridInternal.defaultPopulateCount(),
          toPosition,
        );
        void this.provider()
      ...
      function childrenRetrieved(itemsRange, toPosition) {
        ...
        if (
          firstNotSerializedPosition < toPosition &&
          firstNotSerializedPosition < itemsRange.totalLength
        ) {
          serializeNextChunk.call(this, toPosition);
          return;
        }
...
export class HeapSnapshotGenericObjectNode extends HeapSnapshotGridNode {
  ...
  async appendSourceLocation(div) {
    ...
    const link = await this.dataGridInternal
      .dataDisplayDelegate()
  ...
  async queryObjectContent(heapProfilerModel, objectGroupName) {
    ...
    return (
      ...
      heapProfilerModel
        .runtimeModel()
        .createRemoteObjectFromPrimitiveValue(i18nString(UIStrings.previewIsNotAvailable))
    );
  }
  async tryQueryObjectContent(heapProfilerModel, objectGroupName) {
    if (this.type === 'string') {
      return heapProfilerModel
        .runtimeModel()
        .createRemoteObjectFromPrimitiveValue(this.nameInternal);
    }
  ...
  populateContextMenu(contextMenu, dataDisplayDelegate, heapProfilerModel) {
    if (this.shallowSize !== 0) {
      contextMenu.revealSection().appendItem(
        i18nString(UIStrings.revealInSummaryView),
        () => {
          dataDisplayDelegate.showObject(
            String(this.snapshotNodeId),
            i18nString(UIStrings.summary),
          );
        },
    ...
    if (this.referenceName) {
      for (const match of this.referenceName.matchAll(
        /\((?<objectName>[^@)]*) @(?<snapshotNodeId>\d+)\)/g,
      )) {
        const { objectName, snapshotNodeId } = match.groups;
        contextMenu.revealSection().appendItem(
          i18nString(UIStrings.revealObjectSWithIdSInSummary, {
            PH1: objectName,
            PH2: snapshotNodeId,
          }),
          () => {
...
export class HeapSnapshotRetainingObjectNode extends HeapSnapshotObjectNode {
  ...
  populateContextMenu(contextMenu, dataDisplayDelegate, heapProfilerModel) {
    ...
    if (this.#ignored) {
      contextMenu.revealSection().appendItem(
        i18nString(UIStrings.stopIgnoringThisRetainer),
        async () => {
      ...
      contextMenu.revealSection().appendItem(
        i18nString(UIStrings.ignoreThisRetainer),
        async () => {
  ...
  expandRetainersChain(maxExpandLevels) {
    if (!this.populated) {
      void this.once(HeapSnapshotGridNode.Events.PopulateComplete).then(() =>
        this.expandRetainersChain(maxExpandLevels),
      );
      this.populate();
      return;
    }
    ...
    this.dataGridInternal.dispatchEventToListeners(
      HeapSnapshotSortableDataGridEvents.ExpandRetainersComplete,
    );
  }
  comparator() {
...
export class HeapSnapshotConstructorNode extends HeapSnapshotGridNode {
  ...
  createCell(columnId) {
    const cell =
      columnId === 'object' ? super.createCell(columnId) : this.createValueCell(columnId);
...
export class HeapSnapshotDiffNode extends HeapSnapshotGridNode {
  ...
  constructor(dataGrid, classKey, diffForClass) {
    ...
    this.data = {
      ...
      sizeDelta:
        ...
        i18n.ByteUtilities.bytesToString(Math.abs(this.sizeDelta)),
    };
  }
  get name() {
    return this.nameInternal;
  ...
  createProvider() {
    ...
    const addedNodesProvider = tree.snapshot.createAddedNodesProvider(
      tree.baseSnapshot.uid,
      this.classKey,
    );
...
export class AllocationGridNode extends HeapSnapshotGridNode {
  ...
  async doPopulate() {
    this.populated = true;
    const callers = await this.dataGridInternal.snapshot.allocationNodeCallers(
