...
const moduleUIstr_ = i18n.i18n.registerUIStrings(
  'panels/profiler/ModuleUIStrings.ts',
  ModuleUIStrings.UIStrings,
);
...
export class HeapSnapshotView extends UI.View.SimpleView {
  ...
  constructor(dataDisplayDelegate, profile, registry) {
    ...
    profileType.addEventListener(
      'SnapshotReceived' /* HeapSnapshotProfileTypeEvents.SNAPSHOT_RECEIVED */,
      this.onReceiveSnapshot,
      this,
    );
    profileType.addEventListener(
      'remove-profile-header' /* ProfileTypeEvents.REMOVE_PROFILE_HEADER */,
    ...
    const hasAllocationStacks = registry.trackingHeapSnapshotProfileType
    ...
    this.containmentDataGrid = new HeapSnapshotContainmentDataGrid(
      heapProfilerModel,
      this,
      /* displayName */ i18nString(UIStrings.containment),
    );
    this.containmentDataGrid.addEventListener(
      'SelectedNode' /* DataGrid.DataGrid.Events.SELECTED_NODE */,
    ...
    if (this.allocationStackView) {
      ...
      this.tabbedPane.appendTab(
        'retainers',
        i18nString(UIStrings.retainers),
        this.retainmentWidget,
      );
      this.tabbedPane.appendTab(
        'allocation-stack',
        i18nString(UIStrings.allocationStack),
    ...
    this.resetRetainersButton = new UI.Toolbar.ToolbarButton(
      restoreIgnoredRetainers,
      'clear-list',
      restoreIgnoredRetainers,
    );
    this.resetRetainersButton.setVisible(false);
    this.resetRetainersButton.addEventListener(
    ...
    for (const existingProfile of this.profiles()) {
      existingProfile.addEventListener(
        'ProfileTitleChanged' /* ProfileHeaderEvents.PROFILE_TITLE_CHANGED */,
        this.updateControls,
        this,
      );
    }
  ...
  createOverview() {
    ...
    this.trackingOverviewGrid.addEventListener(
      'IdsRangeChanged' /* Events.IDS_RANGE_CHANGED */,
      this.onIdsRangeChanged.bind(this),
    );
    if (!this.profile.fromFile() && profileType.profileBeingRecorded() === this.profile) {
      profileType.addEventListener(
        'HeapStatsUpdate' /* TrackingHeapSnapshotProfileTypeEvents.HEAP_STATS_UPDATE */,
        this.onHeapStatsUpdate,
        this,
      );
      profileType.addEventListener(
        'TrackingStopped' /* TrackingHeapSnapshotProfileTypeEvents.TRACKING_STOPPED */,
        this.onStopTracking,
        this,
      );
  ...
  async linkifyObject(nodeIndex) {
    ...
    return sourceURL && this.linkifier
      ? this.linkifier.linkifyRawLocation(rawLocation, sourceURL)
      : null;
  }
  async populate() {
    ...
    if (
      this.profile.profileType().id === TrackingHeapSnapshotProfileType.TypeId &&
      ...
      if (samples) {
        ...
        profileSamples.totalTime = Math.max(
          samples.timestamps[samples.timestamps.length - 1] || 0,
          10000,
        );
        if (this.trackingOverviewGrid) {
  ...
  async retrieveStatistics(heapSnapshotProxy) {
    ...
    const otherJSObjectsSize =
      v8heap.total - v8heap.code - v8heap.strings - v8heap.jsArrays - v8heap.system;
    const records = [
      { value: v8heap.code, color: 'var(--app-color-code)', title: i18nString(UIStrings.code) },
      {
        value: v8heap.strings,
        color: 'var(--app-color-strings)',
        title: i18nString(UIStrings.strings),
      },
      {
        value: v8heap.jsArrays,
        color: 'var(--app-color-js-arrays)',
        title: i18nString(UIStrings.jsArrays),
      },
      {
        value: native.typedArrays,
        color: 'var(--app-color-typed-arrays)',
        title: i18nString(UIStrings.typedArrays),
      },
      {
        value: v8heap.system,
        color: 'var(--app-color-system)',
  ...
  onIdsRangeChanged(event) {
    ...
    this.selectedSizeText.setText(
      i18nString(UIStrings.selectedSizeS, {
        PH1: i18n.ByteUtilities.bytesToString(event.data.size),
      }),
    );
  ...
  async performSearchInternal(nextQuery) {
    ...
    this.searchResults = filter
      ? await this.profile.snapshotProxy.search(this.currentSearch, filter)
      : [];
  ...
  jumpToNextSearchResult() {
    ...
    void this.searchThrottler.schedule(
  ...
  async jumpToSearchResult(searchResultIndex) {
    ...
    const node = await this.dataGrid.revealObjectByHeapSnapshotId(
      String(this.searchResults[searchResultIndex]),
    );
    this.selectRevealedNode(node);
  }
  refreshVisibleData() {
  ...
  static ALWAYS_AVAILABLE_FILTERS = [
    ...
    {
      uiName: i18nString(UIStrings.objectsRetainedByEventHandlers),
      filterName: 'objectsRetainedByEventHandlers',
    },
  ];
  changeFilter() {
    ...
    if (selectedIndex >= indexOfFirstAlwaysAvailableFilter) {
      filterName =
        HeapSnapshotView.ALWAYS_AVAILABLE_FILTERS[selectedIndex - indexOfFirstAlwaysAvailableFilter]
          .filterName;
      selectedIndex = 0;
    }
  ...
  setSelectedNodeForDetailsView(nodeItem) {
    ...
    if (dataSource) {
      ...
      if (this.allocationStackView) {
        void this.allocationStackView.setAllocatedObject(
          dataSource.snapshot,
          dataSource.snapshotNodeIndex,
        );
      }
  ...
  async changePerspectiveAndWait(perspectiveTitle) {
    const perspectiveIndex = this.perspectives.findIndex(
    ...
    const option = this.perspectiveSelect
      .options()
      .find((option) => option.value === String(perspectiveIndex));
  ...
  getPopoverRequest(event) {
    ...
    const node =
      this.dataGrid.dataGridNodeFromNode(row) ||
      this.containmentDataGrid.dataGridNodeFromNode(row) ||
      this.constructorsDataGrid.dataGridNodeFromNode(row) ||
      this.diffDataGrid.dataGridNodeFromNode(row) ||
      this.allocationDataGrid?.dataGridNodeFromNode(row) ||
    ...
    return {
      ...
      show: async (popover) => {
        ...
        if (remoteObject instanceof SDK.RemoteObject.RemoteObject) {
          ...
          objectPopoverHelper =
            ObjectUI.ObjectPopoverHelper.ObjectPopoverHelper.buildDescriptionPopover(
...
export class AllocationPerspective extends Perspective {
  ...
  constructor() {
    ...
    this.allocationSplitWidget = new UI.SplitWidget.SplitWidget(
      false,
      true,
...
export class HeapSnapshotProfileType extends Common.ObjectWrapper.eventMixin(ProfileType) {
  ...
  constructor(id, title) {
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.HeapProfilerModel.HeapProfilerModel,
      'ResetProfiles' /* SDK.HeapProfilerModel.Events.RESET_PROFILES */,
      this.resetProfiles,
      this,
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.HeapProfilerModel.HeapProfilerModel,
      'AddHeapSnapshotChunk' /* SDK.HeapProfilerModel.Events.ADD_HEAP_SNAPSHOT_CHUNK */,
      this.addHeapSnapshotChunk,
      this,
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.HeapProfilerModel.HeapProfilerModel,
      'ReportHeapSnapshotProgress' /* SDK.HeapProfilerModel.Events.REPORT_HEAP_SNAPSHOT_PROGRESS */,
      this.reportHeapSnapshotProgress,
      this,
    );
    this.exposeInternals = Common.Settings.Settings.instance().createSetting(
      'expose-internals',
      false,
    );
  ...
  customContent() {
    const exposeInternalsInHeapSnapshotCheckbox = SettingsUI.SettingsUI.createSettingCheckbox(
      i18nString(UIStrings.exposeInternals),
  ...
  async takeHeapSnapshot() {
    ...
    const heapProfilerModel = UI.Context.Context.instance().flavor(
      SDK.HeapProfilerModel.HeapProfilerModel,
    );
    if (!heapProfilerModel) {
      return;
    ...
    this.dispatchEventToListeners(
      'profile-complete' /* ProfileTypeEvents.PROFILE_COMPLETE */,
      profile,
    );
  }
  ...
  reportHeapSnapshotProgress(event) {
    ...
    profile.updateStatus(
      i18nString(UIStrings.percentagePlaceholder, { PH1: ((done / total) * 100).toFixed(0) }),
      true,
    );
    if (finished) {
...
export class TrackingHeapSnapshotProfileType extends Common.ObjectWrapper.eventMixin(
  ...
  constructor() {
    super(
      TrackingHeapSnapshotProfileType.TypeId,
      i18nString(UIStrings.allocationInstrumentationOn),
    );
    this.recordAllocationStacksSettingInternal = Common.Settings.Settings.instance().createSetting(
  ...
  modelAdded(heapProfilerModel) {
    ...
    heapProfilerModel.addEventListener(
      'HeapStatsUpdate' /* SDK.HeapProfilerModel.Events.HEAP_STATS_UPDATED */,
      this.heapStatsUpdate,
      this,
    );
    heapProfilerModel.addEventListener(
      'LastSeenObjectId' /* SDK.HeapProfilerModel.Events.LAST_SEEN_OBJECT_ID */,
  ...
  get buttonTooltip() {
    return this.recording
      ? i18nString(UIStrings.stopRecordingHeapProfile)
  ...
  async startRecordingProfile() {
    ...
    void heapProfilerModel.startTrackingHeapObjects(
      this.recordAllocationStacksSettingInternal.get(),
    );
  }
  customContent() {
  ...
  addNewProfile() {
    ...
    this.dispatchEventToListeners(
      'TrackingStarted' /* TrackingHeapSnapshotProfileTypeEvents.TRACKING_STARTED */,
    );
...
export class HeapProfileHeader extends ProfileHeader {
  ...
  setupWorker() {
    ...
    this.workerProxy.addEventListener(
      'Wait' /* HeapSnapshotModel.HeapSnapshotProxy.HeapSnapshotWorkerProxy.Events.WAIT */,
      (event) => {
  ...
  handleWorkerEvent(eventName, data) {
    if (
      HeapSnapshotModel.HeapSnapshotModel.HeapSnapshotProgressEvent.BrokenSnapshot === eventName
    ) {
  ...
  async saveToFile() {
    ...
    this.fileName =
      this.fileName ||
      'Heap-' +
        Platform.DateUtilities.toISO8601Compact(new Date()) +
    ...
    const onOpen = async (accepted) => {
      ...
      if (this.tempFile) {
        const error = await this.tempFile.copyToOutputStream(
          fileOutputStream,
          this.onChunkTransferred.bind(this),
        );
        if (error) {
          Common.Console.Console.instance().error(
            'Failed to read heap snapshot from temp file: ' + error.message,
          );
        }
        this.didCompleteSnapshotTransfer();
        return;
