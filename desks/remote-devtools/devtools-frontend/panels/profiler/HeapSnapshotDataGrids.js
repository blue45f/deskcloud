...
export class HeapSnapshotSortableDataGrid extends Common.ObjectWrapper.eventMixin(
  ...
  constructor(heapProfilerModel, dataDisplayDelegate, dataGridParameters) {
    ...
    this.addEventListener(
      HeapSnapshotSortableDataGridEvents.SortingComplete,
  ...
  isFilteredOut(node) {
    ...
    if (
      nameFilterValue &&
      (node instanceof HeapSnapshotDiffNode || node instanceof HeapSnapshotConstructorNode) &&
  ...
  wasShown() {
    super.wasShown();
    if (this.nameFilter) {
      this.nameFilter.addEventListener(
  ...
  willHide() {
    super.willHide();
    if (this.nameFilter) {
      this.nameFilter.removeEventListener(
        'TextChanged' /* UI.Toolbar.ToolbarInput.Event.TEXT_CHANGED */,
        this.onNameFilterChanged,
        this,
      );
  ...
  populateContextMenu(contextMenu, gridNode) {
    const node = gridNode;
    node.populateContextMenu(
      contextMenu,
      this.dataDisplayDelegateInternal,
...
export class HeapSnapshotRetainmentDataGrid extends HeapSnapshotContainmentDataGrid {
  ...
  constructor(heapProfilerModel, dataDisplayDelegate) {
    ...
    super(
      heapProfilerModel,
      dataDisplayDelegate,
      i18nString(UIStrings.heapSnapshotRetainment),
      columns,
    );
  }
  createRootNode(snapshot, node) {
...
export class HeapSnapshotDiffDataGrid extends HeapSnapshotViewportDataGrid {
  ...
  async populateChildren() {
    ...
    const diffByClassKey = await this.snapshot.calculateSnapshotDiff(
