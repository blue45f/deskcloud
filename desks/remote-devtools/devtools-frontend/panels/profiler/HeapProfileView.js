...
export class HeapProfileView extends ProfileView {
  ...
  constructor(profileHeader) {
    ...
    if (
      ...
      this.profileType.addEventListener(
        'StatsUpdate' /* SamplingHeapProfileType.Events.STATS_UPDATE */,
        this.onStatsUpdate,
        this,
      );
      void this.profileType
        .once('profile-complete' /* ProfileEvents.PROFILE_COMPLETE */)
        .then(() => {
          this.profileType.removeEventListener(
            'StatsUpdate' /* SamplingHeapProfileType.Events.STATS_UPDATE */,
            this.onStatsUpdate,
            this,
          );
...
export class SamplingHeapProfileTypeBase extends Common.ObjectWrapper.eventMixin(ProfileType) {
  ...
  get buttonTooltip() {
    return this.recording
      ? i18nString(UIStrings.stopHeapProfiling)
      : i18nString(UIStrings.startHeapProfiling);
  }
  buttonClicked() {
    if (this.recording) {
  ...
  async stopRecordingProfile() {
    ...
    this.dispatchEventToListeners(
      'profile-complete' /* ProfileEvents.PROFILE_COMPLETE */,
      recordedProfile,
    );
  }
...
export class SamplingHeapProfileType extends SamplingHeapProfileTypeBase {
  ...
  async stopSampling() {
    ...
    this.updateTimer = 0;
    this.dispatchEventToListeners(
      'RecordingStopped' /* SamplingHeapProfileType.Events.RECORDING_STOPPED */,
    );
    const heapProfilerModel = this.obtainRecordingProfile();
    if (!heapProfilerModel) {
...
export class NodeFormatter {
  ...
  linkifyNode(node) {
    ...
    return this.profileView
      .linkifier()
      .maybeLinkifyConsoleCallFrame(target, node.profileNode.callFrame, options);
  }
}
export class HeapFlameChartDataProvider extends ProfileFlameChartDataProvider {
  ...
  formatValue(value, _precision) {
    return i18nString(UIStrings.skb, {
      PH1: Platform.NumberUtilities.withThousandsSeparator(value / 1e3),
    });
  }
