...
export class ThreadAppender {
  ...
  constructor(
    ...
    this.#colorGenerator = new Common.Color.Generator(
      { min: 30, max: 330, count: undefined },
    ...
    if (!entries || !tree) {
      throw new Error(
        `Could not find data for thread with id ${threadId} in process with id ${processId}`,
      );
    }
    this.#entries = entries;
    this.#tree = tree;
    this.#threadDefaultName = threadName || i18nString(UIStrings.threadS, { PH1: threadId });
    this.isOnMainFrame = Boolean(
      this.#parsedTrace.data.Renderer?.processes.get(processId)?.isOnMainFrame,
    );
   ...
   */
    ...
    if (
      this.threadType === 'RASTERIZER' /* Trace.Handlers.Threads.ThreadType.RASTERIZER */ ||
    ...
    switch (this.threadType) {
      case 'MAIN_THREAD' /* Trace.Handlers.Threads.ThreadType.MAIN_THREAD */:
        return this.isOnMainFrame
          ? 'thread.main' /* VisualLoggingTrackName.THREAD_MAIN */
          : 'thread.frame' /* VisualLoggingTrackName.THREAD_FRAME */;
  ...
  #appendGroupedTrackHeaderAndTitle(trackStartLevel, threadType) {
    const currentTrackCount =
      this.#compatibilityBuilder.getCurrentTrackCountForThreadType(threadType);
  ...
  trackName() {
    ...
    switch (this.threadType) {
      case 'MAIN_THREAD' /* Trace.Handlers.Threads.ThreadType.MAIN_THREAD */:
        threadTypeLabel = this.isOnMainFrame
          ? i18nString(UIStrings.mainS, { PH1: this.#url })
  ...
  #buildNameForAuctionWorklet() {
    const workletMetadataEvent = this.#parsedTrace.data.AuctionWorklets.worklets.get(
      this.#processId,
    );
  ...
  #buildNameForWorker() {
    ...
    let workerName = workerURL
      ? i18nString(UIStrings.workerS, { PH1: workerURL })
      : i18nString(UIStrings.dedicatedWorker);
    const workerTarget =
      workerId !== undefined && SDK.TargetManager.TargetManager.instance().targetById(workerId);
    if (workerTarget) {
  ...
  #appendNodesAtLevel(nodes, startingLevel, parentIsIgnoredListed = false) {
    const invisibleEntries =
      ModificationsManager.ModificationsManager.activeManager()
        ?.getEntriesFilter()
        .invisibleEntries() ?? [];
    ...
    for (const node of nodes) {
      ...
      const entryIsVisible =
        !invisibleEntries.includes(entry) &&
      ...
      const depthInChildTree = this.#appendNodesAtLevel(
        node.children,
        nextLevel,
        entryIsIgnoreListed,
      );
  ...
  colorForEvent(event) {
    if (this.#parsedTrace.data.Meta.traceIsGeneric) {
      return event.name
        ? `hsl(${(Platform.StringUtilities.hashCode(event.name) % 300) + 30}, 40%, 70%)`
        : '#ccc';
    }
  ...
  setPopoverInfo(event, info) {
    if (Trace.Types.Events.isParseHTML(event)) {
      ...
      const range =
        endLine !== -1 || endLine === startLine ? `${startLine}...${endLine}` : startLine;
