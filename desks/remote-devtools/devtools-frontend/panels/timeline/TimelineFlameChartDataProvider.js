...
const str_ = i18n.i18n.registerUIStrings(
  'panels/timeline/TimelineFlameChartDataProvider.ts',
  UIStrings,
);
...
export class TimelineFlameChartDataProvider extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    ThemeSupport.ThemeSupport.instance().addEventListener(
      ...
      () => {
        const headers = [this.framesGroupStyle, this.screenshotsGroupStyle];
        for (const header of headers) {
          header.color =
            ThemeSupport.ThemeSupport.instance().getComputedValue('--sys-color-on-surface');
          header.backgroundColor = ThemeSupport.ThemeSupport.instance().getComputedValue(
            '--sys-color-cdt-base-container',
          );
        }
      },
    );
    Utils.ImageCache.emitter.addEventListener('screenshot-loaded', () =>
  ...
  customizedContextMenu(mouseEvent, entryIndex, groupIndex) {
    ...
    const perfAIEntryPointEnabled = Boolean(
      entry &&
      this.parsedTrace &&
      UI.ActionRegistry.ActionRegistry.instance().hasAction(PERF_AI_ACTION_ID),
    );
    ...
    if (perfAIEntryPointEnabled && this.parsedTrace) {
      ...
      if (callTree) {
        ...
        const submenu = contextMenu
          .footerSection()
          .appendSubMenuItem(action.title(), false, PERF_AI_ACTION_ID);
    ...
    const hideEntryOption = contextMenu.defaultSection().appendItem(
      i18nString(UIStrings.hideFunction),
      () => {
        this.modifyTree(
          'MERGE_FUNCTION' /* PerfUI.FlameChart.FilterAction.MERGE_FUNCTION */,
          entryIndex,
        );
      },
      {
        disabled:
          !possibleActions?.['MERGE_FUNCTION' /* PerfUI.FlameChart.FilterAction.MERGE_FUNCTION */],
        jslogContext: 'hide-function',
      },
    );
    hideEntryOption.setAccelerator(UI.KeyboardShortcut.Keys.H, [
      UI.KeyboardShortcut.Modifiers.None,
    ]);
    hideEntryOption.setIsDevToolsPerformanceMenuItem(true);
    const hideChildrenOption = contextMenu.defaultSection().appendItem(
      i18nString(UIStrings.hideChildren),
      () => {
        this.modifyTree(
          'COLLAPSE_FUNCTION' /* PerfUI.FlameChart.FilterAction.COLLAPSE_FUNCTION */,
          entryIndex,
        );
      },
      {
    ...
    hideChildrenOption.setAccelerator(UI.KeyboardShortcut.Keys.C, [
      UI.KeyboardShortcut.Modifiers.None,
    ]);
    hideChildrenOption.setIsDevToolsPerformanceMenuItem(true);
    const hideRepeatingChildrenOption = contextMenu.defaultSection().appendItem(
      i18nString(UIStrings.hideRepeatingChildren),
      () => {
        this.modifyTree(
          'COLLAPSE_REPEATING_DESCENDANTS' /* PerfUI.FlameChart.FilterAction.COLLAPSE_REPEATING_DESCENDANTS */,
          entryIndex,
        );
      },
      {
    ...
    hideRepeatingChildrenOption.setAccelerator(UI.KeyboardShortcut.Keys.R, [
      UI.KeyboardShortcut.Modifiers.None,
    ]);
    hideRepeatingChildrenOption.setIsDevToolsPerformanceMenuItem(true);
    const resetChildrenOption = contextMenu.defaultSection().appendItem(
      i18nString(UIStrings.resetChildren),
      () => {
        this.modifyTree(
          'RESET_CHILDREN' /* PerfUI.FlameChart.FilterAction.RESET_CHILDREN */,
          entryIndex,
        );
      },
      {
        disabled:
          !possibleActions?.['RESET_CHILDREN' /* PerfUI.FlameChart.FilterAction.RESET_CHILDREN */],
        jslogContext: 'reset-children',
      },
    );
    resetChildrenOption.setAccelerator(UI.KeyboardShortcut.Keys.U, [
      UI.KeyboardShortcut.Modifiers.None,
    ]);
    resetChildrenOption.setIsDevToolsPerformanceMenuItem(true);
    contextMenu.defaultSection().appendItem(
      i18nString(UIStrings.resetTrace),
      () => {
        this.modifyTree(
          'UNDO_ALL_ACTIONS' /* PerfUI.FlameChart.FilterAction.UNDO_ALL_ACTIONS */,
          entryIndex,
        );
      },
      {
    ...
    if (Utils.IgnoreList.isIgnoreListedEntry(entry)) {
      contextMenu.defaultSection().appendItem(
        i18nString(UIStrings.removeScriptFromIgnoreList),
        () => {
      ...
      contextMenu.defaultSection().appendItem(
        i18nString(UIStrings.addScriptToIgnoreList),
        () => {
  ...
  handleFlameChartTransformKeyboardEvent(event, entryIndex, groupIndex) {
    ...
    if (
      ...
      event.code === 'KeyC' &&
      possibleActions['COLLAPSE_FUNCTION' /* PerfUI.FlameChart.FilterAction.COLLAPSE_FUNCTION */]
    ) {
      this.modifyTree(
        'COLLAPSE_FUNCTION' /* PerfUI.FlameChart.FilterAction.COLLAPSE_FUNCTION */,
        entryIndex,
  ...
  buildGroupStyle(extra) {
    const defaultGroupStyle = {
      ...
      backgroundColor: ThemeSupport.ThemeSupport.instance().getComputedValue(
        '--sys-color-cdt-base-container',
      ),
      nestingLevel: 0,
  ...
  buildWithCustomTracksForTest(options) {
    ...
    for (const appender of appenders) {
      const trackName =
        appender instanceof ThreadAppender ? appender.trackName() : appender.appenderName;
      const shouldIncludeTrack =
        options?.filterTracks?.call(null, trackName, visibleTrackIndexCounter) ?? true;
      if (!shouldIncludeTrack) {
        continue;
      }
      const shouldExpandTrack =
        options?.expandTracks?.call(null, trackName, visibleTrackIndexCounter) ?? true;
  ...
  rebuildTimelineData() {
    ...
    if (this.#timelineData) {
      ...
      this.compatibilityTracksAppender
        ?.threadAppenders()
        .forEach((threadAppender) => threadAppender.setHeaderAppended(false));
    }
  }
  /**
  ...
  #processInspectorTrace() {
    ...
    const allTrackAppenders = this.compatibilityTracksAppender
    ...
    for (const appender of allTrackAppenders) {
      ...
      if (this.#timelineData && !this.#timelineData.selectedGroup) {
        if (
          appender instanceof ThreadAppender &&
          (appender.threadType ===
            'MAIN_THREAD' /* Trace.Handlers.Threads.ThreadType.MAIN_THREAD */ ||
            appender.threadType ===
              'CPU_PROFILE') /* Trace.Handlers.Threads.ThreadType.CPU_PROFILE */
        ) {
  ...
  #appendFramesAndScreenshotsTrack() {
    ...
    this.framesGroupStyle.collapsible = hasScreenshots
      ? 0 /* PerfUI.FlameChart.GroupCollapsibleState.ALWAYS */
  ...
  #appendScreenshots(filmStrip) {
    ...
    const traceEnd = Trace.Helpers.Timing.traceWindowMilliSeconds(
    ...
    for (let i = 0; i < filmStrip.frames.length; ++i) {
      ...
      const endTimeMillis = nextFrame
        ? Trace.Helpers.Timing.microToMilli(nextFrame.screenshotEvent.ts)
        : traceEnd;
  ...
  preparePopoverElement(entryIndex) {
    ...
    if (entryType === 'TrackAppender' /* EntryType.TRACK_APPENDER */) {
      ...
      this.dispatchEventToListeners(
        'FlameChartItemHovered' /* Events.FLAME_CHART_ITEM_HOVERED */,
        event,
      );
      ...
      time = i18n.TimeUtilities.preciseMillisToString(
        Trace.Helpers.Timing.microToMilli(frame.duration),
        1,
      );
      if (frame.idle) {
        title = i18nString(UIStrings.idleFrame);
      } else if (frame.dropped) {
        title = frame.isPartial
          ? i18nString(UIStrings.partiallyPresentedFrame)
          : i18nString(UIStrings.droppedFrame);
  ...
  preparePopoverForCollapsedArrow(entryIndex) {
    ...
    const entry = this.entryData[entryIndex];
    const hiddenEntriesAmount = ModificationsManager.activeManager()
      ?.getEntriesFilter()
  ...
  async drawScreenshot(entryIndex, context, barX, barY, barWidth, barHeight) {
    ...
    context.strokeRect(
      imageX - 0.5,
      imageY - 0.5,
      Math.min(barWidth - 1, imageWidth + 1),
      imageHeight,
    );
    context.restore();
  }
  ...
  #appendFrame(frame) {
    ...
    this.entryIndexToTitle.splice(
      index,
      0,
      i18n.TimeUtilities.millisToString(durationMilliseconds, true),
    );
    if (!this.#timelineData) {
      return;
    }
    if (
      Array.isArray(this.#timelineData.entryLevels) &&
      Array.isArray(this.#timelineData.entryTotalTimes) &&
      Array.isArray(this.#timelineData.entryStartTimes)
    ) {
      this.#timelineData.entryLevels.splice(index, 0, this.currentLevel);
      this.#timelineData.entryTotalTimes.splice(index, 0, durationMilliseconds);
      this.#timelineData.entryStartTimes.splice(
        index,
        0,
      ...
      this.#timelineData.entryStartTimes[index] = Trace.Helpers.Timing.microToMilli(
        frame.startTime,
      );
    }
  }
  ...
  entryIndexForSelection(selection) {
    if (
      !selection ||
      selectionIsRange(selection) ||
      Trace.Types.Events.isNetworkTrackEntry(selection.event)
    ) {
      return -1;
    }
