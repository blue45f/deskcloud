...
const DEFAULT_VIEW = (input, _output, target) => {
  render(
    html` <devtools-data-grid
      striped
      name=${i18nString(UIStrings.selectorStats)}
    ...
    >
      <table>
        ...
        ${input.timings.map((timing) => {
          const nonMatches =
            timing[SelectorTimingsKey.MatchAttempts] - timing[SelectorTimingsKey.MatchCount];
          ...
          return html`<tr>
            ...
            <td data-value=${slowPathNonMatches}>${slowPathNonMatches.toFixed(1)}</td>
            ...
            <td data-value=${styleSheetId}>
              ${locations
                ? html`${locations.map(
                    (location, itemIndex) =>
                      html` <devtools-linkifier .data=${location}></devtools-linkifier
...
export class TimelineSelectorStatsView extends UI.Widget.VBox {
  ...
  async updateInvalidationCount(events) {
    ...
    for (const [frameId, backendNodeIds] of frameIdBackendNodeIdsMap) {
      const backendNodeIdMap = await Utils.EntryNodes.domNodesForBackendIds(
        frameId,
        backendNodeIds,
      );
    ...
    for (const invalidatedNode of invalidatedNodes) {
      ...
      for (const selector of invalidatedNode.selectorList) {
        ...
        if (invalidatedNodeMap.has(key)) {
          ...
          invalidatedNodeMap.set(key, {
            subtree: invalidatedNode.subtree,
            nodeList: [invalidatedNodeDomNode],
          });
        }
      }
    }
    for (const event of events) {
      const selectorStats = event
        ? this.#parsedTrace.data.SelectorStats.dataForRecalcStyleEvent.get(event)
        : undefined;
      if (!selectorStats) {
        continue;
      }
  ...
  async aggregateEvents(events) {
    ...
    if (Array.isArray(this.#lastStatsSourceEventOrEvents)) {
      if (
        this.#lastStatsSourceEventOrEvents.length === events.length &&
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      for (const timing of data) {
        const key =
          timing[SelectorTimingsKey.Selector] + '_' + timing[SelectorTimingsKey.StyleSheetId];
        ...
        if (findTiming !== undefined) {
          ...
          findTiming[SelectorTimingsKey.InvalidationCount] +=
            timing[SelectorTimingsKey.InvalidationCount];
        } else {
  ...
  async processSelectorTimings(timings) {
    async function toSourceFileLocation(cssModel, styleSheetId, selectorText, selectorLocations) {
      ...
      const linkData = ranges.map((range) => {
        return {
          ...
          linkText: i18nString(UIStrings.lineNumber, {
            PH1: range.startLine + 1,
            PH2: range.startColumn + 1,
          }),
    ...
    return await Promise.all(
      timings
        .sort((a, b) => b[SelectorTimingsKey.Elapsed] - a[SelectorTimingsKey.Elapsed])
        .map(async (x) => {
          ...
          const locations =
            styleSheetId === 'n/a'
              ? null
              : await toSourceFileLocation(
                  cssModel,
                  styleSheetId,
                  selectorText,
                  this.#selectorLocations,
                );
          return { ...x, locations };
        }),
    );
