...
const DEFAULT_VIEW = (input, output, target) => {
  const { shownInsights, passedInsights, insightSetKey, parsedTrace, renderInsightComponent } =
    input;
  ...
  function renderInsights() {
    ...
    return html`
      ...
      ${passedInsightsTemplates.length
        ? html`
  ...
  Lit.render(
    html`
      <style>
        ${sidebarSingleInsightSetStyles}
      </style>
      <div class="navigation">${renderMetrics()} ${renderInsights()}</div>
    `,
    target,
  );
  // clang-format on
...
export class SidebarSingleInsightSet extends UI.Widget.Widget {
  ...
  #renderInsightComponent(insightSet, insightData, fieldMetrics) {
    ...
    return html`<devtools-widget
      class="insight-component-widget"
      ?highlight-insight=${isActiveInsight && this.#isActiveInsightHighlighted}
  ...
  performUpdate() {
    ...
    const input = {
      ...
      insightSetKey,
      parsedTrace,
      renderInsightComponent: (insightData) =>
        this.#renderInsightComponent(insightSet, insightData, field),
    };
