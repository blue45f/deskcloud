  ...
  for (const warning of warnings) {
    ...
    switch (warning) {
      case 'FORCED_REFLOW': {
        ...
        span.appendChild(
          uiI18n.getFormatLocalizedString(str_, UIStrings.sIsALikelyPerformanceBottleneck, {
            PH1: forcedReflowLink,
          }),
        );
        break;
      }
      case 'IDLE_CALLBACK_OVER_TIME': {
        ...
        const exceededMs = i18n.TimeUtilities.millisToString(
          (duration || 0) - event.args.data['allottedMilliseconds'],
          true,
        );
      ...
      case 'LONG_TASK': {
        ...
        span.appendChild(
          uiI18n.getFormatLocalizedString(str_, UIStrings.sTookS, {
            PH1: longTaskLink,
            PH2: i18n.TimeUtilities.millisToString(duration || 0, true),
          }),
        );
        break;
      }
      case 'LONG_INTERACTION': {
        const longInteractionINPLink = Link.create(
          'https://web.dev/inp',
          i18nString(UIStrings.longInteractionINP),
          undefined,
          'long-interaction',
        );
        span.appendChild(
          uiI18n.getFormatLocalizedString(str_, UIStrings.sIsLikelyPoorPageResponsiveness, {
...
export function buildRowsForWebSocketEvent(event, parsedTrace) {
  ...
  if (initiator && Trace.Types.Events.isWebSocketCreate(initiator)) {
    ...
    if (initiator.args.data.websocketProtocol) {
      rows.push({
        key: i18nString(UIStrings.websocketProtocol),
        value: initiator.args.data.websocketProtocol,
      });
    }
    ...
    if (event.args.data.websocketProtocol) {
      rows.push({
        key: i18nString(UIStrings.websocketProtocol),
        value: event.args.data.websocketProtocol,
      });
    }
  }
...
export function generateInvalidationsList(invalidations) {
  ...
  for (const invalidation of invalidations) {
    ...
    if (
      reason === 'unknown' &&
      Trace.Types.Events.isScheduleStyleInvalidationTracking(invalidation) &&
    ...
    if (
      reason === 'PseudoClass' &&
      Trace.Types.Events.isStyleRecalcInvalidationTracking(invalidation) &&
