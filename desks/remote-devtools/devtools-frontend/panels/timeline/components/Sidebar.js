...
export class SidebarWidget extends UI.Widget.VBox {
   ...
   * closed).
   */
  #hasOpenedOnce = Common.Settings.Settings.instance().createSetting(
  ...
  constructor() {
    ...
    this.#tabbedPane.appendTab(
      'annotations' /* SidebarTabs.ANNOTATIONS */,
      'Annotations',
  ...
  wasShown() {
    ...
    if (
      this.#tabbedPane.selectedTabId === 'insights' /* SidebarTabs.INSIGHTS */ &&
  ...
  #updateAnnotationsCountBadge() {
    ...
    this.#tabbedPane.setBadge(
      'annotations',
      annotations.length > 0 ? annotations.length.toString() : null,
    );
  }
