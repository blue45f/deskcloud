...
export class MetricCard extends HTMLElement {
  ...
  #renderCompareString() {
    ...
    if (localValue === undefined) {
      if (this.#data.metric === 'INP') {
        return html` <div class="compare-text">${i18nString(UIStrings.interactToMeasure)}</div> `;
      }
      return Lit.nothing;
    }
  ...
  #renderDetailedCompareString() {
    ...
    const fieldValue = this.#getFieldValue();
    const fieldRating =
      fieldValue !== undefined ? rateMetric(fieldValue, this.#getThresholds()) : undefined;
    ...
    // clang-format off
    return html`
      <div class="detailed-compare-text">
        ${renderDetailedCompareText({
          metric: i18n.i18n.lockedString(this.#data.metric),
          localRating,
          fieldRating,
  ...
  #renderFieldHistogram() {
    ...
    const goodLabel = html`
      <div class="bucket-label">
        ...
        <span class="bucket-range">
          ${i18nString(UIStrings.leqRange, { PH1: format(thresholds[0]) })}</span
        >
      </div>
    `;
    const needsImprovementLabel = html`
      <div class="bucket-label">
        ...
        <span class="bucket-range">
          ${i18nString(UIStrings.betweenRange, {
            PH1: format(thresholds[0]),
            PH2: format(thresholds[1]),
          })}</span
        >
      </div>
    `;
    const poorLabel = html`
      <div class="bucket-label">
        ...
        <span class="bucket-range">
          ${i18nString(UIStrings.gtRange, { PH1: format(thresholds[1]) })}</span
        >
      </div>
    `;
    // clang-format on
    if (!fieldEnabled) {
      return html`
        <div class="bucket-summaries">${goodLabel} ${needsImprovementLabel} ${poorLabel}</div>
      `;
    }
    // clang-format off
    return html`
      <div class="bucket-summaries histogram">
        ${goodLabel}
        <div
          class="histogram-bar good-bg"
          style="width: ${this.#getBarWidthForRating('good')}"
        ></div>
        <div class="histogram-percent">${this.#getPercentLabelForRating('good')}</div>
        ${needsImprovementLabel}
        <div
          class="histogram-bar needs-improvement-bg"
          style="width: ${this.#getBarWidthForRating('needs-improvement')}"
        ></div>
        <div class="histogram-percent">${this.#getPercentLabelForRating('needs-improvement')}</div>
        ${poorLabel}
        <div
          class="histogram-bar poor-bg"
          style="width: ${this.#getBarWidthForRating('poor')}"
        ></div>
    ...
    return html`
      ...
      <div class="phase-table" role="table">
        <div class="phase-table-row phase-table-header-row" role="row">
          ...
          <div role="columnheader" class="phase-table-value" style="grid-column: 2">
            ${i18nString(UIStrings.localValue)}
          </div>
          ${hasFieldData
            ? html`
                <div
                  ...
                  style="grid-column: 3"
                  title=${i18nString(UIStrings.field75thPercentile)}
                >
                  ${i18nString(UIStrings.fieldP75)}
                </div>
              `
            : nothing}
        </div>
        ${phases.map(
          (phase) => html`
            <div class="phase-table-row" role="row">
              ...
              <div role="cell" class="phase-table-value">
                ${i18n.TimeUtilities.preciseMillisToString(phase[1])}
              </div>
              ${phase[2] !== undefined
                ? html`
                    <div role="cell" class="phase-table-value">
                      ${i18n.TimeUtilities.preciseMillisToString(phase[2])}
                    </div>
                  `
                : nothing}
            </div>
      ...
      <div class="metric-card">
        ...
        >
          <div class="metric-source-block">
            ...
            ${fieldEnabled
              ? html`<div class="metric-source-label">${i18nString(UIStrings.localValue)}</div>`
              : nothing}
          </div>
          ${fieldEnabled
            ? html`
                <div class="metric-source-block">
                  ...
                  <div class="metric-source-label">
                    ${i18nString(UIStrings.field75thPercentile)}
                  </div>
                </div>
              `
            : nothing}
            ...
            <div class="tooltip-scroll">
              <div class="tooltip-contents">
                <div>
                  ...
                  ${localValue && this.#data.phases
                    ? this.#renderPhaseTable(this.#data.phases)
                    : nothing}
                </div>
              </div>
            </div>
        ...
        ${fieldEnabled ? html`<hr class="divider" />` : nothing} ${this.#renderCompareString()}
        ${this.#data.warnings?.map((warning) => html` <div class="warning">${warning}</div> `)}
