...
const str_ = i18n.i18n.registerUIStrings(
  'panels/timeline/components/NetworkRequestTooltip.ts',
  UIStrings,
);
    ...
    html`
      ...
      <div class="performance-card">
        ...
        <div class="network-category">
          <span class="network-category-chip" style=${Lit.Directives.styleMap(chipStyle)}> </span
          >${networkResourceCategory(networkRequest)}
        </div>
        <div class="priority-row">
          ${i18nString(UIStrings.priority)}:
          ${NetworkRequestTooltip.renderPriorityValue(networkRequest)}
        </div>
        ${throttlingTitle
          ? html` <div class="throttled-row">
        ...
        ${Trace.Helpers.Network.isSyntheticNetworkRequestEventRenderBlocking(networkRequest)
          ? html`<div class="render-blocking">${i18nString(UIStrings.renderBlocking)}</div>`
          : Lit.nothing}
        <div class="divider"></div>
...
export class NetworkRequestTooltip extends UI.Widget.Widget {
  ...
  static renderPriorityValue(networkRequest) {
    if (networkRequest.args.data.priority === networkRequest.args.data.initialPriority) {
      return html`${PerfUI.NetworkPriorities.uiLabelForNetworkPriority(
        networkRequest.args.data.priority,
      )}`;
    }
    return html`${PerfUI.NetworkPriorities.uiLabelForNetworkPriority(
        networkRequest.args.data.initialPriority,
      )}
  ...
  static renderTimings(networkRequest) {
    ...
    const throttledTitle = wasThrottled
      ? i18nString(UIStrings.wasThrottled, {
          PH1:
            typeof wasThrottled.conditions.title === 'string'
    ...
    const rightWhisker = html`<span class="whisker-right">
    ...
    return html`
      <div class=${classes} title=${ifDefined(throttledTitle)}>
        ${wasThrottled?.urlPattern
          ? html`<devtools-icon class="indicator" name="watch"></devtools-icon>`
      ...
      <div class="timings-row">
        ${leftWhisker} ${i18nString(UIStrings.queuingAndConnecting)}
      ...
      <div class="timings-row">
        ${rightWhisker} ${i18nString(UIStrings.waitingOnMainThread)}
        <span class="time">
          ${i18n.TimeUtilities.formatMicroSecondsTime(waitingOnMainThread)}
        </span>
      </div>
    `;
  }
  static renderRedirects(networkRequest) {
    ...
    if (networkRequest.args.data.redirects.length > 0) {
      redirectRows.push(html`
        <div class="redirects-row">${i18nString(UIStrings.redirects)}</div>
      `);
  ...
  performUpdate() {
    ...
    if (networkConditions) {
      throttlingTitle =
        typeof networkConditions.conditions.title === 'string'
