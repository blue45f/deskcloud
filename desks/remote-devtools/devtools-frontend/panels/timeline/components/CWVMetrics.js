...
const CWV_METRICS_VIEW = (input, _output, target) => {
  ...
  const localValues = {
    lcp:
      local?.lcp?.value !== undefined
        ? Trace.Helpers.Timing.microToMilli(local?.lcp.value)
        : undefined,
    inp:
      local?.inp?.value !== undefined
        ? Trace.Helpers.Timing.microToMilli(local?.inp.value)
        : undefined,
  };
  const fieldValues = field && {
    lcp:
      field.lcp?.value !== undefined
        ? Trace.Helpers.Timing.microToMilli(field.lcp.value)
        : undefined,
    inp:
      field.inp?.value !== undefined
        ? Trace.Helpers.Timing.microToMilli(field.inp.value)
        : undefined,
  };
  const showFieldMismatchNotice =
    !didDismissFieldMismatchNotice &&
    !!fieldValues &&
    isFieldWorseThanLocal(localValues, fieldValues);
  function renderMetricValue(metric, value, relevantEvent) {
    ...
    if (value === null) {
      ...
      classification =
        'unclassified' /* Trace.Handlers.ModelHandlers.PageLoadMetrics.ScoreClassification.UNCLASSIFIED */;
    } else if (metric === 'LCP') {
      const micros = value;
      ...
      classification =
        Trace.Handlers.ModelHandlers.PageLoadMetrics.scoreClassificationForLargestContentfulPaint(
          micros,
        );
    } else if (metric === 'CLS') {
      valueText = valueDisplay = value ? value.toFixed(2) : '0';
      classification =
        Trace.Handlers.ModelHandlers.LayoutShifts.scoreClassificationForLayoutShift(value);
    } else if (metric === 'INP') {
      const micros = value;
      ...
      valueDisplay = element;
      classification =
        Trace.Handlers.ModelHandlers.UserInteractions.scoreClassificationForInteractionToNextPaint(
          micros,
        );
    } else {
  ...
  const clsEl = renderMetricValue(
    'CLS',
    local?.cls?.value ?? null,
    local?.cls?.worstClusterEvent ?? null,
  );
  ...
  if (showFieldMismatchNotice) {
    ...
    fieldIsDifferentEl = html`
      <div
        class="field-mismatch-notice"
        jslog=${VisualLogging.section('timeline.insights.field-mismatch')}
      >
        ...
        ></devtools-button>
        <div class="field-mismatch-notice__body">
          ${md(i18nString(UIStrings.fieldMismatchNotice))}
        </div>
      </div>
    `;
    // clang-format on
  ...
  const metricsTableEl = html`<div class=${Lit.Directives.classMap(classes)}>
    ...
    ${localMetricsTemplateResult} ${fieldMetricsTemplateResult}
  </div>`;
  Lit.render(
    html`
      <style>
      ...
      ${metricsTableEl} ${fieldIsDifferentEl}
    `,
    target,
  );
};
