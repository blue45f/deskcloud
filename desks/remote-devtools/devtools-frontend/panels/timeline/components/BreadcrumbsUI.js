...
export class BreadcrumbsUI extends HTMLElement {
  ...
  #renderElement(breadcrumb, index) {
    ...
    return html`
      <div
        class="breadcrumb"
        @contextmenu=${(event) => this.#onContextMenu(event, breadcrumb)}
        @click=${() => this.#activateBreadcrumb(breadcrumb)}
      ...
      ${breadcrumb.child !== null
        ? html` <devtools-icon name="chevron-right" class="medium"></devtools-icon>`
        : ''}
    `;
    // clang-format on
  }
  #render() {
    ...
    const output = html`
      ...
      ${this.#initialBreadcrumb === null
        ? Lit.nothing
        : html`<div class="breadcrumbs" jslog=${VisualLogging.section('breadcrumbs')}>
            ${flattenBreadcrumbs(this.#initialBreadcrumb).map((breadcrumb, index) =>
              this.#renderElement(breadcrumb, index),
            )}
          </div>`}
    `;
    // clang-format on
