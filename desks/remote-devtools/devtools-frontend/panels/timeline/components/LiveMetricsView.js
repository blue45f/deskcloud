...
function getLcpFieldPhases(cruxManager) {
  const ttfb = cruxManager.getSelectedFieldMetricData(
    'largest_contentful_paint_image_time_to_first_byte',
  )?.percentiles?.p75;
  const loadDelay = cruxManager.getSelectedFieldMetricData(
    'largest_contentful_paint_image_resource_load_delay',
  )?.percentiles?.p75;
  const loadDuration = cruxManager.getSelectedFieldMetricData(
    'largest_contentful_paint_image_resource_load_duration',
  )?.percentiles?.p75;
  const renderDelay = cruxManager.getSelectedFieldMetricData(
    'largest_contentful_paint_image_element_render_delay',
  )?.percentiles?.p75;
  if (
    typeof ttfb !== 'number' ||
    typeof loadDelay !== 'number' ||
    typeof loadDuration !== 'number' ||
...
function getDeviceRec(cruxManager) {
  ...
  const fractions = cruxManager.getFieldResponse(cruxManager.fieldPageScope, 'ALL')?.record.metrics
    .form_factors?.fractions;
  if (!fractions) {
    return null;
  }
...
function getPageScopeLabel(cruxManager, pageScope) {
  ...
  if (key) {
    return pageScope === 'url'
      ? i18nString(UIStrings.urlOptionWithKey, { PH1: key })
      : i18nString(UIStrings.originOptionWithKey, { PH1: key });
  }
  const baseLabel =
    pageScope === 'url' ? i18nString(UIStrings.urlOption) : i18nString(UIStrings.originOption);
...
function renderLcpCard(input) {
  ...
  const nodeLink =
    input.lcpValue?.nodeRef &&
    PanelsCommon.DOMLinkifier.Linkifier.instance().linkify(input.lcpValue?.nodeRef);
      ...
      ${createMetricCardRef({
        ...
        phases: phases && [
          [
            ...
            phases.timeToFirstByte,
            fieldPhases?.timeToFirstByte,
          ],
          [
            i18nString(UIStrings.resourceLoadDelay),
            phases.resourceLoadDelay,
            fieldPhases?.resourceLoadDelay,
          ],
          [
            i18nString(UIStrings.resourceLoadDuration),
            phases.resourceLoadTime,
            fieldPhases?.resourceLoadTime,
          ],
          [
            i18nString(UIStrings.elementRenderDelay),
            phases.elementRenderDelay,
            fieldPhases?.elementRenderDelay,
          ],
        ],
      })}
    >
...
function renderClsCard(input) {
  ...
  const clusterIsVisible =
    clusterIds.size > 0 &&
    input.layoutShifts.some((layoutShift) => clusterIds.has(layoutShift.uniqueLayoutShiftId));
  // clang-format off
  return html`
    <devtools-metric-card
      ${createMetricCardRef({
        ...
        ? html`
            <div class="related-info" slot="extra-info">
              ...
              <button
                ...
                jslog=${VisualLogging.action('timeline.landing.show-cls-cluster').track({
                  click: true,
                })}
              >
        ...
        ? html`
            <div class="related-info" slot="extra-info">
              ...
              <button
                ...
                jslog=${VisualLogging.action('timeline.landing.show-inp-interaction').track({
                  click: true,
                })}
              >
...
function renderRecordingSettings(input) {
  ...
  return html`
    ...
    ${fieldEnabled
      ? html`
          <ul class="environment-recs-list">
            <li>
              ${uiI18n.getFormatLocalizedStringTemplate(str_, UIStrings.device, {
                PH1: html`<span class="environment-rec">${deviceRec}</span>`,
              })}
            </li>
            <li>
              ${uiI18n.getFormatLocalizedStringTemplate(str_, UIStrings.network, {
                PH1: html`<span class="environment-rec">${networkRec}</span>`,
              })}
            </li>
          </ul>
        `
    ...
    <div class="environment-option">
      <devtools-network-throttling-selector
        .recommendedConditions=${recs.networkConditions}
      ></devtools-network-throttling-selector>
    </div>
    <div class="environment-option">
      <setting-checkbox
...
function renderPageScopeSetting(input) {
  ...
  const shouldDisable =
    !input.cruxManager.pageResult?.['url-ALL'] && !input.cruxManager.pageResult?.['origin-ALL'];
  ...
  return html`
    ...
    >
      <devtools-menu-item .value=${'url'} .selected=${input.cruxManager.fieldPageScope === 'url'}>
        ${urlLabel}
      </devtools-menu-item>
      <devtools-menu-item
        .value=${'origin'}
...
function renderDeviceScopeSetting(input) {
  ...
  const shouldDisable = !input.cruxManager.getFieldResponse(
    input.cruxManager.fieldPageScope,
    'ALL',
  );
  const currentDeviceLabel = getLabelForDeviceOption(
    input.cruxManager,
    input.cruxManager.fieldDeviceOption,
  );
  // clang-format off
  /* eslint-disable @devtools/no-deprecated-component-usages */
  return html`
...
function renderCollectionPeriod(cruxManager) {
  ...
  return html`
    <div class="field-data-message">
      <div>
        ${uiI18n.getFormatLocalizedStringTemplate(str_, UIStrings.collectionPeriod, {
          PH1: html`<span class="collection-period-range">${dateText}</span>`,
        })}
        ${fieldDataHistoryLink}
      </div>
      ${warnings.map((warning) => html` <div class="field-data-warning">${warning}</div> `)}
    </div>
  `;
}
function renderFieldDataMessage(cruxManager) {
  ...
  return html`
    <div class="field-data-message">
      ${uiI18n.getFormatLocalizedStringTemplate(str_, UIStrings.seeHowYourLocalMetricsCompare, {
        PH1: html`<devtools-link href="https://developer.chrome.com/docs/crux"
            ...
            <details>
              <summary>
                <span class="interaction-type">
                  ...
                  ${isInp
                    ? html`<span
                        class="interaction-inp-chip"
                        title=${i18nString(UIStrings.inpInteraction)}
                        >INP</span
                      >`
                    : nothing}
                </span>
              ...
              <div class="phase-table" role="table">
                <div class="phase-table-row phase-table-header-row" role="row">
                  ...
                  <div role="columnheader">
                    ${interaction.longAnimationFrameTimings.length
                      ? html`
                          <button
...
function renderLogSection(input, output) {
  ...
  return html`
    <section class="logs-section" aria-label=${i18nString(UIStrings.eventLogs)}>
      <devtools-widget
        ${widget(LiveMetricsLogs, {
          selectedTab: input.highlightedInteractionId
            ? 'interactions'
            : input.highlightedLayoutShiftClusterIds?.size
              ? 'layout-shifts'
              : undefined,
        })}
      >
        ${renderInteractionsLog(input, output)} ${renderLayoutShiftsLog(input, output)}
      </devtools-widget>
    </section>
  `;
  // clang-format on
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  const fieldEnabled = input.cruxManager.getConfigSetting().get().enabled;
  const liveMetricsTitle = fieldEnabled
    ? i18nString(UIStrings.localAndFieldMetrics)
    : i18nString(UIStrings.localMetrics);
  const helpLink =
    'https://web.dev/articles/lab-and-field-data-differences#lab_data_versus_field_data';
  // clang-format off
      ...
      <div class="live-metrics-view">
        <main class="live-metrics">
          ...
          <div class="metric-cards">
            <div id="lcp">${renderLcpCard(input)}</div>
            <div id="cls">${renderClsCard(input)}</div>
            <div id="inp">${renderInpCard(input)}</div>
          </div>
          <devtools-link
            href=${helpLink}
            class="local-field-link"
        ...
        <aside class="next-steps" aria-labelledby="next-steps-section-title">
          <h2 id="next-steps-section-title" class="section-title">
            ${i18nString(UIStrings.nextSteps)}
          </h2>
          <div id="field-setup" class="settings-card">
            <h3 class="card-title">${i18nString(UIStrings.fieldMetricsTitle)}</h3>
            ${renderFieldDataMessage(input.cruxManager)} ${renderPageScopeSetting(input)}
            ${renderDeviceScopeSetting(input)}
            <div class="field-setup-buttons">
              <devtools-field-settings-dialog></devtools-field-settings-dialog>
            </div>
          </div>
          <div id="recording-settings" class="settings-card">${renderRecordingSettings(input)}</div>
...
export class LiveMetricsView extends UI.Widget.Widget {
  ...
  async #onMetricStatus(event) {
    ...
    const shouldScrollInteractions =
      hasNewInteraction && this.#viewOutput.shouldKeepInteractionsScrolledToBottom?.();
  ...
  wasShown() {
    ...
    liveMetrics.addEventListener(
      'status' /* LiveMetrics.Events.STATUS */,
      this.#onMetricStatus,
      this,
    );
    ...
    this.#deviceModeModel?.addEventListener(
      'Updated' /* EmulationModel.DeviceModeModel.Events.UPDATED */,
  ...
  willHide() {
    super.willHide();
    LiveMetrics.LiveMetrics.instance().removeEventListener(
      'status' /* LiveMetrics.Events.STATUS */,
      this.#onMetricStatus,
      this,
    );
  ...
  Lit.render(
    html`
      ...
      <devtools-tabbed-pane @select=${(event) => input.onTabSelected(event.detail.tabId)}>
        <devtools-toolbar slot="right">
          <devtools-button
            ...
            title=${i18nString(UIStrings.clearCurrentLog)}
            @click=${input.onClear}
      ...
      main component. This should be more performant and doesn't require us to inject live metrics styles twice. -->
        <slot
          name="interactions-log-content"
          id="interactions"
          ?selected=${live(input.selectedTab === 'interactions')}
          title=${i18nString(UIStrings.interactions)}
          jslogcontext="timeline.landing.interactions-log"
        >
        </slot>
        <slot
          name="layout-shifts-log-content"
          id="layout-shifts"
          ?selected=${live(input.selectedTab === 'layout-shifts')}
          title=${i18nString(UIStrings.layoutShifts)}
          jslogcontext="timeline.landing.layout-shifts-log"
        >
        </slot>
