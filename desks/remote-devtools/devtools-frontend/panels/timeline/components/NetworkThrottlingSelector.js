...
const str_ = i18n.i18n.registerUIStrings(
  'panels/timeline/components/NetworkThrottlingSelector.ts',
  UIStrings,
);
...
export class NetworkThrottlingSelector extends HTMLElement {
  ...
  constructor() {
    super();
    this.#customNetworkConditionsSetting = Common.Settings.Settings.instance().moduleSetting(
  ...
  #onMenuItemSelected(event) {
    const newConditions = this.#groups
      .flatMap((g) => g.items)
      .find((item) => {
  ...
  #render = () => {
    ...
    if (
      this.#recommendedConditions &&
      this.#currentConditions === SDK.NetworkManager.NoThrottlingConditions
    ) {
    ...
    const output = html`
      <style>
        ${networkThrottlingSelectorStyles}
      </style>
