...
export const TIMELINE_RANGE_SUMMARY_VIEW_DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html`
      <style>
        ${timelineRangeSummaryViewStyles}
      </style>
...
export function statsForTimeRange(events, startTime, endTime) {
  ...
  function buildRangeStatsCacheIfNeeded(events) {
    ...
    function updateCategory(category, time) {
      ...
      if (
        (statsArrays.time.length && statsArrays.time[statsArrays.time.length - 1] === time) ||
        lastTime > time
      ) {
        return;
      }
      const lastValue =
        statsArrays.value.length > 0 ? statsArrays.value[statsArrays.value.length - 1] : 0;
    ...
    function onStartEvent(e) {
      ...
      const category =
        Trace.Styles.getEventStyle(e.name)?.category.name ||
