...
function findInsightSet(insightSets, navigationId) {
  return insightSets
    ...
    .find((insightSet) =>
      navigationId
        ? navigationId === insightSet.navigation?.args.data?.navigationId
        : !insightSet.navigation,
    );
}
function renderLayoutShiftDetails(
  ...
  const clsInsight = findInsightSet(insightSets, layoutShift.args.data?.navigationId)?.model
    .CLSCulprits;
  if (!clsInsight) {
    return Lit.nothing;
  }
  ...
  const hasCulprits =
    rootCauses &&
    (rootCauses.webFonts.length ||
      rootCauses.iframes.length ||
      rootCauses.nonCompositedAnimations.length ||
    ...
    <table class="layout-shift-details-table">
      <thead class="table-title">
        <tr>
          ...
          ${hasShiftedElements
            ? html` <th>${i18nString(UIStrings.elementsShifted)}</th>`
            : Lit.nothing}
          ${hasCulprits ? html` <th>${i18nString(UIStrings.culprit)}</th> ` : Lit.nothing}
        </tr>
      </thead>
      <tbody>
...
function renderShiftRow(
  ...
  return html` <tr class="shift-row" data-ts=${currentShift.ts}>
    ...
    ${elementsShifted.length
      ...
      : Lit.nothing}
    ${hasCulprits
      ? html` <td class="culprits">
          ${rootCauses?.webFonts.map((fontReq) => renderFontRequest(fontReq))}
          ${rootCauses?.iframes.map((iframe) => renderIframe(iframe))}
          ${rootCauses?.nonCompositedAnimations.map((failure) =>
            renderAnimation(failure, onEventClick),
          )}
          ${rootCauses?.unsizedImages.map((unsizedImage) =>
            renderUnsizedImage(currentShift.args.frame, unsizedImage),
          )}
        </td>`
      : Lit.nothing}
  </tr>`;
...
function renderStartTime(shift, userHasSingleShiftSelected, parsedTrace, onEventClick) {
  ...
  // clang-format off
  return html` <button type="button" class="timeline-link" @click=${() => onEventClick(shift)}>
    ${i18nString(UIStrings.layoutShift, { PH1: shiftTs })}
  </button>`;
  // clang-format on
}
function renderParentCluster(cluster, onEventClick, parsedTrace) {
  ...
  // clang-format off
  return html` <span class="parent-cluster"
    >${i18nString(UIStrings.parentCluster)}:<button
      type="button"
      class="timeline-link parent-cluster-link"
      @click=${() => onEventClick(cluster)}
    >
      ${i18nString(UIStrings.cluster, { PH1: clusterTs })}
    </button>
  </span>`;
  // clang-format on
}
function renderShiftedElements(shift, elementsShifted) {
  // clang-format off
  return html` ${elementsShifted?.map((el) => {
...
function renderAnimation(failure, onEventClick) {
  ...
  // clang-format off
  return html` <span class="culprit">
    <span class="culprit-type">${i18nString(UIStrings.nonCompositedAnimation)}: </span>
    <button type="button" class="culprit-value timeline-link" @click=${() => onEventClick(event)}>
...
function renderIframe(iframeRootCause) {
  ...
  let el;
  if (domLoadingFrame) {
    el = LegacyComponents.Linkifier.Linkifier.linkifyRevealable(
      domLoadingFrame,
      domLoadingFrame.displayName(),
    );
  } else {
