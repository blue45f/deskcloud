...
export class NetworkDependencyTree extends BaseInsightComponent {
  ...
  #renderNetworkTreeRow(node) {
    const requestStyles = Lit.Directives.styleMap({
      ...
      backgroundColor: this.#relatedRequests?.has(node.request)
    ...
    return html`
      <div style=${requestStyles}>
        ...
        <span>
          ${i18n.TimeUtilities.formatMicroSecondsTime(
            Trace.Types.Timing.Micro(node.timeFromInitialRequest),
          )}
        </span>
      </div>
    `;
    ...
    const rows = [
      {
        ...
        // Filter out the empty rows otherwise the `Table` component will render a super short row
        subRows: nodes
          .map((node) => this.mapNetworkDependencyToRow(node))
          .filter((row) => row !== null),
      },
    ];
    if (this.#countOfChains > MAX_CHAINS_TO_SHOW) {
  ...
  #renderNetworkTreeSection() {
    ...
    if (!this.model.rootNodes.length) {
      ...
      return html`
        <style>
          ${networkDependencyTreeInsightStyles}
        </style>
    ...
    return html`
      ...
      <div class="insight-section">
        <div class="max-time">
          ...
          <br />
          <span class="longest">
            ${i18n.TimeUtilities.formatMicroSecondsTime(this.model.maxTime)}</span
          >
        </div>
      </div>
      <div class="insight-section">${this.#renderNetworkDependencyTree(this.model.rootNodes)}</div>
    `;
    // clang-format on
  }
  #renderTooManyPreconnectsWarning() {
    ...
    // clang-format off
    return html`
      <div style=${warningStyles}>${md(i18nString(UIStrings.tooManyPreconnectLinksWarning))}</div>
    `;
    // clang-format on
  }
  #renderPreconnectOriginsTable() {
    ...
    const preconnectOriginsTableTitle = html`
      ...
      <div class="insight-description">
        ${md(i18nString(UIStrings.preconnectOriginsTableDescription))}
      </div>
    `;
    if (!this.model.preconnectedOrigins.length) {
      ...
      return html`
        <div class="insight-section">
          ${preconnectOriginsTableTitle} ${i18nString(UIStrings.noPreconnectOrigins)}
        </div>
      `;
      // clang-format on
    }
      ...
      if (preconnectOrigin.source === 'ResponseHeader') {
        return {
          values: [
            ...
            eventRef(preconnectOrigin.request, { text: preconnectOrigin.headerText }),
          ],
          subRows,
        };
      }
    ...
    return html`
      <div class="insight-section">
        ${preconnectOriginsTableTitle} ${this.#renderTooManyPreconnectsWarning()}
        ${widget(Table, {
          data: {
            insight: this,
  ...
  #renderEstSavingTable() {
    ...
    if (!this.model.preconnectCandidates.length) {
      ...
      return html`
        <div class="insight-section">
          ${estSavingTableTitle} ${i18nString(UIStrings.noPreconnectCandidates)}
        </div>
      `;
      // clang-format on
    }
  ...
  renderContent() {
    return html`
      ${this.#renderNetworkTreeSection()} ${this.#renderPreconnectOriginsTable()}
