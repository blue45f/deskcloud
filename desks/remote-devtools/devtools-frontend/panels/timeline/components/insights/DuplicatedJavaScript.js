...
export class DuplicatedJavaScript extends BaseInsightComponent {
  ...
  #openTreemap() {
    ...
    if (!this.#treemapData) {
      this.#treemapData = Utils.Treemap.createTreemapData(
  ...
  renderContent() {
    ...
    const rows = [...this.model.duplicationGroupedByNodeModules.entries()]
      .slice(0, 10)
      .map(([source, data]) => {
      ...
      <div class="insight-section">
        ${widget(Table, {
          data: {
            insight: this,
            headers: [
              i18nString(UIStrings.columnSource),
              i18nString(UIStrings.columnDuplicatedBytes),
            ],
            rows,
          },
        })}
