...
export class SlowCSSSelector extends BaseInsightComponent {
  ...
  async getSelectorLinks(cssModel, selector) {
    ...
    const links = html` ${locations.map((location, itemIndex) => {
              ...
              rows: [
                ...
                { values: [i18nString(UIStrings.matchCount), this.model.totalMatchCount] },
                {
                  values: [
                    ...
                    i18n.TimeUtilities.millisToString(this.model.totalElapsedMs),
                  ],
                },
              ],
            },
        ...
        <div class="insight-section">
          ${widget(Table, {
            data: {
              insight: this,
              headers: [
                `${i18nString(UIStrings.topSelectorElapsedTime)}: ${time(Trace.Types.Timing.Micro(selector['elapsed (us)']))}`,
              ],
              rows: [
                {
                  values: [
                    html`${selector.selector}
                    ${Lit.Directives.until(this.getSelectorLinks(cssModel, selector))}`,
                  ],
                },
              ],
            },
        ...
        <div class="insight-section">
          ${widget(Table, {
            data: {
              insight: this,
              headers: [
                `${i18nString(UIStrings.topSelectorMatchAttempt)}: ${selector['match_attempts']}`,
              ],
              rows: [
                {
                  values: [
