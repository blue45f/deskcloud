...
export class LegacyJavaScript extends BaseInsightComponent {
  ...
  renderContent() {
    ...
    const rows = [...this.model.legacyJavaScriptResults.entries()]
      .slice(0, 10)
      .map(([script, result]) => {
        const overlays = [];
        if (script.request) {
        ...
        return {
          values: [
            scriptRef(script),
            i18n.ByteUtilities.bytesToString(result.estimatedByteSavings),
          ],
          overlays,
          subRows: result.matches.map((match) => {
            return {
              values: [
                html`<span
                  @click=${() => this.#revealLocation(script, match)}
                  title=${`${script.url}:${match.line}:${match.column}`}
                  >${match.name}</span
                >`,
              ],
            };
