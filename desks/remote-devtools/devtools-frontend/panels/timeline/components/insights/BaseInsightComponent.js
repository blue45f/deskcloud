...
const str_ = i18n.i18n.registerUIStrings(
  'panels/timeline/components/insights/BaseInsightComponent.ts',
  UIStrings,
);
...
const DEFAULT_VIEW = (input, _output, target) => {
  ...
  function renderInsightContent() {
    ...
    // clang-format off
    return html` <div class="insight-body">
      ${minimal
        ? Lit.nothing
        : html`<div class="insight-description">${md(model.description)}</div>`}
        ...
        ? html`
            <div class="ask-ai-btn-wrap">
              <devtools-button
                ...
                jslog=${VisualLogging.action(`timeline.insight-ask-ai.${internalName}`).track({
                  click: true,
                })}
    ...
    html`
      ...
      <div class=${containerClasses}>
        ${minimal
          ...
          : html`
              <header
                @click=${dispatchInsightToggle}
                @keydown=${onHeaderKeyDown}
                jslog=${VisualLogging.action(`timeline.toggle-insight.${internalName}`).track({
                  click: true,
                })}
              ...
              >
                ...
                ${estimatedSavingsString
                  ? html` <slot name="insight-savings" class="insight-savings">
                      <span title=${estimatedSavingsAriaLabel ?? ''}
                        >${estimatedSavingsString}</span
                      >
                    </slot>`
                  : Lit.nothing}
              </header>
...
export class BaseInsightComponent extends UI.Widget.Widget {
    ...
    if (!this.#selected && selected) {
      if (!this.#minimal) {
        ...
        this.element.dispatchEvent(
          new SidebarInsight.InsightProvideOverlays(this.getInitialOverlays(), options),
        );
      }
    }
    if (this.#selected !== selected) {
  ...
  #dispatchInsightToggle() {
    ...
    if (this.#selected) {
      ...
      if (focus) {
        UI.Context.Context.instance().setFlavor(
          AIAssistance.AIContext.AgentFocus,
          focus.withInsight(null),
        );
      }
      return;
    }
    ...
    this.element.dispatchEvent(
      new SidebarInsight.InsightActivated(this.#model, this.data.insightSetKey),
    );
  }
  /**
  ...
  toggleTemporaryOverlays(overlays, options) {
    ...
    if (!overlays) {
      ...
      this.element.dispatchEvent(
        new SidebarInsight.InsightProvideOverlays(
          initialOverlays,
          this.getOverlayOptionsForInitialOverlays(),
        ),
      );
      return;
    }
  ...
  #canShowAskAI() {
    ...
    return (
      aidaAvailability?.enterprisePolicyValue !== Root.Runtime.GenAiEnterprisePolicyValue.DISABLE &&
