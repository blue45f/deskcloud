...
export class DOMSize extends BaseInsightComponent {
  ...
  #renderLargeUpdatesTable() {
    ...
    const rows = this.model.largeUpdates.map((update) => {
      return {
        values: [
          eventRef(update.event, { text: update.label }),
          i18n.TimeUtilities.millisToString(update.duration),
        ],
        overlays: [
          {
            type: 'ENTRY_OUTLINE',
  ...
  renderContent() {
    ...
    return html`<div class="insight-section">
        ${widget(Table, {
          data: {
            ...
            rows: [
              ...
              {
                values: [
                  i18nString(UIStrings.maxChildren),
                  domStatsData.maxChildren?.numChildren ?? 0,
                ],
              },
            ],
          },
      ...
      ${this.#renderNodeTable(domStatsData)} ${this.#renderLargeUpdatesTable()} `;
    // clang-format on
  }
}
