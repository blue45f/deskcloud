...
export class ThirdParties extends BaseInsightComponent {
  ...
  #mainThreadTimeAggregator = {
    ...
    createAggregatedTableRow: (remaining) => {
      const totalMainThreadTime = remaining.reduce(
        (acc, summary) => acc + summary.mainThreadTime,
        0,
      );
      return {
        values: [
          renderOthersLabel(remaining.length),
          i18n.TimeUtilities.millisToString(totalMainThreadTime),
        ],
  ...
  #transferSizeAggregator = {
    ...
    createAggregatedTableRow: (remaining) => {
      ...
      return {
        values: [
          renderOthersLabel(remaining.length),
          i18n.ByteUtilities.formatBytesToKb(totalBytes),
        ],
        overlays: remaining.flatMap((summary) => createOverlaysForSummary(summary) ?? []),
      };
    },
  ...
  renderContent() {
    ...
    if (topTransferSizeEntries.length) {
      const rows = createLimitedRows(
        topTransferSizeEntries,
        this.#transferSizeAggregator,
        MAX_TO_SHOW,
      );
      // clang-format off
      sections.push(html`
        <div class="insight-section">
          ${widget(Table, {
            data: {
              insight: this,
              headers: [
                i18nString(UIStrings.columnThirdParty),
                i18nString(UIStrings.columnTransferSize),
              ],
              rows,
            },
          })}
    ...
    if (topMainThreadTimeEntries.length) {
      const rows = createLimitedRows(
        topMainThreadTimeEntries,
        this.#mainThreadTimeAggregator,
        MAX_TO_SHOW,
      );
      // clang-format off
      sections.push(html`
        <div class="insight-section">
          ${widget(Table, {
            data: {
              ...
              headers: [
                i18nString(UIStrings.columnThirdParty),
                i18nString(UIStrings.columnMainThreadTime),
              ],
              rows,
            },
          })}
