...
function renderItem(input, originMapping, index) {
  const warningIcon = Lit.Directives.until(
    renderOriginWarning(input, originMapping.productionOrigin),
  );
  // clang-format off
  return html`
    <tr data-index=${index} @edit=${input.onCommitEdit} @delete=${input.onRemoveItemRequested}>
      <td data-value=${originMapping.developmentOrigin}>
        <div class="origin" title=${originMapping.developmentOrigin}>
          ${originMapping.developmentOrigin}
        </div>
      </td>
      <td data-value=${originMapping.productionOrigin}>
        ${warningIcon}
        <div class="origin" title=${originMapping.productionOrigin}>
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  Lit.render(
    html`
      ...
      ${input.errorMessage
        ? html`<div class="error-message">${input.errorMessage}</div>`
        : Lit.nothing}
    `,
    target,
  );
...
export class OriginMap extends UI.Widget.VBox {
  ...
  #onCreate(event) {
    ...
    if (
      ...
      (devOrigin === this.#prefillDevelopmentOrigin && !prodOrigin)
    ) {
    ...
    const errors = [
      this.#developmentValidator(devOrigin),
      this.#productionValidator(prodOrigin),
    ].filter(Boolean);
    if (errors.length > 0) {
