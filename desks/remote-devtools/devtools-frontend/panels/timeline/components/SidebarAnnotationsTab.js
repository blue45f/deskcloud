...
const diagramImageUrl = new URL(
  '../../../Images/performance-panel-diagram.svg',
  import.meta.url,
).toString();
const entryLabelImageUrl = new URL(
  '../../../Images/performance-panel-entry-label.svg',
  import.meta.url,
).toString();
const timeRangeImageUrl = new URL(
  '../../../Images/performance-panel-time-range.svg',
  import.meta.url,
).toString();
const deleteAnnotationImageUrl = new URL(
  ...
  #processAnnotationsList(annotations) {
    ...
    const processedAnnotations = annotations.filter((annotation) => {
      ...
      if (annotation.type === 'ENTRIES_LINK' || annotation.type === 'ENTRY_LABEL') {
        const annotationEntry =
          annotation.type === 'ENTRIES_LINK' ? annotation.entryFrom : annotation.entry;
    ...
    processedAnnotations.sort(
      (firstAnnotation, secondAnnotation) =>
        this.#getAnnotationTimestamp(firstAnnotation) -
        this.#getAnnotationTimestamp(secondAnnotation),
    );
...
function detailedAriaDescriptionForAnnotation(annotation) {
  switch (annotation.type) {
    ...
    case 'TIME_RANGE': {
      const from = i18n.TimeUtilities.formatMicroSecondsAsMillisFixedExpanded(
        annotation.bounds.min,
      );
...
function findTextColorForContrast(bgColorText) {
  ...
  const darkColorText = Common.Color.parse(
    ThemeSupport.ThemeSupport.instance().getComputedValue(darkColorToken),
  )?.asLegacyColor();
...
function renderAnnotationIdentifier(annotation, annotationEntryToColorMap) {
  switch (annotation.type) {
    case 'ENTRY_LABEL': {
      ...
      return html`
        <span
          class="annotation-identifier"
          style=${Lit.Directives.styleMap(styleForAnnotationIdentifier)}
        >
          ${entryName}
        </span>
      `;
    ...
    case 'TIME_RANGE': {
      ...
      const timeRangeStartInMs = Math.round(
        Trace.Helpers.Timing.microToMilli(annotation.bounds.min) - minTraceBoundsMilli,
      );
      const timeRangeEndInMs = Math.round(
        Trace.Helpers.Timing.microToMilli(annotation.bounds.max) - minTraceBoundsMilli,
      );
      return html`
    ...
    case 'ENTRIES_LINK': {
      ...
      return html`
        <div class="entries-link">
          <span
            class="annotation-identifier"
            style=${Lit.Directives.styleMap(styleForFromAnnotationIdentifier)}
          >
            ${entryFromName}
          </span>
          <devtools-icon name="arrow-forward" class="inline-icon large"> </devtools-icon>
...
function renderEntryToIdentifier(annotation, annotationEntryToColorMap) {
  if (annotation.entryTo) {
    ...
    return html` <span
      class="annotation-identifier"
      style=${Lit.Directives.styleMap(styleForToAnnotationIdentifier)}
    >
      ${entryToName}
    </span>`;
    // clang-format on
  ...
  return html`<div class="annotation-tutorial-container">
    ...
    <div class="tutorial-card">
      ...
      <div class="tutorial-description">
        ${i18nString(UIStrings.deleteAnnotationTutorialDescription)}
      </div>
    </div>
  </div>`;
  // clang-format on
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html` <style>
        ${sidebarAnnotationsTabStyles}
      </style>
      <span class="annotations">
        ${input.annotations.length === 0
          ? renderTutorial()
          : html` ${input.annotations.map((annotation) => {
                const label = detailedAriaDescriptionForAnnotation(annotation);
                return html` <div
                  class="annotation-container"
                  @click=${() => input.onAnnotationClick(annotation)}
                  @mouseover=${() =>
                    annotation.type === 'ENTRY_LABEL' ? input.onAnnotationHover(annotation) : null}
                  @mouseout=${() =>
                    annotation.type === 'ENTRY_LABEL' ? input.onAnnotationHoverOut() : null}
                  aria-label=${label}
                  tabindex="0"
                  jslog=${VisualLogging.item(
                    `timeline.annotation-sidebar.annotation-${jslogForAnnotation(annotation)}`,
                  ).track({ click: true, resize: true })}
                >
                  <div class="annotation">
                    ${renderAnnotationIdentifier(annotation, input.annotationEntryToColorMap)}
                    <span class="label">
                      ${annotation.type === 'ENTRY_LABEL' || annotation.type === 'TIME_RANGE'
                        ? annotation.label
                        : ''}
                    </span>
                  </div>
                  <button
                    class="delete-button"
                    aria-label=${i18nString(UIStrings.deleteButton, { PH1: label })}
                    @click=${(event) => {
                    ...
                    jslog=${VisualLogging.action('timeline.annotation-sidebar.delete').track({
                      click: true,
                    })}
                  >
