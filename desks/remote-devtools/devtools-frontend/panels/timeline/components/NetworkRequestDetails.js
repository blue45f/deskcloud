...
const str_ = i18n.i18n.registerUIStrings(
  'panels/timeline/components/NetworkRequestDetails.ts',
  UIStrings,
);
          ...
          <div class="network-request-details-cols">
            ...
            ${
              redirectsHtml
                ? html`
                    <div class="column-divider"></div>
                    <div class="network-request-details-col redirect-details">${redirectsHtml}</div>
                  `
                : Lit.nothing
            }
            </div>
...
async function renderPreviewElement(request, target, previewElementsCache) {
  ...
  if (requestPreviewElement) {
    // clang-format off
    return html` <div class="network-request-details-col">${requestPreviewElement}</div>
      <div class="column-divider"></div>`;
    // clang-format on
  }
  return Lit.nothing;
...
function renderRow(title, value) {
  ...
  // clang-format off
  return html` <div class="network-request-details-row">
...
function renderFromCache(request) {
  const cached =
    request.args.data.syntheticData.isMemoryCached || request.args.data.syntheticData.isDiskCached;
  return renderRow(
    i18nString(UIStrings.fromCache),
    cached ? i18nString(UIStrings.yes) : i18nString(UIStrings.no),
  );
}
...
function renderServerTimings(timings) {
  ...
  // clang-format off
  return html` <div class="column-divider"></div>
...
function renderInitiatedBy(request, parsedTrace, target, linkifier) {
  ...
  // to link by URL
  const initiator = parsedTrace
    ? Trace.Extras.Initiators.getNetworkInitiator(parsedTrace.data, request)
    : undefined;
  ...
  // clang-format off
  return html` <div class="network-request-details-item">
