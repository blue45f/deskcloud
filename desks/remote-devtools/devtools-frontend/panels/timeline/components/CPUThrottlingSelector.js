...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  if (
    ...
    input.currentOption === SDK.CPUThrottlingManager.NoThrottlingOption
  ) {
  ...
  const calibrationLabel = hasCalibratedOnce
  ...
  const template = html`
    ...
    >
      ${input.groups.map((group) => {
        return html` <devtools-menu-group .name=${group.name} .title=${group.name}>
          ...
          ${group.name === 'Calibrated presets'
            ? html`<devtools-menu-item
                .value=${-1 /* This won't be displayed unless it has some value. */}
                .title=${calibrationLabel}
                jslog=${VisualLogging.action('cpu-throttling-selector-calibrate').track({
                  click: true,
                })}
...
export class CPUThrottlingSelector extends UI.Widget.Widget {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    super(element);
    this.#currentOption =
      SDK.CPUThrottlingManager.CPUThrottlingManager.instance().cpuThrottlingOption();
