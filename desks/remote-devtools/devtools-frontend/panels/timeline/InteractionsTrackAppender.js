...
export class InteractionsTrackAppender {
  ...
  #appendTrackHeaderAtLevel(currentLevel, expanded) {
    ...
    const group = buildTrackHeader(
      'interactions' /* VisualLoggingTrackName.INTERACTIONS */,
      currentLevel,
  ...
  #appendInteractionsAtLevel(trackStartLevel) {
    const { interactionEventsWithNoNesting, interactionsOverThreshold } =
      this.#parsedTrace.data.UserInteractions;
  ...
  setPopoverInfo(event, info) {
    if (Trace.Types.Events.isSyntheticInteraction(event)) {
      info.additionalElements.push(
        Components.InteractionBreakdown.InteractionBreakdown.createWidgetElement(event),
      );
    }
  }
}
