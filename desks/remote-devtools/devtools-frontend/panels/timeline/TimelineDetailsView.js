...
export class TimelineDetailsPane extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(delegate) {
    ...
    this.defaultDetailsWidget.element.setAttribute(
      'jslog',
      `${VisualLogging.pane('details').track({ resize: true })}`,
    );
  ...
  async setModel(data) {
    ...
    if (data.parsedTrace) {
      this.#summaryContent.filmStrip = Trace.Extras.FilmStrip.fromHandlerData(
  ...
  showEventInPaintProfiler(event) {
    const paintProfilerModel = SDK.TargetManager.TargetManager.instance().models(
      SDK.PaintProfiler.PaintProfilerModel,
    )[0];
  ...
  updateSelectedRangeStats(startTime, endTime) {
    ...
    const isSelectorStatsEnabled = Common.Settings.Settings.instance()
      .createSetting('timeline-capture-selector-stats', false)
      .get();
...
const SUMMARY_DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html`
      ...
      <devtools-widget
        data-related-insight-chips
        ${widget(TimelineComponents.RelatedInsightChips.RelatedInsightChips, {
...
function generateRangeSummaryDetails(input) {
  ...
  return html`${widget(TimelineComponents.TimelineRangeSummaryView.TimelineRangeSummaryView, {
    data: {
      ...
      thirdPartyTreeTemplate: input.selectedRange
        ? html`${widget(ThirdPartyTreeViewWidget, {
            model: {
              parsedTrace: input.parsedTrace,
            ...
            activeSelection:
              ...
              selectionFromRangeMilliSeconds(
                input.selectedRange.startTime,
                input.selectedRange.endTime,
              ),
...
async function renderSelectedEventDetails(input) {
  ...
  const traceRecordingIsFresh = parsedTrace
    ? Tracing.FreshRecording.Tracker.instance().recordingIsFresh(parsedTrace)
    : false;
  if (
    Trace.Types.Events.isSyntheticLayoutShift(selectedEvent) ||
    Trace.Types.Events.isSyntheticLayoutShiftCluster(selectedEvent)
  ) {
    // clang-format off
    return html` <devtools-widget
      data-layout-shift-details
      ${widget(TimelineComponents.LayoutShiftDetails.LayoutShiftDetails, {
        event: selectedEvent,
        parsedTrace: input.parsedTrace,
  ...
  if (Trace.Types.Events.isSyntheticNetworkRequest(selectedEvent)) {
    ...
    return html`
      <devtools-widget
        data-network-request-details
        ${widget(TimelineComponents.NetworkRequestDetails.NetworkRequestDetails, {
  ...
  if (Trace.Types.Events.isLegacyTimelineFrame(selectedEvent) && input.filmStrip) {
    ...
    const content = TimelineUIUtils.generateDetailsContentForFrame(
      selectedEvent,
      input.filmStrip,
      matchedFilmStripFrame,
    );
    return html`${content}`;
  }
...
function getFilmStripFrame(filmStrip, frame) {
  ...
  const frameTimeMilliSeconds = Trace.Helpers.Timing.microToMilli(
    filmStripFrame.screenshotEvent.ts,
  );
