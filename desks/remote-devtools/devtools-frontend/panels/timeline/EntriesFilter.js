...
export class EntriesFilter {
  ...
  #getEntryNode(entry) {
    ...
    // the EntryNode.
    return (
      this.#parsedTrace.data.Samples.entryToNode.get(entry) ??
  ...
  findPossibleActions(entry) {
    ...
    const allVisibleDescendants = this.#findAllDescendantsOfNode(entryNode).filter(
      (descendant) => !this.#invisibleEntries.includes(descendant),
    );
    const allVisibleRepeatingDescendants = this.#findAllRepeatingDescendantsOfNext(
      entryNode,
    ).filter((descendant) => !this.#invisibleEntries.includes(descendant));
    const allInVisibleDescendants = this.#findAllDescendantsOfNode(entryNode).filter((descendant) =>
      this.#invisibleEntries.includes(descendant),
    );
  ...
  findHiddenDescendantsAmount(entry) {
    ...
    return allDescendants.filter((descendant) => this.invisibleEntries().includes(descendant))
      .length;
  }
  /**
  ...
  #firstVisibleParentNodeForEntryNode(node) {
    ...
    while (
      (parent && this.#invisibleEntries.includes(parent.entry)) ||
    ...
    while (children.length > 0) {
      ...
      if (childNode) {
        ...
        if (/* Handle SyntheticProfileCalls */ rootIsProfileCall && childIsProfileCall) {
          ...
          if (
            Trace.Helpers.SamplesIntegrator.SamplesIntegrator.framesAreEqual(
  ...
  revealEntry(entry) {
    ...
    while (
      closestExpandableParent.parent &&
      !this.#expandableEntries.includes(closestExpandableParent.entry)
    ) {
