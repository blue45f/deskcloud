...
export function createUrlLabels(urls) {
  ...
  for (const [index, url] of urls.entries()) {
    ...
    const sameHostAndProtocol =
      previousUrl && url.host === previousUrl.host && url.protocol === previousUrl.protocol;
  ...
  return labels.map((label) =>
    label.length > 1 && label.endsWith('/') ? label.substring(0, label.length - 1) : label,
  );
}
/**
...
export function formatOriginWithEntity(url, entity, parenthesizeEntity) {
  ...
  if (entity.isUnrecognized) {
    ...
    originWithEntity = parenthesizeEntity
      ? `${origin} (${entity.name})`
      : `${origin} - ${entity.name}`;
  }
  originWithEntity = Platform.StringUtilities.trimEndWithMaxLength(
    originWithEntity,
    MAX_ORIGIN_LENGTH,
  );
