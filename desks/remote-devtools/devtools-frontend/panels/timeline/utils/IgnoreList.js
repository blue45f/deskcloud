...
function getUrlAndIgnoreListOptions(entry) {
  ...
  const sourceMappedData =
    SourceMapsResolver.SourceMapsResolver.resolvedCodeLocationForEntry(entry);
...
function isIgnoreListedURL(url, options) {
  return Workspace.IgnoreListManager.IgnoreListManager.instance().isUserIgnoreListedURL(
    url,
    options,
  );
}
...
export function getIgnoredReasonString(entry) {
  ...
  if (
    ignoreListOptions.isKnownThirdParty &&
    ignoreListMgr.automaticallyIgnoreListKnownThirdPartyScripts
  ) {
