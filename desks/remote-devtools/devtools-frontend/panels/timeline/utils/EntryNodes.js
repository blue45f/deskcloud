...
export function nodeIdsForEvent(parsedTrace, event) {
  ...
  if (Trace.Types.Events.isLayout(event)) {
    ...
    Trace.Types.Events.isPaintImage(event) &&
    typeof event.args.data.nodeId !== 'undefined'
  ) {
    foundIds.add(event.args.data.nodeId);
  } else if (
    Trace.Types.Events.isScrollLayer(event) &&
    typeof event.args.data.nodeId !== 'undefined'
  ) {
    foundIds.add(event.args.data.nodeId);
  } else if (
    ...
    const paintImageEvent = parsedTrace.data.ImagePainting.paintImageByDrawLazyPixelRef.get(
    ...
    Trace.Types.Events.isParseMetaViewport(event) &&
    typeof event.args?.data.node_id !== 'undefined'
  ) {
