  ...
  pictureForRasterTile(tileId) {
    ...
    if (!layer) {
      Common.Console.Console.instance().error(
        `Layer ${tile.layer_id} for tile ${tileId} is not found`,
      );
      return Promise.resolve(null);
    }
...
export class TracingLayer {
  ...
  snapshots() {
    return this.paints.map(async (paint) => {
      ...
      const rect = {
        x: snapshot.rect[0],
        y: snapshot.rect[1],
        width: snapshot.rect[2],
        height: snapshot.rect[3],
      };
  ...
  async pictureForRect(targetRect) {
    return await Promise.all(this.paints.map((paint) => paint.picture())).then((pictures) => {
      const filteredPictures = pictures.filter(
        (picture) => picture && rectsOverlap(picture.rect, targetRect),
      );
      const fragments = filteredPictures.map((picture) => ({
        x: picture.rect[0],
        y: picture.rect[1],
        picture: picture.serializedPicture,
      }));
      ...
      return this.paintProfilerModel
        ...
        .then((snapshot) => (snapshot ? { rect, snapshot } : null));
    });
    ...
    function rectsOverlap(a, b) {
      return (
        segmentsOverlap(a[0], a[0] + a[2], b[0], b[0] + b[2]) &&
  ...
  createScrollRects(payload) {
    ...
    if (payload.non_fast_scrollable_region) {
      nonPayloadScrollRects.push(
        this.scrollRectsFromParams(payload.non_fast_scrollable_region, 'NonFastScrollable'),
      );
    }
    if (payload.touch_event_handler_region) {
      nonPayloadScrollRects.push(
        this.scrollRectsFromParams(
          payload.touch_event_handler_region,
          'TouchEventHandler' /* Protocol.LayerTree.ScrollRectType.TouchEventHandler */,
        ),
      );
    }
    if (payload.wheel_event_handler_region) {
      nonPayloadScrollRects.push(
        this.scrollRectsFromParams(
          ...
          'WheelEventHandler' /* Protocol.LayerTree.ScrollRectType.WheelEventHandler */,
        ),
      );
    }
    if (payload.scroll_event_handler_region) {
      nonPayloadScrollRects.push(
        this.scrollRectsFromParams(
          ...
          'RepaintsOnScroll' /* Protocol.LayerTree.ScrollRectType.RepaintsOnScroll */,
        ),
      );
    }
