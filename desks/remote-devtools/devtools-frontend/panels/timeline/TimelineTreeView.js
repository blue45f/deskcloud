...
export class TimelineTreeView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  init() {
    ...
    this.currentThreadSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.dataGrid.addEventListener(
      'SortingChanged' /* DataGrid.DataGrid.Events.SORTING_CHANGED */,
      this.sortingChanged,
      this,
    );
    ...
    this.dataGrid.addEventListener(
      'OpenedNode' /* DataGrid.DataGrid.Events.OPENED_NODE */,
      this.onGridNodeOpened,
      this,
    );
  ...
  wasShown() {
    ...
    this.dataGrid.addEventListener(
      'SelectedNode' /* DataGrid.DataGrid.Events.SELECTED_NODE */,
      this.#onDataGridSelectionChange,
      this,
    );
    this.dataGrid.addEventListener(
      'DeselectedNode' /* DataGrid.DataGrid.Events.DESELECTED_NODE */,
      this.#onDataGridDeselection,
      this,
    );
  }
  lastSelectedNode() {
  ...
  populateToolbar(toolbar) {
    this.caseSensitiveButton = new UI.Toolbar.ToolbarToggle(
      i18nString(UIStrings.matchCase),
      'match-case',
      undefined,
      'match-case',
    );
  ...
  refreshTree(forceRefresh = false) {
    ...
    const countToInsert =
      this.#maxRows !== undefined ? Math.min(this.#maxRows, gridNodes.length) : gridNodes.length;
  ...
  #filterChanged() {
    ...
    this.textFilterInternal.setRegExp(
      searchQuery
        ? Platform.StringUtilities.createSearchRegex(
            searchQuery,
            caseSensitive,
            isRegex,
            matchWholeWord,
  ...
  onHover(node) {
    this.dispatchEventToListeners('TreeRowHovered' /* TimelineTreeView.Events.TREE_ROW_HOVERED */, {
      node,
    });
  }
  onClick(node) {
    this.dispatchEventToListeners('TreeRowClicked' /* TimelineTreeView.Events.TREE_ROW_CLICKED */, {
      node,
    });
  }
  ...
  performSearch(searchConfig, _shouldJump, _jumpBackwards) {
    ...
    this.searchResults = this.root.searchTree((event) =>
      TimelineUIUtils.testContentMatching(
        event,
        searchRegex.regex,
        this.#parsedTrace?.data || undefined,
      ),
    );
  ...
  jumpToNextSearchResult() {
    ...
    this.currentResult = Platform.NumberUtilities.mod(
...
export class GridNode extends DataGrid.SortableDataGrid.SortableDataGridNode {
  ...
  createNameCell(columnId) {
    ...
    if (this.profileNode.isGroupNode()) {
      ...
      this.linkElement = TimelineUIUtils.linkifyTopCallFrame(
        event,
        target,
        linkifier,
        isFreshOrEnhanced,
  ...
  createValueCell(columnId) {
      ...
      case 'start-time':
        {
          ...
          value =
            startTime - Trace.Helpers.Timing.microToMilli(parsedTrace.data.Meta.traceBounds.min);
        }
        break;
      case 'self':
    ...
    if (showPercents && this.treeView.exposePercentages()) {
      textDiv.createChild('span', 'percent-column').textContent = i18nString(
        UIStrings.percentPlaceholder,
        { PH1: ((value / this.grandTotalTime) * 100).toFixed(1) },
      );
    }
    if (maxTime) {
      ...
      cell
        ...
        .createChild('div', 'background-bar').style.width =
  ...
  #bottomUpButtonClicked() {
    ...
    this.treeView.dispatchEventToListeners(
      'TreeRowHovered' /* TimelineTreeView.Events.TREE_ROW_HOVERED */,
      { node: null },
    );
    this.treeView.dispatchEventToListeners(
      'BottomUpButtonClicked' /* TimelineTreeView.Events.BOTTOM_UP_BUTTON_CLICKED */,
      this.profileNode,
    );
  }
}
...
export class TreeGridNode extends GridNode {
  ...
  populate() {
    ...
    for (const node of this.profileNode.children().values()) {
      const gridNode = new TreeGridNode(
        node,
        this.grandTotalTime,
        this.maxSelfTime,
        this.maxTotalTime,
...
export class AggregatedTimelineTreeView extends TimelineTreeView {
  ...
  constructor(element) {
    ...
    this.stackView.addEventListener(
      'SelectionChanged' /* TimelineStackView.Events.SELECTION_CHANGED */,
  ...
  displayInfoForGroupNode(node) {
    ...
    switch (this.groupBySetting.get()) {
      case AggregatedTimelineTreeView.GroupBy.Category: {
        ...
        const category = idIsValid
          ? categories[id] || categories['other']
          : { title: unattributed, color: unattributed };
        const color =
          category instanceof Trace.Styles.TimelineCategory
      ...
      case AggregatedTimelineTreeView.GroupBy.Frame: {
        ...
        const frameName = frame
          ? TimelineUIUtils.displayNameForFrame(frame)
          : i18nString(UIStrings.page);
  ...
  populateToolbar(toolbar) {
    ...
    toolbar.appendToolbarItem(
      new UI.Toolbar.ToolbarSettingComboBox(
        options,
        this.groupBySetting,
        i18nString(UIStrings.groupBy),
      ),
    );
    if (!this.compactMode && this.splitWidget) {
    ...
    switch (groupBy) {
      ...
      case GroupBy.URL:
        return (event) => {
          ...
          return parsedTrace
            ? (Trace.Handlers.Helpers.getNonResolvedURL(event, parsedTrace.data) ?? '')
            : '';
        };
      case GroupBy.Frame:
        return (event) => {
  ...
  onHover(node) {
    if (
      node !== null &&
      this.groupBySetting.get() === AggregatedTimelineTreeView.GroupBy.ThirdParties
    ) {
...
export class CallTreeTimelineTreeView extends AggregatedTimelineTreeView {
  constructor(element) {
    ...
    this.element.setAttribute(
      'jslog',
      `${VisualLogging.pane('call-tree').track({ resize: true })}`,
    );
...
export class BottomUpTimelineTreeView extends AggregatedTimelineTreeView {
  constructor(element) {
    ...
    this.element.setAttribute(
      'jslog',
      `${VisualLogging.pane('bottom-up').track({ resize: true })}`,
    );
...
export class TimelineStackView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(treeView) {
    ...
    this.dataGrid.element.addEventListener('mouseleave', () =>
      this.dispatchEventToListeners(
        'TreeRowHovered' /* TimelineStackView.Events.TREE_ROW_HOVERED */,
        null,
      ),
    );
    this.dataGrid.asWidget().show(this.element);
  ...
  setStack(stack, selectedNode) {
    ...
    const totalTime = Math.max.apply(
      Math,
      stack.map((node) => node.totalTime),
    );
