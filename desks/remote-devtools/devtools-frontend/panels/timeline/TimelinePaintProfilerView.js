...
export class TimelinePaintProfilerView extends UI.SplitWidget.SplitWidget {
  ...
  constructor(parsedTrace) {
    ...
    this.logAndImageSplitWidget = new UI.SplitWidget.SplitWidget(
      true,
      false,
    ...
    this.paintProfilerView.addEventListener(
      'WindowChanged' /* LayerViewer.PaintProfilerView.Events.WINDOW_CHANGED */,
  ...
  async #rasterTilePromise(rasterEvent) {
    ...
    return tracingLayerTree
      ? await tracingLayerTree.pictureForRasterTile(data.tileId.id_ref)
      : null;
  }
  update() {
    ...
    void snapshotPromise.then((snapshotWithRect) => {
      ...
      void snapshot
        .commandLog()
        .then((log) => onCommandLogDone.call(this, snapshot, snapshotWithRect.rect, log || []));
    });
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  // clang-format off
  render(
    html` <div class="paint-profiler-image-view fill">
      <div
        class="paint-profiler-image-container"
        style="-webkit-transform: ${input.imageContainerWebKitTransform}"
      >
        <img
          src=${input.imageURL}
          display=${input.imageContainerHidden ? 'none' : 'block'}
          ${ref(imageElementRef)}
        />
        <div
  ...
  return {
    imageElementNaturalHeight: imageElement.naturalHeight,
...
export class TimelinePaintImageView extends UI.Widget.Widget {
  ...
  constructor(view = DEFAULT_VIEW) {
    ...
    this.#view = view;
    this.transformController = new LayerViewer.TransformController.TransformController(
      this.contentElement,
      true,
    );
    this.transformController.addEventListener(
      'TransformChanged' /* LayerViewer.TransformController.Events.TRANSFORM_CHANGED */,
  ...
  updateImagePosition() {
    ...
    this.transformController.clampOffsets(
      paddingX - bounds.maxX,
      clientWidth - paddingX - bounds.minX,
      0,
      0,
    );
  ...
  performUpdate() {
    const { imageElementNaturalHeight, imageElementNaturalWidth } = this.#view(
      this.#inputData,
      undefined,
      this.contentElement,
    );
    this.#imageElementDimensions = {
      naturalHeight: imageElementNaturalHeight,
      naturalWidth: imageElementNaturalWidth,
    };
