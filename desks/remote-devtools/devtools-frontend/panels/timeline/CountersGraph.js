...
export class CountersGraph extends UI.Widget.VBox {
  ...
  constructor(delegate) {
    ...
    this.countersByName.set(
      ...
      this.createCounter(
        ...
        'js-heap-size-used',
        'hsl(220, 90%, 43%)',
        i18n.ByteUtilities.bytesToString,
      ),
    );
    this.countersByName.set(
      'documents',
      this.createCounter(i18nString(UIStrings.documents), 'documents', 'hsl(0, 90%, 43%)'),
    );
    this.countersByName.set(
      'nodes',
      this.createCounter(i18nString(UIStrings.nodes), 'nodes', 'hsl(120, 90%, 43%)'),
    );
    this.countersByName.set(
      'jsEventListeners',
      this.createCounter(
  ...
  setModel(parsedTrace, events) {
    ...
    const minTime = Trace.Helpers.Timing.traceWindowMilliSeconds(
  ...
  createCounter(uiName, settingsKey, color, formatter) {
    ...
    this.counterUI.push(
      new CounterUI(
        ...
        color,
        counter,
        formatter ?? this.#defaultNumberFormatter.format,
      ),
    );
    return counter;
  }
...
export class CounterUI {
  ...
  constructor(countersPane, title, settingsKey, graphColor, counter, formatter) {
    ...
    this.setting = Common.Settings.Settings.instance().createSetting(
      'timeline-counters-graph-' + settingsKey,
      true,
    );
