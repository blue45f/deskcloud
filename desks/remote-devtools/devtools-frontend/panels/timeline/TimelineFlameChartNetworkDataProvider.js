...
export class TimelineFlameChartNetworkDataProvider {
  ...
  getDecorationPixels(event, unclippedBarX, timeToPixelRatio) {
    ...
    const sendStartTime = Trace.Helpers.Timing.microToMilli(
      event.args.data.syntheticData.sendStartTime,
    );
    const headersEndTime = Trace.Helpers.Timing.microToMilli(
    ...
    const finish = Math.max(
      timeToPixel(Trace.Helpers.Timing.microToMilli(event.args.data.syntheticData.finishTime)),
      headersEnd,
    );
  ...
  #decorateNetworkRequest(
    ...
    const { sendStart, headersEnd, finish, start, end } = this.getDecorationPixels(
      event,
      unclippedBarX,
      timeToPixelRatio,
    );
  ...
  #updateTimelineData(startTime, endTime) {
    ...
    // information.
    this.#maxLevel = this.#networkTrackAppender.relayoutEntriesWithinBounds(
      this.#events,
      startTime,
      endTime,
    );
  ...
  preferredHeight() {
    ...
    return (
      group.style.height *
      (this.isExpanded() ? Platform.NumberUtilities.clamp(this.#maxLevel + 1, 7, 8.5) : 1)
    );
  }
  isExpanded() {
