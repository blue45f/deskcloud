...
const str_ = i18n.i18n.registerUIStrings(
  'panels/timeline/overlays/components/EntryLabelOverlay.ts',
  UIStrings,
);
...
export class LabelAnnotationsConsentDialogVisibilityChange extends Event {
  ...
  constructor(isVisible) {
    super(LabelAnnotationsConsentDialogVisibilityChange.eventName, {
      bubbles: true,
      composed: true,
    });
...
export class EntryLabelOverlay extends HTMLElement {
  ...
  #aiAnnotationsEnabledSetting = Common.Settings.Settings.instance().createSetting(
  ...
  constructor(label, shouldDrawBelowEntry = false) {
    ...
    this.#connectorLineContainer =
      this.#labelPartsWrapper?.querySelector('.connectorContainer') ?? null;
  ...
  #handleLabelInputKeyDown(event) {
    ...
    const allowedKeysAfterReachingLenLimit = ['Backspace', 'Delete', 'ArrowLeft', 'ArrowRight'];
    ...
    if (
      (event.key === Platform.KeyboardUtilities.ENTER_KEY ||
        event.key === Platform.KeyboardUtilities.ESCAPE_KEY) &&
    ...
    if (
      this.#inputField.textContent !== null &&
  ...
  #drawConnector() {
    ...
    this.#connectorLineContainer.setAttribute(
      'height',
      EntryLabelOverlay.LABEL_CONNECTOR_HEIGHT.toString(),
    );
    ...
    const connectorColor =
      ThemeSupport.ThemeSupport.instance().getComputedValue('--color-text-primary');
  ...
  async #showUserAiFirstRunDialog() {
    ...
    const userConsented = await PanelCommon.FreDialog.show({
      ...
      header: {
        iconName: 'pen-spark',
        text: lockedString(UIStringsNotTranslate.freDisclaimerHeader),
      },
      reminderItems: [
        {
          iconName: 'psychiatry',
        ...
        {
          iconName: 'google',
          content: this.#noLogging
            ? lockedString(UIStringsNotTranslate.freDisclaimerPrivacyDataSentToGoogleNoLogging)
  ...
  #setAIButtonRenderState() {
    ...
    if (
      !hasAiExperiment ||
      aiDisabledByEnterprisePolicy ||
      !dataToGenerateLabelAvailable ||
      !labelIsEmpty
    ) {
      ...
      const aiAvailable =
        Root.Runtime.hostConfig.aidaAvailability?.enabled &&
        !Root.Runtime.hostConfig.aidaAvailability?.blockedByAge &&
        !Root.Runtime.hostConfig.aidaAvailability?.blockedByGeo &&
        navigator.onLine;
      if (aiAvailable) {
      ...
      <div class="info-tooltip-container">
        ...
        ${opts.includeSettingsButton
          ? html`
              ...
              >
                ${lockedString(UIStringsNotTranslate.learnMore)}
              </button>
            `
          : Lit.nothing}
      </div>
  ...
  #renderGeneratingLabelAiButton() {
    // clang-format off
    return html`
      <span class="ai-label-loading">
        <devtools-spinner></devtools-spinner>
        <span class="generate-label-text"
          >${lockedString(UIStringsNotTranslate.generatingLabel)}</span
        >
      </span>
    `;
    // clang-format on
    ...
    if (this.#currAIButtonState === 'generation_failed' /* AIButtonState.GENERATION_FAILED */) {
      ...
      return html`
        <span class="ai-label-error">
          ...
          <span class="generate-label-text"
            >${lockedString(UIStringsNotTranslate.generationFailed)}</span
          >
        </span>
      `;
      // clang-format on
    ...
    return html`
      ...
      >
        ...
        ${this.#renderAITooltip({
          textContent: this.#noLogging
            ? lockedString(UIStringsNotTranslate.generateLabelSecurityDisclaimerLoggingOff)
            : lockedString(UIStringsNotTranslate.generateLabelSecurityDisclaimer),
  ...
  #renderDisabledAiButton() {
    ...
    return html`
      ...
      >
        ...
        ${this.#renderAITooltip({
          textContent: noConnection
            ? lockedString(UIStringsNotTranslate.autoAnnotationNotAvailableOfflineDisclaimer)
            : lockedString(UIStringsNotTranslate.autoAnnotationNotAvailableDisclaimer),
  ...
  #render() {
    ...
    Lit.render(
      html` <style>
          ${entryLabelOverlayStyles}
        </style>
        <span
          class="label-parts-wrapper"
          role="region"
          aria-label=${i18nString(UIStrings.entryLabel)}
          @focusout=${this.#handleFocusOutEvent}
        >
          <span class="label-button-input-wrapper">
            <span
              ...
              jslog=${VisualLogging.textField('timeline.annotations.entry-label-input').track({
                keydown: true,
                click: true,
                change: true,
              })}
              tabindex="0"
            ></span>
            ${this.#isLabelEditable && this.#inputField?.innerText !== ''
              ? html`
                  <button
                    class="delete-button"
                    @click=${() => this.dispatchEvent(new EntryLabelRemoveEvent())}
                    jslog=${VisualLogging.action('timeline.annotations.delete-entry-label').track({
                      click: true,
                    })}
                  >
                    <devtools-icon
