...
const renderSection = (section, position) => {
  ...
  // clang-format off
  return html` <div class="timespan-breakdown-overlay-section" style=${style}>
    <div class="timespan-breakdown-overlay-label">
      ${section.showDuration
        ? html`<span class="duration-text"
            >${i18n.TimeUtilities.formatMicroSecondsAsMillisFixed(section.bounds.range)}</span
          > `
        : nothing}
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html` <style>
        ${timespanBreakdownOverlayStyles}
      </style>
