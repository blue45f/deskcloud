...
export class Overlays extends EventTarget {
  ...
  constructor(init) {
    ...
    this.#annotationsHiddenSetting =
      Common.Settings.Settings.instance().moduleSetting('annotations-hidden');
    ...
    init.flameChartsContainers.main.addEventListener('mousemove', (event) =>
      this.#updateMouseCoordinatesProgressEntriesLink.bind(this)(event, 'main'),
    );
    init.flameChartsContainers.network.addEventListener('mousemove', (event) =>
      this.#updateMouseCoordinatesProgressEntriesLink.bind(this)(event, 'network'),
    );
  }
  ...
  #updateMouseCoordinatesProgressEntriesLink(event, chart) {
    if (
      this.#entriesLinkInProgress?.state !==
  ...
  async update() {
    ...
    if (timeRangeOverlays.length > 1) {
      // If there are 0 or 1 overlays, they can't overlap
  ...
  #positionOverlay(overlay, element) {
    ...
    switch (overlay.type) {
      ...
      case 'TIMESPAN_BREAKDOWN': {
        ...
        if (overlay.entry) {
          ...
          const isVisible = Boolean(
            visibleWindow &&
            this.#entryIsVerticallyVisibleOnChart(overlay.entry) &&
      ...
      case 'TIMESTAMP_MARKER': {
        ...
        const isVisible = Boolean(
          visibleWindow &&
          Trace.Helpers.Timing.timestampIsInBounds(visibleWindow, overlay.timestamp),
        );
        this.#setOverlayElementVisibility(element, isVisible);
        if (isVisible) {
      ...
      case 'TIMINGS_MARKER': {
        ...
        const isVisible = Boolean(
          visibleWindow && this.#entryIsHorizontallyVisibleOnChart(overlay.entries[0]),
        );
        this.#setOverlayElementVisibility(element, isVisible);
        if (isVisible) {
          this.#positionTimingOverlay(overlay, element);
      ...
      default: {
        Platform.TypeScriptUtilities.assertNever(
          overlay,
          `Unknown overlay: ${JSON.stringify(overlay)}`,
        );
      }
    }
  }
  #positionInfoBarBanner(overlay, element) {
    ...
    const pixelsHiddenBelowViewport =
      totalHeight - (mainChart.scrollOffsetPixels + mainChart.heightPixels);
  ...
  #positionTimespanBreakdownOverlay(overlay, element) {
    ...
    const rightEdgePixel = this.#xPixelForMicroSeconds(
      'main',
      overlay.sections[overlay.sections.length - 1].bounds.max,
    );
    ...
    if (
      overlay.entry &&
      (overlay.renderLocation === 'BELOW_EVENT' || overlay.renderLocation === 'ABOVE_EVENT')
    ) {
  ...
  #positionEntriesLinkOverlay(overlay, element, entriesToConnect) {
    ...
    if (component) {
      ...
      const bothEntriesInCollapsedTrack = Boolean(
        fromEntryInCollapsedTrack && toEntryInCollapsedTrack,
      );
      ...
      const entryFromVisibility =
        this.entryIsVisibleOnChart(entryFrom) && !fromEntryInCollapsedTrack;
      const entryToVisibility = entryTo
        ? this.entryIsVisibleOnChart(entryTo) && !toEntryInCollapsedTrack
        : false;
      ...
      if (entryFromVisibility) {
        const fromEntryParams = this.#positionEntryBorderOutlineType(
          entriesToConnect.entryFrom,
          entryFromWrapper,
        );
        if (fromEntryParams) {
          ...
          component.fromEntryCoordinateAndDimensions = {
            x: fromEntryX,
            y: fromEntryY,
            length: fromEntryWidth,
            height: fromEntryHeight - fromCutOffHeight,
          };
        } else {
  ...
  #positionEntryBorderOutlineType(entry, element) {
    ...
    const chartWidth =
      chartName === 'main'
        ? this.#dimensions.charts.main?.widthPixels
    ...
    const provider =
      chartName === 'main' ? this.#charts.mainProvider : this.#charts.networkProvider;
    ...
    element.style.left = `${x}px`;
    return {
      entryHeight: totalHeight,
      entryWidth: finalWidth,
      cutOffHeight: totalHeight - height,
      x,
      y,
    };
  }
    ...
    switch (overlay.type) {
      case 'ENTRY_LABEL': {
        ...
        const callTree = parsedTrace
          ? AIAssistance.AICallTree.AICallTree.fromEvent(overlay.entry, parsedTrace)
          : null;
        ...
        component.addEventListener(
          Components.EntryLabelOverlay.EntryLabelRemoveEvent.eventName,
          () => {
            this.dispatchEvent(new AnnotationOverlayActionEvent(overlay, 'Remove'));
          },
        );
        component.addEventListener(
          Components.EntryLabelOverlay.EntryLabelChangeEvent.eventName,
          (event) => {
      ...
      case 'ENTRIES_LINK': {
        ...
        component.addEventListener(
          Components.EntriesLinkOverlay.EntryLinkStartCreating.eventName,
          () => {
            overlay.state =
              'pending_to_event' /* Trace.Types.File.EntriesLinkState.PENDING_TO_EVENT */;
            this.dispatchEvent(new AnnotationOverlayActionEvent(overlay, 'Update'));
      ...
      case 'TIME_RANGE': {
        ...
        component.addEventListener(
          Components.TimeRangeOverlay.TimeRangeLabelChangeEvent.eventName,
          (event) => {
            const newLabel = event.newLabel;
            overlay.label = newLabel;
            this.dispatchEvent(new AnnotationOverlayActionEvent(overlay, 'Update'));
        ...
        component.addEventListener(
          Components.TimeRangeOverlay.TimeRangeRemoveEvent.eventName,
          () => {
            this.dispatchEvent(new AnnotationOverlayActionEvent(overlay, 'Remove'));
          },
        );
      ...
      case 'TIMESPAN_BREAKDOWN': {
        // clang-format off
        render(
          html`${widget(Components.TimespanBreakdownOverlay.TimespanBreakdownOverlay, {
  ...
  #createOverlayPopover(adjustedTimestamp, name, fieldResult) {
    ...
    popoverContents.createChild('span', 'overlay-popover-title').textContent = fieldResult
      ? i18nString(UIStrings.fieldMetricMarkerLocal, { PH1: name })
      : name;
    ...
    if (fieldResult) {
      ...
      popoverContents.createChild('span', 'overlay-popover-title').textContent = i18nString(
        UIStrings.fieldMetricMarkerField,
        {
          PH1: name,
          PH2: scope,
        },
  ...
  #createTimingsMarkerElement(overlay) {
    ...
    for (const entry of overlay.entries) {
      ...
      marker.addEventListener('mousemove', (e) =>
        this.#mouseMoveOverlay(e, entry, title, overlay, markers, marker),
      );
  ...
  yPixelForEventOnChart(event) {
    ...
    let pixelAdjustedForScroll =
      pixelOffsetForLevel - (this.#dimensions.charts[chartName]?.scrollOffsetPixels ?? 0);
