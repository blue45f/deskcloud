...
export class ExtensionTrackAppender {
  ...
  appendTrackAtLevel(trackStartLevel, expanded) {
    const totalEntryCount = Object.values(this.#extensionTopLevelTrack.entriesByTrack).reduce(
      (prev, current) => current.length + prev,
      0,
    );
  ...
  #appendTopLevelHeaderAtLevel(currentLevel, compact, expanded) {
    ...
    const jsLogContext =
      this.#extensionTopLevelTrack.name === '🅰️ Angular'
        ? 'angular-track' /* VisualLoggingTrackName.ANGULAR_TRACK */
  ...
  #appendExtensionTrackData(trackStartLevel) {
    ...
    for (const [trackName, entries] of Object.entries(
      this.#extensionTopLevelTrack.entriesByTrack,
    )) {
      ...
      currentStartLevel = this.#compatibilityBuilder.appendEventsAtLevel(
        entries,
        currentStartLevel,
        this,
      );
  ...
  setPopoverInfo(event, info) {
    info.title =
      Trace.Types.Extensions.isSyntheticExtensionEntry(event) && event.devtoolsObj.tooltipText
