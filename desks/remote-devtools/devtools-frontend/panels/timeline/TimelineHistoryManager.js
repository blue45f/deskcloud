  ...
  return {
    format(...args) {
      if (!intlListFormat) {
        ...
        intlListFormat = new Intl.ListFormat(
...
export class TimelineHistoryManager {
  ...
  constructor(minimapComponent, isNode) {
    ...
    this.#landingPageTitle = isNode
      ? i18nString(UIStrings.nodeLandingPageTitle)
      : i18nString(UIStrings.landingPageTitle);
    ...
    this.allOverviews = [
      {
        constructor: (parsedTrace) => {
          const responsivenessOverviewFromMinimap = this.#minimapComponent
            ?.getControls()
            .find((control) => control instanceof TimelineEventOverviewResponsiveness);
          return (
            responsivenessOverviewFromMinimap ||
            new TimelineEventOverviewResponsiveness(parsedTrace)
          );
        },
        height: 3,
      },
      {
        constructor: (parsedTrace) => {
          const cpuOverviewFromMinimap = this.#minimapComponent
            ?.getControls()
            .find((control) => control instanceof TimelineEventOverviewCPUActivity);
      ...
      {
        constructor: (parsedTrace) => {
          const networkOverviewFromMinimap = this.#minimapComponent
            ?.getControls()
            .find((control) => control instanceof TimelineEventOverviewNetwork);
  ...
  addRecording(newInput) {
    ...
    const modelUsedMoreTimeAgo = this.recordings.reduce((a, b) =>
      lastUsedTime(a.parsedTraceIndex) < lastUsedTime(b.parsedTraceIndex) ? a : b,
    );
  ...
  async showHistoryDropDown() {
    ...
    const index = this.recordings.findIndex(
      (recording) => recording.parsedTraceIndex === activeTraceIndex,
    );
    if (index < 0) {
  ...
  navigate(direction) {
    ...
    const newIndex = Platform.NumberUtilities.clamp(
      index + direction,
      0,
  ...
  #buildAndStorePreviewData(parsedTraceIndex, parsedTrace, filmStrip) {
    ...
    preview.setAttribute(
      'jslog',
      `${VisualLogging.dropDown('timeline.history-item').track({ click: true })}`,
    );
  ...
  #buildTextDetails(metadata, title) {
    ...
    if (metadata) {
      const parts = [
        ...
        metadata.cpuThrottling
          ? i18nString(UIStrings.dSlowdown, { PH1: metadata.cpuThrottling })
          : undefined,
...
export class DropDown {
  ...
  constructor(availableparsedTraceIndexes, landingPageTitle) {
    ...
    this.glassPane.setPointerEventsBehavior(
      'BlockedByGlassPane' /* UI.GlassPane.PointerEventsBehavior.BLOCKED_BY_GLASS_PANE */,
    );
    this.glassPane.setAnchorBehavior(
      'PreferBottom' /* UI.GlassPane.AnchorBehavior.PREFER_BOTTOM */,
    );
    ...
    this.listControl = new UI.ListControl.ListControl(
      listModel,
      this,
  ...
  static show(
    availableparsedTraceIndexes,
    activeparsedTraceIndex,
    anchor,
    landingPageTitle = i18nString(UIStrings.landingPageTitle),
  ) {
...
export class ToolbarButton extends UI.Toolbar.ToolbarItem {
  ...
  constructor(action) {
    ...
    action.addEventListener('Enabled' /* UI.ActionRegistration.Events.ENABLED */, (event) =>
      this.setEnabled(event.data),
    );
