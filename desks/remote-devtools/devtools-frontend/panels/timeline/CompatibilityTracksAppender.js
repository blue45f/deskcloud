...
export function entryIsVisibleInTimeline(entry, parsedTrace) {
  ...
  if (
    Trace.Types.Events.isSchedulePostMessage(entry) ||
    Trace.Types.Events.isHandlePostMessage(entry)
  ) {
    return true;
  }
  ...
  const eventIsTiming =
    Trace.Types.Events.isConsoleTime(entry) ||
    Trace.Types.Events.isPerformanceMeasure(entry) ||
    Trace.Types.Events.isPerformanceMark(entry) ||
    Trace.Types.Events.isConsoleTimeStamp(entry);
  return (eventStyle && !eventStyle.hidden) || eventIsTiming;
}
// These threads have no useful information. Omit them from the UI.
const HIDDEN_THREAD_NAMES = new Set([
...
export class CompatibilityTracksAppender {
  ...
  onThemeChange() {
    for (const group of this.#flameChartData.groups) {
      ...
      group.style.backgroundColor = ThemeSupport.ThemeSupport.instance().getComputedValue(
  ...
  #addThreadAppenders() {
    ...
    for (const { pid, tid, name, type, entries, tree } of threads) {
      if (this.#parsedTrace.data.Meta.traceIsGeneric) {
        ...
        this.#threadAppenders.push(
          new ThreadAppender(
            ...
            tid,
            name,
            'OTHER' /* Trace.Handlers.Threads.ThreadType.OTHER */,
            entries,
            tree,
          ),
        );
      ...
      if (matchingWorklet) {
        ...
        const tids = [
          matchingWorklet.args.data.utilityThread.tid,
          matchingWorklet.args.data.v8HelperThread.tid,
        ];
        if (tids.includes(tid)) {
          this.#threadAppenders.push(
            new ThreadAppender(
              ...
              tid,
              '',
              'AUCTION_WORKLET' /* Trace.Handlers.Threads.ThreadType.AUCTION_WORKLET */,
              entries,
              tree,
            ),
          );
      ...
      this.#threadAppenders.push(
        new ThreadAppender(this, this.#parsedTrace, pid, tid, name, type, entries, tree),
      );
    }
    // Sort first by track order, then break ties by placing busier tracks first.
    this.#threadAppenders.sort(
      (a, b) =>
        threadTrackOrder(a) - threadTrackOrder(b) || b.getEntries().length - a.getEntries().length,
    );
  ...
  getCurrentTrackCountForThreadType(threadType) {
    return this.#threadAppenders.filter(
      (appender) => appender.threadType === threadType && appender.headerAppended(),
    ).length;
  }
  /**
  ...
  appendEventsAtLevel(events, trackStartLevel, appender, eventAppendedCallback) {
    ...
    this.#legacyEntryTypeByLevel.fill(
      'TrackAppender' /* EntryType.TRACK_APPENDER */,
      trackStartLevel,
    );
  ...
  allVisibleTrackAppenders() {
    return this.#allTrackAppenders.filter((track) =>
      this.#visibleTrackNames.has(track.appenderName),
    );
  }
  ...
  popoverInfo(event, level) {
    ...
    const info = {
      ...
      warningElements: TimelineComponents.DetailsView.buildWarningElementsForEvent(
        event,
        this.#parsedTrace,
      ),
      additionalElements: [],
    ...
    const url = URL.parse(
      info.url ??
        SourceMapsResolver.SourceMapsResolver.resolvedURLForEntry(this.#parsedTrace, event) ??
        '',
    );
    if (url) {
