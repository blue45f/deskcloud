...
export class TimelineMiniMap extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor() {
    ...
    this.#overviewComponent.addEventListener(
      'OverviewPaneBreadcrumbAdded' /* PerfUI.TimelineOverviewPane.Events.OVERVIEW_PANE_BREADCRUMB_ADDED */,
      (event) => {
    ...
    this.#breadcrumbsUI.addEventListener(
      TimelineComponents.BreadcrumbsUI.BreadcrumbActivatedEvent.eventName,
      (event) => {
        const { breadcrumb, childBreadcrumbsRemoved } = event;
        this.#activateBreadcrumb(breadcrumb, {
          removeChildBreadcrumbs: Boolean(childBreadcrumbsRemoved),
  ...
  #onOverviewPanelWindowChanged(event) {
    ...
    const left =
      event.data.startTime > 0
        ...
        : traceBoundsState.milli.entireTraceBounds.min;
    const right = Number.isFinite(event.data.endTime)
      ? event.data.endTime
      : traceBoundsState.milli.entireTraceBounds.max;
  ...
  #updateMiniMapBoundsToFitNewWindow(newWindow) {
    ...
    this.#activateBreadcrumb(lastBreadcrumbThatFits, {
  ...
  addBreadcrumb({ startTime, endTime }) {
    ...
    const newVisibleTraceWindow = Trace.Helpers.Timing.traceWindowFromMilliSeconds(
      breadcrumbTimes.startTime,
      breadcrumbTimes.endTime,
    );
