...
export class TimingsTrackAppender {
  ...
  constructor(compatibilityBuilder, parsedTrace, colorGenerator) {
    ...
    this.#extensionMarkers = extensionDataEnabled
      ? this.#parsedTrace.data.ExtensionTraceData.extensionMarkers
      : [];
  }
  /**
  ...
  appendTrackAtLevel(trackStartLevel, expanded) {
    ...
    const performanceMarks = this.#parsedTrace.data.UserTimings.performanceMarks.filter(
      (m) =>
        !Trace.Handlers.ModelHandlers.ExtensionTraceData.extensionDataInPerformanceTiming(m)
          .devtoolsObj,
    );
    ...
    const timestampEvents = this.#parsedTrace.data.UserTimings.timestampEvents.filter(
      (timeStamp) =>
        !Trace.Handlers.ModelHandlers.ExtensionTraceData.extensionDataInConsoleTimeStamp(timeStamp)
          .devtoolsObj,
    );
  ...
  #appendTrackHeaderAtLevel(currentLevel, expanded) {
    ...
    const style = buildGroupStyle({
      ...
      collapsible: trackIsCollapsible
        ? 2 /* PerfUI.FlameChart.GroupCollapsibleState.IF_MULTI_ROW */
        : 1 /* PerfUI.FlameChart.GroupCollapsibleState.NEVER */,
    });
    const group = buildTrackHeader(
      'timings' /* VisualLoggingTrackName.TIMINGS */,
      currentLevel,
  ...
  #appendExtensionsAtLevel(currentLevel) {
    ...
    const minTimeMs = Trace.Helpers.Timing.microToMilli(
      this.#parsedTrace.data.Meta.traceBounds.min,
    );
  ...
  setPopoverInfo(event, info) {
    ...
    if (
      Trace.Types.Events.isMarkerEvent(event) ||
      Trace.Types.Events.isPerformanceMark(event) ||
      Trace.Types.Events.isConsoleTimeStamp(event) ||
      isExtensibilityMarker
    ) {
