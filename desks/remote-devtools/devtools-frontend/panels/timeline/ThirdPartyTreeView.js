...
export class ThirdPartyTreeViewWidget extends TimelineTreeView.TimelineTreeView {
  ...
  constructor(element) {
    ...
    this.element.setAttribute(
      'jslog',
      `${VisualLogging.pane('third-party-tree').track({ hover: true })}`,
    );
    this.init();
  ...
  buildTree() {
    ...
    const filter = new Trace.Extras.TraceFilter.VisibleEventsFilter(
      Trace.Styles.visibleTypes().concat([
        'SyntheticNetworkRequest' /* Trace.Types.Events.Name.SYNTHETIC_NETWORK_REQUEST */,
      ]),
    );
  ...
  onHover(node) {
    if (!node) {
      this.dispatchEventToListeners(
        'TreeRowHovered' /* TimelineTreeView.TimelineTreeView.Events.TREE_ROW_HOVERED */,
        { node: null },
      );
      return;
    }
  ...
  onClick(node) {
    if (!node) {
      this.dispatchEventToListeners(
        'TreeRowClicked' /* TimelineTreeView.TimelineTreeView.Events.TREE_ROW_CLICKED */,
        { node: null },
      );
      return;
    }
  ...
  set onBottomUpButtonClicked(callback) {
    if (!this.#onBottomUpButtonClicked) {
      this.addEventListener(
        'BottomUpButtonClicked' /* TimelineTreeView.TimelineTreeView.Events.BOTTOM_UP_BUTTON_CLICKED */,
        ({ data }) => {
