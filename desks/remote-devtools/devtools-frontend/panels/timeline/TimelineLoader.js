...
export class TimelineLoader {
  ...
  static loadFromParsedJsonFile(contents, client) {
    ...
    globalThis.setTimeout(async () => {
      ...
      try {
        ...
        return loader.reportErrorAndCancelLoading(
          i18nString(UIStrings.malformedTimelineDataS, { PH1: message }),
        );
      }
    });
    return loader;
  ...
  static loadFromCpuProfile(profile, client) {
    ...
    try {
      const contents =
        Trace.Helpers.SamplesIntegrator.SamplesIntegrator.createFakeTraceFromCpuProfile(
          profile,
  ...
  static async loadFromURL(url, client) {
    ...
    client.loadingStarted();
    const allowRemoteFilePaths = Common.Settings.Settings.instance()
  ...
  #processParsedFile(trace) {
    ...
    if ('metadata' in trace) {
      ...
      const noThrottlingString =
        typeof SDK.NetworkManager.NoThrottlingConditions.title === 'string'
          ? SDK.NetworkManager.NoThrottlingConditions.title
          : SDK.NetworkManager.NoThrottlingConditions.title();
      if (
        this.#metadata.networkThrottling === 'No throttling' ||
