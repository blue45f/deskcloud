...
export class LayoutShiftsTrackAppender {
  ...
  #appendTrackHeaderAtLevel(currentLevel, expanded) {
    ...
    const group = buildTrackHeader(
      'layout-shifts' /* VisualLoggingTrackName.LAYOUT_SHIFTS */,
      currentLevel,
  ...
  #appendLayoutShiftsAtLevel(currentLevel) {
    ...
    const allLayoutShifts = this.#parsedTrace.data.LayoutShifts.clusters.flatMap(
  ...
  colorForEvent(event) {
    ...
    if (Trace.Types.Events.isSyntheticLayoutShiftCluster(event)) {
      ...
      if (parsedColor) {
        const colorWithAlpha = parsedColor
          .setAlpha(0.5)
  ...
  setPopoverInfo(event, info) {
    const score = Trace.Types.Events.isSyntheticLayoutShift(event)
      ? (event.args.data?.weighted_score_delta ?? 0)
      : Trace.Types.Events.isSyntheticLayoutShiftCluster(event)
        ? event.clusterCumulativeScore
        : -1;
    // Score isn't a duration, but the UI works anyhow.
    info.formattedTime = score.toFixed(4);
    info.title = Trace.Types.Events.isSyntheticLayoutShift(event)
  ...
  static createShiftViz(event, parsedTrace, maxSize) {
    ...
    const screenshotImageScaleFactor = Math.min(
      beforeImg.naturalWidth / viewportRect.width,
      beforeImg.naturalHeight / viewportRect.height,
      1,
    );
    ...
    const maxSizeScaleFactor = Math.min(
      maxSize.width / beforeImg.naturalWidth,
      maxSize.height / beforeImg.naturalHeight,
      1,
    );
    ...
    const beforeRects =
      event.args.data?.impacted_nodes?.map((node) => toCssPixelRect(node.old_rect)) ?? [];
    const afterRects =
      event.args.data?.impacted_nodes?.map((node) => toCssPixelRect(node.new_rect)) ?? [];
    function startVizAnimation() {
      ...
      beforeRects.forEach((beforeRect, i) => {
        ...
        if (
          [beforeRect.width, beforeRect.height, beforeRect.x, beforeRect.y].every((v) => v === 0)
        ) {
          beforePos = { ...afterPos };
          beforePos.opacity = '0';
        }
        ...
        rectEl.animate(
          [beforePos, beforePos, { ...afterPos, outlineWidth: '4px' }, afterPos, afterPos],
          vizAnimOpts,
        );
      });
    }
