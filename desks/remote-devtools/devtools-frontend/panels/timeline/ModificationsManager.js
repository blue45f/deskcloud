...
export class ModificationsManager extends EventTarget {
  ...
  updateAnnotation(updatedAnnotation) {
    ...
    if (
      overlay &&
      AnnotationHelpers.isTimeRangeLabel(overlay) &&
      ...
      overlay &&
      AnnotationHelpers.isEntriesLink(overlay) &&
  ...
  updateAnnotationOverlay(updatedOverlay) {
    ...
    if (
      (updatedOverlay.type === 'ENTRY_LABEL' &&
        annotationForUpdatedOverlay.type === 'ENTRY_LABEL') ||
    ...
    if (
      updatedOverlay.type === 'ENTRIES_LINK' &&
      annotationForUpdatedOverlay.type === 'ENTRIES_LINK'
    ) {
      this.#annotationsHiddenSetting.set(false);
   ...
   */
  toJSON() {
    const hiddenEntries = this.#entriesFilter
      .invisibleEntries()
      .map((entry) => this.#eventsSerializer.keyForEvent(entry))
      .filter((entry) => entry !== null);
    const expandableEntries = this.#entriesFilter
      .expandableEntries()
      .map((entry) => this.#eventsSerializer.keyForEvent(entry))
      .filter((entry) => entry !== null);
    this.#modifications = {
  ...
  applyModificationsIfPresent() {
    ...
    this.#timelineBreadcrumbs.setInitialBreadcrumbFromLoadedModifications(
      this.#modifications.initialBreadcrumb,
    );
        ...
        this.createAnnotation(
          {
            ...
            entryFrom: this.#eventsSerializer.eventForKey(
              linkBetweenEntries.entryFrom,
              this.#parsedTrace,
            ),
            entryTo: this.#eventsSerializer.eventForKey(
              linkBetweenEntries.entryTo,
              this.#parsedTrace,
            ),
          },
          {
  ...
  #applyEntriesFilterModifications(hiddenEntriesKeys, expandableEntriesKeys) {
    try {
      const hiddenEntries = hiddenEntriesKeys.map((key) =>
        this.#eventsSerializer.eventForKey(key, this.#parsedTrace),
      );
      const expandableEntries = expandableEntriesKeys.map((key) =>
        this.#eventsSerializer.eventForKey(key, this.#parsedTrace),
      );
