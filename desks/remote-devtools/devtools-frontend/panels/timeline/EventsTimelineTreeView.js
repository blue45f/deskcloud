...
export class EventsTimelineTreeView extends TimelineTreeView {
  ...
  constructor(delegate) {
    ...
    this.element.setAttribute(
      'jslog',
      `${VisualLogging.pane('event-log').track({ resize: true })}`,
    );
    this.filtersControl = new Filters();
    this.filtersControl.addEventListener(
      'FilterChanged' /* Events.FILTER_CHANGED */,
  ...
  showDetailsForNode(node) {
    ...
    void TimelineUIUtils.buildTraceEventDetails(
      parsedTrace,
      traceEvent,
      this.linkifier,
      false,
...
export class Filters extends Common.ObjectWrapper.ObjectWrapper {
  ...
  populateToolbar(toolbar) {
    const durationFilterUI = new UI.Toolbar.ToolbarComboBox(
      ...
      i18nString(UIStrings.durationFilter),
      undefined,
      'duration',
    );
    for (const durationMs of Filters.durationFilterPresetsMs) {
      durationFilterUI.addOption(
        durationFilterUI.createOption(
          durationMs
            ? `≥ ${i18n.TimeUtilities.millisToString(durationMs)}`
            : i18nString(UIStrings.all),
          String(durationMs),
        ),
      );
    }
