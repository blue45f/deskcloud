...
export function getAnnotationWindow(annotation) {
  ...
  switch (annotation.type) {
    case 'ENTRY_LABEL': {
      const eventDuration =
        annotation.entry.dur ?? Trace.Helpers.Timing.milliToMicro(minVisibleEntryDuration);
...
export function ariaAnnouncementForModifiedEvent(event) {
  ...
  switch (action) {
    ...
    case 'UpdateTimeRange': {
      ...
      const maxText = i18n.TimeUtilities.formatMicroSecondsAsMillisFixed(
        Trace.Types.Timing.Micro(max - traceBounds.min),
      );
