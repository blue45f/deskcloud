...
export class TimelineFlameChartView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(delegate) {
    ...
    this.networkSplitWidget = new UI.SplitWidget.SplitWidget(
      false,
      false,
    ...
    this.mainDataProvider.addEventListener(
      'DataChanged' /* TimelineFlameChartDataProviderEvents.DATA_CHANGED */,
      () => this.mainFlameChart.scheduleUpdate(),
    );
    this.mainDataProvider.addEventListener(
      'FlameChartItemHovered' /* TimelineFlameChartDataProviderEvents.FLAME_CHART_ITEM_HOVERED */,
    ...
    this.mainFlameChart.addEventListener(
      'LatestDrawDimensions' /* PerfUI.FlameChart.Events.LATEST_DRAW_DIMENSIONS */,
      (dimensions) => {
    ...
    this.networkDataProvider.setPersistedGroupConfigSetting(
    ...
    this.mainFlameChart.addEventListener(
      'MouseMove' /* PerfUI.FlameChart.Events.MOUSE_MOVE */,
      (event) => {
    ...
    this.#overlays = new Overlays.Overlays.Overlays({
      ...
      entryQueries: {
        ...
        isEntryCollapsedByUser: (entry) => {
          return (
            ModificationsManager.activeManager()?.getEntriesFilter().entryIsInvisible(entry) ??
            false
          );
        },
        firstVisibleParentForEntry(entry) {
          return (
            ModificationsManager.activeManager()
              ?.getEntriesFilter()
              .firstVisibleParentEntryForEntry(entry) ?? null
          );
        },
      },
    });
    this.#overlays.addEventListener(
      Overlays.Overlays.ConsentDialogVisibilityChange.eventName,
      (e) => {
        const event = e;
        if (event.isVisible) {
    ...
    this.#overlays.addEventListener(Overlays.Overlays.EntryLabelMouseClick.eventName, (event) => {
      ...
      this.dispatchEventToListeners(
        'EntryLabelAnnotationClicked' /* Events.ENTRY_LABEL_ANNOTATION_CLICKED */,
        {
          entry: overlay.entry,
        },
      );
    ...
    this.#overlays.addEventListener(
      Overlays.Overlays.AnnotationOverlayActionEvent.eventName,
      (event) => {
        ...
        if (action === 'Remove') {
          ...
          if (
            ModificationsManager.activeManager()?.getAnnotationByOverlay(overlay) ===
    ...
    this.element.addEventListener(
      OverlayComponents.EntriesLinkOverlay.EntryLinkStartCreating.eventName,
      () => {
        /**
    ...
    this.chartSplitWidget = new UI.SplitWidget.SplitWidget(
      false,
      true,
    ...
    this.detailsSplitWidget = new UI.SplitWidget.SplitWidget(
      false,
      true,
    ...
    this.mainFlameChart.addEventListener(
      'EntryLabelAnnotationAdded' /* PerfUI.FlameChart.Events.ENTRY_LABEL_ANNOTATION_ADDED */,
      this.onMainAddEntryLabelAnnotation,
      this,
    );
    this.mainDataProvider.addEventListener(
      'EntryLabelAnnotationAdded' /* TimelineFlameChartDataProviderEvents.ENTRY_LABEL_ANNOTATION_ADDED */,
      this.onMainAddEntryLabelAnnotation,
      this,
    );
    this.networkFlameChart.addEventListener(
    ...
    this.mainFlameChart.addEventListener(
      'EntriesLinkAnnotationCreated' /* PerfUI.FlameChart.Events.ENTRIES_LINK_ANNOTATION_CREATED */,
    ...
    this.mainFlameChart.addEventListener(
      'TracksReorderStateChange' /* PerfUI.FlameChart.Events.TRACKS_REORDER_STATE_CHANGED */,
      (event) => {
        this.#overlays.toggleAllOverlaysDisplayed(!event.data);
      },
    );
    this.detailsView.addEventListener(
      'TreeRowHovered' /* TimelineTreeView.Events.TREE_ROW_HOVERED */,
      (e) => {
        if (e.data.events) {
    ...
    this.detailsView.addEventListener(
      'TreeRowClicked' /* TimelineTreeView.Events.TREE_ROW_CLICKED */,
      (e) => {
        if (e.data.events) {
    ...
    this.mainFlameChart.addEventListener(
      'EntrySelected' /* PerfUI.FlameChart.Events.ENTRY_SELECTED */,
      this.onMainEntrySelected,
      this,
    );
    this.networkFlameChart.addEventListener(
      'EntrySelected' /* PerfUI.FlameChart.Events.ENTRY_SELECTED */,
      this.onNetworkEntrySelected,
      this,
    );
    ...
    this.mainFlameChart.addEventListener(
      'EntryInvoked' /* PerfUI.FlameChart.Events.ENTRY_INVOKED */,
      this.#onMainEntryInvoked,
      this,
    );
    this.networkFlameChart.addEventListener(
      'EntryInvoked' /* PerfUI.FlameChart.Events.ENTRY_INVOKED */,
      this.#onNetworkEntryInvoked,
      this,
    );
    this.mainFlameChart.addEventListener(
  ...
  dimThirdPartiesIfRequired() {
    ...
    const dim = Common.Settings.Settings.instance()
      .createSetting('timeline-dim-third-parties', false)
      .get();
  ...
  #updateFlameChartDimmerWithEvents(dimmer, events) {
    if (events) {
      dimmer.active = true;
      dimmer.mainChartIndices = events.map(
        (event) => this.mainDataProvider.indexForEvent(event) ?? -1,
      );
      dimmer.networkChartIndices = events.map(
        (event) => this.networkDataProvider.indexForEvent(event) ?? -1,
      );
    } else {
      dimmer.active = false;
  ...
  #refreshDimming() {
    ...
    this.delegate.set3PCheckboxDisabled(
      Boolean(dimmer && dimmer !== this.#thirdPartyCheckboxDimmer),
    );
    if (!dimmer) {
    ...
    const networkOutline =
      typeof dimmer.outline === 'boolean' ? dimmer.outline : dimmer.outline.network;
  ...
  #dimInsightRelatedEvents(relatedEvents) {
    ...
    const relatedMainIndices = relatedEvents.map(
      (event) => this.mainDataProvider.indexForEvent(event) ?? -1,
    );
    const relatedNetworkIndices = relatedEvents.map(
      (event) => this.networkDataProvider.indexForEvent(event) ?? -1,
    );
      ...
      if (overlay.type === 'TIMESPAN_BREAKDOWN') {
        ...
        if (firstSection && lastSection) {
          bounds = Trace.Helpers.Timing.traceWindowFromMicroSeconds(
            firstSection.bounds.min,
            lastSection.bounds.max,
          );
        }
    ...
    this.#updateFlameChartDimmerWithIndices(
      this.#activeInsightDimmer,
      relatedMainIndices,
      relatedNetworkIndices,
    );
  }
    ...
    for (const marker of this.#markers) {
      for (const event of marker.entries) {
        ...
        if (event.name === 'firstContentfulPaint' /* Trace.Types.Events.Name.MARK_FCP */) {
          ...
          event.name ===
          'largestContentfulPaint::Candidate' /* Trace.Types.Events.Name.MARK_LCP_CANDIDATE */
        ) {
    ...
    const markers = markerEvents.filter(
      (event) =>
        ...
        event.name ===
          'largestContentfulPaint::Candidate' /* Trace.Types.Events.Name.MARK_LCP_CANDIDATE */ ||
        event.name ===
          'largestContentfulPaint::CandidateForSoftNavigation' /* Trace.Types.Events.Name.MARK_LCP_CANDIDATE_FOR_SOFT_NAVIGATION */ ||
  ...
  setOverlays(overlays, options) {
    ...
    const traceBounds =
      TraceBounds.TraceBounds.BoundsManager.instance().state()?.micro.entireTraceBounds;
    if (!traceBounds) {
      return;
    }
    ...
    if (options.updateTraceWindow) {
      ...
      const overlaysBounds = Overlays.Overlays.traceWindowContainingOverlays(
      ...
      if (overlaysBounds) {
        ...
        const expandedBounds = Trace.Helpers.Timing.expandWindowByPercentOrToOneMillisecond(
          overlaysBounds,
          traceBounds,
          percentage,
        );
    ...
    if (entries.length !== 0) {
      const earliestEntry = entries.reduce(
        (earliest, current) => (earliest.ts < current.ts ? earliest : current),
        entries[0],
      );
  ...
  #clearLinkSelectionAnnotation(deleteCurrentLink) {
    ...
    if (
      deleteCurrentLink ||
      this.#linkSelectionAnnotation.state !==
        'connected' /* Trace.Types.File.EntriesLinkState.CONNECTED */
    ) {
  ...
  #handleTimeRangeKeyboardCreation(event) {
    const visibleWindow =
      TraceBounds.TraceBounds.BoundsManager.instance().state()?.micro.timelineTraceWindow;
    if (!visibleWindow) {
      return false;
    }
    ...
    switch (event.key) {
      ...
      case 'ArrowRight': {
        ...
        this.#timeRangeSelectionAnnotation.bounds.range = Trace.Types.Timing.Micro(
          this.#timeRangeSelectionAnnotation.bounds.max -
            this.#timeRangeSelectionAnnotation.bounds.min,
        );
  ...
  #keydownHandler(event) {
    ...
    if (
      ...
      this.#linkSelectionAnnotation.state ===
        'creation_not_started' /* Trace.Types.File.EntriesLinkState.CREATION_NOT_STARTED */
    ) {
      this.#clearLinkSelectionAnnotation(true);
  ...
  runBrickBreakerGame() {
    ...
    if (
      [...this.element.childNodes].find(
        (child) => child instanceof PerfUI.BrickBreaker.BrickBreaker,
      )
    ) {
      return;
    }
  ...
  #onTraceBoundsChange(event) {
    ...
    const userHasReducedMotionSet = globalThis.matchMedia(
      '(prefers-reduced-motion: reduce)',
    ).matches;
    const shouldAnimate =
      Boolean(event.options.shouldAnimate) &&
      (this.#checkReducedMotion ? !userHasReducedMotionSet : true);
  ...
  #registerLoggableGroups() {
    ...
    for (const group of groups) {
      ...
      const loggable =
        this.#loggableForGroupByLogContext.get(group.jslogContext) ?? Symbol(group.jslogContext);
  ...
  updateLinkSelectionAnnotationWithToEntry(dataProvider, entryIndex) {
    ...
    if (toSelectionObject) {
      ...
      const linkBetweenEntriesExists =
        ModificationsManager.activeManager()?.linkAnnotationBetweenEntriesExists(
  ...
  highlightEvent(event) {
    const entryIndex = event
      ? this.mainDataProvider.entryIndexForSelection(selectionFromEvent(event))
      : -1;
    if (entryIndex >= 0) {
  ...
  willHide() {
    super.willHide();
    this.#networkPersistedGroupConfigSetting.removeChangeListener(
      this.resizeToPreferredHeights,
      this,
    );
    Workspace.IgnoreListManager.IgnoreListManager.instance().removeChangeListener(
      this.#boundRefreshAfterIgnoreList,
    );
  }
  wasShown() {
    super.wasShown();
  ...
  async setSelectionAndReveal(selection) {
    ...
    if (
      (selection === null || !selectionIsRange(selection)) &&
      this.#timeRangeSelectionAnnotation &&
  ...
  onEntriesLinkAnnotationCreate(dataProvider, entryFromIndex, linkCreateButton) {
    const fromSelectionObject = entryFromIndex
      ? this.#selectionIfTraceEvent(entryFromIndex, dataProvider)
      : null;
    if (fromSelectionObject) {
      this.#setLinkSelectionAnnotation({
        type: 'ENTRIES_LINK',
        entryFrom: fromSelectionObject,
        state: linkCreateButton
          ? 'creation_not_started' /* Trace.Types.File.EntriesLinkState.CREATION_NOT_STARTED */
          : 'pending_to_event' /* Trace.Types.File.EntriesLinkState.PENDING_TO_EVENT */,
      });
  ...
  handleToEntryOfLinkBetweenEntriesSelection(toIndex) {
    ...
    if (this.#linkSelectionAnnotation && toIndex === -1) {
      ...
      this.#linkSelectionAnnotation &&
      this.#linkSelectionAnnotation?.entryTo &&
  ...
  jumpToNextSearchResult() {
    ...
    const index =
      typeof this.selectedSearchResult !== 'undefined'
    ...
    switch (matchedResult.provider) {
      ...
      default:
        Platform.assertNever(
          matchedResult.provider,
          `Unknown SearchResult[provider]: ${matchedResult.provider}`,
        );
    }
  ...
  #indexOfSearchResult(target) {
    ...
    return (
      this.searchResults?.findIndex((result) => {
