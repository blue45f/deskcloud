...
export class TimelineEventOverviewNetwork extends TimelineEventOverview {
  ...
  #renderWithParsedTrace(start, end) {
    ...
    for (const request of this.#parsedTrace.data.NetworkRequests.byTime) {
      const path = Trace.Helpers.Network.isSyntheticNetworkRequestHighPriority(request)
        ? highPath
        : lowPath;
      ...
      const rectEnd = Math.min(
        Math.ceil((endTime - traceBoundsMilli.min) * scale + 1),
        canvasWidth,
      );
...
export class TimelineEventOverviewCPUActivity extends TimelineEventOverview {
  ...
  constructor(parsedTrace) {
    ...
    this.#start = Trace.Helpers.Timing.traceWindowMilliSeconds(
    ...
    const eventStyle =
      Trace.Styles.getEventStyle(entry.name)?.category || Trace.Styles.getCategoryStyles().other;
  ...
  #draw(parsedTrace) {
    ...
    const drawThreadEntries = (context, threadData) => {
      ...
      const onEntryStart = (entry) => {
        ...
        const index = categoryIndexStack.length
          ? categoryIndexStack[categoryIndexStack.length - 1]
          : idleIndex;
      ...
      function onEntryEnd(entry) {
        const endTimeMilli =
          Trace.Helpers.Timing.microToMilli(entry.ts) +
      ...
      for (let i = categoryOrder.length - 1; i > 0; --i) {
        ...
        const computedColorValue = ThemeSupport.ThemeSupport.instance().getComputedValue(
          categories[categoryOrder[i]].cssVariable,
        );
    ...
    for (const thread of threads) {
      ...
      const isMainThread =
        thread.type === 'MAIN_THREAD' /* Trace.Handlers.Threads.ThreadType.MAIN_THREAD */ ||
...
export class TimelineEventOverviewResponsiveness extends TimelineEventOverview {
  ...
  update(start, end) {
    this.resetCanvas();
    const height = this.height();
    const visibleTimeWindow = !(start && end)
...
export class TimelineFilmStripOverview extends TimelineEventOverview {
  ...
  drawFrames(imageWidth, imageHeight, customStartTime, customEndTime) {
    ...
    const spanTime = customEndTime
      ? customEndTime - zeroTime
      : Trace.Helpers.Timing.microToMilli(this.#filmStrip.spanTime);
    ...
    for (let x = padding; x < width; x += imageWidth + 2 * padding) {
      ...
      const frame = Trace.Extras.FilmStrip.frameClosestToTimestamp(
        this.#filmStrip,
        timeMicroSeconds,
      );
      if (!frame) {
...
export class TimelineEventOverviewMemory extends TimelineEventOverview {
  ...
  update(start, end) {
    ...
    const counterEventsPerTrack = mainRendererIds
      .map((pid) => this.#parsedTrace.data.Memory.updateCountersByProcess.get(pid) || [])
