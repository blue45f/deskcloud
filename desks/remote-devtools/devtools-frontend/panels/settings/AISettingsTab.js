...
export const AI_SETTINGS_TAB_DEFAULT_VIEW = (input, _output, target) => {
  ...
  // clang-format off
  const disabledReasonsExplainer = input.disabledReasons.length
    ? html`
        <div class="disabled-explainer">
          ${input.disabledReasons.map(
            (reason) => html`
    ...
    <div class="shared-disclaimer">
      ...
      <div class="disclaimer-list">
        ${input.sharedDisclaimerBulletPoints.map(
          (item) =>
            html`<div><devtools-icon .name=${item.icon} class="medium"></devtools-icon></div>
              <div>${item.text}</div>`,
        )}
      </div>
    </div>
  ...
  const renderSettingItem = (settingItem) => {
    return html`
      <div>
        <devtools-icon class="extra-large" .name=${settingItem.iconName}> </devtools-icon>
      </div>
    ...
    return html`
      <div class="accordion-header" @click=${input.expandSetting.bind(this, setting)}>
        ...
        <div class="dropdown centered">
          <devtools-button
            .data=${{
              title: settingData.settingExpandState.isSettingExpanded
                ? i18nString(UIStrings.showLess)
                : i18nString(UIStrings.showMore),
              size: 'SMALL' /* Buttons.Button.Size.SMALL */,
              iconName: settingData.settingExpandState.isSettingExpanded
                ? 'chevron-up'
                : 'chevron-down',
      ...
      <div
        class="toggle-container centered"
...
export class AISettingsTab extends UI.Widget.VBox {
  ...
  constructor(view) {
    super();
    try {
      this.#consoleInsightsSetting = Common.Settings.Settings.instance().moduleSetting(
    ...
    try {
      this.#aiAssistanceSetting =
        Common.Settings.Settings.instance().moduleSetting('ai-assistance-enabled');
    } catch {
    ...
    if (Root.Runtime.hostConfig.devToolsAiGeneratedTimelineLabels?.enabled) {
      ...
      this.#aiAnnotationsSetting = Common.Settings.Settings.instance().createSetting(
    ...
    if (Root.Runtime.hostConfig.devToolsAiCodeCompletion?.enabled) {
      // Get an existing setting or, if it does not exist, create a new one.
      this.#aiCodeCompletionSetting = Common.Settings.Settings.instance().createSetting(
        ...
        toConsiderSettingItems: [
          {
            iconName: 'google',
            text: noLogging
              ? i18nString(UIStrings.consoleInsightsSendsDataNoLogging)
        ...
        toConsiderSettingItems: [
          {
            iconName: 'google',
            text: noLogging
              ? i18nString(UIStrings.freestylerSendsDataNoLogging)
        ...
        toConsiderSettingItems: [
          {
            iconName: 'google',
            text: noLogging
              ? i18nString(UIStrings.generatedAiAnnotationsSendDataNoLogging)
    ...
    if (this.#aiCodeCompletionSetting) {
      const settingItems = Root.Runtime.hostConfig.devToolsAiCodeGeneration?.enabled
        ? [
            {
              iconName: 'code',
              text: i18nString(UIStrings.asYouTypeRelevantDataIsBeingSentToGoogle),
            },
            {
              iconName: 'text-analysis',
              text: Host.Platform.isMac()
                ? i18nString(UIStrings.describeCodeInCommentForMacOs)
        ...
        toConsiderSettingItems: [
          {
            iconName: 'google',
            text: noLogging
              ? i18nString(UIStrings.codeSuggestionsSendDataNoLogging)
  ...
  #expandSetting(setting) {
    ...
    settingData.settingExpandState.isSettingExpanded =
      !settingData.settingExpandState.isSettingExpanded;
    this.requestUpdate();
  }
  #toggleSetting(setting, ev) {
    ...
    if (
      ev.target instanceof Switch.Switch.Switch &&
      ev.type !== Switch.Switch.SwitchChangeEvent.eventName
    ) {
      return;
    }
    const settingData = this.#settingToParams.get(setting);
  ...
  #getSharedDisclaimerBulletPoints() {
    const tosLink = html`<devtools-link
      href="https://policies.google.com/terms"
      .jslogContext=${'terms-of-service'}
      >${i18nString(UIStrings.termsOfService)}</devtools-link
    >`;
    const privacyNoticeLink = html`<devtools-link
      href="https://policies.google.com/privacy"
      .jslogContext=${'privacy-notice'}
      >${i18nString(UIStrings.privacyNotice)}</devtools-link
    >`;
    const noLogging =
      Root.Runtime.hostConfig.aidaAvailability?.enterprisePolicyValue ===
      Root.Runtime.GenAiEnterprisePolicyValue.ALLOW_WITHOUT_LOGGING;
    return [
      { icon: 'psychiatry', text: i18nString(UIStrings.experimentalFeatures) },
      {
        icon: 'google',
        text: noLogging
          ? i18nString(UIStrings.sendsDataToGoogleNoLogging)
          : i18nString(UIStrings.sendsDataToGoogle),
      },
      {
        icon: 'corporate-fare',
        text: noLogging
          ? i18nString(UIStrings.dataCollectionNoLogging)
          : i18nString(UIStrings.dataCollection),
      },
      {
        icon: 'policy',
