        ...
        <div class="settings-card-container">
          <devtools-card heading=${i18nString(UIStrings.workspace)}>
            <div class="folder-exclude-pattern">
              <label for="workspace-setting-folder-exclude-pattern"
                >${i18nString(UIStrings.folderExcludePattern)}</label
              >
              <input
                class="harmony-input"
                jslog=${VisualLogging.textField()
                  .track({ keydown: 'Enter', change: true })
                  .context(input.excludePatternSetting.name)}
            ...
            <div class="mappings-info">
              ${i18nString(UIStrings.mappingsAreInferredAutomatically)}
            </div>
          </devtools-card>
          ${input.fileSystems.map(
            (fileSystem) => html`
              <devtools-card heading=${fileSystem.displayName}>
                ...
                <devtools-button
                  ...
                  jslog=${VisualLogging.action()
                    .track({ click: true })
                    .context('settings.remove-file-system')}
          ...
          <div class="add-button-container">
            <devtools-button
              ...
              jslog=${VisualLogging.action()
                .track({ click: true })
                .context('sources.add-folder-to-workspace')}
              @click=${input.onAddClicked}
              >${i18nString(UIStrings.addFolder)}</devtools-button
            >
          </div>
        </div>
      </div>`,
  ...
  performUpdate() {
    const input = {
      excludePatternSetting:
        Persistence.IsolatedFileSystemManager.IsolatedFileSystemManager.instance().workspaceFolderExcludePatternSetting(),
      onAddClicked: () =>
        Persistence.IsolatedFileSystemManager.IsolatedFileSystemManager.instance().addFileSystem(),
      onRemoveClicked: (fs) =>
        Persistence.IsolatedFileSystemManager.IsolatedFileSystemManager.instance().removeFileSystem(
          fs,
        ),
      fileSystems: Persistence.IsolatedFileSystemManager.IsolatedFileSystemManager.instance()
        ...
        .filter((fileSystem) => {
          ...
          return (
            fileSystem instanceof Persistence.IsolatedFileSystem.IsolatedFileSystem &&
