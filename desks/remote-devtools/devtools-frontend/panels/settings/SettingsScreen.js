...
export class SettingsScreen extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const settingsTitleElement = UI.UIUtils.createShadowRootWithCoreStyles(settingsLabelElement, {
      cssFile: settingsScreenStyles,
  ...
  static revealSettingsScreen() {
    ...
    dialog.setPointerEventsBehavior(
      'PierceGlassPane' /* UI.GlassPane.PointerEventsBehavior.PIERCE_GLASS_PANE */,
    );
    dialog.setOutsideTabIndexBehavior(
      'PreserveMainViewTabIndex' /* UI.Dialog.OutsideTabIndexBehavior.PRESERVE_MAIN_VIEW_TAB_INDEX */,
    );
...
export class GenericSettingsTab extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const preRegisteredSettings = Common.Settings.Settings.instance()
    ...
    for (const sectionCategory of explicitSectionOrder) {
      const settingsForSection = preRegisteredSettings.filter(
        (setting) =>
          setting.category === sectionCategory && GenericSettingsTab.isSettingVisible(setting),
      );
  ...
  updateSyncSection() {
    ...
    this.#syncSectionUpdatePromise = new Promise((resolve) =>
      Host.InspectorFrontendHost.InspectorFrontendHostInstance.getSyncInformation(resolve),
    ).then((syncInfo) => {
      this.syncSection.syncInfo = syncInfo;
      if (!syncInfo.isSyncActive || !syncInfo.arePreferencesSynced) {
        this.#updateSyncSectionTimerId = globalThis.setTimeout(
          this.updateSyncSection.bind(this),
          500,
        );
      }
    });
  ...
  createSectionElement(category, settings) {
    ...
    if (category === 'EXTENSIONS' /* Common.Settings.SettingCategory.EXTENSIONS */) {
      ...
      const syncCard = createSettingsCard(
        Common.SettingRegistration.getLocalizedSettingsCategory(
          'ACCOUNT' /* Common.SettingRegistration.SettingCategory.ACCOUNT */,
        ),
  ...
  createStandardSectionElement(category, settings, content) {
    ...
    for (const settingRegistration of settings) {
      const setting = Common.Settings.Settings.instance().moduleSetting(
        settingRegistration.settingName,
      );
...
export class ExperimentsSettingsTab extends UI.Widget.VBox {
      ...
      html`
        <devtools-toolbar>
          <devtools-toolbar-input
            ...
            style="flex-grow:1"
            @change=${this.#onFilterChanged.bind(this)}
          ></devtools-toolbar-input>
        </devtools-toolbar>
      `,
      filterSection,
  ...
  renderExperiments(filterText) {
    ...
    if (filteredExperiments.length) {
      ...
      this.#experimentsSection = createSettingsCard(
        i18nString(UIStrings.experiments),
        warningSection,
        experimentsBlock,
      );
      this.containerElement.appendChild(this.#experimentsSection);
      UI.ARIAUtils.LiveAnnouncer.alert(
        i18nString(UIStrings.experimentsFound, { n: filteredExperiments.length }),
      );
    } else {
  ...
  createExperimentCheckbox(experiment) {
    ...
    if (experiment.feedbackLink) {
      const link = Link.create(
        experiment.feedbackLink,
        undefined,
        undefined,
        `${experiment.name}-feedback`,
      );
  ...
  highlightObject(experiment) {
    if (
      experiment instanceof Root.Runtime.Experiment ||
      experiment instanceof Root.Runtime.HostExperiment
    ) {
...
export class Revealer {
  async reveal(object) {
    ...
    if (
      object instanceof Root.Runtime.Experiment ||
      object instanceof Root.Runtime.HostExperiment
    ) {
...
function renderPrototypeCheckboxes(settings, keys) {
  ...
  const checkboxes = Object.keys(settings).map((name) => {
    ...
    return html`<p class="settings-experiment">
      <devtools-checkbox @change=${showChangeWarning} title=${title} ${bindToSetting(setting)}
        >${title}</devtools-checkbox
      >
    </p>`;
  });
