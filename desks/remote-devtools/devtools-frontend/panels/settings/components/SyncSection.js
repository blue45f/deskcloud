...
const DEFAULT_VIEW = (input, output, target) => {
  const renderSettingCheckboxIfNeeded = () => {
    ...
    const warningText =
      input.warningType === 'SYNC_DISABLED' /* WarningType.SYNC_DISABLED */
        ? i18nString(UIStrings.syncDisabled)
        : i18nString(UIStrings.preferencesSyncDisabled);
    return html`
      <div class="setting-checkbox-container">
        <setting-checkbox class="setting-checkbox" .data=${{ setting: input.syncSetting }}>
        </setting-checkbox>
        ${input.warningType
          ? html` <devtools-button
              ...
              <devtools-tooltip id="settings-sync-info" variant="rich">
                ${warningText}
              </devtools-tooltip>`
          : Lit.nothing}
      </div>
  ...
  const renderAccountInfo = () => {
    if (!input.syncInfo.accountEmail) {
      return html` <div class="not-signed-in">${i18nString(UIStrings.notSignedIn)}</div> `;
    }
    return html` <div class="account-info">
      <img
        class="account-avatar"
        src="data:image/png;base64, ${input.syncInfo.accountImage}"
        alt="Account avatar"
      />
                ...
                ${hasReceiveBadgesCheckbox
                  ? html`
                      ...
                      >
                        ...
                        <span
                          >${i18nTemplate(UIStrings.dataDisclaimer, {
                            PH1: html` <span
                                class="link"
                                tabindex="0"
                              ...
                              <devtools-tooltip id="gdp-profile-tooltip" variant="rich">
                                <div class="tooltip-content" tabindex="0">
                                  ${i18nString(UIStrings.tooltipDisclaimerText)}
                                </div>
                              </devtools-tooltip>`,
                          })}
                        </span>
  ...
  render(
    html`
      ...
      <fieldset>
        ${renderAccountInfo()} ${renderSettingCheckboxIfNeeded()} ${renderGdpSectionIfNeeded()}
      </fieldset>
    `,
    target,
  );
...
export class SyncSection extends UI.Widget.Widget {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    super(element);
    this.#view = view;
    this.#receiveBadgesSetting =
      Common.Settings.Settings.instance().moduleSetting('receive-gdp-badges');
  ...
  performUpdate() {
    ...
    const viewInput = {
      ...
      isEligibleToCreateGdpProfile:
        Host.GdpClient.isGdpProfilesAvailable() && this.#isEligibleToCreateGdpProfile,
  ...
  #onReceiveBadgesSettingClick(e) {
    ...
    void Badges.UserBadges.instance()
      .initialize()
      .then(() => {
        if (!settingCheckbox.checked) {
          return;
        }
        Badges.UserBadges.instance().recordAction(
