  ...
  render(
    html` <style>
      ...
      <div class="excluded-folder-header">
                ...
                (path, index) => html`
                  ...
                  >
                    <td
                      style=${styleMap({
                        backgroundColor:
                          path.status !== 1 /* ExcludedFolderStatus.VALID */
        ...
        ${input.excludedFolderPaths
          .filter(({ status }) => status !== 1 /* ExcludedFolderStatus.VALID */)
          .map(
            ({ status }) =>
