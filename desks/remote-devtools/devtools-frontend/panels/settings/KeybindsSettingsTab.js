...
export class KeybindsSettingsTab extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const keybindsSetSetting =
      Common.Settings.Settings.instance().moduleSetting('active-keybind-set');
    const userShortcutsSetting =
      Common.Settings.Settings.instance().moduleSetting('user-shortcuts');
    ...
    this.list = new UI.ListControl.ListControl(
      this.items,
      this,
    ...
    const restoreDefaultShortcutsButton = UI.UIUtils.createTextButton(
      i18nString(UIStrings.RestoreDefaultShortcuts),
      () => {
    ...
    for (const [originalShortcut, newDescriptors] of editedShortcuts) {
      ...
      if (newDescriptors) {
        UI.ShortcutRegistry.ShortcutRegistry.instance().registerUserShortcut(
          originalShortcut
            .changeKeys(newDescriptors)
            .changeType('UserShortcut' /* UI.KeyboardShortcut.Type.USER_SHORTCUT */),
        );
        if (
          originalShortcut.type === 'UnsetShortcut' /* UI.KeyboardShortcut.Type.UNSET_SHORTCUT */
        ) {
...
export class ShortcutListItem {
  ...
  setupEditor() {
    ...
    this.confirmButton = this.createIconButton(
      ...
      () => {
        ...
        UI.ARIAUtils.LiveAnnouncer.alert(
          i18nString(UIStrings.shortcutChangesApplied, { PH1: this.item.title() }),
        );
      },
    );
    ...
    this.element.appendChild(
      this.createIconButton(
        i18nString(UIStrings.discardChanges),
        'cross',
  ...
  addShortcut() {
    const shortcut = new UI.KeyboardShortcut.KeyboardShortcut(
      [],
      this.item.id(),
      'UnsetShortcut' /* UI.KeyboardShortcut.Type.UNSET_SHORTCUT */,
    );
  ...
  createShortcutRow(shortcut, index) {
    ...
    let icon;
    if (
      shortcut.type !== 'UnsetShortcut' /* UI.KeyboardShortcut.Type.UNSET_SHORTCUT */ &&
      !shortcut.isDefault()
    ) {
    ...
    if (this.isEditing) {
      ...
      shortcutInput.addEventListener(
        'keydown',
        this.onShortcutInputKeyDown.bind(this, shortcut, shortcutInput),
      );
      ...
      shortcutElement.appendChild(
        this.createIconButton(
          i18nString(UIStrings.removeShortcut),
          'bin',
          ...
          () => {
            ...
            this.validateInputs();
            UI.ARIAUtils.LiveAnnouncer.alert(
              i18nString(UIStrings.shortcutRemoved, { PH1: this.item.title() }),
            );
          },
        ),
      );
  ...
  onShortcutInputKeyDown(shortcut, shortcutInput, event) {
    if (event.key !== 'Tab') {
      ...
      const isLastKeyOfShortcut =
        userDescriptors.length === 2 &&
        UI.KeyboardShortcut.KeyboardShortcut.isModifier(userDescriptors[1].key);
  ...
  descriptorForEvent(event) {
    ...
    const codeAndModifiers =
      UI.KeyboardShortcut.KeyboardShortcut.keyCodeAndModifiersFromKey(userKey);
  ...
  resetShortcutsToDefaults() {
    ...
    const disabledDefaults =
      UI.ShortcutRegistry.ShortcutRegistry.instance().disabledDefaultsForAction(this.item.id());
    ...
    this.focus();
    UI.ARIAUtils.LiveAnnouncer.alert(
      i18nString(UIStrings.shortcutChangesRestored, { PH1: this.item.title() }),
    );
  }
  onEscapeKeyPressed(event) {
  ...
  validateInputs() {
    ...
    this.shortcutInputs.forEach((shortcutInput, shortcut) => {
      ...
      if (
        userDescriptors.some((descriptor) =>
          UI.KeyboardShortcut.KeyboardShortcut.isModifier(descriptor.key),
        )
      ) {
