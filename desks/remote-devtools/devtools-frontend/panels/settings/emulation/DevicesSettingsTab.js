...
const str_ = i18n.i18n.registerUIStrings(
  'panels/settings/emulation/DevicesSettingsTab.ts',
  UIStrings,
);
...
export class DevicesSettingsTab extends UI.Widget.VBox {
  ...
  commitEdit(device, editor, isNew) {
    ...
    if (
      uaType === 'Mobile' /* EmulationModel.DeviceModeModel.UA.MOBILE */ ||
  ...
  beginEdit(device) {
    ...
    editor.control('ua-metadata').value = { metaData: device.userAgentMetadata || undefined };
    return editor;
  }
  createEditor() {
    if (this.editor) {
    ...
    const deviceNameField = editor.createInput(
      'title',
      'text',
      i18nString(UIStrings.deviceName),
      titleValidator,
    );
    ...
    screen.appendChild(
      editor.createInput('width', 'text', i18nString(UIStrings.width), widthValidator),
    );
    screen.appendChild(
      editor.createInput('height', 'text', i18nString(UIStrings.height), heightValidator),
    );
    const dpr = editor.createInput(
      'scale',
      'text',
    ...
    UI.UIUtils.createTextChild(
      uaStringFields.createChild('b'),
      i18nString(UIStrings.userAgentString),
    );
    ...
    function titleValidator(_item, _index, input) {
      ...
      if (value.length >= EmulationModel.DeviceModeModel.MaxDeviceNameLength) {
        errorMessage = i18nString(UIStrings.deviceNameMustBeLessThanS, {
          PH1: EmulationModel.DeviceModeModel.MaxDeviceNameLength,
        });
