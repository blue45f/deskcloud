...
export class UserAgentClientHintsForm extends HTMLElement {
  ...
  #handleTreeClick = (key) => {
    ...
    if (
      (key === 'ArrowLeft' && !this.#isFormOpened) ||
      (key === 'ArrowRight' && this.#isFormOpened)
    ) {
      return;
    }
    ...
    return html`
      <label class="full-row label input-field-label-container">
        ...
        <input
          ...
          jslog=${VisualLogging.textField()
            .track({ change: true })
            .context(Platform.StringUtilities.toKebabCase(stateKey))}
        />
      </label>
    `;
  }
  ...
  #renderDeviceModelSection() {
    ...
    const mobileCheckboxInput = this.#showMobileCheckbox
      ? html`
    ...
    return html`
      ...
      <div
        class="full-row brand-row"
        aria-label=${i18nString(UIStrings.deviceProperties)}
        role="group"
      >
        <input
  ...
  #renderUseragent() {
    ...
    return html`
      ...
      >
        <devtools-icon aria-hidden="true" name="plus" class="medium"> </devtools-icon>
  ...
  #renderFullVersionList() {
    ...
    return html`
      ...
      >
        <devtools-icon name="plus" class="medium" aria-hidden="true"> </devtools-icon>
        ${i18nString(UIStrings.addBrand)}
      </div>
    `;
  }
  #renderFormFactorsSection() {
    ...
    return html`
      ...
      <div
        class="full-row form-factors-checkbox-group"
        role="group"
        aria-label=${i18nString(UIStrings.formFactorsGroupAriaLabel)}
      >
        ${checkboxElements}
      </div>
    `;
  ...
  #render() {
    ...
    // clang-format off
    const submitButton = this.#showSubmitButton
      ? html`
          <devtools-button
    ...
    const output = html`
      ...
      <style>
        ${userAgentClientHintsFormStyles}
      </style>
      <section class="root">
        ...
        >
          ...
          ${architectureInput}
          <hr class="section-separator" />
          ${deviceModelSection} ${submitButton}
        </form>
    ...
    for (const [metaDataKey, metaDataValue] of Object.entries(this.#metaData)) {
      if (metaDataKey === 'brands' || metaDataKey === 'fullVersionList') {
        ...
        const isBrandValid = this.#metaData.brands?.every(({ brand, version }) => {
          const brandNameResult =
            EmulationUtils.UserAgentMetadata.validateAsStructuredHeadersString(
              brand,
              i18nString(UIStrings.notRepresentable),
            );
          const brandVersionResult =
            EmulationUtils.UserAgentMetadata.validateAsStructuredHeadersString(
              version,
              i18nString(UIStrings.notRepresentable),
            );
          ...
          for (const ff of formFactors) {
            if (!ALL_PROTOCOL_FORM_FACTORS.includes(ff)) {
              return {
                valid: false,
                errorMessage:
                  i18nString(UIStrings.notRepresentable) + ` (Invalid form factor: ${ff})`,
              };
            }
