...
var N = (n, t) => {
  for (var e in t) T(n, e, { get: t[e], enumerable: !0 });
};
var I = {};
N(I, { DEFAULT_VIEW: () => E, WebauthnPaneImpl: () => g });
...
var {
    render: j,
    html: d,
    Directives: { ref: y, repeat: k, classMap: b },
  } = f,
  { widget: P } = h.Widget,
  s = {
    export: 'Export',
  ...
  x = m.i18n.registerUIStrings('panels/webauthn/WebauthnPane.ts', s),
  o = m.i18n.getLocalizedString.bind(void 0, x),
  z = f.i18nTemplate.bind(void 0, x),
  M = 'https://developer.chrome.com/docs/devtools/webauthn';
function B(n, t, e, a) {
  return d`
...
`,
  O = `-----END ${S} KEY-----`,
  D = { Ctap2: 'ctap2', U2f: 'u2f' };
function q(n, t) {
  let e = o(s.enableVirtualAuthenticator);
  return d`
        ...
        <div class="authenticator-option">
          ...
          <input id="hmac-secret-mc" class="authenticator-option-checkbox" type="checkbox"
              ...
              @change=${(r) => {
                e(
                  r.target.checked
                    ? { hasHmacSecretMc: !0, hasHmacSecret: !0 }
                    : { hasHmacSecretMc: !1 },
                );
              }}
...
function F(n, t, e, a, i, r, p, C, L, V, U) {
  function H(c) {
    if (!c) return;
    let R = globalThis.matchMedia('(prefers-reduced-motion: reduce)').matches;
    c.scrollIntoView({ block: 'nearest', behavior: R ? 'auto' : 'smooth' });
  }
  return d`
    <div class="authenticator-section" data-authenticator-id=${n}
         jslog=${l.section('authenticator')}
          ${y((c) => {
            U.revealSection.set(n, H.bind(null, c));
          })}>
...
var E = (n, t, e) => {
    ...
    <div class="webauthn-pane flex-auto ${b({ enabled: n.enabled })}">
      ...
      <div class="authenticators-view">
         ${k(
           [...n.authenticators.entries()],
           ([a]) => a,
           ([a, i]) =>
             F(
               a,
               i,
               n.activeAuthenticatorId === a,
               n.editingAuthenticatorId === a,
               n.onActivateAuthenticator.bind(n, a),
               n.onEditName.bind(n, a),
               n.onSaveName.bind(n, a),
               n.onRemoveAuthenticator.bind(n, a),
               n.onExportCredential,
               n.onRemoveCredential.bind(n, a),
               t,
             ),
         )}
      </div>
  ...
  g = class extends h.Panel.Panel {
    async #h(t) {
      if (!this.#t) throw new Error('WebAuthn model is not available.');
      let e = await this.#t.addAuthenticator(t),
        a = e.slice(-5);
      return (
        this.#e.set(e, { name: a, options: t, credentials: [] }),
        this.requestUpdate(),
        this.#t.addEventListener('CredentialAdded', this.#f.bind(this, e)),
        this.#t.addEventListener('CredentialAsserted', this.#b.bind(this, e)),
        this.#t.addEventListener('CredentialUpdated', this.#b.bind(this, e)),
        this.#t.addEventListener('CredentialDeleted', this.#$.bind(this, e)),
        e
      );
    }
    #i = null;
    ...
    #s = {
      ...
      transport: 'usb',
      hasResidentKey: !1,
      hasUserVerification: !1,
      hasLargeBlob: !1,
      automaticPresenceSimulation: !0,
      isUserVerified: !0,
    };
    #r = !1;
    #l;
    #v;
    #p = { revealSection: new Map() };
    constructor(t = E) {
      (super('webauthn'),
        (this.#v = t),
        v.TargetManager.TargetManager.instance().observeModels(
          v.WebAuthnModel.WebAuthnModel,
          this,
          { scoped: !0 },
        ),
        (this.#a = A.Settings.Settings.instance().createSetting('webauthn-authenticators', [])),
        this.#c(),
        this.performUpdate());
    }
    performUpdate() {
      let t = {
        enabled: this.#n,
        onToggleEnabled: this.#A.bind(this),
        authenticators: this.#e,
        activeAuthenticatorId: this.#i,
        editingAuthenticatorId: this.#o,
        newAuthenticatorOptions: this.#s,
        internalTransportAvailable: !this.#r,
        updateNewAuthenticatorOptions: this.#w.bind(this),
        addAuthenticator: this.#k.bind(this),
        onActivateAuthenticator: this.#d.bind(this),
        onEditName: this.#E.bind(this),
        onSaveName: this.#I.bind(this),
        onRemoveAuthenticator: this.removeAuthenticator.bind(this),
        onExportCredential: this.#x.bind(this),
        onRemoveCredential: this.#S.bind(this),
      };
      this.#v(t, this.#p, this.contentElement);
    }
    modelAdded(t) {
      t.target() === t.target().outermostTarget() && (this.#t = t);
    }
    modelRemoved(t) {
      t.target() === t.target().outermostTarget() && (this.#t = void 0);
    }
    async #m() {
      let t = null,
        e = this.#a.get();
      for (let a of e) {
        if (!this.#t) continue;
        let i = await this.#h(a);
        ((a.authenticatorId = i), a.active && (t = i));
      }
      (this.#a.set(e), t && this.#d(t));
    }
    async ownerViewDisposed() {
      ((this.#n = !1), await this.#g(!1));
    }
    #f(t, { data: e }) {
      let a = this.#e.get(t);
      a && (a.credentials.push(e.credential), this.requestUpdate());
    }
    #b(t, { data: e }) {
      let a = this.#e.get(t);
      if (!a) return;
      let i = a.credentials.find((r) => r.credentialId === e.credential.credentialId);
      i && (Object.assign(i, e.credential), this.requestUpdate());
    }
    #$(t, { data: e }) {
      let a = this.#e.get(t);
      if (!a) return;
      let i = a.credentials.findIndex((r) => r.credentialId === e.credentialId);
      i < 0 || (a.credentials.splice(i, 1), this.requestUpdate());
    }
    async #g(t) {
      (await this.#l,
        (this.#l = new Promise(async (e) => {
          (t &&
            !this.#u &&
            (u.userMetrics.actionTaken(u.UserMetrics.Action.VirtualAuthenticatorEnvironmentEnabled),
            (this.#u = !0)),
            this.#t && (await this.#t.setVirtualAuthEnvEnabled(t)),
            t ? await this.#m() : this.#y(),
            (this.#l = void 0),
            (this.#n = t),
            this.requestUpdate(),
            e());
    ...
    #w(t) {
      (Object.assign(this.#s, t), this.requestUpdate());
    }
    #c() {
      ((this.#r = !!this.#a.get().find((t) => t.transport === 'internal')),
        this.#r && this.#s.transport === 'internal' && (this.#s.transport = 'nfc'),
        this.requestUpdate());
    }
    async #k() {
      let t = { ...this.#s };
      if (this.#t) {
        ...
        this.#i = e;
        let a = this.#a.get();
        (a.push({ authenticatorId: e, active: !0, ...t }),
          this.#a.set(a.map((i) => ({ ...i, active: i.authenticatorId === e }))),
          this.#c(),
    ...
    #x(t) {
      let e = K;
      for (let i = 0; i < t.privateKey.length; i += 64)
        e +=
          t.privateKey.substring(i, i + 64) +
          `
`;
      e += O;
      let a = document.createElement('a');
      ((a.download = o(s.privateKeypem)),
        (a.href = 'data:application/x-pem-file,' + encodeURIComponent(e)),
        a.click());
    }
    #S(t, e) {
      let a = this.#e.get(t);
      if (!a) return;
      let i = a.credentials.findIndex((r) => r.credentialId === e);
      i < 0 ||
        (a.credentials.splice(i, 1),
        this.requestUpdate(),
        this.#t && this.#t.removeCredential(t, e));
    }
    #E(t) {
      ((this.#o = t), this.requestUpdate());
    }
    #I(t, e) {
      let a = this.#e.get(t);
      a && ((a.name = e), (this.#o = null), this.requestUpdate());
    }
    removeAuthenticator(t) {
      (this.#e.delete(t), this.requestUpdate(), this.#t && this.#t.removeAuthenticator(t));
      let a = this.#a.get().filter((i) => i.authenticatorId !== t);
      if ((this.#a.set(a), this.#i === t)) {
        let i = Array.from(this.#e.keys());
        i.length ? this.#d(i[0]) : (this.#i = null);
      }
      this.#c();
    }
    async #d(t) {
      (await this.#C(),
        this.#t && (await this.#t.setAutomaticPresenceSimulation(t, !0)),
        (this.#i = t));
      let a = this.#a.get().map((i) => ({ ...i, active: i.authenticatorId === t }));
      (this.#a.set(a), this.requestUpdate());
    }
    async #C() {
      (this.#i && this.#t && (await this.#t.setAutomaticPresenceSimulation(this.#i, !1)),
        (this.#i = null),
        this.requestUpdate());
    }
  };
