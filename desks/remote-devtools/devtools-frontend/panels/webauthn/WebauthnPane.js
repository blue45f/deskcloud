...
const {
  render,
  html,
  Directives: { ref, repeat, classMap },
} = Lit;
...
function renderCredentialsDataGrid(authenticatorId, credentials, onExport, onRemove) {
  // clang-format off
  return html` <devtools-data-grid name=${i18nString(UIStrings.credentials)} inline striped>
    <table>
      <thead>
        <tr>
          <th id="credentialId" weight="24" text-overflow="ellipsis">
            ${i18nString(UIStrings.id)}
          </th>
          <th id="isResidentCredential" type="boolean" weight="10">
...
function renderToolbar(enabled, onToggle) {
  ...
  // clang-format off
  return html` <div
    class="webauthn-toolbar-container"
...
function renderNewAuthenticatorSection(options, internalTransportAvailable, onUpdate, onAdd) {
  ...
  // clang-format off
  return html` <div class="new-authenticator-container">
    <label class="new-authenticator-title"> ${i18nString(UIStrings.newAuthenticator)} </label>
    <div class="new-authenticator-form" jslog=${VisualLogging.section('new-authenticator')}>
      <div class="authenticator-option">
        ...
        <select
          id="protocol"
          jslog=${VisualLogging.dropDown('protocol').track({ change: true })}
          ...
          ${Object.values(PROTOCOL_AUTHENTICATOR_VALUES)
            ...
            .map(
              (option) =>
                html` <option
                  value=${option}
                  jslog=${VisualLogging.item(option).track({ click: true })}
                >
                  ${option}
                </option>`,
            )}
    ...
    <div class="authenticator-section-header">
      <div class="authenticator-section-title" role="heading" aria-level="2">
        <devtools-toolbar class="edit-name-toolbar">
          <devtools-button
            title=${i18nString(UIStrings.editName)}
            class=${classMap({ hidden: editing })}
            @click=${onEditName}
            .iconName=${'edit'}
            .variant=${'toolbar' /* Buttons.Button.Variant.TOOLBAR */}
            .jslogContext=${'edit-name'}
          ></devtools-button>
          <devtools-button
            title=${i18nString(UIStrings.saveName)}
            @click=${(e) => onSaveName((e.target.parentElement?.nextSibling).value)}
            .iconName=${'checkmark'}
            .variant=${'toolbar' /* Buttons.Button.Variant.TOOLBAR */}
            class=${classMap({ hidden: !editing })}
            .jslogContext=${'save-name'}
          ></devtools-button>
        </devtools-toolbar>
        <input
          class="authenticator-name-field"
          placeholder=${i18nString(UIStrings.enterNewName)}
          jslog=${VisualLogging.textField('name').track({ keydown: 'Enter', change: true })}
          value=${i18nString(UIStrings.authenticatorS, { PH1: authenticator.name })}
          .disabled=${!editing}
          ${ref((e) => {
      ...
      <div class="active-button-container">
        <label
          title=${i18nString(UIStrings.setSAsTheActiveAuthenticator, { PH1: authenticator.name })}
        >
          <input
            type="radio"
            .checked=${active}
...
function renderAuthenticatorFields(authenticatorId, options) {
  // clang-format off
  return html` <div class="authenticator-fields">
    ...
    <div class="authenticator-field">
      <label class="authenticator-option-label"> ${i18nString(UIStrings.supportsLargeBlob)} </label>
      <div class="authenticator-field-value">
...
export class WebauthnPaneImpl extends UI.Panel.Panel {
  async #addAuthenticator(options) {
    ...
    this.#model.addEventListener(
      'CredentialAdded' /* SDK.WebAuthnModel.Events.CREDENTIAL_ADDED */,
      this.#addCredential.bind(this, authenticatorId),
    );
    this.#model.addEventListener(
      'CredentialAsserted' /* SDK.WebAuthnModel.Events.CREDENTIAL_ASSERTED */,
      this.#updateCredential.bind(this, authenticatorId),
    );
    this.#model.addEventListener(
      'CredentialUpdated' /* SDK.WebAuthnModel.Events.CREDENTIAL_UPDATED */,
      this.#updateCredential.bind(this, authenticatorId),
    );
    this.#model.addEventListener(
      'CredentialDeleted' /* SDK.WebAuthnModel.Events.CREDENTIAL_DELETED */,
  ...
  constructor(view = DEFAULT_VIEW) {
    ...
    SDK.TargetManager.TargetManager.instance().observeModels(
      SDK.WebAuthnModel.WebAuthnModel,
      this,
      { scoped: true },
    );
  ...
  #updateCredential(authenticatorId, { data: event }) {
    ...
    const credential = authenticator.credentials.find(
      (credential) => credential.credentialId === event.credential.credentialId,
    );
    if (!credential) {
      return;
    }
  ...
  #deleteCredential(authenticatorId, { data: event }) {
    ...
    const credentialIndex = authenticator.credentials.findIndex(
    ...
    this.#isEnabling = new Promise(async (resolve) => {
      if (enable && !this.#hasBeenEnabled) {
        ...
        Host.userMetrics.actionTaken(
          Host.UserMetrics.Action.VirtualAuthenticatorEnvironmentEnabled,
        );
  ...
  #updateInternalTransportAvailability() {
    this.#hasInternalAuthenticator = Boolean(
    ...
    if (
      this.#hasInternalAuthenticator &&
      this.#newAuthenticatorOptions.transport ===
        'internal' /* Protocol.WebAuthn.AuthenticatorTransport.Internal */
    ) {
      this.#newAuthenticatorOptions.transport =
        'nfc' /* Protocol.WebAuthn.AuthenticatorTransport.Nfc */;
  ...
  #removeCredential(authenticatorId, credentialId) {
    ...
    const authenticatorIndex = authenticator.credentials.findIndex(
  ...
  removeAuthenticator(authenticatorId) {
    ...
    const newAvailableAuthenticators = prevAvailableAuthenticators.filter(
      (a) => a.authenticatorId !== authenticatorId,
    );
  ...
  async #setActiveAuthenticator(authenticatorId) {
    ...
    const prevAvailableAuthenticators = this.#availableAuthenticatorSetting.get();
    const newAvailableAuthenticators = prevAvailableAuthenticators.map((a) => ({
      ...a,
      active: a.authenticatorId === authenticatorId,
    }));
    this.#availableAuthenticatorSetting.set(newAvailableAuthenticators);
