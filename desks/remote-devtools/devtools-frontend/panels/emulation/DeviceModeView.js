...
export class DeviceModeView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.showMediaInspectorSetting = Common.Settings.Settings.instance().moduleSetting(
      'show-media-query-inspector',
    );
    this.showMediaInspectorSetting.addChangeListener(this.updateUI, this);
    this.showRulersSetting =
      Common.Settings.Settings.instance().moduleSetting('emulation.show-rulers');
    ...
    UI.ZoomManager.ZoomManager.instance().addEventListener(
      'ZoomChanged' /* UI.ZoomManager.Events.ZOOM_CHANGED */,
      this.zoomChanged,
      this,
    );
  }
  createUI() {
    ...
    this.outlineImage.addEventListener(
      'load',
      this.onImageLoaded.bind(this, this.outlineImage, true),
      false,
    );
    this.outlineImage.addEventListener(
      'error',
      this.onImageLoaded.bind(this, this.outlineImage, false),
      false,
    );
    ...
    this.screenImage.addEventListener(
      'load',
      this.onImageLoaded.bind(this, this.screenImage, true),
      false,
    );
    this.screenImage.addEventListener(
      'error',
      this.onImageLoaded.bind(this, this.screenImage, false),
      false,
    );
    ...
    UI.Tooltip.Tooltip.install(
      ...
      i18nString(UIStrings.doubleclickForFullHeight),
    );
  ...
  populatePresetsContainer() {
    ...
    this.presetBlocks = [];
    const inner = this.responsivePresetsContainer.createChild(
      'div',
  ...
  createResizer(element, widthFactor, heightFactor) {
    ...
    element.setAttribute(
      'jslog',
      `${VisualLogging.slider('device-mode-resizer').track({ drag: true })}`,
    );
    ...
    resizer.addEventListener(
      'ResizeStart' /* UI.ResizerWidget.Events.RESIZE_START */,
      this.onResizeStart,
      this,
    );
    resizer.addEventListener(
      'ResizeUpdateXY' /* UI.ResizerWidget.Events.RESIZE_UPDATE_XY */,
      this.onResizeUpdate.bind(this, widthFactor, heightFactor),
    );
    resizer.addEventListener(
      'ResizeEnd' /* UI.ResizerWidget.Events.RESIZE_END */,
      this.onResizeEnd,
      this,
    );
    return resizer;
  ...
  onResizeUpdate(widthFactor, heightFactor, event) {
    ...
    if (this.slowPositionStart) {
      cssOffsetX =
        (event.data.currentX - this.slowPositionStart.x) / 10 +
        this.slowPositionStart.x -
        event.data.startX;
      cssOffsetY =
        (event.data.currentY - this.slowPositionStart.y) / 10 +
    ...
    if (widthFactor && this.resizeStart) {
      ...
      if (
        newWidth >= EmulationModel.DeviceModeModel.MinDeviceSize &&
    ...
    if (heightFactor && this.resizeStart) {
      ...
      if (
        newHeight >= EmulationModel.DeviceModeModel.MinDeviceSize &&
  ...
  updateUI() {
    ...
    const showRulers =
      this.showRulersSetting.get() &&
      this.model.type() !== EmulationModel.DeviceModeModel.Type.None;
    ...
    if (
      !this.cachedCssVisiblePageRect ||
      !cssVisiblePageRect.isEqual(this.cachedCssVisiblePageRect)
    ) {
    ...
    this.contentClip.classList.toggle(
      'device-mode-outline-visible',
      Boolean(this.model.outlineImage()),
    );
    ...
    const mediaInspectorVisible =
      this.showMediaInspectorSetting.get() &&
      this.model.type() !== EmulationModel.DeviceModeModel.Type.None;
  ...
  setNonEmulatedAvailableSize(element) {
    ...
    const availableSize = new Geometry.Size(
