...
export class DeviceModeToolbar {
  ...
  constructor(model, showMediaInspectorSetting, showRulersSetting) {
    ...
    this.showDeviceScaleFactorSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.showUserAgentTypeSetting = Common.Settings.Settings.instance().createSetting(
      'emulation.show-user-agent-type',
      false,
    );
    this.showUserAgentTypeSetting.addChangeListener(this.updateUserAgentTypeVisibility, this);
    this.autoAdjustScaleSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.#element.setAttribute(
      'jslog',
      `${VisualLogging.toolbar('device-mode').track({ resize: true })}`,
    );
    ...
    this.emulatedDevicesList.addEventListener(
      'CustomDevicesUpdated' /* EmulationModel.EmulatedDevices.Events.CUSTOM_DEVICES_UPDATED */,
      this.deviceListChanged,
      this,
    );
    this.emulatedDevicesList.addEventListener(
      'StandardDevicesUpdated' /* EmulationModel.EmulatedDevices.Events.STANDARD_DEVICES_UPDATED */,
      this.deviceListChanged,
      this,
    );
  ...
  createMainToolbar() {
    ...
    const dimensionsSpan = uiI18n.getFormatLocalizedString(str_, UIStrings.dimensions, {
      PH1: this.deviceSelectItem.element,
    });
    ...
    mainToolbar.appendToolbarItem(new UI.Toolbar.ToolbarItem(this.createEmptyToolbarElement()));
    this.scaleItem = new UI.Toolbar.ToolbarComboBox(
      this.onScaleChange.bind(this),
      i18nString(UIStrings.zoom),
      'dark-text',
      'scale',
    );
    ...
    const deviceScaleSpan = uiI18n.getFormatLocalizedString(str_, UIStrings.dpr, {
      PH1: this.deviceScaleItem.element,
    });
    ...
    const saveDataItem =
      MobileThrottling.ThrottlingManager.throttlingManager().createSaveDataOverrideSelector();
    saveDataItem.turnShrinkable();
    mainToolbar.appendToolbarItem(saveDataItem);
    mainToolbar.appendToolbarItem(new UI.Toolbar.ToolbarItem(this.createEmptyToolbarElement()));
    this.modeButton = new UI.Toolbar.ToolbarButton(
      '',
      'screen-rotation',
      undefined,
      'screen-rotation',
    );
    this.modeButton.addEventListener(
      'Click' /* UI.Toolbar.ToolbarButton.Events.CLICK */,
      this.modeMenuClicked,
      this,
    );
    ...
    this.spanButton.addEventListener(
      'Click' /* UI.Toolbar.ToolbarButton.Events.CLICK */,
      this.spanClicked,
      this,
    );
  ...
  createOptionsToolbar() {
    const optionsToolbar = this.#element.createChild(
      'devtools-toolbar',
      'device-mode-toolbar-options',
    );
  ...
  getDevicePostureOptions() {
    ...
    return ['Continuous', 'Folded'].map((title) => ({
      title,
      value: title,
      selected: currentPosture === title,
    }));
  }
  ...
  getDeviceScaleFactorOptions() {
    ...
    const defaultValue =
      this.model.uaSetting().get() === 'Mobile' /* EmulationModel.DeviceModeModel.UA.MOBILE */ ||
      this.model.uaSetting().get() ===
  ...
  appendOptionsMenuItems(contextMenu) {
    ...
    contextMenu
      .footerSection()
      .appendItem(i18nString(UIStrings.resetToDefaults), this.reset.bind(this), {
  ...
  updateDeviceMenuItems() {
    ...
    this.deviceSelectItem.createOption(
      options.responsive.title,
      'Responsive',
      options.responsive.jslogContext,
    );
    ...
    function appendGroup(group, label) {
      ...
      for (const item of group) {
        ...
        option.setAttribute(
          'jslog',
          `${VisualLogging.item(item.jslogContext).track({ click: true })}`,
        );
  ...
  modeMenuClicked(event) {
    ...
    if (
      (device.isDualScreen || device.isFoldableScreen || device.modes.length === 2) &&
    ...
    function addMode(mode, title) {
      contextMenu
        .defaultSection()
        .appendCheckboxItem(title, applyMode.bind(null, mode), {
  ...
  update() {
    if (this.model.type() !== this.cachedModelType) {
      ...
      this.widthInput.disabled =
        this.model.type() !== EmulationModel.DeviceModeModel.Type.Responsive;
      this.heightInput.disabled =
        this.model.type() !== EmulationModel.DeviceModeModel.Type.Responsive;
      this.deviceScaleItem.setEnabled(
        this.model.type() === EmulationModel.DeviceModeModel.Type.Responsive,
      );
    ...
    if (this.model.device() !== this.cachedModelDevice) {
      ...
      if (device) {
        ...
        setTitleForButton(
          this.modeButton,
          modeCount === 2
            ? i18nString(UIStrings.rotate)
            : i18nString(UIStrings.screenOrientationOptions),
        );
      }
    ...
    for (const device of this.allDevices()) {
      if (device.title === this.persistenceSetting.get().device) {
        for (const mode of device.modes) {
          if (
            mode.orientation === this.persistenceSetting.get().orientation &&
