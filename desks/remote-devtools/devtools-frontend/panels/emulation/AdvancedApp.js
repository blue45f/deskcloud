...
export class AdvancedApp {
  ...
  constructor() {
    UI.DockController.DockController.instance().addEventListener(
      'BeforeDockSideChanged' /* UI.DockController.Events.BEFORE_DOCK_SIDE_CHANGED */,
  ...
  presentUI(document) {
    ...
    this.rootSplitWidget = new UI.SplitWidget.SplitWidget(
      false,
      true,
    ...
    this.deviceModeView = DeviceModeWrapper.instance({
    ...
    UI.DockController.DockController.instance().addEventListener(
      'DockSideChanged' /* UI.DockController.Events.DOCK_SIDE_CHANGED */,
      this.onDockSideChange,
      this,
    );
    UI.DockController.DockController.instance().addEventListener(
      'AfterDockSideChanged' /* UI.DockController.Events.AFTER_DOCK_SIDE_CHANGED */,
  ...
  onBeforeDockSideChange(event) {
    if (
      event.data.to === 'undocked' /* UI.DockController.DockState.UNDOCKED */ &&
  ...
  onDockSideChange(event) {
    ...
    if (toDockSide === 'undocked' /* UI.DockController.DockState.UNDOCKED */) {
      ...
      this.toolboxRootView &&
      event &&
      event.data.from === 'undocked' /* UI.DockController.DockState.UNDOCKED */
    ) {
  ...
  updateForDocked(dockSide) {
    ...
    resizerElement.style.transform =
      dockSide === 'right' /* UI.DockController.DockState.RIGHT */
        ? 'translateX(2px)'
        : dockSide === 'left' /* UI.DockController.DockState.LEFT */
          ? 'translateX(-2px)'
          : '';
    ...
    this.rootSplitWidget.setSecondIsSidebar(
      dockSide === 'right' /* UI.DockController.DockState.RIGHT */ ||
        dockSide === 'bottom' /* UI.DockController.DockState.BOTTOM */,
    );
