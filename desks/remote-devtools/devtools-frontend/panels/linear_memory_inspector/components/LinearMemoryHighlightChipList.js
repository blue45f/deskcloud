...
const DEFAULT_VIEW = (input, output, target) => {
  ...
  render(
    html` <style>
        ${linearMemoryHighlightChipListStyles}
      </style>
...
function renderChip(highlightInfo, input) {
  ...
  // Disabled until https://crbug.com/1079231 is fixed.
  // clang-format off
  return html` <div class=${classMap({ focused: isFocused, 'highlight-chip': true })}>
    <button
      class="jump-to-highlight-button"
      title=${i18nString(UIStrings.jumpToAddress)}
      jslog=${VisualLogging.action('linear-memory-inspector.jump-to-highlight').track({
        click: true,
      })}
    ...
    <div class="delete-highlight-container">
      <button
        class="delete-highlight-button"
        title=${i18nString(UIStrings.deleteHighlight)}
        jslog=${VisualLogging.action('linear-memory-inspector.delete-highlight').track({
          click: true,
        })}
        @click=${() => input.onDeleteHighlight(highlightInfo)}
      >
        <devtools-icon name="cross" class="medium"> </devtools-icon>
      </button>
    </div>
  </div>`;
  // clang-format off
