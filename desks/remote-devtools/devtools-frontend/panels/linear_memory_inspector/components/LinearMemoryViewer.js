...
export class LinearMemoryViewer extends HTMLElement {
  ...
  #renderRow(row) {
    const { startIndex, endIndex } = {
      startIndex: row * this.#numBytesInRow,
      endIndex: (row + 1) * this.#numBytesInRow,
    };
    const classMap = {
      address: true,
    ...
    return html`
      <div class="row">
        <span class=${Lit.Directives.classMap(classMap)}
          >${toHexString({ number: startIndex + this.#memoryOffset, pad: 8, prefix: false })}</span
        >
  ...
  #renderByteValues(startIndex, endIndex) {
    ...
    for (let i = startIndex; i < endIndex; ++i) {
      ...
      const byteValue = isSelectableCell
        ? html`${toHexString({ number: this.#memory[i], pad: 2, prefix: false })}`
        : '';
      const onSelectedByte = isSelectableCell ? this.#onSelectedByte.bind(this, actualIndex) : '';
      const jslog = VisualLogging.tableCell('linear-memory-inspector.byte-cell').track({
        click: true,
      });
      cells.push(
        html`<span
          class=${Lit.Directives.classMap(classMap)}
          @click=${onSelectedByte}
          jslog=${jslog}
          >${byteValue}</span
        >`,
      );
  ...
  #renderCharacterValues(startIndex, endIndex) {
    ...
    for (let i = startIndex; i < endIndex; ++i) {
      ...
      const onSelectedByte = isSelectableCell
        ? this.#onSelectedByte.bind(this, i + this.#memoryOffset)
        : '';
      const jslog = VisualLogging.tableCell('linear-memory-inspector.text-cell').track({
        click: true,
      });
      cells.push(
        html`<span
  ...
  #shouldBeHighlighted(index) {
    ...
    return (
      this.#highlightInfo.startAddress <= index &&
  ...
  #isFocusedArea(index) {
    ...
    return (
      this.#focusedMemoryHighlight.startAddress <= index &&
