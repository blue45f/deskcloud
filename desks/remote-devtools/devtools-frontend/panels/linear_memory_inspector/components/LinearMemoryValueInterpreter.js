  ...
  return html`
    <label data-endianness-setting="true" title=${i18nString(UIStrings.changeEndianness)}>
      <select
        ...
        @change=${(e) => onEndiannessChanged(e.target.value)}
      >
        ${['Little Endian' /* Endianness.LITTLE */, 'Big Endian' /* Endianness.BIG */].map(
          (endianness) => {
            return html`<option
              value=${endianness}
              .selected=${currentEndiannes === endianness}
              jslog=${VisualLogging.item(Platform.StringUtilities.toKebabCase(endianness)).track({
                click: true,
                resize: true,
              })}
            >
              ${i18n.i18n.lockedString(endianness)}
            </option>`;
          },
        )}
      </select>
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html`
      ...
      <style>
        ${linearMemoryValueInterpreterStyles}
      </style>
      <div class="value-interpreter">
        <div class="settings-toolbar">
          ...
          <devtools-button
            data-settings="true"
            class="toolbar-button ${input.showSettings ? '' : 'disabled'}"
            title=${i18nString(UIStrings.toggleValueTypeSettings)}
            @click=${input.onSettingsToggle}
