...
export function isValidMode(type, mode) {
  switch (type) {
    ...
    case 'Integer 64-bit' /* ValueType.INT64 */:
      return (
        mode === 'dec' /* ValueTypeMode.DECIMAL */ ||
    ...
    case 'Float 64-bit' /* ValueType.FLOAT64 */:
      return (
        mode === 'sci' /* ValueTypeMode.SCIENTIFIC */ || mode === 'dec' /* ValueTypeMode.DECIMAL */
      );
...
export function getPointerAddress(type, buffer, endianness) {
  ...
  try {
    ...
    return type === 'Pointer 32-bit' /* ValueType.POINTER32 */
      ? dataView.getUint32(0, isLittleEndian)
...
export function isPointer(type) {
  return (
    type === 'Pointer 32-bit' /* ValueType.POINTER32 */ ||
    type === 'Pointer 64-bit' /* ValueType.POINTER64 */
  );
}
export function format(formatData) {
  ...
  try {
    switch (formatData.type) {
      ...
      case 'Integer 16-bit' /* ValueType.INT16 */:
        value = formatData.signed
          ? valueView.getInt16(0, isLittleEndian)
          : valueView.getUint16(0, isLittleEndian);
        return formatInteger(value, formatData.mode);
      case 'Integer 32-bit' /* ValueType.INT32 */:
        value = formatData.signed
          ? valueView.getInt32(0, isLittleEndian)
          : valueView.getUint32(0, isLittleEndian);
        return formatInteger(value, formatData.mode);
      case 'Integer 64-bit' /* ValueType.INT64 */:
        value = formatData.signed
          ? valueView.getBigInt64(0, isLittleEndian)
