...
export const DEFAULT_VIEW = (input, _output, target) => {
  function parse(signed, type) {
    return format({
      ...
      type,
      signed,
      mode: input.valueTypeModes.get(type),
    });
  }
  ...
  render(
    html`
      ...
      <style>
        ${valueInterpreterDisplayStyles}
      </style>
      <div class="value-types">
        ${input.valueTypes.map((type) => {
          const address = isPointer(type)
            ? getPointerAddress(type, input.buffer, input.endianness)
            : 0;
          const jumpDisabled =
            Number.isNaN(address) || BigInt(address) >= BigInt(input.memoryLength);
          const signed = parseSigned(type);
          const unsigned = parseUnsigned(type);
          return isNumber(type)
            ? html` <span class="value-type-cell selectable-text"
                  >${i18n.i18n.lockedString(type)}</span
                >
                <div>
                  <select
                    title=${i18nString(UIStrings.changeValueTypeMode)}
                    data-mode-settings="true"
                    jslog=${VisualLogging.dropDown('linear-memory-inspector.value-type-mode').track(
                      { change: true },
                    )}
                  ...
                  >
                    ${VALUE_TYPE_MODE_LIST.filter((x) => isValidMode(type, x)).map((mode) => {
                      return html` <option
                        value=${mode}
                        .selected=${input.valueTypeModes.get(type) === mode}
            ...
            : isPointer(type)
              ? html` <span class="value-type-cell-no-mode value-type-cell selectable-text"
                    >${i18n.i18n.lockedString(type)}</span
                  >
                  <div class="value-type-cell">
                    <div class="value-type-value-with-link" data-value="true">
                      ...
                      <devtools-button
                        data-jump="true"
                        title=${jumpDisabled
                          ? i18nString(UIStrings.addressOutOfRange)
                          : i18nString(UIStrings.jumpToPointer)}
...
function renderSignedAndUnsigned(signedValue, unsignedValue, type, mode) {
  const showSignedAndUnsigned =
    ...
    mode !== 'hex' /* ValueTypeMode.HEXADECIMAL */ &&
    mode !== 'oct'; /* ValueTypeMode.OCTAL */
  const unsignedRendered = html`<span
    class="value-type-cell selectable-text"
    title=${i18nString(UIStrings.unsignedValue)}
    data-value="true"
    >${unsignedValue}</span
  >`;
  ...
  const showInMultipleLines =
    type === 'Integer 32-bit' /* ValueType.INT32 */ ||
    type === 'Integer 64-bit'; /* ValueType.INT64 */
  const signedRendered = html`<span
    class="selectable-text"
    data-value="true"
    title=${i18nString(UIStrings.signedValue)}
    >${signedValue}</span
  >`;
  if (showInMultipleLines) {
    return html` <div class="value-type-cell">${unsignedRendered} ${signedRendered}</div> `;
  }
  return html`
