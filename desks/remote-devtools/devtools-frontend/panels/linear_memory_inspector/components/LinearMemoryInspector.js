...
export const DEFAULT_VIEW = (input, _output, target) => {
  const navigatorAddressToShow =
    input.currentNavigatorMode === 'Submitted' /* Mode.SUBMITTED */
      ? formatAddress(input.address)
      : input.currentNavigatorAddressLine;
  const navigatorAddressIsValid = isValidAddress(navigatorAddressToShow, input.outerMemoryLength);
  const invalidAddressMsg = i18nString(UIStrings.addressHasToBeANumberBetweenSAnd, {
    PH1: formatAddress(0),
    PH2: formatAddress(input.outerMemoryLength),
  });
  ...
  const focusedMemoryHighlight = getSmallestEnclosingMemoryHighlight(
    highlightedMemoryAreas,
    input.address,
  );
    ...
    html`
      ...
      <div class="view">
        <devtools-linear-memory-inspector-navigator
          ...
          @historynavigation=${input.onNavigateHistory}
        ></devtools-linear-memory-inspector-navigator>
      ...
      ${input.hideValueInspector
        ? nothing
        : html` <div class="value-interpreter">
...
function getSmallestEnclosingMemoryHighlight(highlightedMemoryAreas, address) {
  ...
  for (const highlightedMemory of highlightedMemoryAreas) {
    if (
      highlightedMemory.startAddress <= address &&
...
export class LinearMemoryInspector extends Common.ObjectWrapper.eventMixin(UI.Widget.Widget) {
  ...
  performUpdate() {
    ...
    if (
      ...
      this.#address > this.#memoryOffset + this.#memory.length ||
      this.#address < 0
    ) {
    ...
    if (this.#highlightInfo) {
      ...
      if (
        this.#highlightInfo.startAddress < 0 ||
        this.#highlightInfo.startAddress >= this.#outerMemoryLength
      ) {
        this.#highlightInfo = undefined;
  ...
  #onDeleteMemoryHighlight(highlight) {
    this.dispatchEventToListeners(
      'DeleteMemoryHighlight' /* Events.DELETE_MEMORY_HIGHLIGHT */,
      highlight,
    );
  }
  #onRefreshRequest() {
  ...
  #onEndiannessChanged(endianness) {
    ...
    this.dispatchEventToListeners(
      'SettingsChanged' /* Events.SETTINGS_CHANGED */,
      this.#createSettings(),
    );
    void this.requestUpdate();
  }
  ...
  #navigateHistory(e) {
    return e.data === 'Forward' /* Navigation.FORWARD */
      ? this.#history.rollover()
      : this.#history.rollback();
  }
  #navigatePage(e) {
    const newAddress =
      e.data === 'Forward' /* Navigation.FORWARD */
