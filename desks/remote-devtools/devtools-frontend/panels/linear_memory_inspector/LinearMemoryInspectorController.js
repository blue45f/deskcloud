...
export class LinearMemoryInspectorController extends SDK.TargetManager.SDKModelObserver {
  ...
  constructor() {
    ...
    this.#paneInstance.addEventListener(
      'ViewClosed' /* LmiEvents.VIEW_CLOSED */,
      this.#viewClosed.bind(this),
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.DebuggerModel.DebuggerModel,
    ...
    this.#settings = Common.Settings.Settings.instance().createSetting(
  ...
  async evaluateExpression(callFrame, expressionName) {
    ...
    if ('error' in result) {
      console.error(
        `Tried to evaluate the expression '${expressionName}' but got an error: ${result.error}`,
      );
      return undefined;
    }
  ...
  async reveal({ object, expression }, omitFocus) {
    const response =
      await LinearMemoryInspectorController.retrieveDWARFMemoryObjectAndAddress(object);
  ...
  static extractHighlightInfo(obj, expression) {
    ...
    try {
      highlightInfo = {
        ...
        name: expression
          ? LinearMemoryInspectorController.extractObjectName(obj, expression)
          : expression,
  ...
  #onDebuggerPause(event) {
    ...
    for (const [bufferId, remoteObject] of this.#bufferIdToRemoteObject) {
      if (debuggerModel.runtimeModel() === remoteObject.runtimeModel()) {
        ...
        if (topCallFrame) {
          void this.updateHighlightedMemory(bufferId, topCallFrame)
  ...
  async updateHighlightedMemory(bufferId, callFrame) {
    ...
    const newHighlightInfo = LinearMemoryInspectorController.extractHighlightInfo(
      obj,
      expressionName,
    );
  ...
  #pointToSameMemoryObject(highlightInfoA, highlightInfoB) {
    return (
      highlightInfoA.type === highlightInfoB.type &&
      highlightInfoA.startAddress === highlightInfoB.startAddress
    );
  }
}
