...
const str_ = i18n.i18n.registerUIStrings(
  'panels/linear_memory_inspector/LinearMemoryInspectorPane.ts',
  UIStrings,
);
...
export class LinearMemoryInspectorPane extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  createPlaceholder() {
    ...
    placeholder.createChild('span', 'empty-state-header').textContent = i18nString(
      UIStrings.noOpenInspections,
    );
    ...
    const link = Link.create(
      MEMORY_INSPECTOR_EXPLANATION_URL,
...
export class LinearMemoryInspectorView extends UI.Widget.VBox {
  ...
  constructor(memoryWrapper, address = 0, tabId, hideValueInspector) {
    ...
    this.#inspector =
      new LinearMemoryInspectorComponents.LinearMemoryInspector.LinearMemoryInspector();
    this.#inspector.addEventListener(
      'MemoryRequest' /* LinearMemoryInspectorComponents.LinearMemoryInspector.Events.MEMORY_REQUEST */,
      this.#memoryRequested,
      this,
    );
    this.#inspector.addEventListener(
      'AddressChanged' /* LinearMemoryInspectorComponents.LinearMemoryInspector.Events.ADDRESS_CHANGED */,
      (event) => this.updateAddress(event.data),
    );
    this.#inspector.addEventListener(
      'SettingsChanged' /* LinearMemoryInspectorComponents.LinearMemoryInspector.Events.SETTINGS_CHANGED */,
      (event) => this.saveSettings(event.data),
    );
    this.#inspector.addEventListener(
      'DeleteMemoryHighlight' /* LinearMemoryInspectorComponents.LinearMemoryInspector.Events.DELETE_MEMORY_HIGHLIGHT */,
      (event) => {
  ...
  #memoryRequested(event) {
    ...
    void LinearMemoryInspectorController.getMemoryRange(this.#memoryWrapper, start, end).then(
      (memory) => {
        this.#memory = memory;
        this.#offset = start;
        this.render();
  ...
  #getHighlightInfo() {
    ...
    if (highlightInfo !== undefined) {
      if (
        highlightInfo.startAddress < 0 ||
        highlightInfo.startAddress >= this.#memoryWrapper.length()
      ) {
