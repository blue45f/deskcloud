...
const str_ = i18n.i18n.registerUIStrings(
  'panels/console_counters/WarningErrorCounter.ts',
  UIStrings,
);
...
const DEFAULT_VIEW = (input, _output, target) => {
  ...
  const issueCounterData = {
    ...
    displayMode:
      'OnlyMostImportant' /* IssueCounter.IssueCounter.DisplayMode.ONLY_MOST_IMPORTANT */,
  };
  render(
...
export class WarningErrorCounterWidget extends UI.Widget.Widget {
  ...
  get titlesForTesting() {
    const button = this.contentElement
...
export class WarningErrorCounter {
  ...
  constructor() {
    ...
    const widget = new WarningErrorCounterWidget(
      widgetElement,
      toolbarItem.setVisible.bind(toolbarItem),
    );
    toolbarItem.addEventListener(
      'CompactLayoutUpdated' /* UI.Toolbar.ToolbarItemWithCompactLayoutEvents.COMPACT_LAYOUT_UPDATED */,
