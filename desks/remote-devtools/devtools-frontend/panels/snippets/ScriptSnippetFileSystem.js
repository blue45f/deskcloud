...
export class SnippetFileSystem extends Persistence.PlatformFileSystem.PlatformFileSystem {
  ...
  constructor() {
    ...
    this.lastSnippetIdentifierSetting = Common.Settings.Settings.instance().createSetting(
  ...
  tooltipForURL(url) {
    return i18nString(UIStrings.linkedTo, {
      PH1: unescapeSnippetName(
        Common.ParsedURL.ParsedURL.sliceUrlToEncodedPathString(url, this.path().length),
      ),
    });
  }
  supportsAutomapping() {
...
export async function evaluateScriptSnippet(uiSourceCode) {
  ...
  if ('exceptionDetails' in result && result.exceptionDetails) {
    consoleModel?.addMessage(
      SDK.ConsoleModel.ConsoleMessage.fromException(
        runtimeModel,
