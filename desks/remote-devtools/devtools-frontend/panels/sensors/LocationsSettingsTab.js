...
export class LocationsSettingsTab extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const list = this.customSetting
      .get()
      .map((location) => replaceLocationTitles(location, this.customSetting.defaultValue));
      ...
      if (!location.title) {
        const replacement = defaultValues.find(
          (defaultLocation) =>
            ...
            defaultLocation.long === location.long &&
            defaultLocation.timezoneId === location.timezoneId &&
            defaultLocation.locale === location.locale,
        );
        if (!replacement) {
  ...
  renderItem(location, _editable) {
    ...
    locale.role = 'cell';
    element.createChild('div', 'locations-list-separator');
    element.createChild('div', 'locations-list-text').textContent = String(
      location.accuracy || SDK.EmulationModel.Location.DEFAULT_ACCURACY,
    );
    return element;
  }
  ...
  beginEdit(location) {
    ...
    editor.control('accuracy').value = String(
      location.accuracy || SDK.EmulationModel.Location.DEFAULT_ACCURACY,
    );
    return editor;
  }
  createEditor() {
    ...
    titles.createChild('div', 'locations-list-text locations-list-title').textContent = i18nString(
      UIStrings.locationName,
    );
    ...
    fields
      ...
      .appendChild(
        editor.createInput('title', 'text', i18nString(UIStrings.locationName), titleValidator),
      );
    ...
    cell.appendChild(
      editor.createInput('lat', 'text', i18nString(UIStrings.latitude), latValidator),
    );
    fields.createChild('div', 'locations-list-separator locations-list-separator-invisible');
    cell = fields.createChild(
      'div',
      'locations-list-text locations-list-text-longitude locations-input-container',
    );
    cell.appendChild(
      editor.createInput('long', 'text', i18nString(UIStrings.longitude), longValidator),
    );
    fields.createChild('div', 'locations-list-separator locations-list-separator-invisible');
    ...
    cell.appendChild(
      editor.createInput(
        'timezone-id',
        'text',
        i18nString(UIStrings.timezoneId),
        timezoneIdValidator,
      ),
    );
    fields.createChild('div', 'locations-list-separator locations-list-separator-invisible');
    cell = fields.createChild('div', 'locations-list-text locations-input-container');
    cell.appendChild(
      editor.createInput('locale', 'text', i18nString(UIStrings.locale), localeValidator),
    );
    fields.createChild('div', 'locations-list-separator locations-list-separator-invisible');
    cell = fields.createChild('div', 'locations-list-text locations-input-container');
    cell.appendChild(
      editor.createInput('accuracy', 'text', i18nString(UIStrings.accuracy), accuracyValidator),
    );
    return editor;
