...
const UIStrings = {
  ...
  adjustWithMousewheelOrUpdownKeys:
    'Adjust with mousewheel or up/down keys. {PH1}: ±10, Shift: ±1, Alt: ±0.01',
  /**
...
export class SensorsView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.#locationSetting = Common.Settings.Settings.instance().createSetting(
    ...
    customLocationsSetting.addChangeListener(() =>
      this.renderLocationSection(this.#location, customLocationsSetting),
    );
    this.createPanelSeparator();
    this.deviceOrientationSetting = Common.Settings.Settings.instance().createSetting(
      'emulation.device-orientation-override',
      '',
    );
    this.deviceOrientation = SDK.EmulationModel.DeviceOrientation.parseSetting(
  ...
  renderLocationSection(location, customLocationsSetting) {
    ...
    if (this.#locationOverrideEnabled) {
      if (location.unavailable) {
        ...
        for (const [i, customLocation] of customLocations.entries()) {
          if (
            location.latitude === customLocation.lat &&
            location.longitude === customLocation.long &&
            location.timezoneId === customLocation.timezoneId &&
            location.locale === customLocation.locale
          ) {
            selectedIndex = i + 1;
            break;
          }
    ...
    const modifierKeyMessage = i18nString(UIStrings.adjustWithMousewheelOrUpdownKeys, {
      PH1: cmdOrCtrl,
    });
    ...
    render(
      html`
        <label class="sensors-group-title" id="location-select-label" for="location-select"
          >${i18nString(UIStrings.location)}</label
        >
        <div class="geo-fields">
          ...
          >
            <option value=${NonPresetOptions.NoOverride} jslog=${VisualLogging.item('no-override')}>
              ${i18nString(UIStrings.noOverride)}
            </option>
            <optgroup label=${i18nString(UIStrings.overrides)}>
              ${customLocations.map(
                (customLocation) => html`
                  <option
                    value=${JSON.stringify(customLocation)}
                    jslog=${VisualLogging.item('custom')}
                  >
            ...
            <option value=${NonPresetOptions.Custom} jslog=${VisualLogging.item('other')}>
              ${i18nString(UIStrings.other)}
            </option>
            <optgroup label=${i18nString(UIStrings.error)}>
              <option
                value=${NonPresetOptions.Unavailable}
                jslog=${VisualLogging.item('unavailable')}
              >
          ...
          >
            <div class="latlong-group">
              ...
              />
              <label class="latlong-title" for="latitude-input"
                >${i18nString(UIStrings.latitude)}</label
              >
            </div>
            <div class="latlong-group">
              <!-- @ts-ignore -->
              <input
                ...
                @focus=${this.#onLocationFocus.bind(this)}
              />
              <label class="latlong-title" for="longitude-input"
                >${i18nString(UIStrings.longitude)}</label
              >
            </div>
            <div class="latlong-group">
              <input
                ...
                @focus=${this.#onLocationFocus.bind(this)}
              />
              <label class="timezone-title" for="timezone-input"
                >${i18nString(UIStrings.timezoneId)}</label
              >
              <div
                class="timezone-error"
                ${Directives.ref((el) => {
                  if (el) {
            ...
            <div class="latlong-group">
              ...
              <input
                ...
                @focus=${this.#onLocationFocus.bind(this)}
              />
              <label class="accuracy-title" for="accuracy-input"
                >${i18nString(UIStrings.accuracy)}</label
              >
              <div
  ...
  #locationSelectChanged() {
    ...
    const value =
      this.locationSelectElement.options[this.locationSelectElement.selectedIndex].value;
    if (value === NonPresetOptions.NoOverride) {
      ...
      this.#location = new SDK.EmulationModel.Location(
        ...
        '',
        '',
        SDK.EmulationModel.Location.DEFAULT_ACCURACY,
        true,
      );
    } else {
      this.#locationOverrideEnabled = true;
  ...
  #onLocationKeyDown(event) {
    ...
    const isNumeric =
      input === this.latitudeInput || input === this.longitudeInput || input === this.accuracyInput;
    if (!isNumeric) {
      return;
    }
  ...
  applyLocation() {
    ...
    for (const emulationModel of SDK.TargetManager.TargetManager.instance().models(
      ...
      emulationModel
        .emulateLocation(this.#locationOverrideEnabled ? this.#location : null)
        .catch((err) => {
          switch (err.type) {
  ...
  createDeviceOrientationSection() {
    ...
    render(
      html`
        <label class="sensors-group-title" for="orientation-select"
          >${i18nString(UIStrings.orientation)}</label
        >
        <div class="orientation-content">
          <div class="orientation-fields">
            ...
            >
              <option
                value=${orientationOffOption.orientation}
                jslog=${VisualLogging.item(orientationOffOption.jslogContext)}
              >
                ${orientationOffOption.title}
              </option>
              <option
                value=${customOrientationOption.orientation}
                jslog=${VisualLogging.item('custom')}
              >
                ${customOrientationOption.title}
              </option>
              ${orientationGroups.map(
                (group) => html`
                    ...
                    ${group.value.map(
                      (preset) => html`
                        <option
                          ...
                          jslog=${VisualLogging.item(preset.jslogContext)}
                        >
                          ${preset.title}
                        </option>
                      `,
            ...
            >
              <div class="orientation-inputs-cell">
                ...
                <devtools-button
                  ...
                  jslog=${VisualLogging.action('sensors.reset-device-orientiation').track({
                    click: true,
                  })}
                >
          ...
          >
            ...
            >
              <section class="orientation-box orientation-element">
  ...
  enableOrientationFields(disable) {
    if (disable) {
      ...
      UI.Tooltip.Tooltip.install(
        this.stageElement,
        i18nString(UIStrings.enableOrientationToRotate),
      );
    } else {
      ...
      UI.Tooltip.Tooltip.install(
        this.stageElement,
        i18nString(UIStrings.shiftdragHorizontallyToRotate),
      );
    }
  }
  orientationSelectChanged() {
    ...
    if (value === NonPresetOptions.NoOverride) {
      ...
      this.deviceOrientationOverrideEnabled = true;
      this.deviceOrientation = new SDK.EmulationModel.DeviceOrientation(
        parsedValue[0],
        parsedValue[1],
        parsedValue[2],
      );
      this.setDeviceOrientation(
        ...
        'selectPreset' /* DeviceOrientationModificationSource.SELECT_PRESET */,
      );
    }
  }
  ...
  applyDeviceOrientationUserInput() {
    this.setDeviceOrientation(
      ...
      'userInput' /* DeviceOrientationModificationSource.USER_INPUT */,
    );
  ...
  resetDeviceOrientation() {
    this.setDeviceOrientation(
      ...
      'resetButton' /* DeviceOrientationModificationSource.RESET_BUTTON */,
    );
  ...
  setDeviceOrientation(deviceOrientation, modificationSource) {
    ...
    const animate =
      modificationSource !== 'userDrag'; /* DeviceOrientationModificationSource.USER_DRAG */
    this.setBoxOrientation(deviceOrientation, animate);
    this.deviceOrientation = deviceOrientation;
    this.applyDeviceOrientation();
    UI.ARIAUtils.LiveAnnouncer.alert(
      i18nString(UIStrings.deviceOrientationSetToAlphaSBeta, {
  ...
  onBoxDrag(event) {
    ...
    const newOrientation = new SDK.EmulationModel.DeviceOrientation(
      eulerAngles.alpha,
      eulerAngles.beta,
      eulerAngles.gamma,
    );
    this.setDeviceOrientation(
      newOrientation,
      'userDrag' /* DeviceOrientationModificationSource.USER_DRAG */,
    );
    this.setSelectElementLabel(this.orientationSelectElement, NonPresetOptions.Custom);
    return false;
  }
