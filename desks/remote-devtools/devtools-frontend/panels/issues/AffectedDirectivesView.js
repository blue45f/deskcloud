...
export class AffectedDirectivesView extends AffectedResourcesView {
  ...
  #appendAffectedContentSecurityPolicyDetails(cspIssues) {
    ...
    if (this.issue.code() === IssuesManager.ContentSecurityPolicyIssue.inlineViolationCode) {
      ...
      this.appendColumnTitle(
        header,
        i18nString(UIStrings.resourceC),
      ...
      this.issue.code() === IssuesManager.ContentSecurityPolicyIssue.trustedTypesSinkViolationCode
    ) {
      this.appendColumnTitle(header, i18nString(UIStrings.sourceLocation));
      this.appendColumnTitle(header, i18nString(UIStrings.status));
    } else if (
      this.issue.code() === IssuesManager.ContentSecurityPolicyIssue.trustedTypesPolicyViolationCode
    ) {
      this.appendColumnTitle(header, i18nString(UIStrings.sourceLocation));
      this.appendColumnTitle(header, i18nString(UIStrings.directiveC));
      this.appendColumnTitle(header, i18nString(UIStrings.status));
  ...
  #appendAffectedContentSecurityPolicyDetail(cspIssue) {
    ...
    if (
      this.issue.code() === IssuesManager.ContentSecurityPolicyIssue.inlineViolationCode &&
      model
    ) {
      ...
      const url = cspIssueDetails.blockedURL
        ? cspIssueDetails.blockedURL
