...
export class AffectedResourcesView extends UI.TreeOutline.TreeElement {
  ...
  async createElementCell({ backendNodeId, nodeName, target }, issueCategory) {
    ...
    const anchor = PanelsCommon.DOMLinkifier.Linkifier.instance().linkify(deferredDOMNode, {
      textContent: nodeName || undefined,
  ...
  appendSourceLocation(element, sourceLocation, target) {
    ...
    if (sourceLocation) {
      ...
      sourceAnchor.setAttribute(
        'jslog',
        `${VisualLogging.link('source-location').track({ click: true })}`,
      );
