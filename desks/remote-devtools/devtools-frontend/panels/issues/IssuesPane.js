...
import {
  getGroupIssuesByKindSetting,
  IssueKindView,
  issueKindViewSortPriority,
} from './IssueKindView.js';
...
export class IssuesPane extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.#noIssuesMessageDiv = new UI.EmptyWidget.EmptyWidget(
      '',
      i18nString(UIStrings.issuesPanelDescription),
    );
    ...
    this.#aggregator.addEventListener(
      'AggregatedIssueUpdated' /* IssuesManager.IssueAggregator.Events.AGGREGATED_ISSUE_UPDATED */,
      this.#issueUpdated,
      this,
    );
    this.#aggregator.addEventListener(
      'FullUpdateRequired' /* IssuesManager.IssueAggregator.Events.FULL_UPDATE_REQUIRED */,
      this.#onFullUpdate,
      this,
    );
  ...
  #scheduleIssueViewUpdate(issue) {
    this.#issueViewUpdatePromise = this.#issueViewUpdatePromise.then(() =>
  ...
  async #updateIssueView(issue) {
    ...
    if (!issueView) {
      ...
      const markdownDescription =
        await IssuesManager.MarkdownIssueDescription.createIssueDescriptionFromMarkdown(
          description,
        );
  ...
  #fullUpdate(force) {
    this.#clearViews(
      this.#categoryViews,
      force ? undefined : this.#aggregator.aggregatedIssueCategories(),
    );
  ...
  #showIssuesTreeOrNoIssuesDetectedMessage(issuesCount, hiddenIssueCount) {
    if (issuesCount > 0 || hiddenIssueCount > 0) {
      ...
      const hasOnlyThirdPartyIssues =
        this.#issuesManager.numberOfAllStoredIssues() -
          this.#issuesManager.numberOfThirdPartyCookiePhaseoutIssues() >
        0;
      this.#noIssuesMessageDiv.header = hasOnlyThirdPartyIssues
        ? i18nString(UIStrings.onlyThirdpartyCookieIssues)
        : i18nString(UIStrings.noIssues);
