...
class AffectedMixedContentView extends AffectedResourcesView {
  ...
  appendAffectedMixedContent(mixedContent) {
    ...
    if (mixedContent.request) {
      const networkTab =
        issueTypeToNetworkHeaderMap.get(this.issue.getCategory()) ||
        'headers-component'; /* NetworkForward.UIRequestLocation.UIRequestTabs.HEADERS_COMPONENT */
      element.appendChild(
        this.createRequestCell(mixedContent.request, {
          networkTab,
          additionalOnClickAction() {
            Host.userMetrics.issuesPanelResourceOpened(
              'MixedContent' /* IssuesManager.Issue.IssueCategory.MIXED_CONTENT */,
      ...
      const cell = this.appendIssueDetailCell(
        element,
        filename,
...
export class IssueView extends UI.TreeOutline.TreeElement {
  ...
  constructor(issue, description) {
    ...
    this.#affectedResourceViews = [
      ...
      new AffectedDescendantsWithinSelectElementView(
        this,
        this.#issue,
      ...
      new AffectedSelectivePermissionsInterventionView(
        this,
        this.#issue,
  ...
  onexpand() {
    ...
    if (category === 'Cookie' /* IssuesManager.Issue.IssueCategory.COOKIE */) {
      const cookieIssueSubCategory = IssuesManager.CookieIssue.CookieIssue.getSubCategory(
        this.#issue.code(),
      );
  ...
  #updateFromIssue() {
    ...
    if (this.#hiddenIssuesMenu) {
      const data = {
        menuItemLabel: this.#issue.isHidden()
          ? i18nString(UIStrings.unhideIssuesLikeThis)
          : i18nString(UIStrings.hideIssuesLikeThis),
        menuItemAction: () => {
          const setting = IssuesManager.IssuesManager.getHideIssueByCodeSetting();
          const values = setting.get();
          values[this.#issue.code()] = this.#issue.isHidden()
            ? 'Unhidden' /* IssuesManager.IssuesManager.IssueStatus.UNHIDDEN */
  ...
  #createBody() {
    ...
    UI.ARIAUtils.setSetSize(
      messageElement.listItemElement,
      this.#description.links.length === 0 ? 2 : 3,
    );
