...
export class AffectedSelectivePermissionsInterventionView extends AffectedResourcesView {
  ...
  #renderDetail(issue) {
    ...
    const stackTracePromise =
      details.stackTrace && issuesModel
    ...
    return html`
      <tr class="affected-resource-directive">
        ...
        <td>
          ${Directives.until(
            stackTracePromise,
            html`<span>${i18nString(UIStrings.loading)}</span>`,
          )}
        </td>
        <td class="affected-resource-cell">
          <div class="ad-ancestry-list">
            ${(details.adAncestry?.ancestryChain || []).map((script) => {
              const link = this.#linkifier.linkifyScriptLocation(target, script.scriptId, '', 0);
  ...
  async #resolveStackTrace(stackTrace, issuesModel) {
    ...
    const stackTraceTranslated = await debuggerWorkspaceBinding.createStackTraceFromProtocolRuntime(
      stackTrace,
      issuesModel.target(),
    );
