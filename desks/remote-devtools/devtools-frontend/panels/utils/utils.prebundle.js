...
export class PanelUtils {
    ...
    function iconTitleForRequest(request) {
      ...
      const title =
        typeof throttlingConditions?.conditions.title === 'string'
          ...
          : throttlingConditions?.conditions.title();
      return i18nString(UIStrings.resourceTypeWithThrottling, {
        PH1: request.resourceType().title(),
        PH2: title,
      });
    }
  }
  static iconDataForResourceType(resourceType) {
    ...
    if (
      resourceType.name() === Common.ResourceType.resourceTypes.WebSocket.name() ||
    ...
    if (
      resourceType.name() === Common.ResourceType.resourceTypes.Fetch.name() ||
  ...
  static getIconForSourceFile(uiSourceCode) {
    ...
    const title = binding
      ? PanelCommon.PersistenceUtils.PersistenceUtils.tooltipForUISourceCode(uiSourceCode)
      : undefined;
    // clang-format off
  ...
  static async formatCSSChangesFromDiff(diff) {
    ...
    for (const { currentLineNumber, originalLineNumber, type } of rows) {
      if (
        type !== 'deletion' /* DiffView.DiffView.RowType.DELETION */ &&
        type !== 'addition' /* DiffView.DiffView.RowType.ADDITION */
      ) {
        continue;
      }
      ...
      const { declarationIDToStyleRule, styleRuleIDToStyleRule } = isDeletion
        ? originalRuleMaps
        : currentRuleMaps;
      let styleRule;
      let prefix = '';
...
async function buildStyleRuleMaps(content) {
  const rules = await new Promise((res) => {
    const rules = [];
    Formatter.FormatterWorkerPool.formatterWorkerPool().parseCSS(
      content,
      (isLastChunk, currentRules) => {
