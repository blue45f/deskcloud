...
var O = (i, e) => {
  for (var t in e) kt(i, t, { get: e[t], enumerable: !0 });
};
var Me = {};
O(Me, { CSSOverviewUnusedDeclarations: () => E });
...
var U = {
    ...
    verticalAlignmentAppliedTo:
      'Vertical alignment applied to element which is neither `inline` nor `table-cell`',
  },
  It = ge.i18n.registerUIStrings('panels/css_overview/CSSOverviewUnusedDeclarations.ts', U),
  R = ge.i18n.getLocalizedString.bind(void 0, It),
  E = class {
    static add(e, t, o) {
      let r = e.get(t) || [];
      (r.push(o), e.set(t, r));
    }
    static checkForUnusedPositionValues(e, t, o, r, s, n, a, c) {
      if (o[r] === 'static') {
        if (o[s] !== 'auto') {
          let p = R(U.topAppliedToAStatically);
          this.add(e, p, { declaration: `top: ${o[s]}`, nodeId: t });
        }
        if (o[n] !== 'auto') {
          let p = R(U.leftAppliedToAStatically);
          this.add(e, p, { declaration: `left: ${o[n]}`, nodeId: t });
        }
        if (o[a] !== 'auto') {
          let p = R(U.rightAppliedToAStatically);
          this.add(e, p, { declaration: `right: ${o[a]}`, nodeId: t });
        }
        if (o[c] !== 'auto') {
          let p = R(U.bottomAppliedToAStatically);
          this.add(e, p, { declaration: `bottom: ${o[c]}`, nodeId: t });
        }
      }
    }
    static checkForUnusedWidthAndHeightValues(e, t, o, r, s, n) {
      if (o[r] === 'inline') {
        if (o[s] !== 'auto') {
          let a = R(U.widthAppliedToAnInlineElement);
          this.add(e, a, { declaration: `width: ${o[s]}`, nodeId: t });
        }
        if (o[n] !== 'auto') {
          let a = R(U.heightAppliedToAnInlineElement);
          this.add(e, a, { declaration: `height: ${o[n]}`, nodeId: t });
        }
      }
    }
    static checkForInvalidVerticalAlignment(e, t, o, r, s) {
      if (
        !(!o[r] || o[r].startsWith('inline') || o[r].startsWith('table')) &&
        o[s] !== 'baseline'
      ) {
        let n = R(U.verticalAlignmentAppliedTo);
        this.add(e, n, { declaration: `vertical-align: ${o[s]}`, nodeId: t });
      }
    }
  };
var Re = {};
O(Re, { CSSOverviewModel: () => W });
import * as se from './../../core/common/common.js';
import * as ie from './../../core/root/root.js';
import * as we from './../../core/sdk/sdk.js';
import * as Le from './../../ui/legacy/components/color_picker/color_picker.js';
var W = class extends we.SDKModel.SDKModel {
  #o;
  #t;
  #s;
  constructor(e) {
    (super(e),
      (this.#o = e.runtimeAgent()),
      (this.#t = e.cssAgent()),
      (this.#s = e.domsnapshotAgent()));
  }
  async getNodeStyleStats() {
    let e = new Map(),
      ...
      c = {
        ...
        includeTextColorOpacities: !0,
        includeBlendedBackgroundColors: !0,
      },
      p = (m) =>
        m instanceof se.Color.Legacy
          ? m.hasAlpha()
            ? m.asString('hexa')
            : m.asString('hex')
          : m.asString(),
      f = (m, g, S) => {
        ...
        let Y = h[m];
        if (!Y) return;
        let M = se.Color.parse(Y);
        if (!M || M.asLegacyColor().rgba()[3] === 0) return;
        let v = p(M);
        if (!v) return;
        let L = S.get(v) || new Set();
        return (L.add(g), S.set(v, L), M);
      },
      y = (m) =>
        new Set([
          'altglyph',
      ...
      B = (m) => new Set(['iframe', 'video', 'embed', 'img']).has(m.toLowerCase()),
      he = (m, g) =>
        new Set(['tr', 'td', 'thead', 'tbody']).has(m.toLowerCase()) && g.startsWith('table'),
      X = 0,
      { documents: nt, strings: h } = await this.#s.invoke_captureSnapshot(c);
    for (let { nodes: m, layout: g } of nt) {
      X += g.nodeIndex.length;
      for (let S = 0; S < g.styles.length; S++) {
        let Y = g.styles[S],
          M = g.nodeIndex[S];
        if (!m.backendNodeId || !m.nodeName) continue;
        let v = m.backendNodeId[M],
          L = m.nodeName[M],
          [
            at,
            lt,
        ...
        if (
          ...
          (b.set(te, x), b.set(oe, A), b.set(I, C), n.set(k, b));
        }
        let K =
          fe && g.blendedBackgroundColors && g.blendedBackgroundColors[S] !== -1
            ? se.Color.parse(h[g.blendedBackgroundColors[S]])
            : null;
        if (fe && K) {
          let k = new Le.ContrastInfo.ContrastInfo({
              backgroundColors: [K.asString('hexa')],
              computedFontSize: Z !== -1 ? h[Z] : '',
              computedFontWeight: ee !== -1 ? h[ee] : '',
            }),
            b = fe
              .asLegacyColor()
              .blendWithAlpha(g.textColorOpacities ? g.textColorOpacities[S] : 1);
          k.setColor(b);
          let te = p(b),
            oe = p(K.asLegacyColor()),
            I = `${te}_${oe}`;
          if (ie.Runtime.experiments.isEnabled(ie.ExperimentNames.ExperimentName.APCA)) {
            let x = k.contrastRatioAPCA(),
              A = k.contrastRatioAPCAThreshold();
            if (!(x && A ? Math.abs(x) >= A : !1) && x) {
              let w = {
                nodeId: v,
                contrastRatio: x,
                textColor: b,
                backgroundColor: K,
                thresholdsViolated: { aa: !1, aaa: !1, apca: !0 },
              };
              o.has(I) ? o.get(I).push(w) : o.set(I, [w]);
            }
          } else {
            let x = k.contrastRatioThreshold('aa') || 0,
              A = k.contrastRatioThreshold('aaa') || 0,
              C = k.contrastRatio() || 0;
            if (x > C || A > C) {
              let w = {
                nodeId: v,
                ...
                textColor: b,
                backgroundColor: K,
                thresholdsViolated: { aa: x > C, aaa: A > C, apca: !1 },
              };
              o.has(I) ? o.get(I).push(w) : o.set(I, [w]);
            }
          }
        }
        (E.checkForUnusedPositionValues(a, v, h, wt, bt, yt, St, xt),
          !y(h[L]) && !B(h[L]) && E.checkForUnusedWidthAndHeightValues(a, v, h, ve, Ct, $t),
          Ue !== -1 && !he(h[L], h[ve]) && E.checkForInvalidVerticalAlignment(a, v, h, ve, Ue));
      }
    }
    return {
      backgroundColors: e,
      textColors: t,
      textColorContrastIssues: o,
      fillColors: r,
      borderColors: s,
      fontInfo: n,
      unusedDeclarations: a,
      elementCount: X,
    };
  }
  getComputedStyleForNode(e) {
    return this.#t.invoke_getComputedStyleForNode({ nodeId: e });
  }
  async getMediaQueries() {
    let e = await this.#t.invoke_getMediaQueries(),
      t = new Map();
    if (!e) return t;
    for (let o of e.medias) {
      if (o.source === 'linkedSheet') continue;
      let r = t.get(o.text) || [];
      (r.push(o), t.set(o.text, r));
    }
    return t;
  }
  async getGlobalStylesheetStats() {
    let e = `(function() {
      ...
      { result: t } = await this.#o.invoke_evaluate({ expression: e, returnByValue: !0 });
    if (t.type === 'object') return t.value;
  }
};
we.SDKModel.SDKModel.register(W, { capabilities: 2, autostart: !1 });
var Ne = {};
O(Ne, { CSSOverviewProcessingView: () => Q, DEFAULT_VIEW: () => Pe });
import * as be from './../../core/i18n/i18n.js';
import './../../ui/components/buttons/buttons.js';
import * as De from './../../ui/legacy/legacy.js';
import { html as At, render as Tt } from './../../ui/lit/lit.js';
var Ee = `.overview-processing-view{overflow:hidden;padding:16px;justify-content:center;align-items:center;height:100%}.overview-processing-view h1{font-size:16px;text-align:center;font-weight:normal;margin:0;padding:8px}.overview-processing-view h2{font-size:12px;text-align:center;font-weight:normal;margin:0;padding-top:32px}
/*# sourceURL=${import.meta.resolve('./cssOverviewProcessingView.css')} */`;
var Ve = { cancel: 'Cancel' },
  Ut = be.i18n.registerUIStrings('panels/css_overview/CSSOverviewProcessingView.ts', Ve),
  Mt = be.i18n.getLocalizedString.bind(void 0, Ut),
  Pe = (i, e, t) => {
    Tt(
      At`
    <style>${Ee}</style>
  ...
  Q = class extends De.Widget.Widget {
    #o = () => {};
    #t;
    constructor(e, t = Pe) {
    ...
    performUpdate() {
      this.#t({ onCancel: this.#o }, {}, this.element);
    }
  };
var et = {};
O(et, {
  CSSOverviewCompletedView: () => P,
  DEFAULT_VIEW: () => Ye,
  ELEMENT_DETAILS_DEFAULT_VIEW: () => Ze,
  ElementDetailsView: () => ae,
});
...
import * as q from './../../core/root/root.js';
import * as J from './../../core/sdk/sdk.js';
import * as Qe from './../../models/geometry/geometry.js';
...
import { Directives as Vt, html as u, nothing as $, render as Je } from './../../ui/lit/lit.js';
import * as le from './../../ui/visual_logging/visual_logging.js';
import * as Xe from './../common/common.js';
var re = `@scope to (devtools-widget > *){.overview-completed-view{overflow:auto;--overview-default-padding:28px;--overview-icon-padding:32px}.overview-completed-view .summary ul,
...
var He = {};
O(He, { CSSOverviewSidebarPanel: () => j, DEFAULT_VIEW: () => qe });
import './../../ui/legacy/legacy.js';
import * as xe from './../../core/i18n/i18n.js';
import './../../ui/components/buttons/buttons.js';
import * as _e from './../../ui/legacy/legacy.js';
import { Directives as Lt, html as Oe, render as Rt } from './../../ui/lit/lit.js';
import * as Fe from './../../ui/visual_logging/visual_logging.js';
var ze = `@scope to (devtools-widget > *){.overview-sidebar-panel{display:flex;background:var(--sys-color-cdt-base-container);min-width:fit-content;flex-direction:column}.overview-sidebar-panel-item{height:30px;padding-left:30px;display:flex;align-items:center;color:var(--sys-color-on-surface);white-space:nowrap;&:hover{background:var(--sys-color-state-hover-on-subtle)}&:focus{background:var(--sys-color-state-focus-highlight)}&.selected{background:var(--sys-color-tonal-container);color:var(--sys-color-on-tonal-container)}}.overview-toolbar{border-bottom:1px solid var(--sys-color-divider);flex:0 0 auto}.overview-sidebar-panel-item:focus-visible{outline-width:unset}@media (forced-colors: active){.overview-sidebar-panel-item.selected{forced-color-adjust:none;background:Highlight;color:HighlightText}.overview-sidebar-panel-item:hover{forced-color-adjust:none;background:Highlight;color:HighlightText}}}
/*# sourceURL=${import.meta.resolve('./cssOverviewSidebarPanel.css')} */`;
var { classMap: Et } = Lt,
  Se = { clearOverview: 'Clear overview', cssOverviewPanelSidebar: 'CSS overview panel sidebar' },
  Dt = xe.i18n.registerUIStrings('panels/css_overview/CSSOverviewSidebarPanel.ts', Se),
  We = xe.i18n.getLocalizedString.bind(void 0, Dt),
  qe = (i, e, t) => {
    Rt(
      Oe`
      <style>${ze}</style>
      <div class="overview-sidebar-panel" @click=${(s) => {
        if (s.target instanceof HTMLElement) {
          let n = s.target.dataset.id;
          n && i.onItemClick(n);
        }
      }} @keydown=${(s) => {
        if (!(s.key !== 'Enter' && s.key !== 'ArrowUp' && s.key !== 'ArrowDown')) {
          if (s.target instanceof HTMLElement) {
            let n = s.target.dataset.id;
            n && i.onItemKeyDown(n, s.key);
          }
          s.consume(!0);
        }
      }}
        ...
        ${i.items.map(({ id: s, name: n }) => {
          let a = s === i.selectedId;
          return Oe`
  ...
  j = class extends _e.Widget.VBox {
    #o;
    #t = [];
    #s;
    #i = (e, t) => {};
    #r = () => {};
    constructor(e, t = qe) {
      (super(e, { useShadowDom: !0, delegatesFocus: !0 }), (this.#o = t));
    }
    performUpdate() {
      let e = {
        items: this.#t,
        selectedId: this.#s,
        onReset: this.#r,
        onItemClick: this.#a.bind(this),
        onItemKeyDown: this.#n.bind(this),
      };
      this.#o(e, {}, this.contentElement);
    }
    set items(e) {
    ...
    #e(e, t = !1) {
      return ((this.#s = e), this.requestUpdate(), this.#i(e, t), this.updateComplete);
    }
    #a(e) {
      this.#e(e, !1);
    }
    #n(e, t) {
      if (t === 'Enter') this.#e(e, !0);
      else {
        let o = -1;
        for (let a = 0; a < this.#t.length; a++)
          if (this.#t[a].id === e) {
            o = a;
            break;
          }
        if (o < 0) return;
        let s = (o + (t === 'ArrowDown' ? 1 : -1)) % this.#t.length,
          n = this.#t[s].id;
        if (!n) return;
        this.#e(n, !1).then(() => {
...
var { styleMap: de, ref: G } = Vt,
  ...
  l = {
    ...
    textColorSOverSBackgroundResults:
      'Text color {PH1} over {PH2} background results in low contrast for {PH3} elements',
    aa: 'AA',
    aaa: 'AAA',
    apca: 'APCA',
    element: 'Element',
  ...
  Pt = ye.i18n.registerUIStrings('panels/css_overview/CSSOverviewCompletedView.ts', l),
  d = ye.i18n.getLocalizedString.bind(void 0, Pt);
function Ce(i) {
  let { h: e, s: t, l: o } = i.as('hsl');
  return (
    (e = Math.round(e * 360)),
    (t = Math.round(t * 100)),
    (o = Math.round(o * 100)),
    (o = Math.max(0, o - 15)),
    `1px solid hsl(${e}deg ${t}% ${o}%)`
  );
}
var Nt = new Intl.NumberFormat('en-US'),
  Ye = (i, e, t) => {
    function o(r, s) {
      r && (r.scrollIntoView(), s && r.querySelector('button, [tabindex="0"]')?.focus());
    }
    Je(
      u`
      <style>${re}</style>
      <devtools-split-view direction="column" sidebar-position="first" sidebar-initial-size="200">
        <devtools-widget slot="sidebar" ${Be(j, {
          minimumSize: new Qe.Size(100, 25),
          items: [
            { name: d(l.overviewSummary), id: 'summary' },
            { name: d(l.colors), id: 'colors' },
            { name: d(l.fontInfo), id: 'font-info' },
            { name: d(l.unusedDeclarations), id: 'unused-declarations' },
            { name: d(l.mediaQueries), id: 'media-queries' },
          ],
          selectedId: i.selectedSection,
          onItemSelected: i.onSectionSelected,
          onReset: i.onReset,
        })}>
        </devtools-widget>
        <devtools-split-view sidebar-position="second" slot="main" direction="row" sidebar-initial-size="minimized">
          <div class="vbox overview-completed-view" slot="main" @click=${i.onClick}>
            ...
            <div class="results-section horizontally-padded summary"
                  ${G((r) => {
                    e.revealSection.set('summary', o.bind(null, r));
                  })}>
            ...
            <div class="results-section horizontally-padded colors"
                ${G((r) => {
                  e.revealSection.set('colors', o.bind(null, r));
                })}>
                <h1>${d(l.colors)}</h1>
              ...
              <div class="results-section font-info"
                    ${G((r) => {
                      e.revealSection.set('font-info', o.bind(null, r));
                    })}>
                <h1>${d(l.fontInfo)}</h1>
                ${Ft(i.fontInfo)}
              </div>
              <div class="results-section unused-declarations"
                    ${G((r) => {
                      e.revealSection.set('unused-declarations', o.bind(null, r));
                    })}>
              ...
              <div class="results-section media-queries"
                    ${G((r) => {
                      e.revealSection.set('media-queries', o.bind(null, r));
                    })}>
              <h1>${d(l.mediaQueries)}</h1>
              ${_t(i.mediaQueries)}
            </div>
          </div>
          <devtools-widget slot="sidebar" ${Be((r) => {
            let s = new V.TabbedPane.TabbedPane(r);
            return (
              (e.closeAllTabs = () => {
                s.closeTabs(s.tabIds());
              }),
              (e.addTab = (n, a, c, p) => {
                (s.hasTab(n) || s.appendTab(n, a, c, void 0, void 0, !0, void 0, void 0, p),
                  s.selectTab(n),
                  s.parentWidget().setSidebarMinimized(!1));
              }),
              s.addEventListener(V.TabbedPane.Events.TabClosed, (n) => {
                s.tabIds().length === 0 && s.parentWidget().setSidebarMinimized(!0);
              }),
              s
            );
          })}>
...
function Ot(i, e, t, o, r) {
  return u`
    ...
    <ul>${r.map((s) => ne('border', s))}</ul>`;
}
function Wt(i) {
  return i.length > 0
    ? $e(i, 'unused-declarations')
    : u`<div class="horizontally-padded">${d(l.thereAreNoUnusedDeclarations)}</div>`;
}
function _t(i) {
  return i.length > 0
    ? $e(i, 'media-queries')
    : u`<div class="horizontally-padded">${d(l.thereAreNoMediaQueries)}</div>`;
}
function Ft(i) {
  return i.length > 0
    ? u`${i.map(
        ({ font: e, fontMetrics: t }) => u`
    <section class="font-family">
      <h2>${e}</h2>
      ${qt(e, t)}
    </section>`,
      )}`
    : u`<div>${d(l.thereAreNoFonts)}</div>`;
}
function qt(i, e) {
  return u`
...
function $e(i, e, t = '') {
  let o = i.reduce((r, s) => r + s.nodes.length, 0);
  return u`
      <ul aria-label=${e}>
        ${i.map(({ title: r, nodes: s }) => {
          let n = (100 * s.length) / o,
            a = d(l.nOccurrences, { n: s.length });
          return u`<li>
...
function Bt(i, e) {
  ...
  let t = e[0];
  for (let c of e) Math.abs(c.contrastRatio) < Math.abs(t.contrastRatio) && (t = c);
  let o = t.textColor.asString('hexa'),
    r = t.backgroundColor.asString('hexa'),
    s = q.Runtime.experiments.isEnabled(q.ExperimentNames.ExperimentName.APCA),
    n = d(l.textColorSOverSBackgroundResults, { PH1: o, PH2: r, PH3: e.length }),
    a = Ce(t.backgroundColor.asLegacyColor());
  return u`<li>
    <button
...
function ne(i, e) {
  let t = D.Color.parse(e)?.asLegacyColor();
  return t
    ? u`<li>
...
var P = class i extends V.Widget.VBox {
    onReset = () => {};
    #o = 'summary';
    #t;
    #s;
    ...
    #e;
    #a;
    #n = { revealSection: new Map(), closeAllTabs: () => {}, addTab: (e, t, o, r) => {} };
    constructor(e, t = Ye) {
      (super(e),
        (this.#a = t),
        this.registerRequiredCSS(re),
        (this.#i = new Ge.Linkifier.Linkifier(20, !0)),
        (this.#r = new Map()),
        (this.#e = null));
    }
    set target(e) {
      if (!e) return;
      let t = e.model(J.CSSModel.CSSModel),
        o = e.model(J.DOMModel.DOMModel);
      if (!t || !o) throw new Error('Target must provide CSS and DOM models');
      ((this.#t = t), (this.#s = o));
    }
    #c(e, t) {
      let o = this.#n.revealSection.get(e);
      o && o(t);
    }
    #u() {
      (this.#p(), this.onReset());
      ...
      switch (r) {
        case 'contrast': {
          let n = o.section,
            a = o.key;
          if (!a) return;
          let c = this.#e.textColorContrastIssues.get(a) || [];
          s = { type: r, key: a, nodes: c, section: n };
          break;
        }
        case 'color': {
          let n = o.color,
          ...
          switch (a) {
            case 'text':
              c = this.#e.textColors.get(n);
              break;
            case 'background':
              c = this.#e.backgroundColors.get(n);
              break;
            case 'fill':
              c = this.#e.fillColors.get(n);
              break;
            case 'border':
              c = this.#e.borderColors.get(n);
              break;
          }
          if (!c) return;
          ((c = Array.from(c).map((p) => ({ nodeId: p }))),
            (s = { type: r, color: n, nodes: c, section: a }));
          break;
        }
        case 'unused-declarations': {
          let n = o.label;
          if (!n) return;
          let a = this.#e.unusedDeclarations.get(n);
          if (!a) return;
          s = { type: r, declaration: n, nodes: a };
          break;
        }
        case 'media-queries': {
          let n = o.label;
          if (!n) return;
          let a = this.#e.mediaQueries.get(n);
          if (!a) return;
          s = { type: r, text: n, nodes: a };
          break;
        }
        case 'font-info': {
          let n = o.label;
          if (!o.path) return;
          let [a, c] = o.path.split('/');
          if (!n) return;
          let p = this.#e.fontInfo.get(a);
          if (!p) return;
          let f = p.get(c);
          if (!f) return;
          let y = f.get(n);
          if (!y) return;
          let B = y.map((X) => ({ nodeId: X })),
            he = `${n} (${a}, ${c})`;
          s = { type: r, name: he, nodes: B };
          break;
        }
        default:
          return;
      }
      (e.consume(), this.#d(s), this.requestUpdate());
    }
    performUpdate() {
      if (!this.#e || !('backgroundColors' in this.#e) || !('textColors' in this.#e)) return;
      let e = {
        elementCount: this.#e.elementCount,
        backgroundColors: this.#l(this.#e.backgroundColors),
        textColors: this.#l(this.#e.textColors),
        textColorContrastIssues: this.#e.textColorContrastIssues,
        fillColors: this.#l(this.#e.fillColors),
        borderColors: this.#l(this.#e.borderColors),
        globalStyleStats: this.#e.globalStyleStats,
        mediaQueries: this.#m(this.#e.mediaQueries),
        unusedDeclarations: this.#m(this.#e.unusedDeclarations),
        fontInfo: this.#f(this.#e.fontInfo),
        selectedSection: this.#o,
        onClick: this.#h.bind(this),
        onSectionSelected: this.#c.bind(this),
        onReset: this.#u.bind(this),
      };
      this.#a(e, this.#n, this.element);
    }
    #d(e) {
      let t = '',
        o = '';
      switch (e.type) {
        case 'contrast': {
          let { section: s, key: n } = e;
          ((t = `${s}-${n}`), (o = d(l.contrastIssues)));
          break;
        }
        case 'color': {
          let { section: s, color: n } = e;
          ((t = `${s}-${n}`), (o = `${n.toUpperCase()} (${s})`));
          break;
        }
        case 'unused-declarations': {
          let { declaration: s } = e;
      ...
      if (!r) {
        ...
        ((r = new ae(this.#s, this.#t, this.#i)), (r.data = e.nodes), this.#r.set(t, r));
      }
      this.#n.addTab(t, o, r, e.type);
    }
    #l(e) {
      return Array.from(e.keys()).sort((t, o) => {
        let r = D.Color.parse(t)?.asLegacyColor(),
          s = D.Color.parse(o)?.asLegacyColor();
        return !r || !s ? 0 : D.ColorUtils.luminance(s.rgba()) - D.ColorUtils.luminance(r.rgba());
      });
    }
    #f(e) {
      return Array.from(e.entries()).map(([o, r]) => {
        let s = Array.from(r.entries());
        return { font: o, fontMetrics: s.map(([n, a]) => ({ label: n, values: this.#m(a) })) };
      });
    }
    #m(e) {
      return Array.from(e.entries())
        .sort((t, o) => {
          let r = t[1];
          return o[1].length - r.length;
        })
        .map(([t, o]) => ({ title: t, nodes: o }));
    }
    set overviewData(e) {
  ...
  ae = class extends V.Widget.Widget {
    #o;
    #t;
    #s;
    #i;
    #r;
    constructor(e, t, o, r = Ze) {
      (super(), (this.#o = e), (this.#t = t), (this.#s = o), (this.#r = r), (this.#i = []));
    }
    set data(e) {
      ((this.#i = e), this.requestUpdate());
    }
    async performUpdate() {
      let e = new Set();
      if (!this.#i.length) {
        this.#r({ items: [], visibility: e }, {}, this.element);
        return;
      }
      let [t] = this.#i;
      ('nodeId' in t && t.nodeId && e.add('node-id'),
        'declaration' in t && t.declaration && e.add('declaration'),
        'sourceURL' in t && t.sourceURL && e.add('source-url'),
        'contrastRatio' in t && t.contrastRatio && e.add('contrast-ratio'));
      let o;
      if ('nodeId' in t && e.has('node-id')) {
        let s = this.#i.reduce((n, a) => {
          let c = a.nodeId;
          return P.pushedNodes.has(c) ? n : (P.pushedNodes.add(c), n.add(c));
        }, new Set());
        o = await this.#o.pushNodesByBackendIdsToFrontend(s);
      }
      let r = await Promise.all(
        this.#i.map(async (s) => {
          let n, a;
          if ('nodeId' in s && e.has('node-id')) {
            let c = o?.get(s.nodeId) ?? null;
            c &&
              ((n = Xe.DOMLinkifier.Linkifier.instance().linkify(c)),
              (a = () => c.scrollIntoView()));
          }
          if ('range' in s && s.range && s.styleSheetId && e.has('source-url')) {
            let c = je.TextRange.TextRange.fromObject(s.range),
              p = this.#t.styleSheetHeaderForId(s.styleSheetId);
            if (p) {
              let f = p.lineNumberInSource(c.startLine),
                y = p.columnNumberInSource(c.startLine, c.startColumn),
                B = new J.CSSModel.CSSLocation(p, f, y);
              n = u`${this.#s.linkifyCSSLocation(B)}`;
            }
          }
          return { data: s, link: n, showNode: a };
        }),
      );
      this.#r({ items: r, visibility: e }, {}, this.element);
    }
  };
function Kt(i, e, t) {
  return e
...
function Qt(i) {
  if (!('declaration' in i)) throw new Error('Declaration entry is missing a declaration.');
  return u`<td>${i.declaration}</td>`;
}
function jt(i, e) {
  return 'range' in i && i.range
    ? e
      ? u`<td>${e}</td>`
      : u`<td>${d(l.unableToLink)}</td>`
    : u`<td>${d(l.unableToLinkToInlineStyle)}</td>`;
}
function Gt(i) {
  if (!('contrastRatio' in i)) throw new Error('Contrast ratio entry is missing a contrast ratio.');
  let e = q.Runtime.experiments.isEnabled(q.ExperimentNames.ExperimentName.APCA),
    t = Ke.NumberUtilities.floor(i.contrastRatio, 2),
    o = e ? t + '%' : t,
    r = Ce(i.backgroundColor),
    s = i.textColor.asString(),
    n = i.backgroundColor.asString();
  return u`
    <td>
      <div class="contrast-container-in-grid">
          ...
          ${
            e
              ? u`
            <span>${d(l.apca)}</span>${i.thresholdsViolated.apca ? _() : F()}`
              : u`
...
var rt = {};
O(rt, { CSSOverviewPanel: () => Ae, DEFAULT_VIEW: () => it });
import * as ue from './../../core/host/host.js';
import * as z from './../../core/sdk/sdk.js';
import * as pe from './../../ui/legacy/legacy.js';
import { render as to } from './../../ui/lit/lit.js';
...
var N = {
  ...
  Yt = ke.i18n.registerUIStrings('panels/css_overview/CSSOverviewStartView.ts', N),
  H = ke.i18n.getLocalizedString.bind(void 0, Yt),
  ot = 'https://g.co/devtools/css-overview-feedback',
  Zt = 'https://developer.chrome.com/docs/devtools/css-overview',
  eo = (i, e, t) => {
    Xt(
      Jt`
    <style>${tt}</style>
  ...
  ce = class extends st.Widget.Widget {
    #o;
    onStartCapture = () => {};
    constructor(e, t = eo) {
      (super(e, { useShadowDom: !0, delegatesFocus: !0 }), (this.#o = t), this.performUpdate());
    }
    performUpdate() {
      this.#o({ onStartCapture: this.onStartCapture }, {}, this.contentElement);
    }
  };
var { widget: Ie } = pe.Widget,
  it = (i, e, t) => {
    to(
      i.state === 'start'
        ? Ie(ce, { onStartCapture: i.onStartCapture })
        : i.state === 'processing'
          ? Ie(Q, { onCancel: i.onCancel })
          : Ie(P, { onReset: i.onReset, overviewData: i.overviewData, target: i.target }),
      t,
    );
  },
  Ae = class extends pe.Panel.Panel {
    #o;
    #t;
    #s;
    #i;
    ...
    constructor(e = it) {
      (super('css-overview'),
        ...
        z.TargetManager.TargetManager.instance().addEventListener(
          'InspectedURLChanged',
          this.#m,
          this,
        ),
        (this.#l = e),
        z.TargetManager.TargetManager.instance().observeTargets(this),
        this.#v());
    }
    #f() {
      (ue.userMetrics.actionTaken(ue.UserMetrics.Action.CaptureCssOverviewClicked), this.#x());
    }
    #m() {
      this.#o !== z.TargetManager.TargetManager.instance().inspectedURL() &&
        ((this.#o = z.TargetManager.TargetManager.instance().inspectedURL()), this.#v());
    }
    targetAdded(e) {
      e === z.TargetManager.TargetManager.instance().primaryPageTarget() &&
        (this.#t = e.model(W) ?? void 0);
    }
    targetRemoved() {}
    #g() {
      if (!this.#t) throw new Error('Did not retrieve model information yet.');
      return this.#t;
    }
    #v() {
      ((this.#s = new Map()),
        ...
        (this.#p = {
          ...
          inlineStyles: 0,
          externalSheets: 0,
          stats: { type: 0, class: 0, id: 0, universal: 0, attribute: 0, nonSimple: 0 },
        }),
        (this.#h = new Map()),
        this.#w());
    }
    #w() {
      ((this.#d = 'start'), this.performUpdate());
    }
    #b() {
      ((this.#d = 'processing'), this.performUpdate());
    }
    #S() {
      ((this.#d = 'completed'), this.performUpdate());
    }
    performUpdate() {
      let e = {
        state: this.#d,
        onStartCapture: this.#f.bind(this),
        onCancel: this.#v.bind(this),
        onReset: this.#v.bind(this),
        target: this.#t?.target(),
        overviewData: {
          backgroundColors: this.#s,
          textColors: this.#i,
          textColorContrastIssues: this.#h,
          fillColors: this.#r,
          borderColors: this.#e,
          globalStyleStats: this.#p,
          fontInfo: this.#a,
          elementCount: this.#u,
          mediaQueries: this.#n,
          unusedDeclarations: this.#c,
        },
      };
      this.#l(e, {}, this.contentElement);
    }
    async #x() {
      this.#b();
      let e = this.#g(),
        [
          t,
          {
            ...
            backgroundColors: r,
            textColors: s,
            textColorContrastIssues: n,
            fillColors: a,
            borderColors: c,
            fontInfo: p,
            unusedDeclarations: f,
          },
          y,
        ] = await Promise.all([
          e.getGlobalStylesheetStats(),
          e.getNodeStyleStats(),
          e.getMediaQueries(),
        ]);
      (o && (this.#u = o),
...
export {
  et as CSSOverviewCompletedView,
  Re as CSSOverviewModel,
  rt as CSSOverviewPanel,
  Ne as CSSOverviewProcessingView,
  He as CSSOverviewSidebarPanel,
  Me as CSSOverviewUnusedDeclarations,
};
