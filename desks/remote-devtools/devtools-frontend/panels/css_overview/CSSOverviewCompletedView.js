...
const str_ = i18n.i18n.registerUIStrings(
  'panels/css_overview/CSSOverviewCompletedView.ts',
  UIStrings,
);
    ...
    html` <style>
      ...
      <devtools-split-view direction="column" sidebar-position="first" sidebar-initial-size="200">
        ...
        <devtools-split-view
          ...
          slot="main"
          direction="row"
          sidebar-initial-size="minimized"
        >
          <div class="vbox overview-completed-view" slot="main" @click=${input.onClick}>
            ...
            <div
              class="results-section horizontally-padded summary"
              ${ref((e) => {
            ...
            <div
              class="results-section horizontally-padded colors"
              ${ref((e) => {
            ...
            >
              ...
              ${renderColors(
                ...
                input.textColors,
                input.textColorContrastIssues,
                input.fillColors,
                input.borderColors,
              )}
            </div>
            <div
              class="results-section font-info"
              ${ref((e) => {
            ...
            <div
              class="results-section unused-declarations"
              ${ref((e) => {
            ...
            <div
              class="results-section media-queries"
              ${ref((e) => {
...
function renderColors(
  ...
  // clang-format off
  return html` <h2>${i18nString(UIStrings.backgroundColorsS, { PH1: backgroundColors.length })}</h2>
    <ul>
      ${backgroundColors.map((c) => renderColor('background', c))}
    </ul>

    <h2>${i18nString(UIStrings.textColorsS, { PH1: textColors.length })}</h2>
    <ul>
      ${textColors.map((c) => renderColor('text', c))}
    </ul>
    ...
    <ul>
      ${fillColors.map((c) => renderColor('fill', c))}
    </ul>

    <h2>${i18nString(UIStrings.borderColorsS, { PH1: borderColors.length })}</h2>
    <ul>
      ${borderColors.map((c) => renderColor('border', c))}
    </ul>`;
  // clang-format on
}
function renderUnusedDeclarations(unusedDeclarations) {
  return unusedDeclarations.length > 0
    ...
    : html`<div class="horizontally-padded">
        ${i18nString(UIStrings.thereAreNoUnusedDeclarations)}
      </div>`;
}
...
function renderFontInfo(fonts) {
  return fonts.length > 0
    ? html`${fonts.map(
        ({ font, fontMetrics }) =>
          html` <section class="font-family">
            <h2>${font}</h2>
...
function renderFontMetrics(font, fontMetricInfo) {
  return html` <div class="font-metric">
    ${fontMetricInfo.map(
      ({ label, values }) =>
        html` <div>
          <h3>${label}</h3>
...
function renderGroup(values, type, path = '') {
  ...
  // clang-format off
  return html` <ul aria-label=${type}>
...
function renderContrastIssues(issues) {
  // clang-format off
  return html` <h2>${i18nString(UIStrings.contrastIssuesS, { PH1: issues.size })}</h2>
    <ul>
...
function renderContrastIssue(key, issues) {
  ...
  return html`<li>
    ...
    <div class="block-title">
      ${showAPCA
        ? html` <div class="contrast-warning hidden">
...
export class CSSOverviewCompletedView extends UI.Widget.VBox {
  ...
  #sortColorsByLuminance(srcColors) {
    return Array.from(srcColors.keys()).sort((colA, colB) => {
      ...
      return (
        Common.ColorUtils.luminance(colorB.rgba()) - Common.ColorUtils.luminance(colorA.rgba())
      );
    });
  }
  ...
  render(
    html` <div>
      ...
      >
        <table>
          <tr>
            ${visibility.has('node-id')
              ? html` <th id="node-id" weight="50" sortable>${i18nString(UIStrings.element)}</th>`
              : nothing}
            ${visibility.has('declaration')
              ? html` <th id="declaration" weight="50" sortable>
                  ${i18nString(UIStrings.declaration)}
                </th>`
              : nothing}
            ${visibility.has('source-url')
              ? html` <th id="source-url" weight="100">${i18nString(UIStrings.source)}</th>`
              : nothing}
            ${visibility.has('contrast-ratio')
              ? html` <th id="contrast-ratio" weight="25" width="150px" sortable fixed>
      ...
      this.#data.map(async (item) => {
        ...
        if ('range' in item && item.range && item.styleSheetId && visibility.has('source-url')) {
          ...
          if (styleSheetHeader) {
            ...
            const matchingSelectorLocation = new SDK.CSSModel.CSSLocation(
              styleSheetHeader,
              lineNumber,
              columnNumber,
            );
...
function renderNode(data, link, showNode) {
  ...
  return html` <td>
    ...
    <devtools-icon
      part="show-element"
      name="select-element"
      title=${i18nString(UIStrings.showElement)}
      tabindex="0"
      @click=${() => showNode?.()}
    ></devtools-icon>
  </td>`;
...
function renderContrastRatio(data) {
  ...
  return html` <td>
    <div class="contrast-container-in-grid">
      ...
      ${showAPCA
        ? html` <span>${i18nString(UIStrings.apca)}</span>${data.thresholdsViolated.apca
              ? createClearIcon()
              : createCheckIcon()}`
        : html` <span>${i18nString(UIStrings.aa)}</span>${data.thresholdsViolated.aa
              ? createClearIcon()
              : createCheckIcon()} <span>${i18nString(UIStrings.aaa)}</span>${data
              .thresholdsViolated.aaa
              ? createClearIcon()
              : createCheckIcon()}`}
    </div>
  </td>`;
