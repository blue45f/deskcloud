...
export class CSSOverviewModel extends SDK.SDKModel.SDKModel {
  ...
  async getNodeStyleStats() {
    ...
    const formatColor = (color) => {
      if (color instanceof Common.Color.Legacy) {
        return color.hasAlpha()
          ? color.asString('hexa' /* Common.Color.Format.HEXA */)
          : color.asString('hex' /* Common.Color.Format.HEX */);
      }
    ...
    let elementCount = 0;
    const { documents, strings } =
      await this.#domSnapshotAgent.invoke_captureSnapshot(snapshotConfig);
    for (const { nodes, layout } of documents) {
      ...
      for (let idx = 0; idx < layout.styles.length; idx++) {
        ...
        const [
          ...
          borderTopWidthIdx,
          borderTopColorIdx,
          borderBottomWidthIdx,
          borderBottomColorIdx,
          borderLeftWidthIdx,
          borderLeftColorIdx,
          borderRightWidthIdx,
          borderRightColorIdx,
        ...
        if (textColor && blendedBackgroundColor) {
          ...
          const blendedTextColor = textColor
            .asLegacyColor()
            .blendWithAlpha(layout.textColorOpacities ? layout.textColorOpacities[idx] : 1);
          ...
          if (Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.APCA)) {
            ...
            const passes =
              contrastRatio && threshold ? Math.abs(contrastRatio) >= threshold : false;
        ...
        if (
          verticalAlignIdx !== -1 &&
          !isTableElementWithDefaultStyles(strings[nodeName], strings[displayIdx])
        ) {
...
SDK.SDKModel.SDKModel.register(CSSOverviewModel, {
