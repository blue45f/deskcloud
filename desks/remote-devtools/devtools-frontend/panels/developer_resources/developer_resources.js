...
var F = (r, t) => {
  for (var s in t) C(r, s, { get: t[s], enumerable: !0 });
};
var V = {};
F(V, {
  DEFAULT_VIEW: () => M,
  DeveloperResourcesRevealer: () => y,
...
import { html as z, render as B } from './../../ui/lit/lit.js';
...
import { html as h, nothing as l, render as H } from './../../ui/lit/lit.js';
var S = `@scope to (devtools-widget > *){.data-grid{border:none}::part(url-outer){width:100%;display:inline-flex;justify-content:flex-start}::part(filter-highlight){font-weight:bold}::part(url-prefix){overflow-x:hidden;text-overflow:ellipsis}::part(url-suffix){flex:none}}
...
var i = {
  ...
  P = c.i18n.registerUIStrings('panels/developer_resources/DeveloperResourcesListView.ts', i),
  a = c.i18n.getLocalizedString.bind(void 0, P),
  { withThousandsSeparator: E } = L.NumberUtilities,
  q = (r, t, s) => {
    function o(e, d) {
      if (!e) return '';
      let u = r.filters.find((D) => D.key?.split(',')?.includes(d));
      if (!u?.regex) return '';
      let w = u.regex.exec(e);
      return w?.length ? `${w.index},${w[0].length}` : '';
    }
    H(
      h`
      <style>${S}</style>
  ...
  v = class extends m.Widget.VBox {
    ...
    #s = [];
    constructor(t, s = q) {
      (super(t, { useShadowDom: !0 }), (this.#o = s));
    }
    set selectedItem(t) {
    ...
    #i(t, s) {
      ...
      o &&
        t.clipboardSection().appendItem(
          a(i.copyUrl),
          () => {
            b.InspectorFrontendHost.InspectorFrontendHostInstance.copyText(o);
          },
          { jslogContext: 'copy-url' },
        );
      let e = s.dataset.initiatorUrl;
      e &&
        t.clipboardSection().appendItem(
          a(i.copyInitiatorUrl),
          () => {
            b.InspectorFrontendHost.InspectorFrontendHostInstance.copyText(e);
          },
          { jslogContext: 'copy-initiator-url' },
        );
    }
    set items(t) {
      ((this.#e = [...t]), this.requestUpdate());
    }
    reset() {
      ...
      ((this.#s = t),
        ...
        this.updateComplete.then(() => {
          let s =
              Number(
                this.contentElement
                  .querySelector('devtools-data-grid')
                  ?.getAttribute('aria-rowcount'),
              ) ?? 0,
            o = '';
          (s === 0 ? (o = a(i.noResourceMatches)) : (o = a(i.numberOfResourceMatch, { n: s })),
            m.ARIAUtils.LiveAnnouncer.alert(o));
        }));
    }
    performUpdate() {
      let t = {
...
var { widget: _ } = n.Widget,
  ...
  O = U.i18n.registerUIStrings('panels/developer_resources/DeveloperResourcesView.ts', p),
  g = U.i18n.getLocalizedString.bind(void 0, O),
  y = class {
    async reveal(t) {
      let o = I.PageResourceLoader.PageResourceLoader.instance().getResourcesLoaded().get(t.key);
      if (o)
        return (
          await n.ViewManager.ViewManager.instance().showView('developer-resources'),
          await (
            await n.ViewManager.ViewManager.instance().view('developer-resources').widget()
          ).select(o)
        );
    }
  },
  ...
  $ = class extends n.Widget.VBox {
    ...
    constructor(t = M) {
      (super({ useShadowDom: !0 }),
        (this.#t = t),
        (this.#e = I.PageResourceLoader.PageResourceLoader.instance()),
        this.#e.addEventListener('Update', this.requestUpdate, this),
        this.requestUpdate());
    }
    async performUpdate() {
      let { loading: t, resources: s } = this.#e.getScopedNumberOfResources(),
        o = {
          onFilterChanged: (d) => {
            this.onFilterChanged(d.detail);
          },
          items: this.#e.getResourcesLoaded().values(),
          selectedItem: this.#r,
          onSelect: (d) => {
            this.#r = d;
          },
          ...
          numResources: s,
          numLoading: t,
          loadThroughTargetSetting: this.#e.getLoadThroughTargetSetting(),
        },
        e = {};
      this.#t(o, e, this.contentElement);
    }
    async select(t) {
      (await this.updateComplete, (this.#r = t), this.requestUpdate());
    }
    async selectedItem() {
      return (await this.updateComplete, this.#r);
    }
    onFilterChanged(t) {
      let s = t ? T.StringUtilities.createPlainTextSearchRegex(t, 'i') : null;
      (s ? (this.#o = [{ key: 'url,error-message', regex: s, negative: !1 }]) : (this.#o = []),
        this.requestUpdate());
    }
  };
