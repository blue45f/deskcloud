...
const str_ = i18n.i18n.registerUIStrings(
  'panels/developer_resources/DeveloperResourcesView.ts',
  UIStrings,
);
...
export class DeveloperResourcesRevealer {
  async reveal(resourceInitiatorKey) {
    ...
    if (resource) {
      ...
      const developerResourcesView = await UI.ViewManager.ViewManager.instance()
  ...
  render(
    html` <style>
      ...
      <div
        ...
        jslog=${VisualLogging.panel('developer-resources').track({ resize: true })}
      >
        <div
          class="developer-resource-view-toolbar-container"
        ...
        >
          <devtools-toolbar class="developer-resource-view-toolbar" role="presentation">
            <devtools-toolbar-input
              type="filter"
              placeholder=${i18nString(UIStrings.filterByText)}
        ...
        <div class="developer-resource-view-toolbar-summary">
          <div class="developer-resource-view-message">
            ${input.numLoading > 0
              ? i18nString(UIStrings.resourcesCurrentlyLoading, {
                  PH1: input.numResources,
                  PH2: input.numLoading,
                })
...
export class DeveloperResourcesView extends UI.Widget.VBox {
  ...
  onFilterChanged(text) {
    const textFilterRegExp = text
      ? Platform.StringUtilities.createPlainTextSearchRegex(text, 'i')
      : null;
    if (textFilterRegExp) {
      this.#filters = [{ key: 'url,error-message', regex: textFilterRegExp, negative: false }];
    } else {
      this.#filters = [];
    }
    this.requestUpdate();
