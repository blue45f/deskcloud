...
const str_ = i18n.i18n.registerUIStrings(
  'panels/developer_resources/DeveloperResourcesListView.ts',
  UIStrings,
);
...
const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html` <style>
        ${developerResourcesListViewStyles}
      </style>
      <devtools-data-grid
      ...
      >
        <table>
          <tr>
            <th id="status" sortable fixed width="60px">${i18nString(UIStrings.status)}</th>
            <th id="url" sortable width="250px">${i18nString(UIStrings.url)}</th>
            <th id="initiator" sortable width="80px">${i18nString(UIStrings.initiator)}</th>
            ...
            <th id="error-message" sortable width="200px">${i18nString(UIStrings.error)}</th>
          </tr>
          ${input.items.map((item, index) => {
            const splitURL = /^(.*)(\/[^/]*)$/.exec(item.url);
            return html` <tr
              selected=${item === input.selectedItem || nothing}
            ...
            >
              <td>
                ${item.success === true
                  ? i18nString(UIStrings.success)
                  : item.success === false
                    ? i18nString(UIStrings.failure)
                    : i18nString(UIStrings.pending)}
              </td>
              ...
              <td
                title=${item.initiator.initiatorUrl || ''}
              ...
              >
                ${item.initiator.initiatorUrl || ''}
              </td>
              <td
                aria-label=${item.size !== null
                  ? i18nString(UIStrings.sBytes, { n: item.size })
                  : nothing}
                data-value=${item.size ?? nothing}
              >
                ${item.size !== null ? html`<span>${withThousandsSeparator(item.size)}</span>` : ''}
              </td>
              <td
                aria-label=${item.duration !== null
                  ? i18n.TimeUtilities.millisToString(item.duration)
                  : nothing}
                data-value=${item.duration ?? nothing}
              >
                ${item.duration !== null
                  ? html`<span>${i18n.TimeUtilities.millisToString(item.duration)}</span>`
                  : ''}
              </td>
              <td class="error-message">
                ${item.errorMessage
                  ? html` <devtools-highlight
                      ranges=${highlightRange(item.errorMessage, 'error-message')}
                    >
                      ${item.errorMessage}
                    </devtools-highlight>`
                  : nothing}
...
export class DeveloperResourcesListView extends UI.Widget.VBox {
  ...
  #populateContextMenu(contextMenu, element) {
    ...
    if (initiatorUrl) {
      contextMenu.clipboardSection().appendItem(
        i18nString(UIStrings.copyInitiatorUrl),
        () => {
  ...
  set filters(filters) {
    ...
    void this.updateComplete.then(() => {
      const numberOfResourceMatch =
        Number(
          this.contentElement.querySelector('devtools-data-grid')?.getAttribute('aria-rowcount'),
        ) ?? 0;
      let resourceMatch = '';
