...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  if (!layer) {
    // clang-format off
    Lit.render(
      html`<div class="layer-details-container">
        <devtools-widget
          class="learn-more"
  ...
  const renderStickyPositionConstraint = (constraint) => {
    ...
    return html`
      <span
        >${i18nString(UIStrings.stickyBoxRectangleDimensions, {
          PH1: stickyBoxRect.width,
          PH2: stickyBoxRect.height,
          PH3: stickyBoxRect.x,
          PH4: stickyBoxRect.y,
        })}</span
      >
      <span>, </span>
      <span
        >${i18nString(UIStrings.containingBlocRectangleDimensions, {
  ...
  Lit.render(
    html` <div class="layer-details-container">
      <table>
        <tbody>
          <tr>
            ...
            <td>
              ${i18nString(UIStrings.updateRectangleDimensions, {
                PH1: layer.width(),
                PH2: layer.height(),
                PH3: layer.offsetX(),
                PH4: layer.offsetY(),
          ...
          <tr>
            ...
            <td>
              ${!compositingReasons.length
                ? 'n/a'
                : html`<ul>
                    ${compositingReasons.map((reason) => html`<li>${reason}</li>`)}
                  </ul>`}
            </td>
          </tr>
          <tr>
              ...
              ${slowScrollRects.map(
                (scrollRect, index) =>
                  html` ${index > 0 ? ', ' : ''}
                    <span
                      class="scroll-rect"
                      @click=${(e) => onScrollRectClick(index, e)}
      ...
      ${snapshotSelection
        ? html` <button
            class="devtools-link link-margin text-button link-style"
            @click=${onPaintProfilerRequested}
            jslog=${VisualLogging.action('layers.paint-profiler').track({
              click: true,
              keydown: 'Enter',
            })}
          >
...
export class LayerDetailsView extends Common.ObjectWrapper.eventMixin(UI.Widget.Widget) {
  ...
  invokeProfilerLink() {
    ...
    const snapshotSelection =
      this.selection.type() === 'Snapshot' /* Type.SNAPSHOT */
        ? this.selection
  ...
  performUpdate() {
    ...
    const snapshotSelection =
      (this.selection && this.selection.type() === 'Snapshot' /* Type.SNAPSHOT */
        ? this.selection
        : layer
          ? this.layerSnapshotMap.get(layer)
          : null) || null;
    this.view(
      {
        layer,
...
export const slowScrollRectNames = new Map([
  [
    'NonFastScrollable' /* SDK.LayerTreeBase.Layer.ScrollRectType.NON_FAST_SCROLLABLE */,
    i18nLazyString(UIStrings.nonFastScrollable),
  ],
  [
    'TouchEventHandler' /* SDK.LayerTreeBase.Layer.ScrollRectType.TOUCH_EVENT_HANDLER */,
    i18nLazyString(UIStrings.touchEventHandler),
  ],
  [
    'WheelEventHandler' /* SDK.LayerTreeBase.Layer.ScrollRectType.WHEEL_EVENT_HANDLER */,
    i18nLazyString(UIStrings.wheelEventHandler),
  ],
  [
    'RepaintsOnScroll' /* SDK.LayerTreeBase.Layer.ScrollRectType.REPAINTS_ON_SCROLL */,
    i18nLazyString(UIStrings.repaintsOnScroll),
  ],
  [
