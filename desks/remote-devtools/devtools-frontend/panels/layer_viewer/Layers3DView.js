...
export class Layers3DView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(layerViewHost, view = DEFAULT_VIEW) {
    ...
    this.transformController = new TransformController(
      this.contentElement,
      false,
      false /* preventDefaultOnMouseDown */,
    );
    this.transformController.addEventListener(
      'TransformChanged' /* TransformControllerEvents.TRANSFORM_CHANGED */,
      this.updateData,
      this,
    );
  ...
  initShaders() {
    ...
    const aVertexPositionAttribute = this.gl.getAttribLocation(
      this.shaderProgram,
      'aVertexPosition',
    );
  ...
  updateTransformAndConstraints() {
    ...
    const minScaleConstraint =
      Math.min(
        baseWidth / dimensionsForAutoscale.width,
        baseHeight / dimensionsForAutoscale.width,
      ) / 2;
    ...
    for (let i = 0; i < this.rects.length; ++i) {
      bounds = Geometry.boundsForTransformedPoints(
        scaleAndRotationMatrix,
        this.rects[i].vertices,
        bounds,
      );
    }
    if (bounds) {
      this.transformController.clampOffsets(
        (paddingX - bounds.maxX) / globalThis.devicePixelRatio,
        (canvasWidth - paddingX - bounds.minX) / globalThis.devicePixelRatio,
        (paddingY - bounds.maxY) / globalThis.devicePixelRatio,
        (canvasHeight - paddingY - bounds.minY) / globalThis.devicePixelRatio,
      );
    }
    ...
    this.projectionMatrix = new WebKitCSSMatrix()
  ...
  calculateDepthsAndVisibility() {
    ...
    const root = showInternalLayers
      ? this.layerTree.root()
      : this.layerTree.contentRoot() || this.layerTree.root();
    if (!root) {
      return;
    }
    const queue = [root];
  ...
  calculateLayerScrollRects(layer) {
    ...
    for (let i = 0; i < scrollRects.length; ++i) {
      ...
      rect.calculateVerticesFromRect(
        layer,
        scrollRects[i].rect,
        this.calculateScrollRectDepth(layer, i),
      );
  ...
  drawViewportAndChrome() {
    ...
    const drawChrome =
      Common.Settings.Settings.instance().moduleSetting('frame-viewer-chrome-window').get() &&
      this.chromeTextures.length >= 3 &&
      this.chromeTextures.indexOf(undefined) < 0;
    ...
    this.drawRectangle(
      vertices,
      drawChrome ? this.gl.LINE_STRIP : this.gl.LINE_LOOP,
      ViewportBorderColor,
    );
    if (!drawChrome) {
      return;
    ...
    if (this.chromeTextures[0] && this.chromeTextures[2]) {
      const chromeTextureImage = imageForTexture.get(this.chromeTextures[0]) || {
        naturalHeight: 0,
        naturalWidth: 0,
      };
      const chromeHeight = chromeTextureImage.naturalHeight;
      const middleTextureImage = imageForTexture.get(this.chromeTextures[2]) || {
        naturalHeight: 0,
        naturalWidth: 0,
      };
      const middleFragmentWidth =
        viewportWidth - chromeTextureImage.naturalWidth - middleTextureImage.naturalWidth;
  ...
  selectionFromEventPoint(event) {
    ...
    const projectionMatrix = new WebKitCSSMatrix()
      .scale(1, -1, -1)
      .translate(-1, -1, 0)
      .multiply(this.projectionMatrix);
    const x0 =
      (mouseEvent.clientX - this.#canvasElement.getBoundingClientRect().left) *
      globalThis.devicePixelRatio;
    const y0 =
      -(mouseEvent.clientY - this.#canvasElement.getBoundingClientRect().top) *
      globalThis.devicePixelRatio;
        ...
        .appendItem(
          ...
          () =>
            this.dispatchEventToListeners(
              'PaintProfilerRequested' /* Events.PAINT_PROFILER_REQUESTED */,
              selection,
            ),
          {
  ...
  onMouseUp(event) {
    ...
    if (
      this.mouseDownX &&
      Math.abs(mouseEvent.clientX - this.mouseDownX) < maxDistanceInPixels &&
...
export class LayerTextureManager {
  ...
  setSnapshotsForLayer(layer, snapshots) {
    const oldSnapshotsToTiles = new Map(
      (this.tilesByLayer.get(layer) || []).map((tile) => [tile.snapshot, tile]),
    );
    const newTiles = [];
    const reusedTiles = [];
    ...
    return Promise.all(newTiles.map((tile) => tile.update(gl, this.scale))).then(
      this.textureUpdatedCallback,
    );
  }
  setScale(scale) {
...
export class Rectangle {
  ...
  intersectWithLine(matrix, x0, y0) {
    ...
    const normal = Geometry.crossProduct(
      Geometry.subtract(points[1], points[0]),
      Geometry.subtract(points[2], points[1]),
    );
    ...
    for (i = 0; i < tVects.length; ++i) {
      const product = Geometry.scalarProduct(
        normal,
        Geometry.crossProduct(tVects[i], tVects[(i + 1) % tVects.length]),
      );
      if (product < 0) {
        return undefined;
      }
