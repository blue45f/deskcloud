...
export class LayerTreeOutline extends Common.ObjectWrapper.eventMixin(UI.TreeOutline.TreeOutline) {
  ...
  update() {
    ...
    function updateLayer(layer) {
      ...
      const parent =
        layer === root
          ? this.treeOutline.rootElement()
          : parentLayer && layerToTreeElement.get(parentLayer);
      if (!parent) {
...
export class LayerTreeElement extends UI.TreeOutline.TreeElement {
  ...
  update() {
    ...
    details.textContent = i18nString(UIStrings.updateChildDimension, {
