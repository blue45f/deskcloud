...
export class NetworkItemView extends UI.TabbedPane.TabbedPane {
  ...
  constructor(request, calculator, initialTab) {
    ...
    this.headerElement().setAttribute(
      'jslog',
      `${VisualLogging.toolbar('request-details').track({
        keydown: 'ArrowUp|ArrowLeft|ArrowDown|ArrowRight|Enter|Space',
      })}`,
    );
    if (request.resourceType() === Common.ResourceType.resourceTypes.DirectSocket) {
      this.#firstTab =
        'direct-socket-connection' /* NetworkForward.UIRequestLocation.UIRequestTabs.DIRECT_SOCKET_CONNECTION */;
      this.appendTab(
        'direct-socket-connection' /* NetworkForward.UIRequestLocation.UIRequestTabs.DIRECT_SOCKET_CONNECTION */,
    ...
    if (request.resourceType() === Common.ResourceType.resourceTypes.WebSocket) {
      ...
      this.appendTab(
        'web-socket-frames' /* NetworkForward.UIRequestLocation.UIRequestTabs.WS_FRAMES */,
      ...
      this.appendTab(
        'direct-socket-chunks' /* NetworkForward.UIRequestLocation.UIRequestTabs.DIRECT_SOCKET_CHUNKS */,
        i18nString(UIStrings.messages),
        new ResourceDirectSocketChunkView(request),
        i18nString(UIStrings.directsocketMessages),
      );
    } else if (
      request.mimeType === 'text/event-stream' /* Platform.MimeType.MimeType.EVENTSTREAM */
    ) {
      this.appendTab(
        'eventSource' /* NetworkForward.UIRequestLocation.UIRequestTabs.EVENT_SOURCE */,
      ...
      this.appendTab(
        'response' /* NetworkForward.UIRequestLocation.UIRequestTabs.RESPONSE */,
      ...
      this.appendTab(
        'preview' /* NetworkForward.UIRequestLocation.UIRequestTabs.PREVIEW */,
    ...
    this.appendTab(
      'initiator' /* NetworkForward.UIRequestLocation.UIRequestTabs.INITIATOR */,
      i18nString(UIStrings.initiator),
      new RequestInitiatorView(request),
      i18nString(UIStrings.requestInitiatorCallStack),
    );
    this.appendTab(
      'timing' /* NetworkForward.UIRequestLocation.UIRequestTabs.TIMING */,
    ...
    if (request.trustTokenParams()) {
      ...
      this.appendTab(
        'trust-tokens' /* NetworkForward.UIRequestLocation.UIRequestTabs.TRUST_TOKENS */,
  ...
  maybeAppendCookiesPanel() {
    const cookiesPresent =
      this.#request.hasRequestCookies() || this.#request.responseCookies.length > 0;
    console.assert(
  ...
  maybeAppendDeviceBoundSessionsPanel() {
    ...
    if (deviceBoundSessionsPresent && !this.#deviceBoundSessionsView) {
      ...
      this.appendTab(
        'device-bound-sessions' /* NetworkForward.UIRequestLocation.UIRequestTabs.DEVICE_BOUND_SESSIONS */,
  ...
  async maybeAppendPayloadPanel() {
    ...
    if (this.#request.queryParameters || (await this.#request.requestFormData())) {
      ...
      this.appendTab(
        'payload' /* NetworkForward.UIRequestLocation.UIRequestTabs.PAYLOAD */,
