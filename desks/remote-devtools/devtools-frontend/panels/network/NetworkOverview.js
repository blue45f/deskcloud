...
export class NetworkOverview extends PerfUI.TimelineOverviewPane.TimelineOverviewBase {
  ...
  update() {
    ...
    const newBoundary = new NetworkTimeCalculator.NetworkTimeBoundary(
    ...
    if (!this.lastBoundary || !newBoundary.equals(this.lastBoundary)) {
      ...
      this.lastBoundary = new NetworkTimeCalculator.NetworkTimeBoundary(
        calculator.minimumBoundary(),
        calculator.maximumBoundary(),
      );
    }
    ...
    function drawLines(type) {
      ...
      context.fillStyle = ThemeSupport.ThemeSupport.instance().getComputedValue(
        RequestTimeRangeNameToColor[type],
      );
      for (let i = 0; i < n; ) {
    ...
    for (let i = 0; i < n; ++i) {
      ...
      for (let j = 0; j < timeRanges.length; ++j) {
        ...
        if (
          band !== -1 ||
          type === 'total' /* NetworkTimeCalculator.RequestTimeRangeNames.TOTAL */
        ) {
    ...
    context.beginPath();
    context.strokeStyle = ThemeSupport.ThemeSupport.instance().getComputedValue(
      NetworkLogView.getDCLEventColor(),
    );
    for (let i = this.domContentLoadedEvents.length - 1; i >= 0; --i) {
      const position = calculator.computePosition(
        Trace.Types.Timing.Milli(this.domContentLoadedEvents[i]),
      );
    ...
    context.beginPath();
    context.strokeStyle = ThemeSupport.ThemeSupport.instance().getComputedValue(
      NetworkLogView.getLoadEventColor(),
    );
...
export const RequestTimeRangeNameToColor = {
  ...
  ['sending' /* NetworkTimeCalculator.RequestTimeRangeNames.SENDING */]:
