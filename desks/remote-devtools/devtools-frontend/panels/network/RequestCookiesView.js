  ...
  render(
    html`
      ...
      <div class="request-cookies-view">
        ...
        <div
          class=${input.requestCookies.cookies.length || input.hasBlockedCookies ? '' : 'hidden'}
        >
          <span
            class="request-cookies-title"
            title=${i18nString(UIStrings.cookiesThatWereSentToTheServerIn)}
          >
        ...
        <div
          class="cookies-panel-item ${!input.requestCookies.cookies.length &&
          input.hasBlockedCookies
            ? ''
            : 'hidden'}"
        >
        ...
        ${input.requestCookies.cookies.length > 0
          ? html`
              <devtools-widget
                ${widget(CookieTable.CookiesTable.CookiesTable, {
                  cookiesData: input.requestCookies,
                  inline: true,
                })}
                class="cookie-table cookies-panel-item"
              ></devtools-widget>
            `
          : Lit.nothing}

        <div
          class="cookies-panel-item site-has-cookies-in-other-partition ${input.siteHasCookieInOtherPartition
            ? ''
            : 'hidden'}"
        >
        ...
        <div
          class="request-cookies-title ${input.responseCookies.cookies.length ? '' : 'hidden'}"
          title=${i18nString(UIStrings.cookiesThatWereReceivedFromThe)}
        >
          ${i18nString(UIStrings.responseCookies)}
        </div>

        ${input.responseCookies.cookies.length
          ? html`
              <devtools-widget
                ${widget(CookieTable.CookiesTable.CookiesTable, {
        ...
        <div
          class="request-cookies-title ${input.malformedResponseCookies.length ? '' : 'hidden'}"
          title=${i18nString(UIStrings.cookiesThatWereReceivedFromTheServer)}
        >
          ${i18nString(UIStrings.malformedResponseCookies)}
        </div>

        <div class=${input.malformedResponseCookies.length ? '' : 'hidden'}>
          ${input.malformedResponseCookies.map(
            (malformedCookie) => html`
              <span
                class="cookie-line source-code"
                title=${getMalformedCookieTooltip(malformedCookie)}
              >
                <devtools-icon
                  class="cookie-warning-icon small"
...
function getMalformedCookieTooltip(malformedCookie) {
  if (
    malformedCookie.blockedReasons.includes(
  ...
  return SDK.NetworkRequest.setCookieBlockedReasonToUiString(
    'SyntaxError' /* Protocol.Network.SetCookieBlockedReason.SyntaxError */,
  );
}
export class RequestCookiesView extends UI.Widget.Widget {
  ...
  getRequestCookies() {
    ...
    const requestCookies = this.request
      .includedRequestCookies()
      .map((includedRequestCookie) => includedRequestCookie.cookie);
    ...
    for (const includedCookie of this.request.includedRequestCookies()) {
      if (includedCookie.exemptionReason) {
        requestCookieToExemptionReason.set(includedCookie.cookie, {
          uiString: SDK.NetworkRequest.cookieExemptionReasonToUiString(
            includedCookie.exemptionReason,
          ),
        });
      }
    }
  ...
  getResponseCookies() {
    ...
    if (this.request.responseCookies.length) {
      ...
      for (const blockedCookie of this.request.blockedResponseCookies()) {
        const parsedCookies = SDK.CookieParser.CookieParser.parseSetCookie(
          blockedCookie.cookieLine,
        );
        if (
          (parsedCookies && !parsedCookies.length) ||
      ...
      for (const exemptedCookie of this.request.exemptedResponseCookies()) {
        ...
        const matchedResponseCookie = responseCookies.find(
          (responseCookie) => exemptedCookie.cookieLine === responseCookie.getCookieLine(),
        );
    ...
    return {
      responseCookies,
      responseCookieToBlockedReasons,
      responseCookieToExemptionReason,
  ...
  performUpdate() {
    ...
    const { requestCookies, requestCookieToBlockedReasons, requestCookieToExemptionReason } =
