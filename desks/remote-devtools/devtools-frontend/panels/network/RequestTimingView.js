...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  const addServerTiming = (serverTiming) => {
    ...
    const left =
      serverTiming.value === null
        ? -1
        : scale * (input.endTime - input.startTime - serverTiming.value / 1000);
    const lastRange = input.timeRanges.findLast(
      (range) => range.name !== 'total' /* NetworkTimeCalculator.RequestTimeRangeNames.TOTAL */,
    );
    ...
    return html` <tr class=${classes}>
      <td title=${metricDesc} class="network-timing-metric">${metricDesc}</td>
      ${serverTiming.value === null
        ? nothing
        : html`
              ...
              <div class="network-timing-row">
                ${left < 0 // don't chart values too big or too small
                  ...
                  : html`<span
                      ...
                      data-background=${ifDefined(
                        isTotal ? undefined : colorGenerator.colorForID(serverTiming.metric),
                      )}
                      data-left=${left}
                      data-right=${lastTimingRightEdge}
                      >${'\u200B'}</span
                    >`}
              </div>
            </td>
  ...
  const throttledRequestTitle = input.wasThrottled
    ? i18nString(UIStrings.wasThrottled, {
        PH1:
          typeof input.wasThrottled.conditions.title === 'string'
    ...
    html`<style>
      ...
      >
        ...
        <thead class="network-timing-start">
          ...
          <tr>
            <td colspan="3">
              ${i18nString(UIStrings.queuedAtS, {
                PH1: input.calculator.formatValue(input.requestIssueTime, 2),
              })}
            </td>
          </tr>
          <tr>
            <td colspan="3">
              ${i18nString(UIStrings.startedAtS, {
                PH1: input.calculator.formatValue(input.requestStartTime, 2),
              })}
            </td>
          </tr>
        </thead>
        ${timeRangeGroups.map(
          (group) => html`
            <tr class="network-timing-table-header">
              <td role="heading" aria-level="2">${group.name}</td>
              <td></td>
              ...
              (range) => html`
                <tr>
                  ${isClickable(range)
                    ...
                    : html`<td>${timeRangeTitle(range.name)}</td>`}
                  <td>
                    <div
                      class="network-timing-row"
                      aria-label=${i18nString(UIStrings.startedAtS, {
                        PH1: input.calculator.formatValue(range.start, 2),
                      })}
                    >
                      <span
                        class="network-timing-bar ${range.name}"
                        data-left=${scale * (range.start - input.startTime)}
                        data-right=${scale * (input.endTime - range.end)}
                        >${'\u200B'}</span
                      >
                    </div>
                  </td>
                ...
                ${range.name === 'serviceworker-respondwith' && input.fetchDetails
                  ? html` <tr
                      class="network-fetch-timing-bar-details network-fetch-timing-bar-details-collapsed"
                    >
                      ${input.fetchDetails.element}
                    </tr>`
                  : nothing}
                ${range.name === 'serviceworker-routerevaluation' && input.routerDetails
                  ? html` <tr
                      class="router-evaluation-timing-bar-details network-fetch-timing-bar-details-collapsed"
                    >
        ...
        <tr class="network-timing-footer">
          ...
          <td></td>
          <td
            class=${input.wasThrottled ? 'throttled' : ''}
            title=${ifDefined(throttledRequestTitle)}
          >
            ${input.wasThrottled
              ? html` <devtools-icon name="watch" @click=${revealThrottled}></devtools-icon>`
              : nothing}
        ...
        ${repeat(
          input.serverTimings.filter((item) => item.metric.toLowerCase() !== 'total'),
          addServerTiming,
        )}
        ${repeat(
          input.serverTimings.filter((item) => item.metric.toLowerCase() === 'total'),
        ...
        ${input.serverTimings.length === 0
          ? html` <tr>
              <td colspan="3">
                ${uiI18n.getFormatLocalizedStringTemplate(
                  str_,
                  UIStrings.duringDevelopmentYouCanUseSToAdd,
                  {
                    PH1: html`<devtools-link
                      href="https://web.dev/custom-metrics/#server-timing-api"
                      .jslogContext=${'server-timing-api'}
                      >${i18nString(UIStrings.theServerTimingApi)}</devtools-link
                    >`,
                  },
                )}
              </td>
...
export class RequestTimingView extends UI.Widget.VBox {
  ...
  performUpdate() {
    ...
    const timeRanges = NetworkTimeCalculator.calculateRequestTimeRanges(
      this.#request,
  ...
  #fetchDetailsTree() {
    ...
    const origRequest = Logs.NetworkLog.NetworkLog.instance().originalRequestForURL(
      this.#request.url(),
    );
    if (origRequest) {
    ...
    const response = Logs.NetworkLog.NetworkLog.instance().originalResponseForURL(
      this.#request.url(),
    );
    if (response) {
  ...
  #routerDetailsTree() {
    ...
    matchedSourceTypeElement.textContent = i18nString(UIStrings.routerMatchedSource, {
      PH1: matchedSourceTypeString,
    });
    ...
    actualSourceTypeElement.textContent = i18nString(UIStrings.routerActualSource, {
      PH1: actualSourceTypeString,
    });
  ...
  set request(request) {
    ...
    if (this.isShowing()) {
      this.#request.addEventListener(
        SDK.NetworkRequest.Events.TIMING_CHANGED,
        this.requestUpdate,
        this,
      );
      this.#request.addEventListener(
  ...
  set calculator(calculator) {
    ...
    if (this.isShowing()) {
      this.#calculator.addEventListener(
        'BoundariesChanged' /* NetworkTimeCalculator.Events.BOUNDARIES_CHANGED */,
