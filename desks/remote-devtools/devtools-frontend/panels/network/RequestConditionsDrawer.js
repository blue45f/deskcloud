  ...
  render(
    ...
    html`
      <devtools-toolbar jslog=${VisualLogging.toolbar()}>
        ...
        <devtools-button
          ${bindToAction('network.add-network-request-blocking-pattern')}
        ></devtools-button>
        <devtools-button
          ${bindToAction('network.remove-all-network-request-blocking-patterns')}
        ></devtools-button>
      </devtools-toolbar>
      <div class="empty-state" ${ref((e) => input.list.setEmptyPlaceholder(e ?? null))}>
        ...
        <div class="empty-state-description">
          ${uiI18n.getFormatLocalizedStringTemplate(str_, UIStrings.noThrottlingOrBlockingPattern, {
            PH1: learnMore(),
          })}
        </div>
        <devtools-button
      ...
      <devtools-widget ${widget(UI.Widget.VBox)}> ${input.list.element} </devtools-widget>
    `,
    // clang-format on
    target,
  );
...
function renderItem(
  ...
  const { enabled, originalOrUpgradedURLPattern, constructorStringOrWildcardURL, wildcardURL } =
    condition;
  const toggle = (e) => {
    e.consume(true);
    onToggle(condition);
  ...
  // clang-format off
  return html` <input
      class="blocked-url-checkbox"
      @change=${toggle}
      type="checkbox"
      title=${i18nString(UIStrings.enableThrottlingToggleLabel, {
        PH1: constructorStringOrWildcardURL,
      })}
    ...
    <devtools-button
      ...
      @click=${moveDown}
    ></devtools-button>
    ${originalOrUpgradedURLPattern
      ? html` <devtools-tooltip variant="rich" jslogcontext="url-pattern" id="url-pattern-${index}">
    ...
    ${wildcardURL
      ? html`
          <devtools-icon
            name="warning-filled"
            class="small warning"
            aria-details="url-pattern-warning-${index}"
          >
          </devtools-icon>
          <devtools-tooltip
            variant="rich"
            jslogcontext="url-pattern-warning"
            id="url-pattern-warning-${index}"
          >
    ...
    ${!originalOrUpgradedURLPattern
      ? html` <devtools-icon
            ...
            class="small"
            aria-details="url-pattern-error-${index}"
          >
          </devtools-icon>
          <devtools-tooltip
            variant="rich"
            jslogcontext="url-pattern-warning"
            id="url-pattern-error-${index}"
          >
            ${SDK.NetworkManager.RequestURLPattern.isValidPattern(
    ...
    <devtools-widget
      ?disabled=${!editable || !originalOrUpgradedURLPattern}
      ${widget(AffectedCountWidget, { condition, lookUpRequestCount })}
    ></devtools-widget>`;
  // clang-format on
}
...
export class AffectedCountWidget extends UI.Widget.Widget {
  ...
  willHide() {
    ...
    Logs.NetworkLog.NetworkLog.instance().removeEventListener(
      Logs.NetworkLog.Events.Reset,
      this.requestUpdate,
      this,
    );
  ...
  #onRequestFinished(event) {
    ...
    if (
      (request.appliedNetworkConditionsId &&
        this.#condition.ruleIds.has(request.appliedNetworkConditionsId)) ||
...
export class RequestConditionsDrawer extends UI.Widget.VBox {
  ...
  constructor(target, view = DEFAULT_VIEW) {
    ...
    this.manager.addEventListener(
      'BlockedPatternsChanged' /* SDK.NetworkManager.MultitargetNetworkManager.Events.BLOCKED_PATTERNS_CHANGED */,
      this.update,
      this,
    );
  ...
  performUpdate() {
    ...
    this.list.element.classList.toggle(
      'blocking-disabled',
      !enabled && Boolean(this.manager.requestConditions.count),
    );
    const input = {
    ...
    const validator = (_item, _index, input) => {
      ...
      switch (isValid) {
        ...
        case 'has-regexp-groups' /* SDK.NetworkManager.RequestURLPatternValidity.HAS_REGEXP_GROUPS */:
          return {
            errorMessage: i18nString(UIStrings.patternFailedWithRegExpGroups),
            valid: false,
          };
      }
      return { valid: true };
  ...
  update() {
    ...
    for (; oldIndex < newItems.length; ++oldIndex) {
      ...
      this.list.updateItem(oldIndex, pattern, enabled, /* focusable=*/ false, {
        edit: i18nString(UIStrings.editPattern, { PH1: pattern.constructorStringOrWildcardURL }),
        delete: i18nString(UIStrings.removePattern, {
          PH1: pattern.constructorStringOrWildcardURL,
        }),
      });
    }
  ...
  static async reveal(appliedConditions) {
    ...
    const conditions = drawer.manager.requestConditions.conditions.find(
      (condition) =>
        condition.ruleIds.has(appliedConditions.appliedNetworkConditionsId) &&
        condition.constructorString &&
        condition.constructorString === appliedConditions.urlPattern,
    );
