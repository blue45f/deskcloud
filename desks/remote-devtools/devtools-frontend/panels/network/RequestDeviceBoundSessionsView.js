...
const str_ = i18n.i18n.registerUIStrings(
  'panels/network/RequestDeviceBoundSessionsView.ts',
  UIStrings,
);
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html`
      ...
      <style>
        ${requestDeviceBoundSessionsViewStyles}
      </style>
      <div class="request-device-bound-sessions-view">
        ${deviceBoundSessionUsages.length > 0
          ? html`
                  ...
                  <table>
                    <thead>
                      <tr>
                        ...
                        <th id="deferral-decision" sortable>
                          ${i18nString(UIStrings.deferralDecision)}
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      ${deviceBoundSessionUsages.map(
                        (session) => html`
                          <tr>
...
export class RequestDeviceBoundSessionsView extends UI.Widget.VBox {
  ...
  performUpdate() {
    ...
    this.#view(
      { deviceBoundSessionUsages: this.#request.getDeviceBoundSessionUsages() },
      {},
      this.contentElement,
    );
  }
