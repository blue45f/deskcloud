...
export class SignedExchangeInfoView extends UI.Widget.VBox {
  ...
  constructor(request) {
    ...
    titleElement.createChild('div', 'header-name').textContent = i18nString(
      UIStrings.signedHttpExchange,
    );
    ...
    if (signedExchangeInfo.header) {
      ...
      const requestURLElement = this.formatHeader(
      ...
      headerCategory.createLeaf(
        this.formatHeader(i18nString(UIStrings.responseCode), String(header.responseCode)),
      );
      headerCategory.createLeaf(
        this.formatHeader(i18nString(UIStrings.headerIntegrityHash), header.headerIntegrity),
      );
      this.responseHeadersItem = headerCategory.createLeaf(
        this.formatHeader(i18nString(UIStrings.responseHeaders), ''),
      );
      const responseHeaders = header.responseHeaders;
      for (const name in responseHeaders) {
        const headerTreeElement = new UI.TreeOutline.TreeElement(
          this.formatHeader(name, responseHeaders[name]),
        );
      ...
      for (let i = 0; i < header.signatures.length; ++i) {
        ...
        signatureCategory.createLeaf(
          this.formatHeader(i18nString(UIStrings.label), signature.label),
        );
        signatureCategory.createLeaf(
          this.formatHeaderForHexData(
            ...
            signature.signature,
            errorFieldSet.has(
              'signatureSig' /* Protocol.Network.SignedExchangeErrorField.SignatureSig */,
            ),
          ),
        );
        if (signature.certUrl) {
          const certURLElement = this.formatHeader(
            ...
            signature.certUrl,
            errorFieldSet.has(
              'signatureCertUrl' /* Protocol.Network.SignedExchangeErrorField.SignatureCertUrl */,
            ),
          );
        ...
        signatureCategory.createLeaf(
          this.formatHeader(
            ...
            signature.integrity,
            errorFieldSet.has(
              'signatureIntegrity' /* Protocol.Network.SignedExchangeErrorField.SignatureIntegrity */,
            ),
          ),
        );
        if (signature.certSha256) {
          signatureCategory.createLeaf(
            this.formatHeaderForHexData(
              ...
              errorFieldSet.has(
                'signatureCertSha256' /* Protocol.Network.SignedExchangeErrorField.SignatureCertSha256 */,
              ),
            ),
          );
        }
        signatureCategory.createLeaf(
          this.formatHeader(
            ...
            errorFieldSet.has(
              'signatureValidityUrl' /* Protocol.Network.SignedExchangeErrorField.SignatureValidityUrl */,
            ),
          ),
        );
        signatureCategory.createLeaf().title = this.formatHeader(
          ...
          errorFieldSet.has(
            'signatureTimestamps' /* Protocol.Network.SignedExchangeErrorField.SignatureTimestamps */,
          ),
        );
        signatureCategory.createLeaf().title = this.formatHeader(
    ...
    if (signedExchangeInfo.securityDetails) {
      ...
      securityCategory.createLeaf(
        this.formatHeader(i18nString(UIStrings.subject), securityDetails.subjectName),
      );
      securityCategory.createLeaf(
        this.formatHeader(
          i18nString(UIStrings.validFrom),
          new Date(1000 * securityDetails.validFrom).toUTCString(),
        ),
      );
      securityCategory.createLeaf(
        this.formatHeader(
          i18nString(UIStrings.validUntil),
          new Date(1000 * securityDetails.validTo).toUTCString(),
        ),
      );
      securityCategory.createLeaf(
        this.formatHeader(i18nString(UIStrings.issuer), securityDetails.issuer),
      );
    }
  }
