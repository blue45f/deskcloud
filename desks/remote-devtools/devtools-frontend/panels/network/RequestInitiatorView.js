...
export const DEFAULT_VIEW = (input, _output, target) => {
  const hasInitiatorData =
    input.initiatorGraph.initiators.size > 1 ||
    input.initiatorGraph.initiated.size > 1 ||
    input.stackTrace;
  if (!hasInitiatorData) {
    render(
      html`
        <div
          class="empty-view"
          style="display: flex; justify-content: center; align-items: center; height: 100%; color: var(--sys-color-token-subtle);"
        >
  ...
  const renderInitiatorNodes = (initiators, index, initiated, visited) => {
    ...
    const renderedChildren = isCurrentRequest
      ? renderInitiatedNodes(initiated, request, visited)
      : nothing;
    // clang-format off
    return html` <li role="treeitem" ?selected=${isCurrentRequest} aria-expanded="true" open>
      <span style=${isCurrentRequest ? 'font-weight: bold' : ''}>${request.url()}</span>
      ${hasFurtherInitiatedNodes || renderedChildren !== nothing
        ? html` <ul role="group">
  ...
  const renderInitiatedNodes = (initiated, parentRequest, visited) => {
    ...
    return html`
      ${children.map((child) => {
        ...
        const renderedChildren = shouldRecurse
          ? renderInitiatedNodes(initiated, child, visited)
          : nothing;
        return html`
  ...
  const renderInitiatorChain = (initiatorGraph) => {
    ...
    return html` <li
      role="treeitem"
      class="request-initiator-view-section-title"
      aria-expanded="true"
      open
    >
      ...
      <div class="request-initiator-view-tree" jslog=${VisualLogging.tree('initiator-tree')}>
        <devtools-tree
          .template=${html`
            ...
            ${input.stackTrace || hasInitiatorChain
              ? html` <ul role="tree">
...
export class RequestInitiatorView extends UI.Widget.VBox {
  ...
  static async createStackTracePreview(request, linkifier, focusableLink) {
    ...
    const target =
      networkManager?.target() ?? targetManager.primaryPageTarget() ?? targetManager.rootTarget();
    let stackTrace = null;
