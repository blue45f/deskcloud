...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  if (TextUtils.StreamingContentData.isError(input.contentData)) {
    ...
    // clang-format off
    widgetTemplate = html`<devtools-widget
      ${widget(
        (element) =>
          new SourceFrame.ResourceSourceFrame.SearchableContainer(
            input.request,
            input.mimeType,
            element,
          ),
      )}
      ${widgetRef(SourceFrame.ResourceSourceFrame.SearchableContainer, (widget) => {
        output.revealPosition = widget.revealPosition.bind(widget);
      })}
    ></devtools-widget>`;
    // clang-format on
  } else {
...
export class RequestResponseView extends UI.Widget.VBox {
  ...
  async performUpdate() {
    ...
    if (!TextUtils.StreamingContentData.isError(contentData)) {
      ...
      const isMinified =
        isWasm || !contentData.isTextContent
          ? false
  ...
  getMimeTypeForDisplay() {
    ...
    if (
      Common.ResourceType.ResourceType.simplifyContentType(this.request.mimeType) ===
      'application/json'
    ) {
