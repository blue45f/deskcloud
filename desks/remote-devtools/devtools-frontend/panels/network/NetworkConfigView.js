...
export class NetworkConfigView extends UI.Widget.VBox {
  ...
  static createUserAgentSelectAndInput(title) {
    const userAgentSetting = Common.Settings.Settings.instance().createSetting(
      'custom-user-agent',
      '',
    );
    ...
    userAgentSelectElement.appendChild(
      UI.UIUtils.createOption(customOverride.title, customOverride.value, 'custom'),
    );
    ...
    function userAgentSelected() {
      ...
      if (value !== customOverride.value) {
        ...
        SDK.NetworkManager.MultitargetNetworkManager.instance().setCustomUserAgentOverride(
          value,
          userAgentMetadata,
        );
      } else {
  ...
  createUserAgentSection() {
    ...
    const customUserAgentSetting = Common.Settings.Settings.instance().createSetting(
      'custom-user-agent',
      '',
    );
  ...
  createAcceptedEncodingSection() {
    const useCustomAcceptedEncodingSetting = Common.Settings.Settings.instance().createSetting(
    ...
    const customAcceptedEncodingSetting = Common.Settings.Settings.instance().createSetting(
      ...
      `${'gzip' /* Protocol.Network.ContentEncoding.Gzip */},${'br' /* Protocol.Network.ContentEncoding.Br */},${'deflate' /* Protocol.Network.ContentEncoding.Deflate */}`,
    );
    ...
    encodingsSection.setAttribute(
      'jslog',
      `${VisualLogging.section().context(customAcceptedEncodingSetting.name)}`,
    );
