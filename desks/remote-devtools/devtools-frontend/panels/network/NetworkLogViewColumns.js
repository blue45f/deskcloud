...
export class NetworkLogViewColumns {
  ...
  constructor(networkLogView, timeCalculator, durationCalculator, networkLogLargeRowsSetting) {
    ...
    this.persistentSettings = Common.Settings.Settings.instance().createSetting(
      'network-log-columns',
      {},
    );
  ...
  static convertToDataGridDescriptor(columnConfig) {
    const title =
      columnConfig.title instanceof Function ? columnConfig.title() : columnConfig.title;
    return {
      id: columnConfig.id,
      title: title,
    ...
    for (const currentConfigColumn of defaultColumns) {
      ...
      if (columnConfig.subtitle) {
        const title =
          columnConfig.title instanceof Function ? columnConfig.title() : columnConfig.title;
        const subtitle =
          columnConfig.subtitle instanceof Function
  ...
  setupWaterfall() {
    ...
    this.#dataGrid.addEventListener(
      'PaddingChanged' /* DataGrid.DataGrid.Events.PADDING_CHANGED */,
      () => {
        this.waterfallScrollerWidthIsStale = true;
        this.syncScrollers();
      },
    ...
    this.#dataGrid.addEventListener(
      'ViewportCalculated' /* DataGrid.ViewportDataGrid.Events.VIEWPORT_CALCULATED */,
  ...
  redrawWaterfallColumn() {
    if (!this.waterfallRequestsAreStale) {
      ...
      this.waterfallColumn.update(
        ...
        this.eventDividersShown ? this.eventDividers : undefined,
      );
      return;
    }
    this.syncScrollers();
  ...
  createWaterfallHeader() {
    ...
    function waterfallHeaderClicked() {
      ...
      const sortOrder =
        wasSortedByWaterfall && wasSortedAscending ? sortOrders.Descending : sortOrders.Ascending;
  ...
  sortHandler() {
    ...
    if (columnId === 'waterfall') {
      ...
      const sortFunction = NetworkRequestNode.RequestPropertyComparator.bind(
        null,
        this.activeWaterfallSortId,
      );
  ...
  #headerContextMenu(contextMenu) {
    ...
    const nonRequestResponseHeaders = columnConfigs.filter(
      (columnConfig) => !columnConfig.isRequestHeader && !columnConfig.isResponseHeader,
    );
    ...
    for (const group of hideableGroups.values()) {
      ...
      for (const columnConfig of group) {
        ...
        contextMenu
          .headerSection()
          .appendCheckboxItem(title, this.toggleColumnVisibility.bind(this, columnConfig), {
    ...
    const responseSubMenu = contextMenu
      .footerSection()
      .appendSubMenuItem(i18nString(UIStrings.responseHeaders), false, 'response-headers');
    ...
    const requestSubMenu = contextMenu
      .footerSection()
      .appendSubMenuItem(i18nString(UIStrings.requestHeaders), false, 'request-headers');
    ...
    const waterfallSubMenu = contextMenu
      .footerSection()
      .appendSubMenuItem(i18nString(UIStrings.waterfall), false, 'waterfall');
    waterfallSubMenu
      .defaultSection()
      .appendCheckboxItem(
  ...
  manageRequestCustomHeaderDialog() {
    ...
    const manageCustomHeadersRequest = new NetworkManageCustomHeadersView(
      ...
      (oldHeaderId, headerTitle) =>
        Boolean(
          this.changeCustomHeader(
            `request-header-${oldHeaderId}`,
            headerTitle,
  ...
  manageResponseCustomHeaderDialog() {
    ...
    dialogResponse.setSizeBehavior(
      'MeasureContent' /* UI.GlassPane.SizeBehavior.MEASURE_CONTENT */,
    );
  ...
  addCustomHeader(headerTitle, headerId, index) {
    ...
    const sortingFunction = isRequestHeader
      ? NetworkRequestNode.RequestHeaderStringComparator.bind(null, headerId)
    ...
    if (this.#dataGrid) {
      this.#dataGrid.addColumn(
        NetworkLogViewColumns.convertToDataGridDescriptor(columnConfig),
        index,
      );
    }
    this.saveColumnsSettings();
  ...
  changeCustomHeader(oldHeaderId, newHeaderTitle, newHeaderId) {
    ...
    const currentColumnConfig = this.columns.find(
      (columnConfig) => columnConfig.id === newHeaderId,
    );
  ...
  getPopoverRequest(event) {
    ...
    return {
      ...
      show: async (popover) => {
        ...
        descriptor = content.stackTrace?.addEventListener(
          'UPDATED' /* StackTrace.StackTrace.Events.UPDATED */,
          async () => {
...
const DEFAULT_COLUMNS = [
  ...
  },
  {
    ...
    sortingFunction: NetworkRequestNode.ResponseHeaderStringComparator.bind(
      null,
      'content-encoding',
    ),
  },
