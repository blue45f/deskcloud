...
const str_ = i18n.i18n.registerUIStrings(
  'panels/network/NetworkManageCustomHeadersView.ts',
  UIStrings,
);
...
export class NetworkManageCustomHeadersView extends UI.Widget.VBox {
  ...
  constructor(
    ...
    this.contentElement.createChild('div', 'header').textContent = i18nString(
      UIStrings.manageHeaderColumns,
    );
    ...
    this.contentElement.appendChild(
      UI.UIUtils.createTextButton(
        i18nString(UIStrings.addCustomHeader),
    ...
    columnData.forEach((columnData) =>
      this.columnConfigs.set(columnData.title.toLowerCase(), columnData),
    );
  ...
  headersUpdated() {
    this.list.clear();
    this.columnConfigs.forEach((headerData) =>
      this.list.appendItem({ header: headerData.title }, headerData.editable),
    );
  }
  addButtonClicked() {
  ...
  createEditor() {
    ...
    titles.createChild('div', 'custom-headers-header').textContent = i18nString(
      UIStrings.headerName,
    );
