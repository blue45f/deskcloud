...
UI.ActionRegistration.registerActionExtension({
  ...
  contextTypes() {
    return maybeRetrieveContextTypes((Network) => [
      Network.RequestConditionsDrawer.RequestConditionsDrawer,
    ]);
  },
  async loadActionDelegate() {
    const Network = await loadNetworkModule();
...
Common.Settings.registerSettingExtension({
  ...
  defaultValue: false,
  tags: [i18n.i18n.lockedLazyString('HAR')],
  options: [
    {
      value: true,
      title: i18nLazyString(UIStrings.allowToGenerateHarWithSensitiveData),
...
Common.Settings.registerSettingExtension({
  ...
  settingType: 'boolean' /* Common.Settings.SettingType.BOOLEAN */,
  defaultValue: false,
  tags: [i18nLazyString(UIStrings.colorCode), i18nLazyString(UIStrings.resourceType)],
  options: [
    {
      value: true,
...
Common.Revealer.registerRevealer({
  contextTypes() {
    return [
      ...
      PanelCommon.ExtensionServer.RevealableNetworkRequestFilter,
    ];
  },
  destination: Common.Revealer.RevealerDestination.NETWORK_PANEL,
  async loadRevealer() {
