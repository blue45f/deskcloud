...
export class NetworkSearchScope {
  ...
  async performSearch(searchConfig, progress, searchResultCallback, searchFinishedCallback) {
    const promises = [];
    const requests = this.#networkLog
      .requests()
  ...
  async searchRequest(searchConfig, request, progress) {
    ...
    for (const header of request.requestHeaders()) {
      if (headerMatchesQuery(header)) {
        locations.push(
          NetworkForward.UIRequestLocation.UIRequestLocation.requestHeaderMatch(request, header),
        );
      }
    }
    for (const header of request.responseHeaders) {
      if (headerMatchesQuery(header)) {
        locations.push(
          NetworkForward.UIRequestLocation.UIRequestLocation.responseHeaderMatch(request, header),
        );
      }
    }
    ...
    function stringMatchesQuery(string) {
      ...
      const regExps = searchConfig
        .queries()
        .map((query) => new RegExp(Platform.StringUtilities.escapeForRegExp(query), flags));
      let pos = 0;
  ...
  static async #responseBodyMatches(searchConfig, request) {
    ...
    for (const query of searchConfig.queries()) {
      const tmpMatches = await request.searchInContent(
        query,
