...
const str_ = i18n.i18n.registerUIStrings(
  'panels/network/components/HeaderSectionRow.ts',
  UIStrings,
);
...
export class HeaderSectionRow extends HTMLElement {
  ...
  #render() {
    ...
    const isHeaderNameEditable =
      ...
      this.#header.valueEditable === 1; /* EditingAllowedStatus.ENABLED */
    ...
    const showReloadInfoIcon =
      this.#header.nameEditable || this.#header.isDeleted || this.#isHeaderValueEdited;
    // Disabled until https://crbug.com/1079231 is fixed.
    // clang-format off
    render(
      html`
        <style>
          ${headerSectionRowStyles}
        </style>
        <div class=${rowClasses}>
          <div class=${headerNameClasses}>
            ${this.#header.headerNotSet
              ? html`<div class="header-badge header-badge-text">
                  ${i18n.i18n.lockedString('not-set')}
                </div> `
              : Lit.nothing}
            ${isHeaderNameEditable && !this.#isValidHeaderName
              ? html`<devtools-icon
                  class="inline-icon disallowed-characters medium"
                  title=${UIStrings.headerNamesOnlyLetters}
          ...
          ${showReloadInfoIcon
            ? html`<devtools-icon
                name="info"
                class="row-flex-icon flex-right medium"
                title=${UIStrings.reloadPrompt}
              >
              </devtools-icon>`
            : Lit.nothing}
        </div>
  ...
  #renderHeaderValue() {
    ...
    if (
      this.#header.isDeleted ||
      this.#header.valueEditable !== 1 /* EditingAllowedStatus.ENABLED */
    ) {
      const showEditHeaderButton =
        this.#header.isResponseHeader &&
        !this.#header.isDeleted &&
        this.#header.valueEditable !== 2; /* EditingAllowedStatus.FORBIDDEN */
      // clang-format off
      return html`
        ${this.#header.value || ''} ${this.#maybeRenderHeaderValueSuffix(this.#header)}
          ...
          ${blockedDetails.examples.map(
            (example) =>
              html` <div class="example">
                <code>${example.codeSnippet}</code> ${example.comment
                  ? html`<span class="comment"> ${example.comment()}</span>`
                  : ''}
              </div>`,
          )}
    ...
    if (blockedDetails?.reveal) {
      ...
      return html`
        <div class="devtools-link" @click=${blockedDetails.reveal}>
          <devtools-icon name="issue-exclamation-filled" class="inline-icon medium"> </devtools-icon
    ...
    if (event.key === 'Escape') {
      ...
      if (target.matches('.header-name devtools-editable-span')) {
        ...
        if (this.#header?.originalName) {
          const headerNameElement = this.#shadow.querySelector(
  ...
  #onHeaderValueEdit(event) {
    const editable = event.target;
    const isEdited =
      this.#header?.originalValue !== undefined &&
      !compareHeaders(this.#header?.originalValue || '', editable.value);
