...
const str_ = i18n.i18n.registerUIStrings(
  'panels/network/components/ResponseHeaderSection.ts',
  UIStrings,
);
...
class ResponseHeaderSectionBase extends HTMLElement {
  ...
  highlightHeaders(data) {
    if (
      data.toReveal?.section ===
      'Response' /* NetworkForward.UIRequestLocation.UIHeaderSection.RESPONSE */
    ) {
      this.headerDetails
        .filter((header) => compareHeaders(header.name, data.toReveal?.header?.toLowerCase()))
        .forEach((header) => {
...
export class ResponseHeaderSection extends ResponseHeaderSectionBase {
  ...
  set data(data) {
        ...
        if (
          ...
          const followLink = () => {
            Host.userMetrics.issuesPanelOpenedFrom(
              1 /* Host.UserMetrics.IssueOpener.LEARN_MORE_LINK_COEP */,
            );
            if (this.#request) {
    ...
    this.highlightHeaders(data);
    const dataAssociatedWithRequest = this.#request.getAssociatedData(
  ...
  async #loadOverridesFileInfo() {
    ...
    try {
      const contentData = await this.#uiSourceCode
        .requestContentData()
  ...
  #markOverrides() {
    ...
    while (indexActual < this.headerDetails.length) {
      ...
      while (
        indexActual < this.headerDetails.length - 1 &&
      ...
      while (
        indexOriginal < originalHeaders.length &&
        originalHeaders[indexOriginal].name < currentName
      ) {
        indexOriginal++;
      }
      if (
        ...
        while (
          indexOriginal < originalHeaders.length - 1 &&
          originalHeaders[indexOriginal + 1].name === currentName
        ) {
          indexOriginal++;
        ...
        indexOriginal++;
        if (
          currentName !== 'set-cookie' &&
          !headerNotSet &&
          !compareHeaders(actualValue, originalValue)
        ) {
          this.#headerEditors
            .filter((header) => compareHeaders(header.name, currentName))
            .forEach((header) => {
    ...
    this.#headerEditors
      ...
      .forEach((header) => {
        if (
          this.#request?.originalResponseHeaders.find(
            (originalHeader) =>
              Platform.StringUtilities.toLowerCaseString(originalHeader.name) === 'set-cookie' &&
              compareHeaders(originalHeader.value, header.value),
          ) === undefined
        ) {
          header.isOverride = true;
        }
  ...
  #fileNameFromUrl(url) {
    const rawPath =
      Persistence.NetworkPersistenceManager.NetworkPersistenceManager.instance().rawPathFromUrl(
        url,
        true,
      );
  ...
  #removeEntryFromOverrides(rawFileName, headerName, headerValue) {
    for (let blockIndex = this.#overrides.length - 1; blockIndex >= 0; blockIndex--) {
      ...
      const foundIndex = block.headers.findIndex(
        (header) =>
          compareHeaders(header.name, headerName) && compareHeaders(header.value, headerValue),
      );
      if (foundIndex < 0) {
        continue;
      }
  ...
  #updateOverrides(headerName, headerValue, index) {
    ...
    if (this.#request.originalResponseHeaders.length === 0) {
      this.#request.originalResponseHeaders = this.#request.sortedResponseHeaders.map(
    ...
    if (headerName === 'set-cookie') {
      ...
      headersToUpdate = this.#headerEditors.filter(
        (header) =>
          compareHeaders(header.name, headerName) &&
    ...
    if (headerName === 'set-cookie') {
      ...
      const foundIndex = block.headers.findIndex(
        (header) =>
          compareHeaders(header.name, previousName) && compareHeaders(header.value, previousValue),
      );
      if (foundIndex >= 0) {
        block.headers.splice(foundIndex, 1);
      }
    ...
    if (!compareHeaders(this.#headerEditors[index].name, previousName)) {
      for (let i = 0; i < block.headers.length; ++i) {
        if (
          compareHeaders(block.headers[i].name, previousName) &&
  ...
  #onAddHeaderClick() {
    ...
    this.#updateOverrides(
      this.#headerEditors[index].name,
      this.#headerEditors[index].value || '',
      index,
    );
    this.#render();
