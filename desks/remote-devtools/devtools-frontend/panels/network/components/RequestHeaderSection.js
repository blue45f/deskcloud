...
const str_ = i18n.i18n.registerUIStrings(
  'panels/network/components/RequestHeaderSection.ts',
  UIStrings,
);
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  render(
    html`
      <style>
        ${requestHeaderSectionStyles}
      </style>
      ${input.isProvisionalHeaders
        ? renderProvisionalHeadersWarning(input.isRequestCached)
        : Lit.nothing}
      ${headers.map(
        (header) => html`
    ...
    <div class="call-to-action">
      <div class="call-to-action-body">
        <div class="explanation" title=${cautionTitle}>
          ...
          ${cautionText}
          <devtools-link
            href="https://developer.chrome.com/docs/devtools/network/reference/#provisional-headers"
            class="link"
...
export class RequestHeaderSection extends UI.Widget.Widget {
  ...
  set toReveal(toReveal) {
    ...
    if (
      toReveal.section === 'Request' /* NetworkForward.UIRequestLocation.UIHeaderSection.REQUEST */
    ) {
      this.#headers
        .filter((header) => header.name === toReveal.header?.toLowerCase())
        .forEach((header) => {
