...
const str_ = i18n.i18n.registerUIStrings(
  'panels/network/components/RequestTrustTokensView.ts',
  UIStrings,
);
...
const renderResultSection = (status, description, issuedTokenCount) => {
  ...
  return html`
    ...
    <devtools-report-value>
      <span>
        <devtools-icon
          class="status-icon medium ${status === 'Success' ? 'success' : 'failure'}"
        ...
        <strong
          >${status === 'Success'
            ...
            : i18nString(UIStrings.failure)}</strong
        >
...
const renderParameterSection = (params) => {
  ...
  return html`
    <devtools-report-section-header
      jslog=${VisualLogging.pane('trust-tokens').track({ resize: true })}
    >
...
export class RequestTrustTokensView extends UI.Widget.Widget {
  ...
  performUpdate() {
    ...
    if (trustTokenParams) {
      viewInput.params = [
        {
          name: i18nString(UIStrings.type),
          value: trustTokenParams.operation.toString(),
          isCode: true,
        },
      ];
      if (
        trustTokenParams.operation ===
        'Redemption' /* Protocol.Network.TrustTokenOperationType.Redemption */
      ) {
      ...
      if (trustTokenParams.issuers && trustTokenParams.issuers.length > 0) {
        viewInput.params.push({
          name: i18nString(UIStrings.issuers),
          value: trustTokenParams.issuers,
        });
      }
      if (trustTokenResult?.topLevelOrigin) {
        viewInput.params.push({
          name: i18nString(UIStrings.topLevelOrigin),
          value: trustTokenResult.topLevelOrigin,
        });
      }
      if (trustTokenResult?.issuerOrigin) {
        viewInput.params.push({
          name: i18nString(UIStrings.issuer),
          value: trustTokenResult.issuerOrigin,
        });
      }
    }
    if (trustTokenResult) {
      ...
      viewInput.issuedTokenCount =
        trustTokenResult.type === 'Issuance' /* Protocol.Network.TrustTokenOperationType.Issuance */
...
export function statusConsideredSuccess(status) {
  return (
    ...
    status ===
      'AlreadyExists' /* Protocol.Network.TrustTokenOperationDoneEventStatus.AlreadyExists */ ||
    status ===
      'FulfilledLocally' /* Protocol.Network.TrustTokenOperationDoneEventStatus.FulfilledLocally */
  );
}
