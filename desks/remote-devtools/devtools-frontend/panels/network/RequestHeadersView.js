...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  const statusText = [
    input.request.statusCode,
    input.request.getInferredStatusText(),
    comment,
  ].join(' ');
  // clang-format off
  render(
    html`
      <style>
        ${NetworkComponents.RequestHeaderSection.requestHeadersViewStyles}
      </style>
      <style>
      ...
      ${renderCategory({
        ...
        forceOpen:
          input.toReveal?.section ===
          'General' /* NetworkForward.UIRequestLocation.UIHeaderSection.GENERAL */,
        loggingContext: 'general',
        contents: html`<div jslog=${VisualLogging.section('general')}>
          ${renderGeneralRow(
            input,
            i18nString(UIStrings.requestUrl),
            input.request.url(),
            'request-url',
          )}
          ${input.request.statusCode
      ...
      ${!input.request?.earlyHintsHeaders || input.request.earlyHintsHeaders.length === 0
        ...
        : renderCategory({
            ...
            forceOpen:
              input.toReveal?.section ===
              'EarlyHints' /* NetworkForward.UIRequestLocation.UIHeaderSection.EARLY_HINTS */,
        ...
        contents: input.showResponseHeadersText
          ...
          : html`
              <devtools-response-header-section
                ...
                jslog=${VisualLogging.section('response-headers')}
              ></devtools-response-header-section>
            `,
      })}
      ${renderCategory({
        ...
        forceOpen:
          input.toReveal?.section ===
          'Request' /* NetworkForward.UIRequestLocation.UIHeaderSection.REQUEST */,
        loggingContext: 'request-headers',
        contents:
          input.showRequestHeadersText && requestHeadersText
            ? renderRawHeaders(requestHeadersText)
            : html` <devtools-widget
                ${widget(NetworkComponents.RequestHeaderSection.RequestHeaderSection, {
                  request: input.request,
                  toReveal: input.toReveal,
                })}
                jslog=${VisualLogging.section('request-headers')}
              ></devtools-widget>`,
      })}
    `,
    // clang-format on
...
export class RequestHeadersView extends UI.Widget.Widget {
  ...
  #resetAndRefreshHeadersView() {
    this.#request?.deleteAssociatedData(
      NetworkComponents.ResponseHeaderSection.RESPONSE_HEADER_SECTION_DATA_KEY,
    );
    this.requestUpdate();
  }
  #refreshHeadersView() {
  ...
  #getHeaderOverridesFileUrl() {
    ...
    const fileUrl =
      Persistence.NetworkPersistenceManager.NetworkPersistenceManager.instance().fileUrlFromNetworkUrl(
        this.#request.url(),
        /* ignoreInactive */ true,
      );
    return (
...
function renderHeaderOverridesLink(input) {
  ...
  const overridesSetting = Common.Settings.Settings.instance().moduleSetting(
    'persistence-network-overrides-enabled',
  );
  // Disabled until https://crbug.com/1079231 is fixed.
  // clang-format off
  const fileIcon = html` <devtools-icon
    name="document"
    class=${'medium' + overridesSetting.get() ? 'inline-icon dot purple' : 'inline-icon'}
  >
  </devtools-icon>`;
  // clang-format on
  // Disabled until https://crbug.com/1079231 is fixed.
  ...
  return html`
    ...
    >
      <devtools-icon name="help" class="inline-icon medium"> </devtools-icon>
    </devtools-link>
    <devtools-link
...
function renderRawHeaders(text) {
  return html`<div class="row raw-headers-row">
    <devtools-widget
      class="raw-headers"
      ${widget(ShowMoreDetailsWidget, { text })}
    ></devtools-widget>
  </div>`;
}
...
export function renderCategory(data) {
  const expandedSetting = Common.Settings.Settings.instance().createSetting(
    'request-info-' + data.name + '-category-expanded',
    true,
  );
          ...
          <div class="hide-when-closed">
            ${data.checked !== undefined
              ? html` <devtools-checkbox
                  ...
                  @change=${data.onToggleRawHeaders}
