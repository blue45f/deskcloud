...
const str_ = i18n.i18n.registerUIStrings(
  'panels/network/ResourceDirectSocketChunkView.ts',
  UIStrings,
);
...
export class ResourceDirectSocketChunkView extends ResourceChunkView {
  constructor(request) {
    super(
      ...
      i18nString(UIStrings.directSocketChunk),
      i18nString(UIStrings.filterUsingRegex),
    );
    this.element.setAttribute(
      'jslog',
      `${VisualLogging.pane('direct-socket-messages').track({ resize: true })}`,
    );
  }
  getRequestChunks() {
  ...
  createGridItem(chunk) {
    return new ResourceChunkNode(
      chunk,
      this.request.directSocketInfo?.type === SDK.NetworkRequest.DirectSocketType.UDP_BOUND,
    );
  }
  wasShown() {
    super.wasShown();
    ...
    this.request.addEventListener(
      SDK.NetworkRequest.Events.DIRECTSOCKET_CHUNK_ADDED,
...
class ResourceChunkNode extends DataGridItem {
  ...
  constructor(chunk, boundSocket) {
    ...
    let description;
    const length = i18n.ByteUtilities.bytesToString(
      Platform.StringUtilities.base64ToSize(chunk.data),
    );
  ...
  createCells(element) {
    element.classList.toggle(
      ...
      this.chunk.type === SDK.NetworkRequest.DirectSocketChunkType.SEND,
    );
    element.classList.toggle(
      'resource-chunk-view-row-receive',
      this.chunk.type === SDK.NetworkRequest.DirectSocketChunkType.RECEIVE,
    );
    ...
    if (!this.#binaryView) {
      if (this.dataText().length > 0) {
        this.#binaryView = new BinaryResourceView(
          ...
          Common.ResourceType.resourceTypes.DirectSocket,
        );
      }
    }
