...
export const DEFAULT_VIEW = (input, _output, target) => {
  const { contexts, selectedContextIndex, onContextSelectorSelectionChanged, contextRealtimeData } =
    input;
  const selectedContext = selectedContextIndex > -1 ? contexts[selectedContextIndex] : null;
  const titleForContext = (context) =>
    context.contextType + ' (' + context.contextId.substr(-6) + ')';
  const selectorTitle = i18nString(UIStrings.audioContextS, {
  ...
  render(
    html` <style>
      ...
      <div class="web-audio-content-container vbox flex-auto">
        ${!selectedContext
          ? html` <div class="web-audio-details-container vbox flex-auto">
              ...
              <div class="context-detail-container">
                <div class="context-detail-header">
                  <div class="context-detail-title">
                    ${selectedContext.contextType === 'realtime'
                      ? i18n.i18n.lockedString('AudioContext')
                ...
                ${selectedContext.contextType === 'realtime'
                  ? html` <div class="context-detail-row">
                      <div class="context-detail-row-entry">
                        ${i18nString(UIStrings.callbackBufferSize)}
                      </div>
                      <div class="context-detail-row-value">
                        ${selectedContext.callbackBufferSize} frames
                      </div>
                    </div>`
                  : ''}
                <div class="context-detail-row">
                  <div class="context-detail-row-entry">
                    ${i18nString(UIStrings.maxOutputChannels)}
                  </div>
                  <div class="context-detail-row-value">
                    ${selectedContext.maxOutputChannelCount} ch
                  </div>
                </div>
              </div>
            </div>`}
        <div class="web-audio-summary-container">
          ${contextRealtimeData
            ? html`<div class="context-summary-container">
                <span
                  >${i18nString(UIStrings.currentTime)}:
                  ${contextRealtimeData.currentTime.toFixed(3)} s</span
                >
                <span>❘</span>
                <span
                  >${i18nString(UIStrings.callbackInterval)}: μ =
                  ${(contextRealtimeData.callbackIntervalMean * 1000).toFixed(3)} ms, σ =
                  ${(Math.sqrt(contextRealtimeData.callbackIntervalVariance) * 1000).toFixed(3)}
                  ms</span
                >
                <span>❘</span>
                <span
                  >${i18nString(UIStrings.renderCapacity)}:
...
export class WebAudioView extends UI.Widget.VBox {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    super({
      jslog: `${VisualLogging.panel('web-audio').track({ resize: true })}`,
      useShadowDom: true,
    });
    this.view = view;
    this.contextSelectorItems = new UI.ListModel.ListModel();
    this.contextSelectorItems.addEventListener(
      'ItemsReplaced' /* UI.ListModel.Events.ITEMS_REPLACED */,
      this.requestUpdate,
      this,
    );
  ...
  addEventListeners(webAudioModel) {
    ...
    webAudioModel.addEventListener(
      'ContextCreated' /* ModelEvents.CONTEXT_CREATED */,
      this.contextCreated,
      this,
    );
    webAudioModel.addEventListener(
      'ContextDestroyed' /* ModelEvents.CONTEXT_DESTROYED */,
      this.contextDestroyed,
      this,
    );
    webAudioModel.addEventListener(
      'ContextChanged' /* ModelEvents.CONTEXT_CHANGED */,
      this.contextChanged,
      this,
    );
  ...
  onContextSelectorSelectionChanged(contextId) {
    this.selectedContextIndex = this.contextSelectorItems.findIndex(
  ...
  contextDestroyed(event) {
    ...
    if (index > -1) {
      ...
      const newSelectedIndex = selectedContext
        ? this.contextSelectorItems.indexOf(selectedContext)
        : -1;
  ...
  contextChanged(event) {
    ...
    const index = this.contextSelectorItems.findIndex(
      (context) => context.contextId === changedContext.contextId,
    );
    if (index > -1) {
