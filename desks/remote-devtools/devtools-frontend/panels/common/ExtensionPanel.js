...
export class ExtensionPanel extends UI.Panel.Panel {
  ...
  onSearchCanceled() {
    this.server.notifySearchAction(
      this.id,
      'cancelSearch' /* Extensions.ExtensionAPI.PrivateAPI.Panels.SearchAction.CancelSearch */,
    );
  ...
  performSearch(searchConfig, _shouldJump, _jumpBackwards) {
    ...
    this.server.notifySearchAction(
      this.id,
      'performSearch' /* Extensions.ExtensionAPI.PrivateAPI.Panels.SearchAction.PerformSearch */,
      query,
    );
  }
  supportsMatchCounts() {
  ...
  jumpToNextSearchResult() {
    this.server.notifySearchAction(
      this.id,
      'nextSearchResult' /* Extensions.ExtensionAPI.PrivateAPI.Panels.SearchAction.NextSearchResult */,
    );
  }
  jumpToPreviousSearchResult() {
    this.server.notifySearchAction(
      this.id,
      'previousSearchResult' /* Extensions.ExtensionAPI.PrivateAPI.Panels.SearchAction.PreviousSearchResult */,
    );
  }
...
export class ExtensionSidebarPane extends UI.View.SimpleView {
  ...
  #setObject(object, title, callback) {
    ...
    void UI.UIUtils.Renderer.render(object, { title, editable: false, expand: true }).then(
      (result) => {
        if (!result) {
          callback();
          return;
