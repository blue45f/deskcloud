...
function getTooltipDisclaimerText(noLogging, panel) {
  switch (panel) {
    case 'console' /* AiCodeCompletion.AiCodeCompletion.ContextFlavor.CONSOLE */:
      return noLogging
        ? lockedString(
            UIStringsNotTranslate.tooltipDisclaimerTextForAiCodeGenerationNoLoggingInConsole,
          )
    ...
    case 'sources' /* AiCodeCompletion.AiCodeCompletion.ContextFlavor.SOURCES */:
      return noLogging
        ? lockedString(
            UIStringsNotTranslate.tooltipDisclaimerTextForAiCodeGenerationNoLoggingInSources,
          )
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  switch (input.displayState) {
    case AiCodeGenerationTeaserDisplayState.TRIGGER: {
      ...
      const screenReaderText =
        (Host.Platform.isMac()
          ? UIStringsNotTranslate.cmdItoGenerateCode
          : UIStringsNotTranslate.ctrlItoGenerateCode) +
        ' ' +
        toLearnHowYourDataIsBeingUsedScreenReaderOnly;
      const cmdOrCtrl = Host.Platform.isMac()
        ? lockedString(UIStringsNotTranslate.cmd)
        : lockedString(UIStringsNotTranslate.ctrl);
      const toGenerateCode = html`<span class="ai-code-generation-keyboard-action">
          <span>${cmdOrCtrl}</span>
          <span>${lockedString(UIStringsNotTranslate.i)}</span> </span
        >&nbsp;${lockedString(UIStringsNotTranslate.toGenerateCode)}`;
      const toLearnHowYourDataIsBeingUsedVisible = html`<span
          class="ai-code-generation-keyboard-action"
        >
          <span>${cmdOrCtrl}</span>
          <span>${lockedString(UIStringsNotTranslate.period)}</span> </span
        >&nbsp;${lockedString(UIStringsNotTranslate.toLearnHowYourDataIsBeingUsed)}`;
      ...
      teaserLabel = html`<div class="ai-code-generation-teaser-trigger">
        ...
        <span
          class="ai-code-generation-teaser-screen-reader-only"
          aria-atomic="true"
          aria-live="assertive"
        >
        ...
        >
          <div class="disclaimer-tooltip-container">
            <div class="tooltip-text">${tooltipDisclaimerText}</div>
            <span
              tabIndex="0"
              class="link"
              role="link"
              ...
              >${lockedString(UIStringsNotTranslate.manageInSettings)}</span
            >
    ...
    case AiCodeGenerationTeaserDisplayState.DISCOVERY: {
      ...
      const teaserText =
        input.panel === 'console' /* AiCodeCompletion.AiCodeCompletion.ContextFlavor.CONSOLE */
    ...
    case AiCodeGenerationTeaserDisplayState.LOADING: {
      ...
      // clang-format off
      teaserLabel = html` <div class="ai-code-generation-teaser-screen-reader-only">
          ${teaserAriaLabel}
        </div>
        <span class="ai-code-generation-spinner" aria-hidden="true">
          &nbsp;${lockedString(UIStringsNotTranslate.generating)}
          <span class="ai-code-generation-keyboard-action"
            ><span>${lockedString(UIStringsNotTranslate.esc)}</span></span
          >
          ${lockedString(UIStringsNotTranslate.toCancel)}&nbsp;
        </span>
        <span
          class="ai-code-generation-timer"
          aria-hidden="true"
          ${Directives.ref((el) => {
            if (el) {
  ...
  render(
    html`
      ...
      <style>
        @scope to (devtools-widget > *) { ${UI.inspectorCommonStyles} }
      </style>
      <div class="ai-code-generation-teaser">&nbsp;${teaserLabel}</div>
    `,
    target,
  );
  // clang-format on
...
export class AiCodeGenerationTeaser extends UI.Widget.Widget {
  ...
  #timerIntervalId;
  #loadStartTime;
  #aiCodeGenerationUsedSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.#view(
      {
        ...
        showDiscoveryTeaser:
          !this.#aiCodeGenerationUsedSetting.get() &&
          !AiCodeGenerationTeaser.#discoveryTeaserShownInSession,
        panel: this.#panel,
      },
      this.#viewOutput,
      this.contentElement,
