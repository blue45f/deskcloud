...
const DEFAULT_VIEW = (input, _output, target) => {
  // clang-format off
  render(
    html`${input.tagName || input.pseudo
      ? html` <style>
            ${domLinkifierStyles}
          </style>
          <span class="monospace">
            <button
              class="node-link text-button link-style ${classMap({
            ...
            >
              ${[
                input.tagName
                  ? html`<span class="node-label-name">${input.tagName}</span>`
                  : nothing,
                input.id ? html`<span class="node-label-id">#${input.id}</span>` : nothing,
                ...input.classes.map(
                  (className) => html`<span class="extra node-label-class">.${className}</span>`,
                ),
                input.pseudo
                  ? html`<span class="extra node-label-pseudo">${input.pseudo}</span>`
                  : nothing,
              ]}
            </button>
          </span>`
      : i18nString(UIStrings.node)}`,
    target,
  );
  // clang-format on
};
    ...
    if (classAttribute) {
      ...
      if (classes.length) {
        ...
        for (let i = 0; i < classes.length; ++i) {
          ...
          if (
            className &&
            !options.hiddenClassList?.includes(className) &&
            !foundClasses.has(className)
          ) {
...
const DEFERRED_DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    html` <style>
      ...
      <button
        class="node-link text-button link-style"
