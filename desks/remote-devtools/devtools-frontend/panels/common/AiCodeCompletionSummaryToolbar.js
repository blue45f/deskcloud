...
export const DEFAULT_SUMMARY_TOOLBAR_VIEW = (input, _output, target) => {
  ...
  // clang-format off
  const disclaimer =
    input.disclaimerTooltipId && input.spinnerTooltipId
      ? html`<devtools-widget
          ...
          class="disclaimer-widget"
        ></devtools-widget>`
      : nothing;
  const recitationNotice =
    input.citations && input.citations.size > 0
      ? html`<div class="ai-code-completion-recitation-notice">
          ...
          <span
            ...
            tabIndex="0"
          >
            ${lockedString(UIStringsNotTranslate.viewSources)}&nbsp;${lockedString(
              '(' + input.citations.size + ')',
            )}
          </span>
          <devtools-tooltip
              ...
              ${Directives.repeat(
                ...
                (citation) =>
                  html`<devtools-link
                    tabIndex="0"
                    href=${citation}
                    jslogcontext="ai-code-completion-citations.citation-link"
                    >${citation}</devtools-link
                  >`,
              )}
  ...
  render(
    html`
      ...
      <div class=${toolbarClasses}>${disclaimer} ${recitationNotice}</div>
    `,
    target,
  );
  // clang-format on
