    ...
    html`
        ...
        <div class="section">
          ...
          <div class="text-container">
            ...
            <div class="section-text">
              <div>${i18nString(UIStrings.tailorProfileBody)}</div>
              <br />
              <div>
                ${uiI18n.getFormatLocalizedStringTemplate(
                  str_,
                  UIStrings.tailorProfileBodyDisclaimer,
                  {
                    PH1: html`<devtools-link
                      href=${CONTENT_POLICY_URL}
                      class="link"
                      jslogcontext="content-policy"
                      >${i18nString(UIStrings.contentPolicy)}</devtools-link
                    >`,
                    PH2: html`<devtools-link
                      href=${TERMS_OF_SERVICE_URL}
                      class="link"
                      jslogcontext="terms-of-service"
                      >${i18nString(UIStrings.termsOfService)}</devtools-link
                    >`,
                    PH3: html`<devtools-link
                      href=${PRIVACY_POLICY_URL}
                      class="link"
                      jslogcontext="privacy-policy"
                      >${i18nString(UIStrings.privacyPolicy)}</devtools-link
                    >`,
                  },
                )}
              </div>
      ...
      <div class="buttons">
        <devtools-button
          ...
          @click=${() => UIHelpers.openInNewTab(GDP_PROGRAM_URL)}
          >${i18nString(UIStrings.learnMore)}</devtools-button
        >
        <div class="right-buttons">
          ...
          <devtools-button
            ...
            >${i18nString(UIStrings.signUp)}</devtools-button
          >
        </div>
      </div>
    `,
...
export class GdpSignUpDialog extends UI.Widget.VBox {
  ...
  async #onSignUpClick() {
    ...
    this.requestUpdate();
    const syncInfo = await new Promise((resolve) =>
      Host.InspectorFrontendHost.InspectorFrontendHostInstance.getSyncInformation(resolve),
    );
    const user = syncInfo.accountFullName ?? '';
    const emailPreference = this.#keepMeUpdated
      ? Host.GdpClient.EmailPreference.ENABLED
      : Host.GdpClient.EmailPreference.DISABLED;
    const result = await Host.GdpClient.GdpClient.instance().createProfile({
      user,
      emailPreference,
    });
    if (result) {
      ...
      Snackbars.Snackbar.Snackbar.show(
        { message: i18nString(UIStrings.signUpFailed) },
        this.#dialog.contentElement,
      );
