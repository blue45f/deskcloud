...
export class PersistenceUtils {
  static tooltipForUISourceCode(uiSourceCode) {
    ...
    if (binding.network.contentType().isFromSourceMap()) {
      return i18nString(UIStrings.linkedToSourceMapS, {
        PH1: Platform.StringUtilities.trimMiddle(binding.network.url(), 150),
      });
    }
    return i18nString(UIStrings.linkedToS, {
      PH1: Platform.StringUtilities.trimMiddle(binding.network.url(), 150),
    });
  }
...
export class LinkDecorator extends Common.ObjectWrapper.ObjectWrapper {
  ...
  bindingChanged(event) {
    ...
    this.dispatchEventToListeners(
      'LinkIconChanged' /* Components.Linkifier.LinkDecorator.Events.LINK_ICON_CHANGED */,
      binding.network,
    );
  }
