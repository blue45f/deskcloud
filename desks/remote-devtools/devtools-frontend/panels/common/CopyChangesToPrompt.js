...
export class CopyChangesToPrompt extends UI.Widget.Widget {
  ...
  async performUpdate() {
    ...
    const diffs = await Promise.all(
      this.#getModifiledFiles().map(async (modifiedUISourceCode) => {
...
const GEMINI_CHANGES_VIEW = (input, _output, target) => {
  ...
  const promptForPatchAgentCSS = input.patchAgentCSSChange
    ? buildPatchAgentCSSPrompt(input.patchAgentCSSChange)
    : '';
  const finalPrompt = [PREAMBLE, promptForChangesPanel, promptForPatchAgentCSS]
    .filter((x) => x.length > 0)
    .join(`\n`);
  function copyClick() {
  ...
  render(
    html`<devtools-button
      ...
      @click=${copyClick}
      >Copy prompt to clipboard</devtools-button
    > `,
    target,
  );
  // clang-format on
