...
export class ConsoleContextSelector {
  ...
  constructor() {
    ...
    this.items.addEventListener('ItemsReplaced' /* UI.ListModel.Events.ITEMS_REPLACED */, () =>
  ...
  executionContextCreated(executionContext) {
    this.items.insertWithComparator(
      executionContext,
      executionContext.runtimeModel.executionContextComparator(),
    );
    if (
      executionContext === UI.Context.Context.instance().flavor(SDK.RuntimeModel.ExecutionContext)
    ) {
  ...
  executionContextChangedExternally({ data: executionContext }) {
    if (
      executionContext &&
      !SDK.TargetManager.TargetManager.instance().isInScope(executionContext.target())
    ) {
      return;
    }
    this.dropDown.selectItem(executionContext);
  ...
  isTopContext(executionContext) {
    ...
    const resourceTreeModel = executionContext
      .target()
      .model(SDK.ResourceTreeModel.ResourceTreeModel);
    const frame =
      executionContext.frameId && resourceTreeModel?.frameForId(executionContext.frameId);
    if (!frame) {
      return false;
    }
  ...
  subtitleFor(executionContext) {
    ...
    if (
      ...
      sameTargetParentFrame.securityOrigin !== executionContext.origin
    ) {
  ...
  itemSelected(item) {
    this.#toolbarItem.element.classList.toggle(
      'highlight',
      !this.isTopContext(item) && this.hasTopContext(),
    );
    const title = item
      ? i18nString(UIStrings.javascriptContextS, { PH1: this.titleFor(item) })
  ...
  callFrameSelectedInUI() {
    const callFrame = UI.Context.Context.instance().flavor(
      StackTrace.StackTrace.DebuggableFrameFlavor,
    );
