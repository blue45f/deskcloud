...
export class PromptBuilder {
  ...
  async getMessageSourceCode() {
    ...
    const content = await mappedLocation?.uiSourceCode
      .requestContentData()
      .then((contentDataOrError) =>
    ...
    if (text.length > MAX_CODE_SIZE && (firstNewline < 0 || firstNewline > MAX_CODE_SIZE)) {
      ...
      const { formattedContent, formattedMapping } =
        await Formatter.ScriptFormatter.formatScriptContent(
          mappedLocation?.uiSourceCode.mimeType() ?? 'text/javascript',
          text,
        );
      const [lineNumber, columnNumber] = formattedMapping.originalToFormatted(
        mappedLocation?.lineNumber ?? 0,
        mappedLocation?.columnNumber ?? 0,
      );
      return { text: formattedContent, columnNumber, lineNumber };
    }
    return {
      text,
      columnNumber: mappedLocation?.columnNumber ?? 0,
      lineNumber: mappedLocation?.lineNumber ?? 0,
    };
  }
  async buildPrompt(sourcesTypes = Object.values(SourceType)) {
    ...
    const stacktrace = sourcesTypes.includes(SourceType.STACKTRACE)
      ? await formatStackTrace(this.#consoleMessage)
      : '';
    ...
    return {
      ...
      isPageReloadRecommended:
        sourcesTypes.includes(SourceType.NETWORK_REQUEST) &&
        Boolean(this.#consoleMessage.consoleMessage().getAffectedResources()?.requestId) &&
        !relatedRequest,
    };
  }
...
export function formatRelatedCode({ text, columnNumber, lineNumber }, maxCodeSize = MAX_CODE_SIZE) {
  ...
  while (
    lines[currentLineNumber] !== undefined &&
  ...
  while (
    lines[currentLineNumber] !== undefined &&
    relatedCodeSize + lines[currentLineNumber].length <= maxCodeSize
  ) {
    ...
    if (
      ...
      if (
        ...
        !nextWhitespace.startsWith(whitespace)
      ) {
