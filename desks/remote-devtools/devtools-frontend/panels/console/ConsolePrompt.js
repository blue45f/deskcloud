...
export class ConsolePrompt extends Common.ObjectWrapper.eventMixin(UI.Widget.Widget) {
  ...
  #getJavaScriptCompletionExtensions() {
    ...
    if (Root.Runtime.Runtime.queryParam('noJavaScriptCompletion') !== 'true') {
      return [CodeMirror.javascript.javascript(), TextEditor.JavaScript.completion()];
    }
  ...
  editorKeymap() {
    return [
      ...
      {
        mac: 'Ctrl-p',
        run: () => this.#editorHistory.moveHistory(-1 /* Direction.BACKWARD */, true),
      },
      {
        mac: 'Ctrl-n',
        run: () => this.#editorHistory.moveHistory(1 /* Direction.FORWARD */, true),
      },
      {
        key: 'Escape',
        run: () => {
  ...
  async handleEnter(forceEvaluate) {
    ...
    if (await this.enterWillEvaluate(forceEvaluate)) {
      ...
      if (this.aiCodeCompletionProvider) {
        const teaserMode = this.editor.editor.state.field(
          TextEditor.AiCodeCompletionProvider.aiCodeCompletionTeaserModeState,
        );
