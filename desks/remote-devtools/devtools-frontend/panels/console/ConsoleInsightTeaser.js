...
const DATA_USAGE_URL =
  'https://developer.chrome.com/docs/devtools/ai-assistance/get-started#data-use';
  ...
  return html`
    <div class="teaser-tooltip-container">
      <div class="response-container">
        ...
        <div>
          You can get quick answers from
          <devtools-link
...
export class ConsoleInsightTeaser extends UI.Widget.Widget {
  ...
  constructor(uuid, consoleViewMessage, element, view) {
    ...
    this.#isForWarning =
      this.#consoleViewMessage.consoleMessage().level ===
    ...
    this.#state = this.#builtInAi.hasSession()
      ? 'ready' /* State.READY */
      : 'no-model' /* State.NO_MODEL */;
  ...
  #onTellMeMoreClick(event) {
    ...
    if (
      this.#getConsoleInsightsEnabledSetting()?.getIfNotDisabled() &&
  ...
  #onSessionCreation() {
    if (
      this.#builtInAi.hasSession() &&
      (this.#state === 'no-model' /* State.NO_MODEL */ ||
        this.#state === 'downloading') /* State.DOWNLOADING */
    ) {
    ...
    const result = await PanelCommon.FreDialog.show({
      ...
      reminderItems: [
        ...
        },
        {
          iconName: 'google',
          content: noLogging
            ? lockedString(UIStringsNotTranslate.consoleInsightsSendsDataNoLogging)
          ...
          if (this.#eventListeners.length === 0) {
            this.#eventListeners = [
              this.#builtInAi.addEventListener(
                'downloadProgressChanged' /* AiAssistanceModel.BuiltInAi.Events.DOWNLOAD_PROGRESS_CHANGED */,
  ...
  abortTeaserGeneration() {
    ...
    if (
      ...
      if (this.#startTime) {
        if (this.#mainText) {
          Host.userMetrics.consoleInsightTeaserAbortedAfterFirstCharacter(
            performance.now() - this.#startTime,
          );
        } else {
          Host.userMetrics.consoleInsightTeaserAbortedBeforeFirstCharacter(
            performance.now() - this.#startTime,
          );
        }
      }
    ...
    try {
      for await (const chunk of this.#getOnDeviceInsight()) {
        ...
        if (!firstChunkReceived) {
          ...
          Host.userMetrics.consoleInsightTeaserFirstChunkGenerated(
            firstChunkTime - this.#startTime,
          );
          Host.userMetrics.consoleInsightTeaserFirstChunkGeneratedMedium(
            firstChunkTime - this.#startTime,
          );
        }
      }
  ...
  #hasTellMeMoreButton() {
    ...
    if (
      this.#aidaAvailability !== 'available' /* Host.AidaClient.AidaAccessPreconditions.AVAILABLE */
    ) {
      return false;
    }
    return true;
