...
export const DEFAULT_PANE_VIEW = (input, _output, target) => {
  ...
  render(
    html` <style>
        ${consolePinPaneStyles}
      </style>
      <div
        class="console-pins monospace"
        jslog=${VisualLogging.pane('console-pins')}
        @contextmenu=${input.onContextMenu}
      >
        ${input.pins.map((pin) =>
          widget(ConsolePinPresenter, {
            pin,
...
export class ConsolePinPane extends UI.Widget.VBox {
  ...
  contextMenuEventFired(event) {
        ...
        if (targetPin instanceof ConsolePinPresenter) {
          contextMenu
            ...
            .appendItem(
              ...
              () => (targetPin.pin ? this.removePin(targetPin.pin) : undefined),
    ...
    contextMenu
      .editSection()
      .appendItem(i18nString(UIStrings.removeAllExpressions), this.removeAllPins.bind(this), {
...
export const DEFAULT_VIEW = (input, output, target) => {
  const deleteIconLabel = input.expression
    ? i18nString(UIStrings.removeExpressionS, { PH1: input.expression })
...
function renderResult(result, isEditing) {
  ...
  if (result && SDK.RuntimeModel.RuntimeModel.isSideEffectFailure(result)) {
    return html`<span
      ...
      title=${i18nString(UIStrings.evaluateAllowingSideEffects)}
      >(…)</span
    >`;
  }
...
export class ConsolePinPresenter extends UI.Widget.Widget {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    ...
    this.#pinEditor = {
      ...
      workingCopyWithHint: () =>
        this.#editor ? TextEditor.Config.contentIncludingHint(this.#editor.editor) : '',
  ...
  wasShown() {
    ...
    this.#pin?.addEventListener(
      'EVALUATE_RESULT_READY' /* ConsolePinEvent.EVALUATE_RESULT_READY */,
      this.requestUpdate,
      this,
    );
    this.requestUpdate();
  ...
  #createInitialEditorState(doc) {
    const extensions = [
      CodeMirror.EditorView.contentAttributes.of({
        'aria-label': i18nString(UIStrings.liveExpressionEditor),
      }),
      ...
      CodeMirror.keymap.of([
        {
          ...
          run: (view) => {
            view.dispatch({
              changes: { from: 0, to: view.state.doc.length, insert: this.#pin?.expression ?? '' },
            });
            this.#focusOut?.();
            return true;
          },
  ...
  #onBlur(editor) {
    ...
    editor.dispatch({
      ...
      changes: !commitedAsIs
        ? { from: 0, to: editor.state.doc.length, insert: this.#pin.expression }
        : undefined,
    });
    this.requestUpdate();
  }
...
export class ConsolePinModel {
  ...
  #save() {
    const expressions = this.#pins
      .values()
      .map((pin) => pin.expression)
      .toArray();
