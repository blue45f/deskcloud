...
export class ConsoleViewMessage {
  ...
  buildMessage() {
    ...
    if (this.message.source === Common.Console.FrontendMessageSource.ConsoleAPI) {
      switch (this.message.type) {
        ...
        default: {
          if (
            this.message.type === 'assert' /* Protocol.Runtime.ConsoleAPICalledEventType.Assert */
          ) {
      ...
      const messageInParameters =
        this.message.parameters && messageText === this.message.parameters[0];
  ...
  formatAsNetworkRequest() {
    ...
    if (this.message.level === 'error' /* Protocol.Log.LogEntryLevel.Error */) {
      ...
      const fragment = this.linkifyWithCustomLinkifier(
        messageText,
  ...
  buildMessageAnchor() {
    ...
    const linkify = ({ stackFrameWithBreakpoint, scriptId, stackTrace, url, line, column }) => {
      ...
      if (stackFrameWithBreakpoint) {
        return this.linkifier.maybeLinkifyConsoleCallFrame(
          runtimeModel.target(),
          stackFrameWithBreakpoint,
          {
            inlineFrameIndex: 0,
  ...
  buildMessageWithStackTrace(runtimeModel) {
    ...
    this.expandTrace = (expand) => {
      ...
      const stackTableState = expand
        ? i18nString(UIStrings.stackMessageExpanded)
        : i18nString(UIStrings.stackMessageCollapsed);
  ...
  buildMessageHelper(target, stackTrace, icon) {
    ...
    const stackTraceTarget =
      target ?? targetManager.primaryPageTarget() ?? targetManager.rootTarget();
    ...
    if (stackTraceTarget && stackTrace) {
      ...
      void Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance()
        ...
        .then(() => {
          const selectableLinks = stackTracePreview.linkElements.map((element) => ({
            element,
            forceSelect: () => element.focus(),
          }));
  ...
  format(rawParameters) {
    ...
    const shouldFormatMessage =
      SDK.RemoteObject.RemoteObject.type(parameters[0]) === 'string' &&
    ...
    for (let i = 0; i < parameters.length; ++i) {
      ...
      if (shouldFormatMessage && parameters[i].type === 'string') {
        appendOrShow(
          formattedResult,
          this.linkifyStringAsFragment(parameters[i].description || ''),
        );
      } else {
  ...
  formatParameter(output, forceObjectFormat, includePreview) {
    ...
    switch (outputType) {
      ...
      case 'webassemblymemory':
        ...
        break;
      case 'node':
        element = output.isNode()
          ? this.formatParameterAsNode(output)
          : this.formatParameterAsObject(output, false);
        break;
      case 'trustedtype':
  ...
  formatParameterAsObject(obj, includePreview) {
    ...
    const renderPreview = (includeNullOrUndefined) => {
      if (obj.preview) {
        ...
        render(
          this.previewFormatter.renderObjectPreview(obj.preview, includeNullOrUndefined),
          titleElement,
        );
        ObjectUI.ObjectPropertiesSection.ObjectPropertiesSection.appendMemoryIcon(
          titleElement,
          obj,
        );
      }
    ...
    if (includePreview && obj.preview) {
      ...
      void ObjectUI.ObjectPropertiesSection.ObjectPropertiesSection.formatObjectAsFunction(
        obj,
        functionElement,
        false,
      );
    ...
    section.root.addEventListener(
      'filter-changed' /* ObjectUI.ObjectPropertiesSection.ObjectTreeNodeBase.Events.FILTER_CHANGED */,
  ...
  formatParameterAsFunction(originalFunction, includePreview) {
    ...
    function formatTargetFunction(targetFunction) {
      ...
      const promise =
        ObjectUI.ObjectPropertiesSection.ObjectPropertiesSection.formatObjectAsFunction(
          targetFunction,
          functionElement,
          true,
          includePreview,
      ...
      result.addEventListener(
        'contextmenu',
        this.contextMenuEventFired.bind(this, originalFunction),
        false,
      );
  ...
  renderPropertyPreviewOrAccessor(object, property, propertyPath) {
    if (property.type === 'accessor') {
      return this.formatAsAccessorProperty(
        object,
        propertyPath.map((property) => property.name.toString()),
        false,
      );
    }
    return this.renderPropertyPreview(
      property.type,
      'subtype' in property ? property.subtype : undefined,
      null,
      property.value,
    );
  }
  ...
  renderPropertyPreview(type, subtype, className, description) {
    ...
    render(
      this.previewFormatter.renderPropertyPreview(type, subtype, className, description),
      fragment,
    );
    return fragment;
  }
  createRemoteObjectAccessorPropertySpan(object, propertyPath, callback) {
    ...
    function onInvokeGetterClick(event) {
      ...
      if (object) {
        void object
          .callFunction(invokeGetter, [{ value: JSON.stringify(propertyPath) }])
          .then(callback);
      }
    }
  ...
  formatAsAccessorProperty(object, propertyPath, isArrayEntry) {
    const rootElement = this.createRemoteObjectAccessorPropertySpan(
      object,
      propertyPath,
      ...
      if (wasThrown) {
        ...
        if (type !== 'function' && object.description) {
          if (type === 'string' || subtype === 'regexp' || subtype === 'trustedtype') {
            ...
            description = Platform.StringUtilities.trimEndWithMaxLength(
              object.description,
              maxLength,
            );
          }
        ...
        rootElement.appendChild(
          this.renderPropertyPreview(type, subtype, object.className, description),
        );
      }
    }
    return rootElement;
  ...
  matchesFilterRegex(regexObject) {
    ...
    return (
      (Boolean(anchorText) && regexObject.test(anchorText.trim())) ||
  ...
  updateTimestamp() {
    ...
    if (Common.Settings.Settings.instance().moduleSetting('console-timestamps-enabled').get()) {
      ...
      this.timestampElement.textContent =
        UI.UIUtils.formatTimestamp(this.message.timestamp, false) + ' ';
      UI.Tooltip.Tooltip.install(
        this.timestampElement,
        UI.UIUtils.formatTimestamp(this.message.timestamp, true),
      );
  ...
  maybeHandleOnKeyDown(event) {
    ...
    if (this.expandTrace && isWrapperFocused) {
      if (
        (event.key === 'ArrowLeft' && this.traceExpanded) ||
        (event.key === 'ArrowRight' && !this.traceExpanded)
      ) {
  ...
  contentElement() {
    ...
    const shouldIncludeTrace =
      Boolean(this.message.stackTrace) &&
  ...
  #startTeaserGeneration() {
    ...
    if (this.shouldShowTeaser()) {
      ...
      // Removes teaser if preferences have changed
      this.#teaser?.detach();
  ...
  toMessageElement() {
    ...
    this.elementInternal.classList.toggle(
      'console-adjacent-user-command-result',
      this.#adjacentUserCommandResult,
    );
    return this.elementInternal;
  }
  updateMessageElement() {
    ...
    this.elementInternal.setAttribute(
      'jslog',
      `${VisualLogging.item('console-message').track({
        click: true,
        resize: true,
    ...
    if (
      UI.ActionRegistry.ActionRegistry.instance().hasAction(EXPLAIN_HOVER_ACTION_ID) &&
  ...
  shouldShowInsights() {
    if (
      this.message.source === Common.Console.FrontendMessageSource.ConsoleAPI &&
    ...
    if (
      ...
      this.message.source === Common.Console.FrontendMessageSource.SELF_XSS
    ) {
      return false;
    }
    return (
  ...
  #createHoverButton() {
    ...
    button.setAttribute(
      'jslog',
      `${VisualLogging.action(EXPLAIN_HOVER_ACTION_ID).track({ click: true })}`,
    );
  ...
  shouldRenderAsWarning() {
    return (
      (this.message.level === 'verbose' /* Protocol.Log.LogEntryLevel.Verbose */ ||
  ...
  async getInlineFrames(debuggerModel, url, lineNumber, columnNumber) {
    ...
    const uiSourceCodes = projects
      .map((project) => project.uiSourceCodeForURL(url))
      .flat()
      .filter((f) => !!f);
    const scripts = uiSourceCodes
      .map((uiSourceCode) => debuggerWorkspaceBinding.scriptsForUISourceCode(uiSourceCode))
      .flat();
    if (scripts.length) {
      const location = new SDK.DebuggerModel.Location(
        debuggerModel,
        scripts[0].scriptId,
        lineNumber || 0,
        columnNumber,
      );
      const functionInfo = await debuggerWorkspaceBinding.pluginManager.getFunctionInfo(
        scripts[0],
        location,
      );
  ...
  async expandInlineStackFrames(
    ...
    for (let f = 0; f < frames.length; ++f) {
      ...
      this.selectableChildren.push({
        element: scriptLocationLink,
        forceSelect: () => scriptLocationLink.focus(),
      });
  ...
  tryFormatAsError(string, exceptionDetails) {
    ...
    const linkInfos = StackTrace.ErrorStackParser.parseSourcePositionsFromErrorStack(
      runtimeModel,
      string,
    );
    if (!linkInfos?.length) {
    ...
    if (exceptionDetails?.stackTrace) {
      StackTrace.ErrorStackParser.augmentErrorStackWithScriptIds(
        linkInfos,
    ...
    for (let i = 0; i < linkInfos.length; ++i) {
      ...
      if (!link && exceptionDetails && line.startsWith('SyntaxError')) {
        ...
        const maybeScriptLocation = this.createScriptLocationLinkForSyntaxError(
          debuggerModel,
          exceptionDetails,
        );
      ...
      const prefixWithoutFunction = link.prefix.substring(
        0,
        link.prefix.lastIndexOf(' ', link.prefix.length - 3),
      );
    ...
    for (const token of tokens) {
      ...
      switch (token.type) {
        case 'url': {
          ...
          const sourceURL = Common.ParsedURL.ParsedURL.removeWasmFunctionInfoFromURL(
            splitResult.url,
          );
          let linkNode;
          if (splitResult) {
  ...
  static tokenizeMessageText(string) {
    ...
    return results.map((result) => ({
      text: result.value,
      type: tokenizerTypes[result.regexIndex],
    }));
  }
  groupKey() {
...
export class ConsoleGroupViewMessage extends ConsoleViewMessage {
  ...
  constructor(consoleMessage, linkifier, requestResolver, issueResolver, onToggle, onResize) {
    ...
    this.collapsedInternal =
      consoleMessage.type ===
      'startGroupCollapsed' /* Protocol.Runtime.ConsoleAPICalledEventType.StartGroupCollapsed */;
  ...
  maybeHandleOnKeyDown(event) {
    ...
    if (focusedChildIndex === -1) {
      if (
        (event.key === 'ArrowLeft' && !this.collapsedInternal) ||
  ...
  setGroupEnd(viewMessage) {
    if (
      ...
      'endGroup' /* Protocol.Runtime.ConsoleAPICalledEventType.EndGroup */
    ) {
...
export class ConsoleCommand extends ConsoleViewMessage {
  ...
  contentElement() {
    ...
    this.formattedCommand.textContent = Platform.StringUtilities.replaceControlCharacters(
      this.text,
    );
    ...
    if (this.formattedCommand.textContent.length < MaxLengthToIgnoreHighlighter) {
      void CodeHighlighter.CodeHighlighter.highlightNode(
...
export class ConsoleTableMessageView extends ConsoleViewMessage {
  ...
  constructor(consoleMessage, linkifier, requestResolver, issueResolver, onResize) {
    super(consoleMessage, linkifier, requestResolver, issueResolver, onResize);
    console.assert(
      consoleMessage.type === 'table' /* Protocol.Runtime.ConsoleAPICalledEventType.Table */,
    );
    this.dataGrid = null;
  }
  wasShown() {
  ...
  buildTableMessage() {
    ...
    for (let i = 0; i < preview.properties.length; ++i) {
      ...
      for (let j = 0; j < rowSubProperties.length; ++j) {
        ...
        if (columnRendered) {
          const cellElement = this.renderPropertyPreviewOrAccessor(actualTable, cellProperty, [
            rowProperty,
            cellProperty,
          ]).firstElementChild;
    ...
    const columnDisplayNames = columnNames.map((name) =>
      name === rawValueColumnSymbol ? i18nString(UIStrings.value) : name.toString(),
    );
