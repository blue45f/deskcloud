  ...
  for (let match = re.exec(fmt); match !== null; match = re.exec(fmt)) {
    ...
    switch (specifier) {
      ...
      case undefined: {
        ...
        const value = [...currentStyle.entries()]
          .map(([key, val]) => `${key}:${val.trimStart()}`)
          .join(';');
        const type = 'style';
        tokens.push({ type, value });
        substitution = '';
  ...
  return [...cssString]
    .map((char) => {
      const charCodes = new Set(
        [char.toLowerCase(), char.toUpperCase()].map((c) => c.charCodeAt(0).toString(16)),
      );
      const charCodeRegex = [...charCodes]
        .map((charCode) => `\\\\0{0,${6 - charCode.length}}${charCode}[ \\n\\t]?`)
        .join('|');
...
export const updateStyle = (currentStyle, styleToAdd) => {
  ...
  const GOOD_IMAGESET_REGEX =
    /^image-set\((?:(?:(?:url|type)\("[^\\"]*"\)|[\d.]+(?:x|dpi|dpcm|dppx)),?\s*)+\)/i;
  currentStyle.clear();
  ...
  for (const property of buffer.style) {
    if (
      !ALLOWED_PROPERTY_PREFIXES.some(
