              ...
              ${input.attributes?.map(
                (attribute) =>
                  html` <li role="treeitem">
                    ...
                    <span
                      class="ax-name monospace"
                      @mousedown=${onStartEditing.bind(null, attribute)}
                    >
                      ${attribute.name}
                    </span>
                    <span class="separator" @mousedown=${onStartEditing.bind(null, attribute)}
                      >${':\xA0'}</span
                    >
                    <devtools-prompt
...
export class ARIAAttributesPane extends AccessibilitySubPane {
  ...
  performUpdate() {
    ...
    const attributes =
      this.node()
        ?.attributes()
        ?.filter((attribute) => this.isARIAAttribute(attribute)) ?? [];
    const propertyCompletions = new Map(
      attributes.map((attribute) => [attribute, ariaMetadata().valuesForProperty(attribute.name)]),
    );
    const input = {
