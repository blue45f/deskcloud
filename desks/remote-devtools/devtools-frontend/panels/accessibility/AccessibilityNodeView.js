...
const str_ = i18n.i18n.registerUIStrings(
  'panels/accessibility/AccessibilityNodeView.ts',
  UIStrings,
);
...
export class AXNodeSubPane extends AccessibilitySubPane {
  ...
  constructor() {
    ...
    this.ignoredInfo = this.createInfo(
      i18nString(UIStrings.accessibilityNodeNotExposed),
      'ax-ignored-info',
      'hidden',
    );
...
export class AXNodePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  static createSimpleValueElement(type, value) {
    ...
    if (
      !type ||
      type === 'valueUndefined' /* Protocol.Accessibility.AXValueType.ValueUndefined */ ||
  ...
  static createExclamationMark(tooltip) {
    const exclamationElement = UI.UIUtils.createIconLabel({
  ...
  appendValueElement(value) {
    ...
    const element = AXNodePropertyTreeElement.createSimpleValueElement(
      value.type,
      String(value.value),
    );
...
export class AXValueSourceTreeElement extends AXNodePropertyTreeElement {
  ...
  appendRelatedNodeWithIdref(relatedNode, idref) {
    ...
    const nodeTreeElement = new AXRelatedNodeSourceTreeElement(
      { deferredNode, idref },
      relatedNode,
    );
    this.appendChild(nodeTreeElement);
  ...
  update() {
    ...
    if (this.source.invalid) {
      const exclamationMark = AXNodePropertyTreeElement.createExclamationMark(
        i18nString(UIStrings.invalidSource),
      );
    ...
    if (this.source.attributeValue) {
      ...
      const valueElement = AXNodePropertyTreeElement.createSimpleValueElement(
        'valueUndefined' /* Protocol.Accessibility.AXValueType.ValueUndefined */,
  ...
  onattach() {
    ...
    if (this.value.text) {
      this.listItemElement.appendChild(
        AXNodePropertyTreeElement.createSimpleValueElement(
          'computedString' /* Protocol.Accessibility.AXValueType.ComputedString */,
          this.value.text,
        ),
      );
...
export class AXRelatedNodeElement {
  ...
  render() {
    ...
    if (this.deferredNode) {
      ...
      const valueElement = AXNodePropertyTreeElement.createExclamationMark(
        i18nString(UIStrings.noNodeWithThisId),
      );
...
export class AXNodeIgnoredReasonTreeElement extends AXNodePropertyTreeElement {
    ...
    switch (reason) {
      ...
      case 'hiddenByChildTree':
        reasonElement = uiI18n.getFormatLocalizedString(
          str_,
          UIStrings.elementIsHiddenByChildTree,
          {},
        );
        break;
      ...
      case 'ariaHiddenElement': {
        const ariaHiddenSpan = (document.createElement('span', { is: 'source-code' }).textContent =
          'aria-hidden');
        reasonElement = uiI18n.getFormatLocalizedString(str_, UIStrings.elementIsPlaceholder, {
          PH1: ariaHiddenSpan,
        });
        break;
      }
      case 'ariaHiddenSubtree': {
        const ariaHiddenSpan = (document.createElement('span', { is: 'source-code' }).textContent =
          'aria-hidden');
        const trueSpan = (document.createElement('span', { is: 'source-code' }).textContent =
          'true');
        reasonElement = uiI18n.getFormatLocalizedString(
          str_,
      ...
      case 'inertSubtree':
        reasonElement = uiI18n.getFormatLocalizedString(
          str_,
          UIStrings.elementIsInAnInertSubTree,
          {},
        );
        break;
      case 'inheritsPresentation':
        reasonElement = uiI18n.getFormatLocalizedString(
          str_,
          UIStrings.elementsInheritsPresentational,
          {},
        );
        break;
      case 'labelContainer':
      ...
      case 'presentationalRole': {
        ...
        const rolePresentationSpan = (document.createElement('span', {
          is: 'source-code',
        }).textContent = 'role=' + role);
        reasonElement = uiI18n.getFormatLocalizedString(str_, UIStrings.elementHasPlaceholder, {
      ...
      case 'probablyPresentational':
        reasonElement = uiI18n.getFormatLocalizedString(
          str_,
          UIStrings.elementIsPresentational,
          {},
        );
        break;
      case 'uninteresting':
  ...
  onattach() {
    this.listItemElement.removeChildren();
    this.reasonElement = AXNodeIgnoredReasonTreeElement.createReasonElement(
      this.property.name,
      this.axNode,
    );
