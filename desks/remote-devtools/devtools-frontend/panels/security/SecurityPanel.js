...
export class SecurityPanel extends UI.Panel.Panel {
  ...
  constructor(view = DEFAULT_VIEW) {
    ...
    this.sidebar.element.setAttribute(
      'jslog',
      `${VisualLogging.pane('sidebar').track({ resize: true })}`,
    );
  ...
  static createCertificateViewerButtonForOrigin(text, origin) {
    const certificateButton = UI.UIUtils.createTextButton(
      text,
      async (e) => {
        e.consume();
        const names =
          await SDK.NetworkManager.MultitargetNetworkManager.instance().getCertificate(origin);
        if (names.length > 0) {
  ...
  modelAdded(securityModel) {
    ...
    this.eventListeners = [
      securityModel.addEventListener(
        Events.VisibleSecurityStateChanged,
...
export class SecurityMainView extends UI.Widget.VBox {
    ...
    this.lockSpectrum = new Map([
      ...
      [
        ...
        lockSpectrum.appendChild(
          getSecurityStateIconForOverview(
            'insecure' /* Protocol.Security.SecurityState.Insecure */,
            'lock-icon lock-icon-insecure',
          ),
        ),
      ],
    ]);
    UI.Tooltip.Tooltip.install(
      this.getLockSpectrumDiv('secure' /* Protocol.Security.SecurityState.Secure */),
      i18nString(UIStrings.secure),
    );
    UI.Tooltip.Tooltip.install(
      this.getLockSpectrumDiv('neutral' /* Protocol.Security.SecurityState.Neutral */),
      i18nString(UIStrings.info),
    );
    UI.Tooltip.Tooltip.install(
      this.getLockSpectrumDiv('insecure' /* Protocol.Security.SecurityState.Insecure */),
  ...
  addExplanation(parent, explanation) {
    ...
    if (explanation.title) {
      ...
      explanationHeader.createChild(
        'span',
        'security-explanation-title-' + explanation.securityState,
      ).textContent = explanation.summary;
    } else {
    ...
    if (explanation.certificate.length) {
      text.appendChild(
        SecurityPanel.createCertificateViewerButtonForCert(
  ...
  updateVisibleSecurityState(visibleSecurityState) {
    ...
    if (this.securityState === 'insecure' /* Protocol.Security.SecurityState.Insecure */) {
      ...
      this.securityState === 'insecure-broken' /* Protocol.Security.SecurityState.InsecureBroken */
    ) {
      this.getLockSpectrumDiv(
        'insecure' /* Protocol.Security.SecurityState.Insecure */,
  ...
  getSecuritySummaryAndExplanations(visibleSecurityState) {
    ...
    if (securityStateIssueIds.includes('malicious-content')) {
      ...
      securityState === 'insecure-broken' /* Protocol.Security.SecurityState.InsecureBroken */ &&
    ...
    if (securityStateIssueIds.includes('scheme-is-not-cryptographic')) {
      if (
        securityState === 'neutral' /* Protocol.Security.SecurityState.Neutral */ &&
  ...
  explainSafetyTipSecurity(visibleSecurityState, summary, explanations) {
    ...
    if (securityStateIssueIds.includes('bad_reputation')) {
      ...
      currentExplanations.push({
        ...
        description: formattedDescriptionSafety,
      });
    }
  ...
  explainCertificateSecurity(visibleSecurityState, explanations) {
      ...
      if (certificateSecurityState.certificateHasWeakSignature) {
        explanations.push(
          new SecurityStyleExplanation(
            ...
            'none' /* Protocol.Security.MixedContentType.None */,
          ),
        );
      } else {
        explanations.push(
    ...
    if (
      ...
      securityStateIssueIds.includes('cert-missing-subject-alt-name')
    ) {
      explanations.push(
        new SecurityStyleExplanation(
          'insecure' /* Protocol.Security.SecurityState.Insecure */,
    ...
    if (certificateSecurityState && certificateSecurityState.certificateNetworkError !== null) {
      explanations.push(
        new SecurityStyleExplanation(
          ...
          i18nString(UIStrings.thisSiteIsMissingAValidTrusted, {
  ...
  explainConnectionSecurity(visibleSecurityState, explanations) {
    ...
    if (certificateSecurityState.obsoleteSslProtocol) {
      recommendations.push(
        i18nString(UIStrings.sIsObsoleteEnableTlsOrLater, {
          PH1: certificateSecurityState.protocol,
        }),
      );
    }
    ...
    if (certificateSecurityState.obsoleteSslCipher) {
      recommendations.push(
        i18nString(UIStrings.sIsObsoleteEnableAnAesgcmbased, {
    ...
    if (securityStateIssueIds.includes('ran-mixed-content')) {
      ...
      explanations.push(
        new SecurityStyleExplanation(
          ...
          [],
          'blockable' /* Protocol.Security.MixedContentType.Blockable */,
        ),
      );
    }
  ...
  showNetworkFilter(filterKey, e) {
    ...
    void Common.Revealer.reveal(
      NetworkForward.UIFilter.UIRequestFilter.filters([
...
export class SecurityOriginView extends UI.Widget.VBox {
  ...
  constructor(origin, originState) {
    ...
    const originNetworkButton = UI.UIUtils.createTextButton(
      i18nString(UIStrings.viewRequestsInNetworkPanel),
      (event) => {
        event.consume();
        ...
        void Common.Revealer.reveal(
          NetworkForward.UIFilter.UIRequestFilter.filters([
            ...
            {
              filterType: NetworkForward.UIFilter.FilterType.Scheme,
    ...
    if (originState.securityDetails) {
      ...
      if (originState.securityDetails.keyExchange && originState.securityDetails.keyExchangeGroup) {
        table.addRow(
          ...
          originState.securityDetails.keyExchange +
            ' with ' +
            originState.securityDetails.keyExchangeGroup,
        );
      ...
      if (originState.securityDetails.serverSignatureAlgorithm) {
        // See https://www.iana.org/assignments/tls-parameters/tls-parameters.xhtml#tls-signaturescheme
        let sigString = SignatureSchemeStrings.get(
          originState.securityDetails.serverSignatureAlgorithm,
        );
        sigString ??=
          i18nString(UIStrings.unknownField) +
          ' (' +
          originState.securityDetails.serverSignatureAlgorithm +
          ')';
      ...
      if (
        sctListLength ||
        ctCompliance !== 'unknown' /* Protocol.Network.CertificateTransparencyCompliance.Unknown */
      ) {
      ...
      for (let i = 0; i < sctListLength; i++) {
        ...
        sctTable.addRow(
          i18nString(UIStrings.signatureData),
      ...
      switch (ctCompliance) {
        case 'compliant' /* Protocol.Network.CertificateTransparencyCompliance.Compliant */:
          sctSection.createChild('div', 'origin-view-section-notes').textContent = i18nString(
            UIStrings.thisRequestCompliesWithChromes,
          );
          break;
        case 'not-compliant' /* Protocol.Network.CertificateTransparencyCompliance.NotCompliant */:
          sctSection.createChild('div', 'origin-view-section-notes').textContent = i18nString(
            UIStrings.thisRequestDoesNotComplyWith,
          );
          break;
      ...
      if (originState.loadedFromCache) {
        noteSection.createChild('div').textContent = i18nString(
          UIStrings.thisResponseWasLoadedFromCache,
        );
      }
      noteSection.createChild('div').textContent = i18nString(
        UIStrings.theSecurityDetailsAboveAreFrom,
      );
    } else if (
      originState.securityState === 'secure' /* Protocol.Security.SecurityState.Secure */
    ) {
      ...
      secureSection.createChild('div').textContent = i18nString(
        UIStrings.thisOriginIsANonhttpsSecure,
      );
    } else if (
      originState.securityState !== 'unknown' /* Protocol.Security.SecurityState.Unknown */
    ) {
      ...
      notSecureSection.createChild('div').textContent = i18nString(
        UIStrings.yourConnectionToThisOriginIsNot,
      );
    } else {
      ...
      noInfoSection.createChild('div').textContent = i18nString(
        UIStrings.noSecurityDetailsAreAvailableFor,
      );
    }
  }
