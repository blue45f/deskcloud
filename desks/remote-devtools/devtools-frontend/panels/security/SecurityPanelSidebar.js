...
export class SecurityPanelSidebar extends UI.Widget.VBox {
  ...
  constructor(element) {
    super(element);
    this.#securitySidebarLastItemSetting = Common.Settings.Settings.instance().createSetting(
      ...
      [
        OriginGroup.NonSecure,
        {
          ...
          icon: getSecurityStateIconForDetailedView(
            ...
            `lock-icon lock-icon-${'insecure' /* Protocol.Security.SecurityState.Insecure */}`,
          ),
        },
      ],
      [
        OriginGroup.Secure,
        {
          ...
          icon: getSecurityStateIconForDetailedView(
            ...
            `lock-icon lock-icon-${'secure' /* Protocol.Security.SecurityState.Secure */}`,
          ),
        },
      ],
      [
        OriginGroup.Unknown,
        {
          ...
          icon: getSecurityStateIconForDetailedView(
            ...
            `lock-icon lock-icon-${'unknown' /* Protocol.Security.SecurityState.Unknown */}`,
          ),
        },
      ],
    ]);
    ...
    this.#mainViewReloadMessage = new UI.TreeOutline.TreeElement(
  ...
  #renderTreeElement(element) {
    if (element instanceof OriginTreeElement) {
      const securityState =
        element.securityState() ?? 'unknown'; /* Protocol.Security.SecurityState.Unknown */
      const isOverviewElement = element.listItemElement.classList.contains(
        'security-main-view-sidebar-tree-item',
      );
      const icon = isOverviewElement
        ...
        : getSecurityStateIconForDetailedView(
            securityState,
            `security-property security-property-${securityState}`,
          );
      element.setLeadingIcons([icon]);
      if (isOverviewElement) {
        element.title = i18nString(UIStrings.overview);
      } else {
        const elementTitle = createHighlightedUrl(
          element.origin() ?? Platform.DevToolsPath.EmptyUrlString,
          securityState,
        );
