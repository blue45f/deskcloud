...
const DEFAULT_VIEW = (input, _output, target) => {
  const renderAddress = () => {
    const createSpan = (startIndex, endIndex) => {
      ...
      const templateLines = textContentLines.map((line, i) =>
        i === textContentLines.length - 1 ? line : html`${line}<br />`,
      );
      const hasMatches = input.matches.some(
        (match) => match.startIndex <= startIndex && match.endIndex > startIndex,
      );
      if (!hasMatches) {
      ...
      const spanClasses = Lit.Directives.classMap({
        ...
        highlighted: input.highlightedMatches.some(
          (match) => match.startIndex <= startIndex && match.endIndex > startIndex,
        ),
      });
    ...
    return html` <div class="address">${spans}</div> `;
  };
  const renderFilledFields = () => {
    const highlightedGridRows = new Set(
      input.highlightedMatches.map((match) => match.filledFieldIndex),
    );
    // Disabled until https://crbug.com/1079231 is fixed.
    // clang-format off
    return html`
      <div class="grid-wrapper" role="region" aria-label=${i18nString(UIStrings.formInspector)}>
        <devtools-data-grid striped class="filled-fields-grid">
          <table>
            <tr>
              <th id="name" weight="50" sortable>${i18nString(UIStrings.formField)}</th>
              <th id="autofill-type" weight="50" sortable>
                ${i18nString(UIStrings.predictedAutofillValue)}
              </th>
              ...
              (field, index) =>
                html` <tr
                  style=${styleMap({
                    ...
                    'background-color': highlightedGridRows.has(index)
                ...
                >
                  ...
                  <td>
                    ${field.autofillType}
                    ${field.fillingStrategy ===
                    'autocompleteAttribute' /* FillingStrategy.AutocompleteAttribute */
                      ? html`<devtools-adorner
                          .name=${field.fillingStrategy}
                          title=${i18nString(UIStrings.autocompleteAttribute)}
                        >
                          <span>${i18nString(UIStrings.attr)}</span>
                        </devtools-adorner>`
                      : field.fillingStrategy ===
                          'autofillInferred' /* FillingStrategy.AutofillInferred */
                        ? html`<devtools-adorner
                            .name=${field.fillingStrategy}
                            title=${i18nString(UIStrings.inferredByHeuristics)}
                          >
          ...
          <div class="placeholder-container" jslog=${VisualLogging.pane('autofill-empty')}>
            <div class="empty-state">
              ...
              <div class="empty-state-description">
                ...
                <devtools-link href=${AUTOFILL_INFO_URL} class="link" jslogcontext="learn-more"
      ...
      <main>
        <div class="content-container" jslog=${VisualLogging.pane('autofill')}>
          <div
            class="right-to-left"
            role="region"
            aria-label=${i18nString(UIStrings.addressPreview)}
          >
            <div class="header">
...
export class AutofillView extends UI.Widget.VBox {
  #view;
  #autofillManager;
  #autoOpenViewSetting = Common.Settings.Settings.instance().createSetting(
  ...
  wasShown() {
    ...
    this.#autofillManager.addEventListener(
      'AddressFormFilled' /* AutofillManager.AutofillManager.Events.ADDRESS_FORM_FILLED */,
  ...
  performUpdate() {
    const onHighlightMatchesInAddress = (startIndex) => {
      this.#highlightedMatches = this.#matches.filter(
        (match) => match.startIndex <= startIndex && match.endIndex > startIndex,
      );
      this.requestUpdate();
    };
    const onHighlightMatchesInFilledFiels = (rowIndex) => {
      ...
      this.#highlightedMatches = this.#matches.filter(
        (match) => match.filledFieldIndex === rowIndex,
      );
      this.requestUpdate();
    };
