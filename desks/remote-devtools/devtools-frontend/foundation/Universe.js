...
export class Universe {
  ...
  constructor(options) {
    ...
    const targetManager = new SDK.TargetManager.TargetManager(
      context,
      options.overrideAutoStartModels,
    );
    ...
    const ignoreListManager = new Workspace.IgnoreListManager.IgnoreListManager(
      settings,
      targetManager,
    );
    context.set(Workspace.IgnoreListManager.IgnoreListManager, ignoreListManager);
    const resourceMapping = new Bindings.ResourceMapping.ResourceMapping(targetManager, workspace);
    const cssWorkspaceBinding = new Bindings.CSSWorkspaceBinding.CSSWorkspaceBinding(
      resourceMapping,
      targetManager,
    );
