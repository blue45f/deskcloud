...
export class Encoder {
  ...
  encode() {
    ...
    if (scope.isStackFrame) flags |= 4 /* OriginalScopeFlags.IS_STACK_FRAME */;
    this.#encodeTag('B' /* EncodedTag.ORIGINAL_SCOPE_START */)
      .#encodeUnsigned(flags)
      .#encodeUnsigned(encodedLine)
    ...
    this.#scopeState.line = line;
    this.#scopeState.column = column;
    this.#encodeTag('C' /* EncodedTag.ORIGINAL_SCOPE_END */)
      .#encodeUnsigned(encodedLine)
      .#encodeUnsigned(encodedColumn)
      .#finishItem();
  }
    ...
    if (range.isStackFrame) flags |= 4 /* GeneratedRangeFlags.IS_STACK_FRAME */;
    if (range.isHidden) flags |= 8 /* GeneratedRangeFlags.IS_HIDDEN */;
    ...
    for (let i = 0; i < range.values.length; ++i) {
      ...
      this.#encodeTag('H' /* EncodedTag.GENERATED_RANGE_SUBRANGE_BINDING */).#encodeUnsigned(i);
      ...
      for (let j = 1; j < value.length; ++j) {
        ...
        const encodedColumn =
          encodedLine === 0 ? subRange.from.column - lastColumn : subRange.from.column;
        ...
        const binding =
          subRange.value === undefined ? 0 : this.#resolveNamesIdx(subRange.value) + 1;
    ...
    for (const val of range.values) {
      if (val === null || val === undefined) {
        ...
        const binding =
          initialValue.value === undefined ? 0 : this.#resolveNamesIdx(initialValue.value) + 1;
    ...
    this.#encodeTag('I' /* EncodedTag.GENERATED_RANGE_CALL_SITE */)
      .#encodeUnsigned(sourceIndex)
      .#encodeUnsigned(line)
      .#encodeUnsigned(column)
      .#finishItem();
  }
