...
export class SafeScopeInfoBuilder extends ScopeInfoBuilder {
  ...
  startScope(line, column, options) {
    ...
    if (precedingSibling && comparePositions(precedingSibling.end, { line, column }) > 0) {
      throw new Error(
        `Scope start (${line}, ${column}) must not precede preceding siblings' end (${
          (precedingSibling.end.line, precedingSibling.end.column)
        })`,
      );
    }
  ...
  startRange(line, column, options) {
    ...
    if (precedingSibling && comparePositions(precedingSibling.end, { line, column }) > 0) {
      throw new Error(
        `Range start (${line}, ${column}) must not precede preceding siblings' end (${
          (precedingSibling.end.line, precedingSibling.end.column)
        })`,
      );
    }
