...
const pointerDeviceTypes = new Set(['mouse', 'pen', 'touch']);
...
function parseClickAttributes(step) {
  ...
  if (deviceType) {
    if (!isKnownDeviceType(deviceType)) {
      throw new Error(
        `'deviceType' for click steps must be one of the following: ${[...pointerDeviceTypes].join(
          ', ',
        )}`,
      );
    }
  ...
  if (button) {
    if (!isKnownMouseButton(button)) {
      throw new Error(
        `'button' for click steps must be one of the following: ${[...mouseButtonMap.keys()].join(
          ', ',
        )}`,
      );
    }
...
function parseStep(step, idx) {
  ...
  if (!isString(step.type)) {
    throw new Error(
      idx ? `Type of the step ${idx} is not a string` : 'Type of the step is not a string',
    );
  }
  switch (step.type) {
    case StepType.Click:
...
class PuppeteerStringifyExtension extends StringifyExtension {
      ...
      for (const event of step.assertedEvents) {
        switch (event.type) {
          case AssertedEventType.Navigation: {
            out.appendLine(
              `promises.push(${'frame' in step && step.frame ? 'frame' : 'targetPage'}.waitForNavigation());`,
            );
            break;
          }
          default:
  ...
  #appendTarget(out, target) {
    if (target === 'main') {
      ...
      out.appendLine(
        `const target = await browser.waitForTarget(t => t.url() === ${formatJSONAsJS(target, out.getIndent())}, { timeout });`,
      );
  ...
  #appendScrollStep(out, step) {
    if ('selectors' in step) {
      ...
      out.appendLine(
        `await targetPage.evaluate((x, y) => { globalThis.scroll(x, y); }, ${step.x}, ${step.y})`,
      );
    }
  }
  ...
  #appendWaitExpressionStep(out, step) {
    out.appendLine(
      `await ${step.frame ? 'frame' : 'targetPage'}.waitForFunction(${formatJSONAsJS(step.expression, out.getIndent())}, { timeout });`,
    );
  }
  #appendWaitForElementStep(out, step) {
    this.#shouldAppendWaitForElementHelper = true;
    out.appendLine(
      `await waitForElement(${formatJSONAsJS(step, out.getIndent())}, ${step.frame ? 'frame' : 'targetPage'}, timeout);`,
    );
  }
}
...
async function createRunner(flowOrExtension, maybeExtension) {
  ...
  const flow = !(flowOrExtension instanceof RunnerExtension) ? flowOrExtension : undefined;
...
class LighthouseStringifyExtension extends PuppeteerStringifyExtension {
  ...
  async beforeAllSteps(out, flow) {
    ...
    out.appendLine(
      `const lhFlow = await lhApi.startFlow(page, {name: ${formatJSONAsJS(flow.title, out.getIndent())}, config, flags});`,
    );
  }
  async stringifyStep(out, step, flow) {
    if (step.type === StepType.SetViewport) {
