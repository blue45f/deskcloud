...
const X = new t((O, T) => {
    if (40 == O.next) {
      let T = O.peek(-1);
      (y(T) || ((Q = T) >= 48 && Q <= 57) || 95 == T || 45 == T) && O.acceptToken(2, 1);
    }
    var Q;
  }),
  P = new t((O) => {
    if ($.indexOf(O.peek(-1)) > -1) {
      let { next: T } = O;
      (y(T) || 95 == T || 35 == T || 46 == T || 91 == T || 58 == T || 45 == T) &&
        O.acceptToken(110);
    }
  }),
  m = new t((O) => {
    if ($.indexOf(O.peek(-1)) < 0) {
      let { next: T } = O;
      if ((37 == T && (O.advance(), O.acceptToken(1)), y(T))) {
        do {
          O.advance();
        } while (y(O.next));
        O.acceptToken(1);
  ...
  n = o({
    'import charset namespace keyframes media supports when': i.definitionKeyword,
    'from to selector': i.keyword,
    NamespaceName: i.namespace,
    KeyframeName: i.labelName,
    TagName: i.tagName,
    ClassName: i.className,
    PseudoClassName: i.constant(i.className),
    IdName: i.labelName,
    'FeatureName PropertyName PropertyVariable': i.propertyName,
    AttributeName: i.attributeName,
    NumberLiteral: i.number,
    KeywordQuery: i.keyword,
    UnaryQueryOp: i.operatorKeyword,
    'CallTag ValueName': i.atom,
    VariableName: i.variableName,
    'AtKeyword Interpolation': i.special(i.variableName),
    Callee: i.operatorKeyword,
    Unit: i.unit,
    'UniversalSelector NestingSelector': i.definitionOperator,
    MatchOp: i.compareOperator,
    'ChildOp SiblingOp, LogicOp': i.logicOperator,
    BinOp: i.arithmeticOperator,
    Important: i.modifier,
    'Comment LineComment': i.blockComment,
    ColorLiteral: i.color,
    'ParenthesizedContent StringLiteral': i.string,
    Escape: i.special(i.string),
    ': ...': i.punctuation,
    'PseudoOp #': i.derefOperator,
    '; ,': i.separator,
    '( )': i.paren,
    '[ ]': i.squareBracket,
    '{ }': i.brace,
  ...
  p = a.deserialize({
    ...
    tokenizers: [P, m, X, 0, 1, 2, 3, 4],
    topRules: { StyleSheet: [0, 5] },
    specialized: [
      { term: 116, get: (O) => c[O] || -1 },
      { term: 23, get: (O) => f[O] || -1 },
    ],
    tokenPrec: 2180,
  }),
  s = T.define({
    name: 'less',
    parser: p.configure({ props: [e.add({ Declaration: l() }), S.add({ Block: r })] }),
    languageData: {
  ...
  W = O((O) => 'VariableName' == O.name || 'AtKeyword' == O.name);
function Z() {
  return new Q(s, s.data.of({ autocomplete: W }));
}
export { Z as less, W as lessCompletionSource, s as lessLanguage };
