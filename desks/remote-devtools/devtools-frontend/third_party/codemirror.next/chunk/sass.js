...
function P(O) {
  let $;
  return 47 == O.next && (47 == ($ = O.peek(1)) || 42 == $);
}
const s = new $(
    (O, $) => {
      if ($.dialectEnabled(0)) {
        let e;
        if (O.next < 0 && $.canShift(176)) O.acceptToken(176);
        else if (((e = O.peek(-1)) == d || e < 0) && $.canShift(175)) {
          let $ = 0;
          for (; O.next != d && l.includes(O.next); ) (O.advance(), $++);
          O.next == d || P(O) ? O.acceptToken(175, -$) : $ && O.acceptToken(177);
        } else if (O.next == d) O.acceptToken(174, 1);
        else if (l.includes(O.next)) {
          for (O.advance(); O.next != d && l.includes(O.next); ) O.advance();
          O.acceptToken(177);
        }
      } else {
        let $ = 0;
        for (; l.includes(O.next); ) (O.advance(), $++);
        $ && O.acceptToken(177);
      }
    },
    { contextual: !0 },
        ...
        if ($ > -1) {
          ...
          for (O.advance(); O.next >= 0; )
            if (O.next == d) {
              O.advance();
              let e = 0;
              for (; O.next != d && l.includes(O.next); ) (e++, O.advance());
              if (e < $) {
                Q = -e - 1;
                break;
              }
            } else {
              if (e && 42 == O.next && 47 == O.peek(1)) {
                Q = 2;
                break;
              }
              O.advance();
            }
          return void O.acceptToken(e ? 7 : 6, Q);
        }
      }
      if (47 == O.next) {
        for (; O.next != d && O.next >= 0; ) O.advance();
        O.acceptToken(6);
      } else {
        for (O.advance(); O.next >= 0; ) {
          let { next: $ } = O;
          if ((O.advance(), 42 == $ && 47 == O.next)) {
            O.advance();
            break;
          }
        }
  ...
  y = new $((O, $) => {
    ...
    let e = $.context.depth;
    if (O.next < 0 && e) O.acceptToken(169);
    else if (O.peek(-1) == d) {
      let $ = 0;
      for (; O.next != d && l.includes(O.next); ) (O.advance(), $++);
      $ == e || O.next == d || P(O) || ($ < e ? O.acceptToken(169, -$) : O.acceptToken(168));
    }
  }),
  u = new $((O, $) => {
    for (let e = !1, Q = 0, t = 0; ; t++) {
      let { next: a } = O;
      if (!(c(a) || 45 == a || 95 == a || (e && X(a)))) {
        if (35 == a && 123 == O.peek(1)) {
          O.acceptToken(5, 2);
          break;
        }
        e &&
          O.acceptToken(2 == Q && $.canShift(4) ? 4 : $.canShift(173) ? 173 : 40 == a ? 171 : 172);
        break;
      }
      (!e && (45 != a || t > 0) && (e = !0), Q === t && 45 == a && Q++, O.advance());
    }
  }),
  f = new $((O) => {
    if (125 == O.next) {
      for (O.advance(); c(O.next) || 45 == O.next || 95 == O.next || X(O.next); ) O.advance();
      35 == O.next && 123 == O.peek(1) ? O.acceptToken(2, 2) : O.acceptToken(1);
    }
  }),
  Y = new $((O) => {
    if (l.includes(O.peek(-1))) {
      let { next: $ } = O;
      (c($) ||
        95 == $ ||
        35 == $ ||
  ...
  h = new $((O) => {
    if (!l.includes(O.peek(-1))) {
      let { next: $ } = O;
      if ((37 == $ && (O.advance(), O.acceptToken(3)), c($))) {
        do {
          O.advance();
        } while (c(O.next) || X(O.next));
        O.acceptToken(3);
      }
    }
  });
...
const z = new Q({
    start: new w(null, 0),
    shift: (O, $, e, Q) => (168 == $ ? new w(O, e.pos - Q.pos) : 169 == $ ? O.parent : O),
    hash: (O) => O.hash,
  }),
  T = e({
  ...
  k = { __proto__: null, layer: 166, not: 184, only: 184, selector: 190 },
  U = O.deserialize({
    version: 14,
    states:
    ...
    tokenizers: [y, Y, f, h, u, s, m, p, 0, 1, 2, 3, 4],
    topRules: { StyleSheet: [0, 10], Styles: [1, 145] },
    dialects: { indented: 0 },
    specialized: [
      { term: 172, get: (O) => Z[O] || -1 },
      { term: 171, get: (O) => G[O] || -1 },
      { term: 80, get: (O) => q[O] || -1 },
      { term: 173, get: (O) => k[O] || -1 },
    ],
    tokenPrec: 3217,
  }),
  j = n.define({
  ...
  b = j.configure({
    dialect: 'indented',
    props: [
      r.add({ 'Block RuleSet': (O) => O.baseIndent + O.unit }),
      i.add({ Block: (O) => ({ from: O.from, to: O.to }) }),
    ],
  }),
  v = a((O) => 'VariableName' == O.name || 'SassVariableName' == O.name);
function _(O) {
  return new R((null == O ? void 0 : O.indented) ? b : j, j.data.of({ autocomplete: v }));
}
export { _ as sass, v as sassCompletionSource, j as sassLanguage };
