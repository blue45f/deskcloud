...
export class RegisteredFileStrings {
  ...
  getLocalizedStringSetFor(locale) {
    ...
    this.localizedStringSet = new LocalizedStringSet(
      this.filename,
...
export class LocalizedStringSet {
  ...
  getMessageFormatterFor(message) {
    const keyname = Object.keys(this.stringStructure).find(
      (key) => this.stringStructure[key] === message,
    );
    if (!keyname) {
    ...
    return new IntlMessageFormat.IntlMessageFormat(
      messageToTranslate,
      this.localeForFormatter,
      undefined,
      { ignoreTag: true },
    );
  }
  getSimpleLocalizedString(message) {
    ...
    try {
      ...
      const formatter = new IntlMessageFormat.IntlMessageFormat(
        message,
        this.localeForFormatter,
        undefined,
        { ignoreTag: true },
