      ...
      if (root.payload.role?.value === 'Iframe') {
        ...
        try {
          ...
          const handle = __addDisposableResource(
            env_1,
            await this.#realm.adoptBackendNode(root.payload.backendDOMNodeId),
            false,
          );
...
class AXNode {
  ...
  constructor(realm, payload) {
    ...
    this.#description = this.payload.description ? this.payload.description.value : undefined;
    this.#realm = realm;
    ...
    const node = {
      ...
      elementHandle: async () => {
        ...
        try {
          ...
          const handle = __addDisposableResource(
            env_2,
            await this.#realm.adoptBackendNode(this.payload.backendDOMNodeId),
            false,
          );
