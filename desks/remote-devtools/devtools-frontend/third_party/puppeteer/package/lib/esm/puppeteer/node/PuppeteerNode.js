...
export class PuppeteerNode extends Puppeteer {
  ...
  executablePath(optsOrChannel) {
    ...
    if (typeof optsOrChannel === 'string') {
      return this.#getLauncher('chrome').executablePath(optsOrChannel, /* validatePath= */ false);
    }
  ...
  async trimCache() {
    ...
    await Promise.all(
      puppeteerBrowsers.map(async (item) => {
        const tag =
          this.configuration?.[item.product]?.version ?? PUPPETEER_REVISIONS[item.product];
