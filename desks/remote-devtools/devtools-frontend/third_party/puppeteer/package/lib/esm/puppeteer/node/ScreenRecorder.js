...
let ScreenRecorder = (() => {
  ...
  return class ScreenRecorder extends _classSuper {
    static {
      ...
      __esDecorate(
        this,
        (_private_writeFrame_descriptor = {
        ...
        _private_writeFrame_decorators,
        {
          ...
          static: false,
          private: true,
          access: { has: (obj) => #writeFrame in obj, get: (obj) => obj.#writeFrame },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _stop_decorators,
        {
          ...
          static: false,
          private: false,
          access: { has: (obj) => 'stop' in obj, get: (obj) => obj.stop },
          metadata: _metadata,
        },
        null,
        _instanceExtraInitializers,
      ...
      this.#lastFrame = lastValueFrom(
        fromEmitterEvent(client, 'Page.screencastFrame').pipe(
          ...
          concatMap(([{ timestamp: previousTimestamp, buffer }, { timestamp }]) => {
            return from(
              Array(Math.round(fps * Math.max(timestamp - previousTimestamp, 0))).fill(buffer),
            );
          }),
          map((buffer) => {
    ...
    async stop() {
      ...
      await Promise.all(
        Array(Math.max(1, Math.round((this.#fps * (performance.now() - timestamp)) / 1000)))
          .fill(buffer)
     ...
     * @internal
     */
    async [((_private_writeFrame_decorators = [guarded()]),
