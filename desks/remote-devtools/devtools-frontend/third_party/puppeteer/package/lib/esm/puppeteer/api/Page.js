...
let Page = (() => {
  ...
  return class Page extends _classSuper {
        ...
        .pipe(
          mergeMap((originalRequest) => {
            return concat(
              of(1),
              merge(
                fromEmitterEvent(this, 'requestfailed' /* PageEvent.RequestFailed */),
                fromEmitterEvent(this, 'requestfinished' /* PageEvent.RequestFinished */),
                fromEmitterEvent(this, 'response' /* PageEvent.Response */).pipe(
                  map((response) => {
          ...
          takeUntil(fromEmitterEvent(this, 'close' /* PageEvent.Close */)),
          startWith(0),
        )
    ...
    waitForRequest(urlOrPredicate, options = {}) {
      ...
      const observable$ = fromEmitterEvent(this, 'request' /* PageEvent.Request */).pipe(
        filterAsync(urlOrPredicate),
        raceWith(
          timeout(ms),
          fromAbortSignal(signal),
          fromEmitterEvent(this, 'close' /* PageEvent.Close */).pipe(
            map(() => {
    ...
    waitForResponse(urlOrPredicate, options = {}) {
      ...
      const observable$ = fromEmitterEvent(this, 'response' /* PageEvent.Response */).pipe(
        filterAsync(urlOrPredicate),
        raceWith(
          timeout(ms),
          fromAbortSignal(signal),
    ...
    async waitForFrame(urlOrPredicate, options = {}) {
      ...
      return await firstValueFrom(
        merge(
          fromEmitterEvent(this, 'frameattached' /* PageEvent.FrameAttached */),
          fromEmitterEvent(this, 'framenavigated' /* PageEvent.FrameNavigated */),
          from(this.frames()),
        ).pipe(
    ...
    async screenshot(userOptions = {}) {
      ...
      try {
        const _guard = __addDisposableResource(
          env_2,
          await this.browserContext().startScreenshot(),
          false,
        );
        const options = {
          ...userOptions,
