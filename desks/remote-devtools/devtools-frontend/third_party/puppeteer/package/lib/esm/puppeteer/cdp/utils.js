...
export function createEvaluationError(details) {
  ...
  if (!details.exception) {
    ...
    (details.exception.type !== 'object' || details.exception.subtype !== 'error') &&
  ...
  if (details.stackTrace && stackLines.length < Error.stackTraceLimit) {
    for (const frame of details.stackTrace.callFrames.reverse()) {
      if (PuppeteerURL.isPuppeteerURL(frame.url) && frame.url !== PuppeteerURL.INTERNAL_URL) {
        ...
        stackLines.unshift(
          `    at ${frame.functionName || url.functionName} (${url.functionName} at ${url.siteString}, <anonymous>:${frame.lineNumber}:${frame.columnNumber})`,
        );
      } else {
        stackLines.push(
