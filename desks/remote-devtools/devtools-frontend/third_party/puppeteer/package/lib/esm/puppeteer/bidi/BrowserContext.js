  ...
  return class BidiBrowserContext extends _classSuper {
    static {
      ...
      __esDecorate(
        ...
        _trustedEmitter_initializers,
        _trustedEmitter_extraInitializers,
      );
      if (_metadata)
    ...
    #trustedEmitter_accessor_storage = __runInitializers(
      this,
      _trustedEmitter_initializers,
      new EventEmitter(),
    );
        ...
        if (browsingContext.originalOpener) {
          for (const context of this.userContext.browsingContexts) {
            if (context.id === browsingContext.originalOpener) {
              this.#pages.get(context).trustedEmitter.emit('popup' /* PageEvent.Popup */, page);
            }
          }
        }
      });
    ...
    #createPage(browsingContext) {
      ...
      page.trustedEmitter.on('framenavigated' /* PageEvent.FrameNavigated */, (frame) => {
        ...
        if (target === undefined) {
          this.trustedEmitter.emit(
            'targetchanged' /* BrowserContextEvent.TargetChanged */,
            pageTarget,
          );
        } else {
      ...
      page.trustedEmitter.on('framedetached' /* PageEvent.FrameDetached */, (frame) => {
        ...
        this.trustedEmitter.emit(
          'targetdestroyed' /* BrowserContextEvent.TargetDestroyed */,
          target,
        );
      });
    ...
    async newPage(options) {
      ...
      try {
        const _guard = __addDisposableResource(
          env_1,
          await this.waitForScreenshotOperations(),
          false,
        );
        const type =
          options?.type === 'window'
    ...
    async overridePermissions(origin, permissions) {
      ...
      await Promise.all(
        Array.from(WEB_PERMISSION_TO_PROTOCOL_PERMISSION.keys()).map((permission) => {
