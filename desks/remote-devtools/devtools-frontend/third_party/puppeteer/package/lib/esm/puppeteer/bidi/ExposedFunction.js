...
export class ExposableFunction {
  ...
  #handleMessage = async (params) => {
    ...
    try {
      ...
      const dataHandle = __addDisposableResource(
        env_1,
        BidiJSHandle.from(params.data, realm),
        false,
      );
      ...
      try {
        ...
        try {
          const argsHandle = __addDisposableResource(
            env_2,
            await dataHandle.evaluateHandle(([, , args]) => {
              return args;
            }),
            false,
          );
