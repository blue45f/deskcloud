...
export class BoundsManager extends EventTarget {
  ...
  state() {
    ...
    const entireBoundsMilli = Trace.Helpers.Timing.traceWindowMilliSeconds(
      this.#currentState.entireTraceBounds,
    );
    const minimapBoundsMilli = Trace.Helpers.Timing.traceWindowMilliSeconds(
      this.#currentState.minimapTraceBounds,
    );
    const timelineTraceWindowMilli = Trace.Helpers.Timing.traceWindowMilliSeconds(
      this.#currentState.timelineTraceWindow,
    );
    return {
  ...
  setTimelineVisibleWindow(
    ...
    if (!options.ignoreMiniMapBounds) {
      ...
      newWindow.min = Trace.Types.Timing.Micro(
        Math.max(this.#currentState.minimapTraceBounds.min, newWindow.min),
      );
      newWindow.max = Trace.Types.Timing.Micro(
        Math.min(this.#currentState.minimapTraceBounds.max, newWindow.max),
      );
    }
    if (newWindow.min === existingWindow.min && newWindow.max === existingWindow.max) {
    ...
    this.dispatchEvent(
      new StateChangedEvent(this.state(), 'VISIBLE_WINDOW', {
