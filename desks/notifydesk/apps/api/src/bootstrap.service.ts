...
/** 앱 부트스트랩 시 (시드 활성) 데모 테넌트·템플릿·인박스 알림 시드. 멱등. */
@Injectable()
export class BootstrapService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    // 데모 시드를 끄려면 NOTIFYDESK_SEED_DEMO=false.
    const demo = process.env.NOTIFYDESK_SEED_DEMO !== 'false'
    try {
      const result = await runSeed(this.dbs, { demo })
      if (result.seeded) {
        this.logger.log('데모 테넌트·템플릿·인박스 시드 완료 (pk_demo / sk_demo)')
      }
    } catch (err) {
