import { ArrowUpRight, Bell, FileText, Inbox, Moon, Search, Send, Sun, Trash2 } from 'lucide-react'
import {
  useCallback,
  useEffect,
  useId,
...
import { Badge, ChannelBadge, PlanBadge, StatusBadge } from '@/components/ui/badge'
...
/**
 * getComputedStyle 으로 토큰을 즉시 해석한다. CSSOM(외부 시스템) 을 읽으므로
 * useSyncExternalStore 를 쓴다 — getSnapshot 은 같은 테마에서 동일 문자열을 돌려줘
 * 안정적이고, themeKey 가 바뀌면 한 번 재렌더되어 새 값으로 수렴한다. (effect 불필요)
 */
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-bg/85 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            <Bell className="size-5 shrink-0 text-accent-strong" aria-hidden />
            <span className="truncate text-sm font-bold tracking-tight text-text">NotifyDesk</span>
        ...
        <main id="main-content" tabIndex={-1} className="min-w-0 pb-20 outline-none">
          ...
          <div className="border-b border-border py-14">
            ...
            <h1 className="mt-3 max-w-[18ch] text-[clamp(2rem,5vw,3rem)] font-semibold tracking-tight text-balance text-text">
              알림 인프라를 받치는 디자인 토큰과 컴포넌트
            </h1>
            <p className="mt-4 max-w-[65ch] text-pretty text-text-muted">
              이 페이지는 NotifyDesk 의 실제 디자인 시스템을 그대로 렌더링합니다. 색상은{' '}
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_minmax(0,32ch)]">
              <div className="space-y-4">
                {TYPE_SCALE.map((t) => (
                  ...
                  >
                    <span className={cn('text-text', t.cls)}>새 알림이 도착했습니다</span>
                    <span className="shrink-0 font-mono text-xs text-text-subtle">{t.label}</span>
                  </div>
                ))}
                <div className="flex items-center gap-4 pt-1">
                  <span className="font-mono text-sm text-text">mono · pk_live_a1b2 · sk_••••</span>
              ...
              <div className="rounded-lg border border-border bg-surface p-5">
                ...
                <p className="max-w-[68ch] text-pretty text-text-muted">
                  NotifyDesk 는 알림을 한 줄로 보내고 인박스로 받습니다. 서버는 secret 키로
                  발송하고, 브라우저 벨은 publishable 키로 자기 인박스만 읽습니다. 본문은 65–75ch
                  범위로 줄길이를 제한해 장문도 편안하게 읽히도록 합니다.
                </p>
              </div>
            </div>
          </Section>
          ...
          >
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Demo caption="도메인 칩 — ChannelBadge(3종) · StatusBadge(3종) · PlanBadge">
                <ChannelBadge channel="in_app" />
                <ChannelBadge channel="email" />
                <ChannelBadge channel="web_push" />
            ...
            {/* Stat cards */}
            <h3 className="mt-12 mb-3 text-base font-semibold text-text">지표 카드</h3>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <StatCard
                ...
                tone="accent"
              />
              <StatCard icon={Inbox} label="발송 로그" value="842" hint="인박스 건수" />
              <StatCard icon={Bell} label="템플릿" value="6" hint="알림 종류" />
              <StatCard label="요금제" value="Pro" hint="무제한" tone="success" />
            </div>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <Card>
                <CardContent className="space-y-4">
                  <Field label="템플릿 key" htmlFor="d-name" hint="발송 시 식별자로 쓰입니다.">
                    <Input id="d-name" placeholder="order.shipped" className="font-mono" />
                  </Field>
                  <Field
                    label="recipientId"
                    htmlFor="d-error"
                    error="recipientId 는 영숫자·._@- 만 가능합니다."
                  >
                    <Input
                      id="d-error"
                  ...
                  <Field label="발송 방식" htmlFor="d-type">
                    <Select id="d-type" defaultValue="template">
                      <option value="template">템플릿 렌더</option>
              ...
              <Card>
                <CardContent className="space-y-4">
                  <Field label="본문 템플릿" htmlFor="d-textarea" hint="{{변수}} 로 치환됩니다.">
                    <Textarea id="d-textarea" placeholder="{{name}}님, 주문이 발송되었어요." />
                  </Field>
                  <Field label="잠긴 필드" htmlFor="d-disabled" hint="slug 는 변경 불가">
                    <Input id="d-disabled" defaultValue="offhours" disabled className="font-mono" />
                  </Field>
                  <div className="flex items-center gap-2">
              ...
              <CardContent>
                <Tabs defaultValue="react">
                  <TabsList>
                    ...
                    <TabsTrigger value="vanilla">스크립트</TabsTrigger>
                    <TabsTrigger value="server">서버 SDK</TabsTrigger>
                  </TabsList>
                  <TabsContent value="react" className="pt-4 text-sm text-text-muted">
                    활성 탭은 하단 보더가 accent 색으로 강조됩니다.
                  </TabsContent>
                  <TabsContent value="vanilla" className="pt-4 text-sm text-text-muted">
                    IIFE 스크립트 임베드 스니펫이 여기에 들어갑니다.
                  </TabsContent>
                  <TabsContent value="server" className="pt-4 text-sm text-text-muted">
                    secret 키 발송 예제가 여기에 들어갑니다.
                  </TabsContent>
                </Tabs>
              </CardContent>
            </Card>
              ...
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      새 키가 발급되고 기존 pk_/sk_ 는 즉시 무효화됩니다.
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
                    <DialogClose asChild>
            ...
            <CodeBlock
              code={`import { NotificationBell } from '@notifydesk/widget'
...
/>`}
            ...
            <Card className="max-w-md">
              <CardHeader action={<PlanBadge plan="pro" />}>
                <CardTitle>order.shipped</CardTitle>
                <CardDescription>in-app · email · 최근 갱신 2026.06.15</CardDescription>
              </CardHeader>
              ...
              <CardFooter className="flex items-center justify-between">
                <span className="font-mono text-xs text-text-subtle">tenant: offhours</span>
                <Button variant="ghost" size="sm">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="flex flex-col gap-2">
                <EmptyState
                  ...
                  description="대시보드 컴포저나 SDK 로 알림을 보내면 여기에 표시됩니다."
                  action={
              ...
              <div className="flex flex-col gap-2">
                ...
                />
                <p className="px-0.5 text-xs text-text-subtle">
                  오류 상태는 빈 상태와 구분해 표시하고, 재시도 동작을 함께 제공합니다.
                </p>
              </div>
            </div>
          </Section>
          ...
          <p className="border-t border-border pt-8 text-xs text-text-subtle">
            이 페이지는 NotifyDesk 의 실제 토큰과 컴포넌트만 사용합니다. 새 색·새 컴포넌트를 만들지
            않습니다.
          </p>
        </main>
      </div>
