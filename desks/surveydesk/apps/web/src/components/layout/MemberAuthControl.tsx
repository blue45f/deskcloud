 ...
 *
 * 이 컨트롤은 기존 어드민 토큰 콘솔 로그인(/login → /app)과 **별개**다.
 * 로그아웃 상태면 "로그인" 버튼으로 AuthDialog 를 열고(이메일/비번 + 게스트로 시작),
 * 로그인 상태면 이메일(또는 "게스트")과 로그아웃을 보여준다. 통합 로그인 모듈의
 * 데모 겸 실제 사용처다.
 */
export function MemberAuthControl({ className }: { className?: string }) {
  ...
  if (loading) {
    ...
    return (
      <div
        className={cn('h-8 w-16 animate-pulse rounded-md bg-surface-2', className)}
        aria-hidden
      />
    )
  }
