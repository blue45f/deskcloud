...
/** 앱 부트스트랩 시 (시드 활성) 데모 테넌트·샘플 파일 시드. 멱등. */
@Injectable()
export class BootstrapService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    // 데모 시드를 끄려면 FILEDESK_SEED_DEMO=false.
    const demo = process.env.FILEDESK_SEED_DEMO !== 'false'
    try {
      const result = await runSeed(this.dbs, { demo, pepper: this.cfg.keyPepper })
      if (result.seeded) {
        this.logger.log('데모 테넌트·샘플 파일 시드 완료 (pk_demo / sk_demo)')
      }
    } catch (err) {
