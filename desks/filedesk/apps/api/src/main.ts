...
async function bootstrap(): Promise<void> {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('FileDesk API')
    .setDescription(
      '외부 온보딩형(멀티테넌트) 파일 업로드/스토리지 — 가입(pk_/sk_)·업로드(multipart/base64)·서빙(public/private)·목록·통계·삭제'
    )
    .setVersion('0.1.0')
  ...
  Logger.log(
    `FileDesk API (${cfg.mode}, storage=${cfg.storageDriver}) → http://localhost:${cfg.port}  · docs: /api/docs`,
    'Bootstrap'
  )
}
