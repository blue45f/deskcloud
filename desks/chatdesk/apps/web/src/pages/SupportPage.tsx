...
/** 상태 뱃지 톤 — Badge tone 으로 진행도를 색에 매핑. */
const statusTone: Record<InquiryStatus, 'info' | 'warning' | 'success' | 'neutral'> = {
  new: 'info',
  in_progress: 'warning',
  resolved: 'success',
  closed: 'neutral',
...
function StatusBadge({ status }: { status: InquiryStatus }) {
  return (
    <Badge tone={statusTone[status] ?? 'neutral'} size="sm">
...
function InquiryBoard({ reloadToken }: { reloadToken: number }) {
  const [state, setState] = useState<BoardState>({ phase: 'loading' })
  const [reloadKey, setReloadKey] = useState(0)

  // 목록 조회. 상태 변경은 모두 비동기 콜백에서만(set-state-in-effect 회피).
  // reloadToken(외부, 새 문의 등록 시) / reloadKey(내부, 새로고침 버튼) 둘 다 트리거.
  useEffect(() => {
        ...
        <div className="mx-auto max-w-4xl space-y-6 px-4 py-8 sm:px-6">
          <section className="rounded-lg border border-border bg-surface p-6">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-7 text-pretty text-text-muted">
              ChatDesk 에 대한 제휴·버그·의견·이용 문의를 남겨 주세요. 접수된 문의는 아래 게시판에
          ...
          <section className="space-y-4">
            <div>
              <h2 className="flex items-center gap-2 text-lg font-semibold text-text">
                <MessageSquarePlus className="size-4.5 text-text-subtle" aria-hidden />
                문의 남기기
              </h2>
            ...
            {submitted ? (
              ...
              >
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="이메일 (선택)" htmlFor={`${fieldId}-email`}>
                    <Input
                      id={`${fieldId}-email`}
                      type="email"
                      value={contactEmail}
          ...
          <InquiryBoard reloadToken={boardToken} />
        </div>
      </main>
