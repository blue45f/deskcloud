/**
 * @authdesk/widget/vanilla — 프레임워크 무관 로더.
 *
 * 비-React 사이트가 <script> 한 줄로 로그인/가입 폼을 띄울 수 있게 한다:
 *
 *   <div id="authdesk-form"></div>
 *   <script src=".../authdesk-widget.js"></script>
 *   <script>
 *     AuthDesk.init({
 *       target: '#authdesk-form',
 *       publishableKey: 'pk_…',
 *       endpoint: 'https://auth.example.com',
 *       onAuthenticated: function (r) { console.log(r.user.email) },
 *     })
 *   </script>
 *
 * 또는 명령형으로:
 *   const handle = AuthDesk.mount({ publishableKey, endpoint })
 *   handle.unmount()
 *
...
import { AuthForm, type AuthFormProps } from './react'

export interface MountOptions extends AuthFormProps {
...
function resolveContainer(target: MountOptions['target']): HTMLElement {
  ...
  host.setAttribute('data-authdesk-widget', '')
...
export function mount(options: MountOptions): WidgetHandle {
  ...
  root.render(createElement(AuthForm, widgetProps))

  return {
    container,
    unmount: () => {
      root.unmount()
      if (container.hasAttribute('data-authdesk-widget') && container.parentNode) {
 ...
 * window.AuthDesk.init({ publishableKey, endpoint }) 형태로 호출된다.
 */
export function init(options: MountOptions): WidgetHandle {
  return mount(options)
}

/** IIFE 빌드가 window.AuthDesk 로 노출하는 네임스페이스 형태. */
export const AuthDesk = { mount, init }
