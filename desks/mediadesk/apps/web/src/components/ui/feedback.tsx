...
/**
 * 데이터 조회 실패 상태 — 친근한 안내 + 다시 시도. 목록/그리드가 에러일 때
 * 빈 상태로 잘못 보이지 않도록 별도 표시한다. role="alert" 로 스크린리더에 announce.
 */
export function ErrorState({
  ...
  const detail =
    description ??
    (error instanceof Error && error.message ? error.message : '잠시 후 다시 시도해 주세요.')
  return (
    <div
      role="alert"
      className={cn(
    ...
    >
      ...
      <p className="mt-1 max-w-sm text-[0.8125rem] text-text-muted">{detail}</p>
      {onRetry ? (
        <Button
          variant="secondary"
          size="sm"
