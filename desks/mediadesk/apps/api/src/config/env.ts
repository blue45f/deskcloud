 ...
 * 실제 설정 로딩은 `loadConfig()`(config.ts) 가 담당합니다. 여기서는 검증만 ADD.
 */
...
export const envSchema = z.object({
  ...
  MEDIADESK_MODE: z.enum(['self-hosted', 'saas']).optional(),
  PORT: numericString,
  WEB_ORIGIN: optionalNonEmpty,
  PUBLIC_BASE_URL: optionalNonEmpty,
  DATABASE_URL: optionalNonEmpty,
  PGLITE_DIR: optionalNonEmpty,
  STORAGE_DRIVER: z.enum(['local', 's3']).optional(),
  STORAGE_LOCAL_DIR: optionalNonEmpty,
  DERIVATIVE_CACHE_DIR: optionalNonEmpty,
  MAX_UPLOAD_BYTES: numericString,
  FREE_PLAN_MAX_BYTES: numericString,
  FREE_PLAN_MAX_COUNT: numericString,
...
export function validateEnv(
  ...
  // neon.tech URL 가드 — 이 프로젝트는 로컬 PGlite/qa-postgres 만 사용한다(라이브 DB 금지).
  if (env.DATABASE_URL && /neon\.tech/i.test(env.DATABASE_URL)) {
    warn('[env] 경고: DATABASE_URL 이 neon.tech 를 가리킵니다. MediaDesk 는 로컬 DB만 사용합니다.')
  }
