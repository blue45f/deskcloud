...
- **OUT** — `@heejun/web-config-preset`(DeskCloud보다 넓은 소비자), `aidigestdesk`(별도 플랫폼 — 여기에 vendoring 하지 않음, 스탠드얼론 blue45f/aidigestdesk). OUT 레포/서비스는 `@desk/*` 또는 `@heejun/deskcloud` 를 발행 caret로 소비합니다.

  `aidigestdesk`는 GH Pages / Toss in-app 으로 운영되는 별도 플랫폼(SaaS 빌링 없음)이라
  이 모노레포에 vendoring 하지 않습니다. 트리에서 제거했고 캐노니컬은 스탠드얼론
  `blue45f/aidigestdesk` 단독으로 유지합니다.
...
**상태**: 물리 통합(16 Desk + platform + SDK + deploy stack) 완료. `aidigestdesk`는 별도 플랫폼이라
여기에 vendoring 하지 않습니다(트리에서 제거 — 스탠드얼론 blue45f/aidigestdesk). 2026-06-22 기준 루트 검증, 로컬 compose
