...
연결되어 있으며, `aidigestdesk`는 운영 상 일반 서비스 성격으로 source/workspace/runtime
통합 제외 대상이다.
...
| `aidigestdesk`         | 분리 운영, sibling 원본 보관  | 모노레포 source/workspace/runtime 제외     |
...
Desk가 아니라 분리 운영 서비스라서 404를 반환하는지도 확인한다.
