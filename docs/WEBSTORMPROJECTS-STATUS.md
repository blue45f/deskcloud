...
DeskCloud 통합본은 다음 17개 Desk를 `desks/*` 워크스페이스로 포함한다.
...
| AIDigestDesk   | `desks/aidigestdesk`    | `/desks/aidigestdesk`    |
...
DeskCloud 내부 workspace로 남아 있다. `spa-seo-gateway`, `remote-devtools`,
`aidigestdesk`의 예전 최상위 원본 폴더는 아직 sibling 저장소로 존재하지만 실제 통합
운영 기준 소스는 위 workspace 경로다.
...
| `aidigestdesk`         | 통합됨, sibling 원본 보관     | 통합 경로 `desks/aidigestdesk`             |
